...
Candidate **`8a52bfd7b38bf8ed41210f3b1f49fbca6c5f7954`**, exported on 2026-09-07, passed all **927 regression suites**, the **four standard station package probes**, and additional physical **Ember** and **Cinder cargo** package checks. Source and import cache stayed unchanged; the matrix retained three explicitly accepted `RENDER-001` shutdown-warning blocks. Package checks reported zero diagnostics. The release record binds the exact source, executable, full matrix, standard probes and inventory; all automated runs used Linux Godot.

The player now retains Ember floor contact with a 2 mm collision recovery margin while station ramp clearance stays within its existing limit. Step assistance accepts the small movement attempted immediately after hitting a step; its landing-distance, height and capsule-clearance limits remain intact. The exported Cinder route is walkable in both directions without jumping.
