...
- **Ember Moon:** the Destination Board exposes the bounded Ember journey, physical cruise/final approach and streamed landing. Disembarking now starts the mandatory survey; completing both checkpoints saves one reward before admitting return. The reproduced pad floor-contact failure is fixed in source and passes focused physical checks; full regression and package qualification are pending. The catalog does not make every listed world visitable. Full planetary breadth, uninterrupted human expedition review and representative hardware performance remain open. See [Phase 10](ROADMAP.md#4-visitable-planets-with-complete-atmosphere-to-surface-loops).
...
Candidate **`3c59ae57f86e3111241e73034507f02f01d46f23`**, exported on 2026-09-07, passed **925 of 927 regression suites** and all **four standard station package probes**. The added physical Ember package probe failed near the pad after successfully walking both checkpoints, saving the reward once and admitting return. A separate outdated boarding fixture has since been corrected and passes its focused check. This older candidate is not a qualified expedition playtest build. All package probes used Linux Godot.

The source repair increases the player's collision recovery margin from 1 mm to 1 cm, retaining floor contact at the exact reproduced position. The extended movement suite passes all 20 checks, five repeated real-Host pad crossings pass, and boarding, cabin and foot-placement checks pass. Host support guards remain intact. The replacement full matrix and package checks must pass before this repair qualifies the expedition.
