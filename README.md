...
- **Aurora:** choose Aurora on the Destination Board while piloting a supported craft. The outbound and homeward legs now fly through physical cruise, committed origin shifts and a final approach to a real berth. Walk the coastal trail to an optional two-stop survey, reboard, and choose the return action to dock at Mudds. The Halyard also supports sleeping aboard. Native performance and an uninterrupted human review of this journey remain open.
...
The latest playable checkpoint is recorded in the [active roadmap](ROADMAP.md). It includes physical travel to Ember and Aurora, the Halyard liveaboard cabin, and an Aurora trail branch that leads to the second survey observation. The current source also names an active planetary final approach in the HUD instead of displaying `ACCELERATING`. Focused automated checks cover these changes; a full matrix on the latest source and uninterrupted human review remain open.

The next acceptance milestone is a normal-controls packaged playthrough covering boarding, live combat, planetary arrival, landing, disembarkation and return. Native rendered reliability, representative hardware performance, controller comfort and audible mix still need direct review. Automated rendered checks use an isolated display or test guest.
...
Board the Halyard at Fleet Dock, then open **Esc → Destination Board → Aurora**. The journey flies through cruise and assisted final approach before landing. Press **E** to leave the ship after landing; walk up its port airstair to enter the cabin. If the hatch is closed, look at it near the top of the stair and press **E** to open it. Walk aft along the central aisle, look at either bunk and press **E** to sleep. Press the displayed wake control to stand up again.
