...
`assets/ships/markings/*.svg` contains seventeen transparent 512 × 256 hull-paint
assets: thirteen craft registrations (`torrent`, `arrow`, `zenith`, `jovian`,
`halyard`, `bulwark`, `cinder-interceptor`, `cinder-bomber`, `cinder-cargo`, `range`,
`skirmisher`, `picket`, `courier`) and four common functional markings (`intake`,
`service`, `rescue`, `exhaust`). Each filename has the `.svg` extension.
