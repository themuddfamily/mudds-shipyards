...
`assets/ships/markings/*.svg` contains twenty-one 512 × 256 paint assets:
...
`service`, `rescue`, `exhaust`) and four freight-container plates
(`freight-ardent`, `freight-meridian`, `freight-cinder`, `freight-yard`). Each
filename has the `.svg` extension.

The seventeen hull assets are transparent. The four freight plates are the one
