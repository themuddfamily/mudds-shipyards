...
## Original fleet registration and safety markings

`assets/ships/markings/*.svg` contains twenty-two 512 × 256 paint assets:
thirteen craft registrations (`torrent`, `arrow`, `zenith`, `jovian`, `halyard`,
`bulwark`, `cinder-interceptor`, `cinder-bomber`, `cinder-cargo`, `range`,
`skirmisher`, `picket`, `courier`), four common functional markings (`intake`,
`service`, `rescue`, `exhaust`), four freight-container plates
(`freight-ardent`, `freight-meridian`, `freight-cinder`, `freight-yard`) and one
stores-tote plate (`freight-tote`). Each filename has the `.svg` extension.

The seventeen hull assets are transparent. The five freight plates are the one
deliberate exception and carry an opaque pale `#d9ddd6` ground, because they are
not ink printed onto a hull: they are the painted steel data plate bolted to a
container, drawn as a surface of the container's own mesh by
`scripts/world/freight_container_kit.gd` rather than projected as a decal. Three
carry an operator identity and unit code for the Jovian freight berth's stock;
`freight-yard` is the yard's own plate, used at Dock 04 where all seven
containers are drawn through one batch and the operator is carried by the paint.
`freight-tote` is the shipyard stores mark — owner, stock code, "RETURNABLE
TOTE" and an amber this-way-up arrow — carried on both long sides of every small
crate and tote built by `scripts/world/freight_crate_kit.gd` (FREIGHT-CRATE-001):
one plate for all three tote finishes, because a returnable tote is the yard's
rather than any operator's.

These are project-original geometric lettering and vector artwork generated by
`python3 tools/generate_ship_markings.py`. All lettering is explicit SVG paths;
there are no external fonts, linked images, third-party source assets, or image
generation inputs. They are modern shipyard styling, not historical markings.
The generator deterministically replaces the same assets using Python's standard
library. SVGs use simple paths, circles, transforms, and flat fills/strokes for
Godot's SVG importer. The main background is transparent; graphite `#283139`
provides registration contrast on pale hulls and amber `#be893a` marks hazards.

Use the 2:1 artwork aspect ratio. Suggested physical footprint: registration
2.0–3.2 m wide × 1.0–1.6 m high on fighters, 3–5 m × 1.5–2.5 m on cargo hulls;
safety markings about 0.5–1.2 m × 0.25–0.6 m. These are starting points for
...
512px seamless PBR channels generated by `tools/generate_ship_finish_maps.py`.
The analytic height field supplies fine paint grain and restrained roughness
variation; hull panels and joints are modelled separately. No third-party or
