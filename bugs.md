...
## EMBER-RETURN-001 — Planetary test rejects the final walk back to boarding — **REPRODUCED**

- Severity: **P1 in the automated planetary journey**, reproduced on clean checkpoint source `5a15ee4c0b29dccf16064abf28ac14c0d6b05d91`. Owner: stabilization workstream. Observed 2026-09-08 during `tests/ember_surface_loop_production_binding_test.gd`. Native/human reproduction remains `NOT_RUN`.
...
- Comparison on 2026-09-08: unchanged `1ecec4f` gameplay with test-only diagnostics **did not reproduce the boarding failure**. The real survey, single persisted reward, fixed-pad return, exact BoardingArea return, retained coordinator cadence, real reboard and takeoff all passed. The original 1200 s timeout ended the run with exit 124 during accepted ascent at 14,751.1 / 20,000 m. There were 58 passing assertions and no errors or failures; full handback and suite completion remain unqualified. Evidence: `/tmp/ember-return-baseline.log`, private worktree `/tmp/mudds-ember-return`, private profile `/tmp/ember-return-baseline-profile`.
- The existing fixture now logs the late rejection reason, position and floor contact, cleans up after a failed boarding return, and does not await another signal from a FAILED binding. A private forced-FAILED probe of the actual helper exited 0 with `EMBER_TERMINAL_WAIT_PROBE_OK failed_state_preserved=true signals=0`; log `/tmp/ember-terminal-wait-probe.log`. The original failing run is not retroactively explained by this repair.
- Clean-source reproduction on 2026-09-08: the complete unchanged fixture on `5a15ee4c0` exited **1**, with **59 passing assertions and one failure**, before its original 1200 s timeout. Real terrain traversal, both mandatory checkpoints, a single persisted reward, replay rejection and crossing back onto the pad passed. The first boarding-return leg then rejected with `surface_support_lost`: `move_back`, step 30, landing-local position `(20.15918, -0.172855, 0.851868)`, `on_floor=true`. Terminal cleanup worked and the remaining stale-frame/handback red checks ran. Evidence: `/tmp/mudds-fulljourney-5a15ee4-private/fulljourney.log`; owned session `88056` terminal. This reproduces the return failure without the withheld optimization, but does not yet identify the failing support predicate or explain the native graphics crash.
- Next action: reproduce the exact pad return with real physical contact and inspect the support ray, collider, frame and capsule witnesses before changing a bound. Preserve airborne, foreign-collider, exact outbound-pad and rebase rejection checks. The idle-handoff optimization remains withheld.
