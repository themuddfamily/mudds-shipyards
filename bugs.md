...
- Severity: **candidate P1**, not reproduced in the unchanged-source comparison below. Owner: stabilization workstream. Observed 2026-09-08 during `tests/ember_surface_loop_production_binding_test.gd`.
...
- Next action: retain the candidate and withheld optimization; capture the exact rejection if it recurs. Short matched profiling separated the test overhead and confirmed repeated production diagnostic construction; focused caller/activity observations reduce that work while preserving validation. This optimization does not explain or close the earlier boarding failure. Do not change movement, support, rebase or handoff authority without a reproduced cause.
