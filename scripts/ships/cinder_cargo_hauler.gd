...
## Cargo glazing seats on the retained pressure-wall and sill tops. The broad
...
func _install_cargo_canopy() -> void:
	var rings := _cargo_canopy_rings()
	var glass := _canopy_pivot.get_node("CanopyGlass") as MeshInstance3D
	glass.mesh = _cargo_canopy_glass(rings, glass.mesh.surface_get_material(0))
	...
	for side in [-1, 1]:
		...
		_cargo_canopy_member(prefix + "CanopyLowerRail", lower, 0.045)
		_cargo_canopy_member(prefix + "CanopyTopRail", shoulder, 0.020)
		...
		_cargo_canopy_member(prefix + "CanopyLatchHook", PackedVector3Array([
		...
		_cargo_canopy_member(prefix + "CanopyLowerPressureSeal", seal, 0.019)
		_cargo_canopy_member(prefix + "CanopyLaminateEdge", laminate, 0.008)
		for end in [0, rings.size() - 1]:
			var bow := PackedVector3Array()
			for step in 17:
				bow.append(rings[end][step if side < 0 else 32 - step])
			_cargo_canopy_member(prefix + ("CanopyNoseFrame" if end == 0 else "CanopyRearUpright"), bow, 0.023,
	...
	_cargo_canopy_member("CanopyNosePressureSeal", front, 0.045)
	_cargo_canopy_member("CanopyRearFrame", rear, 0.032)
	for index in rear.size():
		rear[index].y -= 0.019
	_cargo_canopy_member("CanopyRearPressureSeal", rear, 0.019)
...
func _cargo_canopy_member(member_name: String, path: PackedVector3Array, radius: float, tangent := Vector3.ZERO) -> void:
	var member := _canopy_pivot.get_node(member_name) as MeshInstance3D
	var finish := member.mesh.surface_get_material(0)
	member.transform = Transform3D.IDENTITY
	member.mesh = _canopy_frame_mesh(path, radius, tangent)
	member.mesh.surface_set_material(0, finish)


func _cargo_canopy_rings() -> Array[PackedVector3Array]:
...
func _cargo_canopy_glass(rings: Array[PackedVector3Array], finish: Material) -> ArrayMesh:
