...
static var _shared_service_cassettes: Dictionary = {}
...
## Closed rounded cassette and five formed vanes replace disconnected stock
## bars. Each size reuses material-free geometry; coatings remain caller-owned.
func _service_bay(parent: Node3D, tag: String, at: Vector3, width: float, length: float, frame: Material, dark: Material, metal: Material) -> void:
	...
	if not _shared_service_cassettes.has(size):
		_shared_service_cassettes[size] = [_service_cassette_frame(size), _service_cassette_vanes(size)]
	_box(parent, tag + "Recess", at + Vector3(0, 0.025, 0), Vector3(width, 0.035, length), dark)
	for part in 2:
		var node := MeshInstance3D.new()
		node.name = tag + ("Frame" if part == 0 else "Vanes")
		node.position = at
		node.mesh = _shared_service_cassettes[size][part]
		node.material_override = frame if part == 0 else metal
