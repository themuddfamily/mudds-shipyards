...
# Formed pilot grips and boot share the existing rubber seal finish; the
# retired grip upholstery no longer contributes an active material family.
const TORRENT_RENDER_UNIQUE_MATERIAL_RESOURCE_COUNT := 40
...
func _install_torrent_hero_presentation() -> void:
	...
	if imported_cockpit == null or imported_canopy == null:
		...
		presentation.queue_free()
		return
	# Torrent's globally dedicated layer 18 glazing is omitted only from the
	# seated view. On-foot, chase, and ordinary cameras retain the complete shell.
