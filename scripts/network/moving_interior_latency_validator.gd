...
const POLICY_VERSION: StringName = &"moving_interior_latency_validator_v1"
...
func validate_trace(
	...
	var previous_tick := -1
	var previous_local_origin := Vector3.ZERO
	var previous_frame_origin := Vector3.ZERO
	var previous_world_origin := Vector3.ZERO
	var has_previous_pose := false
	...
	for item_index in trace.size():
		...
		if not bool(authority_result.get("accepted", false)):
			...
			continue
		accepted_count += 1
		var prior_tick := previous_tick
		if bool(authority_result.get("gap_detected", false)) and previous_tick >= 0:
			var gap_ticks := tick - previous_tick
		...
		if has_previous_pose:
			...
			var local_step := local_origin.distance_to(previous_local_origin)
			...
			var frame_step := frame_transform.origin.distance_to(previous_frame_origin)
			...
			var world_step := world_origin.distance_to(previous_world_origin)
		...
		previous_local_origin = local_origin
		previous_frame_origin = frame_transform.origin
		previous_world_origin = world_origin
