...
##
## The rail-fastener pass then replaces eight childless decorative leaves with
	...
	for candidate in find_children("*", "", true, false):
		...
		if candidate is MultiMeshInstance3D:
			...
			_built_mesh_contracts[relative_path] = {
				...
				"visibility_parent": mesh_instance.visibility_parent,
				"custom_aabb": mesh_instance.custom_aabb,
...
func _built_mesh_contracts_are_live() -> bool:
	...
	for relative_path_value in _built_mesh_contracts:
		...
		if (
			...
			or mesh_instance.visibility_parent != (contract.get("visibility_parent", NodePath()) as NodePath)
			or not mesh_instance.custom_aabb.is_equal_approx(contract.get("custom_aabb", AABB()) as AABB)
