...
func _test_integration_audit(world: ShipyardWorld, module: FleetDockComb) -> void:
	...
	_check(bool(report.valid), "production comb placement and connector pass the delegated integration audit")
	...
	_check(bool(report.empty_deferred_docks) and bool(report.placement_authored), "integration distinguishes empty source-bounded docks from authored placement")
...
func _test_landing_and_berth_authority_unchanged(world: ShipyardWorld, module: FleetDockComb) -> void:
	...
	_check(exact_berth_roster, "production berth registry remains the exact Central/Arrow/Jovian three")
	...
	_check(all_live and all_transforms_exact, "all three authoritative berth identities and transforms remain exact")
	...
	_check(module.get_deferred_dock_roster().size() == 3, "three non-authoritative empty dock landmarks remain independently visible")

	var module_aabb := module.get_bounds_contract().world_aabb as AABB
	var capture_overlap := false
	for berth_id in EXPECTED_BERTH_IDS:
		...
		capture_overlap = capture_overlap or module_aabb.intersects(capture_aabb)
	_check(not capture_overlap, "comb footprint stays outside every production landing-assist capture volume")
