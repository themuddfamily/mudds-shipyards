...
func _test_integration_audit(world: ShipyardWorld, module: FleetDockComb) -> void:
	...
	_check(
		int(report.external_assignment_count) == 1
		and int(report.deferred_empty_dock_count) == 2
		and not bool(report.historical_class_to_berth_mapping)
		and bool(report.placement_authored),
		"integration separates one modern external assignment from two deferred docks"
	)
...
func _test_landing_and_berth_authority_unchanged(world: ShipyardWorld, module: FleetDockComb) -> void:
	...
	_check(exact_berth_roster, "production berth registry contains the exact four assigned physical berths")
	...
	_check(all_live and all_transforms_exact, "all four authoritative berth identities and transforms remain exact")
	...
	_check(
		module.get_assigned_dock_roster().size() == 1
		and module.get_deferred_dock_roster().size() == 2,
		"one assigned and two deferred dock landmarks remain independently visible and non-authoritative"
	)
	var assigned := module.get_assigned_dock_roster()[0] as Dictionary
