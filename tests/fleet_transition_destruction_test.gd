...
func _test_loss_during_disembarking_and_reuse() -> void:
	...
	_check(craft.get_instance_id() == original_instance_id and game.get_flyable_ships().size() == 9, "transition recovery reuses the same craft and preserves the nine-ship fleet")
	_check(
		berth != null
