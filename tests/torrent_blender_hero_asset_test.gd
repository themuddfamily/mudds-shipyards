...
func _run() -> void:
	...
	_check(int(audit.get("lod0_triangle_count", 0)) == 75540 and int(audit.get("lod0_mesh_count", 999)) == 17, "LOD0 preserves its measured close topology inside the 18-node draw budget")
	_check(int(audit.get("lod1_triangle_count", 0)) == 10286 and int(audit.get("lod1_mesh_count", 999)) == 5, "LOD1 preserves its complete silhouette at the exact five-node budget")
	...
	_check(int(batching.get("source_mesh_count_total", 0)) == 319 and _mesh_counts_match(source_counts, {"CanopyPivot": 17, "CockpitArt": 39, "LOD0": 242, "LOD1": 21, "SemanticAnchors": 0}), "manifest pins the complete 319-mesh editable semantic roster")
	...
	_check(runtime_triangles == 100108, "runtime GLB carries the measured 100,108-triangle production topology")
	...
	var far_thermal := presentation.get_lod1_root().get_node_or_null(
		^"LOD1StaticBatch_ThermalCeramic"
	) as MeshInstance3D
	var aft_cassette_vertices := 0
	if far_thermal != null:
		for surface_index in far_thermal.mesh.get_surface_count():
			var arrays := far_thermal.mesh.surface_get_arrays(surface_index)
			for vertex in arrays[Mesh.ARRAY_VERTEX] as PackedVector3Array:
				var local_vertex := root_art.to_local(far_thermal.to_global(vertex))
				if is_equal_approx(local_vertex.z, 3.47) \
					and absf(local_vertex.x) <= 1.32 \
					and local_vertex.y >= 0.75 and local_vertex.y <= 1.70:
					aft_cassette_vertices += 1
	_check(aft_cassette_vertices >= 8
		and far_thermal.get_meta("torrent_material_role", &"") == &"ThermalCeramic",
		"far LOD retains the aft cassette's dark octagonal face in the existing thermal batch")
	...
	_check(int(art_quality.get("close_triangle_count", 0)) == 89822 and int(art_quality.get("far_triangle_count", 0)) == 13116 and int(art_quality.get("total_triangle_count", 0)) == 100108, "manifest separates close cabin/canopy density from the complete far silhouette")
	...
	_check(bool(uv_contract.get("all_source_meshes_mapped", false)) and int(uv_contract.get("mesh_count", 0)) == 319 and int(uv_contract.get("degenerate_polygon_count", -1)) == 0, "every editable mesh owns finite non-degenerate UV0 before batching")
	_check(
	...
	_check(
		...
		and int(textured_orientation.get("triangle_count", 0)) == 25872
		...
		"actual GLB keeps all 25,872 textured hull triangles in one UV orientation apart from bounded float32 bevel slivers"
	)
	var live_uv_orientation := _runtime_textured_uv_orientation(root_art)
	_check(
		int(live_uv_orientation.get("triangle_count", 0)) == 25872
	...
	_check(
		...
		and float(plate_winding.get("minimum_signed_volume_m3", 0.0)) > 0.32
...
func _hull_runtime_materials_use_registered_maps(asset_root: Node3D) -> bool:
	...
	for candidate in asset_root.find_children("*", "MeshInstance3D", true, false):
		...
		if (
			...
			or material.albedo_texture.resource_path != ShipSurfaceDetail.PAINT_ALBEDO_PATH
...
func _primary_display_sightlines_are_clear(hero: HeroShip, side_displays: bool = false) -> bool:
	...
	if screen == null or cockpit == null or camera == null:
		return false
	...
	for target in targets:
		for node in cockpit.find_children("*", "MeshInstance3D", true, false):
			...
			if mesh_instance == screen or mesh_instance.mesh == null:
				continue
			var eye_local := mesh_instance.to_local(camera.global_position)
			var target_local := mesh_instance.to_local(target)
