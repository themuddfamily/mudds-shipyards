...
const STAGING_INTERVENTIONS: Array[Dictionary] = [
	...
	},
	{
		...
		"action": "Applies accepted ship propulsion demand, calls public camera methods, and invokes private GameFlow landing, engagement, projectile, or exit handlers where the harness names those transitions.",
...
func _run() -> void:
	...
	# Wake and launch the freighter through accepted hover and throttle demand.
	...
	_check(
		await _wake_engine_with_hover_demand(jovian),
		"Jovian accepts hover demand and wakes propulsion in the same physics tick"
	)
	Input.action_press("move_forward")
	if not await _wait_for_phase(game, GameFlow.Phase.FREE_FLIGHT, 5.0):
		Input.action_release("move_forward")
		_fail("Jovian automatic propulsion did not enter the sandbox free-flight phase")
		await _dispose(game)
		_finish()
		return
	...
	_check(
		await _idle_engine_offline_exact(jovian),
		"landed Jovian idles OFFLINE at the automatic physics-time boundary"
	)
	...
	# it records automatic propulsion accepting real hover demand and both animated
	# engine exhausts.
	_check(
		await _wake_engine_with_hover_demand(ship),
		"engine-start capture wakes propulsion from accepted hover demand"
	)
	# Retain the established exhaust-animation settle on the physics clock before
	# recording the unchanged rear composition.
	await _advance_simulated(0.82)
	_check(
		str((ship.call("get_telemetry") as Dictionary).get("engine_state", "")).to_upper() == "ONLINE",
		"engine-start capture retains live automatic propulsion after its settle"
	)
	_frame_camera(
	...
	if not await _wait_for_phase(game, GameFlow.Phase.LAUNCH, 0.5):
		_fail("automatic propulsion departure did not advance to launch")
		await _dispose(game)
		_finish()
		return
	...
	_check(
		await _idle_engine_offline_exact(ship),
		"landed Torrent idles OFFLINE at the automatic physics-time boundary"
	)
	game.call("_try_exit_ship")
	if not await _wait_for_phase(game, GameFlow.Phase.DISEMBARKING, 1.0):
...
func _wake_engine_with_hover_demand(ship: Node) -> bool:
	Input.action_press(&"hover")
	await physics_frame
	await process_frame
	var command := ship.call("get_last_ship_command") as ShipCommand
	var accepted := (
		StringName((ship.call("get_telemetry") as Dictionary).get("engine_state", &""))
...
func _idle_engine_offline_exact(ship: Node) -> bool:
	# Reset the production accumulator with one accepted demand tick, then advance
	# only the physics clock that owns the published 1.5-second idle boundary.
	if not await _wake_engine_with_hover_demand(ship):
		return false
	...
	for _tick in idle_ticks - 1:
		await physics_frame
	await process_frame
	if StringName((ship.call("get_telemetry") as Dictionary).get("engine_state", &"")) != HeroShip.ENGINE_ONLINE:
		return false
	# Floating-point accumulation can place the nominal threshold infinitesimally
	# below 1.5; permit one bounded crossing tick, matching the production tests.
	await physics_frame
	if StringName((ship.call("get_telemetry") as Dictionary).get("engine_state", &"")) != HeroShip.ENGINE_OFFLINE:
		await physics_frame
	await process_frame
	return StringName((ship.call("get_telemetry") as Dictionary).get("engine_state", &"")) == HeroShip.ENGINE_OFFLINE
...
func _release_actions() -> void:
	for action in ["interact", "move_forward", "fire", "sprint_boost", "brake", "hover"]:
