...
func _run() -> void:
	...
	_check(
		...
		"actor removal still follows the existing terminal loss path"
	)
...
func _assert_route_intent(
	...
	var expected_target := ROUTE.get_checkpoint_position(expected_target_index)
	var expected_direction := (expected_target - entity_position).normalized()
	...
	_check(
		...
		and (feedback.get("route_target_position") as Vector3).is_equal_approx(expected_target)
		and (feedback.get("route_direction_world") as Vector3).is_equal_approx(
			expected_direction
		)
		and visible_arrow_direction.is_equal_approx(expected_direction),
