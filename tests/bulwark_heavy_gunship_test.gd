...
func _test_fitted_canopy(craft: HeroShip) -> void:
	var hinge := craft.get_node("BulwarkHeavyGunshipVisual/CanopyHinge") as Node3D
	...
	var cockpit := craft.get_node("BulwarkHeavyGunshipVisual/CockpitInterior")
	...
	# Every interior point above the rim must have actual glass on BOTH sides
	# of a transverse ray. Fixed pressure walls/sills form the enclosure's
	# boundary and are tested as rim support above; their joining bevels may
	...
	for side in [-1, 1]:
		...
		var keeper := hinge.get_node(prefix + "CanopyLatchHook") as MeshInstance3D
	...
	_check(hinge.rotation.x > 1.0 and glass.mesh == glass_stock and glass.is_visible_in_tree()
		and keeper.is_visible_in_tree(), "the common hinge opens the complete Bulwark lid and attached latch keepers")
