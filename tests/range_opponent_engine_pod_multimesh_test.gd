...
func _run() -> void:
	...
	var batch := visual.get_node_or_null(^"RangeEnginePodBatch") as MultiMeshInstance3D \
	...
	var expected_transforms: Array[Transform3D] = [
		Transform3D(expected_basis, Vector3(-2.67, 0.05, 3.1)),
		Transform3D(expected_basis, Vector3(2.67, 0.05, 3.1)),
	]
	...
	_check(
		...
		"the two engine-pod shells use one bounded 3D MultiMesh submission"
	)
	_check(
		transforms == expected_transforms
			and multi.custom_aabb.is_equal_approx(expected_bounds)
			and names == PackedStringArray(["PortEnginePod", "StarboardEnginePod"]),
	...
	for child in visual.get_children():
		...
		if mesh_instance != null and (
			mesh_instance.position.is_equal_approx(Vector3(-2.67, 0.05, 3.1))
				or mesh_instance.position.is_equal_approx(Vector3(2.67, 0.05, 3.1))
		):
			ordinary_pods += 1
	...
	for child in visual.get_children():
		var mesh_instance := child as MeshInstance3D
		if mesh_instance != null and (
			mesh_instance.position.is_equal_approx(Vector3(-2.67, 0.05, 4.42))
				or mesh_instance.position.is_equal_approx(Vector3(2.67, 0.05, 4.42))
		):
			plume_count += 1
		var light := child as OmniLight3D
		if light != null and (
			light.position.is_equal_approx(Vector3(-2.67, 0.05, 4.08))
				or light.position.is_equal_approx(Vector3(2.67, 0.05, 4.08))
		):
			engine_light_count += 1
	_check(
		colliders.size() == 7
			and plume_count == 2
			and engine_light_count == 2
			and opponent.get_node_or_null(^"PortMuzzle") is Marker3D
			and opponent.get_node_or_null(^"StarboardMuzzle") is Marker3D,
		"collision, animated engine peers and authoritative weapon muzzles remain independent"
	)
	...
	if _failures.is_empty():
		print("RANGE_OPPONENT_ENGINE_POD_MULTIMESH_TEST_OK: %d checks" % _assertions)
		quit(0)
	else:
