...
func _test_persistence(game: GameFlow, filesystem: Fixture.FakeFilesystem, store: UserDataStore) -> void:
	...
	# Multiple edits share one confirmed baseline, including different fields.
	var original_mode := settings.window_mode
	game.call("_on_setting_change_requested", &"display_resolution", "1280x720")
	var first_generation := int((game.get("_pending_display_confirmation") as Dictionary).generation)
	game.call("_on_setting_change_requested", &"window_mode", RuntimeSettings.WindowMode.BORDERLESS)
	game.call("_on_setting_change_requested", &"display_resolution", "2560x1440")
	generation = int((game.get("_pending_display_confirmation") as Dictionary).generation)
	game.call("_on_display_settings_keep_requested", first_generation)
	_check(not (game.get("_pending_display_confirmation") as Dictionary).is_empty(), "stale Keep cannot accept a subsequent preview")
	game.call("_on_setting_change_requested", &"master_volume", 0.7)
	adapter.load()
	_check(fresh.display_resolution == baseline and fresh.window_mode == original_mode, "save during repeated previews retains the original confirmed display")
	game.call("_on_display_settings_revert_requested", generation)
	_check(settings.display_resolution == baseline and settings.window_mode == original_mode, "repeated previews revert resolution and mode to the original baseline")
	_check(is_equal_approx(settings.master_volume, 0.7), "chained revert preserves unrelated accepted settings")
	game.call("_on_setting_change_requested", &"display_resolution", "1280x720")
	game.call("_on_setting_change_requested", &"display_resolution", "2560x1440")
	generation = int((game.get("_pending_display_confirmation") as Dictionary).generation)
	game.call("_on_display_settings_keep_requested", generation)
	adapter.load()
	_check(fresh.display_resolution == "2560x1440", "Keep after multiple previews saves the latest candidate")
	game.call("_on_setting_change_requested", &"display_resolution", "1280x720")
	generation = int((game.get("_pending_display_confirmation") as Dictionary).generation)
	game.call("_on_display_settings_revert_requested", generation)
	_check(settings.display_resolution == "2560x1440", "a later transaction starts from the newly confirmed display")
	game.call("_on_settings_reset_requested")
