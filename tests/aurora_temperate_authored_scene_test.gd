...
func run() -> void:
	...
	check(terrain_snapshot.get("profile_id") == &"aurora_temperate_terrain" and int(terrain_snapshot.get("ring_count", 0)) == 5 and int(terrain_snapshot.get("render_vertex_count", 0)) == 21125 and int(terrain_snapshot.get("render_triangle_count", 0)) == 40096 and int(terrain_snapshot.get("collision_triangle_count", 0)) == 32768 and int(terrain_snapshot.get("collision_ring_count", 0)) == 1 and is_equal_approx(float(terrain_snapshot.get("collision_maximum_distance_m", 0.0)), 1500.0) and bool((scene.audit().get("terrain_clipmap", {}) as Dictionary).get("valid", false)), "Aurora instantiates its five-ring spherical terrain and 1.5 km relief collision")
