...
func _run() -> void:
	...
	var definitions := [
		...
		{"id": &"board", "checkpoint_id": &"board_seen", "controller_prompt": "A: board spacecraft", "keyboard_prompt": "E: board spacecraft", "accessible_prompt": "Board the spacecraft when the boarding prompt is visible."},
	]
	...
	_check(first.accepted and tutorial.current_step().id == &"board" and tutorial.current_prompt() == "E: board spacecraft", "accepted checkpoint advances exactly one step")
	...
	if _failures.is_empty():
		print("TUTORIAL_PROGRESSION_CONTRACT_TEST_OK: 15 assertions")
	else:
