...
const RESIDENT_FINGERPRINT := "13bb83b5356f04213655db5d824fb62e73f9d697065e16f7427dd989d601339f"
const CINDER_LOADED_FINGERPRINT := "7943fd103015f62e90456c367a75aba6b677d43e09f1113663a254cf6477df38"
...
func _run() -> void:
	...
	_check(
		int(resident.get("total_triangles", -1)) == 1683905
			...
			and int(resident.get("unique_meshes", -1)) == 2627,
		"resident geometry freezes 1,683,905 triangles / 5,691 meshes / 5,698 surfaces / 2,627 unique meshes"
	)
	_check(
	...
	_check(
		int(loaded.get("total_triangles", -1)) == 1801362
			...
			and int(loaded.get("unique_meshes", -1)) == 2757,
		"loaded geometry freezes 1,801,362 triangles / 5,857 meshes / 5,864 surfaces / 2,757 unique meshes"
	)
	_check(
