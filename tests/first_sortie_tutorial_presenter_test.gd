...
func _run() -> void:
	...
	var launch := presenter.present_snapshot({"step_id": &"launch", "generation": 4, "revision": 2})
	_check(launch.prompt.contains("CLEAR BAY // APPLY THRUST") and launch.revision == 2, "launch prompt names its next action")
	...
	var accessible := presenter.present_snapshot({"step_id": &"return_land", "generation": 4, "revision": 4, "accessible": true})
	...
	var stale_revision := presenter.present_snapshot({"step_id": &"board", "generation": 4, "revision": 3})
	...
	_check(
		...
		and dismiss.completion_intent.revision == 4
	...
	var actor_loss := presenter.present_snapshot({"step_id": &"exit", "generation": 4, "revision": 5, "actor_attached": false})
