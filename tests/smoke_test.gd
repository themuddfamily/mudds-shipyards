...
func _run() -> void:
	...
	_check(fleet.size() == 3, "production scene registers exactly three physical flyable craft")
	_check(fleet.has(ship) and fleet.has(arrow_ship) and fleet.has(jovian_ship), "fleet registry contains Torrent, Arrow, and Jovian")
	...
	if world != null:
		...
		_check(world.call("get_berth_ids").size() == 3, "world provides exactly three production landing berths")
		...
		_check(berth_feedback_nodes.size() == 3, "each authoritative production berth owns one visual feedback component")
		var feedback_audit: Dictionary = world.call("get_ship_berth_feedback_audit_report")
		_check(
			bool(feedback_audit.get("valid", false))
			and int(feedback_audit.get("component_count", 0)) == 3,
