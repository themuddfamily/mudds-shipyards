...
func _run() -> void:
	...
	_check((hud.get("_server_browser_title") as Label).text == "SEARCHING…", "loading state is explicit")
	...
	_check(rows.get_child_count() == 2 and (rows.get_child(1) as Button).disabled, "full sessions stay visible but cannot be selected")
	...
	hud.apply_server_browser_result({"accepted": false, "reason": &"directory_timeout", "message": "Directory timed out.", "retryable": true})
	_check((hud.get("_server_browser_title") as Label).text == "SERVER LIST UNAVAILABLE", "error state is explicit")
	hud.request_server_browser_refresh()
	_check(_intents.size() == 2 and _intents[1].payload.action == &"refresh", "refresh is forwarded as a caller-owned intent")
	hud.queue_free()
	await process_frame
