...
func _test_service_and_visual_detail(module: HabitatSpine) -> void:
	...
	_test_mess_trestle_foot_batch(module)
...
func _test_mess_bench_leg_batch(module: HabitatSpine) -> void:
	var mess := module.get_node_or_null(
		^"Structure/ObservationCommon/CommonMess"
	) as Node3D
	var batch := mess.get_node_or_null(^"MessBenchLegs") as MultiMeshInstance3D if mess != null else null
	_check(
		mess != null and batch != null and batch.multimesh != null,
		"four mess-bench legs resolve through one visual-only MultiMesh"
	)
	if mess == null or batch == null or batch.multimesh == null:
		return
	var expected := [
		Transform3D(Basis.IDENTITY, Vector3(4.72, 0.195, 22.52)),
		Transform3D(Basis.IDENTITY, Vector3(4.72, 0.195, 24.08)),
		Transform3D(Basis.IDENTITY, Vector3(6.38, 0.195, 22.52)),
		Transform3D(Basis.IDENTITY, Vector3(6.38, 0.195, 24.08)),
	]
	var authored := batch.get_meta("authored_instance_transforms", []) as Array
	var transforms_exact := authored.size() == expected.size()
	for index in mini(authored.size(), expected.size()):
	...
	_check(
		transforms_exact
		and batch.multimesh.instance_count == HabitatSpine.MESS_BENCH_LEG_COPY_COUNT
		and batch.multimesh.visible_instance_count == -1
		and batch.multimesh.mesh.get_aabb().size.is_equal_approx(Vector3(0.38, 0.39, 0.10))
		and batch.multimesh.custom_aabb.is_equal_approx(AABB(
			Vector3(4.53, 0.0, 22.47), Vector3(2.04, 0.39, 1.66)
		))
		and batch.material_override == (mess.get_node(^"MessTrestleTie") as MeshInstance3D).material_override
		and batch.cast_shadow == GeometryInstance3D.SHADOW_CASTING_SETTING_ON
		and batch.layers == 1,
		"mess-bench leg batch preserves four exact transforms, extent, culling, structural material, shadows and render layer"
	)
	var physical_bench_count := 0
	for child in mess.get_children():
		var bench := child as StaticBody3D
		var mesh := bench.get_node_or_null(^"Mesh") as MeshInstance3D if bench != null else null
		if mesh != null and mesh.mesh.get_aabb().size.is_equal_approx(Vector3(0.44, 0.09, 2.12)):
			physical_bench_count += 1
	_check(
		mess.find_children("MessBenchLeg", "MeshInstance3D", false, false).is_empty()
		and physical_bench_count == 2
		and batch.get_child_count() == 0
		and batch.find_children("*", "CollisionObject3D", true, false).is_empty()
		and batch.get_groups().is_empty()
		and bool(batch.get_meta("visual_detail_only", false))
		and StringName(batch.get_meta("authored_source_name", &"")) == &"MessBenchLeg",
		"only authority-free bench-leg visuals are batched while both physical named benches remain intact"
	)
	var report := module.get_render_allocation_report()
	_check(
		int(report.mess_bench_leg_legacy_renderer_nodes) == 4
		and int(report.mess_bench_leg_renderer_nodes) == 1
...
func _test_mess_trestle_foot_batch(module: HabitatSpine) -> void:
	var mess := module.get_node_or_null(
		^"Structure/ObservationCommon/CommonMess"
	) as Node3D
	...
	_check(
		batch != null and batch.multimesh != null
			...
			and int(report.mess_trestle_foot_legacy_renderer_nodes) == 2
			and int(report.mess_trestle_foot_renderer_nodes) == 1
			and int(report.mess_trestle_foot_legacy_submissions) == 2
...
func _test_garden_rack_crown_batch(module: HabitatSpine) -> void:
	var racks := module.get_node_or_null(
		^"Structure/SideBranchGarden/GardenGrowRacks"
	) as Node3D
	var batch := racks.get_node_or_null(^"RackCrowns") as MultiMeshInstance3D if racks != null else null
	_check(
		racks != null and batch != null and batch.multimesh != null,
		"five garden-rack crowns resolve through one visual-only MultiMesh"
	)
	if racks == null or batch == null or batch.multimesh == null:
		return
	var expected: Array[Transform3D] = []
	for rack_index in 6:
		if rack_index == 4:
		...
		var radians := deg_to_rad(rack_angle)
		var rack_transform := Transform3D(
			Basis.from_euler(Vector3(0.0, deg_to_rad(rack_angle - 90.0), 0.0)),
			Vector3(14.4 + sin(radians) * 3.05, 0.0, 20.2 + cos(radians) * 3.05)
		)
		expected.append(
			rack_transform * Transform3D(Basis.IDENTITY, Vector3(0.0, 2.30, 0.0))
		)
	var authored := batch.get_meta("authored_instance_transforms", []) as Array
	var transforms_exact := authored.size() == expected.size()
	for index in mini(authored.size(), expected.size()):
	...
	var upright := racks.get_node_or_null(^"GrowRack01/RackUpright/Mesh") as MeshInstance3D
	_check(
		transforms_exact
		and batch.multimesh.instance_count == HabitatSpine.GARDEN_RACK_CROWN_COPY_COUNT
		and batch.multimesh.visible_instance_count == -1
		and batch.multimesh.mesh.get_aabb().size.is_equal_approx(Vector3(0.13, 0.12, 1.86))
		and batch.multimesh.custom_aabb.is_equal_approx(expected_bounds)
		and upright != null
		and batch.material_override == upright.material_override
		and batch.cast_shadow == GeometryInstance3D.SHADOW_CASTING_SETTING_ON
		and batch.layers == 1,
		"rack-crown batch preserves all five transforms, extent, culling, structural material, shadows and render layer"
	)
	_check(
		racks.find_children("RackCrown", "MeshInstance3D", true, false).is_empty()
		and racks.find_children("GrowRack*", "Node3D", false, false).size() == 5
		and batch.get_child_count() == 0
		and batch.get_script() == null
		and batch.get_groups().is_empty()
		and batch.find_children("*", "CollisionObject3D", true, false).is_empty()
		and batch.find_children("*", "Area3D", true, false).is_empty()
		and bool(batch.get_meta("visual_detail_only", false))
		and StringName(batch.get_meta("authored_source_name", &"")) == &"RackCrown",
		"only authority-free crown visuals are batched while physical rack trays and the rest of each rack remain intact"
	)
	var report := module.get_render_allocation_report()
	_check(
		int(report.garden_rack_crown_legacy_renderer_nodes) == 5
		and int(report.garden_rack_crown_renderer_nodes) == 1
		and int(report.garden_rack_crown_legacy_submissions) == 5
		and int(report.garden_rack_crown_submissions) == 1
