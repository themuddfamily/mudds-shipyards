...
func _run() -> void:
	...
	var canonical_host := presenter.manual_join_intent(" EXAMPLE.TEST. ", 27101, "Pilot One")
	_check(canonical_host.accepted and canonical_host.address == "example.test", "manual join normalizes a hostname without resolving it")
	...
	_check(ipv6.accepted and ipv6.form.address == "fe80::1" and ipv6.focus_target == &"manual_join", "bracketed IPv6 is normalized for caller-owned transport")
	...
	for invalid_address in ["[]", "fe80::1%eth0", "example.test:27101", "bad_host"]:
		var invalid := presenter.manual_join_intent(invalid_address, 27101, "Pilot One")
		_check(not invalid.accepted and invalid.reason == &"invalid_address", "manual join rejects invalid endpoint syntax: %s" % invalid_address)
