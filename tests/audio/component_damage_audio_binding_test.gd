...
func _run() -> void:
	...
	_check(bool(binding.present_damage_snapshot({"stage": &"destroyed", "health_ratio": 0.0}).accepted), "destroyed component state is presented")
	_check(_has_cue(&"ship_destroyed"), "destroyed state emits the existing destruction cue")
	var destruction_cue_count := _events.count(&"ship_destroyed")
	_check(bool(binding.present_damage_snapshot({"stage": &"destroyed", "health_ratio": 0.0}).accepted), "duplicate destroyed state remains accepted")
	_check(_events.count(&"ship_destroyed") == destruction_cue_count, "duplicate destroyed state does not replay the destruction cue")
