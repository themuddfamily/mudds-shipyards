...
func _run() -> void:
	...
	_check(bool(exported.accepted) and filesystem.files.has("user://diagnostics/exports/diagnostics-support-00000001.json") and filesystem.files.has("user://diagnostics/exports/diagnostics-support-index.json"), "caller-authorized export publishes bounded generation bundle and index")
	...
	_check(bool(sink.export_support_bundle("user://diagnostics/exports", 2).accepted), "second support generation publishes")
	_check(bool(sink.export_support_bundle("user://diagnostics/exports", 3).accepted), "third support generation publishes")
	_check(bool(sink.export_support_bundle("user://diagnostics/exports", 4).accepted) and not filesystem.files.has("user://diagnostics/exports/diagnostics-support-00000001.json"), "support rotation retains at most three generation bundles")
	var deleted := sink.delete_all_support_exports("user://diagnostics/exports")
	_check(bool(deleted.accepted) and not filesystem.files.has("user://diagnostics/exports/diagnostics-support-index.json"), "caller-authorized delete removes only owned support exports")
