...
func _test_service_presentations(berths: Node3D, audit: Dictionary) -> void:
	...
	_check(
		...
		and int(audit.get("static_bodies", -1)) == 9
		and int(audit.get("collision_shapes", -1)) == 12
		and int(audit.get("mesh_instances", -1)) == 35
		...
		and int(audit.get("descendants", -1)) == 78
		and int(budgets.get("static_bodies", -1)) == 9
		and int(budgets.get("collision_shapes", -1)) == 12
		and int(budgets.get("mesh_instances", -1)) == 35
		...
		and int(budgets.get("descendants", -1)) == 78,
		"the three pads, service silhouettes, and shared access freeze at 35 world renderers, 33 total / 12 service mesh resources, 5 guide lights, 78 descendants, 9 walkable bodies, and 12 exact shapes"
	)
...
func _expected_pad_piece_specs(pad_index: int) -> Array[Dictionary]:
	if pad_index != 2:
		return [{"name": "", "position": Vector3.ZERO, "size": Vector3(28.0, 0.6, 42.0)}]
	return [
...
func _test_access_circulation(berths: Node3D, audit: Dictionary) -> void:
	...
	_check(
		...
		and int(access.get("support_meshes", -1)) == 9
		...
		"Dock 04/05/06 share six collision-backed access surfaces, visible support, and clear landing envelopes"
	)
	...
	var dock04 := circulation.get_node_or_null(^"Dock04CargoRamp") as StaticBody3D \
		if circulation != null else null
	var dock05 := circulation.get_node_or_null(^"Dock05BomberRamp") as StaticBody3D \
		if circulation != null else null
	var branch := circulation.get_node_or_null(^"Dock06Branch") as StaticBody3D \
		if circulation != null else null
	var dock06 := circulation.get_node_or_null(^"Dock06InterceptorRamp") as StaticBody3D \
		if circulation != null else null
	_check(
		...
		and absf(dock05.position.x - 19.0) < 0.06
		and branch.position.is_equal_approx(Vector3(-10.5, -0.3, 7.0))
		and absf(dock06.position.z - 12.0) < 0.06,
		"the shared spine meets the Fleet Dock edge and three bounded ramps meet their unchanged pad edges"
	)
	...
	if collision != null:
		...
		_check(
			...
			and (disabled.get("errors", PackedStringArray()) as PackedStringArray).has(
				"access surface collision drift: Dock04CargoRamp"
			),
			"disabling a required access ramp is structured red"
		)
	...
	_check(dock04_pad_collision != null, "Dock 04 pad owns its co-located production collision")
	if dock04_pad_collision != null:
		var aligned_position := dock04_pad_collision.position
		dock04_pad_collision.position.y = -0.3
		...
		_check(
			...
			and (old_offset.get("errors", PackedStringArray()) as PackedStringArray).has(
				"pad collision/render alignment drift: dock_04_cargo"
			),
			"restoring the old embedded-floor collider offset is structured red"
		)
		dock04_pad_collision.position = aligned_position
