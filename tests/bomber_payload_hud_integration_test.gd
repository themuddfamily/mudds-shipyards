...
func _run() -> void:
	...
	await process_frame
	var next_flight_detail := (hud.get("_runtime_status_detail") as Label).text
	...
	_check(not hud.apply_bomber_payload_snapshot(stale_request) and (hud.get("_runtime_status_detail") as Label).text == expiry_detail, "a future release cannot carry a stale request sequence")
	...
	_check(not hud.apply_bomber_payload_snapshot(mismatched_terminal) and (hud.get("_runtime_status_detail") as Label).text == expiry_detail, "one terminal sequence cannot repaint expiry as impact")
	...
	await process_frame
	var network_flight_detail := (hud.get("_runtime_status_detail") as Label).text
	_check(network_flight_detail.contains("[IN FLIGHT]") and network_flight_detail.contains("NEXT // TRACK SERVER AUTHORITY") and (hud.get("_runtime_status_actions") as HBoxContainer).get_child_count() == 0, "a replica is readable but exposes no local release action")
