...
func _run() -> void:
	...
	_check(_requests.size() == 2, "both controls emit caller-owned persistence intents")
	...
	hud.set_settings_snapshot({"multiplayer_display_name": "Captain", "network_default_port": 29000})
	_check(name_control.text == "Captain" and int(port_control.value) == 29000, "server-browser defaults update from caller snapshots")
	hud.queue_free()
	await process_frame
