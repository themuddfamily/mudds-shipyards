...
func _run() -> void:
	...
	_check(detail.text.contains("CRAFT // CINDER-CARGO-HAULER") and detail.text.contains("SEAT AVAILABLE"), "initial detached Cinder state reaches HUD")
	...
	var receipt := craft.submit_crew_intent(1, 72, &"binding_loadmaster", RoleProfile.ACTION_PASSENGER_CARGO_MANIFEST, {"manifest_id": &"binding_manifest", "route_id": &"dock_04_cargo", "ready": true}, 2)
	_check(bool(receipt.get("consumed", false)), "caller submits the manifest receipt")
	_check(detail.text.contains("SEAT MANIFEST_READY") and detail.text.contains("GENERATION // 1"), "accepted manifest signal updates HUD")
	var released := craft.release_crew_role(1, 72, &"binding_loadmaster", Hauler.LOADMASTER_STATION_SEAT_ID, 3, 1)
	_check(bool(released.get("accepted", false)), "caller releases the loadmaster role")
	_check(detail.text.contains("SEAT RELEASED") and detail.text.contains("GENERATION // 2"), "release signal updates HUD with fenced generation")
