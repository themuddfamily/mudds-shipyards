...
func _test_live_station_coverage(
	...
	#
	# Re-frozen 2620 -> 2616 when the Central service line's four anonymous black
	# bin-stock blocks moved from ordinary MeshInstances into one mapped
	...
	# 1655 -> 1561 (-94), including the seven post-stage couriers' 35 structural
	# bindings. The corresponding live batch roster is 220/125 below.
	...
	#
	# Re-frozen 2371/45/849/1477 -> 2276/45/849/1382 for the Phase 10 §2 dressing
	...
	# surface per distinct source material, so 95 submissions that shared a
	...
	# the same 95 and the 0.22/0.28 buckets do not move at all. No recipe, no
	...
	# data plate does.
	#
	# Re-frozen 1956/45/643/1268 -> 1945/45/643/1257 by the tenth node trim
	# (batched pieces present one mapped surface each). Previously re-frozen
	# 1937 -> 1956 by the station-operations
...
func _test_recent_module_material_rosters(world: ShipyardWorld) -> void:
	...
	_check(
		exact,
		"Central, Modern, Fabrication, Observation and Salvage material consumers retain their exact local rosters"
	)
