...
- [ ] Expand the Phase 2 single-interceptor encounter into varied weapons, opponents, tactics, and combat scenarios.
  ...
  - [x] Make the existing *Heavy Breach* picket-and-screen scenario reachable through the production game loop. **Player-visible outcome:** the on-foot interaction sensor now finds a collision-backed Heavy Breach board seated on the launch-transition deck; `E` arms one sortie, a second `E` cancels it cleanly, and the same board can immediately arm a new generation. The retained objective tells the player to choose a combat-capable craft. Boarding, engine readiness, or a stale departure callback cannot start combat against the avatar or consume a later admission: only a real supported weapon source physically leaving its registered berth in `FREE_FLIGHT` launches the charged picket and one screening wing. The Heavy Breach HUD then has activity-objective precedence and refreshes its live elapsed/objective state at 10 Hz. An armed sortie suppresses the default Cinder activity and the in-range station-defence directory route, so two encounter authorities cannot start together. `HeavyBreachActivityBoard` still owns only physical admission and generation fencing; `GameFlow` owns the seat/departure handoff; `EncounterScenarioDirector`, `LiveCombatAuthority`, and the existing damage path retain objective, combat, and damage authority respectively. `tests/heavy_breach_game_flow_production_route_test.gd` proves the embodied board→cancel/re-arm→Arrow seat→physical thrust/departure route and concurrency guards in 17 focused assertions; the supported placement and armed frame were also inspected locally in real 1600×900 Forward+, which is not a native/package human gate. **Still open:** Heavy Breach reward persistence, packaged/native combat readability and audio, human balance review, and the broader weapons/damage parent item.
