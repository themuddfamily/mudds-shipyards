...
- The station's own published outbound aim, the `LaunchGate` marker, sat at **y = 8.0**, which is *inside* the band that beam blocks. Measured against the real production hulls on the launch centreline, as ship-origin altitude: a Torrent holding altitude out of the corridor is stopped by `RangeHeader` for every altitude from **4.8 to 10.2 m**, and the marker pointed at the middle of it. The aperture is the same height right across the gate's 63 m — every sampled x from −30 to +30 gives the same 4.70 m ceiling — so there is no lateral way round it, and over the top needs y ≥ 10.3 (Torrent) or 10.8 (Jovian), an 8 m climb inside the 56 m available. **Under is the only line all five flyables can take**, which is the line the Cinder Reach beacon chain was already drawn for: `RouteBeaconAlpha` is at y = −9. The marker is re-aimed `(0, 8, -64) -> (0, 2.5, -64)`, the centre of the measured **1.24 – 3.80 m** fleet-worst Jovian lane between the launch arm deck and the beam's 8.5 m underside, and published as `ShipyardWorld.get_outbound_clearance_band()` so the marker, the flight review's first waypoint and the regression all read one number. The 2026-08-16 five-craft rerun measured Halyard as the deepest hull at `28.35 m`, but its `3.90 / 1.10 m` gate/corridor bounds do not displace Jovian's `3.80 / 1.30 m` sampled worst case, so the published `3.80 / 1.24 m` constants remain unchanged.
