#!/usr/bin/env python3
"""Validate version-79 source-hash evidence and closure evidence."""
...
def validate_v79(value: Any, label: str = "evidence_closure_v79") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v79(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_EVIDENCE_CLOSURE_V79_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_EVIDENCE_CLOSURE_V79_VALID")
    return 0
