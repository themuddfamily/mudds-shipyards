#!/usr/bin/env python3
"""Validate version-264 package source-hash provenance evidence."""
...
def validate_v264(value: Any, label: str = "source_provenance_v264") -> list[str]:
    """Return violations; an empty list means v264 evidence is valid."""
    ...
    count_fields = ("source_acknowledged_count", "package_acknowledged_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v264(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V264_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V264_VALID")
    return 0
