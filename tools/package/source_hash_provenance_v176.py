#!/usr/bin/env python3
"""Validate version-176 package source-hash provenance evidence."""
...
def validate_v176(value: Any, label: str = "source_provenance_v176") -> list[str]:
    """Return violations; an empty list means v176 evidence is valid."""
    ...
    count_fields = ("source_digest_count", "package_digest_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v176(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V176_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V176_VALID")
    return 0
