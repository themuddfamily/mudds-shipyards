#!/usr/bin/env python3
"""Validate version-43 source/hash linked authority and provenance evidence."""
...
def validate_v43(value: Any, label: str = "linked_v43") -> list[str]:
    ...
    required = ("build_label", "source_commit", "package_version", "authority_id", "provenance_id", "link_id")
    for key in required:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v43(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_LINKED_AUTHORITY_PROVENANCE_VERSION_V43_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_LINKED_AUTHORITY_PROVENANCE_VERSION_V43_VALID")
    return 0
