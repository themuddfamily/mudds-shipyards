#!/usr/bin/env python3
"""Validate version-270 package source-hash provenance evidence."""
...
def validate_v270(value: Any, label: str = "source_provenance_v270") -> list[str]:
    """Return violations; an empty list means v270 evidence is valid."""
    ...
    count_fields = ("source_reconciled_count", "package_reconciled_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v270(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V270_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V270_VALID")
    return 0
