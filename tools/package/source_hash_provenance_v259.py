#!/usr/bin/env python3
"""Validate version-259 package source-hash provenance evidence."""
...
def validate_v259(value: Any, label: str = "source_provenance_v259") -> list[str]:
    """Return violations; an empty list means v259 evidence is valid."""
    ...
    count_fields = ("source_shipped_count", "package_shipped_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v259(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V259_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V259_VALID")
    return 0
