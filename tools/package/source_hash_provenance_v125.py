#!/usr/bin/env python3
"""Validate version-125 source-hash/provenance evidence."""
...
def validate_v125(value: Any, label: str = "source_provenance_v125") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v125(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V125_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V125_VALID")
    return 0
