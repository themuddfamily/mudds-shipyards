#!/usr/bin/env python3
"""Validate version-121 source-hash/provenance evidence."""
...
def validate_v121(value: Any, label: str = "source_provenance_v121") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v121(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V121_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V121_VALID")
    return 0
