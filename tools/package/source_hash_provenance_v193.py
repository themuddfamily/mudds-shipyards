#!/usr/bin/env python3
"""Validate version-193 package source-hash provenance evidence."""
...
def validate_v193(value: Any, label: str = "source_provenance_v193") -> list[str]:
    """Return violations; an empty list means v193 evidence is valid."""
    ...
    count_fields = ("source_invariant_count", "package_invariant_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v193(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V193_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V193_VALID")
    return 0
