#!/usr/bin/env python3
"""Validate version-186 package source-hash provenance evidence."""
...
def validate_v186(value: Any, label: str = "source_provenance_v186") -> list[str]:
    """Return violations; an empty list means v186 evidence is valid."""
    ...
    count_fields = ("source_state_count", "package_state_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v186(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V186_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V186_VALID")
    return 0
