#!/usr/bin/env python3
"""Validate version-251 package source-hash provenance evidence."""
...
def validate_v251(value: Any, label: str = "source_provenance_v251") -> list[str]:
    """Return violations; an empty list means v251 evidence is valid."""
    ...
    count_fields = ("source_in_progress_count", "package_in_progress_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v251(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V251_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V251_VALID")
    return 0
