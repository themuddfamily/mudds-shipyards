#!/usr/bin/env python3
"""Validate version-199 package source-hash provenance evidence."""
...
def validate_v199(value: Any, label: str = "source_provenance_v199") -> list[str]:
    """Return violations; an empty list means v199 evidence is valid."""
    ...
    count_fields = ("source_callback_count", "package_callback_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v199(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V199_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V199_VALID")
    return 0
