#!/usr/bin/env python3
"""Validate version-120 source-hash closure/provenance evidence."""
...
def validate_v120(value: Any, label: str = "closure_provenance_v120") -> list[str]:
    """Return violations; an empty list means closure/provenance evidence is valid."""
    ...
    if isinstance(closure, dict) and closure.get("status") == "PASS":
        for key in ("closure_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(provenance, dict) and provenance.get("status") == "PASS":
        for key in ("provenance_id", "closure_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v120(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_CLOSURE_PROVENANCE_V120_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_CLOSURE_PROVENANCE_V120_VALID")
    return 0
