...
from tools.package.source_hash_provenance_v326 import validate_v326


def record() -> dict:
    binding = {
        "source_id": "source-326", "source_commit": "b" * 40, "source_hash": "c" * 64,
        "package_version": "32.6.0", "source_artifact_hash_count": 2,
        "package_artifact_hash_count": 3, "authorization_attestation_id": "authorization-attestation-326",
    ...
    return {"schema_version": 326, "build_label": "source-provenance-v326", **binding,
            "source": {"status": "PASS", "evidence": "source", **binding, "identified": True},
            "authorization_attestation": {"status": "PASS", "evidence": "attestation",
                "authorization_attestation_id": "authorization-attestation-326",
...
class SourceHashProvenanceV326Test(unittest.TestCase):
    def test_accepts_attestation_binding_and_not_run_gates(self):
        self.assertEqual(validate_v326(record()), [])
    ...
    def test_rejects_schema_or_authorized_flag(self):
        ...
        errors = validate_v326(item)
        self.assertTrue(any("schema_version must be 326" in error for error in errors))
