#!/usr/bin/env python3
"""Validate version-234 package source-hash provenance evidence."""
...
def validate_v234(value: Any, label: str = "source_provenance_v234") -> list[str]:
    """Return violations; an empty list means v234 evidence is valid."""
    ...
    count_fields = ("source_retained_count", "package_retained_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v234(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V234_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V234_VALID")
    return 0
