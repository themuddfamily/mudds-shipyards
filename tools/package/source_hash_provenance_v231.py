#!/usr/bin/env python3
"""Validate version-231 package source-hash provenance evidence."""
...
def validate_v231(value: Any, label: str = "source_provenance_v231") -> list[str]:
    """Return violations; an empty list means v231 evidence is valid."""
    ...
    count_fields = ("source_finalized_count", "package_finalized_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v231(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V231_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V231_VALID")
    return 0
