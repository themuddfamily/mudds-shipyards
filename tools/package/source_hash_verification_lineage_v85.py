#!/usr/bin/env python3
"""Validate version-85 source-hash verification and lineage evidence."""
...
def validate_v85(value: Any, label: str = "verification_lineage_v85") -> list[str]:
    """Return violations; an empty list means verification lineage evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_id", "verification_id", "lineage_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(lineage, dict) and lineage.get("status") == "PASS":
        for key in ("lineage_id", "verification_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v85(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_VERIFICATION_LINEAGE_V85_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_VERIFICATION_LINEAGE_V85_VALID")
    return 0
