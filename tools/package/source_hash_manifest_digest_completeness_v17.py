#!/usr/bin/env python3
"""Validate version-17 source/hash manifest digest completeness evidence."""
...
def validate_v17(value: Any, label: str = "completeness_v17") -> list[str]:
    """Return violations; an empty list means digest completeness is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_commit", "manifest_id", "manifest_digest"):
    ...
    audit = value.get("completeness_audit")
    _status(audit, f"{label}.completeness_audit", errors)
    if isinstance(audit, dict) and audit.get("status") == "PASS":
        if audit.get("manifest_id") != value.get("manifest_id"):
            errors.append(f"{label}.completeness_audit.manifest_id must match manifest_id")
        if audit.get("entry_count") != len(entries):
            errors.append(f"{label}.completeness_audit.entry_count must equal entries count")
        if audit.get("complete_digest_count") != complete:
            errors.append(f"{label}.completeness_audit.complete_digest_count must equal complete digest count")
        if audit.get("all_digests_present") is not True:
            errors.append(f"{label}.completeness_audit.all_digests_present must be true when status is PASS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v17(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_MANIFEST_DIGEST_COMPLETENESS_V17_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_MANIFEST_DIGEST_COMPLETENESS_V17_VALID")
    return 0
