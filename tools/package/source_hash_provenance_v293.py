#!/usr/bin/env python3
"""Validate version-293 package ledger/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=293;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v293(v:Any,label:str="source_provenance_v293")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","ledger_id","ledger_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","ledger_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","ledger_id","ledger_digest","ledger_entry_count")
    ...
    q=v.get("ledger");_status(q,f"{label}.ledger",e)
    if isinstance(q,dict) and q.get("status")=="PASS":
        for k in ("ledger_id","ledger_digest","source_hash","package_artifact_hash_count","ledger_entry_count"):
            if q.get(k)!=v.get(k):e.append(f"{label}.ledger.{k} must match {k}")
        if q.get("complete") is not True:e.append(f"{label}.ledger.complete must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v293(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V293_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V293_VALID");return 0
