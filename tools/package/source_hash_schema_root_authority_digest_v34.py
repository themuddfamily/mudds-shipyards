#!/usr/bin/env python3
"""Validate version-34 source/hash schema root-authority digest evidence."""
...
def validate_v34(value: Any, label: str = "schema_v34") -> list[str]:
    """Return violations; an empty list means schema/root evidence is valid."""
    ...
    if isinstance(root, dict) and root.get("status") == "PASS":
        if root.get("schema_version") != value.get("schema_version") or root.get("record_version") != value.get("record_version"):
            errors.append(f"{label}.root schema/version must match declared values")
    ...
    if isinstance(authority, dict) and authority.get("status") == "PASS":
        if authority.get("schema_version") != value.get("schema_version") or authority.get("record_version") != value.get("record_version"):
            errors.append(f"{label}.authority schema/version must match declared values")
    ...
    if isinstance(pair, dict) and pair.get("status") == "PASS":
        if pair.get("schema_version") != value.get("schema_version"):
            errors.append(f"{label}.pair.schema_version must match schema_version")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v34(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_SCHEMA_ROOT_AUTHORITY_DIGEST_V34_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_SCHEMA_ROOT_AUTHORITY_DIGEST_V34_VALID")
    return 0
