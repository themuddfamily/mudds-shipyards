#!/usr/bin/env python3
"""Validate version-167 package source-hash provenance evidence."""
...
def validate_v167(value: Any, label: str = "source_provenance_v167") -> list[str]:
    """Return violations; an empty list means v167 evidence is valid."""
    ...
    count_fields = ("source_tag_count", "package_tag_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v167(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V167_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V167_VALID")
    return 0
