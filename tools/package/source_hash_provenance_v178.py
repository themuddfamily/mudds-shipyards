#!/usr/bin/env python3
"""Validate version-178 package source-hash provenance evidence."""
...
def validate_v178(value: Any, label: str = "source_provenance_v178") -> list[str]:
    """Return violations; an empty list means v178 evidence is valid."""
    ...
    count_fields = ("source_record_count", "package_record_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v178(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V178_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V178_VALID")
    return 0
