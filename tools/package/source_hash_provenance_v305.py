#!/usr/bin/env python3
"""Validate version-305 package attestation-root/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=305;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v305(v:Any,label:str="source_provenance_v305")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","attestation_root_id","attestation_root_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","attestation_root_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","attestation_root_id","attestation_root_digest","attestation_root_entry_count")
    ...
    r=v.get("attestation_root");_status(r,f"{label}.attestation_root",e)
    if isinstance(r,dict) and r.get("status")=="PASS":
        for k in ("attestation_root_id","attestation_root_digest","source_hash","package_artifact_hash_count","attestation_root_entry_count"):
            if r.get(k)!=v.get(k):e.append(f"{label}.attestation_root.{k} must match {k}")
        if r.get("verified") is not True:e.append(f"{label}.attestation_root.verified must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v305(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V305_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V305_VALID");return 0
