#!/usr/bin/env python3
"""Validate version-68 source-hash evidence and lineage evidence."""
...
def validate_v68(value: Any, label: str = "evidence_lineage_v68") -> list[str]:
    """Return violations; an empty list means evidence lineage is valid."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v68(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_EVIDENCE_LINEAGE_V68_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_EVIDENCE_LINEAGE_V68_VALID")
    return 0
