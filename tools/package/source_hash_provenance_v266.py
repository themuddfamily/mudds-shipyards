#!/usr/bin/env python3
"""Validate version-266 package source-hash provenance evidence."""
...
def validate_v266(value: Any, label: str = "source_provenance_v266") -> list[str]:
    """Return violations; an empty list means v266 evidence is valid."""
    ...
    count_fields = ("source_inspected_count", "package_inspected_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v266(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V266_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V266_VALID")
    return 0
