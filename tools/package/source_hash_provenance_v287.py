#!/usr/bin/env python3
"""Validate version-287 package manifest/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=287;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v287(v:Any,label:str="source_provenance_v287")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","manifest_id","manifest_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","manifest_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","manifest_id","manifest_digest","manifest_entry_count")
    ...
    m=v.get("manifest");_status(m,f"{label}.manifest",e)
    if isinstance(m,dict) and m.get("status")=="PASS":
        for k in ("manifest_id","manifest_digest","source_hash","package_artifact_hash_count","manifest_entry_count"):
            if m.get(k)!=v.get(k):e.append(f"{label}.manifest.{k} must match {k}")
        if m.get("bound") is not True:e.append(f"{label}.manifest.bound must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v287(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V287_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V287_VALID");return 0
