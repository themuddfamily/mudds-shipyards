#!/usr/bin/env python3
"""Validate version-42 source/hash linked authority and provenance evidence."""
...
def validate_v42(value: Any, label: str = "linked_v42") -> list[str]:
    """Return violations; an empty list means linked version evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_commit", "package_version", "authority_id", "provenance_id", "link_id"):
        if not _text(value.get(key)):
            errors.append(f"{label}.{key} is required")
    authority = value.get("authority")
    _status(authority, f"{label}.authority", errors)
    if isinstance(authority, dict) and authority.get("status") == "PASS":
        for key in ("authority_id", "source_commit", "package_version"):
    ...
    if isinstance(provenance, dict) and provenance.get("status") == "PASS":
        for key in ("provenance_id", "source_commit", "package_version"):
    ...
    if isinstance(link, dict) and link.get("status") == "PASS":
        for key in ("link_id", "authority_id", "provenance_id", "source_commit", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v42(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_LINKED_AUTHORITY_PROVENANCE_VERSION_V42_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_LINKED_AUTHORITY_PROVENANCE_VERSION_V42_VALID")
    return 0
