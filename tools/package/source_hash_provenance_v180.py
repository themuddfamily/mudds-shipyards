#!/usr/bin/env python3
"""Validate version-180 package source-hash provenance evidence."""
...
def validate_v180(value: Any, label: str = "source_provenance_v180") -> list[str]:
    """Return violations; an empty list means v180 evidence is valid."""
    ...
    count_fields = ("source_entry_count", "package_entry_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v180(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V180_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V180_VALID")
    return 0
