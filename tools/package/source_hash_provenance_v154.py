#!/usr/bin/env python3
"""Validate version-154 package source-hash provenance evidence."""
...
def validate_v154(value: Any, label: str = "source_provenance_v154") -> list[str]:
    """Return violations; an empty list means v154 evidence is valid."""
    ...
    count_fields = ("source_path_count", "package_file_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v154(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V154_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V154_VALID")
    return 0
