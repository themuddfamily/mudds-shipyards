#!/usr/bin/env python3
"""Validate version-30 source/hash paired root authority evidence."""
...
def validate_v30(value: Any, label: str = "authority_v30") -> list[str]:
    """Return violations; an empty list means root authority evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_commit", "source_digest", "artifact_digest", "root_id", "authority_id", "authority_digest"):
        if not _text(value.get(key)):
            errors.append(f"{label}.{key} is required")
    for key in ("source_digest", "artifact_digest", "authority_digest"):
    ...
    if isinstance(root, dict) and root.get("status") == "PASS":
        ...
        if root.get("source_digest") != value.get("source_digest") or root.get("artifact_digest") != value.get("artifact_digest"):
            errors.append(f"{label}.root digests must match declared digests")
    ...
    if isinstance(authority, dict) and authority.get("status") == "PASS":
        ...
        if authority.get("root_id") != value.get("root_id"):
            errors.append(f"{label}.authority.root_id must match root_id")
    ...
    if isinstance(pair, dict) and pair.get("status") == "PASS":
        if pair.get("root_id") != value.get("root_id") or pair.get("authority_id") != value.get("authority_id"):
            errors.append(f"{label}.pair root/authority IDs must match declared IDs")
        if pair.get("authority_digest") != value.get("authority_digest"):
            errors.append(f"{label}.pair.authority_digest must match authority_digest")
        if pair.get("authorized") is not True:
            errors.append(f"{label}.pair.authorized must be true when status is PASS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v30(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PAIRED_ROOT_AUTHORITY_V30_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PAIRED_ROOT_AUTHORITY_V30_VALID")
    return 0
