#!/usr/bin/env python3
"""Validate version-301 package provenance graph-root/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=301;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v301(v:Any,label:str="source_provenance_v301")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","root_id","root_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","root_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","root_id","root_digest","root_entry_count")
    ...
    r=v.get("root");_status(r,f"{label}.root",e)
    if isinstance(r,dict) and r.get("status")=="PASS":
        for k in ("root_id","root_digest","source_hash","package_artifact_hash_count","root_entry_count"):
            if r.get(k)!=v.get(k):e.append(f"{label}.root.{k} must match {k}")
        if r.get("verified") is not True:e.append(f"{label}.root.verified must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v301(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V301_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V301_VALID");return 0
