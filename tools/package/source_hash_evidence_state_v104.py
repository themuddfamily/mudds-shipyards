#!/usr/bin/env python3
"""Validate version-104 source-hash evidence and state evidence."""
...
def validate_v104(value: Any, label: str = "evidence_state_v104") -> list[str]:
    """Return violations; an empty list means evidence state is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_id", "evidence_id", "state_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(state, dict) and state.get("status") == "PASS":
        for key in ("state_id", "evidence_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v104(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_EVIDENCE_STATE_V104_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_EVIDENCE_STATE_V104_VALID")
    return 0
