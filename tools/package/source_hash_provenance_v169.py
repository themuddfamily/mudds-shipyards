#!/usr/bin/env python3
"""Validate version-169 package source-hash provenance evidence."""
...
def validate_v169(value: Any, label: str = "source_provenance_v169") -> list[str]:
    """Return violations; an empty list means v169 evidence is valid."""
    ...
    count_fields = ("source_release_count", "package_release_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v169(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V169_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V169_VALID")
    return 0
