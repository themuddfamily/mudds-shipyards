#!/usr/bin/env python3
"""Validate version-290 package target/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=290;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v290(v:Any,label:str="source_provenance_v290")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","target_id","target_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","target_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","target_id","target_digest","target_entry_count")
    ...
    t=v.get("target");_status(t,f"{label}.target",e)
    if isinstance(t,dict) and t.get("status")=="PASS":
        for k in ("target_id","target_digest","source_hash","package_artifact_hash_count","target_entry_count"):
            if t.get(k)!=v.get(k):e.append(f"{label}.target.{k} must match {k}")
        if t.get("bound") is not True:e.append(f"{label}.target.bound must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v290(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V290_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V290_VALID");return 0
