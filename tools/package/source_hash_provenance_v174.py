#!/usr/bin/env python3
"""Validate version-174 package source-hash provenance evidence."""
...
def validate_v174(value: Any, label: str = "source_provenance_v174") -> list[str]:
    """Return violations; an empty list means v174 evidence is valid."""
    ...
    count_fields = ("source_byte_count", "package_byte_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v174(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V174_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V174_VALID")
    return 0
