#!/usr/bin/env python3
"""Validate version-3 source/hash summary digest audit evidence."""
...
def validate_v3(value: Any, label: str = "audit_v3") -> list[str]:
    """Return violations; an empty list means the v3 audit is valid."""
    ...
    if isinstance(audit, dict) and audit.get("status") == "PASS":
        for key in ("source_bound", "digest_reproducible", "reviewed"):
    ...
    if isinstance(review, dict) and review.get("status") == "PASS":
        ...
        if not _text(review.get("reviewer")):
            errors.append(f"{label}.review.reviewer is required when status is PASS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v3(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_AUDIT_SUMMARY_DIGEST_V3_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_AUDIT_SUMMARY_DIGEST_V3_VALID")
    return 0
