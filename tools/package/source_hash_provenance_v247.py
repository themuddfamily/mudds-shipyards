#!/usr/bin/env python3
"""Validate version-247 package source-hash provenance evidence."""
...
def validate_v247(value: Any, label: str = "source_provenance_v247") -> list[str]:
    """Return violations; an empty list means v247 evidence is valid."""
    ...
    count_fields = ("source_quarantined_count", "package_quarantined_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v247(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V247_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V247_VALID")
    return 0
