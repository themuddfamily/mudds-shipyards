#!/usr/bin/env python3
"""Validate version-187 package source-hash provenance evidence."""
...
def validate_v187(value: Any, label: str = "source_provenance_v187") -> list[str]:
    """Return violations; an empty list means v187 evidence is valid."""
    ...
    count_fields = ("source_status_count", "package_status_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v187(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V187_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V187_VALID")
    return 0
