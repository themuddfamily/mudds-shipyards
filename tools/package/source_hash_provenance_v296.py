#!/usr/bin/env python3
"""Validate version-296 package provenance-root/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=296;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v296(v:Any,label:str="source_provenance_v296")->list[str]:
    ...
    if isinstance(r,dict) and r.get("status")=="PASS":
        ...
        if r.get("closed") is not True:e.append(f"{label}.root.closed must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v296(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V296_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V296_VALID");return 0
