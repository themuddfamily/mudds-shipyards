#!/usr/bin/env python3
"""Validate version-165 package source-hash provenance evidence."""
...
def validate_v165(value: Any, label: str = "source_provenance_v165") -> list[str]:
    """Return violations; an empty list means v165 evidence is valid."""
    ...
    count_fields = ("source_metadata_count", "package_metadata_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v165(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V165_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V165_VALID")
    return 0
