#!/usr/bin/env python3
"""Validate version-222 package source-hash provenance evidence."""
...
def validate_v222(value: Any, label: str = "source_provenance_v222") -> list[str]:
    """Return violations; an empty list means v222 evidence is valid."""
    ...
    count_fields = ("source_active_count", "package_active_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v222(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V222_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V222_VALID")
    return 0
