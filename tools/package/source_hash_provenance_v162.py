#!/usr/bin/env python3
"""Validate version-162 package source-hash provenance evidence."""
...
def validate_v162(value: Any, label: str = "source_provenance_v162") -> list[str]:
    """Return violations; an empty list means v162 evidence is valid."""
    ...
    count_fields = ("source_key_count", "package_key_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v162(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V162_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V162_VALID")
    return 0
