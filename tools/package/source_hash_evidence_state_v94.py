#!/usr/bin/env python3
"""Validate version-94 source-hash evidence and state evidence."""
...
def validate_v94(value: Any, label: str = "evidence_state_v94") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v94(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_EVIDENCE_STATE_V94_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_EVIDENCE_STATE_V94_VALID")
    return 0
