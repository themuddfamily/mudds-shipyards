#!/usr/bin/env python3
"""Validate version-163 package source-hash provenance evidence."""
...
def validate_v163(value: Any, label: str = "source_provenance_v163") -> list[str]:
    """Return violations; an empty list means v163 evidence is valid."""
    ...
    count_fields = ("source_value_count", "package_value_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v163(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V163_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V163_VALID")
    return 0
