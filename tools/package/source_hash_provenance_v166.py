#!/usr/bin/env python3
"""Validate version-166 package source-hash provenance evidence."""
...
def validate_v166(value: Any, label: str = "source_provenance_v166") -> list[str]:
    """Return violations; an empty list means v166 evidence is valid."""
    ...
    count_fields = ("source_flag_count", "package_flag_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v166(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V166_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V166_VALID")
    return 0
