#!/usr/bin/env python3
"""Validate version-198 package source-hash provenance evidence."""
...
def validate_v198(value: Any, label: str = "source_provenance_v198") -> list[str]:
    """Return violations; an empty list means v198 evidence is valid."""
    ...
    count_fields = ("source_handler_count", "package_handler_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v198(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V198_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V198_VALID")
    return 0
