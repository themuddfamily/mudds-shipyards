#!/usr/bin/env python3
"""Validate version-271 package source-hash provenance evidence."""
...
def validate_v271(value: Any, label: str = "source_provenance_v271") -> list[str]:
    """Return violations; an empty list means v271 evidence is valid."""
    ...
    count_fields = ("source_bound_count", "package_bound_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v271(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V271_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V271_VALID")
    return 0
