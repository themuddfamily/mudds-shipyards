#!/usr/bin/env python3
"""Validate version-267 package source-hash provenance evidence."""
...
def validate_v267(value: Any, label: str = "source_provenance_v267") -> list[str]:
    """Return violations; an empty list means v267 evidence is valid."""
    ...
    count_fields = ("source_scanned_count", "package_scanned_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v267(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V267_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V267_VALID")
    return 0
