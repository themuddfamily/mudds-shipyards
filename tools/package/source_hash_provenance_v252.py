#!/usr/bin/env python3
"""Validate version-252 package source-hash provenance evidence."""
...
def validate_v252(value: Any, label: str = "source_provenance_v252") -> list[str]:
    """Return violations; an empty list means v252 evidence is valid."""
    ...
    count_fields = ("source_requeued_count", "package_requeued_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v252(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V252_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V252_VALID")
    return 0
