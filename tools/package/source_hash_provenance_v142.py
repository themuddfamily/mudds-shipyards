#!/usr/bin/env python3
"""Validate version-142 package source-hash provenance evidence."""
...
def validate_v142(value: Any, label: str = "source_provenance_v142") -> list[str]:
    """Return violations; an empty list means v142 evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    fields = ("build_label", "source_id", "provenance_id", "source_commit", "source_hash", "source_version", "package_version", "source_commit_date", "package_release_date")
    for key in fields:
        if not _text(value.get(key)):
            errors.append(f"{label}.{key} is required")
    binding_fields = ("source_id", "source_commit", "source_hash", "source_version", "package_version", "source_commit_date", "package_release_date")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v142(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V142_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V142_VALID")
    return 0
