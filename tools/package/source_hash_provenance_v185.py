#!/usr/bin/env python3
"""Validate version-185 package source-hash provenance evidence."""
...
def validate_v185(value: Any, label: str = "source_provenance_v185") -> list[str]:
    """Return violations; an empty list means v185 evidence is valid."""
    ...
    count_fields = ("source_evidence_type_count", "package_evidence_type_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v185(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V185_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V185_VALID")
    return 0
