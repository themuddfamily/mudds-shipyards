#!/usr/bin/env python3
"""Validate version-105 source-hash provenance and closure evidence."""
...
def validate_v105(value: Any, label: str = "provenance_closure_v105") -> list[str]:
    """Return violations; an empty list means provenance closure evidence is valid."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v105(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_CLOSURE_V105_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_CLOSURE_V105_VALID")
    return 0
