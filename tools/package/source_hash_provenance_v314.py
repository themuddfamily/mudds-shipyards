#!/usr/bin/env python3
"""Validate version-314 package provenance approval/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=314;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v314(v:Any,label:str="source_provenance_v314")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","approval_id","approval_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","approval_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","approval_id","approval_digest","approval_entry_count")
    ...
    a=v.get("approval");_status(a,f"{label}.approval",e)
    if isinstance(a,dict) and a.get("status")=="PASS":
        for k in ("approval_id","approval_digest","source_hash","package_artifact_hash_count","approval_entry_count"):
            if a.get(k)!=v.get(k):e.append(f"{label}.approval.{k} must match {k}")
        if a.get("approved") is not True:e.append(f"{label}.approval.approved must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v314(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V314_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V314_VALID");return 0
