#!/usr/bin/env python3
"""Validate version-196 package source-hash provenance evidence."""
...
def validate_v196(value: Any, label: str = "source_provenance_v196") -> list[str]:
    """Return violations; an empty list means v196 evidence is valid."""
    ...
    count_fields = ("source_endpoint_count", "package_endpoint_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v196(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V196_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V196_VALID")
    return 0
