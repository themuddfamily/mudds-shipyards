#!/usr/bin/env python3
"""Validate version-221 package source-hash provenance evidence."""
...
def validate_v221(value: Any, label: str = "source_provenance_v221") -> list[str]:
    """Return violations; an empty list means v221 evidence is valid."""
    ...
    count_fields = ("source_queued_count", "package_queued_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v221(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V221_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V221_VALID")
    return 0
