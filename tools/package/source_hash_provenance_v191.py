#!/usr/bin/env python3
"""Validate version-191 package source-hash provenance evidence."""
...
def validate_v191(value: Any, label: str = "source_provenance_v191") -> list[str]:
    """Return violations; an empty list means v191 evidence is valid."""
    ...
    count_fields = ("source_test_case_count", "package_test_case_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v191(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V191_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V191_VALID")
    return 0
