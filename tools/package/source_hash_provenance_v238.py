#!/usr/bin/env python3
"""Validate version-238 package source-hash provenance evidence."""
...
def validate_v238(value: Any, label: str = "source_provenance_v238") -> list[str]:
    """Return violations; an empty list means v238 evidence is valid."""
    ...
    count_fields = ("source_verified_count", "package_verified_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v238(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V238_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V238_VALID")
    return 0
