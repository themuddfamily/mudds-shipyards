#!/usr/bin/env python3
"""Validate version-190 package source-hash provenance evidence."""
...
def validate_v190(value: Any, label: str = "source_provenance_v190") -> list[str]:
    """Return violations; an empty list means v190 evidence is valid."""
    ...
    count_fields = ("source_test_count", "package_test_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v190(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V190_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V190_VALID")
    return 0
