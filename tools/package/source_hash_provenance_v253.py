#!/usr/bin/env python3
"""Validate version-253 package source-hash provenance evidence."""
...
def validate_v253(value: Any, label: str = "source_provenance_v253") -> list[str]:
    """Return violations; an empty list means v253 evidence is valid."""
    ...
    count_fields = ("source_retried_count", "package_retried_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v253(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V253_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V253_VALID")
    return 0
