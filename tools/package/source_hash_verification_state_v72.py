#!/usr/bin/env python3
"""Validate version-72 source-hash verification and state evidence."""
...
def validate_v72(value: Any, label: str = "verification_state_v72") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v72(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_VERIFICATION_STATE_V72_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_VERIFICATION_STATE_V72_VALID")
    return 0
