#!/usr/bin/env python3
"""Validate version-31 source/hash paired root/authority digest evidence."""
...
def validate_v31(value: Any, label: str = "authority_v31") -> list[str]:
    """Return violations; an empty list means root/authority digest evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_commit", "root_id", "root_digest", "authority_id", "authority_digest"):
    ...
    if isinstance(pair, dict) and pair.get("status") == "PASS":
        if pair.get("root_digest") != value.get("root_digest") or pair.get("authority_digest") != value.get("authority_digest"):
        ...
        if pair.get("consistent") is not True:
            errors.append(f"{label}.pair.consistent must be true when status is PASS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v31(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PAIRED_ROOT_AUTHORITY_DIGEST_V31_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PAIRED_ROOT_AUTHORITY_DIGEST_V31_VALID")
    return 0
