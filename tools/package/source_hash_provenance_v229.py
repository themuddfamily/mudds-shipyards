#!/usr/bin/env python3
"""Validate version-229 package source-hash provenance evidence."""
...
def validate_v229(value: Any, label: str = "source_provenance_v229") -> list[str]:
    """Return violations; an empty list means v229 evidence is valid."""
    ...
    count_fields = ("source_loaded_count", "package_loaded_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v229(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V229_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V229_VALID")
    return 0
