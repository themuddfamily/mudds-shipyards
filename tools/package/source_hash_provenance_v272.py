#!/usr/bin/env python3
"""Validate version-272 package source-hash provenance evidence."""
...
def validate_v272(value: Any, label: str = "source_provenance_v272") -> list[str]:
    """Return violations; an empty list means v272 evidence is valid."""
    ...
    count_fields = ("source_evidence_count", "package_evidence_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v272(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V272_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V272_VALID")
    return 0
