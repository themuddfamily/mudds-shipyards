#!/usr/bin/env python3
"""Validate version-299 package provenance-edge/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=299;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v299(v:Any,label:str="source_provenance_v299")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","edge_id","edge_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","edge_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","edge_id","edge_digest","edge_entry_count")
    ...
    q=v.get("edge");_status(q,f"{label}.edge",e)
    if isinstance(q,dict) and q.get("status")=="PASS":
        for k in ("edge_id","edge_digest","source_hash","package_artifact_hash_count","edge_entry_count"):
            if q.get(k)!=v.get(k):e.append(f"{label}.edge.{k} must match {k}")
        if q.get("bound") is not True:e.append(f"{label}.edge.bound must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v299(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V299_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V299_VALID");return 0
