#!/usr/bin/env python3
"""Validate version-235 package source-hash provenance evidence."""
...
def validate_v235(value: Any, label: str = "source_provenance_v235") -> list[str]:
    """Return violations; an empty list means v235 evidence is valid."""
    ...
    count_fields = ("source_preserved_count", "package_preserved_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v235(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V235_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V235_VALID")
    return 0
