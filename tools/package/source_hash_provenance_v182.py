#!/usr/bin/env python3
"""Validate version-182 package source-hash provenance evidence."""
...
def validate_v182(value: Any, label: str = "source_provenance_v182") -> list[str]:
    """Return violations; an empty list means v182 evidence is valid."""
    ...
    count_fields = ("source_artifact_checksum_count", "package_artifact_checksum_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v182(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V182_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V182_VALID")
    return 0
