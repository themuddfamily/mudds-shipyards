#!/usr/bin/env python3
"""Validate version-204 package source-hash provenance evidence."""
...
def validate_v204(value: Any, label: str = "source_provenance_v204") -> list[str]:
    """Return violations; an empty list means v204 evidence is valid."""
    ...
    count_fields = ("source_pass_count", "package_pass_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v204(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V204_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V204_VALID")
    return 0
