#!/usr/bin/env python3
"""Validate version-206 package source-hash provenance evidence."""
...
def validate_v206(value: Any, label: str = "source_provenance_v206") -> list[str]:
    """Return violations; an empty list means v206 evidence is valid."""
    ...
    count_fields = ("source_fail_count", "package_fail_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v206(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V206_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V206_VALID")
    return 0
