#!/usr/bin/env python3
"""Validate version-192 package source-hash provenance evidence."""
...
def validate_v192(value: Any, label: str = "source_provenance_v192") -> list[str]:
    """Return violations; an empty list means v192 evidence is valid."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v192(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V192_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V192_VALID")
    return 0
