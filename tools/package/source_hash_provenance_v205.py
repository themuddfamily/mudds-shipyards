#!/usr/bin/env python3
"""Validate version-205 package source-hash provenance evidence."""
...
def validate_v205(value: Any, label: str = "source_provenance_v205") -> list[str]:
    """Return violations; an empty list means v205 evidence is valid."""
    ...
    count_fields = ("source_unknown_count", "package_unknown_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v205(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V205_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V205_VALID")
    return 0
