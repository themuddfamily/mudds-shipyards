#!/usr/bin/env python3
"""Validate version-112 source-hash audit and state evidence."""
...
def validate_v112(value: Any, label: str = "audit_state_v112") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v112(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_AUDIT_STATE_V112_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_AUDIT_STATE_V112_VALID")
    return 0
