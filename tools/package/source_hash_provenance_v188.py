#!/usr/bin/env python3
"""Validate version-188 package source-hash provenance evidence."""
...
def validate_v188(value: Any, label: str = "source_provenance_v188") -> list[str]:
    """Return violations; an empty list means v188 evidence is valid."""
    ...
    count_fields = ("source_result_count", "package_result_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v188(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V188_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V188_VALID")
    return 0
