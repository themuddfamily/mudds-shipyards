#!/usr/bin/env python3
"""Validate version-164 package source-hash provenance evidence."""
...
def validate_v164(value: Any, label: str = "source_provenance_v164") -> list[str]:
    """Return violations; an empty list means v164 evidence is valid."""
    ...
    count_fields = ("source_field_count", "package_field_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v164(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V164_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V164_VALID")
    return 0
