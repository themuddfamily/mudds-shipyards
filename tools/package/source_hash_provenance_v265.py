#!/usr/bin/env python3
"""Validate version-265 package source-hash provenance evidence."""
...
def validate_v265(value: Any, label: str = "source_provenance_v265") -> list[str]:
    """Return violations; an empty list means v265 evidence is valid."""
    ...
    count_fields = ("source_confirmed_count", "package_confirmed_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v265(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V265_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V265_VALID")
    return 0
