#!/usr/bin/env python3
"""Validate version-291 package scope/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=291;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v291(v:Any,label:str="source_provenance_v291")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","scope_id","scope_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","scope_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","scope_id","scope_digest","scope_entry_count")
    ...
    q=v.get("scope");_status(q,f"{label}.scope",e)
    if isinstance(q,dict) and q.get("status")=="PASS":
        for k in ("scope_id","scope_digest","source_hash","package_artifact_hash_count","scope_entry_count"):
            if q.get(k)!=v.get(k):e.append(f"{label}.scope.{k} must match {k}")
        if q.get("closed") is not True:e.append(f"{label}.scope.closed must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v291(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V291_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V291_VALID");return 0
