#!/usr/bin/env python3
"""Validate version-230 package source-hash provenance evidence."""
...
def validate_v230(value: Any, label: str = "source_provenance_v230") -> list[str]:
    """Return violations; an empty list means v230 evidence is valid."""
    ...
    count_fields = ("source_resolved_count", "package_resolved_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v230(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V230_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V230_VALID")
    return 0
