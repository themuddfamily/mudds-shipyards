#!/usr/bin/env python3
"""Validate version-24 source/hash identity reconciliation evidence."""
...
def validate_v24(value: Any, label: str = "identity_v24") -> list[str]:
    """Return violations; an empty list means identity reconciliation is valid."""
    ...
    if isinstance(reconciliation, dict) and reconciliation.get("status") == "PASS":
        for key in ("identity_id", "reconciliation_id"):
        ...
        if reconciliation.get("match") is not True:
            errors.append(f"{label}.reconciliation.match must be true when status is PASS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v24(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_IDENTITY_RECONCILIATION_V24_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_IDENTITY_RECONCILIATION_V24_VALID")
    return 0
