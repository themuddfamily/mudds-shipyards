#!/usr/bin/env python3
"""Validate version-295 package manifest-chain/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=295;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v295(v:Any,label:str="source_provenance_v295")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","manifest_chain_id","manifest_chain_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","manifest_chain_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","manifest_chain_id","manifest_chain_digest","manifest_chain_entry_count")
    ...
    m=v.get("manifest_chain");_status(m,f"{label}.manifest_chain",e)
    if isinstance(m,dict) and m.get("status")=="PASS":
        for k in ("manifest_chain_id","manifest_chain_digest","source_hash","package_artifact_hash_count","manifest_chain_entry_count"):
            if m.get(k)!=v.get(k):e.append(f"{label}.manifest_chain.{k} must match {k}")
        if m.get("complete") is not True:e.append(f"{label}.manifest_chain.complete must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v295(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V295_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V295_VALID");return 0
