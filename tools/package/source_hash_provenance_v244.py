#!/usr/bin/env python3
"""Validate version-244 package source-hash provenance evidence."""
...
def validate_v244(value: Any, label: str = "source_provenance_v244") -> list[str]:
    """Return violations; an empty list means v244 evidence is valid."""
    ...
    count_fields = ("source_accepted_count", "package_accepted_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v244(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V244_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V244_VALID")
    return 0
