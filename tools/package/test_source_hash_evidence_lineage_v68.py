...
from tools.package.source_hash_evidence_lineage_v68 import validate_v68
...
def record():
    ...
    digest = "f" * 64
    source_id = "source-68"
    evidence_id = "evidence-68"
    lineage_id = "lineage-68"
    source_version = "src-68"
    ...
    return {
        "schema_version": 68,
        "build_label": "evidence-lineage-v68",
        **common,
...
class SourceHashEvidenceLineageV68Test(unittest.TestCase):
    def test_accepts_captured_traced_record(self):
        self.assertEqual(validate_v68(record()), [])
    ...
    def test_rejects_schema_or_lineage_flags(self):
        ...
        errors = validate_v68(item)
        self.assertTrue(any("schema_version must be 68" in error for error in errors))
    ...
    def test_native_not_run_cannot_carry_platform(self):
        ...
        self.assertTrue(any("platform must be null" in error for error in validate_v68(item)))
