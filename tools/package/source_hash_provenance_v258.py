#!/usr/bin/env python3
"""Validate version-258 package source-hash provenance evidence."""
...
def validate_v258(value: Any, label: str = "source_provenance_v258") -> list[str]:
    """Return violations; an empty list means v258 evidence is valid."""
    ...
    count_fields = ("source_delivered_count", "package_delivered_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v258(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V258_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V258_VALID")
    return 0
