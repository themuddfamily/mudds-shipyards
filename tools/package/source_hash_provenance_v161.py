#!/usr/bin/env python3
"""Validate version-161 package source-hash provenance evidence."""
...
def validate_v161(value: Any, label: str = "source_provenance_v161") -> list[str]:
    """Return violations; an empty list means v161 evidence is valid."""
    ...
    count_fields = ("source_certificate_count", "package_certificate_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v161(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V161_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V161_VALID")
    return 0
