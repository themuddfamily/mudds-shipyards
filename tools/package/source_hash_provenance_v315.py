#!/usr/bin/env python3
"""Validate version-315 package provenance authorization/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=315;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v315(v:Any,label:str="source_provenance_v315")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","authorization_id","authorization_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","authorization_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","authorization_id","authorization_digest","authorization_entry_count")
    ...
    a=v.get("authorization");_status(a,f"{label}.authorization",e)
    if isinstance(a,dict) and a.get("status")=="PASS":
        for k in ("authorization_id","authorization_digest","source_hash","package_artifact_hash_count","authorization_entry_count"):
            if a.get(k)!=v.get(k):e.append(f"{label}.authorization.{k} must match {k}")
        if a.get("authorized") is not True:e.append(f"{label}.authorization.authorized must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v315(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V315_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V315_VALID");return 0
