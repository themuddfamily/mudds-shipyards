#!/usr/bin/env python3
"""Validate version-139 package source-hash provenance evidence."""
...
def validate_v139(value: Any, label: str = "source_provenance_v139") -> list[str]:
    """Return violations; an empty list means v139 evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    fields = ("build_label", "source_id", "provenance_id", "source_commit", "source_hash", "source_version", "package_version", "attestation_method", "package_distribution")
    for key in fields:
        if not _text(value.get(key)):
            errors.append(f"{label}.{key} is required")
    binding_fields = ("source_id", "source_commit", "source_hash", "source_version", "package_version", "attestation_method", "package_distribution")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v139(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V139_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V139_VALID")
    return 0
