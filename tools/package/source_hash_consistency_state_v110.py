#!/usr/bin/env python3
"""Validate version-110 source-hash consistency and state evidence."""
...
def validate_v110(value: Any, label: str = "consistency_state_v110") -> list[str]:
    """Return violations; an empty list means consistency state evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_id", "consistency_id", "state_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(state, dict) and state.get("status") == "PASS":
        for key in ("state_id", "consistency_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v110(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_CONSISTENCY_STATE_V110_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_CONSISTENCY_STATE_V110_VALID")
    return 0
