#!/usr/bin/env python3
"""Validate version-74 source-hash source and closure evidence."""
...
def validate_v74(value: Any, label: str = "source_closure_v74") -> list[str]:
    ...
    for key in ("build_label", "source_id", "closure_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v74(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_SOURCE_CLOSURE_V74_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_SOURCE_CLOSURE_V74_VALID")
    return 0
