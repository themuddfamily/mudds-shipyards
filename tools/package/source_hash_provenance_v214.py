#!/usr/bin/env python3
"""Validate version-214 package source-hash provenance evidence."""
...
def validate_v214(value: Any, label: str = "source_provenance_v214") -> list[str]:
    """Return violations; an empty list means v214 evidence is valid."""
    ...
    count_fields = ("source_assertion_count", "package_assertion_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v214(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V214_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V214_VALID")
    return 0
