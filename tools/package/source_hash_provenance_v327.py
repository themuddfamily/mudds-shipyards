#!/usr/bin/env python3
"""Validate package source/hash provenance and authorization evidence for schema 327."""
...
def validate_v327(value: Any, label: str = "source_provenance_v327") -> list[str]:
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    required = ("build_label", "source_id", "source_commit", "source_hash", "package_version",
    ...
    attestation_keys = ("authorization_attestation_id", "authorization_attestation_digest", "source_hash",
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v327(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V327_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V327_VALID")
    return 0
