#!/usr/bin/env python3
"""Validate version-246 package source-hash provenance evidence."""
...
def validate_v246(value: Any, label: str = "source_provenance_v246") -> list[str]:
    """Return violations; an empty list means v246 evidence is valid."""
    ...
    count_fields = ("source_denied_count", "package_denied_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v246(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V246_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V246_VALID")
    return 0
