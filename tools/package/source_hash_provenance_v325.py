#!/usr/bin/env python3
"""Validate version-325 package authorization attestation/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=325;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v325(v:Any,label:str="source_provenance_v325")->list[str]:
    ...
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","authorization_attestation_id","authorization_attestation_digest","authorization_attestation_entry_count")
    ...
    a=v.get("authorization_attestation");_status(a,f"{label}.authorization_attestation",e)
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v325(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V325_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V325_VALID");return 0
