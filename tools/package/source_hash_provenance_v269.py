#!/usr/bin/env python3
"""Validate version-269 package source-hash provenance evidence."""
...
def validate_v269(value: Any, label: str = "source_provenance_v269") -> list[str]:
    """Return violations; an empty list means v269 evidence is valid."""
    ...
    count_fields = ("source_correlated_count", "package_correlated_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v269(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V269_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V269_VALID")
    return 0
