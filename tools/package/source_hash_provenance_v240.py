#!/usr/bin/env python3
"""Validate version-240 package source-hash provenance evidence."""
...
def validate_v240(value: Any, label: str = "source_provenance_v240") -> list[str]:
    """Return violations; an empty list means v240 evidence is valid."""
    ...
    count_fields = ("source_approved_count", "package_approved_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v240(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V240_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V240_VALID")
    return 0
