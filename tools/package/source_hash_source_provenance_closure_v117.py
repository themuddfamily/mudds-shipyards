#!/usr/bin/env python3
"""Validate version-117 source/provenance closure evidence."""
...
def validate_v117(value: Any, label: str = "source_provenance_closure_v117") -> list[str]:
    """Return violations; an empty list means source closure evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_id", "provenance_id", "closure_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(provenance, dict) and provenance.get("status") == "PASS":
        for key in ("provenance_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(closure, dict) and closure.get("status") == "PASS":
        for key in ("closure_id", "provenance_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v117(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_SOURCE_PROVENANCE_CLOSURE_V117_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_SOURCE_PROVENANCE_CLOSURE_V117_VALID")
    return 0
