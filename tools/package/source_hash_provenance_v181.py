#!/usr/bin/env python3
"""Validate version-181 package source-hash provenance evidence."""
...
def validate_v181(value: Any, label: str = "source_provenance_v181") -> list[str]:
    """Return violations; an empty list means v181 evidence is valid."""
    ...
    count_fields = ("source_path_checksum_count", "package_path_checksum_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v181(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V181_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V181_VALID")
    return 0
