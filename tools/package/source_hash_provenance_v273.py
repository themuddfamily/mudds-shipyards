#!/usr/bin/env python3
"""Validate version-273 package source-hash provenance evidence."""
...
def validate_v273(value: Any, label: str = "source_provenance_v273") -> list[str]:
    """Return violations; an empty list means v273 evidence is valid."""
    ...
    count_fields = ("source_proof_count", "package_proof_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v273(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V273_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V273_VALID")
    return 0
