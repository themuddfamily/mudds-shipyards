#!/usr/bin/env python3
"""Validate version-115 source-hash attestation and lineage evidence."""
...
def validate_v115(value: Any, label: str = "attestation_lineage_v115") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v115(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_ATTESTATION_LINEAGE_V115_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_ATTESTATION_LINEAGE_V115_VALID")
    return 0
