#!/usr/bin/env python3
"""Validate version-200 package source-hash provenance evidence."""
...
def validate_v200(value: Any, label: str = "source_provenance_v200") -> list[str]:
    """Return violations; an empty list means v200 evidence is valid."""
    ...
    count_fields = ("source_event_count", "package_event_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v200(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V200_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V200_VALID")
    return 0
