#!/usr/bin/env python3
"""Validate version-227 package source-hash provenance evidence."""
...
def validate_v227(value: Any, label: str = "source_provenance_v227") -> list[str]:
    """Return violations; an empty list means v227 evidence is valid."""
    ...
    count_fields = ("source_imported_count", "package_imported_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v227(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V227_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V227_VALID")
    return 0
