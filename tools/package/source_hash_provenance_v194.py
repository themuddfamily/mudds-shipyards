#!/usr/bin/env python3
"""Validate version-194 package source-hash provenance evidence."""
...
def validate_v194(value: Any, label: str = "source_provenance_v194") -> list[str]:
    """Return violations; an empty list means v194 evidence is valid."""
    ...
    count_fields = ("source_contract_count", "package_contract_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v194(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V194_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V194_VALID")
    return 0
