#!/usr/bin/env python3
"""Validate version-101 source-hash evidence and closure evidence."""
...
def validate_v101(value: Any, label: str = "evidence_closure_v101") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v101(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_EVIDENCE_CLOSURE_V101_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_EVIDENCE_CLOSURE_V101_VALID")
    return 0
