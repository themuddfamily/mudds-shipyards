#!/usr/bin/env python3
"""Validate version-160 package source-hash provenance evidence."""
...
def validate_v160(value: Any, label: str = "source_provenance_v160") -> list[str]:
    """Return violations; an empty list means v160 evidence is valid."""
    ...
    count_fields = ("source_signature_count", "package_signature_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v160(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V160_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V160_VALID")
    return 0
