#!/usr/bin/env python3
"""Validate version-300 package provenance-graph/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=300;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v300(v:Any,label:str="source_provenance_v300")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","graph_id","graph_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","graph_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","graph_id","graph_digest","graph_entry_count")
    ...
    g=v.get("graph");_status(g,f"{label}.graph",e)
    if isinstance(g,dict) and g.get("status")=="PASS":
        for k in ("graph_id","graph_digest","source_hash","package_artifact_hash_count","graph_entry_count"):
            if g.get(k)!=v.get(k):e.append(f"{label}.graph.{k} must match {k}")
        if g.get("complete") is not True:e.append(f"{label}.graph.complete must be true when status is PASS")
    for n in ("native_execution","hardware_execution","human_review"):
        gate=v.get(n);_status(gate,f"{label}.{n}",e)
        if isinstance(gate,dict) and gate.get("status")=="NOT_RUN":
            for k in ("platform","hardware","reviewer","evidence_path"):
                if gate.get(k) is not None:e.append(f"{label}.{n}.{k} must be null when status is NOT_RUN")
    return e
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v300(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V300_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V300_VALID");return 0
