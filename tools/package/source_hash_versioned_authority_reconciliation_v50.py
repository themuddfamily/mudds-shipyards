#!/usr/bin/env python3
"""Validate version-50 source-hash authority/reconciliation release evidence."""
...
def validate_v50(value: Any, label: str = "authority_reconciliation_v50") -> list[str]:
    """Return violations; an empty list means release evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_commit", "source_hash", "source_version", "package_version", "release_channel", "authority_id"):
    ...
    if isinstance(authority, dict) and authority.get("status") == "PASS":
        for key in ("authority_id", "source_commit", "source_hash", "source_version", "package_version", "release_channel"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v50(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_VERSIONED_AUTHORITY_RECONCILIATION_V50_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_VERSIONED_AUTHORITY_RECONCILIATION_V50_VALID")
    return 0
