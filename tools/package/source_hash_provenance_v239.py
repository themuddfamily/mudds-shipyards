#!/usr/bin/env python3
"""Validate version-239 package source-hash provenance evidence."""
...
def validate_v239(value: Any, label: str = "source_provenance_v239") -> list[str]:
    """Return violations; an empty list means v239 evidence is valid."""
    ...
    count_fields = ("source_validated_count", "package_validated_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v239(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V239_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V239_VALID")
    return 0
