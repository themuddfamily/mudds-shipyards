#!/usr/bin/env python3
"""Validate package source/hash provenance and authorization evidence for schema 382."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=382;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v382(v:Any,label:str="source_provenance_v382")->list[str]:
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v382(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V382_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V382_VALID");return 0
