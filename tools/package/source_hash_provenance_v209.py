#!/usr/bin/env python3
"""Validate version-209 package source-hash provenance evidence."""
...
def validate_v209(value: Any, label: str = "source_provenance_v209") -> list[str]:
    """Return violations; an empty list means v209 evidence is valid."""
    ...
    count_fields = ("source_cancelled_count", "package_cancelled_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v209(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V209_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V209_VALID")
    return 0
