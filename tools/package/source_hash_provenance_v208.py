#!/usr/bin/env python3
"""Validate version-208 package source-hash provenance evidence."""
...
def validate_v208(value: Any, label: str = "source_provenance_v208") -> list[str]:
    """Return violations; an empty list means v208 evidence is valid."""
    ...
    count_fields = ("source_skipped_count", "package_skipped_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v208(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V208_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V208_VALID")
    return 0
