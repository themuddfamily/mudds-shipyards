#!/usr/bin/env python3
"""Validate version-262 package source-hash provenance evidence."""
...
def validate_v262(value: Any, label: str = "source_provenance_v262") -> list[str]:
    """Return violations; an empty list means v262 evidence is valid."""
    ...
    count_fields = ("source_published_count", "package_published_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v262(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V262_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V262_VALID")
    return 0
