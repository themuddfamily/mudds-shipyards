#!/usr/bin/env python3
"""Validate version-184 package source-hash provenance evidence."""
...
def validate_v184(value: Any, label: str = "source_provenance_v184") -> list[str]:
    """Return violations; an empty list means v184 evidence is valid."""
    ...
    count_fields = ("source_digest_algorithm_count", "package_digest_algorithm_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v184(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V184_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V184_VALID")
    return 0
