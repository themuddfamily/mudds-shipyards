...
from tools.package.source_hash_attestation_lineage_v95 import validate_v95
...
def record():
    ...
    digest = "c" * 64
    source_id = "source-95"
    attestation_id = "attestation-95"
    lineage_id = "lineage-95"
    source_version = "src-95"
    ...
    return {
        "schema_version": 95,
        "build_label": "attestation-lineage-v95",
        **common,
...
class SourceHashAttestationLineageV95Test(unittest.TestCase):
    def test_accepts_attested_traced_record(self):
        self.assertEqual(validate_v95(record()), [])

    def test_requires_attestation_and_lineage_hash_binding(self):
        item = record()
        item["attestation"]["source_hash"] = "d" * 64
        item["lineage"]["attestation_id"] = "attestation-other"
        errors = validate_v95(item)
    ...
    def test_rejects_schema_or_lineage_flags(self):
        ...
        errors = validate_v95(item)
        self.assertTrue(any("schema_version must be 95" in error for error in errors))
    ...
    def test_native_not_run_cannot_carry_platform(self):
        ...
        self.assertTrue(any("platform must be null" in error for error in validate_v95(item)))
