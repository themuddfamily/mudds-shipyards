#!/usr/bin/env python3
"""Validate version-122 source-hash provenance/closure evidence."""
...
def validate_v122(value: Any, label: str = "provenance_closure_v122") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v122(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_CLOSURE_V122_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_CLOSURE_V122_VALID")
    return 0
