#!/usr/bin/env python3
"""Validate package source/hash provenance and authorization evidence for schema 405."""
...
def validate_v405(value: Any, label: str = "source_provenance_v405") -> list[str]:
    if not isinstance(value, dict):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v405(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V405_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V405_VALID")
    return 0
