#!/usr/bin/env python3
"""Validate version-211 package source-hash provenance evidence."""
...
def validate_v211(value: Any, label: str = "source_provenance_v211") -> list[str]:
    """Return violations; an empty list means v211 evidence is valid."""
    ...
    count_fields = ("source_retry_count", "package_retry_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v211(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V211_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V211_VALID")
    return 0
