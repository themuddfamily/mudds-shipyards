#!/usr/bin/env python3
"""Validate version-158 package source-hash provenance evidence."""
...
def validate_v158(value: Any, label: str = "source_provenance_v158") -> list[str]:
    """Return violations; an empty list means v158 evidence is valid."""
    ...
    count_fields = ("source_reviewer_count", "package_reviewer_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v158(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V158_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V158_VALID")
    return 0
