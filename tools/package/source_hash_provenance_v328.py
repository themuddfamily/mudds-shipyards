#!/usr/bin/env python3
"""Validate package source/hash provenance and authorization evidence for schema 328."""
...
def validate_v328(value: Any, label: str = "source_provenance_v328") -> list[str]:
    if not isinstance(value, dict):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v328(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V328_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V328_VALID")
    return 0
