#!/usr/bin/env python3
"""Validate version-219 package source-hash provenance evidence."""
...
def validate_v219(value: Any, label: str = "source_provenance_v219") -> list[str]:
    """Return violations; an empty list means v219 evidence is valid."""
    ...
    count_fields = ("source_selected_count", "package_selected_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v219(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V219_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V219_VALID")
    return 0
