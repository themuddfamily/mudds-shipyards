#!/usr/bin/env python3
"""Validate version-260 package source-hash provenance evidence."""
...
def validate_v260(value: Any, label: str = "source_provenance_v260") -> list[str]:
    """Return violations; an empty list means v260 evidence is valid."""
    ...
    count_fields = ("source_deployed_count", "package_deployed_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v260(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V260_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V260_VALID")
    return 0
