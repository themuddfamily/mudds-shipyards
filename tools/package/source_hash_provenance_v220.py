#!/usr/bin/env python3
"""Validate version-220 package source-hash provenance evidence."""
...
def validate_v220(value: Any, label: str = "source_provenance_v220") -> list[str]:
    """Return violations; an empty list means v220 evidence is valid."""
    ...
    count_fields = ("source_filtered_count", "package_filtered_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v220(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V220_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V220_VALID")
    return 0
