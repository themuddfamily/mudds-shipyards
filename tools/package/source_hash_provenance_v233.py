#!/usr/bin/env python3
"""Validate version-233 package source-hash provenance evidence."""
...
def validate_v233(value: Any, label: str = "source_provenance_v233") -> list[str]:
    """Return violations; an empty list means v233 evidence is valid."""
    ...
    count_fields = ("source_archived_count", "package_archived_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v233(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V233_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V233_VALID")
    return 0
