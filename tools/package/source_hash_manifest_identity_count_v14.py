#!/usr/bin/env python3
"""Validate version-14 source/hash manifest identity and count evidence."""
...
def validate_v14(value: Any, label: str = "identity_v14") -> list[str]:
    """Return violations; an empty list means identity/count evidence is valid."""
    ...
    count = value.get("count_audit")
    _status(count, f"{label}.count_audit", errors)
    if isinstance(count, dict) and count.get("status") == "PASS":
        if count.get("manifest_id") != value.get("manifest_id"):
            errors.append(f"{label}.count_audit.manifest_id must match manifest_id")
        if count.get("declared_count") != len(entries):
            errors.append(f"{label}.count_audit.declared_count must equal entries count")
        if count.get("unique_entries") is not True:
            errors.append(f"{label}.count_audit.unique_entries must be true when status is PASS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v14(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_MANIFEST_IDENTITY_COUNT_V14_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_MANIFEST_IDENTITY_COUNT_V14_VALID")
    return 0
