#!/usr/bin/env python3
"""Validate version-203 package source-hash provenance evidence."""
...
def validate_v203(value: Any, label: str = "source_provenance_v203") -> list[str]:
    """Return violations; an empty list means v203 evidence is valid."""
    ...
    count_fields = ("source_failure_count", "package_failure_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v203(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V203_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V203_VALID")
    return 0
