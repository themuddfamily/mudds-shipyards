...
class WindowsDistributionAssemblerTest(unittest.TestCase):
    ...
    def test_assembles_payload_manifest_and_reproducible_zip(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("installer-operation.log", collector)
            ...
            self.assertIn("Write-OperationLog", powershell)
            self.assertIn("Select-Object -Last 8", powershell)
            self.assertIn("action=$safeAction version=$safeVersion result=$safeResult", powershell)
