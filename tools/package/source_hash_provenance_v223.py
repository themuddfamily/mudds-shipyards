#!/usr/bin/env python3
"""Validate version-223 package source-hash provenance evidence."""
...
def validate_v223(value: Any, label: str = "source_provenance_v223") -> list[str]:
    """Return violations; an empty list means v223 evidence is valid."""
    ...
    count_fields = ("source_complete_count", "package_complete_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v223(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V223_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V223_VALID")
    return 0
