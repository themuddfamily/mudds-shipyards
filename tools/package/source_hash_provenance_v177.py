#!/usr/bin/env python3
"""Validate version-177 package source-hash provenance evidence."""
...
def validate_v177(value: Any, label: str = "source_provenance_v177") -> list[str]:
    """Return violations; an empty list means v177 evidence is valid."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v177(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V177_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V177_VALID")
    return 0
