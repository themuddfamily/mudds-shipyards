#!/usr/bin/env python3
"""Validate version-179 package source-hash provenance evidence."""
...
def validate_v179(value: Any, label: str = "source_provenance_v179") -> list[str]:
    """Return violations; an empty list means v179 evidence is valid."""
    ...
    count_fields = ("source_item_count", "package_item_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v179(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V179_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V179_VALID")
    return 0
