...
from tools.package.source_hash_verification_state_v72 import validate_v72
...
def record():
    ...
    digest = "d" * 64
    source_id = "source-72"
    verification_id = "verification-72"
    state_id = "state-72"
    source_version = "src-72"
    ...
    return {
        "schema_version": 72,
        "build_label": "verification-state-v72",
        **common,
...
class SourceHashVerificationStateV72Test(unittest.TestCase):
    def test_accepts_verified_valid_state(self):
        self.assertEqual(validate_v72(record()), [])
    ...
    def test_rejects_schema_or_state_flags(self):
        ...
        errors = validate_v72(item)
        self.assertTrue(any("schema_version must be 72" in error for error in errors))
    ...
    def test_native_not_run_cannot_carry_platform(self):
        ...
        self.assertTrue(any("platform must be null" in error for error in validate_v72(item)))
