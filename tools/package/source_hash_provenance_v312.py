#!/usr/bin/env python3
"""Validate version-312 package attestation-receipt/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=312;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v312(v:Any,label:str="source_provenance_v312")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","receipt_id","receipt_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","receipt_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","receipt_id","receipt_digest","receipt_entry_count")
    ...
    r=v.get("receipt");_status(r,f"{label}.receipt",e)
    if isinstance(r,dict) and r.get("status")=="PASS":
        for k in ("receipt_id","receipt_digest","source_hash","package_artifact_hash_count","receipt_entry_count"):
            if r.get(k)!=v.get(k):e.append(f"{label}.receipt.{k} must match {k}")
        if r.get("accepted") is not True:e.append(f"{label}.receipt.accepted must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v312(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V312_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V312_VALID");return 0
