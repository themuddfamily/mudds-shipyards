#!/usr/bin/env python3
"""Validate version-173 package source-hash provenance evidence."""
...
def validate_v173(value: Any, label: str = "source_provenance_v173") -> list[str]:
    """Return violations; an empty list means v173 evidence is valid."""
    ...
    count_fields = ("source_line_count", "package_line_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v173(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V173_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V173_VALID")
    return 0
