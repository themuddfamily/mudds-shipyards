#!/usr/bin/env python3
"""Validate version-311 package attestation-checkpoint/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=311;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v311(v:Any,label:str="source_provenance_v311")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","checkpoint_id","checkpoint_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","checkpoint_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","checkpoint_id","checkpoint_digest","checkpoint_entry_count")
    ...
    c=v.get("checkpoint");_status(c,f"{label}.checkpoint",e)
    if isinstance(c,dict) and c.get("status")=="PASS":
        for k in ("checkpoint_id","checkpoint_digest","source_hash","package_artifact_hash_count","checkpoint_entry_count"):
            if c.get(k)!=v.get(k):e.append(f"{label}.checkpoint.{k} must match {k}")
        if c.get("complete") is not True:e.append(f"{label}.checkpoint.complete must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v311(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V311_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V311_VALID");return 0
