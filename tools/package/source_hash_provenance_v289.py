#!/usr/bin/env python3
"""Validate version-289 package release-channel/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=289;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v289(v:Any,label:str="source_provenance_v289")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","channel_id","channel_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","channel_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","channel_id","channel_digest","channel_entry_count")
    ...
    c=v.get("channel");_status(c,f"{label}.channel",e)
    if isinstance(c,dict) and c.get("status")=="PASS":
        for k in ("channel_id","channel_digest","source_hash","package_artifact_hash_count","channel_entry_count"):
            if c.get(k)!=v.get(k):e.append(f"{label}.channel.{k} must match {k}")
        if c.get("bound") is not True:e.append(f"{label}.channel.bound must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v289(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V289_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V289_VALID");return 0
