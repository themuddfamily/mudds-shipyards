#!/usr/bin/env python3
"""Validate version-237 package source-hash provenance evidence."""
...
def validate_v237(value: Any, label: str = "source_provenance_v237") -> list[str]:
    """Return violations; an empty list means v237 evidence is valid."""
    ...
    count_fields = ("source_checked_count", "package_checked_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v237(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V237_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V237_VALID")
    return 0
