#!/usr/bin/env python3
"""Validate version-157 package source-hash provenance evidence."""
...
def validate_v157(value: Any, label: str = "source_provenance_v157") -> list[str]:
    """Return violations; an empty list means v157 evidence is valid."""
    ...
    count_fields = ("source_license_count", "package_license_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v157(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V157_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V157_VALID")
    return 0
