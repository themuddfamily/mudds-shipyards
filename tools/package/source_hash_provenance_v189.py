#!/usr/bin/env python3
"""Validate version-189 package source-hash provenance evidence."""
...
def validate_v189(value: Any, label: str = "source_provenance_v189") -> list[str]:
    """Return violations; an empty list means v189 evidence is valid."""
    ...
    count_fields = ("source_check_count", "package_check_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v189(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V189_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V189_VALID")
    return 0
