#!/usr/bin/env python3
"""Validate version-153 package source-hash provenance evidence."""
...
def validate_v153(value: Any, label: str = "source_provenance_v153") -> list[str]:
    """Return violations; an empty list means v153 evidence is valid."""
    ...
    count_fields = ("source_artifact_count", "package_entry_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v153(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V153_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V153_VALID")
    return 0
