#!/usr/bin/env python3
"""Validate version-250 package source-hash provenance evidence."""
...
def validate_v250(value: Any, label: str = "source_provenance_v250") -> list[str]:
    """Return violations; an empty list means v250 evidence is valid."""
    ...
    count_fields = ("source_pending_count", "package_pending_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v250(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V250_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V250_VALID")
    return 0
