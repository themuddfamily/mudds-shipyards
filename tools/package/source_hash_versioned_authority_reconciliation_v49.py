#!/usr/bin/env python3
"""Validate version-49 versioned source-hash authority reconciliation evidence."""
...
def validate_v49(value: Any, label: str = "authority_reconciliation_v49") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v49(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_VERSIONED_AUTHORITY_RECONCILIATION_V49_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_VERSIONED_AUTHORITY_RECONCILIATION_V49_VALID")
    return 0
