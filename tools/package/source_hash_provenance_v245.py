#!/usr/bin/env python3
"""Validate version-245 package source-hash provenance evidence."""
...
def validate_v245(value: Any, label: str = "source_provenance_v245") -> list[str]:
    """Return violations; an empty list means v245 evidence is valid."""
    ...
    count_fields = ("source_rejected_count", "package_rejected_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v245(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V245_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V245_VALID")
    return 0
