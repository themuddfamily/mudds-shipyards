#!/usr/bin/env python3
"""Validate version-195 package source-hash provenance evidence."""
...
def validate_v195(value: Any, label: str = "source_provenance_v195") -> list[str]:
    """Return violations; an empty list means v195 evidence is valid."""
    ...
    count_fields = ("source_api_count", "package_api_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v195(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V195_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V195_VALID")
    return 0
