#!/usr/bin/env python3
"""Validate version-170 package source-hash provenance evidence."""
...
def validate_v170(value: Any, label: str = "source_provenance_v170") -> list[str]:
    """Return violations; an empty list means v170 evidence is valid."""
    ...
    count_fields = ("source_version_count", "package_version_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v170(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V170_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V170_VALID")
    return 0
