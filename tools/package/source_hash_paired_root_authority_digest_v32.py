#!/usr/bin/env python3
"""Validate version-32 source/hash paired root/authority digest evidence."""
...
def validate_v32(value: Any, label: str = "authority_v32") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v32(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PAIRED_ROOT_AUTHORITY_DIGEST_V32_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PAIRED_ROOT_AUTHORITY_DIGEST_V32_VALID")
    return 0
