...
from tools.package.source_hash_evidence_state_v94 import validate_v94
...
def record():
    ...
    digest = "a" * 64
    source_id = "source-94"
    evidence_id = "evidence-94"
    state_id = "state-94"
    source_version = "src-94"
    ...
    return {
        "schema_version": 94,
        "build_label": "evidence-state-v94",
        **common,
...
class SourceHashEvidenceStateV94Test(unittest.TestCase):
    def test_accepts_captured_valid_state(self):
        self.assertEqual(validate_v94(record()), [])
    ...
    def test_rejects_schema_or_state_flags(self):
        ...
        errors = validate_v94(item)
        self.assertTrue(any("schema_version must be 94" in error for error in errors))
    ...
    def test_native_not_run_cannot_carry_platform(self):
        ...
        self.assertTrue(any("platform must be null" in error for error in validate_v94(item)))
