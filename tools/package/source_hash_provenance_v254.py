#!/usr/bin/env python3
"""Validate version-254 package source-hash provenance evidence."""
...
def validate_v254(value: Any, label: str = "source_provenance_v254") -> list[str]:
    """Return violations; an empty list means v254 evidence is valid."""
    ...
    count_fields = ("source_attempted_count", "package_attempted_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v254(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V254_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V254_VALID")
    return 0
