#!/usr/bin/env python3
"""Validate version-216 package source-hash provenance evidence."""
...
def validate_v216(value: Any, label: str = "source_provenance_v216") -> list[str]:
    """Return violations; an empty list means v216 evidence is valid."""
    ...
    count_fields = ("source_suite_count", "package_suite_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v216(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V216_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V216_VALID")
    return 0
