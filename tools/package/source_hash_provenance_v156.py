#!/usr/bin/env python3
"""Validate version-156 package source-hash provenance evidence."""
...
def validate_v156(value: Any, label: str = "source_provenance_v156") -> list[str]:
    """Return violations; an empty list means v156 evidence is valid."""
    ...
    count_fields = ("source_dependency_count", "package_dependency_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v156(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V156_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V156_VALID")
    return 0
