#!/usr/bin/env python3
"""Validate version-197 package source-hash provenance evidence."""
...
def validate_v197(value: Any, label: str = "source_provenance_v197") -> list[str]:
    """Return violations; an empty list means v197 evidence is valid."""
    ...
    count_fields = ("source_route_count", "package_route_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v197(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V197_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V197_VALID")
    return 0
