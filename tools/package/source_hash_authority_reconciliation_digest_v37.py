#!/usr/bin/env python3
"""Validate version-37 source/hash authority/reconciliation digest evidence."""
...
def validate_v37(value: Any, label: str = "authority_v37") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v37(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_AUTHORITY_RECONCILIATION_DIGEST_V37_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_AUTHORITY_RECONCILIATION_DIGEST_V37_VALID")
    return 0
