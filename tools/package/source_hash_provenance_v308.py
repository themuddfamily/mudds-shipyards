#!/usr/bin/env python3
"""Validate version-308 package attestation-edge/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=308;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v308(v:Any,label:str="source_provenance_v308")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","attestation_edge_id","attestation_edge_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","attestation_edge_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","attestation_edge_id","attestation_edge_digest","attestation_edge_entry_count")
    ...
    a=v.get("attestation_edge");_status(a,f"{label}.attestation_edge",e)
    if isinstance(a,dict) and a.get("status")=="PASS":
        for k in ("attestation_edge_id","attestation_edge_digest","source_hash","package_artifact_hash_count","attestation_edge_entry_count"):
            if a.get(k)!=v.get(k):e.append(f"{label}.attestation_edge.{k} must match {k}")
        if a.get("bound") is not True:e.append(f"{label}.attestation_edge.bound must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v308(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V308_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V308_VALID");return 0
