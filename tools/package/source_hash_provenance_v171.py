#!/usr/bin/env python3
"""Validate version-171 package source-hash provenance evidence."""
...
def validate_v171(value: Any, label: str = "source_provenance_v171") -> list[str]:
    """Return violations; an empty list means v171 evidence is valid."""
    ...
    count_fields = ("source_commit_count", "package_commit_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v171(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V171_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V171_VALID")
    return 0
