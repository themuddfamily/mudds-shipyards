#!/usr/bin/env python3
"""Validate version-249 package source-hash provenance evidence."""
...
def validate_v249(value: Any, label: str = "source_provenance_v249") -> list[str]:
    """Return violations; an empty list means v249 evidence is valid."""
    ...
    count_fields = ("source_deferred_count", "package_deferred_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v249(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V249_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V249_VALID")
    return 0
