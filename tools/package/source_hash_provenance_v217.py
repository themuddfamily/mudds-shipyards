#!/usr/bin/env python3
"""Validate version-217 package source-hash provenance evidence."""
...
def validate_v217(value: Any, label: str = "source_provenance_v217") -> list[str]:
    """Return violations; an empty list means v217 evidence is valid."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v217(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V217_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V217_VALID")
    return 0
