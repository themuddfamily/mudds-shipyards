#!/usr/bin/env python3
"""Validate version-224 package source-hash provenance evidence."""
...
def validate_v224(value: Any, label: str = "source_provenance_v224") -> list[str]:
    """Return violations; an empty list means v224 evidence is valid."""
    ...
    count_fields = ("source_observed_count", "package_observed_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v224(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V224_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V224_VALID")
    return 0
