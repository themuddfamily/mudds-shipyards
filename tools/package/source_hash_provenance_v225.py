#!/usr/bin/env python3
"""Validate version-225 package source-hash provenance evidence."""
...
def validate_v225(value: Any, label: str = "source_provenance_v225") -> list[str]:
    """Return violations; an empty list means v225 evidence is valid."""
    ...
    count_fields = ("source_emitted_count", "package_emitted_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v225(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V225_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V225_VALID")
    return 0
