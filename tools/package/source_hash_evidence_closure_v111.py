#!/usr/bin/env python3
"""Validate version-111 source-hash evidence and closure evidence."""
...
def validate_v111(value: Any, label: str = "evidence_closure_v111") -> list[str]:
    """Return violations; an empty list means evidence closure is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_id", "evidence_id", "closure_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(closure, dict) and closure.get("status") == "PASS":
        for key in ("closure_id", "evidence_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v111(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_EVIDENCE_CLOSURE_V111_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_EVIDENCE_CLOSURE_V111_VALID")
    return 0
