#!/usr/bin/env python3
"""Validate version-241 package source-hash provenance evidence."""
...
def validate_v241(value: Any, label: str = "source_provenance_v241") -> list[str]:
    """Return violations; an empty list means v241 evidence is valid."""
    ...
    count_fields = ("source_signed_count", "package_signed_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v241(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V241_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V241_VALID")
    return 0
