#!/usr/bin/env python3
"""Validate version-248 package source-hash provenance evidence."""
...
def validate_v248(value: Any, label: str = "source_provenance_v248") -> list[str]:
    """Return violations; an empty list means v248 evidence is valid."""
    ...
    count_fields = ("source_suppressed_count", "package_suppressed_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v248(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V248_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V248_VALID")
    return 0
