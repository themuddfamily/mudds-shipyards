#!/usr/bin/env python3
"""Validate version-257 package source-hash provenance evidence."""
...
def validate_v257(value: Any, label: str = "source_provenance_v257") -> list[str]:
    """Return violations; an empty list means v257 evidence is valid."""
    ...
    count_fields = ("source_dispatched_count", "package_dispatched_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v257(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V257_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V257_VALID")
    return 0
