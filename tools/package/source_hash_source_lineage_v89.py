#!/usr/bin/env python3
"""Validate version-89 source-hash source and lineage evidence."""
...
def validate_v89(value: Any, label: str = "source_lineage_v89") -> list[str]:
    """Return violations; an empty list means source lineage evidence is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_id", "lineage_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(lineage, dict) and lineage.get("status") == "PASS":
        for key in ("lineage_id", "source_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v89(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_SOURCE_LINEAGE_V89_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_SOURCE_LINEAGE_V89_VALID")
    return 0
