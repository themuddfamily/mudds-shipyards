#!/usr/bin/env python3
"""Validate version-215 package source-hash provenance evidence."""
...
def validate_v215(value: Any, label: str = "source_provenance_v215") -> list[str]:
    """Return violations; an empty list means v215 evidence is valid."""
    ...
    count_fields = ("source_case_count", "package_case_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v215(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V215_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V215_VALID")
    return 0
