#!/usr/bin/env python3
"""Validate version-242 package source-hash provenance evidence."""
...
def validate_v242(value: Any, label: str = "source_provenance_v242") -> list[str]:
    """Return violations; an empty list means v242 evidence is valid."""
    ...
    count_fields = ("source_attested_count", "package_attested_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v242(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V242_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V242_VALID")
    return 0
