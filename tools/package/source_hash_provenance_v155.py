#!/usr/bin/env python3
"""Validate version-155 package source-hash provenance evidence."""
...
def validate_v155(value: Any, label: str = "source_provenance_v155") -> list[str]:
    """Return violations; an empty list means v155 evidence is valid."""
    ...
    count_fields = ("source_module_count", "package_module_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v155(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V155_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V155_VALID")
    return 0
