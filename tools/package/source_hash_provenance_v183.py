#!/usr/bin/env python3
"""Validate version-183 package source-hash provenance evidence."""
...
def validate_v183(value: Any, label: str = "source_provenance_v183") -> list[str]:
    """Return violations; an empty list means v183 evidence is valid."""
    ...
    count_fields = ("source_checksum_count", "package_checksum_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v183(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V183_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V183_VALID")
    return 0
