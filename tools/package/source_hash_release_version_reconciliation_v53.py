#!/usr/bin/env python3
"""Validate version-53 source-hash release/version reconciliation evidence."""
...
def validate_v53(value: Any, label: str = "release_reconciliation_v53") -> list[str]:
    ...
    for key in ("build_label", "release_id", "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(reconciliation, dict) and reconciliation.get("status") == "PASS":
        for key in ("release_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v53(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_RELEASE_VERSION_RECONCILIATION_V53_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_RELEASE_VERSION_RECONCILIATION_V53_VALID")
    return 0
