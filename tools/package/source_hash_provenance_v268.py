#!/usr/bin/env python3
"""Validate version-268 package source-hash provenance evidence."""
...
def validate_v268(value: Any, label: str = "source_provenance_v268") -> list[str]:
    """Return violations; an empty list means v268 evidence is valid."""
    ...
    count_fields = ("source_mapped_count", "package_mapped_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v268(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V268_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V268_VALID")
    return 0
