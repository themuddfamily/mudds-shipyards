#!/usr/bin/env python3
"""Validate version-297 package provenance-leaf/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=297;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v297(v:Any,label:str="source_provenance_v297")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","leaf_id","leaf_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","leaf_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","leaf_id","leaf_digest","leaf_entry_count")
    ...
    q=v.get("leaf");_status(q,f"{label}.leaf",e)
    if isinstance(q,dict) and q.get("status")=="PASS":
        for k in ("leaf_id","leaf_digest","source_hash","package_artifact_hash_count","leaf_entry_count"):
            if q.get(k)!=v.get(k):e.append(f"{label}.leaf.{k} must match {k}")
        if q.get("verified") is not True:e.append(f"{label}.leaf.verified must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v297(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V297_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V297_VALID");return 0
