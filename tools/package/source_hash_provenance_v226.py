#!/usr/bin/env python3
"""Validate version-226 package source-hash provenance evidence."""
...
def validate_v226(value: Any, label: str = "source_provenance_v226") -> list[str]:
    """Return violations; an empty list means v226 evidence is valid."""
    ...
    count_fields = ("source_exported_count", "package_exported_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v226(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V226_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V226_VALID")
    return 0
