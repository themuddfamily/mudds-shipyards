#!/usr/bin/env python3
"""Validate aggregate normalized source/hash evidence for a package manifest."""
...
def validate_aggregate(value: Any, label: str = "aggregate") -> list[str]:
    """Return violations; an empty list means the aggregate evidence is valid."""
    ...
    expected_count = value.get("aggregate_count")
    if not isinstance(expected_count, int) or expected_count != len(entries):
        errors.append(f"{label}.aggregate_count must equal entries count")
    audit = value.get("audit")
    _status(audit, f"{label}.audit", errors)
    if isinstance(audit, dict) and audit.get("status") == "PASS":
        for key in ("source_bound", "hashes_complete", "aggregate_matches"):
            if audit.get(key) is not True:
                errors.append(f"{label}.audit.{key} must be true when status is PASS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_aggregate(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("NORMALIZED_SOURCE_HASH_AGGREGATE_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("NORMALIZED_SOURCE_HASH_AGGREGATE_VALID")
    return 0
