#!/usr/bin/env python3
"""Validate version-51 source-hash release/version authority reconciliation evidence."""
...
def validate_v51(value: Any, label: str = "release_reconciliation_v51") -> list[str]:
    """Return violations; an empty list means release reconciliation is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "release_id", "source_commit", "source_hash", "source_version", "package_version", "authority_id"):
    ...
    if isinstance(authority, dict) and authority.get("status") == "PASS":
        for key in ("authority_id", "release_id", "source_commit", "source_hash", "source_version", "package_version"):
        ...
        if authority.get("authorized") is not True:
            errors.append(f"{label}.authority.authorized must be true when status is PASS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v51(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_RELEASE_VERSION_AUTHORITY_RECONCILIATION_V51_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_RELEASE_VERSION_AUTHORITY_RECONCILIATION_V51_VALID")
    return 0
