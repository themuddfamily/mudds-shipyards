#!/usr/bin/env python3
"""Validate version-294 package evidence-chain/source binding evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=294;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v294(v:Any,label:str="source_provenance_v294")->list[str]:
    ...
    for k in ("build_label","source_id","source_commit","source_hash","package_version","chain_id","chain_digest"):
        if not _text(v.get(k)):e.append(f"{label}.{k} is required")
    for k in ("source_artifact_hash_count","package_artifact_hash_count","chain_entry_count"):
        if not _count(v.get(k)):e.append(f"{label}.{k} must be a non-negative integer")
    b=("source_id","source_commit","source_hash","package_version","source_artifact_hash_count","package_artifact_hash_count","chain_id","chain_digest","chain_entry_count")
    ...
    c=v.get("chain");_status(c,f"{label}.chain",e)
    if isinstance(c,dict) and c.get("status")=="PASS":
        for k in ("chain_id","chain_digest","source_hash","package_artifact_hash_count","chain_entry_count"):
            if c.get(k)!=v.get(k):e.append(f"{label}.chain.{k} must match {k}")
        if c.get("complete") is not True:e.append(f"{label}.chain.complete must be true when status is PASS")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v294(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V294_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V294_VALID");return 0
