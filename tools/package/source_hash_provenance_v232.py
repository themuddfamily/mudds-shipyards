#!/usr/bin/env python3
"""Validate version-232 package source-hash provenance evidence."""
...
def validate_v232(value: Any, label: str = "source_provenance_v232") -> list[str]:
    """Return violations; an empty list means v232 evidence is valid."""
    ...
    count_fields = ("source_closed_count", "package_closed_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v232(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V232_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V232_VALID")
    return 0
