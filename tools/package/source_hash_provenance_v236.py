#!/usr/bin/env python3
"""Validate version-236 package source-hash provenance evidence."""
...
def validate_v236(value: Any, label: str = "source_provenance_v236") -> list[str]:
    """Return violations; an empty list means v236 evidence is valid."""
    ...
    count_fields = ("source_maintained_count", "package_maintained_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v236(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V236_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V236_VALID")
    return 0
