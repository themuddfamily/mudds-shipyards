#!/usr/bin/env python3
"""Validate version-207 package source-hash provenance evidence."""
...
def validate_v207(value: Any, label: str = "source_provenance_v207") -> list[str]:
    """Return violations; an empty list means v207 evidence is valid."""
    ...
    count_fields = ("source_not_run_count", "package_not_run_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v207(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V207_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V207_VALID")
    return 0
