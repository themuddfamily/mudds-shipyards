#!/usr/bin/env python3
"""Validate version-45 dual-version source/hash authority provenance links."""
...
def validate_v45(value: Any, label: str = "dual_link_v45") -> list[str]:
    """Return violations; an empty list means dual-version link evidence is valid."""
    ...
    if isinstance(link, dict) and link.get("status") == "PASS":
        ...
        for key in ("source_commit", "source_version", "package_version"):
            if link.get(key) != value.get(key):
                errors.append(f"{label}.link.{key} must match {key}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v45(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_DUAL_VERSION_AUTHORITY_PROVENANCE_LINK_V45_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_DUAL_VERSION_AUTHORITY_PROVENANCE_LINK_V45_VALID")
    return 0
