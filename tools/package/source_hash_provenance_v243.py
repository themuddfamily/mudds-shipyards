#!/usr/bin/env python3
"""Validate version-243 package source-hash provenance evidence."""
...
def validate_v243(value: Any, label: str = "source_provenance_v243") -> list[str]:
    """Return violations; an empty list means v243 evidence is valid."""
    ...
    count_fields = ("source_certified_count", "package_certified_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v243(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V243_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V243_VALID")
    return 0
