#!/usr/bin/env python3
"""Validate version-213 package source-hash provenance evidence."""
...
def validate_v213(value: Any, label: str = "source_provenance_v213") -> list[str]:
    """Return violations; an empty list means v213 evidence is valid."""
    ...
    count_fields = ("source_error_count", "package_error_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v213(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V213_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V213_VALID")
    return 0
