#!/usr/bin/env python3
"""Validate normalized manifest source/hash counts from recorded metadata."""
...
def validate_rollup(value: Any, label: str = "rollup") -> list[str]:
    """Return violations; an empty list means the source/hash count is valid."""
    ...
    for key in ("declared_count", "normalized_count", "source_bound_count", "hashed_count"):
        if not isinstance(value.get(key), int) or value[key] < 0:
            errors.append(f"{label}.{key} must be a non-negative integer")
    for key in ("declared_count", "normalized_count", "source_bound_count", "hashed_count"):
        if isinstance(value.get(key), int) and value[key] != len(entries):
            errors.append(f"{label}.{key} must equal entries count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("MANIFEST_NORMALIZED_SOURCE_HASH_COUNT_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("MANIFEST_NORMALIZED_SOURCE_HASH_COUNT_VALID")
    return 0
