#!/usr/bin/env python3
"""Validate version-124 source-hash/provenance evidence."""
...
def validate_v124(value: Any, label: str = "source_provenance_v124") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v124(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V124_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V124_VALID")
    return 0
