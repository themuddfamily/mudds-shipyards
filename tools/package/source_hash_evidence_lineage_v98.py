#!/usr/bin/env python3
"""Validate version-98 source-hash evidence and lineage evidence."""
...
def validate_v98(value: Any, label: str = "evidence_lineage_v98") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v98(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_EVIDENCE_LINEAGE_V98_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_EVIDENCE_LINEAGE_V98_VALID")
    return 0
