#!/usr/bin/env python3
"""Validate version-172 package source-hash provenance evidence."""
...
def validate_v172(value: Any, label: str = "source_provenance_v172") -> list[str]:
    """Return violations; an empty list means v172 evidence is valid."""
    ...
    count_fields = ("source_patch_count", "package_patch_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v172(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V172_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V172_VALID")
    return 0
