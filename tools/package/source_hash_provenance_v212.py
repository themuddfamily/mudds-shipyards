#!/usr/bin/env python3
"""Validate version-212 package source-hash provenance evidence."""
...
def validate_v212(value: Any, label: str = "source_provenance_v212") -> list[str]:
    """Return violations; an empty list means v212 evidence is valid."""
    ...
    count_fields = ("source_timeout_count", "package_timeout_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v212(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V212_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V212_VALID")
    return 0
