#!/usr/bin/env python3
"""Validate version-210 package source-hash provenance evidence."""
...
def validate_v210(value: Any, label: str = "source_provenance_v210") -> list[str]:
    """Return violations; an empty list means v210 evidence is valid."""
    ...
    count_fields = ("source_warning_count", "package_warning_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v210(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V210_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V210_VALID")
    return 0
