#!/usr/bin/env python3
"""Validate version-86 source-hash audit and closure evidence."""
...
def validate_v86(value: Any, label: str = "audit_closure_v86") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v86(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_AUDIT_CLOSURE_V86_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_AUDIT_CLOSURE_V86_VALID")
    return 0
