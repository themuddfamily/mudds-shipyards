#!/usr/bin/env python3
"""Validate version-60 source-hash attestation/reconciliation evidence."""
...
def validate_v60(value: Any, label: str = "source_reconciliation_v60") -> list[str]:
    """Return violations; an empty list means source evidence is valid."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v60(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_ATTESTATION_SOURCE_RECONCILIATION_V60_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_ATTESTATION_SOURCE_RECONCILIATION_V60_VALID")
    return 0
