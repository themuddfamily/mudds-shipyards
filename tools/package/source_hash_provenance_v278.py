#!/usr/bin/env python3
"""Validate version-278 package source-hash provenance evidence."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from typing import Any
SCHEMA_VERSION=278;STATES={"PASS","FAIL","NOT_RUN","UNKNOWN"}
...
def validate_v278(v:Any,label:str="source_provenance_v278")->list[str]:
    ...
    cf=("source_metadata_attestation_count","package_metadata_attestation_count")
    for k in cf:
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("record",type=Path);a=p.parse_args(argv);e=validate_v278(json.loads(a.record.read_text(encoding="utf-8")))
    if e:print("SOURCE_HASH_PROVENANCE_V278_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("SOURCE_HASH_PROVENANCE_V278_VALID");return 0
