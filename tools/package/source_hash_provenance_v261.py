#!/usr/bin/env python3
"""Validate version-261 package source-hash provenance evidence."""
...
def validate_v261(value: Any, label: str = "source_provenance_v261") -> list[str]:
    """Return violations; an empty list means v261 evidence is valid."""
    ...
    count_fields = ("source_released_count", "package_released_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v261(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V261_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V261_VALID")
    return 0
