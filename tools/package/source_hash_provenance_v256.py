#!/usr/bin/env python3
"""Validate version-256 package source-hash provenance evidence."""
...
def validate_v256(value: Any, label: str = "source_provenance_v256") -> list[str]:
    """Return violations; an empty list means v256 evidence is valid."""
    ...
    count_fields = ("source_handled_count", "package_handled_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v256(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V256_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V256_VALID")
    return 0
