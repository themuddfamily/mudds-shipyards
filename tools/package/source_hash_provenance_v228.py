#!/usr/bin/env python3
"""Validate version-228 package source-hash provenance evidence."""
...
def validate_v228(value: Any, label: str = "source_provenance_v228") -> list[str]:
    """Return violations; an empty list means v228 evidence is valid."""
    ...
    count_fields = ("source_indexed_count", "package_indexed_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v228(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V228_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V228_VALID")
    return 0
