#!/usr/bin/env python3
"""Validate version-218 package source-hash provenance evidence."""
...
def validate_v218(value: Any, label: str = "source_provenance_v218") -> list[str]:
    """Return violations; an empty list means v218 evidence is valid."""
    ...
    count_fields = ("source_run_count", "package_run_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v218(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V218_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V218_VALID")
    return 0
