#!/usr/bin/env python3
"""Validate version-47 dual-version source-hash reconciliation evidence."""
...
def validate_v47(value: Any, label: str = "reconciliation_v47") -> list[str]:
    """Return violations; an empty list means source-hash reconciliation is valid."""
    if not isinstance(value, dict):
        return [f"{label} must be an object"]
    errors: list[str] = []
    if value.get("schema_version") != SCHEMA_VERSION:
        errors.append(f"{label}.schema_version must be {SCHEMA_VERSION}")
    for key in ("build_label", "source_commit", "source_hash", "source_version", "package_version", "authority_id", "provenance_id"):
    ...
    for name, identity in (("authority", "authority_id"), ("provenance", "provenance_id")):
        ...
        if isinstance(record, dict) and record.get("status") == "PASS":
            for key in (identity, "source_commit", "source_hash", "source_version", "package_version"):
    ...
    if isinstance(reconciliation, dict) and reconciliation.get("status") == "PASS":
        for key in ("authority_id", "provenance_id", "source_commit", "source_hash", "source_version", "package_version"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v47(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_DUAL_VERSION_RECONCILIATION_V47_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_DUAL_VERSION_RECONCILIATION_V47_VALID")
    return 0
