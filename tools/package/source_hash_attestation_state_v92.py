#!/usr/bin/env python3
"""Validate version-92 source-hash attestation and state evidence."""
...
def validate_v92(value: Any, label: str = "attestation_state_v92") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v92(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_ATTESTATION_STATE_V92_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_ATTESTATION_STATE_V92_VALID")
    return 0
