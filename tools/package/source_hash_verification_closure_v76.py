#!/usr/bin/env python3
"""Validate version-76 source-hash verification and closure evidence."""
...
def validate_v76(value: Any, label: str = "verification_closure_v76") -> list[str]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v76(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_VERIFICATION_CLOSURE_V76_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_VERIFICATION_CLOSURE_V76_VALID")
    return 0
