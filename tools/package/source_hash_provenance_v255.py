#!/usr/bin/env python3
"""Validate version-255 package source-hash provenance evidence."""
...
def validate_v255(value: Any, label: str = "source_provenance_v255") -> list[str]:
    """Return violations; an empty list means v255 evidence is valid."""
    ...
    count_fields = ("source_processed_count", "package_processed_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v255(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V255_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V255_VALID")
    return 0
