#!/usr/bin/env python3
"""Validate version-175 package source-hash provenance evidence."""
...
def validate_v175(value: Any, label: str = "source_provenance_v175") -> list[str]:
    """Return violations; an empty list means v175 evidence is valid."""
    ...
    count_fields = ("source_hash_count", "package_hash_count")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v175(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_PROVENANCE_V175_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_PROVENANCE_V175_VALID")
    return 0
