#!/usr/bin/env python3
"""Validate version-41 source/hash linked authority/provenance evidence."""
...
def validate_v41(value: Any, label: str = "linked_v41") -> list[str]:
    ...
    for key in ("build_label", "source_commit", "authority_id", "provenance_id", "link_id"):
    ...
    if isinstance(link, dict) and link.get("status") == "PASS":
        for key in ("link_id", "authority_id", "provenance_id"):
            if link.get(key) != value.get(key):
                errors.append(f"{label}.link.{key} must match {key}")
        if link.get("source_commit") != value.get("source_commit"):
            errors.append(f"{label}.link.source_commit must match source_commit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v41(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_LINKED_AUTHORITY_PROVENANCE_V41_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_LINKED_AUTHORITY_PROVENANCE_V41_VALID")
    return 0
