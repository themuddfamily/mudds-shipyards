#!/usr/bin/env python3
"""Validate version-5 source/hash summary digest audit evidence."""
...
def validate_v5(value: Any, label: str = "audit_v5") -> list[str]:
    """Return violations; an empty list means the v5 audit is valid."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate_v5(json.loads(args.record.read_text(encoding="utf-8")))
    if errors:
        print("SOURCE_HASH_AUDIT_SUMMARY_DIGEST_V5_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("SOURCE_HASH_AUDIT_SUMMARY_DIGEST_V5_VALID")
    return 0
