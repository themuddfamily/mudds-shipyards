#!/usr/bin/env python3
"""Validate v54 source-bound release/version reconciliation summaries."""
...
SCHEMA = "audio_cleanup_release_version_reconciliation_source_summary_v54"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "release_id", "release_version", "reconciliation_id", "reconciliation_source"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_RELEASE_VERSION_RECONCILIATION_SOURCE_ONLY":
        errors.append("claim must be AUTOMATED_RELEASE_VERSION_RECONCILIATION_SOURCE_ONLY")
    ...
    for index, record in enumerate(records):
        ...
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("release_id", "release_version", "reconciliation_id", "reconciliation_source", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records source-bound release/reconciliation pairs must agree")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_RELEASE_VERSION_RECONCILIATION_SOURCE_V54_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_RELEASE_VERSION_RECONCILIATION_SOURCE_V54_VALID")
    return 0
