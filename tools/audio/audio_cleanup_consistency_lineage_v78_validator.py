#!/usr/bin/env python3
"""Validate v78 consistency/lineage summaries."""
...
SCHEMA = "audio_cleanup_consistency_lineage_v78"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "consistency_id", "lineage_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_CONSISTENCY_LINEAGE_ONLY":
        errors.append("claim must be AUTOMATED_CONSISTENCY_LINEAGE_ONLY")
    ...
    for key in ("consistency_digest", "lineage_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("consistency_digest"), record.get("lineage_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("consistency_id", "lineage_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records consistency/lineage digest pairs must agree")
    if summary.get("consistency_lineage_pass") is not True:
        errors.append("consistency_lineage_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_CONSISTENCY_LINEAGE_V78_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_CONSISTENCY_LINEAGE_V78_VALID")
    return 0
