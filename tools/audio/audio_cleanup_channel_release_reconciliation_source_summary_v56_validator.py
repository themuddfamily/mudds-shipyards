#!/usr/bin/env python3
"""Validate v56 source-bound channel/release reconciliation summaries."""
...
SCHEMA = "audio_cleanup_channel_release_reconciliation_source_summary_v56"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "channel_id", "channel_source", "release_id", "reconciliation_id", "release_version"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_CHANNEL_RELEASE_RECONCILIATION_SOURCE_ONLY":
        errors.append("claim must be AUTOMATED_CHANNEL_RELEASE_RECONCILIATION_SOURCE_ONLY")
    ...
    for index, record in enumerate(records):
        ...
        for key in ("channel_id", "channel_source", "release_id", "reconciliation_id", "release_version", "canonicalization"):
    ...
    if len(triples) > 1:
        errors.append("records source-bound channel/release/reconciliation triples must agree")
    if summary.get("source_channel_reconciliation_pass") is not True:
        errors.append("source_channel_reconciliation_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_CHANNEL_RELEASE_RECONCILIATION_SOURCE_V56_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_CHANNEL_RELEASE_RECONCILIATION_SOURCE_V56_VALID")
    return 0
