#!/usr/bin/env python3
"""Validate v37 authority/reconciliation digest summaries."""
...
SCHEMA = "audio_cleanup_authority_reconciliation_digest_summary_v37"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "authority_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_AUTHORITY_RECONCILIATION_DIGEST_ONLY":
        errors.append("claim must be AUTOMATED_AUTHORITY_RECONCILIATION_DIGEST_ONLY")
    ...
    if len(pairs) > 1:
        errors.append("records authority/reconciliation digest pairs must agree")
    if summary.get("authority_reconciliation_pass") is not True:
        errors.append("authority_reconciliation_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_AUTHORITY_RECONCILIATION_DIGEST_V37_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_AUTHORITY_RECONCILIATION_DIGEST_V37_VALID")
    return 0
