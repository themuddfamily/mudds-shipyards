#!/usr/bin/env python3
"""Validate landing/interior ambience transition evidence without audition."""
...
SCHEMA = "audio_ambience_landing_interior_transition_v1"
REQUIRED_PATHS = {("exterior", "landing_approach"), ("landing_approach", "interior"), ("interior", "landing_approach"), ("landing_approach", "exterior")}
STATES = {"exterior", "landing_approach", "interior"}
...
def validate_ledger(ledger: Any) -> list[str]:
    ...
    if ledger.get("claim") != "AUTOMATED_LANDING_INTERIOR_ONLY":
        errors.append("claim must be AUTOMATED_LANDING_INTERIOR_ONLY")
    ...
    for index, state in enumerate(states):
        ...
        if state.get("voice_authority") != "presentation_only":
            errors.append(f"{prefix}.voice_authority must be presentation_only")
    ...
    if REQUIRED_PATHS - seen_paths:
        errors.append("transitions missing required landing/interior paths")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_AMBIENCE_LANDING_INTERIOR_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_AMBIENCE_LANDING_INTERIOR_VALID")
    return 0
