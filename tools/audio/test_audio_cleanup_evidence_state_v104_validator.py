"""Focused tests for v104 audio cleanup evidence/state summaries."""
...
import audio_cleanup_evidence_state_v104_validator as validator  # noqa: E402
...
def summary() -> dict:
    ...
    def record(record_id: str, artifact: str) -> dict:
        return {"record_id": record_id, "evidence_digest": evidence,
                "state_digest": state_digest, "evidence_id": "evidence-v104",
                "state_model": "cleanup-state-v1", "state": "closed",
                "evidence": artifact, "state_pass": True}

    return {"schema": "audio_cleanup_evidence_state_v104", "revision": "a" * 40,
            "owner": "audio-evidence-owner", "summary_id": "cleanup-evidence-state-v104",
            "evidence_bundle": "artifacts/audio/evidence-state-v104.json",
            "evidence_id": "evidence-v104", "state_model": "cleanup-state-v1",
            "state": "closed", "claim": "AUTOMATED_EVIDENCE_STATE_ONLY",
            "boundary_note": "Evidence state does not establish native audibility.",
            "record_ids": ["record-a", "record-b"], "evidence_digest": evidence,
            "state_digest": state_digest, "records": [record("record-a", "artifacts/audio/a.json"),
                                                        record("record-b", "artifacts/audio/b.json")],
            "evidence_state_pass": True}


class AudioCleanupEvidenceStateV104Tests(unittest.TestCase):
