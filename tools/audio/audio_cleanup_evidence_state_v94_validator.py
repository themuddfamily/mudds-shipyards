#!/usr/bin/env python3
"""Validate v94 evidence/state summaries."""
...
SCHEMA = "audio_cleanup_evidence_state_v94"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "evidence_id", "state_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_EVIDENCE_STATE_ONLY":
        errors.append("claim must be AUTOMATED_EVIDENCE_STATE_ONLY")
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("evidence_digest"), record.get("state_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("evidence_id", "state_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records evidence/state digest pairs must agree")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_EVIDENCE_STATE_V94_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_EVIDENCE_STATE_V94_VALID")
    return 0
