#!/usr/bin/env python3
"""Validate v42 linked authority/provenance/version summaries."""
...
SCHEMA = "audio_cleanup_linked_authority_provenance_version_summary_v42"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "authority_id", "provenance_id", "provenance_version"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_LINKED_AUTHORITY_PROVENANCE_VERSION_ONLY":
        errors.append("claim must be AUTOMATED_LINKED_AUTHORITY_PROVENANCE_VERSION_ONLY")
    ...
    for index, record in enumerate(records):
        ...
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("authority_id", "provenance_id", "provenance_version", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records authority/provenance digest pairs must agree")
    if summary.get("authority_provenance_version_pass") is not True:
        errors.append("authority_provenance_version_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_LINKED_AUTHORITY_PROVENANCE_VERSION_V42_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_LINKED_AUTHORITY_PROVENANCE_VERSION_V42_VALID")
    return 0
