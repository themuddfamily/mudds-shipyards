#!/usr/bin/env python3
"""Validate v102 audio cleanup attestation/state summaries."""
...
SCHEMA = "audio_cleanup_attestation_state_v102"
CLAIM = "AUTOMATED_ATTESTATION_STATE_ONLY"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "attestation_bundle", "attestation_id", "state_model"):
    ...
    for index, record in enumerate(records):
        ...
        for key in ("attestation_id", "state_model", "state"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_ATTESTATION_STATE_V102_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_ATTESTATION_STATE_V102_VALID")
    return 0
