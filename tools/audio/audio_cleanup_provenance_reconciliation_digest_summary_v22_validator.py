#!/usr/bin/env python3
"""Validate v22 cleanup provenance reconciliation digest summaries."""
...
SCHEMA = "audio_cleanup_provenance_reconciliation_digest_summary_v22"
SHA256_RE = re.compile(r"^[0-9a-f]{64}$")
PROVENANCE_STATUSES = {"project_original", "licensed", "permission_recorded"}
...
def validate_summary(summary: Any) -> list[str]:
    ...
    if summary.get("claim") != "AUTOMATED_PROVENANCE_RECONCILIATION_ONLY":
        errors.append("claim must be AUTOMATED_PROVENANCE_RECONCILIATION_ONLY")
    ...
    agreed = summary.get("provenance_digest")
    if not _digest(agreed):
        errors.append("provenance_digest must be a lowercase 64-character digest")
    ...
    for index, record in enumerate(records):
        ...
        if record.get("provenance_status") not in PROVENANCE_STATUSES:
            errors.append(f"{prefix}.provenance_status is invalid")
    ...
    if digests and agreed not in digests:
        errors.append("provenance_digest must match records")
    if summary.get("provenance_pass") is not True:
        errors.append("provenance_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_PROVENANCE_V22_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_PROVENANCE_V22_VALID")
    return 0
