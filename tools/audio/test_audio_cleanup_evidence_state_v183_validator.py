"""Focused tests for v183 audio cleanup evidence/state summaries."""

import copy, sys, unittest
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent))
import audio_cleanup_evidence_state_v183_validator as validator  # noqa: E402
...
def summary() -> dict:
    ...
    def record(record_id: str, artifact: str) -> dict:
        return {"record_id": record_id, "evidence_digest": evidence, "state_digest": state_digest,
                "evidence_id": "evidence-v183", "state_model": "cleanup-state-v1", "state": "closed",
                "evidence": artifact, "state_pass": True}
    return {"schema": "audio_cleanup_evidence_state_v183", "revision": "a" * 40,
            "owner": "audio-evidence-owner", "summary_id": "cleanup-evidence-state-v183",
            "evidence_bundle": "artifacts/audio/evidence-state-v183.json", "evidence_id": "evidence-v183",
            ...
            "evidence_digest": evidence, "state_digest": state_digest, "record_ids": ["record-a", "record-b"],
            "records": [record("record-a", "artifacts/audio/a.json"), record("record-b", "artifacts/audio/b.json")],
            "evidence_state_pass": True}

class AudioCleanupEvidenceStateV183Tests(unittest.TestCase):
