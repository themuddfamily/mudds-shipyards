#!/usr/bin/env python3
"""Validate v111 audio cleanup evidence/closure summaries."""
...
SCHEMA = "audio_cleanup_evidence_closure_v111"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "evidence_id", "closure_id"):
    ...
    for index, record in enumerate(records):
        ...
        for key in ("evidence_id", "closure_id"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_EVIDENCE_CLOSURE_V111_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_EVIDENCE_CLOSURE_V111_VALID")
    return 0
