#!/usr/bin/env python3
"""Validate v189 audio cleanup evidence/state summaries."""
from __future__ import annotations
import argparse,json,re
from pathlib import Path
from typing import Any
SCHEMA="audio_cleanup_evidence_state_v189";CLAIM="AUTOMATED_EVIDENCE_STATE_ONLY";NOT_RUN="NOT_RUN";STATES={"open","ready","closed"};SHA256=re.compile(r"^[0-9a-f]{64}$")
def _text(v:Any)->bool:return isinstance(v,str) and bool(v.strip())
def _digest(v:Any)->bool:return isinstance(v,str) and bool(SHA256.fullmatch(v))
def _roster(v:Any)->bool:return isinstance(v,list) and bool(v) and all(_text(x) for x in v) and len(v)==len(set(v)) and v==sorted(v)
def validate_summary(s:Any)->list[str]:
    ...
    for k in ("native_status","stale_callback_status"):
        if s.get(k)!=NOT_RUN:e.append(f"{k} must be NOT_RUN")
    if not _text(s.get("boundary_note")):e.append("boundary_note is required")
    if s.get("state") not in STATES:e.append("state must be open, ready, or closed")
    ids=s.get("record_ids")
    if not _roster(ids):e.append("record_ids must be ordered, unique, and non-empty")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("summary",type=Path);e=validate_summary(json.loads(p.parse_args(argv).summary.read_text(encoding="utf-8")))
    if e:print("AUDIO_CLEANUP_EVIDENCE_STATE_V189_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("AUDIO_CLEANUP_EVIDENCE_STATE_V189_VALID");return 0
