#!/usr/bin/env python3
"""Validate v86 audit/closure summaries."""
...
SCHEMA = "audio_cleanup_audit_closure_v86"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "audit_id", "closure_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_AUDIT_CLOSURE_ONLY":
        errors.append("claim must be AUTOMATED_AUDIT_CLOSURE_ONLY")
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("audit_digest"), record.get("closure_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("audit_id", "closure_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records audit/closure digest pairs must agree")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_AUDIT_CLOSURE_V86_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_AUDIT_CLOSURE_V86_VALID")
    return 0
