#!/usr/bin/env python3
"""Validate v68 evidence/lineage summaries."""
...
SCHEMA = "audio_cleanup_evidence_lineage_v68"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "evidence_id", "lineage_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_EVIDENCE_LINEAGE_ONLY":
        errors.append("claim must be AUTOMATED_EVIDENCE_LINEAGE_ONLY")
    ...
    for key in ("evidence_digest", "lineage_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("evidence_digest"), record.get("lineage_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("evidence_id", "lineage_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records evidence/lineage digest pairs must agree")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_EVIDENCE_LINEAGE_V68_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_EVIDENCE_LINEAGE_V68_VALID")
    return 0
