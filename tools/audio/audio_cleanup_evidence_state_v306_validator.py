#!/usr/bin/env python3
"""Validate v306 audio cleanup evidence/state summaries without runtime claims."""
...
SCHEMA = "audio_cleanup_evidence_state_v306"
...
def _ordered_ids(value: Any) -> bool:
...
def validate_summary(summary: Any) -> list[str]:
    ...
    if not _ordered_ids(record_ids):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_EVIDENCE_STATE_V306_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_EVIDENCE_STATE_V306_VALID")
    return 0
