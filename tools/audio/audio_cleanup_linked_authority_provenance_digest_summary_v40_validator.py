#!/usr/bin/env python3
"""Validate v40 linked authority/provenance digest summaries."""
...
SCHEMA = "audio_cleanup_linked_authority_provenance_digest_summary_v40"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "authority_id", "provenance_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_LINKED_AUTHORITY_PROVENANCE_DIGEST_ONLY":
        errors.append("claim must be AUTOMATED_LINKED_AUTHORITY_PROVENANCE_DIGEST_ONLY")
    ...
    if summary.get("authority_provenance_pass") is not True:
        errors.append("authority_provenance_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_LINKED_AUTHORITY_PROVENANCE_V40_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_LINKED_AUTHORITY_PROVENANCE_V40_VALID")
    return 0
