#!/usr/bin/env python3
"""Validate landing/cabin ambience stop-endpoint evidence without audition."""
...
SCHEMA = "audio_landing_cabin_stop_endpoint_v1"
REQUIRED_ENDPOINTS = {"landing_abort", "landing_complete", "cabin_exit", "source_detach"}
...
def validate_ledger(ledger: Any) -> list[str]:
    ...
    if ledger.get("claim") != "AUTOMATED_STOP_ONLY":
        errors.append("claim must be AUTOMATED_STOP_ONLY")
    ...
    for index, endpoint in enumerate(endpoints):
        ...
        if name not in REQUIRED_ENDPOINTS:
            ...
            seen.add(name)
        for key in ("trigger", "stop_evidence", "route_evidence"):
            if not _text(endpoint.get(key)):
                errors.append(f"{prefix}.{key} is required")
        if endpoint.get("terminal_gain_db") != -80:
            errors.append(f"{prefix}.terminal_gain_db must be -80 dB")
        if endpoint.get("bus") != "Ambience":
            errors.append(f"{prefix}.bus must be Ambience")
        for key in ("stops_voice", "clears_binding", "presentation_only"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_LANDING_CABIN_STOP_ENDPOINT_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_LANDING_CABIN_STOP_ENDPOINT_VALID")
    return 0
