#!/usr/bin/env python3
"""Validate v53 release/version reconciliation summaries."""
...
SCHEMA = "audio_cleanup_release_version_reconciliation_summary_v53"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "release_id", "release_version", "reconciliation_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_RELEASE_VERSION_RECONCILIATION_ONLY":
        errors.append("claim must be AUTOMATED_RELEASE_VERSION_RECONCILIATION_ONLY")
    if not _text(summary.get("boundary_note")):
        errors.append("boundary_note is required")
    ids, versions = summary.get("record_ids"), summary.get("release_versions")
    if not _roster(ids):
    ...
    if _text(summary.get("release_version")) and isinstance(versions, list) and summary["release_version"] not in versions:
        errors.append("release_version must be in release_versions")
    for key in ("reconciliation_digest", "release_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("reconciliation_digest"), record.get("release_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("release_id", "release_version", "reconciliation_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records release/reconciliation digest pairs must agree")
    if summary.get("release_reconciliation_pass") is not True:
        errors.append("release_reconciliation_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_RELEASE_VERSION_RECONCILIATION_V53_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_RELEASE_VERSION_RECONCILIATION_V53_VALID")
    return 0
