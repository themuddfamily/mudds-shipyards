#!/usr/bin/env python3
"""Validate v265 audio cleanup evidence/state summaries."""
from __future__ import annotations
import argparse, json, re
from pathlib import Path
from typing import Any

SCHEMA="audio_cleanup_evidence_state_v265"; CLAIM="AUTOMATED_EVIDENCE_STATE_ONLY"; NOT_RUN="NOT_RUN"
STATES={"open","ready","closed"}; SHA256=re.compile(r"^[0-9a-f]{64}$")
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("summary",type=Path)
    e=validate_summary(json.loads(p.parse_args(argv).summary.read_text(encoding="utf-8")))
    if e:print("AUDIO_CLEANUP_EVIDENCE_STATE_V265_INVALID");print("\n".join(f"- {x}" for x in e));return 1
    print("AUDIO_CLEANUP_EVIDENCE_STATE_V265_VALID");return 0
