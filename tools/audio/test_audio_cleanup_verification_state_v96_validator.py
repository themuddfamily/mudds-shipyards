"""Focused tests for v96 verification/state summaries."""
...
import audio_cleanup_verification_state_v96_validator as validator  # noqa: E402


def summary() -> dict:
    verification, state = "a" * 64, "b" * 64
    def record(rid: str, evidence: str) -> dict:
        return {"record_id": rid, "verification_digest": verification, "state_digest": state, "verification_id": "verification-v96", "state_id": "state-v96", "canonicalization": "json-sorted-v1", "evidence": evidence, "state_pass": True}
    return {"schema": "audio_cleanup_verification_state_v96", "revision": "a" * 40, "owner": "audio-evidence-owner", "summary_id": "cleanup-verification-state-v96", "evidence_bundle": "artifacts/audio/verification-state-v96.json", "canonicalization": "json-sorted-v1", "verification_id": "verification-v96", "state_id": "state-v96", "claim": "AUTOMATED_VERIFICATION_STATE_ONLY", "boundary_note": "Verification state does not establish native audibility.", "record_ids": ["record-a", "record-b"], "verification_digest": verification, "state_digest": state, "records": [record("record-a", "artifacts/audio/a.json"), record("record-b", "artifacts/audio/b.json")], "verification_state_pass": True}


class AudioCleanupVerificationStateV96Tests(unittest.TestCase):
    def test_valid_verification_state_summary(self):
