#!/usr/bin/env python3
"""Validate v64 verification/attestation summaries."""
...
SCHEMA = "audio_cleanup_verification_attestation_v64"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "verification_id", "attestation_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_VERIFICATION_ATTESTATION_ONLY":
        errors.append("claim must be AUTOMATED_VERIFICATION_ATTESTATION_ONLY")
    ...
    for key in ("verification_digest", "attestation_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("verification_digest"), record.get("attestation_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("verification_id", "attestation_id", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("verification_pass") is not True:
            errors.append(f"{prefix}.verification_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(pairs) > 1:
        errors.append("records verification/attestation digest pairs must agree")
    if summary.get("verification_attestation_pass") is not True:
        errors.append("verification_attestation_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_VERIFICATION_ATTESTATION_V64_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_VERIFICATION_ATTESTATION_V64_VALID")
    return 0
