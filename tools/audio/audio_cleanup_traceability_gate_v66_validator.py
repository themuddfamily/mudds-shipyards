#!/usr/bin/env python3
"""Validate v66 traceability/gate summaries."""
...
SCHEMA = "audio_cleanup_traceability_gate_v66"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "traceability_id", "gate_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_TRACEABILITY_GATE_ONLY":
        errors.append("claim must be AUTOMATED_TRACEABILITY_GATE_ONLY")
    ...
    for key in ("traceability_digest", "gate_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("traceability_digest"), record.get("gate_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("traceability_id", "gate_id", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("gate_pass") is not True:
            errors.append(f"{prefix}.gate_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(pairs) > 1:
        errors.append("records traceability/gate digest pairs must agree")
    if summary.get("traceability_gate_pass") is not True:
        errors.append("traceability_gate_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_TRACEABILITY_GATE_V66_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_TRACEABILITY_GATE_V66_VALID")
    return 0
