#!/usr/bin/env python3
"""Validate v51 release/version authority-reconciliation summaries."""
...
SCHEMA = "audio_cleanup_release_version_authority_reconciliation_summary_v51"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "release_id", "authority_id", "reconciliation_id", "release_version"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_RELEASE_VERSION_AUTHORITY_RECONCILIATION_ONLY":
        errors.append("claim must be AUTOMATED_RELEASE_VERSION_AUTHORITY_RECONCILIATION_ONLY")
    ...
    for index, record in enumerate(records):
        ...
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("release_id", "authority_id", "reconciliation_id", "release_version", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("release_pass") is not True:
            errors.append(f"{prefix}.release_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(pairs) > 1:
        errors.append("records release authority/reconciliation pairs must agree")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_RELEASE_VERSION_AUTHORITY_RECONCILIATION_V51_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_RELEASE_VERSION_AUTHORITY_RECONCILIATION_V51_VALID")
    return 0
