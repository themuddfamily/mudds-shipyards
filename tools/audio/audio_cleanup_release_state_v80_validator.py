#!/usr/bin/env python3
"""Validate v80 release/state summaries."""
...
SCHEMA = "audio_cleanup_release_state_v80"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "release_id", "state_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_RELEASE_STATE_ONLY":
        errors.append("claim must be AUTOMATED_RELEASE_STATE_ONLY")
    ...
    for key in ("release_digest", "state_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("release_digest"), record.get("state_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("release_id", "state_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records release/state digest pairs must agree")
    if summary.get("release_state_pass") is not True:
        errors.append("release_state_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_RELEASE_STATE_V80_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_RELEASE_STATE_V80_VALID")
    return 0
