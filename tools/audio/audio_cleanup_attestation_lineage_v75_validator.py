#!/usr/bin/env python3
"""Validate v75 attestation/lineage summaries."""
...
SCHEMA = "audio_cleanup_attestation_lineage_v75"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "attestation_id", "lineage_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_ATTESTATION_LINEAGE_ONLY":
        errors.append("claim must be AUTOMATED_ATTESTATION_LINEAGE_ONLY")
    ...
    for key in ("attestation_digest", "lineage_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("attestation_digest"), record.get("lineage_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("attestation_id", "lineage_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records attestation/lineage digest pairs must agree")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_ATTESTATION_LINEAGE_V75_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_ATTESTATION_LINEAGE_V75_VALID")
    return 0
