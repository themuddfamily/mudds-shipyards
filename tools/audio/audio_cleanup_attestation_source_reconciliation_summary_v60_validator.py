#!/usr/bin/env python3
"""Validate v60 attestation/source reconciliation summaries."""
...
SCHEMA = "audio_cleanup_attestation_source_reconciliation_summary_v60"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "attestation_id", "source_id", "reconciliation_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_ATTESTATION_SOURCE_RECONCILIATION_ONLY":
        errors.append("claim must be AUTOMATED_ATTESTATION_SOURCE_RECONCILIATION_ONLY")
    ...
    for key in ("attestation_digest", "source_digest", "reconciliation_digest"):
    ...
    for index, record in enumerate(records):
        ...
        keys = ("attestation_digest", "source_digest", "reconciliation_digest")
        for key in keys:
        ...
        for key in ("attestation_id", "source_id", "reconciliation_id", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("attestation_pass") is not True:
            errors.append(f"{prefix}.attestation_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(triples) > 1:
        errors.append("records attestation/source/reconciliation triples must agree")
    if summary.get("attestation_reconciliation_pass") is not True:
        errors.append("attestation_reconciliation_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_ATTESTATION_SOURCE_RECONCILIATION_V60_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_ATTESTATION_SOURCE_RECONCILIATION_V60_VALID")
    return 0
