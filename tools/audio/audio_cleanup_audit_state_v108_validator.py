#!/usr/bin/env python3
"""Validate v108 audio cleanup audit/state summaries."""
...
SCHEMA = "audio_cleanup_audit_state_v108"
CLAIM = "AUTOMATED_AUDIT_STATE_ONLY"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "audit_bundle", "audit_id", "state_model"):
    ...
    for index, record in enumerate(records):
        ...
        for key in ("audit_id", "state_model", "state"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("audit")):
            errors.append(f"{prefix}.audit is required")
        if record.get("audit_pass") is not True:
            errors.append(f"{prefix}.audit_pass must be true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_AUDIT_STATE_V108_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_AUDIT_STATE_V108_VALID")
    return 0
