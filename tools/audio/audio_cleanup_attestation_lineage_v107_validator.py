#!/usr/bin/env python3
"""Validate v107 audio cleanup attestation/lineage summaries."""
...
SCHEMA = "audio_cleanup_attestation_lineage_v107"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "attestation_bundle", "attestation_id", "lineage_id"):
    ...
    for key in ("attestation_digest", "lineage_root"):
    ...
    for index, record in enumerate(records):
        ...
        for key in ("attestation_digest", "lineage_root", "lineage_leaf"):
            if not _digest(record.get(key)):
                errors.append(f"{prefix}.{key} must be a lowercase 64-character digest")
        if _digest(record.get("attestation_digest")) and record.get("attestation_digest") != summary.get("attestation_digest"):
            errors.append(f"{prefix}.attestation_digest must match summary")
        ...
        for key in ("attestation_id", "lineage_id"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("attestation")):
            errors.append(f"{prefix}.attestation is required")
    ...
    if summary.get("attestation_lineage_pass") is not True:
        errors.append("attestation_lineage_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_ATTESTATION_LINEAGE_V107_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_ATTESTATION_LINEAGE_V107_VALID")
    return 0
