#!/usr/bin/env python3
"""Validate v105 audio cleanup provenance/closure summaries."""
...
SCHEMA = "audio_cleanup_provenance_closure_v105"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "provenance_bundle", "provenance_id", "closure_id"):
    ...
    for index, record in enumerate(records):
        ...
        for key in ("provenance_id", "closure_id"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_PROVENANCE_CLOSURE_V105_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_PROVENANCE_CLOSURE_V105_VALID")
    return 0
