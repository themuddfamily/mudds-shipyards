"""Focused tests for v340 audio cleanup evidence/state summaries."""
import copy, sys, unittest
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent))
import audio_cleanup_evidence_state_v340_validator as validator  # noqa: E402

def summary() -> dict:
    evidence_digest, state_digest = "a" * 64, "b" * 64
    def record(record_id: str, evidence: str) -> dict:
        return {"record_id": record_id, "evidence_digest": evidence_digest, "state_digest": state_digest, "evidence_id": "evidence-v340", "state_model": "cleanup-state-v1", "state": "closed", "evidence": evidence, "state_pass": True}
    return {"schema": "audio_cleanup_evidence_state_v340", "revision": "a" * 40, "owner": "audio-evidence-owner", "summary_id": "cleanup-evidence-state-v340", "evidence_bundle": "artifacts/audio/evidence-state-v340.json", "evidence_id": "evidence-v340", "state_model": "cleanup-state-v1", "state": "closed", "claim": "AUTOMATED_EVIDENCE_STATE_ONLY", "detached_status": "NOT_RUN", "native_status": "NOT_RUN", "hardware_status": "NOT_RUN", "human_review_status": "NOT_RUN", "boundary_note": "Automated evidence does not establish detached, native, hardware, or human-review outcomes.", "evidence_digest": evidence_digest, "state_digest": state_digest, "record_ids": ["record-a", "record-b"], "records": [record("record-a", "artifacts/audio/a.json"), record("record-b", "artifacts/audio/b.json")], "evidence_state_pass": True}

class AudioCleanupEvidenceStateV340Tests(unittest.TestCase):
