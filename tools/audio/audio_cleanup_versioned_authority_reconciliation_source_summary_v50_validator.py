#!/usr/bin/env python3
"""Validate v50 source-bound versioned authority/reconciliation summaries."""
...
SCHEMA = "audio_cleanup_versioned_authority_reconciliation_source_summary_v50"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "authority_id", "reconciliation_id", "reconciliation_source", "authority_version", "reconciliation_version"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_VERSIONED_AUTHORITY_RECONCILIATION_SOURCE_ONLY":
        errors.append("claim must be AUTOMATED_VERSIONED_AUTHORITY_RECONCILIATION_SOURCE_ONLY")
    ...
    for key in ("authority_versions", "reconciliation_versions"):
        if not _roster(summary.get(key)):
            errors.append(f"{key} must be ordered, unique, and non-empty")
    for key, roster_key in (("authority_version", "authority_versions"), ("reconciliation_version", "reconciliation_versions")):
    ...
    for index, record in enumerate(records):
        ...
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("authority_id", "reconciliation_id", "reconciliation_source", "authority_version", "reconciliation_version", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records source-bound authority/reconciliation pairs must agree")
    if summary.get("source_reconciliation_pass") is not True:
        errors.append("source_reconciliation_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_VERSIONED_AUTHORITY_RECONCILIATION_SOURCE_V50_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_VERSIONED_AUTHORITY_RECONCILIATION_SOURCE_V50_VALID")
    return 0
