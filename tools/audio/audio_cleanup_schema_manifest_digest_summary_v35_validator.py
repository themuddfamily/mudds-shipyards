#!/usr/bin/env python3
"""Validate v35 schema/manifest digest summaries."""
...
SCHEMA = "audio_cleanup_schema_manifest_digest_summary_v35"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "manifest_id", "schema_version"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_SCHEMA_MANIFEST_DIGEST_ONLY":
        errors.append("claim must be AUTOMATED_SCHEMA_MANIFEST_DIGEST_ONLY")
    ...
    for key in ("manifest_digest", "schema_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("manifest_digest"), record.get("schema_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("manifest_id", "schema_version", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("manifest_pass") is not True:
            errors.append(f"{prefix}.manifest_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(pairs) > 1:
        errors.append("records manifest/schema digest pairs must agree")
    if summary.get("manifest_digest_pass") is not True:
        errors.append("manifest_digest_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_SCHEMA_MANIFEST_DIGEST_V35_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_SCHEMA_MANIFEST_DIGEST_V35_VALID")
    return 0
