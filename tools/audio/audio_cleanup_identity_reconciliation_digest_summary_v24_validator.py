#!/usr/bin/env python3
"""Validate v24 cleanup identity-reconciliation digest summaries."""
...
SCHEMA = "audio_cleanup_identity_reconciliation_digest_summary_v24"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    if summary.get("claim") != "AUTOMATED_IDENTITY_RECONCILIATION_DIGEST_ONLY":
        errors.append("claim must be AUTOMATED_IDENTITY_RECONCILIATION_DIGEST_ONLY")
    if not _text(summary.get("boundary_note")):
        errors.append("boundary_note is required")
    identity_ids = summary.get("identity_ids")
    if not _ordered_unique(identity_ids):
        errors.append("identity_ids must be ordered, unique, and non-empty")
    agreed = summary.get("identity_digest")
    if not _digest(agreed):
        errors.append("identity_digest must be a lowercase 64-character digest")
    ...
    for index, record in enumerate(records):
        ...
        if record.get("identity_ids") != identity_ids:
            errors.append(f"{prefix}.identity_ids must match ordered summary roster")
        digest = record.get("identity_digest")
        if not _digest(digest):
            errors.append(f"{prefix}.identity_digest must be a lowercase 64-character digest")
        else:
    ...
    if len(record_digests) > 1:
        errors.append("records identity_digest values must agree")
    if record_digests and agreed not in record_digests:
        errors.append("identity_digest must match records")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_IDENTITY_RECONCILIATION_V24_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_IDENTITY_RECONCILIATION_V24_VALID")
    return 0
