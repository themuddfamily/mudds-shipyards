#!/usr/bin/env python3
"""Validate v103 audio cleanup audit/lineage summaries."""
...
SCHEMA = "audio_cleanup_audit_lineage_v103"
CLAIM = "AUTOMATED_AUDIT_LINEAGE_ONLY"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "audit_bundle", "audit_id", "lineage_id"):
    ...
    for key in ("lineage_root", "audit_digest"):
    ...
    for index, record in enumerate(records):
        ...
        for key in ("audit_digest", "lineage_root", "lineage_leaf"):
            if not _digest(record.get(key)):
                errors.append(f"{prefix}.{key} must be a lowercase 64-character digest")
        if _digest(record.get("audit_digest")) and record.get("audit_digest") != summary.get("audit_digest"):
            errors.append(f"{prefix}.audit_digest must match summary")
        ...
        for key in ("audit_id", "lineage_id"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_AUDIT_LINEAGE_V103_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_AUDIT_LINEAGE_V103_VALID")
    return 0
