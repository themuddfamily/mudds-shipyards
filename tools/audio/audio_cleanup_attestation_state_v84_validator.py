#!/usr/bin/env python3
"""Validate v84 attestation/state summaries."""
...
SCHEMA = "audio_cleanup_attestation_state_v84"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "attestation_id", "state_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_ATTESTATION_STATE_ONLY":
        errors.append("claim must be AUTOMATED_ATTESTATION_STATE_ONLY")
    ...
    for key in ("attestation_digest", "state_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("attestation_digest"), record.get("state_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("attestation_id", "state_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records attestation/state digest pairs must agree")
    if summary.get("attestation_state_pass") is not True:
        errors.append("attestation_state_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_ATTESTATION_STATE_V84_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_ATTESTATION_STATE_V84_VALID")
    return 0
