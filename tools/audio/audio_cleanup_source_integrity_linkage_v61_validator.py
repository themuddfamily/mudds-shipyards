#!/usr/bin/env python3
"""Validate v61 source-integrity/linkage summaries."""
...
SCHEMA = "audio_cleanup_source_integrity_linkage_v61"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "source_id", "linkage_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_SOURCE_INTEGRITY_LINKAGE_ONLY":
        errors.append("claim must be AUTOMATED_SOURCE_INTEGRITY_LINKAGE_ONLY")
    ...
    for key in ("source_digest", "linkage_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("source_digest"), record.get("linkage_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("source_id", "linkage_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records source/linkage digest pairs must agree")
    if summary.get("source_integrity_pass") is not True:
        errors.append("source_integrity_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_SOURCE_INTEGRITY_LINKAGE_V61_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_SOURCE_INTEGRITY_LINKAGE_V61_VALID")
    return 0
