#!/usr/bin/env python3
"""Validate v101 evidence/closure summaries."""
...
SCHEMA = "audio_cleanup_evidence_closure_v101"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "evidence_id", "closure_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_EVIDENCE_CLOSURE_ONLY":
        errors.append("claim must be AUTOMATED_EVIDENCE_CLOSURE_ONLY")
    ...
    for key in ("evidence_digest", "closure_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("evidence_digest"), record.get("closure_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("evidence_id", "closure_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records evidence/closure digest pairs must agree")
    if summary.get("evidence_closure_pass") is not True:
        errors.append("evidence_closure_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_EVIDENCE_CLOSURE_V101_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_EVIDENCE_CLOSURE_V101_VALID")
    return 0
