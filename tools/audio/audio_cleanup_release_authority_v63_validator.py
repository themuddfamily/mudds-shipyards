#!/usr/bin/env python3
"""Validate v63 release/authority summaries."""
...
SCHEMA = "audio_cleanup_release_authority_v63"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "release_id", "authority_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_RELEASE_AUTHORITY_ONLY":
        errors.append("claim must be AUTOMATED_RELEASE_AUTHORITY_ONLY")
    ...
    for key in ("release_digest", "authority_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("release_digest"), record.get("authority_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("release_id", "authority_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records release/authority digest pairs must agree")
    if summary.get("release_authority_pass") is not True:
        errors.append("release_authority_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_RELEASE_AUTHORITY_V63_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_RELEASE_AUTHORITY_V63_VALID")
    return 0
