#!/usr/bin/env python3
"""Validate v65 readiness/consistency summaries."""
...
SCHEMA = "audio_cleanup_readiness_consistency_v65"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "readiness_id", "consistency_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_READINESS_CONSISTENCY_ONLY":
        errors.append("claim must be AUTOMATED_READINESS_CONSISTENCY_ONLY")
    ...
    for key in ("readiness_digest", "consistency_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("readiness_digest"), record.get("consistency_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("readiness_id", "consistency_id", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("readiness_pass") is not True:
            errors.append(f"{prefix}.readiness_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(pairs) > 1:
        errors.append("records readiness/consistency digest pairs must agree")
    if summary.get("readiness_consistency_pass") is not True:
        errors.append("readiness_consistency_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_READINESS_CONSISTENCY_V65_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_READINESS_CONSISTENCY_V65_VALID")
    return 0
