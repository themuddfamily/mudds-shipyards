#!/usr/bin/env python3
"""Validate v110 audio cleanup consistency/state summaries."""
...
SCHEMA = "audio_cleanup_consistency_state_v110"
CLAIM = "AUTOMATED_CONSISTENCY_STATE_ONLY"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "consistency_bundle", "consistency_id", "state_model"):
    ...
    for index, record in enumerate(records):
        ...
        for key in ("consistency_id", "state_model", "state"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("consistency")):
            errors.append(f"{prefix}.consistency is required")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_CONSISTENCY_STATE_V110_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_CONSISTENCY_STATE_V110_VALID")
    return 0
