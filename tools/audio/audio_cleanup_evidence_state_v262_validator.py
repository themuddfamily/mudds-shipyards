#!/usr/bin/env python3
"""Validate v262 audio cleanup evidence/state summaries."""
...
SCHEMA = "audio_cleanup_evidence_state_v262"
...
def main(argv: list[str] | None = None) -> int:
    ...
    value = json.loads(parser.parse_args(argv).summary.read_text(encoding="utf-8"))
    errors = validate_summary(value)
    if errors:
        print("AUDIO_CLEANUP_EVIDENCE_STATE_V262_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_EVIDENCE_STATE_V262_VALID")
    return 0
