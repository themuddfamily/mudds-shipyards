#!/usr/bin/env python3
"""Validate v39 linked-authority digest summaries."""
...
SCHEMA = "audio_cleanup_linked_authority_digest_summary_v39"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "authority_id", "authority_link_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_LINKED_AUTHORITY_DIGEST_ONLY":
        errors.append("claim must be AUTOMATED_LINKED_AUTHORITY_DIGEST_ONLY")
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("authority_digest"), record.get("link_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("authority_id", "authority_link_id", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("authority_link_pass") is not True:
            errors.append(f"{prefix}.authority_link_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(pairs) > 1:
        errors.append("records linked authority digest pairs must agree")
    if summary.get("linked_authority_pass") is not True:
        errors.append("linked_authority_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_LINKED_AUTHORITY_DIGEST_V39_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_LINKED_AUTHORITY_DIGEST_V39_VALID")
    return 0
