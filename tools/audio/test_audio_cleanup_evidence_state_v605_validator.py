"""Focused tests for v605 audio cleanup evidence/state summaries."""
import copy,sys,unittest
from pathlib import Path
sys.path.insert(0,str(Path(__file__).resolve().parent))
import audio_cleanup_evidence_state_v605_validator as validator # noqa: E402
def summary()->dict:
    ed,sd="a"*64,"b"*64
    def record(rid,ev):return {"record_id":rid,"evidence_digest":ed,"state_digest":sd,"evidence_id":"evidence-v605","state_model":"cleanup-state-v1","state":"closed","evidence":ev,"state_pass":True}
    return {"schema":"audio_cleanup_evidence_state_v605","revision":"a"*40,"owner":"audio-evidence-owner","summary_id":"cleanup-evidence-state-v605","evidence_bundle":"artifacts/audio/evidence-state-v605.json","evidence_id":"evidence-v605","state_model":"cleanup-state-v1","state":"closed","claim":"AUTOMATED_EVIDENCE_STATE_ONLY","detached_status":"NOT_RUN","native_status":"NOT_RUN","hardware_status":"NOT_RUN","human_review_status":"NOT_RUN","boundary_note":"Automated evidence does not establish detached, native, hardware, or human-review outcomes.","evidence_digest":ed,"state_digest":sd,"record_ids":["record-a","record-b"],"records":[record("record-a","artifacts/audio/a.json"),record("record-b","artifacts/audio/b.json")],"evidence_state_pass":True}
class AudioCleanupEvidenceStateV605Tests(unittest.TestCase):
