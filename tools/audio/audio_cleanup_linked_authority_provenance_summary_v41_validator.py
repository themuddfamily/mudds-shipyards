#!/usr/bin/env python3
"""Validate v41 linked authority/provenance summaries."""
...
SCHEMA = "audio_cleanup_linked_authority_provenance_summary_v41"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "authority_id", "provenance_id", "provenance_source"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_LINKED_AUTHORITY_PROVENANCE_ONLY":
        errors.append("claim must be AUTOMATED_LINKED_AUTHORITY_PROVENANCE_ONLY")
    ...
    for index, record in enumerate(records):
        ...
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("authority_id", "provenance_id", "provenance_source", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("provenance_link_pass") is not True:
            errors.append(f"{prefix}.provenance_link_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(pairs) > 1:
        errors.append("records authority/provenance pairs must agree")
    if summary.get("authority_provenance_summary_pass") is not True:
        errors.append("authority_provenance_summary_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_LINKED_AUTHORITY_PROVENANCE_SUMMARY_V41_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_LINKED_AUTHORITY_PROVENANCE_SUMMARY_V41_VALID")
    return 0
