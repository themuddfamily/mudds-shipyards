"""Focused tests for v262 audio cleanup evidence/state summaries."""
import copy
import sys
import unittest
...
import audio_cleanup_evidence_state_v262_validator as validator  # noqa: E402
...
def summary() -> dict:
    ...
    def record(record_id: str, evidence: str) -> dict:
        return {"record_id": record_id, "evidence_digest": evidence_digest,
                "state_digest": state_digest, "evidence_id": "evidence-v262",
                "state_model": "cleanup-state-v1", "state": "closed",
                "evidence": evidence, "state_pass": True}
    return {"schema": "audio_cleanup_evidence_state_v262", "revision": "a" * 40,
            "owner": "audio-evidence-owner", "summary_id": "cleanup-evidence-state-v262",
            "evidence_bundle": "artifacts/audio/evidence-state-v262.json",
            "evidence_id": "evidence-v262", "state_model": "cleanup-state-v1",
...
class AudioCleanupEvidenceStateV262Tests(unittest.TestCase):
    def test_valid_evidence_state_summary(self):
        self.assertEqual(validator.validate_summary(summary()), [])

    def test_not_run_semantics_are_required(self):
        value = copy.deepcopy(summary())
        value["native_status"] = value["stale_callback_status"] = "PASS"
