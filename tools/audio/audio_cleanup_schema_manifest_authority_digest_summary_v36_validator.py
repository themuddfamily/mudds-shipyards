#!/usr/bin/env python3
"""Validate v36 schema/manifest/authority digest summaries."""
...
SCHEMA = "audio_cleanup_schema_manifest_authority_digest_summary_v36"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "manifest_id", "authority_id", "schema_version"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_SCHEMA_MANIFEST_AUTHORITY_DIGEST_ONLY":
        errors.append("claim must be AUTOMATED_SCHEMA_MANIFEST_AUTHORITY_DIGEST_ONLY")
    ...
    for key in ("manifest_digest", "schema_digest", "authority_digest"):
    ...
    for index, record in enumerate(records):
        ...
        triple = (record.get("manifest_digest"), record.get("schema_digest"), record.get("authority_digest"))
        if all(_digest(item) for item in triple):
            triples.add(triple)
        for key in ("manifest_id", "authority_id", "schema_version", "canonicalization"):
    ...
    if len(triples) > 1:
        errors.append("records manifest/schema/authority digest triples must agree")
    if summary.get("authority_digest_pass") is not True:
        errors.append("authority_digest_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_SCHEMA_MANIFEST_AUTHORITY_DIGEST_V36_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_SCHEMA_MANIFEST_AUTHORITY_DIGEST_V36_VALID")
    return 0
