#!/usr/bin/env python3
"""Validate v52 release/version linkage summaries."""
...
SCHEMA = "audio_cleanup_release_version_linkage_summary_v52"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "release_id", "release_version", "linkage_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_RELEASE_VERSION_LINKAGE_ONLY":
        errors.append("claim must be AUTOMATED_RELEASE_VERSION_LINKAGE_ONLY")
    ...
    for key in ("linkage_digest", "release_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("linkage_digest"), record.get("release_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("release_id", "release_version", "linkage_id", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("linkage_pass") is not True:
            errors.append(f"{prefix}.linkage_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(pairs) > 1:
        errors.append("records release/linkage digest pairs must agree")
    if summary.get("release_linkage_pass") is not True:
        errors.append("release_linkage_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_RELEASE_VERSION_LINKAGE_V52_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_RELEASE_VERSION_LINKAGE_V52_VALID")
    return 0
