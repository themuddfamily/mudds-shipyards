#!/usr/bin/env python3
"""Validate v76 verification/closure summaries."""
...
SCHEMA = "audio_cleanup_verification_closure_v76"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "verification_id", "closure_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_VERIFICATION_CLOSURE_ONLY":
        errors.append("claim must be AUTOMATED_VERIFICATION_CLOSURE_ONLY")
    ...
    for key in ("verification_digest", "closure_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("verification_digest"), record.get("closure_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("verification_id", "closure_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records verification/closure digest pairs must agree")
    if summary.get("verification_closure_pass") is not True:
        errors.append("verification_closure_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_VERIFICATION_CLOSURE_V76_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_VERIFICATION_CLOSURE_V76_VALID")
    return 0
