#!/usr/bin/env python3
"""Validate v59 source/channel/release summaries."""
...
SCHEMA = "audio_cleanup_source_channel_release_summary_v59"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "source_id", "channel_id", "release_id", "release_version"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_SOURCE_CHANNEL_RELEASE_ONLY":
        errors.append("claim must be AUTOMATED_SOURCE_CHANNEL_RELEASE_ONLY")
    ...
    for key in ("source_digest", "channel_digest", "release_digest"):
    ...
    for index, record in enumerate(records):
        ...
        keys = ("source_digest", "channel_digest", "release_digest")
        for key in keys:
        ...
        for key in ("source_id", "channel_id", "release_id", "release_version", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("summary_pass") is not True:
            errors.append(f"{prefix}.summary_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(triples) > 1:
        errors.append("records source/channel/release triples must agree")
    if summary.get("source_channel_release_pass") is not True:
        errors.append("source_channel_release_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_SOURCE_CHANNEL_RELEASE_V59_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_SOURCE_CHANNEL_RELEASE_V59_VALID")
    return 0
