#!/usr/bin/env python3
"""Validate v14 cleanup manifest identity/count consensus summaries."""
...
SCHEMA = "audio_cleanup_manifest_identity_count_consensus_summary_v14"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    if summary.get("claim") != "AUTOMATED_MANIFEST_COUNT_ONLY":
        errors.append("claim must be AUTOMATED_MANIFEST_COUNT_ONLY")
    ...
    if not _non_negative_int(count) or count <= 0:
    ...
    for index, record in enumerate(records):
        ...
        if record.get("count_pass") is not True:
            errors.append(f"{prefix}.count_pass must be true")
    if summary.get("identity_count_pass") is not True:
        errors.append("identity_count_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_MANIFEST_COUNT_V14_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_MANIFEST_COUNT_V14_VALID")
    return 0
