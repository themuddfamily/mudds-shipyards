"""Focused tests for v225 audio cleanup evidence/state summaries."""
import copy,sys,unittest
from pathlib import Path
sys.path.insert(0,str(Path(__file__).resolve().parent))
import audio_cleanup_evidence_state_v225_validator as validator # noqa: E402
def summary()->dict:
    d,t="a"*64,"b"*64
    def r(i,a):return {"record_id":i,"evidence_digest":d,"state_digest":t,"evidence_id":"evidence-v225","state_model":"cleanup-state-v1","state":"closed","evidence":a,"state_pass":True}
    return {"schema":"audio_cleanup_evidence_state_v225","revision":"a"*40,"owner":"audio-evidence-owner","summary_id":"cleanup-evidence-state-v225","evidence_bundle":"artifacts/audio/evidence-state-v225.json","evidence_id":"evidence-v225","state_model":"cleanup-state-v1","state":"closed","claim":"AUTOMATED_EVIDENCE_STATE_ONLY","native_status":"NOT_RUN","stale_callback_status":"NOT_RUN","boundary_note":"Evidence state does not establish native audibility.","evidence_digest":d,"state_digest":t,"record_ids":["record-a","record-b"],"records":[r("record-a","artifacts/audio/a.json"),r("record-b","artifacts/audio/b.json")],"evidence_state_pass":True}
class AudioCleanupEvidenceStateV225Tests(unittest.TestCase):
