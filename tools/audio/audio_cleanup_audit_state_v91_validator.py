#!/usr/bin/env python3
"""Validate v91 audit/state summaries."""
...
SCHEMA = "audio_cleanup_audit_state_v91"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "audit_id", "state_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_AUDIT_STATE_ONLY":
        errors.append("claim must be AUTOMATED_AUDIT_STATE_ONLY")
    ...
    for key in ("audit_digest", "state_digest"):
    ...
    for index, record in enumerate(records):
        ...
        pair = (record.get("audit_digest"), record.get("state_digest"))
        if _digest(pair[0]) and _digest(pair[1]):
            pairs.add(pair)
        for key in ("audit_id", "state_id", "canonicalization"):
    ...
    if len(pairs) > 1:
        errors.append("records audit/state digest pairs must agree")
    if summary.get("audit_state_pass") is not True:
        errors.append("audit_state_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_AUDIT_STATE_V91_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_AUDIT_STATE_V91_VALID")
    return 0
