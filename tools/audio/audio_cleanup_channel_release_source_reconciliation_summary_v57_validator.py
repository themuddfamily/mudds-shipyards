#!/usr/bin/env python3
"""Validate v57 channel/release source reconciliation summaries."""
...
SCHEMA = "audio_cleanup_channel_release_source_reconciliation_summary_v57"
...
def validate_summary(summary: Any) -> list[str]:
    ...
    for key in ("revision", "owner", "summary_id", "evidence_bundle", "canonicalization", "channel_id", "release_id", "release_version", "source_id", "reconciliation_id"):
        if not _text(summary.get(key)):
            errors.append(f"{key} is required")
    if summary.get("claim") != "AUTOMATED_CHANNEL_RELEASE_SOURCE_RECONCILIATION_ONLY":
        errors.append("claim must be AUTOMATED_CHANNEL_RELEASE_SOURCE_RECONCILIATION_ONLY")
    ...
    for key in ("channel_digest", "release_digest", "source_digest", "reconciliation_digest"):
    ...
    for index, record in enumerate(records):
        ...
        keys = ("channel_digest", "release_digest", "source_digest", "reconciliation_digest")
        for key in keys:
        ...
        for key in ("channel_id", "release_id", "release_version", "source_id", "reconciliation_id", "canonicalization"):
            if record.get(key) != summary.get(key):
                errors.append(f"{prefix}.{key} must match summary")
        if not _text(record.get("evidence")):
            errors.append(f"{prefix}.evidence is required")
        if record.get("source_reconciliation_pass") is not True:
            errors.append(f"{prefix}.source_reconciliation_pass must be true")
    if isinstance(ids, list) and seen != set(ids):
        errors.append("record_ids must exactly match records")
    if len(quadruples) > 1:
        errors.append("records channel/release/source/reconciliation quadruples must agree")
    if summary.get("channel_source_reconciliation_pass") is not True:
        errors.append("channel_source_reconciliation_pass must be true")
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("AUDIO_CLEANUP_CHANNEL_RELEASE_SOURCE_RECONCILIATION_V57_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("AUDIO_CLEANUP_CHANNEL_RELEASE_SOURCE_RECONCILIATION_V57_VALID")
    return 0
