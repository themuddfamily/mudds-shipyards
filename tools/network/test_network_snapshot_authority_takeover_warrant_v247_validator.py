...
try:
    from .network_snapshot_authority_takeover_warrant_v247_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_warrant_v247_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["warrant_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['warrant_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "warrant_id": "server-authority-warrant-v1",
            "authority_takeover_token": "server-authority-takeover-token-v18",
            "snapshot_id": "snapshot-authority-v130", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 19,
            "sequence": 20142, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverWarrantV247ValidatorTest(unittest.TestCase):
    def test_accepts_authority_warrant(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_warrant_binding(self):
        report = _report()
        report["records"][0]["warrant_id"] = "server-authority-warrant-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority warrant" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority warrant records" in error for error in errors))
