...
try:
    from .network_snapshot_authority_takeover_review_v263_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_review_v263_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["review_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['review_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "review_id": "server-authority-review-v1",
            "authority_takeover_token": "server-authority-takeover-token-v34",
            "snapshot_id": "snapshot-authority-v146", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 35,
            "sequence": 36158, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverReviewV263ValidatorTest(unittest.TestCase):
    def test_accepts_authority_review(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_review_binding(self):
        report = _report()
        report["records"][0]["review_id"] = "server-authority-review-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority review" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority review records" in error for error in errors))
