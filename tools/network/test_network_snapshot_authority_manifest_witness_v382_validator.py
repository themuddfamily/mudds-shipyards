...
try:
    from .network_snapshot_authority_manifest_witness_v382_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_manifest_witness_v382_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-manifest-witness-assertion-v1",
        "integrity_gate_id": "authority-manifest-witness-v382",
        "snapshot_id": "snapshot-authority-v265", "authority": "server", "sequence": 43082,
...
def _report():
    ...
    return {
        "schema_version": 382,
        "evidence_scope": "network_snapshot_authority_manifest_witness_v382",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-manifest-witness-v382",
        "snapshot_id": "snapshot-authority-v265",
        "assertion_id": "authority-snapshot-manifest-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 154, "release": "release-1",
...
class NetworkSnapshotAuthorityManifestWitnessV382ValidatorTest(unittest.TestCase):
    def test_accepts_manifest_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_manifest_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-manifest"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority manifest witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["manifest"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority manifest members" in error for error in errors))
