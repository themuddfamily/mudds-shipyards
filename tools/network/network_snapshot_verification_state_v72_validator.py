#!/usr/bin/env python3
"""Validate detached v72 snapshot verification/state evidence."""
...
SCHEMA_VERSION = 72
EVIDENCE_SCOPE = "network_snapshot_verification_state_v72"
...
def _verification_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "verification", "state", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for verification/state snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "verification": VERIFICATION, "state": STATE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("verification", report.get("verification")), ("state", report.get("state")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match verification/state")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("verification", report.get("verification")), ("state", report.get("state")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match verification/state")
        ...
        if not _digest(verification_digest):
            ...
            errors.append(f"{prefix}.verification_digest must bind verification/state")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_VERIFICATION_STATE_V72_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_VERIFICATION_STATE_V72_VALID")
    return 0
