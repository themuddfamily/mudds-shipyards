...
try:
    from .network_snapshot_authority_outcome_v152_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_outcome_v152_validator import validate_snapshot
...
class NetworkSnapshotAuthorityOutcomeV152ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_outcome_binding(self):
        report = _report()
        report["records"][0]["authority_outcome"] = "server-authority-outcome-old"
        self.assertTrue(any("bind authority outcome" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v34"
        self.assertTrue(any("bind authority outcome" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority outcome records" in error for error in validate_snapshot(report)))
