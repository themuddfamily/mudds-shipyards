...
try:
    from .network_snapshot_authority_seal_witness_v403_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_seal_witness_v403_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-seal-witness-assertion-v1",
        "integrity_gate_id": "authority-seal-witness-v403",
        "snapshot_id": "snapshot-authority-v286", "authority": "server", "sequence": 45203,
...
def _report():
    ...
    return {
        "schema_version": 403,
        "evidence_scope": "network_snapshot_authority_seal_witness_v403",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-seal-witness-v403",
        "snapshot_id": "snapshot-authority-v286",
        "assertion_id": "authority-snapshot-seal-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 175, "release": "release-1",
...
class NetworkSnapshotAuthoritySealWitnessV403ValidatorTest(unittest.TestCase):
    def test_accepts_seal_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_seal_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["authority"] = "client"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority seal witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["seal"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority seal members" in error for error in errors))
