#!/usr/bin/env python3
"""Validate v36 schema/manifest authority digest evidence."""
...
def _schema_digest(schema: dict[str, Any]) -> str:
    return hashlib.sha256(f"{schema.get('name')}|{schema.get('version')}".encode("utf-8")).hexdigest()


def _authority_digest(manifest: dict[str, Any]) -> str:
    payload = f"{AUTHORITY}|{manifest.get('manifest_id')}|{manifest.get('schema_digest')}|{manifest.get('sequence')}|{manifest.get('digest')}"
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def validate_digests(report: Any, label: str = "digests") -> list[str]:
    """Return schema/manifest digests, entry references, counts, and mutation errors."""
    ...
    if schema.get("name") != SCHEMA_NAME:
        errors.append(f"{label}.schema.name must be {SCHEMA_NAME}")
    if not _positive_int(schema.get("version")):
        errors.append(f"{label}.schema.version must be positive")
    schema_digest = schema.get("schema_digest")
    if not _digest(schema_digest):
        errors.append(f"{label}.schema.schema_digest must be lowercase SHA-256")
    elif schema_digest != _schema_digest(schema):
        errors.append(f"{label}.schema.schema_digest must bind schema")
    ...
    if manifest.get("schema_version") != schema.get("version"):
        errors.append(f"{label}.manifest.schema_version must match schema")
    if manifest.get("schema_digest") != schema_digest:
        errors.append(f"{label}.manifest.schema_digest must match schema")
    ...
    elif authority_digest != _authority_digest(manifest):
        errors.append(f"{label}.manifest.authority_digest must bind schema and manifest")
    ...
    for index, entry in enumerate(entries):
        ...
        if entry.get("schema_digest") != schema_digest:
            errors.append(f"{prefix}.schema_digest must match schema")
        if entry.get("authority_digest") != authority_digest:
            errors.append(f"{prefix}.authority_digest must match manifest")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match schema manifest")
        if counts.get("mutations") != 0:
            errors.append(f"{label}.counts.mutations must be zero")
    return errors


def validate_digests_file(report_path: Path) -> list[str]:
    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return [f"unable to read {report_path}: {exc}"]
    return validate_digests(report, str(report_path))
...
def main() -> int:
    ...
    errors = validate_digests_file(args.digests)
    if errors:
        print("NETWORK_SNAPSHOT_SCHEMA_MANIFEST_AUTHORITY_DIGEST_V36_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_SCHEMA_MANIFEST_AUTHORITY_DIGEST_V36_VALID")
    return 0
