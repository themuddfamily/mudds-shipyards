#!/usr/bin/env python3
"""Validate detached v284 network snapshot authority-takeover ticket evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v167"
...
def _ticket_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('ticket_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover ticket evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority ticket")
    ...
    items = report.get("ticket_items")
    if not isinstance(items, list):
        errors.append(f"{label}.ticket_items must be an array")
        items = []
    ids: set[str] = set()
    issued = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.ticket_items[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority ticket")
        ...
        ticket_digest = item.get("ticket_digest")
        if not _digest(ticket_digest):
            errors.append(f"{prefix}.ticket_digest must be lowercase SHA-256")
        elif ticket_digest != _ticket_digest(item):
            errors.append(f"{prefix}.ticket_digest must bind authority ticket")
        if item.get("issued") is not True:
            errors.append(f"{prefix}.issued must be true")
        else:
            issued += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority ticket items")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "items": len(items), "unique": len(ids), "issued": issued, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority ticket items")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_TICKET_V284_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_TICKET_V284_VALID")
    return 0
