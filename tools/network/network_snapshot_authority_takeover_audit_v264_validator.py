#!/usr/bin/env python3
"""Validate detached v264 network snapshot authority takeover audit evidence."""
...
SCHEMA_VERSION = 264
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_audit_v264"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUDIT_ID = "server-authority-audit-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v35"
SNAPSHOT_ID = "snapshot-authority-v147"
...
def _audit_digest(record: dict[str, Any]) -> str:
    fields = (
        "audit_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('audit_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover audit evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority audit")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority audit")
        ...
        if not _digest(audit_digest):
            ...
            errors.append(f"{prefix}.audit_digest must bind authority audit")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority audit records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority audit records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_AUDIT_V264_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_AUDIT_V264_VALID")
    return 0
