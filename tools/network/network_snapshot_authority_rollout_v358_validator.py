#!/usr/bin/env python3
"""Validate detached v358 authority/snapshot rollout evidence."""
...
AUTHORITY = "server"
ROLLOUT_ID = "authority-rollout-v358"
SNAPSHOT_ID = "snapshot-authority-v241"
ASSERTION_ID = "authority-snapshot-rollout-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 130
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot rollout evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority rollout")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority rollout")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority rollout")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority rollout members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority rollout members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ROLLOUT_V358_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ROLLOUT_V358_VALID")
    return 0
