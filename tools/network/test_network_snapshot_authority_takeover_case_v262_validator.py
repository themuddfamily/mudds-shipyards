...
try:
    from .network_snapshot_authority_takeover_case_v262_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_case_v262_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["case_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['case_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "case_id": "server-authority-case-v1",
            "authority_takeover_token": "server-authority-takeover-token-v33",
            "snapshot_id": "snapshot-authority-v145", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 34,
            "sequence": 35157, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverCaseV262ValidatorTest(unittest.TestCase):
    def test_accepts_authority_case(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_case_binding(self):
        report = _report()
        report["records"][0]["case_id"] = "server-authority-case-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority case" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority case records" in error for error in errors))
