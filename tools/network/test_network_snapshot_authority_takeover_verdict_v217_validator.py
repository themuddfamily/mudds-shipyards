...
try:
    from .network_snapshot_authority_takeover_verdict_v217_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_verdict_v217_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["verdict_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['verdict_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_verdict": "server-authority-takeover-verdict-v1",
            "snapshot_id": "snapshot-authority-v100", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3073, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverVerdictV217ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_verdict_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_verdict_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_verdict"] = "server-authority-takeover-verdict-old"
        self.assertTrue(any("bind authority takeover verdict" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v99"
        self.assertTrue(any("bind authority takeover verdict" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover verdict records" in error for error in validate_snapshot(report)))
