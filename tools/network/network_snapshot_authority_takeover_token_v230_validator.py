#!/usr/bin/env python3
"""Validate detached v230 network snapshot authority-takeover token evidence."""
...
SCHEMA_VERSION = 230
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_token_v230"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v1"
SNAPSHOT_ID = "snapshot-authority-v113"
...
def _token_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_token", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('token_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover token evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover token")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover token")
        ...
        if not _digest(token_digest):
            errors.append(f"{prefix}.token_digest must be lowercase SHA-256")
        elif token_digest != _token_digest(record):
            errors.append(f"{prefix}.token_digest must bind authority takeover token")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover token records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover token records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_TOKEN_V230_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_TOKEN_V230_VALID")
    return 0
