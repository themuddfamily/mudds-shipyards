...
try:
    from .network_snapshot_authority_takeover_epoch_v322_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_epoch_v322_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "epoch_id": "authority-snapshot-epoch-v1", "takeover_id": "authority-takeover-v322",
        "snapshot_id": "snapshot-authority-v205", "authority": "server", "sequence": 42312,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "advanced": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "epoch_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["epoch_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['epoch_digest']}" for item in items
    )
    return {
        "schema_version": 322,
        "evidence_scope": "network_snapshot_authority_takeover_epoch_v322",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v322",
        "snapshot_id": "snapshot-authority-v205", "epoch_id": "authority-snapshot-epoch-v1",
        "source": "server_snapshot", "snapshot_version": 94, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v322", "snapshot_id": "snapshot-authority-v205",
            "epoch_id": "authority-snapshot-epoch-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 94,
            "sequence": 42312, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "epoch_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"epoch_members": 2, "unique": 2, "advanced": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverEpochV322ValidatorTest(unittest.TestCase):
    def test_accepts_epoch_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_epoch_binding(self):
        report = _report()
        report["epoch_members"][0]["epoch_id"] = "wrong-epoch"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority epoch" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["epoch_members"][1]["order"] = 1
        report["epoch_members"][1]["item_id"] = report["epoch_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["epoch_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["epoch_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority epoch members" in error for error in errors))
