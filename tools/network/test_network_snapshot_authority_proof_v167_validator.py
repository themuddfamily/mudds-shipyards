...
try:
    from .network_snapshot_authority_proof_v167_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_proof_v167_validator import validate_snapshot
...
class NetworkSnapshotAuthorityProofV167ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_proof_binding(self):
        report = _report()
        report["records"][0]["authority_proof"] = "server-authority-proof-old"
        self.assertTrue(any("bind authority proof" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v49"
        self.assertTrue(any("bind authority proof" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority proof records" in error for error in validate_snapshot(report)))
