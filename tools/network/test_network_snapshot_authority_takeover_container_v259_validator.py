...
try:
    from .network_snapshot_authority_takeover_container_v259_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_container_v259_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["container_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['container_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "container_id": "server-authority-container-v1",
            "authority_takeover_token": "server-authority-takeover-token-v30",
            "snapshot_id": "snapshot-authority-v142", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 31,
            "sequence": 32154, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverContainerV259ValidatorTest(unittest.TestCase):
    def test_accepts_authority_container(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_container_binding(self):
        report = _report()
        report["records"][0]["container_id"] = "server-authority-container-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority container records" in error for error in errors))
