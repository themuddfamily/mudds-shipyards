#!/usr/bin/env python3
"""Validate detached v353 authority/snapshot completeness evidence."""
...
AUTHORITY = "server"
COMPLETENESS_ID = "authority-completeness-v353"
SNAPSHOT_ID = "snapshot-authority-v236"
ASSERTION_ID = "authority-snapshot-completeness-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 125
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot completeness evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("completeness_id", report.get("completeness_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority completeness")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority completeness")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority completeness")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority completeness members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority completeness members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_COMPLETENESS_V353_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_COMPLETENESS_V353_VALID")
    return 0
