...
try:
    from .network_snapshot_authority_status_v153_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_status_v153_validator import validate_snapshot
...
class NetworkSnapshotAuthorityStatusV153ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_status_binding(self):
        report = _report()
        report["records"][0]["authority_status"] = "server-authority-status-old"
        self.assertTrue(any("bind authority status" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v35"
        self.assertTrue(any("bind authority status" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority status records" in error for error in validate_snapshot(report)))
