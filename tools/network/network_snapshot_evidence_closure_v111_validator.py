#!/usr/bin/env python3
"""Validate detached v111 snapshot evidence/closure evidence."""
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached evidence/closure snapshot evidence."""
    errors: list[str] = []
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_EVIDENCE_CLOSURE_V111_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_EVIDENCE_CLOSURE_V111_VALID")
    return 0
