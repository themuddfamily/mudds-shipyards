#!/usr/bin/env python3
"""Validate detached v196 network snapshot authority-confirmation evidence."""
...
SCHEMA_VERSION = 196
EVIDENCE_SCOPE = "network_snapshot_authority_confirmation_v196"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_CONFIRMATION = "server-authority-confirmation-v1"
SNAPSHOT_ID = "snapshot-authority-v79"
...
def _confirmation_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_confirmation", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-confirmation snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_confirmation", report.get("authority_confirmation")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CONFIRMATION_V196_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CONFIRMATION_V196_VALID")
    return 0
