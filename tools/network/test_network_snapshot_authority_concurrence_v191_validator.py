...
try:
    from .network_snapshot_authority_concurrence_v191_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_concurrence_v191_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["concurrence_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['concurrence_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_concurrence": "server-authority-concurrence-v1",
            "snapshot_id": "snapshot-authority-v74", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2969, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityConcurrenceV191ValidatorTest(unittest.TestCase):
    def test_accepts_concurrent_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_concurrence_binding(self):
        report = _report()
        report["records"][0]["authority_concurrence"] = "server-authority-concurrence-old"
        self.assertTrue(any("bind authority concurrence" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v73"
        self.assertTrue(any("bind authority concurrence" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority concurrence records" in error for error in validate_snapshot(report)))
