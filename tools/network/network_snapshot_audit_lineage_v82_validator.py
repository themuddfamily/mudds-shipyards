#!/usr/bin/env python3
"""Validate detached v82 snapshot audit/lineage evidence."""
...
SCHEMA_VERSION = 82
EVIDENCE_SCOPE = "network_snapshot_audit_lineage_v82"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUDIT_ID = "snapshot-audit-v2"
...
def _audit_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "audit_id", "lineage", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('audit_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()


def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for audit/lineage snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "audit_id": AUDIT_ID, "lineage": LINEAGE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("audit_id", report.get("audit_id")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match audit/lineage")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("audit_id", report.get("audit_id")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match audit/lineage")
        ...
        if not _digest(audit_digest):
            errors.append(f"{prefix}.audit_digest must be lowercase SHA-256")
        elif audit_digest != _audit_digest(record):
            errors.append(f"{prefix}.audit_digest must bind audit/lineage")
        if record.get("audited") is not True:
            errors.append(f"{prefix}.audited must be true")
        else:
            audited += 1
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match audit records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "audited": audited, "mutations": mutations}
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match audit records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUDIT_LINEAGE_V82_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUDIT_LINEAGE_V82_VALID")
    return 0
