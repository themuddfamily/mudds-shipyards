...
try:
    from .network_snapshot_authority_chain_witness_v378_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_chain_witness_v378_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-chain-witness-assertion-v1",
        "integrity_gate_id": "authority-chain-witness-v378",
        "snapshot_id": "snapshot-authority-v261", "authority": "server", "sequence": 42738,
...
def _report():
    ...
    return {
        "schema_version": 378,
        "evidence_scope": "network_snapshot_authority_chain_witness_v378",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-chain-witness-v378",
        "snapshot_id": "snapshot-authority-v261",
        "assertion_id": "authority-snapshot-chain-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 150, "release": "release-1",
...
class NetworkSnapshotAuthorityChainWitnessV378ValidatorTest(unittest.TestCase):
    def test_accepts_chain_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_chain_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["assertion_id"] = "wrong-witness"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority chain witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority chain members" in error for error in errors))
