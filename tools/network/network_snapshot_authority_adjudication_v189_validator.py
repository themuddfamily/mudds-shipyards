#!/usr/bin/env python3
"""Validate detached v189 network snapshot authority-adjudication evidence."""
...
SCHEMA_VERSION = 189
EVIDENCE_SCOPE = "network_snapshot_authority_adjudication_v189"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ADJUDICATION = "server-authority-adjudication-v1"
SNAPSHOT_ID = "snapshot-authority-v72"
...
def _record_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_adjudication", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('record_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-adjudication snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_adjudication": AUTHORITY_ADJUDICATION,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_adjudication", report.get("authority_adjudication")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority adjudication")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority adjudication")
        ...
        elif record_digest != _record_digest(record):
            errors.append(f"{prefix}.record_digest must bind authority adjudication")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority adjudication records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority adjudication records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ADJUDICATION_V189_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ADJUDICATION_V189_VALID")
    return 0
