...
try:
    from .network_snapshot_authority_takeover_seal_v233_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_seal_v233_validator import validate_snapshot
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['seal_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "seal_id": "server-authority-seal-v1",
            "authority_takeover_token": "server-authority-takeover-token-v4",
            "snapshot_id": "snapshot-authority-v116", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 5,
            "sequence": 6128, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverSealV233ValidatorTest(unittest.TestCase):
    def test_accepts_authority_seal(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_seal_binding(self):
        report = _report()
        report["records"][0]["seal_id"] = "server-authority-seal-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority seal records" in error for error in errors))
