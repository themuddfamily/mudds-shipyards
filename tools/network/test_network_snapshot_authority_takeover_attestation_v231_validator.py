...
try:
    from .network_snapshot_authority_takeover_attestation_v231_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_attestation_v231_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "attestation_id": "server-authority-attestation-v1",
            "authority_takeover_token": "server-authority-takeover-token-v2",
            "snapshot_id": "snapshot-authority-v114", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 3,
            "sequence": 4126, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverAttestationV231ValidatorTest(unittest.TestCase):
    def test_accepts_authority_attestation(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_attestation_binding(self):
        report = _report()
        report["records"][0]["attestation_id"] = "server-authority-attestation-old"
