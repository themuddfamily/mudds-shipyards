#!/usr/bin/env python3
"""Validate detached v257 network snapshot authority takeover envelope evidence."""
...
SCHEMA_VERSION = 257
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_envelope_v257"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
ENVELOPE_ID = "server-authority-envelope-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v28"
SNAPSHOT_ID = "snapshot-authority-v140"
...
def _envelope_digest(record: dict[str, Any]) -> str:
    fields = (
        "envelope_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('envelope_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for index, record in enumerate(records):
        ...
        elif envelope_digest != _envelope_digest(record):
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority envelope records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority envelope records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ENVELOPE_V257_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ENVELOPE_V257_VALID")
    return 0
