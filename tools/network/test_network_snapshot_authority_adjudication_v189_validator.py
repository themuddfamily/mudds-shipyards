...
try:
    from .network_snapshot_authority_adjudication_v189_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_adjudication_v189_validator import validate_snapshot
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['record_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_adjudication": "server-authority-adjudication-v1",
            "snapshot_id": "snapshot-authority-v72", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2961, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityAdjudicationV189ValidatorTest(unittest.TestCase):
    def test_accepts_adjudicated_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_adjudication_binding(self):
        report = _report()
        report["records"][0]["authority_adjudication"] = "server-authority-adjudication-old"
        self.assertTrue(any("bind authority adjudication" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v71"
        self.assertTrue(any("bind authority adjudication" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority adjudication records" in error for error in validate_snapshot(report)))
