...
try:
    from .network_snapshot_authority_takeover_confirmation_v269_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_confirmation_v269_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "confirmation_id": "server-authority-confirmation-v1",
            "authority_takeover_token": "server-authority-takeover-token-v40",
            "snapshot_id": "snapshot-authority-v152", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 41,
            "sequence": 42164, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverConfirmationV269ValidatorTest(unittest.TestCase):
    def test_accepts_authority_confirmation(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_confirmation_binding(self):
        report = _report()
        report["records"][0]["confirmation_id"] = "server-authority-confirmation-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority confirmation" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority confirmation records" in error for error in errors))
