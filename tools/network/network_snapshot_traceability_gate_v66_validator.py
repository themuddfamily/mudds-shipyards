#!/usr/bin/env python3
"""Validate detached v66 snapshot traceability/gate evidence."""
...
SCHEMA_VERSION = 66
EVIDENCE_SCOPE = "network_snapshot_traceability_gate_v66"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
TRACE_ID = "snapshot-trace-v1"
GATE = "passed"
...
def _trace_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "trace_id", "gate", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('trace_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()


def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for traceability/gate snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "trace_id": TRACE_ID, "gate": GATE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("trace_id", report.get("trace_id")), ("gate", report.get("gate")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match trace/gate")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("trace_id", report.get("trace_id")), ("gate", report.get("gate")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match trace/gate")
        ...
        if not _digest(trace_digest):
            errors.append(f"{prefix}.trace_digest must be lowercase SHA-256")
        elif trace_digest != _trace_digest(record):
            errors.append(f"{prefix}.trace_digest must bind trace/gate")
        if record.get("gated") is not True:
            errors.append(f"{prefix}.gated must be true")
        else:
            gated += 1
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match trace records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "gated": gated, "mutations": mutations}
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match trace records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_TRACEABILITY_GATE_V66_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_TRACEABILITY_GATE_V66_VALID")
    return 0
