...
try:
    from .network_snapshot_source_state_v83_validator import validate_snapshot
except ImportError:
    from network_snapshot_source_state_v83_validator import validate_snapshot
...
def _record(order, check_id, digest):
    record = {
        "order": order, "check_id": check_id, "authority": "server", "source": "server_snapshot", "state": "stable", "release": "release-1", "version": 2, "sequence": 420,
        "expected_digest": digest, "observed_digest": digest, "sourced": True, "mutation_fields": [], "state_changed": False,
    }
    material = "|".join(str(record[key]) for key in ("authority", "source", "state", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"))
...
def _report():
    ...
    return {
        "schema_version": 83, "evidence_scope": "network_snapshot_source_state_v83", "evidence_mode": "detached_contract_fixture", "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "source": "server_snapshot", "state": "stable", "snapshot_version": 2, "release": "release-1",
        "native_claims": False, "uses_live_network": False, "snapshot_detached": True, "no_mutation_guarantee": True,
        "snapshot": {"authority": "server", "source": "server_snapshot", "state": "stable", "release": "release-1", "version": 2, "sequence": 420, "digest": hashlib.sha256(b"snapshot").hexdigest()},
        "records": records,
...
class NetworkSnapshotSourceStateV83ValidatorTest(unittest.TestCase):
    def test_accepts_source_state_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_source_binding(self):
        report = _report()
        report["records"][0]["source"] = "client_snapshot"
        self.assertTrue(any("bind source/state" in error for error in validate_snapshot(report)))
