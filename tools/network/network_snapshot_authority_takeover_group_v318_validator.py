#!/usr/bin/env python3
"""Validate detached v318 authority/snapshot group evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v318"
SNAPSHOT_ID = "snapshot-authority-v201"
GROUP_ID = "authority-snapshot-group-v1"
...
def _group_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('group_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot group evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority group")
    ...
    items = report.get("group_members")
    if not isinstance(items, list):
        errors.append(f"{label}.group_members must be an array")
        items = []
    ids: set[str] = set()
    admitted = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.group_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority group")
        ...
        group_digest = item.get("group_digest")
        if not _digest(group_digest):
            errors.append(f"{prefix}.group_digest must be lowercase SHA-256")
        elif group_digest != _group_digest(item):
            errors.append(f"{prefix}.group_digest must bind authority group")
        if item.get("admitted") is not True:
            errors.append(f"{prefix}.admitted must be true")
        else:
            admitted += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority group members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "group_members": len(items), "unique": len(ids), "admitted": admitted, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority group members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_GROUP_V318_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_GROUP_V318_VALID")
    return 0
