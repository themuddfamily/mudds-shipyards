#!/usr/bin/env python3
"""Validate detached v238 network snapshot authority takeover proof evidence."""
...
SCHEMA_VERSION = 238
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_proof_v238"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
PROOF_ID = "server-authority-proof-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v9"
SNAPSHOT_ID = "snapshot-authority-v121"
...
def _proof_digest(record: dict[str, Any]) -> str:
    fields = (
        "proof_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('proof_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover proof evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority proof")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority proof")
        ...
        if not _digest(proof_digest):
            errors.append(f"{prefix}.proof_digest must be lowercase SHA-256")
        elif proof_digest != _proof_digest(record):
            errors.append(f"{prefix}.proof_digest must bind authority proof")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority proof records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority proof records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PROOF_V238_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PROOF_V238_VALID")
    return 0
