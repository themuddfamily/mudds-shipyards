#!/usr/bin/env python3
"""Validate detached v161 network snapshot authority-continuity evidence."""
...
SCHEMA_VERSION = 161
EVIDENCE_SCOPE = "network_snapshot_authority_continuity_v161"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_CONTINUITY = "server-authority-continuity-v1"
SNAPSHOT_ID = "snapshot-authority-v44"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_continuity", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_continuity", report.get("authority_continuity")),
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority continuity")
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority continuity")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority continuity records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority continuity records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CONTINUITY_V161_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CONTINUITY_V161_VALID")
    return 0
