#!/usr/bin/env python3
"""Validate detached v322 authority/snapshot epoch evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v322"
SNAPSHOT_ID = "snapshot-authority-v205"
EPOCH_ID = "authority-snapshot-epoch-v1"
...
def _epoch_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('epoch_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot epoch evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority epoch")
    ...
    items = report.get("epoch_members")
    if not isinstance(items, list):
        errors.append(f"{label}.epoch_members must be an array")
        items = []
    ids: set[str] = set()
    advanced = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.epoch_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority epoch")
        ...
        epoch_digest = item.get("epoch_digest")
        if not _digest(epoch_digest):
            errors.append(f"{prefix}.epoch_digest must be lowercase SHA-256")
        elif epoch_digest != _epoch_digest(item):
            errors.append(f"{prefix}.epoch_digest must bind authority epoch")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority epoch members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "epoch_members": len(items), "unique": len(ids), "advanced": advanced, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority epoch members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_EPOCH_V322_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_EPOCH_V322_VALID")
    return 0
