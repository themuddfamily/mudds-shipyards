...
try:
    from .network_snapshot_authority_takeover_boundary_v328_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_boundary_v328_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "boundary_id": "authority-snapshot-boundary-v1", "takeover_id": "authority-takeover-v328",
        "snapshot_id": "snapshot-authority-v211", "authority": "server", "sequence": 42348,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "enclosed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "boundary_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["boundary_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['boundary_digest']}" for item in items
    )
    return {
        "schema_version": 328,
        "evidence_scope": "network_snapshot_authority_takeover_boundary_v328",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v328",
        "snapshot_id": "snapshot-authority-v211", "boundary_id": "authority-snapshot-boundary-v1",
        "source": "server_snapshot", "snapshot_version": 100, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v328", "snapshot_id": "snapshot-authority-v211",
            "boundary_id": "authority-snapshot-boundary-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 100,
            "sequence": 42348, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "boundary_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"boundary_members": 2, "unique": 2, "enclosed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverBoundaryV328ValidatorTest(unittest.TestCase):
    def test_accepts_boundary_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_boundary_binding(self):
        report = _report()
        report["boundary_members"][0]["boundary_id"] = "wrong-boundary"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority boundary" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["boundary_members"][1]["order"] = 1
        report["boundary_members"][1]["item_id"] = report["boundary_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["boundary_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["enclosed"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.enclosed" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["boundary_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority boundary members" in error for error in errors))
