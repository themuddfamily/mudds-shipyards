...
try:
    from .network_snapshot_authority_takeover_slice_v320_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_slice_v320_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "slice_id": "authority-snapshot-slice-v1", "takeover_id": "authority-takeover-v320",
        "snapshot_id": "snapshot-authority-v203", "authority": "server", "sequence": 42300,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "covered": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "slice_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["slice_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['slice_digest']}" for item in items
    )
    return {
        "schema_version": 320,
        "evidence_scope": "network_snapshot_authority_takeover_slice_v320",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v320",
        "snapshot_id": "snapshot-authority-v203", "slice_id": "authority-snapshot-slice-v1",
        "source": "server_snapshot", "snapshot_version": 92, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v320", "snapshot_id": "snapshot-authority-v203",
            "slice_id": "authority-snapshot-slice-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 92,
            "sequence": 42300, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "slice_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"slice_members": 2, "unique": 2, "covered": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverSliceV320ValidatorTest(unittest.TestCase):
    def test_accepts_slice_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_slice_binding(self):
        report = _report()
        report["slice_members"][0]["slice_id"] = "wrong-slice"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority slice" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["slice_members"][1]["order"] = 1
        report["slice_members"][1]["item_id"] = report["slice_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["slice_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["covered"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.covered" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["slice_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority slice members" in error for error in errors))
