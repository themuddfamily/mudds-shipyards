...
try:
    from .network_snapshot_authority_takeover_cursor_v326_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_cursor_v326_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "cursor_id": "authority-snapshot-cursor-v1", "takeover_id": "authority-takeover-v326",
        "snapshot_id": "snapshot-authority-v209", "authority": "server", "sequence": 42336,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "positioned": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "cursor_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["cursor_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['cursor_digest']}" for item in items
    )
    return {
        "schema_version": 326,
        "evidence_scope": "network_snapshot_authority_takeover_cursor_v326",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v326",
        "snapshot_id": "snapshot-authority-v209", "cursor_id": "authority-snapshot-cursor-v1",
        "source": "server_snapshot", "snapshot_version": 98, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v326", "snapshot_id": "snapshot-authority-v209",
            "cursor_id": "authority-snapshot-cursor-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 98,
            "sequence": 42336, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "cursor_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"cursor_members": 2, "unique": 2, "positioned": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverCursorV326ValidatorTest(unittest.TestCase):
    def test_accepts_cursor_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_cursor_binding(self):
        report = _report()
        report["cursor_members"][0]["cursor_id"] = "wrong-cursor"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority cursor" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["cursor_members"][1]["order"] = 1
        report["cursor_members"][1]["item_id"] = report["cursor_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["cursor_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["positioned"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.positioned" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["cursor_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority cursor members" in error for error in errors))
