...
try:
    from .network_snapshot_authority_takeover_proof_v307_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_proof_v307_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "proof_id": "authority-snapshot-proof-v1", "takeover_id": "authority-takeover-v307",
        "snapshot_id": "snapshot-authority-v190", "authority": "server", "sequence": 42222,
...
def _report():
    ...
    return {
        ...
        "authority": "server", "takeover_id": "authority-takeover-v307",
        "snapshot_id": "snapshot-authority-v190", "proof_id": "authority-snapshot-proof-v1",
        "source": "server_snapshot", "snapshot_version": 79, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v307", "snapshot_id": "snapshot-authority-v190",
            "proof_id": "authority-snapshot-proof-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 79,
            "sequence": 42222, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "proofs": items,
...
class NetworkSnapshotAuthorityTakeoverProofV307ValidatorTest(unittest.TestCase):
    def test_accepts_proof_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_proof_binding(self):
        report = _report()
        report["proofs"][0]["proof_id"] = "wrong-proof"
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["proofs"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["proven"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.proven" in error for error in errors))
