#!/usr/bin/env python3
"""Validate detached v164 network snapshot authority-signature evidence."""
...
SCHEMA_VERSION = 164
EVIDENCE_SCOPE = "network_snapshot_authority_signature_v164"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_SIGNATURE = "server-authority-signature-v1"
SNAPSHOT_ID = "snapshot-authority-v47"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_signature", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_signature", report.get("authority_signature")),
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority signature")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority signature records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority signature records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_SIGNATURE_V164_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_SIGNATURE_V164_VALID")
    return 0
