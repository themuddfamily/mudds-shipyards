#!/usr/bin/env python3
"""Validate detached v311 authority/snapshot manifest evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v311"
SNAPSHOT_ID = "snapshot-authority-v194"
MANIFEST_ID = "authority-snapshot-manifest-v1"
...
def _manifest_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('manifest_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot manifest evidence."""
    errors: list[str] = []
    ...
    items = report.get("manifest_entries")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority manifest")
        ...
        elif manifest_digest != _manifest_digest(item):
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {
            "manifest_entries": len(items), "unique": len(ids), "listed": listed, "mutations": mutations,
        }
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MANIFEST_V311_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MANIFEST_V311_VALID")
    return 0
