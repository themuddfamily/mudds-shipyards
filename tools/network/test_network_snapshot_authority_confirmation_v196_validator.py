...
try:
    from .network_snapshot_authority_confirmation_v196_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_confirmation_v196_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["confirmation_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['confirmation_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_confirmation": "server-authority-confirmation-v1",
            "snapshot_id": "snapshot-authority-v79", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2989, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityConfirmationV196ValidatorTest(unittest.TestCase):
    def test_accepts_confirmed_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_confirmation_binding(self):
        report = _report()
        report["records"][0]["authority_confirmation"] = "server-authority-confirmation-old"
        self.assertTrue(any("bind authority confirmation" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v78"
        self.assertTrue(any("bind authority confirmation" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority confirmation records" in error for error in validate_snapshot(report)))
