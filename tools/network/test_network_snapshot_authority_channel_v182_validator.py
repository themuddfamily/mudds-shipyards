...
try:
    from .network_snapshot_authority_channel_v182_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_channel_v182_validator import validate_snapshot
...
class NetworkSnapshotAuthorityChannelV182ValidatorTest(unittest.TestCase):
    def test_accepts_authority_channel_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_channel_binding(self):
        report = _report()
        report["records"][0]["authority_channel"] = "server-authority-channel-old"
        self.assertTrue(any("bind authority channel" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v64"
        self.assertTrue(any("bind authority channel" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority channel records" in error for error in validate_snapshot(report)))
