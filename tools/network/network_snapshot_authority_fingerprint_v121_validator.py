#!/usr/bin/env python3
"""Validate detached v121 network snapshot authority-fingerprint evidence."""
...
SCHEMA_VERSION = 121
EVIDENCE_SCOPE = "network_snapshot_authority_fingerprint_v121"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_FINGERPRINT = "server-authority-fingerprint-v2"
SNAPSHOT_ID = "snapshot-authority-v4"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_fingerprint", "snapshot_id", "authority", "source", "release", "version",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-fingerprint snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_fingerprint": AUTHORITY_FINGERPRINT,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (
        ("authority_fingerprint", report.get("authority_fingerprint")), ("snapshot_id", report.get("snapshot_id")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_FINGERPRINT_V121_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_FINGERPRINT_V121_VALID")
    return 0
