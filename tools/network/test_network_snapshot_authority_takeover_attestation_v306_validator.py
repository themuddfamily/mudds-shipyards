...
try:
    from .network_snapshot_authority_takeover_attestation_v306_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_attestation_v306_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "attestation_id": "authority-snapshot-attestation-v1", "takeover_id": "authority-takeover-v306",
        "snapshot_id": "snapshot-authority-v189", "authority": "server", "sequence": 42216,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "attested": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "attestation_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["attestation_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['attestation_digest']}" for item in items
    )
    return {
        "schema_version": 306,
        "evidence_scope": "network_snapshot_authority_takeover_attestation_v306",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v306",
        "snapshot_id": "snapshot-authority-v189", "attestation_id": "authority-snapshot-attestation-v1",
        "source": "server_snapshot", "snapshot_version": 78, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v306", "snapshot_id": "snapshot-authority-v189",
            "attestation_id": "authority-snapshot-attestation-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 78,
            "sequence": 42216, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "attestations": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"attestations": 2, "unique": 2, "attested": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverAttestationV306ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_attestation_binding(self):
        report = _report()
        report["attestations"][0]["attestation_id"] = "wrong-attestation"
    ...
    def test_rejects_order_and_duplicate_identity(self):
        ...
        report["attestations"][1]["item_id"] = report["attestations"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["attestations"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["attested"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.attested" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority attestations" in error for error in errors))
