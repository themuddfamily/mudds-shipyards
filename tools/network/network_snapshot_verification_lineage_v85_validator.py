#!/usr/bin/env python3
"""Validate detached v85 snapshot verification/lineage evidence."""
...
SCHEMA_VERSION = 85
EVIDENCE_SCOPE = "network_snapshot_verification_lineage_v85"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
VERIFICATION = "snapshot-verification-v2"
LINEAGE = "snapshot-lineage-root-v2"
...
def _verification_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "verification", "lineage", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for verification/lineage snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "verification": VERIFICATION, "lineage": LINEAGE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("verification", report.get("verification")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match verification/lineage")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("verification", report.get("verification")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match verification/lineage")
        ...
        if not _digest(verification_digest):
            ...
            errors.append(f"{prefix}.verification_digest must bind verification/lineage")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_VERIFICATION_LINEAGE_V85_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_VERIFICATION_LINEAGE_V85_VALID")
    return 0
