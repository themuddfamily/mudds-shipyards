#!/usr/bin/env python3
"""Validate detached v106 snapshot consistency/state evidence."""
...
SCHEMA_VERSION = 106
EVIDENCE_SCOPE = "network_snapshot_consistency_state_v106"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
CONSISTENCY_ID = "snapshot-consistency-v4"
STATE = "stable"
...
def _consistency_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority", "consistency_id", "state", "source", "release", "version",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('consistency_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached consistency/state snapshot evidence."""
    errors: list[str] = []
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_CONSISTENCY_STATE_V106_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_CONSISTENCY_STATE_V106_VALID")
    return 0
