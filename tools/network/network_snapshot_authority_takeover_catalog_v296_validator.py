#!/usr/bin/env python3
"""Validate detached v296 network snapshot authority-takeover catalog evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v179"
...
def _catalog_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('catalog_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover catalog evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority catalog")
    ...
    items = report.get("catalog_entries")
    if not isinstance(items, list):
        errors.append(f"{label}.catalog_entries must be an array")
        items = []
    ids: set[str] = set()
    cataloged = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.catalog_entries[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority catalog")
        ...
        if not _digest(catalog_digest):
            errors.append(f"{prefix}.catalog_digest must be lowercase SHA-256")
        elif catalog_digest != _catalog_digest(item):
            errors.append(f"{prefix}.catalog_digest must bind authority catalog")
        if item.get("cataloged") is not True:
            errors.append(f"{prefix}.cataloged must be true")
        else:
            cataloged += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority catalog entries")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "entries": len(items), "unique": len(ids), "cataloged": cataloged, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority catalog entries")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CATALOG_V296_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CATALOG_V296_VALID")
    return 0
