...
try:
    from .network_snapshot_authority_channel_witness_v388_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_channel_witness_v388_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-channel-witness-assertion-v1",
        "integrity_gate_id": "authority-channel-witness-v388",
        "snapshot_id": "snapshot-authority-v271", "authority": "server", "sequence": 43688,
...
def _report():
    ...
    return {
        "schema_version": 388,
        "evidence_scope": "network_snapshot_authority_channel_witness_v388",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-channel-witness-v388",
        "snapshot_id": "snapshot-authority-v271",
        "assertion_id": "authority-snapshot-channel-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 160, "release": "release-1",
...
class NetworkSnapshotAuthorityChannelWitnessV388ValidatorTest(unittest.TestCase):
    def test_accepts_channel_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_channel_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-channel"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority channel witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority channel members" in error for error in errors))
