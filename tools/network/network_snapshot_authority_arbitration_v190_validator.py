#!/usr/bin/env python3
"""Validate detached v190 network snapshot authority-arbitration evidence."""
...
SCHEMA_VERSION = 190
EVIDENCE_SCOPE = "network_snapshot_authority_arbitration_v190"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ARBITRATION = "server-authority-arbitration-v1"
SNAPSHOT_ID = "snapshot-authority-v73"
...
def _arbitration_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_arbitration", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('arbitration_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-arbitration snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_arbitration", report.get("authority_arbitration")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority arbitration")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority arbitration")
        ...
        arbitration_digest = record.get("arbitration_digest")
        if not _digest(arbitration_digest):
            errors.append(f"{prefix}.arbitration_digest must be lowercase SHA-256")
        elif arbitration_digest != _arbitration_digest(record):
            errors.append(f"{prefix}.arbitration_digest must bind authority arbitration")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority arbitration records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority arbitration records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ARBITRATION_V190_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ARBITRATION_V190_VALID")
    return 0
