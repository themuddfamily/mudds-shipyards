...
try:
    from .network_snapshot_authority_anchor_witness_v402_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_anchor_witness_v402_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-anchor-witness-assertion-v1",
        "integrity_gate_id": "authority-anchor-witness-v402",
        "snapshot_id": "snapshot-authority-v285", "authority": "server", "sequence": 45102,
...
def _report():
    ...
    return {
        "schema_version": 402,
        "evidence_scope": "network_snapshot_authority_anchor_witness_v402",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-anchor-witness-v402",
        "snapshot_id": "snapshot-authority-v285",
        "assertion_id": "authority-snapshot-anchor-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 174, "release": "release-1",
...
class NetworkSnapshotAuthorityAnchorWitnessV402ValidatorTest(unittest.TestCase):
    def test_accepts_anchor_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_anchor_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-anchor"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority anchor witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["anchor"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority anchor members" in error for error in errors))
