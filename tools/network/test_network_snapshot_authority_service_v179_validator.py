...
try:
    from .network_snapshot_authority_service_v179_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_service_v179_validator import validate_snapshot
...
class NetworkSnapshotAuthorityServiceV179ValidatorTest(unittest.TestCase):
    def test_accepts_authority_service_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_service_binding(self):
        report = _report()
        report["records"][0]["authority_service"] = "server-authority-service-old"
        self.assertTrue(any("bind authority service" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v61"
        self.assertTrue(any("bind authority service" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority service records" in error for error in validate_snapshot(report)))
