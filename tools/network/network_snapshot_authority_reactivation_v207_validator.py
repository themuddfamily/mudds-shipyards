#!/usr/bin/env python3
"""Validate detached v207 network snapshot authority-reactivation evidence."""
...
SCHEMA_VERSION = 207
EVIDENCE_SCOPE = "network_snapshot_authority_reactivation_v207"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_REACTIVATION = "server-authority-reactivation-v1"
SNAPSHOT_ID = "snapshot-authority-v90"
...
def _reactivation_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_reactivation", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('reactivation_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-reactivation snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_reactivation": AUTHORITY_REACTIVATION,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_reactivation", report.get("authority_reactivation")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority reactivation")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority reactivation")
        ...
        reactivation_digest = record.get("reactivation_digest")
        if not _digest(reactivation_digest):
            errors.append(f"{prefix}.reactivation_digest must be lowercase SHA-256")
        elif reactivation_digest != _reactivation_digest(record):
            errors.append(f"{prefix}.reactivation_digest must bind authority reactivation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority reactivation records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority reactivation records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_REACTIVATION_V207_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_REACTIVATION_V207_VALID")
    return 0
