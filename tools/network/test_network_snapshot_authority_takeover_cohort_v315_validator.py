...
try:
    from .network_snapshot_authority_takeover_cohort_v315_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_cohort_v315_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "cohort_id": "authority-snapshot-cohort-v1", "takeover_id": "authority-takeover-v315",
        "snapshot_id": "snapshot-authority-v198", "authority": "server", "sequence": 42270,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "joined": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "cohort_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["cohort_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['cohort_digest']}" for item in items
    )
    return {
        "schema_version": 315,
        "evidence_scope": "network_snapshot_authority_takeover_cohort_v315",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v315",
        "snapshot_id": "snapshot-authority-v198", "cohort_id": "authority-snapshot-cohort-v1",
        "source": "server_snapshot", "snapshot_version": 87, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v315", "snapshot_id": "snapshot-authority-v198",
            "cohort_id": "authority-snapshot-cohort-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 87,
            "sequence": 42270, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "cohort_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"cohort_members": 2, "unique": 2, "joined": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverCohortV315ValidatorTest(unittest.TestCase):
    def test_accepts_cohort_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_cohort_binding(self):
        report = _report()
        report["cohort_members"][0]["cohort_id"] = "wrong-cohort"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority cohort" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["cohort_members"][1]["order"] = 1
        report["cohort_members"][1]["item_id"] = report["cohort_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["cohort_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["joined"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.joined" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["cohort_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority cohort members" in error for error in errors))
