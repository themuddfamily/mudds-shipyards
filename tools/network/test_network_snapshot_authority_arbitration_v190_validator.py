...
try:
    from .network_snapshot_authority_arbitration_v190_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_arbitration_v190_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["arbitration_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['arbitration_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_arbitration": "server-authority-arbitration-v1",
            "snapshot_id": "snapshot-authority-v73", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2965, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityArbitrationV190ValidatorTest(unittest.TestCase):
    def test_accepts_arbitrated_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_arbitration_binding(self):
        report = _report()
        report["records"][0]["authority_arbitration"] = "server-authority-arbitration-old"
        self.assertTrue(any("bind authority arbitration" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v72"
        self.assertTrue(any("bind authority arbitration" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority arbitration records" in error for error in validate_snapshot(report)))
