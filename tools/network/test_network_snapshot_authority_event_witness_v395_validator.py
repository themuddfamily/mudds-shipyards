...
try:
    from .network_snapshot_authority_event_witness_v395_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_event_witness_v395_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-event-witness-assertion-v1",
        "integrity_gate_id": "authority-event-witness-v395",
        "snapshot_id": "snapshot-authority-v278", "authority": "server", "sequence": 44395,
...
def _report():
    ...
    return {
        "schema_version": 395,
        "evidence_scope": "network_snapshot_authority_event_witness_v395",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-event-witness-v395",
        "snapshot_id": "snapshot-authority-v278",
        "assertion_id": "authority-snapshot-event-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 167, "release": "release-1",
...
class NetworkSnapshotAuthorityEventWitnessV395ValidatorTest(unittest.TestCase):
    def test_accepts_event_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_event_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["integrity_gate_id"] = "wrong-event"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority event witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["event"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority event members" in error for error in errors))
