#!/usr/bin/env python3
"""Validate detached v278 network snapshot authority-takeover proof evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v161"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for index, item in enumerate(items):
        ...
        if not isinstance(item.get("assertion"), str) or not item["assertion"]:
            errors.append(f"{prefix}.assertion must be non-empty")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PROOF_V278_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PROOF_V278_VALID")
    return 0
