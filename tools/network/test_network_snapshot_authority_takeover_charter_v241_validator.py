...
try:
    from .network_snapshot_authority_takeover_charter_v241_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_charter_v241_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["charter_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['charter_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "charter_id": "server-authority-charter-v1",
            "authority_takeover_token": "server-authority-takeover-token-v12",
            "snapshot_id": "snapshot-authority-v124", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 13,
            "sequence": 14136, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverCharterV241ValidatorTest(unittest.TestCase):
    def test_accepts_authority_charter(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_charter_binding(self):
        report = _report()
        report["records"][0]["charter_id"] = "server-authority-charter-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority charter" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority charter records" in error for error in errors))
