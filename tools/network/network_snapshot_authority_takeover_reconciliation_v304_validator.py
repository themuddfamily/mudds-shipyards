#!/usr/bin/env python3
"""Validate detached v304 snapshot/authority takeover reconciliation evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v304"
...
def _reconciliation_digest(item: dict[str, Any]) -> str:
    fields = (
        "takeover_id", "snapshot_id", "sequence", "subject",
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('reconciliation_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached snapshot/authority reconciliation evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority reconciliation")
    ...
    items = report.get("reconciliations")
    if not isinstance(items, list):
        errors.append(f"{label}.reconciliations must be an array")
        items = []
    ids: set[str] = set()
    matched = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.reconciliations[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority reconciliation")
        ...
        reconciliation_digest = item.get("reconciliation_digest")
        if not _digest(reconciliation_digest):
            errors.append(f"{prefix}.reconciliation_digest must be lowercase SHA-256")
        elif reconciliation_digest != _reconciliation_digest(item):
            errors.append(f"{prefix}.reconciliation_digest must bind authority reconciliation")
        if authority_digest == snapshot_digest and _digest(authority_digest):
            matched += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority reconciliations")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "reconciliations": len(items), "unique": len(ids),
            "matched": matched, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority reconciliations")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RECONCILIATION_V304_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RECONCILIATION_V304_VALID")
    return 0
