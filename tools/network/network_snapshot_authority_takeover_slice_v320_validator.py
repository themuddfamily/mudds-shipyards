#!/usr/bin/env python3
"""Validate detached v320 authority/snapshot slice evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v320"
SNAPSHOT_ID = "snapshot-authority-v203"
SLICE_ID = "authority-snapshot-slice-v1"
...
def _slice_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('slice_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot slice evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority slice")
    ...
    items = report.get("slice_members")
    if not isinstance(items, list):
        errors.append(f"{label}.slice_members must be an array")
        items = []
    ids: set[str] = set()
    covered = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.slice_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority slice")
        ...
        slice_digest = item.get("slice_digest")
        if not _digest(slice_digest):
            errors.append(f"{prefix}.slice_digest must be lowercase SHA-256")
        elif slice_digest != _slice_digest(item):
            errors.append(f"{prefix}.slice_digest must bind authority slice")
        if item.get("covered") is not True:
            errors.append(f"{prefix}.covered must be true")
        else:
            covered += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority slice members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "slice_members": len(items), "unique": len(ids), "covered": covered, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority slice members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SLICE_V320_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SLICE_V320_VALID")
    return 0
