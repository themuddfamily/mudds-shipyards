#!/usr/bin/env python3
"""Validate detached v220 network snapshot authority-takeover claim evidence."""
...
SCHEMA_VERSION = 220
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_claim_v220"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_CLAIM = "server-authority-takeover-claim-v1"
SNAPSHOT_ID = "snapshot-authority-v103"
...
def _claim_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_claim", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('claim_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover claim evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_claim": AUTHORITY_TAKEOVER_CLAIM,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_claim", report.get("authority_takeover_claim")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover claim")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover claim")
        ...
        if not _digest(claim_digest):
            errors.append(f"{prefix}.claim_digest must be lowercase SHA-256")
        elif claim_digest != _claim_digest(record):
            errors.append(f"{prefix}.claim_digest must bind authority takeover claim")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover claim records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover claim records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CLAIM_V220_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CLAIM_V220_VALID")
    return 0
