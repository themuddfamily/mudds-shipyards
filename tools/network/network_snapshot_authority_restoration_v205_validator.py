#!/usr/bin/env python3
"""Validate detached v205 network snapshot authority-restoration evidence."""
...
SCHEMA_VERSION = 205
EVIDENCE_SCOPE = "network_snapshot_authority_restoration_v205"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_RESTORATION = "server-authority-restoration-v1"
SNAPSHOT_ID = "snapshot-authority-v88"
...
def _restoration_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_restoration", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('restoration_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-restoration snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_restoration": AUTHORITY_RESTORATION,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_restoration", report.get("authority_restoration")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority restoration")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority restoration")
        ...
        restoration_digest = record.get("restoration_digest")
        if not _digest(restoration_digest):
            errors.append(f"{prefix}.restoration_digest must be lowercase SHA-256")
        elif restoration_digest != _restoration_digest(record):
            errors.append(f"{prefix}.restoration_digest must bind authority restoration")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority restoration records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority restoration records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_RESTORATION_V205_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_RESTORATION_V205_VALID")
    return 0
