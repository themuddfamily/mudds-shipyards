#!/usr/bin/env python3
"""Validate detached v53 snapshot release/version reconciliation evidence."""
...
SCHEMA_VERSION = 53
EVIDENCE_SCOPE = "network_snapshot_release_version_reconciliation_v53"
...
def _reconciliation_digest(check: dict[str, Any]) -> str:
    material = "|".join(
        str(check.get(key))
        for key in ("authority", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest")
    )
...
def _aggregate(checks: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{check.get('order')}|{check.get('check_id')}|{check.get('reconciliation_digest')}"
        for check in checks
    )
    return hashlib.sha256(material.encode("utf-8")).hexdigest()
...
def validate_checks(report: Any, label: str = "checks") -> list[str]:
    """Return errors for release/version reconciliation evidence."""
    ...
    for index, check in enumerate(checks):
        ...
        if not _digest(reconciliation_digest):
            ...
            errors.append(f"{prefix}.reconciliation_digest must bind release/version")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match reconciliations")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match reconciliations")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_RELEASE_VERSION_RECONCILIATION_V53_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_RELEASE_VERSION_RECONCILIATION_V53_VALID")
    return 0
