#!/usr/bin/env python3
"""Validate detached v414 authority/snapshot evidence-reconciliation evidence."""
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot evidence-reconciliation evidence."""
    errors: list[str] = []
    ...
    for key, value in (("integrity_gate_id", report.get("integrity_gate_id")),
        ...
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority evidence reconciliation")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (("assertion_id", report.get("assertion_id")),
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority evidence reconciliation")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority evidence reconciliation")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_RECONCILIATION_V414_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_RECONCILIATION_V414_VALID")
    return 0
