#!/usr/bin/env python3
"""Validate detached v64 snapshot verification/attestation evidence."""
...
SCHEMA_VERSION = 64
EVIDENCE_SCOPE = "network_snapshot_verification_attestation_v64"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
VERIFICATION = "snapshot-verification-v1"
ATTESTATION = "server-attestation-v1"
...
def _verification_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "verification", "attestation", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('verification_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()


def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for verification/attestation snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "verification": VERIFICATION, "attestation": ATTESTATION,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("verification", report.get("verification")), ("attestation", report.get("attestation")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match verification/attestation")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("verification", report.get("verification")), ("attestation", report.get("attestation")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match verification/attestation")
        ...
        if not _digest(verification_digest):
            errors.append(f"{prefix}.verification_digest must be lowercase SHA-256")
        elif verification_digest != _verification_digest(record):
            errors.append(f"{prefix}.verification_digest must bind verification/attestation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match verification records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "verified": verified, "mutations": mutations}
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match verification records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_VERIFICATION_ATTESTATION_V64_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_VERIFICATION_ATTESTATION_V64_VALID")
    return 0
