...
try:
    from .network_snapshot_authority_takeover_mandate_v242_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_mandate_v242_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["mandate_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['mandate_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "mandate_id": "server-authority-mandate-v1",
            "authority_takeover_token": "server-authority-takeover-token-v13",
            "snapshot_id": "snapshot-authority-v125", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 14,
            "sequence": 15137, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverMandateV242ValidatorTest(unittest.TestCase):
    def test_accepts_authority_mandate(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_mandate_binding(self):
        report = _report()
        report["records"][0]["mandate_id"] = "server-authority-mandate-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority mandate" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority mandate records" in error for error in errors))
