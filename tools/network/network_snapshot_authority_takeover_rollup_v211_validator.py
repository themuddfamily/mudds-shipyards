#!/usr/bin/env python3
"""Validate detached v211 network snapshot authority-takeover rollup evidence."""
...
SCHEMA_VERSION = 211
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_rollup_v211"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_ROLLUP = "server-authority-takeover-rollup-v1"
SNAPSHOT_ID = "snapshot-authority-v94"
...
def _rollup_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_rollup", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('rollup_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover rollup snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_rollup": AUTHORITY_TAKEOVER_ROLLUP,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_rollup", report.get("authority_takeover_rollup")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover rollup")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover rollup")
        ...
        rollup_digest = record.get("rollup_digest")
        if not _digest(rollup_digest):
            errors.append(f"{prefix}.rollup_digest must be lowercase SHA-256")
        elif rollup_digest != _rollup_digest(record):
            errors.append(f"{prefix}.rollup_digest must bind authority takeover rollup")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover rollup records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover rollup records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ROLLUP_V211_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ROLLUP_V211_VALID")
    return 0
