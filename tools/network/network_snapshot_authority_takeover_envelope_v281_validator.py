#!/usr/bin/env python3
"""Validate detached v281 network snapshot authority-takeover envelope evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v164"
...
def _envelope_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('envelope_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover envelope evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("envelope_id", report.get("envelope_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority envelope")
    ...
    items = report.get("envelope_items")
    if not isinstance(items, list):
        errors.append(f"{label}.envelope_items must be an array")
        items = []
    ids: set[str] = set()
    wrapped = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.envelope_items[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority envelope")
        ...
        if not isinstance(item.get("channel"), str) or not item["channel"]:
            errors.append(f"{prefix}.channel must be non-empty")
        ...
        if not _digest(envelope_digest):
            errors.append(f"{prefix}.envelope_digest must be lowercase SHA-256")
        elif envelope_digest != _envelope_digest(item):
            errors.append(f"{prefix}.envelope_digest must bind authority envelope")
        if item.get("wrapped") is not True:
            errors.append(f"{prefix}.wrapped must be true")
        else:
            wrapped += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority envelope items")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "items": len(items), "unique": len(ids), "wrapped": wrapped, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority envelope items")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ENVELOPE_V281_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ENVELOPE_V281_VALID")
    return 0
