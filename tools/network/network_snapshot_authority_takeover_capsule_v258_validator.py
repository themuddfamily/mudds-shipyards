#!/usr/bin/env python3
"""Validate detached v258 network snapshot authority takeover capsule evidence."""
...
SCHEMA_VERSION = 258
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_capsule_v258"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
CAPSULE_ID = "server-authority-capsule-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v29"
SNAPSHOT_ID = "snapshot-authority-v141"
...
def _capsule_digest(record: dict[str, Any]) -> str:
    fields = (
        "capsule_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('capsule_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover capsule evidence."""
    errors: list[str] = []
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority capsule")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority capsule records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority capsule records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CAPSULE_V258_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CAPSULE_V258_VALID")
    return 0
