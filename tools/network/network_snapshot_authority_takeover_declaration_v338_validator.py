#!/usr/bin/env python3
"""Validate detached v338 authority/snapshot declaration evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v338"
SNAPSHOT_ID = "snapshot-authority-v221"
DECLARATION_ID = "authority-snapshot-declaration-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 110
...
def _declaration_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('declaration_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot declaration evidence."""
    errors: list[str] = []
    ...
    snapshot_fields = (
        ...
        ("declaration_id", report.get("declaration_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority declaration")
    ...
    items = report.get("declaration_members")
    if not isinstance(items, list):
        errors.append(f"{label}.declaration_members must be an array")
        items = []
    ids: set[str] = set()
    declared = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.declaration_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority declaration")
        ...
        declaration_digest = item.get("declaration_digest")
        if not _digest(declaration_digest):
            errors.append(f"{prefix}.declaration_digest must be lowercase SHA-256")
        elif declaration_digest != _declaration_digest(item):
            errors.append(f"{prefix}.declaration_digest must bind authority declaration")
        if item.get("declared") is not True:
            errors.append(f"{prefix}.declared must be true")
        else:
            declared += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority declaration members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "declaration_members": len(items), "unique": len(ids), "declared": declared, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority declaration members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_DECLARATION_V338_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_DECLARATION_V338_VALID")
    return 0
