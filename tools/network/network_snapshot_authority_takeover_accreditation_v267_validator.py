#!/usr/bin/env python3
"""Validate detached v267 network snapshot authority takeover accreditation evidence."""
...
SCHEMA_VERSION = 267
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_accreditation_v267"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
ACCREDITATION_ID = "server-authority-accreditation-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v38"
SNAPSHOT_ID = "snapshot-authority-v150"
...
def _accreditation_digest(record: dict[str, Any]) -> str:
    fields = (
        "accreditation_id", "authority_takeover_token", "snapshot_id", "authority", "source",
        "release", "version", "check_id", "sequence", "expected_digest",
        "observed_digest",
    )
    return hashlib.sha256(
        "|".join(str(record.get(field)) for field in fields).encode()
    ).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('accreditation_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover accreditation evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority_takeover_token": AUTHORITY_TAKEOVER_TOKEN,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("accreditation_id", report.get("accreditation_id")),
        ("authority_takeover_token", report.get("authority_takeover_token")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority accreditation")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority accreditation")
        ...
        accreditation_digest = record.get("accreditation_digest")
        if not _digest(accreditation_digest):
            errors.append(f"{prefix}.accreditation_digest must be lowercase SHA-256")
        elif accreditation_digest != _accreditation_digest(record):
            errors.append(f"{prefix}.accreditation_digest must bind authority accreditation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority accreditation records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "records": len(records), "unique": len(ids),
            "authorized": authorized, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority accreditation records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ACCREDITATION_V267_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ACCREDITATION_V267_VALID")
    return 0
