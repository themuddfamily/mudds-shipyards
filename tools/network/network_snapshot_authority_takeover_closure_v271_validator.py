#!/usr/bin/env python3
"""Validate detached v271 network snapshot authority-takeover closure evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v154"
...
def _rollup_digest(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('record_id')}|{record.get('closure_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("closure_id", report.get("closure_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority closure")
    ...
    records = report.get("closure_records")
    if not isinstance(records, list):
        errors.append(f"{label}.closure_records must be an array")
        records = []
    ids: set[str] = set()
    closed = mutations = 0
    for index, record in enumerate(records):
        prefix = f"{label}.closure_records[{index}]"
        ...
        for key, value in (
            ...
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority closure")
        ...
        if not _digest(closure_digest):
            ...
            errors.append(f"{prefix}.closure_digest must bind authority closure")
        if record.get("closed") is not True:
            errors.append(f"{prefix}.closed must be true")
        else:
            closed += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority closure records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "records": len(records), "unique": len(ids), "closed": closed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority closure records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CLOSURE_V271_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CLOSURE_V271_VALID")
    return 0
