#!/usr/bin/env python3
"""Validate detached v336 authority/snapshot notice evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v336"
SNAPSHOT_ID = "snapshot-authority-v219"
NOTICE_ID = "authority-snapshot-notice-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 108
...
def _notice_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('notice_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot notice evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority notice")
    ...
    items = report.get("notice_members")
    if not isinstance(items, list):
        errors.append(f"{label}.notice_members must be an array")
        items = []
    ids: set[str] = set()
    posted = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.notice_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority notice")
        ...
        notice_digest = item.get("notice_digest")
        if not _digest(notice_digest):
            errors.append(f"{prefix}.notice_digest must be lowercase SHA-256")
        elif notice_digest != _notice_digest(item):
            errors.append(f"{prefix}.notice_digest must bind authority notice")
        if item.get("posted") is not True:
            errors.append(f"{prefix}.posted must be true")
        else:
            posted += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority notice members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "notice_members": len(items), "unique": len(ids), "posted": posted, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority notice members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_NOTICE_V336_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_NOTICE_V336_VALID")
    return 0
