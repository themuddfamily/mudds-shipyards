#!/usr/bin/env python3
"""Validate v49 versioned authority/reconciliation evidence."""
...
def _record_digest(record: dict[str, Any]) -> str:
    return hashlib.sha256(f"{record.get('authority')}|{record.get('version')}|{record.get('record_id')}|{record.get('expected_digest')}|{record.get('observed_digest')}".encode("utf-8")).hexdigest()
...
def validate_versioned(report: Any, label: str = "versioned") -> list[str]:
    """Return versioned state, record reconciliation, aggregate, count, and mutation errors."""
    ...
    for name, state in (("before", before), ("after", after)):
        ...
        elif state.get("authority") != AUTHORITY or state.get("version") != SNAPSHOT_VERSION:
            errors.append(f"{label}.{name} authority/version must match contract")
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(reconciliation_digest):
            ...
            errors.append(f"{prefix}.reconciliation_digest must bind snapshot version")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match versioned records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match versioned records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_VERSIONED_AUTHORITY_RECONCILIATION_V49_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_VERSIONED_AUTHORITY_RECONCILIATION_V49_VALID")
    return 0
