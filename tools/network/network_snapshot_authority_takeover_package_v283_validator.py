#!/usr/bin/env python3
"""Validate detached v283 network snapshot authority-takeover package evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v166"
...
def _package_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('package_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover package evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority package")
    ...
    items = report.get("package_items")
    if not isinstance(items, list):
        errors.append(f"{label}.package_items must be an array")
        items = []
    ids: set[str] = set()
    packed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.package_items[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority package")
        ...
        if not _digest(package_digest):
            errors.append(f"{prefix}.package_digest must be lowercase SHA-256")
        elif package_digest != _package_digest(item):
            errors.append(f"{prefix}.package_digest must bind authority package")
        if item.get("packed") is not True:
            errors.append(f"{prefix}.packed must be true")
        else:
            packed += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority package items")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "items": len(items), "unique": len(ids), "packed": packed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority package items")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PACKAGE_V283_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PACKAGE_V283_VALID")
    return 0
