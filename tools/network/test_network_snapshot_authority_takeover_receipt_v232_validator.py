...
try:
    from .network_snapshot_authority_takeover_receipt_v232_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_receipt_v232_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "receipt_id": "server-authority-receipt-v1",
            "authority_takeover_token": "server-authority-takeover-token-v3",
            "snapshot_id": "snapshot-authority-v115", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 4,
            "sequence": 5127, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverReceiptV232ValidatorTest(unittest.TestCase):
    def test_accepts_authority_receipt(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_receipt_binding(self):
        report = _report()
        report["records"][0]["receipt_id"] = "server-authority-receipt-old"
