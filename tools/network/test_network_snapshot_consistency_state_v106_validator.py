...
try:
    from .network_snapshot_consistency_state_v106_validator import validate_snapshot
except ImportError:
    from network_snapshot_consistency_state_v106_validator import validate_snapshot
...
def _report():
    ...
    aggregate = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['consistency_digest']}"
...
class NetworkSnapshotConsistencyStateV106ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_consistency_binding(self):
        report = _report()
        report["records"][0]["consistency_id"] = "snapshot-consistency-v3"
