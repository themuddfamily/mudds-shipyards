#!/usr/bin/env python3
"""Validate detached stale snapshot digest no-mutation sequence evidence."""
...
REQUIRED = {"stale_digest", "stale_sequence", "digest_sequence_mismatch"}
...
def validate_digest_guard(report: Any, label: str = "guard") -> list[str]:
    """Return digest-first stale rejection and sequence preservation errors."""
    ...
    for index, attempt in enumerate(attempts):
        ...
        if kind not in REQUIRED:
            errors.append(f"{prefix}.kind is not a required digest guard")
        else:
            seen.add(kind)
        if attempt.get("accepted") is not False or attempt.get("status") != "stale_snapshot_digest" or attempt.get("server_rejected") is not True:
            errors.append(f"{prefix} must be stale_snapshot_digest rejected")
        if attempt.get("state_changed") is not False or attempt.get("mutation_fields") != []:
            errors.append(f"{prefix} must have no mutation")
        if attempt.get("after_digest") != current.get("digest") or attempt.get("after_sequence") != current.get("sequence") or attempt.get("after_peer_generation") != current.get("peer_generation"):
            errors.append(f"{prefix} must preserve current digest, sequence, and generation")
        if kind == "stale_digest" and attempt.get("attempted_digest") == current.get("digest"):
            errors.append(f"{prefix}.attempted_digest must be stale")
        if kind in {"stale_sequence", "digest_sequence_mismatch"} and attempt.get("attempted_sequence", current["sequence"]) >= current["sequence"]:
...
def validate_digest_guard_file(report_path: Path) -> list[str]:
    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return [f"unable to read {report_path}: {exc}"]
    return validate_digest_guard(report, str(report_path))
...
def main() -> int:
    ...
    errors = validate_digest_guard_file(args.guard)
    if errors:
        print("NETWORK_STALE_SNAPSHOT_DIGEST_NO_MUTATION_SEQUENCE_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_STALE_SNAPSHOT_DIGEST_NO_MUTATION_SEQUENCE_VALID")
    return 0
