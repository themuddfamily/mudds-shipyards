#!/usr/bin/env python3
"""Validate detached v337 authority/snapshot announcement evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v337"
SNAPSHOT_ID = "snapshot-authority-v220"
ANNOUNCEMENT_ID = "authority-snapshot-announcement-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 109
...
def _announcement_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('announcement_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot announcement evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("takeover_id", report.get("takeover_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority announcement")
    ...
    items = report.get("announcement_members")
    if not isinstance(items, list):
        errors.append(f"{label}.announcement_members must be an array")
        items = []
    ids: set[str] = set()
    announced = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.announcement_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority announcement")
        ...
        announcement_digest = item.get("announcement_digest")
        if not _digest(announcement_digest):
            errors.append(f"{prefix}.announcement_digest must be lowercase SHA-256")
        elif announcement_digest != _announcement_digest(item):
            errors.append(f"{prefix}.announcement_digest must bind authority announcement")
        if item.get("announced") is not True:
            errors.append(f"{prefix}.announced must be true")
        else:
            announced += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority announcement members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "announcement_members": len(items), "unique": len(ids), "announced": announced, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority announcement members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ANNOUNCEMENT_V337_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ANNOUNCEMENT_V337_VALID")
    return 0
