...
try:
    from .network_snapshot_authority_domain_v174_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_domain_v174_validator import validate_snapshot
...
class NetworkSnapshotAuthorityDomainV174ValidatorTest(unittest.TestCase):
    def test_accepts_authority_domain_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_domain_binding(self):
        report = _report()
        report["records"][0]["authority_domain"] = "server-authority-domain-old"
        self.assertTrue(any("bind authority domain" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v56"
        self.assertTrue(any("bind authority domain" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority domain records" in error for error in validate_snapshot(report)))
