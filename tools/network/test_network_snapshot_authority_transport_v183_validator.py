...
try:
    from .network_snapshot_authority_transport_v183_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_transport_v183_validator import validate_snapshot
...
class NetworkSnapshotAuthorityTransportV183ValidatorTest(unittest.TestCase):
    def test_accepts_authority_transport_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_transport_binding(self):
        report = _report()
        report["records"][0]["authority_transport"] = "server-authority-transport-old"
        self.assertTrue(any("bind authority transport" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v65"
        self.assertTrue(any("bind authority transport" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority transport records" in error for error in validate_snapshot(report)))
