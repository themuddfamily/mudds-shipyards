...
try:
    from .network_snapshot_authority_settlement_v192_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_settlement_v192_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["settlement_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['settlement_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_settlement": "server-authority-settlement-v1",
            "snapshot_id": "snapshot-authority-v75", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2973, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthoritySettlementV192ValidatorTest(unittest.TestCase):
    def test_accepts_settled_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_settlement_binding(self):
        report = _report()
        report["records"][0]["authority_settlement"] = "server-authority-settlement-old"
        self.assertTrue(any("bind authority settlement" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v74"
        self.assertTrue(any("bind authority settlement" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority settlement records" in error for error in validate_snapshot(report)))
