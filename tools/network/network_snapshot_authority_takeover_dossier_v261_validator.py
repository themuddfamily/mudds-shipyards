#!/usr/bin/env python3
"""Validate detached v261 network snapshot authority takeover dossier evidence."""
...
SCHEMA_VERSION = 261
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_dossier_v261"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
DOSSIER_ID = "server-authority-dossier-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v32"
SNAPSHOT_ID = "snapshot-authority-v144"
...
def _dossier_digest(record: dict[str, Any]) -> str:
    fields = (
        "dossier_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('dossier_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover dossier evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("dossier_id", report.get("dossier_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority dossier")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority dossier")
        ...
        dossier_digest = record.get("dossier_digest")
        if not _digest(dossier_digest):
            errors.append(f"{prefix}.dossier_digest must be lowercase SHA-256")
        elif dossier_digest != _dossier_digest(record):
            errors.append(f"{prefix}.dossier_digest must bind authority dossier")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority dossier records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority dossier records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_DOSSIER_V261_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_DOSSIER_V261_VALID")
    return 0
