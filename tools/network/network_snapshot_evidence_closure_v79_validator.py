#!/usr/bin/env python3
"""Validate detached v79 snapshot evidence/closure evidence."""
...
SCHEMA_VERSION = 79
EVIDENCE_SCOPE = "network_snapshot_evidence_closure_v79"
...
def _evidence_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "evidence_id", "closure", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for evidence/closure snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "evidence_id": EVIDENCE_ID, "closure": CLOSURE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("evidence_id", report.get("evidence_id")), ("closure", report.get("closure")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match evidence/closure")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("evidence_id", report.get("evidence_id")), ("closure", report.get("closure")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match evidence/closure")
        ...
        if not _digest(evidence_digest):
            ...
            errors.append(f"{prefix}.evidence_digest must bind evidence/closure")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_EVIDENCE_CLOSURE_V79_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_EVIDENCE_CLOSURE_V79_VALID")
    return 0
