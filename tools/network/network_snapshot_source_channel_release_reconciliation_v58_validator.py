#!/usr/bin/env python3
"""Validate detached v58 snapshot source/channel/release reconciliation evidence."""
...
SCHEMA_VERSION = 58
EVIDENCE_SCOPE = "network_snapshot_source_channel_release_reconciliation_v58"
...
def _reconciliation_digest(check: dict[str, Any]) -> str:
    material = "|".join(str(check.get(key)) for key in (
        "authority", "source", "channel", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_SOURCE_CHANNEL_RELEASE_RECONCILIATION_V58_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_SOURCE_CHANNEL_RELEASE_RECONCILIATION_V58_VALID")
    return 0
