#!/usr/bin/env python3
"""Validate detached v244 network snapshot authority takeover decree evidence."""
...
SCHEMA_VERSION = 244
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_decree_v244"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
DECREE_ID = "server-authority-decree-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v15"
SNAPSHOT_ID = "snapshot-authority-v127"
...
def _decree_digest(record: dict[str, Any]) -> str:
    fields = (
        "decree_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('decree_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover decree evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("decree_id", report.get("decree_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority decree")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority decree")
        ...
        decree_digest = record.get("decree_digest")
        if not _digest(decree_digest):
            errors.append(f"{prefix}.decree_digest must be lowercase SHA-256")
        elif decree_digest != _decree_digest(record):
            errors.append(f"{prefix}.decree_digest must bind authority decree")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority decree records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority decree records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_DECREE_V244_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_DECREE_V244_VALID")
    return 0
