#!/usr/bin/env python3
"""Validate detached v362 authority/snapshot validation evidence."""
...
AUTHORITY = "server"
VALIDATION_ID = "authority-validation-v362"
SNAPSHOT_ID = "snapshot-authority-v245"
ASSERTION_ID = "authority-snapshot-validation-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 134
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot validation evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority validation")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority validation")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority validation")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority validation members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority validation members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_VALIDATION_V362_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_VALIDATION_V362_VALID")
    return 0
