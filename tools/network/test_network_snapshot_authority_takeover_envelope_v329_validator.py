...
try:
    from .network_snapshot_authority_takeover_envelope_v329_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_envelope_v329_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "envelope_id": "authority-snapshot-envelope-v1", "takeover_id": "authority-takeover-v329",
        "snapshot_id": "snapshot-authority-v212", "authority": "server", "sequence": 42354,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "sealed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "envelope_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["envelope_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['envelope_digest']}" for item in items
    )
    return {
        "schema_version": 329,
        "evidence_scope": "network_snapshot_authority_takeover_envelope_v329",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v329",
        "snapshot_id": "snapshot-authority-v212", "envelope_id": "authority-snapshot-envelope-v1",
        "source": "server_snapshot", "snapshot_version": 101, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v329", "snapshot_id": "snapshot-authority-v212",
            "envelope_id": "authority-snapshot-envelope-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 101,
            "sequence": 42354, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "envelope_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"envelope_members": 2, "unique": 2, "sealed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverEnvelopeV329ValidatorTest(unittest.TestCase):
    def test_accepts_envelope_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_envelope_binding(self):
        report = _report()
        report["envelope_members"][0]["envelope_id"] = "wrong-envelope"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority envelope" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["envelope_members"][1]["order"] = 1
        report["envelope_members"][1]["item_id"] = report["envelope_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["envelope_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["sealed"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.sealed" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["envelope_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority envelope members" in error for error in errors))
