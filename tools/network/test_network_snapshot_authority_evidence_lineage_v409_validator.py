...
try:
    from .network_snapshot_authority_evidence_lineage_v409_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_evidence_lineage_v409_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "snapshot_id": "snapshot-authority-v292", "authority": "server", "sequence": 45909,
...
def _report():
    items = [
        ...
        _item(2, "evidence", hashlib.sha256(b"evidence").hexdigest(), "evidence"),
    ]
    material = "\n".join(
    ...
    return {
        ...
        "authority": "server", "integrity_gate_id": "authority-evidence-lineage-v409",
        "snapshot_id": "snapshot-authority-v292",
        "assertion_id": "authority-snapshot-evidence-lineage-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 181, "release": "release-1",
...
class NetworkSnapshotAuthorityEvidenceLineageV409ValidatorTest(unittest.TestCase):
    def test_accepts_evidence_lineage_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_evidence_lineage_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-lineage"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority evidence lineage" in error for error in errors))
