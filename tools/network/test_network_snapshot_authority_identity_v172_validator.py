...
try:
    from .network_snapshot_authority_identity_v172_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_identity_v172_validator import validate_snapshot
...
class NetworkSnapshotAuthorityIdentityV172ValidatorTest(unittest.TestCase):
    def test_accepts_authority_identity_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_identity_binding(self):
        report = _report()
        report["records"][0]["authority_identity"] = "server-authority-identity-old"
        self.assertTrue(any("bind authority identity" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v54"
        self.assertTrue(any("bind authority identity" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority identity records" in error for error in validate_snapshot(report)))
