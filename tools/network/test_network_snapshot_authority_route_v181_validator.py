...
try:
    from .network_snapshot_authority_route_v181_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_route_v181_validator import validate_snapshot
...
class NetworkSnapshotAuthorityRouteV181ValidatorTest(unittest.TestCase):
    def test_accepts_authority_route_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_route_binding(self):
        report = _report()
        report["records"][0]["authority_route"] = "server-authority-route-old"
        self.assertTrue(any("bind authority route" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v63"
        self.assertTrue(any("bind authority route" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority route records" in error for error in validate_snapshot(report)))
