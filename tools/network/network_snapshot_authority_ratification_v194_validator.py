#!/usr/bin/env python3
"""Validate detached v194 network snapshot authority-ratification evidence."""
...
SCHEMA_VERSION = 194
EVIDENCE_SCOPE = "network_snapshot_authority_ratification_v194"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_RATIFICATION = "server-authority-ratification-v1"
SNAPSHOT_ID = "snapshot-authority-v77"
...
def _ratification_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_ratification", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('ratification_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-ratification snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_ratification": AUTHORITY_RATIFICATION,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_ratification", report.get("authority_ratification")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority ratification")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority ratification")
        ...
        ratification_digest = record.get("ratification_digest")
        if not _digest(ratification_digest):
            errors.append(f"{prefix}.ratification_digest must be lowercase SHA-256")
        elif ratification_digest != _ratification_digest(record):
            errors.append(f"{prefix}.ratification_digest must bind authority ratification")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority ratification records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority ratification records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_RATIFICATION_V194_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_RATIFICATION_V194_VALID")
    return 0
