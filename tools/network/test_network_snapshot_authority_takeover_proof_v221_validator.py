...
try:
    from .network_snapshot_authority_takeover_proof_v221_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_proof_v221_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["proof_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['proof_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_proof": "server-authority-takeover-proof-v1",
            "snapshot_id": "snapshot-authority-v104", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3089, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverProofV221ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_proof_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_proof_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_proof"] = "server-authority-takeover-proof-old"
        self.assertTrue(any("bind authority takeover proof" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v103"
        self.assertTrue(any("bind authority takeover proof" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover proof records" in error for error in validate_snapshot(report)))
