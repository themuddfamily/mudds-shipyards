#!/usr/bin/env python3
"""Validate detached v83 snapshot source/state evidence."""
...
SCHEMA_VERSION = 83
EVIDENCE_SCOPE = "network_snapshot_source_state_v83"
...
def _source_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "source", "state", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for source/state snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "source": SOURCE, "state": STATE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("state", report.get("state")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match source/state")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("state", report.get("state")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match source/state")
        ...
        if not _digest(source_digest):
            ...
            errors.append(f"{prefix}.source_digest must bind source/state")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_SOURCE_STATE_V83_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_SOURCE_STATE_V83_VALID")
    return 0
