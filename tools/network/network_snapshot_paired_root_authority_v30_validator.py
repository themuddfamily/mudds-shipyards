#!/usr/bin/env python3
"""Validate v30 paired snapshot root authority evidence."""
...
def _pair_digest(root: dict[str, Any]) -> str:
    left = root.get("left", {})
    right = root.get("right", {})
    return hashlib.sha256(f"{AUTHORITY}|{root.get('pair_id')}|{left.get('digest')}|{right.get('digest')}".encode("utf-8")).hexdigest()


def validate_root(report: Any, label: str = "root") -> list[str]:
    """Return root pair, member reference, count, and no-mutation errors."""
    ...
    root_pair = report.get("root_pair")
    if not isinstance(root_pair, dict):
        errors.append(f"{label}.root_pair must be an object")
        root_pair = {}
    if root_pair.get("authority") != AUTHORITY:
        errors.append(f"{label}.root_pair.authority must be {AUTHORITY}")
    if not isinstance(root_pair.get("pair_id"), str) or not root_pair.get("pair_id"):
        errors.append(f"{label}.root_pair.pair_id must be non-empty")
    left = root_pair.get("left")
    right = root_pair.get("right")
    ...
    pair_digest = root_pair.get("pair_digest")
    if not _digest(pair_digest):
        errors.append(f"{label}.root_pair.pair_digest must be lowercase SHA-256")
    elif pair_digest != _pair_digest(root_pair):
        errors.append(f"{label}.root_pair.pair_digest must bind root pair")
    ...
    for index, member in enumerate(members):
        ...
        if member.get("root_pair_digest") != pair_digest:
            errors.append(f"{prefix}.root_pair_digest must match root pair")
    ...
    if not isinstance(counts, dict):
        ...
        expected = {"members": len(members), "unique": len(member_ids), "reconciled": reconciled_count, "mutations": mutation_count}
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match root members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_PAIRED_ROOT_AUTHORITY_V30_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_PAIRED_ROOT_AUTHORITY_V30_VALID")
    return 0
