...
try:
    from .network_snapshot_authority_checkpoint_witness_v401_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_checkpoint_witness_v401_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-checkpoint-witness-assertion-v1",
        "integrity_gate_id": "authority-checkpoint-witness-v401",
        "snapshot_id": "snapshot-authority-v284", "authority": "server", "sequence": 45001,
...
def _report():
    ...
    return {
        "schema_version": 401,
        "evidence_scope": "network_snapshot_authority_checkpoint_witness_v401",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-checkpoint-witness-v401",
        "snapshot_id": "snapshot-authority-v284",
        "assertion_id": "authority-snapshot-checkpoint-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 173, "release": "release-1",
...
class NetworkSnapshotAuthorityCheckpointWitnessV401ValidatorTest(unittest.TestCase):
    def test_accepts_checkpoint_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_checkpoint_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-checkpoint"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority checkpoint witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["checkpoint"]
