...
try:
    from .network_snapshot_authority_takeover_grant_v236_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_grant_v236_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["grant_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['grant_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "grant_id": "server-authority-grant-v1",
            "authority_takeover_token": "server-authority-takeover-token-v7",
            "snapshot_id": "snapshot-authority-v119", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 8,
            "sequence": 9131, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverGrantV236ValidatorTest(unittest.TestCase):
    def test_accepts_authority_grant(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_grant_binding(self):
        report = _report()
        report["records"][0]["grant_id"] = "server-authority-grant-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority grant records" in error for error in errors))
