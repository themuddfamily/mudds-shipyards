#!/usr/bin/env python3
"""Validate detached v272 network snapshot authority-takeover attestation evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v155"
...
def _rollup_digest(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('record_id')}|{record.get('attestation_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    records = report.get("attestations")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority attestation records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "records": len(records), "unique": len(ids), "attested": attested, "mutations": mutations,
        }
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ATTESTATION_V272_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ATTESTATION_V272_VALID")
    return 0
