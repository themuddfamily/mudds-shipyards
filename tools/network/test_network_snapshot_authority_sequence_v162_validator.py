...
try:
    from .network_snapshot_authority_sequence_v162_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_sequence_v162_validator import validate_snapshot
...
class NetworkSnapshotAuthoritySequenceV162ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_sequence_binding(self):
        report = _report()
        report["records"][0]["authority_sequence"] = "server-authority-sequence-old"
        self.assertTrue(any("bind authority sequence" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v44"
        self.assertTrue(any("bind authority sequence" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority sequence records" in error for error in validate_snapshot(report)))
