#!/usr/bin/env python3
"""Validate detached v222 network snapshot authority-takeover evidence."""
...
SCHEMA_VERSION = 222
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_evidence_v222"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_EVIDENCE = "server-authority-takeover-evidence-v1"
SNAPSHOT_ID = "snapshot-authority-v105"
...
def _evidence_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_evidence", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('evidence_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_evidence": AUTHORITY_TAKEOVER_EVIDENCE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_evidence", report.get("authority_takeover_evidence")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover evidence")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover evidence")
        ...
        if not _digest(evidence_digest):
            ...
            errors.append(f"{prefix}.evidence_digest must bind authority takeover evidence")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover evidence records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover evidence records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_EVIDENCE_V222_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_EVIDENCE_V222_VALID")
    return 0
