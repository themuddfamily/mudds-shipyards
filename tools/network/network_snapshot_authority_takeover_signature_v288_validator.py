#!/usr/bin/env python3
"""Validate detached v288 network snapshot authority-takeover signature evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v171"
...
def _signature_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('signature_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover signature evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("signature_id", report.get("signature_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority signature")
    ...
    items = report.get("signatures")
    if not isinstance(items, list):
        errors.append(f"{label}.signatures must be an array")
        items = []
    ids: set[str] = set()
    signed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.signatures[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority signature")
        ...
        signature_digest = item.get("signature_digest")
        if not _digest(signature_digest):
            errors.append(f"{prefix}.signature_digest must be lowercase SHA-256")
        elif signature_digest != _signature_digest(item):
            errors.append(f"{prefix}.signature_digest must bind authority signature")
        if item.get("signed") is not True:
            errors.append(f"{prefix}.signed must be true")
        else:
            signed += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority signatures")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "signatures": len(items), "unique": len(ids), "signed": signed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority signatures")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SIGNATURE_V288_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SIGNATURE_V288_VALID")
    return 0
