#!/usr/bin/env python3
"""Validate detached v224 network snapshot authority-takeover closure evidence."""
...
SCHEMA_VERSION = 224
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_closure_v224"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_CLOSURE = "server-authority-takeover-closure-v1"
SNAPSHOT_ID = "snapshot-authority-v107"
...
def _closure_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_closure", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('closure_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover closure evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_closure": AUTHORITY_TAKEOVER_CLOSURE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_closure", report.get("authority_takeover_closure")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover closure")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover closure")
        ...
        closure_digest = record.get("closure_digest")
        if not _digest(closure_digest):
            errors.append(f"{prefix}.closure_digest must be lowercase SHA-256")
        elif closure_digest != _closure_digest(record):
            errors.append(f"{prefix}.closure_digest must bind authority takeover closure")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover closure records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover closure records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CLOSURE_V224_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CLOSURE_V224_VALID")
    return 0
