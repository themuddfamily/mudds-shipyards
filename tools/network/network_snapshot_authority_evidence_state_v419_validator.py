#!/usr/bin/env python3
"""Validate detached v419 authority/snapshot evidence-state evidence."""
from __future__ import annotations
import argparse, hashlib, json, re
from pathlib import Path
from typing import Any
SCHEMA_VERSION=419; EVIDENCE_SCOPE="network_snapshot_authority_evidence_state_v419"; EVIDENCE_MODE="detached_contract_fixture"; POLICY_VERSION="network_replication_interest_authority_v1"; AUTHORITY="server"; INTEGRITY_GATE_ID="authority-evidence-state-v419"; SNAPSHOT_ID="snapshot-authority-v302"; ASSERTION_ID="authority-snapshot-evidence-state-assertion-v1"; SOURCE="server_snapshot"; SNAPSHOT_VERSION=191; RELEASE_ID="release-1"; SHA256=re.compile(r"^[0-9a-f]{64}$"); NOT_RUN_CHECKS=("stale_check","native_run","hardware_run","human_review")
...
def validate_snapshot(report:Any,label:str="snapshot")->list[str]:
    ...
    for k,v in expected.items():
        if report.get(k)!=v:e.append(f"{label}.{k} must be {v}")
    for k in ("native_claims","uses_live_network"):
        if report.get(k) is not False:e.append(f"{label}.{k} must be false")
    for k in ("snapshot_detached","no_mutation_guarantee"):
        if report.get(k) is not True:e.append(f"{label}.{k} must be true")
    for k in NOT_RUN_CHECKS:_validate_not_run(report.get(k),f"{label}.{k}",e)
    ...
    for k,v in (("integrity_gate_id",report.get("integrity_gate_id")),("snapshot_id",report.get("snapshot_id")),("assertion_id",report.get("assertion_id")),("authority",AUTHORITY),("source",report.get("source")),("release",report.get("release")),("version",report.get("snapshot_version"))):
        if s.get(k)!=v:e.append(f"{label}.snapshot.{k} must match authority evidence state")
    ...
    if not isinstance(items,list):e.append(f"{label}.assertion_members must be an array");items=[]
    ...
    for n,i in enumerate(items):
        ...
        iid=i.get("item_id")
        if not isinstance(iid,str) or not iid:e.append(f"{p}.item_id must be non-empty")
        elif iid in ids:e.append(f"{p}.item_id must be unique")
        else:ids.add(iid)
        for k,v in (("assertion_id",report.get("assertion_id")),("integrity_gate_id",report.get("integrity_gate_id")),("snapshot_id",report.get("snapshot_id")),("authority",AUTHORITY)):
            if i.get(k)!=v:e.append(f"{p}.{k} must bind authority evidence state")
        ...
        aid=i.get("assertion_digest")
        if not _digest(aid):e.append(f"{p}.assertion_digest must be lowercase SHA-256")
        elif aid!=_assertion_digest(i):e.append(f"{p}.assertion_digest must bind authority evidence state")
    ...
    r=report.get("rollup_digest")
    if not _digest(r):e.append(f"{label}.rollup_digest must be lowercase SHA-256")
    elif r!=_rollup_digest(items):e.append(f"{label}.rollup_digest must match authority evidence members")
    c=report.get("counts")
    if not isinstance(c,dict):e.append(f"{label}.counts must be an object")
    else:
        x={"assertion_members":len(items),"unique":len(ids),"asserted":asserted,"mutations":mutations}
        for k,v in x.items():
...
def main()->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("snapshot");e=validate_snapshot_file(Path(p.parse_args().snapshot))
    if e:
        print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_STATE_V419_INVALID");[print(f"- {x}") for x in e];return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_STATE_V419_VALID");return 0
