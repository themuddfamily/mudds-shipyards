...
try:
    from .network_snapshot_authority_state_witness_v385_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_state_witness_v385_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-state-witness-assertion-v1",
        "integrity_gate_id": "authority-state-witness-v385",
        "snapshot_id": "snapshot-authority-v268", "authority": "server", "sequence": 43385,
...
def _report():
    ...
    return {
        "schema_version": 385,
        "evidence_scope": "network_snapshot_authority_state_witness_v385",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-state-witness-v385",
        "snapshot_id": "snapshot-authority-v268",
        "assertion_id": "authority-snapshot-state-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 157, "release": "release-1",
...
class NetworkSnapshotAuthorityStateWitnessV385ValidatorTest(unittest.TestCase):
    def test_accepts_state_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_state_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-state"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority state witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority state members" in error for error in errors))
