#!/usr/bin/env python3
"""Validate detached v126 network snapshot authority-key evidence."""
...
SCHEMA_VERSION = 126
EVIDENCE_SCOPE = "network_snapshot_authority_key_v126"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_KEY = "server-authority-key-v2"
SNAPSHOT_ID = "snapshot-authority-v9"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_key", "snapshot_id", "authority", "source", "release", "version",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-key snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_key": AUTHORITY_KEY,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (
        ("authority_key", report.get("authority_key")), ("snapshot_id", report.get("snapshot_id")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_KEY_V126_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_KEY_V126_VALID")
    return 0
