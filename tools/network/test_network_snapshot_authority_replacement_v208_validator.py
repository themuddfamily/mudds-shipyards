...
try:
    from .network_snapshot_authority_replacement_v208_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_replacement_v208_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["replacement_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['replacement_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_replacement": "server-authority-replacement-v1",
            "snapshot_id": "snapshot-authority-v91", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3037, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityReplacementV208ValidatorTest(unittest.TestCase):
    def test_accepts_replaced_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_replacement_binding(self):
        report = _report()
        report["records"][0]["authority_replacement"] = "server-authority-replacement-old"
        self.assertTrue(any("bind authority replacement" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v90"
        self.assertTrue(any("bind authority replacement" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority replacement records" in error for error in validate_snapshot(report)))
