#!/usr/bin/env python3
"""Validate detached v125 network snapshot authority-root evidence."""
...
SCHEMA_VERSION = 125
EVIDENCE_SCOPE = "network_snapshot_authority_root_v125"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ROOT = "server-authority-root-v2"
SNAPSHOT_ID = "snapshot-authority-v8"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_root", "snapshot_id", "authority", "source", "release", "version",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-root snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_root": AUTHORITY_ROOT,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (
        ("authority_root", report.get("authority_root")), ("snapshot_id", report.get("snapshot_id")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ROOT_V125_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ROOT_V125_VALID")
    return 0
