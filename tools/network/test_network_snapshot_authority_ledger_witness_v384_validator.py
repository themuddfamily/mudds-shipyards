...
try:
    from .network_snapshot_authority_ledger_witness_v384_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_ledger_witness_v384_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-ledger-witness-assertion-v1",
        "integrity_gate_id": "authority-ledger-witness-v384",
        "snapshot_id": "snapshot-authority-v267", "authority": "server", "sequence": 43284,
...
def _report():
    ...
    return {
        "schema_version": 384,
        "evidence_scope": "network_snapshot_authority_ledger_witness_v384",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-ledger-witness-v384",
        "snapshot_id": "snapshot-authority-v267",
        "assertion_id": "authority-snapshot-ledger-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 156, "release": "release-1",
...
class NetworkSnapshotAuthorityLedgerWitnessV384ValidatorTest(unittest.TestCase):
    def test_accepts_ledger_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_ledger_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["assertion_id"] = "wrong-ledger"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority ledger witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["ledger"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority ledger members" in error for error in errors))
