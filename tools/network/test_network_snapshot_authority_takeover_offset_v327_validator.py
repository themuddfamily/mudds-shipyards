...
try:
    from .network_snapshot_authority_takeover_offset_v327_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_offset_v327_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "offset_id": "authority-snapshot-offset-v1", "takeover_id": "authority-takeover-v327",
        "snapshot_id": "snapshot-authority-v210", "authority": "server", "sequence": 42342,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "advanced": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "offset_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["offset_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['offset_digest']}" for item in items
    )
    return {
        "schema_version": 327,
        "evidence_scope": "network_snapshot_authority_takeover_offset_v327",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v327",
        "snapshot_id": "snapshot-authority-v210", "offset_id": "authority-snapshot-offset-v1",
        "source": "server_snapshot", "snapshot_version": 99, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v327", "snapshot_id": "snapshot-authority-v210",
            "offset_id": "authority-snapshot-offset-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 99,
            "sequence": 42342, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "offset_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"offset_members": 2, "unique": 2, "advanced": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverOffsetV327ValidatorTest(unittest.TestCase):
    def test_accepts_offset_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_offset_binding(self):
        report = _report()
        report["offset_members"][0]["offset_id"] = "wrong-offset"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority offset" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["offset_members"][1]["order"] = 1
        report["offset_members"][1]["item_id"] = report["offset_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["offset_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["offset_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority offset members" in error for error in errors))
