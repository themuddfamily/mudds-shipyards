...
try:
    from .network_snapshot_authority_reactivation_v207_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_reactivation_v207_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["reactivation_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['reactivation_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_reactivation": "server-authority-reactivation-v1",
            "snapshot_id": "snapshot-authority-v90", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3033, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityReactivationV207ValidatorTest(unittest.TestCase):
    def test_accepts_reactivated_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_reactivation_binding(self):
        report = _report()
        report["records"][0]["authority_reactivation"] = "server-authority-reactivation-old"
        self.assertTrue(any("bind authority reactivation" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v89"
        self.assertTrue(any("bind authority reactivation" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority reactivation records" in error for error in validate_snapshot(report)))
