#!/usr/bin/env python3
"""Validate v32 paired root authority digest/count evidence."""
...
def _authority_digest(root: dict[str, Any]) -> str:
    ...
    payload = f"{AUTHORITY}|{root.get('pair_id')}|{root.get('member_count')}|{left.get('sequence')}|{left.get('digest')}|{right.get('sequence')}|{right.get('digest')}"
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def validate_digest(report: Any, label: str = "digest") -> list[str]:
    """Return counted root authority digest and no-mutation errors."""
    ...
    if not _sequence(root.get("member_count")):
        errors.append(f"{label}.root_pair.member_count must be non-negative")
    left = root.get("left")
    right = root.get("right")
    ...
    if not _digest(authority_digest):
        ...
        errors.append(f"{label}.root_pair.authority_digest must bind root and member count")
    ...
    if _sequence(root.get("member_count")) and root["member_count"] != len(members):
        errors.append(f"{label}.root_pair.member_count must match members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_PAIRED_ROOT_AUTHORITY_DIGEST_V32_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_PAIRED_ROOT_AUTHORITY_DIGEST_V32_VALID")
    return 0
