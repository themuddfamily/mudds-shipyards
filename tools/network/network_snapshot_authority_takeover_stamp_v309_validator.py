#!/usr/bin/env python3
"""Validate detached v309 authority/snapshot stamp evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v309"
SNAPSHOT_ID = "snapshot-authority-v192"
STAMP_ID = "authority-snapshot-stamp-v1"
...
def _stamp_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('stamp_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot stamp evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority stamp")
    ...
    items = report.get("stamps")
    if not isinstance(items, list):
        errors.append(f"{label}.stamps must be an array")
        items = []
    ids: set[str] = set()
    stamped = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.stamps[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority stamp")
        ...
        stamp_digest = item.get("stamp_digest")
        if not _digest(stamp_digest):
            errors.append(f"{prefix}.stamp_digest must be lowercase SHA-256")
        elif stamp_digest != _stamp_digest(item):
            errors.append(f"{prefix}.stamp_digest must bind authority stamp")
        if item.get("stamped") is not True:
            errors.append(f"{prefix}.stamped must be true")
        else:
            stamped += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority stamps")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "stamps": len(items), "unique": len(ids), "stamped": stamped, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority stamps")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_STAMP_V309_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_STAMP_V309_VALID")
    return 0
