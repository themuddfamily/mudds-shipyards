#!/usr/bin/env python3
"""Validate detached v341 authority/snapshot reconciliation evidence."""
...
AUTHORITY = "server"
RECONCILIATION_ID = "authority-reconciliation-v341"
SNAPSHOT_ID = "snapshot-authority-v224"
ASSERTION_ID = "authority-snapshot-reconciliation-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 113
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot reconciliation evidence."""
    errors: list[str] = []
    ...
    for index, item in enumerate(items):
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority reconciliation")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority reconciliation members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority reconciliation members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_RECONCILIATION_V341_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_RECONCILIATION_V341_VALID")
    return 0
