#!/usr/bin/env python3
"""Validate detached v236 network snapshot authority takeover grant evidence."""
...
SCHEMA_VERSION = 236
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_grant_v236"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
GRANT_ID = "server-authority-grant-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v7"
SNAPSHOT_ID = "snapshot-authority-v119"
...
def _grant_digest(record: dict[str, Any]) -> str:
    fields = (
        "grant_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('grant_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover grant evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority grant")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority grant")
        ...
        grant_digest = record.get("grant_digest")
        if not _digest(grant_digest):
            errors.append(f"{prefix}.grant_digest must be lowercase SHA-256")
        elif grant_digest != _grant_digest(record):
            errors.append(f"{prefix}.grant_digest must bind authority grant")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority grant records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority grant records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_GRANT_V236_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_GRANT_V236_VALID")
    return 0
