#!/usr/bin/env python3
"""Validate v48 dual-version snapshot authority evidence."""
...
def _authority_digest(record: dict[str, Any]) -> str:
    return hashlib.sha256(f"{record.get('authority')}|{record.get('authority_version')}|{record.get('provenance_version')}|{record.get('record_id')}|{record.get('sequence')}|{record.get('digest')}".encode("utf-8")).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    return hashlib.sha256("\n".join(f"{record.get('order')}|{record.get('record_id')}|{record.get('authority_digest')}" for record in records).encode("utf-8")).hexdigest()


def validate_authority(report: Any, label: str = "authority") -> list[str]:
    """Return dual-version authority bindings, aggregate, count, and mutation errors."""
    ...
    if not isinstance(snapshot, dict) or not _sequence(snapshot.get("sequence")) or not _digest(snapshot.get("digest")):
        ...
        snapshot = {}
    elif snapshot.get("authority") != AUTHORITY:
    ...
    for index, record in enumerate(records):
        ...
        if record.get("digest") != snapshot.get("digest"):
            errors.append(f"{prefix}.digest must match snapshot")
        if not _digest(record.get("digest")):
        ...
        if not _digest(authority_digest):
            ...
            errors.append(f"{prefix}.authority_digest must bind dual-version authority")
    ...
    if not isinstance(counts, dict):
        ...
        expected = {"records": len(records), "unique": len(ids), "valid": valid_count, "mutations": mutation_count}
...
def validate_authority_file(report_path: Path) -> list[str]:
    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return [f"unable to read {report_path}: {exc}"]
    return validate_authority(report, str(report_path))
...
def main() -> int:
    ...
    errors = validate_authority_file(args.authority)
    if errors:
        print("NETWORK_SNAPSHOT_DUAL_VERSION_AUTHORITY_V48_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_DUAL_VERSION_AUTHORITY_V48_VALID")
    return 0
