...
try:
    from .network_snapshot_authority_evidence_attestation_v410_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_evidence_attestation_v410_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-evidence-attestation-assertion-v1",
        "integrity_gate_id": "authority-evidence-attestation-v410",
        "snapshot_id": "snapshot-authority-v293", "authority": "server", "sequence": 46110,
...
def _report():
    items = [
        ...
        _item(2, "attestation", hashlib.sha256(b"attestation").hexdigest(), "attestation"),
    ]
    material = "\n".join(
    ...
    return {
        "schema_version": 410,
        "evidence_scope": "network_snapshot_authority_evidence_attestation_v410",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-evidence-attestation-v410",
        "snapshot_id": "snapshot-authority-v293",
        "assertion_id": "authority-snapshot-evidence-attestation-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 182, "release": "release-1",
...
class NetworkSnapshotAuthorityEvidenceAttestationV410ValidatorTest(unittest.TestCase):
    def test_accepts_evidence_attestation_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_evidence_attestation_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-attestation"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority evidence attestation" in error for error in errors))
