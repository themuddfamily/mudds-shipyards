...
try:
    from .network_snapshot_authority_takeover_bundle_v314_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_bundle_v314_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "bundle_id": "authority-snapshot-bundle-v1", "takeover_id": "authority-takeover-v314",
        "snapshot_id": "snapshot-authority-v197", "authority": "server", "sequence": 42264,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "bundled": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "bundle_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["bundle_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['bundle_digest']}" for item in items
    )
    return {
        "schema_version": 314,
        "evidence_scope": "network_snapshot_authority_takeover_bundle_v314",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v314",
        "snapshot_id": "snapshot-authority-v197", "bundle_id": "authority-snapshot-bundle-v1",
        "source": "server_snapshot", "snapshot_version": 86, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v314", "snapshot_id": "snapshot-authority-v197",
            "bundle_id": "authority-snapshot-bundle-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 86,
            "sequence": 42264, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "bundle_entries": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"bundle_entries": 2, "unique": 2, "bundled": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverBundleV314ValidatorTest(unittest.TestCase):
    def test_accepts_bundle_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_bundle_binding(self):
        report = _report()
        report["bundle_entries"][0]["bundle_id"] = "wrong-bundle"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority bundle" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["bundle_entries"][1]["order"] = 1
        report["bundle_entries"][1]["item_id"] = report["bundle_entries"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["bundle_entries"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["bundled"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.bundled" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["bundle_entries"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority bundle entries" in error for error in errors))
