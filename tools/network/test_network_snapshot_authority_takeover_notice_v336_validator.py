...
try:
    from .network_snapshot_authority_takeover_notice_v336_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_notice_v336_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "notice_id": "authority-snapshot-notice-v1", "takeover_id": "authority-takeover-v336",
        "snapshot_id": "snapshot-authority-v219", "authority": "server", "sequence": 42396,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "posted": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "notice_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["notice_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['notice_digest']}" for item in items
    )
    return {
        "schema_version": 336,
        "evidence_scope": "network_snapshot_authority_takeover_notice_v336",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v336",
        "snapshot_id": "snapshot-authority-v219", "notice_id": "authority-snapshot-notice-v1",
        "source": "server_snapshot", "snapshot_version": 108, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v336", "snapshot_id": "snapshot-authority-v219",
            "notice_id": "authority-snapshot-notice-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 108,
            "sequence": 42396, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "notice_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"notice_members": 2, "unique": 2, "posted": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverNoticeV336ValidatorTest(unittest.TestCase):
    def test_accepts_notice_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_notice_binding(self):
        report = _report()
        report["notice_members"][0]["notice_id"] = "wrong-notice"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority notice" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["notice_members"][1]["order"] = 1
        report["notice_members"][1]["item_id"] = report["notice_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["notice_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["posted"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.posted" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["notice_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority notice members" in error for error in errors))
