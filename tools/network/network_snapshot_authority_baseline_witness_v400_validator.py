#!/usr/bin/env python3
"""Validate detached v400 authority/snapshot baseline-witness evidence."""
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot baseline-witness evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority baseline witness")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority baseline witness")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority baseline witness")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority baseline members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority baseline members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_BASELINE_WITNESS_V400_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_BASELINE_WITNESS_V400_VALID")
    return 0
