#!/usr/bin/env python3
"""Validate detached v206 network snapshot authority-deactivation evidence."""
...
SCHEMA_VERSION = 206
EVIDENCE_SCOPE = "network_snapshot_authority_deactivation_v206"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_DEACTIVATION = "server-authority-deactivation-v1"
SNAPSHOT_ID = "snapshot-authority-v89"
...
def _deactivation_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_deactivation", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('deactivation_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-deactivation snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_deactivation", report.get("authority_deactivation")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority deactivation")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority deactivation")
        ...
        deactivation_digest = record.get("deactivation_digest")
        if not _digest(deactivation_digest):
            errors.append(f"{prefix}.deactivation_digest must be lowercase SHA-256")
        elif deactivation_digest != _deactivation_digest(record):
            errors.append(f"{prefix}.deactivation_digest must bind authority deactivation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority deactivation records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority deactivation records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_DEACTIVATION_V206_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_DEACTIVATION_V206_VALID")
    return 0
