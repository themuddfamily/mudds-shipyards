#!/usr/bin/env python3
"""Validate detached v294 network snapshot authority-takeover ledger evidence."""
...
SCHEMA_VERSION = 294
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_ledger_v294"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
LEDGER_ID = "server-authority-ledger-v1"
TAKEOVER_TOKEN = "server-authority-takeover-token-v65"
SNAPSHOT_ID = "snapshot-authority-v177"
...
def _ledger_digest(item: dict[str, Any]) -> str:
    fields = (
        "ledger_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "event", "item_id", "expected_digest", "observed_digest",
    )
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('ledger_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover ledger evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority ledger")
    ...
    items = report.get("ledger_entries")
    if not isinstance(items, list):
        errors.append(f"{label}.ledger_entries must be an array")
        items = []
    ids: set[str] = set()
    logged = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.ledger_entries[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority ledger")
        ...
        if not isinstance(item.get("event"), str) or not item["event"]:
            errors.append(f"{prefix}.event must be non-empty")
        ...
        ledger_digest = item.get("ledger_digest")
        if not _digest(ledger_digest):
            errors.append(f"{prefix}.ledger_digest must be lowercase SHA-256")
        elif ledger_digest != _ledger_digest(item):
            errors.append(f"{prefix}.ledger_digest must bind authority ledger")
        if item.get("logged") is not True:
            errors.append(f"{prefix}.logged must be true")
        else:
            logged += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority ledger entries")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "entries": len(items), "unique": len(ids), "logged": logged, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority ledger entries")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LEDGER_V294_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LEDGER_V294_VALID")
    return 0
