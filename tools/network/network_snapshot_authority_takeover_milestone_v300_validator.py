#!/usr/bin/env python3
"""Validate detached v300 network snapshot authority-takeover milestone evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v183"
...
def _milestone_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('milestone_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover milestone evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority milestone")
    ...
    if not isinstance(items, list):
        errors.append(f"{label}.milestones must be an array")
        items = []
    ids: set[str] = set()
    reached = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.milestones[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority milestone")
        ...
        milestone_digest = item.get("milestone_digest")
        if not _digest(milestone_digest):
            errors.append(f"{prefix}.milestone_digest must be lowercase SHA-256")
        elif milestone_digest != _milestone_digest(item):
            errors.append(f"{prefix}.milestone_digest must bind authority milestone")
        if item.get("reached") is not True:
            errors.append(f"{prefix}.reached must be true")
        else:
            reached += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority milestones")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "milestones": len(items), "unique": len(ids), "reached": reached, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority milestones")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MILESTONE_V300_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MILESTONE_V300_VALID")
    return 0
