...
try:
    from .network_snapshot_authority_takeover_window_v321_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_window_v321_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "window_id": "authority-snapshot-window-v1", "takeover_id": "authority-takeover-v321",
        "snapshot_id": "snapshot-authority-v204", "authority": "server", "sequence": 42306,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "observed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "window_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["window_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['window_digest']}" for item in items
    )
    return {
        "schema_version": 321,
        "evidence_scope": "network_snapshot_authority_takeover_window_v321",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v321",
        "snapshot_id": "snapshot-authority-v204", "window_id": "authority-snapshot-window-v1",
        "source": "server_snapshot", "snapshot_version": 93, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v321", "snapshot_id": "snapshot-authority-v204",
            "window_id": "authority-snapshot-window-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 93,
            "sequence": 42306, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "window_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"window_members": 2, "unique": 2, "observed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverWindowV321ValidatorTest(unittest.TestCase):
    def test_accepts_window_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_window_binding(self):
        report = _report()
        report["window_members"][0]["window_id"] = "wrong-window"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority window" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["window_members"][1]["order"] = 1
        report["window_members"][1]["item_id"] = report["window_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["window_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["observed"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.observed" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["window_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority window members" in error for error in errors))
