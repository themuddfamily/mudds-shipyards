#!/usr/bin/env python3
"""Validate detached v372 authority/snapshot recovery evidence."""
...
AUTHORITY = "server"
RECOVERY_ID = "authority-recovery-v372"
SNAPSHOT_ID = "snapshot-authority-v255"
ASSERTION_ID = "authority-snapshot-recovery-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 144
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot recovery evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority recovery")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority recovery")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority recovery")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority recovery members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority recovery members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_RECOVERY_V372_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_RECOVERY_V372_VALID")
    return 0
