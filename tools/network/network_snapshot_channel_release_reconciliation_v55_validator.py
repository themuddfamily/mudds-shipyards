#!/usr/bin/env python3
"""Validate detached v55 snapshot channel/release reconciliation evidence."""
...
SCHEMA_VERSION = 55
EVIDENCE_SCOPE = "network_snapshot_channel_release_reconciliation_v55"
...
def validate_checks(report: Any, label: str = "checks") -> list[str]:
    ...
    for index, check in enumerate(checks):
        ...
        elif reconciliation_digest != _check_digest(check):
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_CHANNEL_RELEASE_RECONCILIATION_V55_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_CHANNEL_RELEASE_RECONCILIATION_V55_VALID")
    return 0
