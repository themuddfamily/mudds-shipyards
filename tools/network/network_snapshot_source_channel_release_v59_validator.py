#!/usr/bin/env python3
"""Validate detached v59 snapshot source/channel/release evidence."""
...
SCHEMA_VERSION = 59
EVIDENCE_SCOPE = "network_snapshot_source_channel_release_v59"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for source/channel/release-bound snapshot evidence."""
    errors: list[str] = []
    ...
    for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("channel", report.get("channel")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match source/channel/release")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("channel", report.get("channel")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match source/channel/release")
        ...
        if not _digest(evidence_digest):
            ...
            errors.append(f"{prefix}.evidence_digest must bind source/channel/release")
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {"records": len(records), "unique": len(ids), "accepted": accepted, "mutations": mutations}
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_SOURCE_CHANNEL_RELEASE_V59_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_SOURCE_CHANNEL_RELEASE_V59_VALID")
    return 0
