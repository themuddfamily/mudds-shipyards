...
try:
    from .network_snapshot_authority_handoff_v209_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_handoff_v209_validator import validate_snapshot
...
def _record(order, check_id, digest):
    record = {
        "order": order,
        "check_id": check_id,
        "authority_handoff": "server-authority-handoff-v1",
        "snapshot_id": "snapshot-authority-v92",
        "authority": "server",
    ...
    fields = (
        "authority_handoff", "snapshot_id", "authority", "source", "release",
        "version", "check_id", "sequence", "expected_digest", "observed_digest",
    )
    record["handoff_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['handoff_digest']}"
        for record in records
    )
    return {
        "schema_version": 209,
        "evidence_scope": "network_snapshot_authority_handoff_v209",
        ...
        "snapshot": {
            "authority_handoff": "server-authority-handoff-v1",
            "snapshot_id": "snapshot-authority-v92", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3041, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityHandoffV209ValidatorTest(unittest.TestCase):
    def test_accepts_handoff_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_handoff_binding(self):
        report = _report()
        report["records"][0]["authority_handoff"] = "server-authority-handoff-old"
        self.assertTrue(any("bind authority handoff" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v91"
        self.assertTrue(any("bind authority handoff" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority handoff records" in error for error in validate_snapshot(report)))
