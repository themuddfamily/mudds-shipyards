#!/usr/bin/env python3
"""Validate detached v307 authority/snapshot proof evidence."""
...
SCHEMA_VERSION = 307
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_proof_v307"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v307"
SNAPSHOT_ID = "snapshot-authority-v190"
PROOF_ID = "authority-snapshot-proof-v1"
...
def _proof_digest(item: dict[str, Any]) -> str:
    fields = (
        "proof_id", "takeover_id", "snapshot_id", "sequence", "subject",
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('proof_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot proof evidence."""
    errors: list[str] = []
    ...
    items = report.get("proofs")
    if not isinstance(items, list):
        errors.append(f"{label}.proofs must be an array")
        items = []
    ids: set[str] = set()
    proven = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.proofs[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority proof")
        ...
        elif proof_digest != _proof_digest(item):
            errors.append(f"{prefix}.proof_digest must bind authority proof")
        if item.get("proven") is not True:
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority proofs")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "proofs": len(items), "unique": len(ids), "proven": proven, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority proofs")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PROOF_V307_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PROOF_V307_VALID")
    return 0
