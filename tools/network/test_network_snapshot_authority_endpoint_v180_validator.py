...
try:
    from .network_snapshot_authority_endpoint_v180_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_endpoint_v180_validator import validate_snapshot
...
class NetworkSnapshotAuthorityEndpointV180ValidatorTest(unittest.TestCase):
    def test_accepts_authority_endpoint_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_endpoint_binding(self):
        report = _report()
        report["records"][0]["authority_endpoint"] = "server-authority-endpoint-old"
        self.assertTrue(any("bind authority endpoint" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v62"
        self.assertTrue(any("bind authority endpoint" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority endpoint records" in error for error in validate_snapshot(report)))
