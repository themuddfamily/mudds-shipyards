...
try:
    from .network_snapshot_authority_takeover_receipt_v213_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_receipt_v213_validator import validate_snapshot
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['receipt_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_receipt": "server-authority-takeover-receipt-v1",
            "snapshot_id": "snapshot-authority-v96", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3057, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverReceiptV213ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_receipt_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_receipt_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_receipt"] = "server-authority-takeover-receipt-old"
        self.assertTrue(any("bind authority takeover receipt" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v95"
        self.assertTrue(any("bind authority takeover receipt" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover receipt records" in error for error in validate_snapshot(report)))
