...
try:
    from .network_snapshot_authority_response_v151_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_response_v151_validator import validate_snapshot
...
class NetworkSnapshotAuthorityResponseV151ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_response_binding(self):
        report = _report()
        report["records"][0]["authority_response"] = "server-authority-response-old"
        self.assertTrue(any("bind authority response" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v33"
        self.assertTrue(any("bind authority response" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority response records" in error for error in validate_snapshot(report)))
