#!/usr/bin/env python3
"""Validate detached v131 network snapshot authority-challenge evidence."""
...
SCHEMA_VERSION = 131
EVIDENCE_SCOPE = "network_snapshot_authority_challenge_v131"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_CHALLENGE = "server-authority-challenge-v2"
SNAPSHOT_ID = "snapshot-authority-v14"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    expected = {"schema_version": SCHEMA_VERSION, "evidence_scope": EVIDENCE_SCOPE, "evidence_mode": EVIDENCE_MODE, "policy_version": POLICY_VERSION, "authority": AUTHORITY, "authority_challenge": AUTHORITY_CHALLENGE, "snapshot_id": SNAPSHOT_ID, "source": SOURCE, "snapshot_version": SNAPSHOT_VERSION, "release": RELEASE_ID}
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority_challenge", report.get("authority_challenge")), ("snapshot_id", report.get("snapshot_id")), ("authority", AUTHORITY), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CHALLENGE_V131_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CHALLENGE_V131_VALID")
    return 0
