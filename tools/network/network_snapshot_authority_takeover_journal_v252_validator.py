#!/usr/bin/env python3
"""Validate detached v252 network snapshot authority takeover journal evidence."""
...
SCHEMA_VERSION = 252
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_journal_v252"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
JOURNAL_ID = "server-authority-journal-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v23"
SNAPSHOT_ID = "snapshot-authority-v135"
...
def _journal_digest(record: dict[str, Any]) -> str:
    fields = (
        "journal_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('journal_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover journal evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("journal_id", report.get("journal_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority journal")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority journal")
        ...
        if not _digest(journal_digest):
            errors.append(f"{prefix}.journal_digest must be lowercase SHA-256")
        elif journal_digest != _journal_digest(record):
            errors.append(f"{prefix}.journal_digest must bind authority journal")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority journal records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority journal records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_JOURNAL_V252_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_JOURNAL_V252_VALID")
    return 0
