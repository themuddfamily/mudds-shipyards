#!/usr/bin/env python3
"""Validate detached v105 snapshot provenance/closure evidence."""
...
SCHEMA_VERSION = 105
EVIDENCE_SCOPE = "network_snapshot_provenance_closure_v105"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
PROVENANCE_ID = "snapshot-provenance-v4"
CLOSURE = "closed"
...
def _provenance_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority", "provenance_id", "closure", "source", "release", "version",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('provenance_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached provenance/closure snapshot evidence."""
    errors: list[str] = []
    ...
    for index, record in enumerate(records):
        ...
        if record.get("provenanced") is not True:
            errors.append(f"{prefix}.provenanced must be true")
        else:
            provenanced += 1
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_PROVENANCE_CLOSURE_V105_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_PROVENANCE_CLOSURE_V105_VALID")
    return 0
