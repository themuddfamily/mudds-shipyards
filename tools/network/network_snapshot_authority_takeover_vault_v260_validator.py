#!/usr/bin/env python3
"""Validate detached v260 network snapshot authority takeover vault evidence."""
...
def _vault_digest(record: dict[str, Any]) -> str:
    fields = (
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('vault_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover vault evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority vault")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority vault")
        ...
        vault_digest = record.get("vault_digest")
        if not _digest(vault_digest):
            errors.append(f"{prefix}.vault_digest must be lowercase SHA-256")
        elif vault_digest != _vault_digest(record):
            errors.append(f"{prefix}.vault_digest must bind authority vault")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority vault records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority vault records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_VAULT_V260_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_VAULT_V260_VALID")
    return 0
