#!/usr/bin/env python3
"""Validate detached v231 network snapshot authority attestation evidence."""
...
SCHEMA_VERSION = 231
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_attestation_v231"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
ATTESTATION_ID = "server-authority-attestation-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v2"
SNAPSHOT_ID = "snapshot-authority-v114"
...
def _attestation_digest(record: dict[str, Any]) -> str:
    fields = (
        "attestation_id", "authority_takeover_token", "snapshot_id", "authority",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority attestation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority attestation records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority attestation records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ATTESTATION_V231_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ATTESTATION_V231_VALID")
    return 0
