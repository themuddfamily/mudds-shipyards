#!/usr/bin/env python3
"""Validate detached v315 authority/snapshot cohort evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v315"
SNAPSHOT_ID = "snapshot-authority-v198"
COHORT_ID = "authority-snapshot-cohort-v1"
...
def _cohort_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('cohort_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot cohort evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority cohort")
    ...
    items = report.get("cohort_members")
    if not isinstance(items, list):
        errors.append(f"{label}.cohort_members must be an array")
        items = []
    ids: set[str] = set()
    joined = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.cohort_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority cohort")
        ...
        cohort_digest = item.get("cohort_digest")
        if not _digest(cohort_digest):
            errors.append(f"{prefix}.cohort_digest must be lowercase SHA-256")
        elif cohort_digest != _cohort_digest(item):
            errors.append(f"{prefix}.cohort_digest must bind authority cohort")
        if item.get("joined") is not True:
            errors.append(f"{prefix}.joined must be true")
        else:
            joined += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority cohort members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "cohort_members": len(items), "unique": len(ids), "joined": joined, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority cohort members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_COHORT_V315_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_COHORT_V315_VALID")
    return 0
