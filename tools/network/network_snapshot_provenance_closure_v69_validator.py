#!/usr/bin/env python3
"""Validate detached v69 snapshot provenance/closure evidence."""
...
SCHEMA_VERSION = 69
EVIDENCE_SCOPE = "network_snapshot_provenance_closure_v69"
...
def _provenance_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "provenance", "closure", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for provenance/closure snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "provenance": PROVENANCE, "closure": CLOSURE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("provenance", report.get("provenance")), ("closure", report.get("closure")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match provenance/closure")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("provenance", report.get("provenance")), ("closure", report.get("closure")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match provenance/closure")
        ...
        if not _digest(provenance_digest):
            ...
            errors.append(f"{prefix}.provenance_digest must bind provenance/closure")
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {"records": len(records), "unique": len(ids), "closed": closed, "mutations": mutations}
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_PROVENANCE_CLOSURE_V69_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_PROVENANCE_CLOSURE_V69_VALID")
    return 0
