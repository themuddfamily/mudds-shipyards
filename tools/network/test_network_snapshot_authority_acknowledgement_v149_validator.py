...
try:
    from .network_snapshot_authority_acknowledgement_v149_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_acknowledgement_v149_validator import validate_snapshot
...
class NetworkSnapshotAuthorityAcknowledgementV149ValidatorTest(unittest.TestCase):
    def test_accepts_authority_acknowledgement_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_acknowledgement_binding(self):
        report = _report()
        report["records"][0]["authority_acknowledgement"] = "server-authority-acknowledgement-old"
        self.assertTrue(any("bind authority acknowledgement" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v31"
        self.assertTrue(any("bind authority acknowledgement" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority acknowledgement records" in error for error in validate_snapshot(report)))
