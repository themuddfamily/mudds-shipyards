...
try:
    from .network_snapshot_authority_scope_v175_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_scope_v175_validator import validate_snapshot
...
class NetworkSnapshotAuthorityScopeV175ValidatorTest(unittest.TestCase):
    def test_accepts_authority_scope_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_scope_binding(self):
        report = _report()
        report["records"][0]["authority_scope"] = "server-authority-scope-old"
        self.assertTrue(any("bind authority scope" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v57"
        self.assertTrue(any("bind authority scope" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority scope records" in error for error in validate_snapshot(report)))
