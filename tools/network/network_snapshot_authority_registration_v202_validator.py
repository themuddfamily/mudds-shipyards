#!/usr/bin/env python3
"""Validate detached v202 network snapshot authority-registration evidence."""
...
SCHEMA_VERSION = 202
EVIDENCE_SCOPE = "network_snapshot_authority_registration_v202"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_REGISTRATION = "server-authority-registration-v1"
SNAPSHOT_ID = "snapshot-authority-v85"
...
def _registration_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_registration", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('registration_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-registration snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_registration", report.get("authority_registration")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority registration")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority registration")
        ...
        if not _digest(registration_digest):
            errors.append(f"{prefix}.registration_digest must be lowercase SHA-256")
        elif registration_digest != _registration_digest(record):
            errors.append(f"{prefix}.registration_digest must bind authority registration")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority registration records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority registration records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_REGISTRATION_V202_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_REGISTRATION_V202_VALID")
    return 0
