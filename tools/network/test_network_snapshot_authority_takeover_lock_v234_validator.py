...
try:
    from .network_snapshot_authority_takeover_lock_v234_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_lock_v234_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["lock_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['lock_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "lock_id": "server-authority-lock-v1",
            "authority_takeover_token": "server-authority-takeover-token-v5",
            "snapshot_id": "snapshot-authority-v117", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 6,
            "sequence": 7129, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverLockV234ValidatorTest(unittest.TestCase):
    def test_accepts_authority_lock(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_lock_binding(self):
        report = _report()
        report["records"][0]["lock_id"] = "server-authority-lock-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority lock" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority lock records" in error for error in errors))
