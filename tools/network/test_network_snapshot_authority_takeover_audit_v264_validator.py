...
try:
    from .network_snapshot_authority_takeover_audit_v264_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_audit_v264_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "audit_id": "server-authority-audit-v1",
            "authority_takeover_token": "server-authority-takeover-token-v35",
            "snapshot_id": "snapshot-authority-v147", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 36,
            "sequence": 37159, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverAuditV264ValidatorTest(unittest.TestCase):
    def test_accepts_authority_audit(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_audit_binding(self):
        report = _report()
        report["records"][0]["audit_id"] = "server-authority-audit-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority audit records" in error for error in errors))
