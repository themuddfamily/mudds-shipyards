#!/usr/bin/env python3
"""Validate detached v242 network snapshot authority takeover mandate evidence."""
...
SCHEMA_VERSION = 242
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_mandate_v242"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
MANDATE_ID = "server-authority-mandate-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v13"
SNAPSHOT_ID = "snapshot-authority-v125"
...
def _mandate_digest(record: dict[str, Any]) -> str:
    fields = (
        "mandate_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('mandate_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover mandate evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("mandate_id", report.get("mandate_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority mandate")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority mandate")
        ...
        mandate_digest = record.get("mandate_digest")
        if not _digest(mandate_digest):
            errors.append(f"{prefix}.mandate_digest must be lowercase SHA-256")
        elif mandate_digest != _mandate_digest(record):
            errors.append(f"{prefix}.mandate_digest must bind authority mandate")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority mandate records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority mandate records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MANDATE_V242_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MANDATE_V242_VALID")
    return 0
