...
try:
    from .network_snapshot_authority_deactivation_v206_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_deactivation_v206_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["deactivation_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['deactivation_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_deactivation": "server-authority-deactivation-v1",
            "snapshot_id": "snapshot-authority-v89", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3029, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityDeactivationV206ValidatorTest(unittest.TestCase):
    def test_accepts_deactivated_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_deactivation_binding(self):
        report = _report()
        report["records"][0]["authority_deactivation"] = "server-authority-deactivation-old"
        self.assertTrue(any("bind authority deactivation" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v88"
        self.assertTrue(any("bind authority deactivation" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority deactivation records" in error for error in validate_snapshot(report)))
