#!/usr/bin/env python3
"""Validate detached v67 snapshot audit/closure evidence."""
...
SCHEMA_VERSION = 67
EVIDENCE_SCOPE = "network_snapshot_audit_closure_v67"
...
def _audit_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "audit_id", "closure", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for audit/closure snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "audit_id": AUDIT_ID, "closure": CLOSURE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("audit_id", report.get("audit_id")), ("closure", report.get("closure")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match audit/closure")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("audit_id", report.get("audit_id")), ("closure", report.get("closure")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match audit/closure")
        ...
        if not _digest(audit_digest):
            ...
            errors.append(f"{prefix}.audit_digest must bind audit/closure")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUDIT_CLOSURE_V67_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUDIT_CLOSURE_V67_VALID")
    return 0
