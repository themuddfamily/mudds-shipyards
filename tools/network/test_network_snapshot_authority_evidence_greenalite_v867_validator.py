...
try:
 from .network_snapshot_authority_evidence_greenalite_v867_validator import validate_snapshot
except ImportError:
 from network_snapshot_authority_evidence_greenalite_v867_validator import validate_snapshot
def _item(o,i,d,s):
 x={"order":o,"item_id":i,"assertion_id":"authority-snapshot-evidence-greenalite-assertion-v1","integrity_gate_id":"authority-evidence-greenalite-v867","snapshot_id":"snapshot-authority-v750","authority":"server","sequence":50767,"subject":s,"authority_digest":d,"snapshot_digest":d,"asserted":True,"mutation_fields":[],"state_changed":False};f=("assertion_id","integrity_gate_id","snapshot_id","sequence","subject","authority_digest","snapshot_digest");x["assertion_digest"]=hashlib.sha256("|".join(str(x[k])for k in f).encode()).hexdigest();return x
def _nr(r):return{"status":"NOT_RUN","evidence":None,"reason":r}
def _report():
 a=[_item(1,"authority",hashlib.sha256(b"authority").hexdigest(),"authority"),_item(2,"greenalite",hashlib.sha256(b"greenalite").hexdigest(),"greenalite")];m="\n".join(f"{x['order']}|{x['item_id']}|{x['assertion_digest']}"for x in a)
 return{"schema_version":867,"evidence_scope":"network_snapshot_authority_evidence_greenalite_v867","evidence_mode":"detached_contract_fixture","policy_version":"network_replication_interest_authority_v1","authority":"server","integrity_gate_id":"authority-evidence-greenalite-v867","snapshot_id":"snapshot-authority-v750","assertion_id":"authority-snapshot-evidence-greenalite-assertion-v1","source":"server_snapshot","snapshot_version":639,"release":"release-1","native_claims":False,"uses_live_network":False,"snapshot_detached":True,"no_mutation_guarantee":True,"stale_check":_nr("Detached fixture does not execute stale replay checks."),"native_run":_nr("Native transport is outside this validator scope."),"hardware_run":_nr("Hardware validation is outside this validator scope."),"human_review":_nr("Human review is outside this validator scope."),"snapshot":{"integrity_gate_id":"authority-evidence-greenalite-v867","snapshot_id":"snapshot-authority-v750","assertion_id":"authority-snapshot-evidence-greenalite-assertion-v1","authority":"server","source":"server_snapshot","release":"release-1","version":639,"sequence":50767,"digest":hashlib.sha256(b"snapshot").hexdigest()},"assertion_members":a,"rollup_digest":hashlib.sha256(m.encode()).hexdigest(),"counts":{"assertion_members":2,"unique":2,"asserted":2,"mutations":0}}
class NetworkSnapshotAuthorityEvidenceGreenaliteV867ValidatorTest(unittest.TestCase):
 def test_accepts_evidence_greenalite_rollup(self):self.assertEqual(validate_snapshot(_report()),[])
 def test_rejects_evidence_greenalite_binding(self):r=_report();r["assertion_members"][0]["snapshot_id"]="wrong-greenalite";self.assertTrue(any("bind authority evidence greenalite"in e for e in validate_snapshot(r)))
