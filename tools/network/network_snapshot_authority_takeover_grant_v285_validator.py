#!/usr/bin/env python3
"""Validate detached v285 network snapshot authority-takeover grant evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v168"
...
def _grant_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('grant_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    items = report.get("grant_items")
    if not isinstance(items, list):
        errors.append(f"{label}.grant_items must be an array")
        items = []
    ids: set[str] = set()
    granted = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.grant_items[{index}]"
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority grant items")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "items": len(items), "unique": len(ids), "granted": granted, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority grant items")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_GRANT_V285_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_GRANT_V285_VALID")
    return 0
