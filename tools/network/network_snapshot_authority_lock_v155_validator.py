#!/usr/bin/env python3
"""Validate detached v155 network snapshot authority-lock evidence."""
...
SCHEMA_VERSION = 155
EVIDENCE_SCOPE = "network_snapshot_authority_lock_v155"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_LOCK = "server-authority-lock-v1"
SNAPSHOT_ID = "snapshot-authority-v38"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_lock", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_lock", report.get("authority_lock")),
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority lock")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_LOCK_V155_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_LOCK_V155_VALID")
    return 0
