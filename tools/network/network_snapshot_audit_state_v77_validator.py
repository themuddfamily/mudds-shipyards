#!/usr/bin/env python3
"""Validate detached v77 snapshot audit/state evidence."""
...
SCHEMA_VERSION = 77
EVIDENCE_SCOPE = "network_snapshot_audit_state_v77"
...
def _audit_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "audit_id", "state", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for audit/state snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "audit_id": AUDIT_ID, "state": STATE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("audit_id", report.get("audit_id")), ("state", report.get("state")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match audit/state")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("audit_id", report.get("audit_id")), ("state", report.get("state")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match audit/state")
        ...
        if not _digest(audit_digest):
            ...
            errors.append(f"{prefix}.audit_digest must bind audit/state")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUDIT_STATE_V77_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUDIT_STATE_V77_VALID")
    return 0
