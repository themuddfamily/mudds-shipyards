#!/usr/bin/env python3
"""Validate detached v324 authority/snapshot checkpoint evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v324"
SNAPSHOT_ID = "snapshot-authority-v207"
CHECKPOINT_ID = "authority-snapshot-checkpoint-v1"
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('checkpoint_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot checkpoint evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority checkpoint")
    ...
    items = report.get("checkpoint_members")
    if not isinstance(items, list):
        errors.append(f"{label}.checkpoint_members must be an array")
        items = []
    ids: set[str] = set()
    confirmed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.checkpoint_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority checkpoint")
        ...
        if not _digest(checkpoint_digest):
            errors.append(f"{prefix}.checkpoint_digest must be lowercase SHA-256")
        elif checkpoint_digest != _checkpoint_digest(item):
            errors.append(f"{prefix}.checkpoint_digest must bind authority checkpoint")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority checkpoint members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "checkpoint_members": len(items), "unique": len(ids), "confirmed": confirmed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority checkpoint members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHECKPOINT_V324_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHECKPOINT_V324_VALID")
    return 0
