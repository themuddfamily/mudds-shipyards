...
try:
 from .network_snapshot_authority_evidence_reliability_v528_validator import validate_snapshot
except ImportError:
 from network_snapshot_authority_evidence_reliability_v528_validator import validate_snapshot
def _item(o,i,d,s):
 x={"order":o,"item_id":i,"assertion_id":"authority-snapshot-evidence-reliability-assertion-v1","integrity_gate_id":"authority-evidence-reliability-v528","snapshot_id":"snapshot-authority-v411","authority":"server","sequence":50428,"subject":s,"authority_digest":d,"snapshot_digest":d,"asserted":True,"mutation_fields":[],"state_changed":False};f=("assertion_id","integrity_gate_id","snapshot_id","sequence","subject","authority_digest","snapshot_digest");x["assertion_digest"]=hashlib.sha256("|".join(str(x[k])for k in f).encode()).hexdigest();return x
