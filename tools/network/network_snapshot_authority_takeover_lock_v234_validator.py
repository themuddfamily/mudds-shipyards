#!/usr/bin/env python3
"""Validate detached v234 network snapshot authority takeover lock evidence."""
...
SCHEMA_VERSION = 234
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_lock_v234"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
LOCK_ID = "server-authority-lock-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v5"
SNAPSHOT_ID = "snapshot-authority-v117"
...
def _lock_digest(record: dict[str, Any]) -> str:
    fields = (
        "lock_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('lock_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover lock evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority lock")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority lock")
        ...
        if not _digest(lock_digest):
            errors.append(f"{prefix}.lock_digest must be lowercase SHA-256")
        elif lock_digest != _lock_digest(record):
            errors.append(f"{prefix}.lock_digest must bind authority lock")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority lock records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority lock records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LOCK_V234_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LOCK_V234_VALID")
    return 0
