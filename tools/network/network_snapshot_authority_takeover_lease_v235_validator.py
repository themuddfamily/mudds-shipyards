#!/usr/bin/env python3
"""Validate detached v235 network snapshot authority takeover lease evidence."""
...
SCHEMA_VERSION = 235
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_lease_v235"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
LEASE_ID = "server-authority-lease-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v6"
SNAPSHOT_ID = "snapshot-authority-v118"
...
def _lease_digest(record: dict[str, Any]) -> str:
    fields = (
        "lease_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('lease_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover lease evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority lease")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority lease")
        ...
        lease_digest = record.get("lease_digest")
        if not _digest(lease_digest):
            errors.append(f"{prefix}.lease_digest must be lowercase SHA-256")
        elif lease_digest != _lease_digest(record):
            errors.append(f"{prefix}.lease_digest must bind authority lease")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority lease records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority lease records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LEASE_V235_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LEASE_V235_VALID")
    return 0
