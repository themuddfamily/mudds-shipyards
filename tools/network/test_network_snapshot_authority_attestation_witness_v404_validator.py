...
try:
    from .network_snapshot_authority_attestation_witness_v404_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_attestation_witness_v404_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-attestation-witness-assertion-v1",
        "integrity_gate_id": "authority-attestation-witness-v404",
        "snapshot_id": "snapshot-authority-v287", "authority": "server", "sequence": 45304,
...
def _report():
    ...
    return {
        "schema_version": 404,
        "evidence_scope": "network_snapshot_authority_attestation_witness_v404",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-attestation-witness-v404",
        "snapshot_id": "snapshot-authority-v287",
        "assertion_id": "authority-snapshot-attestation-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 176, "release": "release-1",
...
class NetworkSnapshotAuthorityAttestationWitnessV404ValidatorTest(unittest.TestCase):
    def test_accepts_attestation_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_attestation_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-attestation"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority attestation witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["attestation"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority attestation members" in error for error in errors))
