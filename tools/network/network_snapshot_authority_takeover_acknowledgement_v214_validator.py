#!/usr/bin/env python3
"""Validate detached v214 network snapshot authority-takeover acknowledgement evidence."""
...
SCHEMA_VERSION = 214
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_acknowledgement_v214"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_ACKNOWLEDGEMENT = "server-authority-takeover-acknowledgement-v1"
SNAPSHOT_ID = "snapshot-authority-v97"
...
def _acknowledgement_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_acknowledgement", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('acknowledgement_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover acknowledgement evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_acknowledgement": AUTHORITY_TAKEOVER_ACKNOWLEDGEMENT,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_acknowledgement", report.get("authority_takeover_acknowledgement")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover acknowledgement")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover acknowledgement")
        ...
        acknowledgement_digest = record.get("acknowledgement_digest")
        if not _digest(acknowledgement_digest):
            errors.append(f"{prefix}.acknowledgement_digest must be lowercase SHA-256")
        elif acknowledgement_digest != _acknowledgement_digest(record):
            errors.append(f"{prefix}.acknowledgement_digest must bind authority takeover acknowledgement")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover acknowledgement records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover acknowledgement records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ACKNOWLEDGEMENT_V214_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ACKNOWLEDGEMENT_V214_VALID")
    return 0
