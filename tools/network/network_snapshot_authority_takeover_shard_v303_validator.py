#!/usr/bin/env python3
"""Validate detached v303 network snapshot authority-takeover shard evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v186"
...
def _shard_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('shard_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover shard evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority shard")
    ...
    items = report.get("shards")
    if not isinstance(items, list):
        errors.append(f"{label}.shards must be an array")
        items = []
    ids: set[str] = set()
    allocated = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.shards[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority shard")
        if item.get("sequence") != snapshot.get("sequence"):
            errors.append(f"{prefix}.sequence must match snapshot")
        if not isinstance(item.get("bucket"), str) or not item["bucket"]:
            errors.append(f"{prefix}.bucket must be non-empty")
        ...
        shard_digest = item.get("shard_digest")
        if not _digest(shard_digest):
            errors.append(f"{prefix}.shard_digest must be lowercase SHA-256")
        elif shard_digest != _shard_digest(item):
            errors.append(f"{prefix}.shard_digest must bind authority shard")
        if item.get("allocated") is not True:
            errors.append(f"{prefix}.allocated must be true")
        else:
            allocated += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority shards")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "shards": len(items), "unique": len(ids), "allocated": allocated, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority shards")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SHARD_V303_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SHARD_V303_VALID")
    return 0
