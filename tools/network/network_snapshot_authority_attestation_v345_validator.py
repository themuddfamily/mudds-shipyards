#!/usr/bin/env python3
"""Validate detached v345 authority/snapshot attestation evidence."""
...
AUTHORITY = "server"
ATTESTATION_ID = "authority-attestation-v345"
SNAPSHOT_ID = "snapshot-authority-v228"
ASSERTION_ID = "authority-snapshot-attestation-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 117
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for index, item in enumerate(items):
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority attestation")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority attestation members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority attestation members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ATTESTATION_V345_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ATTESTATION_V345_VALID")
    return 0
