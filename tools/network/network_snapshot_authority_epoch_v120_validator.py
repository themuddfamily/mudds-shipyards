#!/usr/bin/env python3
"""Validate detached v120 network snapshot authority-epoch evidence."""
...
SCHEMA_VERSION = 120
EVIDENCE_SCOPE = "network_snapshot_authority_epoch_v120"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_EPOCH = "server-authority-epoch-v2"
SNAPSHOT_ID = "snapshot-authority-v3"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_epoch", "snapshot_id", "authority", "source", "release", "version",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-epoch/snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_epoch": AUTHORITY_EPOCH,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (
        ("authority_epoch", report.get("authority_epoch")), ("snapshot_id", report.get("snapshot_id")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_EPOCH_V120_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_EPOCH_V120_VALID")
    return 0
