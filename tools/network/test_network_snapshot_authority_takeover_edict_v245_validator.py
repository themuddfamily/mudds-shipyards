...
try:
    from .network_snapshot_authority_takeover_edict_v245_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_edict_v245_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["edict_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['edict_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "edict_id": "server-authority-edict-v1",
            "authority_takeover_token": "server-authority-takeover-token-v16",
            "snapshot_id": "snapshot-authority-v128", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 17,
            "sequence": 18140, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverEdictV245ValidatorTest(unittest.TestCase):
    def test_accepts_authority_edict(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_edict_binding(self):
        report = _report()
        report["records"][0]["edict_id"] = "server-authority-edict-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority edict" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority edict records" in error for error in errors))
