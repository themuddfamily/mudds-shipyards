...
try:
    from .network_snapshot_authority_source_witness_v387_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_source_witness_v387_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-source-witness-assertion-v1",
        "integrity_gate_id": "authority-source-witness-v387",
        "snapshot_id": "snapshot-authority-v270", "authority": "server", "sequence": 43587,
...
def _report():
    ...
    return {
        "schema_version": 387,
        "evidence_scope": "network_snapshot_authority_source_witness_v387",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-source-witness-v387",
        "snapshot_id": "snapshot-authority-v270",
        "assertion_id": "authority-snapshot-source-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 159, "release": "release-1",
...
class NetworkSnapshotAuthoritySourceWitnessV387ValidatorTest(unittest.TestCase):
    def test_accepts_source_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_source_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["integrity_gate_id"] = "wrong-source"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority source witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["source"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority source members" in error for error in errors))
