...
try:
    from .network_snapshot_authority_takeover_lineage_v226_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_lineage_v226_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["lineage_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['lineage_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_lineage": "server-authority-takeover-lineage-v1",
            "snapshot_id": "snapshot-authority-v109", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3109, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverLineageV226ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_lineage_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_lineage_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_lineage"] = "server-authority-takeover-lineage-old"
        self.assertTrue(any("bind authority takeover lineage" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v108"
        self.assertTrue(any("bind authority takeover lineage" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover lineage records" in error for error in validate_snapshot(report)))
