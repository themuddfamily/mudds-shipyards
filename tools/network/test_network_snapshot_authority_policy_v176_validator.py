...
try:
    from .network_snapshot_authority_policy_v176_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_policy_v176_validator import validate_snapshot
...
class NetworkSnapshotAuthorityPolicyV176ValidatorTest(unittest.TestCase):
    def test_accepts_authority_policy_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_policy_binding(self):
        report = _report()
        report["records"][0]["authority_policy"] = "server-authority-policy-old"
        self.assertTrue(any("bind authority policy" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v58"
        self.assertTrue(any("bind authority policy" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority policy records" in error for error in validate_snapshot(report)))
