...
try:
    from .network_snapshot_authority_endorsement_v195_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_endorsement_v195_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["endorsement_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['endorsement_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_endorsement": "server-authority-endorsement-v1",
            "snapshot_id": "snapshot-authority-v78", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2985, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityEndorsementV195ValidatorTest(unittest.TestCase):
    def test_accepts_endorsed_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_endorsement_binding(self):
        report = _report()
        report["records"][0]["authority_endorsement"] = "server-authority-endorsement-old"
        self.assertTrue(any("bind authority endorsement" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v77"
        self.assertTrue(any("bind authority endorsement" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority endorsement records" in error for error in validate_snapshot(report)))
