#!/usr/bin/env python3
"""Validate detached v254 network snapshot authority takeover bundle evidence."""
...
SCHEMA_VERSION = 254
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_bundle_v254"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
BUNDLE_ID = "server-authority-bundle-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v25"
SNAPSHOT_ID = "snapshot-authority-v137"
...
def _bundle_digest(record: dict[str, Any]) -> str:
    fields = (
        "bundle_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('bundle_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for index, record in enumerate(records):
        ...
        elif bundle_digest != _bundle_digest(record):
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority bundle records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority bundle records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_BUNDLE_V254_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_BUNDLE_V254_VALID")
    return 0
