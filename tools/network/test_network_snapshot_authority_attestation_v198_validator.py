...
try:
    from .network_snapshot_authority_attestation_v198_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_attestation_v198_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "authority_attestation": "server-authority-attestation-v1",
            "snapshot_id": "snapshot-authority-v81", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2997, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityAttestationV198ValidatorTest(unittest.TestCase):
    def test_accepts_attested_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_attestation_binding(self):
        report = _report()
        report["records"][0]["authority_attestation"] = "server-authority-attestation-old"
        self.assertTrue(any("bind authority attestation" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v80"
        self.assertTrue(any("bind authority attestation" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority attestation records" in error for error in validate_snapshot(report)))
