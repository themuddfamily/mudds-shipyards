#!/usr/bin/env python3
"""Validate detached v295 network snapshot authority-takeover registry evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v178"
...
def _registry_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('registry_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover registry evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority registry")
    ...
    items = report.get("registry_entries")
    if not isinstance(items, list):
        errors.append(f"{label}.registry_entries must be an array")
        items = []
    ids: set[str] = set()
    registered = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.registry_entries[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority registry")
        ...
        if not _digest(registry_digest):
            errors.append(f"{prefix}.registry_digest must be lowercase SHA-256")
        elif registry_digest != _registry_digest(item):
            errors.append(f"{prefix}.registry_digest must bind authority registry")
        if item.get("registered") is not True:
            errors.append(f"{prefix}.registered must be true")
        else:
            registered += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority registry entries")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "entries": len(items), "unique": len(ids), "registered": registered, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority registry entries")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_REGISTRY_V295_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_REGISTRY_V295_VALID")
    return 0
