...
try:
    from .network_snapshot_authority_takeover_closure_v224_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_closure_v224_validator import validate_snapshot
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['closure_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_closure": "server-authority-takeover-closure-v1",
            "snapshot_id": "snapshot-authority-v107", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3101, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverClosureV224ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_closure_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_closure_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_closure"] = "server-authority-takeover-closure-old"
        self.assertTrue(any("bind authority takeover closure" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v106"
        self.assertTrue(any("bind authority takeover closure" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover closure records" in error for error in validate_snapshot(report)))
