...
try:
    from .network_snapshot_authority_takeover_writ_v246_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_writ_v246_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["writ_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['writ_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "writ_id": "server-authority-writ-v1",
            "authority_takeover_token": "server-authority-takeover-token-v17",
            "snapshot_id": "snapshot-authority-v129", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 18,
            "sequence": 19141, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverWritV246ValidatorTest(unittest.TestCase):
    def test_accepts_authority_writ(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_writ_binding(self):
        report = _report()
        report["records"][0]["writ_id"] = "server-authority-writ-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority writ" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority writ records" in error for error in errors))
