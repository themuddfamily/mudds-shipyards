#!/usr/bin/env python3
"""Validate detached v71 snapshot release/lineage evidence."""
...
SCHEMA_VERSION = 71
EVIDENCE_SCOPE = "network_snapshot_release_lineage_v71"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
LINEAGE = "snapshot-lineage-root-v1"
...
def _release_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "release", "lineage", "source", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('release_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()


def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for release/lineage snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "release": RELEASE_ID, "lineage": LINEAGE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("release", report.get("release")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match release/lineage")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("release", report.get("release")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match release/lineage")
        ...
        if not _digest(release_digest):
            errors.append(f"{prefix}.release_digest must be lowercase SHA-256")
        elif release_digest != _release_digest(record):
            errors.append(f"{prefix}.release_digest must bind release/lineage")
        if record.get("released") is not True:
            errors.append(f"{prefix}.released must be true")
        else:
            released += 1
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match release records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "released": released, "mutations": mutations}
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match release records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_RELEASE_LINEAGE_V71_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_RELEASE_LINEAGE_V71_VALID")
    return 0
