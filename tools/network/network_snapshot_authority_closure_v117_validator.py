#!/usr/bin/env python3
"""Validate detached v117 network snapshot authority/closure evidence."""
...
SCHEMA_VERSION = 117
EVIDENCE_SCOPE = "network_snapshot_authority_closure_v117"
...
def _authority_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority", "closure", "source", "release", "version", "check_id",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('authority_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/closure snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (
        ("authority", AUTHORITY), ("closure", report.get("closure")),
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(authority_digest):
            ...
            errors.append(f"{prefix}.authority_digest must bind authority/closure")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CLOSURE_V117_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CLOSURE_V117_VALID")
    return 0
