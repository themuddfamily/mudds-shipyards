#!/usr/bin/env python3
"""Validate detached v228 network snapshot authority-takeover signature evidence."""
...
SCHEMA_VERSION = 228
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_signature_v228"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_SIGNATURE = "server-authority-takeover-signature-v1"
SNAPSHOT_ID = "snapshot-authority-v111"
...
def _signature_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_signature", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('signature_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_signature": AUTHORITY_TAKEOVER_SIGNATURE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_signature", report.get("authority_takeover_signature")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover signature")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover signature")
        ...
        elif signature_digest != _signature_digest(record):
            errors.append(f"{prefix}.signature_digest must bind authority takeover signature")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover signature records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover signature records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SIGNATURE_V228_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SIGNATURE_V228_VALID")
    return 0
