#!/usr/bin/env python3
"""Validate detached v282 network snapshot authority-takeover capsule evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v165"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    items = report.get("capsule_slots")
    if not isinstance(items, list):
        errors.append(f"{label}.capsule_slots must be an array")
        items = []
    ids: set[str] = set()
    filled = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.capsule_slots[{index}]"
        ...
        if not isinstance(item.get("slot"), str) or not item["slot"]:
            errors.append(f"{prefix}.slot must be non-empty")
        ...
        if item.get("filled") is not True:
            errors.append(f"{prefix}.filled must be true")
        else:
            filled += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority capsule slots")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "slots": len(items), "unique": len(ids), "filled": filled, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority capsule slots")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CAPSULE_V282_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CAPSULE_V282_VALID")
    return 0
