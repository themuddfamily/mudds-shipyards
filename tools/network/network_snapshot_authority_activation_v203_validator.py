#!/usr/bin/env python3
"""Validate detached v203 network snapshot authority-activation evidence."""
...
SCHEMA_VERSION = 203
EVIDENCE_SCOPE = "network_snapshot_authority_activation_v203"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ACTIVATION = "server-authority-activation-v1"
SNAPSHOT_ID = "snapshot-authority-v86"
...
def _activation_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_activation", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('activation_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-activation snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_activation", report.get("authority_activation")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority activation")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority activation")
        ...
        activation_digest = record.get("activation_digest")
        if not _digest(activation_digest):
            errors.append(f"{prefix}.activation_digest must be lowercase SHA-256")
        elif activation_digest != _activation_digest(record):
            errors.append(f"{prefix}.activation_digest must bind authority activation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority activation records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority activation records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ACTIVATION_V203_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ACTIVATION_V203_VALID")
    return 0
