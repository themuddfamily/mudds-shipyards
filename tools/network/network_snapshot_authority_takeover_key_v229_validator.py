#!/usr/bin/env python3
"""Validate detached v229 network snapshot authority-takeover key evidence."""
...
SCHEMA_VERSION = 229
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_key_v229"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_KEY = "server-authority-takeover-key-v1"
SNAPSHOT_ID = "snapshot-authority-v112"
...
def _key_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_key", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('key_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover key evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_key": AUTHORITY_TAKEOVER_KEY,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_key", report.get("authority_takeover_key")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover key")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover key")
        ...
        if not _digest(key_digest):
            errors.append(f"{prefix}.key_digest must be lowercase SHA-256")
        elif key_digest != _key_digest(record):
            errors.append(f"{prefix}.key_digest must bind authority takeover key")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover key records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover key records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_KEY_V229_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_KEY_V229_VALID")
    return 0
