#!/usr/bin/env python3
"""Validate detached v57 snapshot channel/release/source reconciliation evidence."""
...
SCHEMA_VERSION = 57
EVIDENCE_SCOPE = "network_snapshot_channel_release_source_reconciliation_v57"
...
def _reconciliation_digest(check: dict[str, Any]) -> str:
    material = "|".join(str(check.get(key)) for key in (
        "authority", "channel", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_checks(report: Any, label: str = "checks") -> list[str]:
    """Return errors for source/channel/release-bound reconciliation evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "channel": CHANNEL, "source": SOURCE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("channel", report.get("channel")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match source/channel/release contract")
    ...
    for index, check in enumerate(checks):
        ...
        for key, value in (("authority", AUTHORITY), ("channel", report.get("channel")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if check.get(key) != value:
                errors.append(f"{prefix}.{key} must match source/channel/release contract")
        ...
        if not _digest(reconciliation_digest):
            ...
            errors.append(f"{prefix}.reconciliation_digest must bind source/channel/release")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match source reconciliations")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match source reconciliations")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_CHANNEL_RELEASE_SOURCE_RECONCILIATION_V57_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_CHANNEL_RELEASE_SOURCE_RECONCILIATION_V57_VALID")
    return 0
