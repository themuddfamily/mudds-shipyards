#!/usr/bin/env python3
"""Validate detached v317 authority/snapshot set evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v317"
SNAPSHOT_ID = "snapshot-authority-v200"
SET_ID = "authority-snapshot-set-v1"
...
def _set_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('set_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot set evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority set")
    ...
    items = report.get("set_members")
    if not isinstance(items, list):
        errors.append(f"{label}.set_members must be an array")
        items = []
    ids: set[str] = set()
    included = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.set_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority set")
        ...
        set_digest = item.get("set_digest")
        if not _digest(set_digest):
            errors.append(f"{prefix}.set_digest must be lowercase SHA-256")
        elif set_digest != _set_digest(item):
            errors.append(f"{prefix}.set_digest must bind authority set")
        if item.get("included") is not True:
            errors.append(f"{prefix}.included must be true")
        else:
            included += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority set members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "set_members": len(items), "unique": len(ids), "included": included, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority set members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SET_V317_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SET_V317_VALID")
    return 0
