...
try:
    from .network_snapshot_authority_takeover_seal_v310_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_seal_v310_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "seal_id": "authority-snapshot-seal-v1", "takeover_id": "authority-takeover-v310",
        "snapshot_id": "snapshot-authority-v193", "authority": "server", "sequence": 42240,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "sealed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "seal_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["seal_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['seal_digest']}" for item in items
    )
    return {
        "schema_version": 310,
        "evidence_scope": "network_snapshot_authority_takeover_seal_v310",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v310",
        "snapshot_id": "snapshot-authority-v193", "seal_id": "authority-snapshot-seal-v1",
        "source": "server_snapshot", "snapshot_version": 82, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v310", "snapshot_id": "snapshot-authority-v193",
            "seal_id": "authority-snapshot-seal-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 82,
            "sequence": 42240, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "seals": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"seals": 2, "unique": 2, "sealed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverSealV310ValidatorTest(unittest.TestCase):
    def test_accepts_seal_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_seal_binding(self):
        report = _report()
        report["seals"][0]["seal_id"] = "wrong-seal"
    ...
    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["seals"][1]["order"] = 1
        report["seals"][1]["item_id"] = report["seals"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["seals"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["seals"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority seals" in error for error in errors))
