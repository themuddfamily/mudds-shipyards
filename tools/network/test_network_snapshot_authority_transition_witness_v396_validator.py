...
try:
    from .network_snapshot_authority_transition_witness_v396_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_transition_witness_v396_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-transition-witness-assertion-v1",
        "integrity_gate_id": "authority-transition-witness-v396",
        "snapshot_id": "snapshot-authority-v279", "authority": "server", "sequence": 44496,
...
def _report():
    ...
    return {
        "schema_version": 396,
        "evidence_scope": "network_snapshot_authority_transition_witness_v396",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-transition-witness-v396",
        "snapshot_id": "snapshot-authority-v279",
        "assertion_id": "authority-snapshot-transition-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 168, "release": "release-1",
...
class NetworkSnapshotAuthorityTransitionWitnessV396ValidatorTest(unittest.TestCase):
    def test_accepts_transition_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_transition_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-transition"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority transition witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["transition"]
