#!/usr/bin/env python3
"""Validate detached v312 authority/snapshot ledger evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v312"
SNAPSHOT_ID = "snapshot-authority-v195"
LEDGER_ID = "authority-snapshot-ledger-v1"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot ledger evidence."""
    errors: list[str] = []
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority ledger")
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {
            "ledger_entries": len(items), "unique": len(ids), "recorded": recorded, "mutations": mutations,
        }
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LEDGER_V312_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LEDGER_V312_VALID")
    return 0
