...
try:
    from .network_snapshot_authority_takeover_claim_v220_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_claim_v220_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["claim_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['claim_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_claim": "server-authority-takeover-claim-v1",
            "snapshot_id": "snapshot-authority-v103", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3085, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverClaimV220ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_claim_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_claim_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_claim"] = "server-authority-takeover-claim-old"
        self.assertTrue(any("bind authority takeover claim" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v102"
        self.assertTrue(any("bind authority takeover claim" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover claim records" in error for error in validate_snapshot(report)))
