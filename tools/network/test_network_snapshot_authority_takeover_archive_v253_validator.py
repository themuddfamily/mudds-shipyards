...
try:
    from .network_snapshot_authority_takeover_archive_v253_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_archive_v253_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["archive_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['archive_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "archive_id": "server-authority-archive-v1",
            "authority_takeover_token": "server-authority-takeover-token-v24",
            "snapshot_id": "snapshot-authority-v136", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 25,
            "sequence": 26148, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverArchiveV253ValidatorTest(unittest.TestCase):
    def test_accepts_authority_archive(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_archive_binding(self):
        report = _report()
        report["records"][0]["archive_id"] = "server-authority-archive-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority archive" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority archive records" in error for error in errors))
