...
try:
    from .network_snapshot_authority_takeover_index_v313_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_index_v313_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "index_id": "authority-snapshot-index-v1", "takeover_id": "authority-takeover-v313",
        "snapshot_id": "snapshot-authority-v196", "authority": "server", "sequence": 42258,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "indexed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "index_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["index_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['index_digest']}" for item in items
    )
    return {
        "schema_version": 313,
        "evidence_scope": "network_snapshot_authority_takeover_index_v313",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v313",
        "snapshot_id": "snapshot-authority-v196", "index_id": "authority-snapshot-index-v1",
        "source": "server_snapshot", "snapshot_version": 85, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v313", "snapshot_id": "snapshot-authority-v196",
            "index_id": "authority-snapshot-index-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 85,
            "sequence": 42258, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "index_entries": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"index_entries": 2, "unique": 2, "indexed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverIndexV313ValidatorTest(unittest.TestCase):
    def test_accepts_index_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_index_binding(self):
        report = _report()
        report["index_entries"][0]["index_id"] = "wrong-index"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority index" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["index_entries"][1]["order"] = 1
        report["index_entries"][1]["item_id"] = report["index_entries"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["index_entries"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["indexed"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.indexed" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["index_entries"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority index entries" in error for error in errors))
