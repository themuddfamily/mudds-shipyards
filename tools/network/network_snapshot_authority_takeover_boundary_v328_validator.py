#!/usr/bin/env python3
"""Validate detached v328 authority/snapshot boundary evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v328"
SNAPSHOT_ID = "snapshot-authority-v211"
BOUNDARY_ID = "authority-snapshot-boundary-v1"
...
def _boundary_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('boundary_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot boundary evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority boundary")
    ...
    items = report.get("boundary_members")
    if not isinstance(items, list):
        errors.append(f"{label}.boundary_members must be an array")
        items = []
    ids: set[str] = set()
    enclosed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.boundary_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority boundary")
        ...
        boundary_digest = item.get("boundary_digest")
        if not _digest(boundary_digest):
            errors.append(f"{prefix}.boundary_digest must be lowercase SHA-256")
        elif boundary_digest != _boundary_digest(item):
            errors.append(f"{prefix}.boundary_digest must bind authority boundary")
        if item.get("enclosed") is not True:
            errors.append(f"{prefix}.enclosed must be true")
        else:
            enclosed += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority boundary members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "boundary_members": len(items), "unique": len(ids), "enclosed": enclosed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority boundary members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_BOUNDARY_V328_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_BOUNDARY_V328_VALID")
    return 0
