...
try:
    from .network_snapshot_authority_takeover_set_v317_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_set_v317_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "set_id": "authority-snapshot-set-v1", "takeover_id": "authority-takeover-v317",
        "snapshot_id": "snapshot-authority-v200", "authority": "server", "sequence": 42282,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "included": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "set_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["set_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['set_digest']}" for item in items
    )
    return {
        "schema_version": 317,
        "evidence_scope": "network_snapshot_authority_takeover_set_v317",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v317",
        "snapshot_id": "snapshot-authority-v200", "set_id": "authority-snapshot-set-v1",
        "source": "server_snapshot", "snapshot_version": 89, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v317", "snapshot_id": "snapshot-authority-v200",
            "set_id": "authority-snapshot-set-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 89,
            "sequence": 42282, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "set_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"set_members": 2, "unique": 2, "included": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverSetV317ValidatorTest(unittest.TestCase):
    def test_accepts_set_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_set_binding(self):
        report = _report()
        report["set_members"][0]["set_id"] = "wrong-set"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority set" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["set_members"][1]["order"] = 1
        report["set_members"][1]["item_id"] = report["set_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["set_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["included"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.included" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["set_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority set members" in error for error in errors))
