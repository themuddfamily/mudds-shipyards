...
try:
    from .network_snapshot_authority_principal_v171_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_principal_v171_validator import validate_snapshot
...
class NetworkSnapshotAuthorityPrincipalV171ValidatorTest(unittest.TestCase):
    def test_accepts_authority_principal_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_principal_binding(self):
        report = _report()
        report["records"][0]["authority_principal"] = "server-authority-principal-old"
        self.assertTrue(any("bind authority principal" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v53"
        self.assertTrue(any("bind authority principal" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority principal records" in error for error in validate_snapshot(report)))
