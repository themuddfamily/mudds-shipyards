#!/usr/bin/env python3
"""Validate detached v122 network snapshot authority-proof evidence."""
...
SCHEMA_VERSION = 122
EVIDENCE_SCOPE = "network_snapshot_authority_proof_v122"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_PROOF = "server-authority-proof-v2"
SNAPSHOT_ID = "snapshot-authority-v5"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_proof", "snapshot_id", "authority", "source", "release", "version",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-proof snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_proof": AUTHORITY_PROOF,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (
        ("authority_proof", report.get("authority_proof")), ("snapshot_id", report.get("snapshot_id")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_PROOF_V122_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_PROOF_V122_VALID")
    return 0
