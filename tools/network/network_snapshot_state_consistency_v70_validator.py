#!/usr/bin/env python3
"""Validate detached v70 snapshot state/consistency evidence."""
...
SCHEMA_VERSION = 70
EVIDENCE_SCOPE = "network_snapshot_state_consistency_v70"
...
def _state_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "state", "consistency", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('state_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()


def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for state/consistency snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "state": STATE, "consistency": CONSISTENCY,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("state", report.get("state")), ("consistency", report.get("consistency")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match state/consistency")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("state", report.get("state")), ("consistency", report.get("consistency")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match state/consistency")
        ...
        if not _digest(state_digest):
            errors.append(f"{prefix}.state_digest must be lowercase SHA-256")
        elif state_digest != _state_digest(record):
            errors.append(f"{prefix}.state_digest must bind state/consistency")
        if record.get("stable") is not True:
            errors.append(f"{prefix}.stable must be true")
        else:
            stable += 1
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match state records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "stable": stable, "mutations": mutations}
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match state records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_STATE_CONSISTENCY_V70_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_STATE_CONSISTENCY_V70_VALID")
    return 0
