...
try:
    from .network_snapshot_authority_takeover_audit_v225_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_audit_v225_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_audit": "server-authority-takeover-audit-v1",
            "snapshot_id": "snapshot-authority-v108", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3105, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverAuditV225ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_audit_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_audit_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_audit"] = "server-authority-takeover-audit-old"
        self.assertTrue(any("bind authority takeover audit" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v107"
        self.assertTrue(any("bind authority takeover audit" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover audit records" in error for error in validate_snapshot(report)))
