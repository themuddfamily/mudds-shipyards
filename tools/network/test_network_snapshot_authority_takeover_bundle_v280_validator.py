...
try:
    from .network_snapshot_authority_takeover_bundle_v280_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_bundle_v280_validator import validate_snapshot


def _part(order, item_id, digest, part):
    item = {
        "order": order, "item_id": item_id,
        "bundle_id": "server-authority-bundle-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v51",
        "snapshot_id": "snapshot-authority-v163", "sequence": 42175,
        "part": part, "expected_digest": digest, "observed_digest": digest,
        "assembled": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
...
def _report():
    items = [
        _part(1, "authority", hashlib.sha256(b"authority").hexdigest(), "authority"),
        _part(2, "snapshot", hashlib.sha256(b"snapshot").hexdigest(), "snapshot"),
    ]
    material = "\n".join(
    ...
    return {
        ...
        "authority": "server", "bundle_id": "server-authority-bundle-v1",
        "takeover_token": "server-authority-takeover-token-v51",
        "snapshot_id": "snapshot-authority-v163", "source": "server_snapshot",
        "snapshot_version": 52, "release": "release-1",
        ...
        "snapshot": {
            ...
            "takeover_token": "server-authority-takeover-token-v51",
            "snapshot_id": "snapshot-authority-v163", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 52,
            "sequence": 42175, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "bundle_parts": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"parts": 2, "unique": 2, "assembled": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverBundleV280ValidatorTest(unittest.TestCase):
    def test_accepts_bundle_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_bundle_binding(self):
        report = _report()
        report["bundle_parts"][0]["takeover_token"] = "wrong-token"
    ...
    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["bundle_parts"][1]["order"] = 1
        report["bundle_parts"][1]["item_id"] = report["bundle_parts"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_rollup_and_counts(self):
        ...
        report["counts"]["parts"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority bundle parts" in error for error in errors))
        self.assertTrue(any("counts.parts" in error for error in errors))

    def test_rejects_mutation(self):
        report = _report()
        report["bundle_parts"][0]["mutation_fields"] = ["authority"]
