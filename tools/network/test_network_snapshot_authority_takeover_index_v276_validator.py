...
try:
    from .network_snapshot_authority_takeover_index_v276_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_index_v276_validator import validate_snapshot


def _item(order, item_id, digest, category):
    item = {
        "order": order, "item_id": item_id,
        "index_id": "server-authority-index-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v47",
        "snapshot_id": "snapshot-authority-v159", "sequence": 42171,
        "category": category, "expected_digest": digest, "observed_digest": digest,
        "indexed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "index_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "category", "item_id", "expected_digest", "observed_digest",
    )
...
def _report():
    ...
    return {
        "schema_version": 276,
        "evidence_scope": "network_snapshot_authority_takeover_index_v276",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "index_id": "server-authority-index-v1",
        "takeover_token": "server-authority-takeover-token-v47",
        "snapshot_id": "snapshot-authority-v159", "source": "server_snapshot",
        "snapshot_version": 48, "release": "release-1",
        ...
        "snapshot": {
            "index_id": "server-authority-index-v1",
            "takeover_token": "server-authority-takeover-token-v47",
            "snapshot_id": "snapshot-authority-v159", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 48,
            "sequence": 42171, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "index_items": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"items": 2, "unique": 2, "indexed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverIndexV276ValidatorTest(unittest.TestCase):
    def test_accepts_index_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_index_binding(self):
        report = _report()
        report["index_items"][0]["takeover_token"] = "wrong-token"
    ...
    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["index_items"][1]["order"] = 1
        report["index_items"][1]["item_id"] = report["index_items"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_rollup_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority index items" in error for error in errors))
        self.assertTrue(any("counts.items" in error for error in errors))

    def test_rejects_mutation(self):
        report = _report()
        report["index_items"][0]["mutation_fields"] = ["authority"]
