#!/usr/bin/env python3
"""Validate detached v334 authority/snapshot message evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v334"
SNAPSHOT_ID = "snapshot-authority-v217"
MESSAGE_ID = "authority-snapshot-message-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 106
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('message_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot message evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority message")
    ...
    items = report.get("message_members")
    if not isinstance(items, list):
        errors.append(f"{label}.message_members must be an array")
        items = []
    ids: set[str] = set()
    acknowledged = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.message_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority message")
        ...
        message_digest = item.get("message_digest")
        if not _digest(message_digest):
            errors.append(f"{prefix}.message_digest must be lowercase SHA-256")
        elif message_digest != _message_digest(item):
            errors.append(f"{prefix}.message_digest must bind authority message")
        if item.get("acknowledged") is not True:
            errors.append(f"{prefix}.acknowledged must be true")
        else:
            acknowledged += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority message members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "message_members": len(items), "unique": len(ids), "acknowledged": acknowledged, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority message members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MESSAGE_V334_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MESSAGE_V334_VALID")
    return 0
