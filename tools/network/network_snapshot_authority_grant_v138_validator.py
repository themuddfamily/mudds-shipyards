#!/usr/bin/env python3
"""Validate detached v138 network snapshot authority-grant evidence."""
...
SCHEMA_VERSION = 138
EVIDENCE_SCOPE = "network_snapshot_authority_grant_v138"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_GRANT = "server-authority-grant-v2"
SNAPSHOT_ID = "snapshot-authority-v21"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-grant snapshot evidence."""
    errors: list[str] = []
    if not isinstance(report, dict):
        return [f"{label} must be an object"]
    expected = {"schema_version": SCHEMA_VERSION, "evidence_scope": EVIDENCE_SCOPE, "evidence_mode": EVIDENCE_MODE, "policy_version": POLICY_VERSION, "authority": AUTHORITY, "authority_grant": AUTHORITY_GRANT, "snapshot_id": SNAPSHOT_ID, "source": SOURCE, "snapshot_version": SNAPSHOT_VERSION, "release": RELEASE_ID}
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority_grant", report.get("authority_grant")), ("snapshot_id", report.get("snapshot_id")), ("authority", AUTHORITY), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_GRANT_V138_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_GRANT_V138_VALID")
    return 0
