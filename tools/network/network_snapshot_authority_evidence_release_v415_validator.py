#!/usr/bin/env python3
"""Validate detached v415 authority/snapshot evidence-release evidence."""
...
SCHEMA_VERSION = 415
EVIDENCE_SCOPE = "network_snapshot_authority_evidence_release_v415"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
INTEGRITY_GATE_ID = "authority-evidence-release-v415"
SNAPSHOT_ID = "snapshot-authority-v298"
ASSERTION_ID = "authority-snapshot-evidence-release-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 187
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot evidence-release evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("integrity_gate_id", report.get("integrity_gate_id")),
                       ("snapshot_id", report.get("snapshot_id")), ("assertion_id", report.get("assertion_id")),
                       ("authority", AUTHORITY), ("source", report.get("source")),
                       ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority evidence release")
    ...
    for index, item in enumerate(items):
        ...
        if not isinstance(item_id, str) or not item_id:
            ...
            ids.add(item_id)
        for key, value in (("assertion_id", report.get("assertion_id")),
                           ("integrity_gate_id", report.get("integrity_gate_id")),
                           ("snapshot_id", report.get("snapshot_id")), ("authority", AUTHORITY)):
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority evidence release")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority evidence release")
        ...
        if item.get("mutation_fields") != [] or item.get("state_changed") is not False:
            mutations += 1; errors.append(f"{prefix} must have no mutation")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_RELEASE_V415_INVALID")
        for error in errors: print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_RELEASE_V415_VALID")
    return 0
