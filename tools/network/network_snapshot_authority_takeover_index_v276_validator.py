#!/usr/bin/env python3
"""Validate detached v276 network snapshot authority-takeover index evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v159"
...
def _index_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('index_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover index evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority index")
    ...
    items = report.get("index_items")
    if not isinstance(items, list):
        errors.append(f"{label}.index_items must be an array")
        items = []
    ids: set[str] = set()
    indexed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.index_items[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority index")
        ...
        index_digest = item.get("index_digest")
        if not _digest(index_digest):
            errors.append(f"{prefix}.index_digest must be lowercase SHA-256")
        elif index_digest != _index_digest(item):
            errors.append(f"{prefix}.index_digest must bind authority index")
        if item.get("indexed") is not True:
            errors.append(f"{prefix}.indexed must be true")
        else:
            indexed += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority index items")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "items": len(items), "unique": len(ids), "indexed": indexed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority index items")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INDEX_V276_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INDEX_V276_VALID")
    return 0
