#!/usr/bin/env python3
"""Validate detached v293 network snapshot authority-takeover branch evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v176"
...
def _branch_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('branch_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover branch evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority branch")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority branch")
        ...
        branch_digest = item.get("branch_digest")
        if not _digest(branch_digest):
            errors.append(f"{prefix}.branch_digest must be lowercase SHA-256")
        elif branch_digest != _branch_digest(item):
            errors.append(f"{prefix}.branch_digest must bind authority branch")
        if item.get("selected") is not True:
            errors.append(f"{prefix}.selected must be true")
        else:
            selected += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority branches")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "branches": len(items), "unique": len(ids), "selected": selected, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority branches")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_BRANCH_V293_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_BRANCH_V293_VALID")
    return 0
