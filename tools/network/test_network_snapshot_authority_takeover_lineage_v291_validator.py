...
try:
    from .network_snapshot_authority_takeover_lineage_v291_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_lineage_v291_validator import validate_snapshot


def _item(order, item_id, digest, ancestor):
    item = {
        "order": order, "item_id": item_id,
        "lineage_id": "server-authority-lineage-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v62",
        "snapshot_id": "snapshot-authority-v174", "sequence": 42186,
        "ancestor": ancestor, "expected_digest": digest, "observed_digest": digest,
        "linked": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "lineage_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "ancestor", "item_id", "expected_digest", "observed_digest",
    )
    item["lineage_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['lineage_digest']}" for item in items
    )
    return {
        "schema_version": 291,
        "evidence_scope": "network_snapshot_authority_takeover_lineage_v291",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "lineage_id": "server-authority-lineage-v1",
        "takeover_token": "server-authority-takeover-token-v62",
        "snapshot_id": "snapshot-authority-v174", "source": "server_snapshot",
        "snapshot_version": 63, "release": "release-1",
        ...
        "snapshot": {
            "lineage_id": "server-authority-lineage-v1",
            "takeover_token": "server-authority-takeover-token-v62",
            "snapshot_id": "snapshot-authority-v174", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 63,
            "sequence": 42186, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "lineage_entries": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"entries": 2, "unique": 2, "linked": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverLineageV291ValidatorTest(unittest.TestCase):
    def test_accepts_lineage_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_lineage_binding(self):
        report = _report()
        report["lineage_entries"][0]["takeover_token"] = "wrong-token"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority lineage" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["lineage_entries"][1]["order"] = 1
        report["lineage_entries"][1]["item_id"] = report["lineage_entries"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_rollup_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority lineage entries" in error for error in errors))
        self.assertTrue(any("counts.entries" in error for error in errors))

    def test_rejects_mutation(self):
        report = _report()
        report["lineage_entries"][0]["mutation_fields"] = ["authority"]
