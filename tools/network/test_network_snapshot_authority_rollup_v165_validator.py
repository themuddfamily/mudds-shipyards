...
try:
    from .network_snapshot_authority_rollup_v165_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_rollup_v165_validator import validate_snapshot
...
class NetworkSnapshotAuthorityRollupV165ValidatorTest(unittest.TestCase):
    def test_accepts_authority_rollup_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_rollup_binding(self):
        report = _report()
        report["records"][0]["authority_rollup"] = "server-authority-rollup-old"
        self.assertTrue(any("bind authority rollup" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v47"
        self.assertTrue(any("bind authority rollup" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority rollup records" in error for error in validate_snapshot(report)))
