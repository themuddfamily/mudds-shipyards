...
try:
    from .network_snapshot_authority_activation_v203_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_activation_v203_validator import validate_snapshot
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['activation_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_activation": "server-authority-activation-v1",
            "snapshot_id": "snapshot-authority-v86", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3017, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityActivationV203ValidatorTest(unittest.TestCase):
    def test_accepts_activated_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_activation_binding(self):
        report = _report()
        report["records"][0]["authority_activation"] = "server-authority-activation-old"
        self.assertTrue(any("bind authority activation" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v85"
        self.assertTrue(any("bind authority activation" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority activation records" in error for error in validate_snapshot(report)))
