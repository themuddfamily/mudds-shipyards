...
try:
    from .network_snapshot_authority_handle_v170_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_handle_v170_validator import validate_snapshot
...
class NetworkSnapshotAuthorityHandleV170ValidatorTest(unittest.TestCase):
    def test_accepts_authority_handle_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_handle_binding(self):
        report = _report()
        report["records"][0]["authority_handle"] = "server-authority-handle-old"
        self.assertTrue(any("bind authority handle" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v52"
        self.assertTrue(any("bind authority handle" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority handle records" in error for error in validate_snapshot(report)))
