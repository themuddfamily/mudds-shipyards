#!/usr/bin/env python3
"""Validate detached v305 authority/snapshot witness evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v305"
SNAPSHOT_ID = "snapshot-authority-v188"
WITNESS_ID = "authority-snapshot-witness-v1"
...
def _witness_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('witness_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot witness evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority witness")
    ...
    for index, item in enumerate(items):
        prefix = f"{label}.witnesses[{index}]"
        ...
        witness_digest = item.get("witness_digest")
        if not _digest(witness_digest):
            errors.append(f"{prefix}.witness_digest must be lowercase SHA-256")
        elif witness_digest != _witness_digest(item):
            errors.append(f"{prefix}.witness_digest must bind authority witness")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority witnesses")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "witnesses": len(items), "unique": len(ids), "verified": verified, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority witnesses")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WITNESS_V305_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WITNESS_V305_VALID")
    return 0
