#!/usr/bin/env python3
"""Validate detached v200 network snapshot authority-admission evidence."""
...
SCHEMA_VERSION = 200
EVIDENCE_SCOPE = "network_snapshot_authority_admission_v200"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ADMISSION = "server-authority-admission-v1"
SNAPSHOT_ID = "snapshot-authority-v83"
...
def _admission_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_admission", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('admission_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-admission snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_admission", report.get("authority_admission")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority admission")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority admission")
        ...
        admission_digest = record.get("admission_digest")
        if not _digest(admission_digest):
            errors.append(f"{prefix}.admission_digest must be lowercase SHA-256")
        elif admission_digest != _admission_digest(record):
            errors.append(f"{prefix}.admission_digest must bind authority admission")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority admission records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority admission records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ADMISSION_V200_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ADMISSION_V200_VALID")
    return 0
