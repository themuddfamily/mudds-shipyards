#!/usr/bin/env python3
"""Validate detached v319 authority/snapshot partition evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v319"
SNAPSHOT_ID = "snapshot-authority-v202"
PARTITION_ID = "authority-snapshot-partition-v1"
...
def _partition_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('partition_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot partition evidence."""
    errors: list[str] = []
    ...
    snapshot_fields = (
        ...
        ("partition_id", report.get("partition_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority partition")
    ...
    items = report.get("partition_members")
    if not isinstance(items, list):
        errors.append(f"{label}.partition_members must be an array")
        items = []
    ids: set[str] = set()
    assigned = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.partition_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority partition")
        ...
        partition_digest = item.get("partition_digest")
        if not _digest(partition_digest):
            errors.append(f"{prefix}.partition_digest must be lowercase SHA-256")
        elif partition_digest != _partition_digest(item):
            errors.append(f"{prefix}.partition_digest must bind authority partition")
        if item.get("assigned") is not True:
            errors.append(f"{prefix}.assigned must be true")
        else:
            assigned += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority partition members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "partition_members": len(items), "unique": len(ids), "assigned": assigned, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority partition members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PARTITION_V319_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PARTITION_V319_VALID")
    return 0
