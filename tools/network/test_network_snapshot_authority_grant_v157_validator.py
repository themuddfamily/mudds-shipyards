...
try:
    from .network_snapshot_authority_grant_v157_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_grant_v157_validator import validate_snapshot
...
class NetworkSnapshotAuthorityGrantV157ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_grant_binding(self):
        report = _report()
        report["records"][0]["authority_grant"] = "server-authority-grant-old"
        self.assertTrue(any("bind authority grant" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v39"
        self.assertTrue(any("bind authority grant" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority grant records" in error for error in validate_snapshot(report)))
