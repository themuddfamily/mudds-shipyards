#!/usr/bin/env python3
"""Validate v45 dual-version linked authority/provenance evidence."""
...
SCHEMA_VERSION = 45
EVIDENCE_SCOPE = "network_snapshot_dual_version_authority_provenance_link_v45"
...
def _link_digest(link: dict[str, Any]) -> str:
    return hashlib.sha256(f"{link.get('authority')}|{link.get('authority_version')}|{link.get('provenance_version')}|{link.get('link_id')}|{link.get('parent_digest')}|{link.get('child_digest')}".encode("utf-8")).hexdigest()


def _link_root(links: list[dict[str, Any]]) -> str:
...
def validate_links(report: Any, label: str = "links") -> list[str]:
    """Return dual-version chain, link root, count, and no-mutation errors."""
    ...
    for index, link in enumerate(links):
        ...
        if link.get("parent_digest") != current:
            errors.append(f"{prefix}.parent_digest must match dual-version predecessor")
        ...
        if not _digest(link_digest):
            ...
            errors.append(f"{prefix}.link_digest must bind dual versions")
    ...
    if _digest(report.get("final_digest")) and current != report.get("final_digest"):
        errors.append(f"{label}.final_digest must match dual-version chain")
    link_root = report.get("link_root")
    if not _digest(link_root):
        errors.append(f"{label}.link_root must be lowercase SHA-256")
    elif link_root != _link_root(links):
        errors.append(f"{label}.link_root must match ordered dual-version links")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match dual-version links")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_DUAL_VERSION_AUTHORITY_PROVENANCE_LINK_V45_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_DUAL_VERSION_AUTHORITY_PROVENANCE_LINK_V45_VALID")
    return 0
