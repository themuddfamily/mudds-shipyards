#!/usr/bin/env python3
"""Validate detached v240 network snapshot authority takeover covenant evidence."""
...
SCHEMA_VERSION = 240
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_covenant_v240"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
COVENANT_ID = "server-authority-covenant-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v11"
SNAPSHOT_ID = "snapshot-authority-v123"
...
def _covenant_digest(record: dict[str, Any]) -> str:
    fields = (
        "covenant_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('covenant_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover covenant evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("covenant_id", report.get("covenant_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority covenant")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority covenant")
        ...
        covenant_digest = record.get("covenant_digest")
        if not _digest(covenant_digest):
            errors.append(f"{prefix}.covenant_digest must be lowercase SHA-256")
        elif covenant_digest != _covenant_digest(record):
            errors.append(f"{prefix}.covenant_digest must bind authority covenant")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority covenant records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority covenant records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_COVENANT_V240_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_COVENANT_V240_VALID")
    return 0
