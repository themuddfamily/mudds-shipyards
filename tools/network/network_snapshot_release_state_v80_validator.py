#!/usr/bin/env python3
"""Validate detached v80 snapshot release/state evidence."""
...
SCHEMA_VERSION = 80
EVIDENCE_SCOPE = "network_snapshot_release_state_v80"
...
def _release_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "release", "state", "source", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for release/state snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "release": RELEASE_ID, "state": STATE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("release", report.get("release")), ("state", report.get("state")), ("source", report.get("source")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match release/state")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("release", report.get("release")), ("state", report.get("state")), ("source", report.get("source")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match release/state")
        ...
        if not _digest(release_digest):
            ...
            errors.append(f"{prefix}.release_digest must bind release/state")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_RELEASE_STATE_V80_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_RELEASE_STATE_V80_VALID")
    return 0
