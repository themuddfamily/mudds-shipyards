#!/usr/bin/env python3
"""Validate detached v321 authority/snapshot window evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v321"
SNAPSHOT_ID = "snapshot-authority-v204"
WINDOW_ID = "authority-snapshot-window-v1"
...
def _window_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('window_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot window evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority window")
    ...
    items = report.get("window_members")
    if not isinstance(items, list):
        errors.append(f"{label}.window_members must be an array")
        items = []
    ids: set[str] = set()
    observed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.window_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority window")
        ...
        window_digest = item.get("window_digest")
        if not _digest(window_digest):
            errors.append(f"{prefix}.window_digest must be lowercase SHA-256")
        elif window_digest != _window_digest(item):
            errors.append(f"{prefix}.window_digest must bind authority window")
        if item.get("observed") is not True:
            errors.append(f"{prefix}.observed must be true")
        else:
            observed += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority window members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "window_members": len(items), "unique": len(ids), "observed": observed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority window members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WINDOW_V321_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WINDOW_V321_VALID")
    return 0
