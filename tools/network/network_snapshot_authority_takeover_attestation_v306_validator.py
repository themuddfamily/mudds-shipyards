#!/usr/bin/env python3
"""Validate detached v306 authority/snapshot attestation evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v306"
SNAPSHOT_ID = "snapshot-authority-v189"
ATTESTATION_ID = "authority-snapshot-attestation-v1"
...
def _attestation_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('attestation_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot attestation evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority attestation")
    ...
    items = report.get("attestations")
    if not isinstance(items, list):
        errors.append(f"{label}.attestations must be an array")
        items = []
    ids: set[str] = set()
    attested = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.attestations[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority attestation")
        ...
        attestation_digest = item.get("attestation_digest")
        if not _digest(attestation_digest):
            errors.append(f"{prefix}.attestation_digest must be lowercase SHA-256")
        elif attestation_digest != _attestation_digest(item):
            errors.append(f"{prefix}.attestation_digest must bind authority attestation")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority attestations")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "attestations": len(items), "unique": len(ids), "attested": attested, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority attestations")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ATTESTATION_V306_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ATTESTATION_V306_VALID")
    return 0
