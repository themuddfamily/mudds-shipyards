...
try:
    from .network_snapshot_authority_takeover_permit_v237_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_permit_v237_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["permit_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['permit_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "permit_id": "server-authority-permit-v1",
            "authority_takeover_token": "server-authority-takeover-token-v8",
            "snapshot_id": "snapshot-authority-v120", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 9,
            "sequence": 10132, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverPermitV237ValidatorTest(unittest.TestCase):
    def test_accepts_authority_permit(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_permit_binding(self):
        report = _report()
        report["records"][0]["permit_id"] = "server-authority-permit-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority permit records" in error for error in errors))
