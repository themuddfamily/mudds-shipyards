...
try:
    from .network_snapshot_authority_registration_v202_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_registration_v202_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["registration_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['registration_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_registration": "server-authority-registration-v1",
            "snapshot_id": "snapshot-authority-v85", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3013, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityRegistrationV202ValidatorTest(unittest.TestCase):
    def test_accepts_registered_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_registration_binding(self):
        report = _report()
        report["records"][0]["authority_registration"] = "server-authority-registration-old"
        self.assertTrue(any("bind authority registration" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v84"
        self.assertTrue(any("bind authority registration" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority registration records" in error for error in validate_snapshot(report)))
