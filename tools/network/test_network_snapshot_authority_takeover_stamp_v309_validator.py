...
try:
    from .network_snapshot_authority_takeover_stamp_v309_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_stamp_v309_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "stamp_id": "authority-snapshot-stamp-v1", "takeover_id": "authority-takeover-v309",
        "snapshot_id": "snapshot-authority-v192", "authority": "server", "sequence": 42234,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "stamped": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "stamp_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["stamp_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['stamp_digest']}" for item in items
    )
    return {
        "schema_version": 309,
        "evidence_scope": "network_snapshot_authority_takeover_stamp_v309",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v309",
        "snapshot_id": "snapshot-authority-v192", "stamp_id": "authority-snapshot-stamp-v1",
        "source": "server_snapshot", "snapshot_version": 81, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v309", "snapshot_id": "snapshot-authority-v192",
            "stamp_id": "authority-snapshot-stamp-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 81,
            "sequence": 42234, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "stamps": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"stamps": 2, "unique": 2, "stamped": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverStampV309ValidatorTest(unittest.TestCase):
    def test_accepts_stamp_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_stamp_binding(self):
        report = _report()
        report["stamps"][0]["stamp_id"] = "wrong-stamp"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority stamp" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["stamps"][1]["order"] = 1
        report["stamps"][1]["item_id"] = report["stamps"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["stamps"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["stamped"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.stamped" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["stamps"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority stamps" in error for error in errors))
