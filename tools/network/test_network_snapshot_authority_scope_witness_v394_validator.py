...
try:
    from .network_snapshot_authority_scope_witness_v394_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_scope_witness_v394_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-scope-witness-assertion-v1",
        "integrity_gate_id": "authority-scope-witness-v394",
        "snapshot_id": "snapshot-authority-v277", "authority": "server", "sequence": 44294,
...
def _report():
    ...
    return {
        "schema_version": 394,
        "evidence_scope": "network_snapshot_authority_scope_witness_v394",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-scope-witness-v394",
        "snapshot_id": "snapshot-authority-v277",
        "assertion_id": "authority-snapshot-scope-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 166, "release": "release-1",
...
class NetworkSnapshotAuthorityScopeWitnessV394ValidatorTest(unittest.TestCase):
    def test_accepts_scope_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_scope_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-scope"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority scope witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["scope"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority scope members" in error for error in errors))
