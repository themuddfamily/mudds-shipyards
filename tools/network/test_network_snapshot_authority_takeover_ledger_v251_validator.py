...
try:
    from .network_snapshot_authority_takeover_ledger_v251_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_ledger_v251_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["ledger_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['ledger_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "ledger_id": "server-authority-ledger-v1",
            "authority_takeover_token": "server-authority-takeover-token-v22",
            "snapshot_id": "snapshot-authority-v134", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 23,
            "sequence": 24146, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverLedgerV251ValidatorTest(unittest.TestCase):
    def test_accepts_authority_ledger(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_ledger_binding(self):
        report = _report()
        report["records"][0]["ledger_id"] = "server-authority-ledger-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority ledger records" in error for error in errors))
