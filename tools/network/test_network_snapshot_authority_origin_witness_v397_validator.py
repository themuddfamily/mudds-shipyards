...
try:
    from .network_snapshot_authority_origin_witness_v397_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_origin_witness_v397_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-origin-witness-assertion-v1",
        "integrity_gate_id": "authority-origin-witness-v397",
        "snapshot_id": "snapshot-authority-v280", "authority": "server", "sequence": 44597,
...
def _report():
    ...
    return {
        "schema_version": 397,
        "evidence_scope": "network_snapshot_authority_origin_witness_v397",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-origin-witness-v397",
        "snapshot_id": "snapshot-authority-v280",
        "assertion_id": "authority-snapshot-origin-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 169, "release": "release-1",
...
class NetworkSnapshotAuthorityOriginWitnessV397ValidatorTest(unittest.TestCase):
    def test_accepts_origin_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_origin_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-origin"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority origin witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["origin"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority origin members" in error for error in errors))
