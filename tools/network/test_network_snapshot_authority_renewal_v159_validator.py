...
try:
    from .network_snapshot_authority_renewal_v159_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_renewal_v159_validator import validate_snapshot
...
class NetworkSnapshotAuthorityRenewalV159ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_renewal_binding(self):
        report = _report()
        report["records"][0]["authority_renewal"] = "server-authority-renewal-old"
        self.assertTrue(any("bind authority renewal" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v41"
        self.assertTrue(any("bind authority renewal" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority renewal records" in error for error in validate_snapshot(report)))
