...
try:
    from .network_snapshot_authority_takeover_ticket_v284_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_ticket_v284_validator import validate_snapshot


def _item(order, item_id, digest, action):
    item = {
        "order": order, "item_id": item_id,
        "ticket_id": "server-authority-ticket-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v55",
        "snapshot_id": "snapshot-authority-v167", "sequence": 42179,
        "action": action, "expected_digest": digest, "observed_digest": digest,
        "issued": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "ticket_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "action", "item_id", "expected_digest", "observed_digest",
    )
    item["ticket_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['ticket_digest']}" for item in items
    )
    return {
        "schema_version": 284,
        "evidence_scope": "network_snapshot_authority_takeover_ticket_v284",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "ticket_id": "server-authority-ticket-v1",
        "takeover_token": "server-authority-takeover-token-v55",
        "snapshot_id": "snapshot-authority-v167", "source": "server_snapshot",
        "snapshot_version": 56, "release": "release-1",
        ...
        "snapshot": {
            "ticket_id": "server-authority-ticket-v1",
            "takeover_token": "server-authority-takeover-token-v55",
            "snapshot_id": "snapshot-authority-v167", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 56,
            "sequence": 42179, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "ticket_items": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"items": 2, "unique": 2, "issued": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverTicketV284ValidatorTest(unittest.TestCase):
    def test_accepts_ticket_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_ticket_binding(self):
        report = _report()
        report["ticket_items"][0]["takeover_token"] = "wrong-token"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority ticket" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["ticket_items"][1]["order"] = 1
        report["ticket_items"][1]["item_id"] = report["ticket_items"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_rollup_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority ticket items" in error for error in errors))
        self.assertTrue(any("counts.items" in error for error in errors))

    def test_rejects_mutation(self):
        report = _report()
        report["ticket_items"][0]["mutation_fields"] = ["authority"]
