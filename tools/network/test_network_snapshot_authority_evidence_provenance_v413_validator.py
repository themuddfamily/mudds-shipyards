...
try:
    from .network_snapshot_authority_evidence_provenance_v413_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_evidence_provenance_v413_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-evidence-provenance-assertion-v1",
        "integrity_gate_id": "authority-evidence-provenance-v413",
        "snapshot_id": "snapshot-authority-v296", "authority": "server", "sequence": 46413,
...
def _report():
    items = [_item(1, "authority", hashlib.sha256(b"authority").hexdigest(), "authority"),
             _item(2, "provenance", hashlib.sha256(b"provenance").hexdigest(), "provenance")]
    material = "\n".join(f"{item['order']}|{item['item_id']}|{item['assertion_digest']}" for item in items)
    return {
        "schema_version": 413,
        "evidence_scope": "network_snapshot_authority_evidence_provenance_v413",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-evidence-provenance-v413",
        "snapshot_id": "snapshot-authority-v296",
        "assertion_id": "authority-snapshot-evidence-provenance-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 185, "release": "release-1",
        ...
        "snapshot": {
            "integrity_gate_id": "authority-evidence-provenance-v413", "snapshot_id": "snapshot-authority-v296",
            "assertion_id": "authority-snapshot-evidence-provenance-assertion-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 185, "sequence": 46413,
...
class NetworkSnapshotAuthorityEvidenceProvenanceV413ValidatorTest(unittest.TestCase):
    def test_accepts_evidence_provenance_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_evidence_provenance_binding(self):
        report = _report(); report["assertion_members"][0]["snapshot_id"] = "wrong-provenance"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority evidence provenance" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["provenance"]; report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
