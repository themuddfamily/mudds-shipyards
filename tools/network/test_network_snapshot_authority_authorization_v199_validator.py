...
try:
    from .network_snapshot_authority_authorization_v199_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_authorization_v199_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "authority_authorization": "server-authority-authorization-v1",
            "snapshot_id": "snapshot-authority-v82", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3001, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityAuthorizationV199ValidatorTest(unittest.TestCase):
    def test_accepts_authorized_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_authorization_binding(self):
        report = _report()
        report["records"][0]["authority_authorization"] = "server-authority-authorization-old"
        self.assertTrue(any("bind authority authorization" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v81"
        self.assertTrue(any("bind authority authorization" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority authorization records" in error for error in validate_snapshot(report)))
