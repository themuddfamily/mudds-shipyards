#!/usr/bin/env python3
"""Validate detached v127 network snapshot authority-certificate evidence."""
...
SCHEMA_VERSION = 127
EVIDENCE_SCOPE = "network_snapshot_authority_certificate_v127"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_CERTIFICATE = "server-authority-certificate-v2"
SNAPSHOT_ID = "snapshot-authority-v10"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_certificate", "snapshot_id", "authority", "source", "release", "version",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('binding_digest')}" for r in records)
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-certificate snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_certificate": AUTHORITY_CERTIFICATE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority_certificate", report.get("authority_certificate")), ("snapshot_id", report.get("snapshot_id")), ("authority", AUTHORITY), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CERTIFICATE_V127_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CERTIFICATE_V127_VALID")
    return 0
