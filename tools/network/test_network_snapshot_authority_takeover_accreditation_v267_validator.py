...
try:
    from .network_snapshot_authority_takeover_accreditation_v267_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_accreditation_v267_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["accreditation_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['accreditation_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "accreditation_id": "server-authority-accreditation-v1",
            "authority_takeover_token": "server-authority-takeover-token-v38",
            "snapshot_id": "snapshot-authority-v150", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 39,
            "sequence": 40162, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverAccreditationV267ValidatorTest(unittest.TestCase):
    def test_accepts_authority_accreditation(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_accreditation_binding(self):
        report = _report()
        report["records"][0]["accreditation_id"] = "server-authority-accreditation-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority accreditation" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority accreditation records" in error for error in errors))
