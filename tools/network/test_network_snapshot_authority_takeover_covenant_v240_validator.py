...
try:
    from .network_snapshot_authority_takeover_covenant_v240_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_covenant_v240_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["covenant_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['covenant_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "covenant_id": "server-authority-covenant-v1",
            "authority_takeover_token": "server-authority-takeover-token-v11",
            "snapshot_id": "snapshot-authority-v123", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 12,
            "sequence": 13135, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverCovenantV240ValidatorTest(unittest.TestCase):
    def test_accepts_authority_covenant(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_covenant_binding(self):
        report = _report()
        report["records"][0]["covenant_id"] = "server-authority-covenant-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority covenant" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority covenant records" in error for error in errors))
