#!/usr/bin/env python3
"""Validate detached v290 network snapshot authority-takeover root evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v173"
...
def _root_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('root_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover root evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority root")
    ...
    items = report.get("roots")
    if not isinstance(items, list):
        errors.append(f"{label}.roots must be an array")
        items = []
    ids: set[str] = set()
    anchored = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.roots[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority root")
        ...
        if not _digest(root_digest):
            errors.append(f"{prefix}.root_digest must be lowercase SHA-256")
        elif root_digest != _root_digest(item):
            errors.append(f"{prefix}.root_digest must bind authority root")
        if item.get("anchored") is not True:
            errors.append(f"{prefix}.anchored must be true")
        else:
            anchored += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority roots")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "roots": len(items), "unique": len(ids), "anchored": anchored, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority roots")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ROOT_V290_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ROOT_V290_VALID")
    return 0
