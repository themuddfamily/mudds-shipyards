#!/usr/bin/env python3
"""Validate detached v291 network snapshot authority-takeover lineage evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v174"
...
def _lineage_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('lineage_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover lineage evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("lineage_id", report.get("lineage_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority lineage")
    ...
    items = report.get("lineage_entries")
    if not isinstance(items, list):
        errors.append(f"{label}.lineage_entries must be an array")
        items = []
    ids: set[str] = set()
    linked = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.lineage_entries[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority lineage")
        if item.get("sequence") != snapshot.get("sequence"):
            errors.append(f"{prefix}.sequence must match snapshot")
        if not isinstance(item.get("ancestor"), str) or not item["ancestor"]:
            errors.append(f"{prefix}.ancestor must be non-empty")
        ...
        if not _digest(lineage_digest):
            errors.append(f"{prefix}.lineage_digest must be lowercase SHA-256")
        elif lineage_digest != _lineage_digest(item):
            errors.append(f"{prefix}.lineage_digest must bind authority lineage")
        if item.get("linked") is not True:
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority lineage entries")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "entries": len(items), "unique": len(ids), "linked": linked, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority lineage entries")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LINEAGE_V291_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LINEAGE_V291_VALID")
    return 0
