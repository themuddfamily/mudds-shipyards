...
try:
    from .network_snapshot_authority_takeover_capsule_v331_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_capsule_v331_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "capsule_id": "authority-snapshot-capsule-v1", "takeover_id": "authority-takeover-v331",
        "snapshot_id": "snapshot-authority-v214", "authority": "server", "sequence": 42366,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "enclosed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "capsule_id", "takeover_id", "snapshot_id", "sequence", "subject",
...
def _report():
    ...
    return {
        "schema_version": 331,
        "evidence_scope": "network_snapshot_authority_takeover_capsule_v331",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v331",
        "snapshot_id": "snapshot-authority-v214", "capsule_id": "authority-snapshot-capsule-v1",
        "source": "server_snapshot", "snapshot_version": 103, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v331", "snapshot_id": "snapshot-authority-v214",
            "capsule_id": "authority-snapshot-capsule-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 103,
            "sequence": 42366, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "capsule_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"capsule_members": 2, "unique": 2, "enclosed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverCapsuleV331ValidatorTest(unittest.TestCase):
    def test_accepts_capsule_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_capsule_binding(self):
        report = _report()
        report["capsule_members"][0]["capsule_id"] = "wrong-capsule"
    ...
    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["capsule_members"][1]["order"] = 1
        report["capsule_members"][1]["item_id"] = report["capsule_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["capsule_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["capsule_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority capsule members" in error for error in errors))
