#!/usr/bin/env python3
"""Validate detached v148 network snapshot authority-receipt evidence."""
...
SCHEMA_VERSION = 148
EVIDENCE_SCOPE = "network_snapshot_authority_receipt_v148"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_RECEIPT = "server-authority-receipt-v1"
SNAPSHOT_ID = "snapshot-authority-v31"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_receipt", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority receipt")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_RECEIPT_V148_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_RECEIPT_V148_VALID")
    return 0
