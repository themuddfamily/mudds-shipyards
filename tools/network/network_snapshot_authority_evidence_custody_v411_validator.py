#!/usr/bin/env python3
"""Validate detached v411 authority/snapshot evidence-custody evidence."""
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot evidence-custody evidence."""
    errors: list[str] = []
    ...
    for key, value in (
        ...
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority evidence custody")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (("assertion_id", report.get("assertion_id")),
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority evidence custody")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority evidence custody")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_CUSTODY_V411_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_CUSTODY_V411_VALID")
    return 0
