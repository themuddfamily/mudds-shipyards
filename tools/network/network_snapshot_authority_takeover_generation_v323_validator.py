#!/usr/bin/env python3
"""Validate detached v323 authority/snapshot generation evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v323"
SNAPSHOT_ID = "snapshot-authority-v206"
GENERATION_ID = "authority-snapshot-generation-v1"
...
def _generation_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('generation_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot generation evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority generation")
    ...
    items = report.get("generation_members")
    if not isinstance(items, list):
        errors.append(f"{label}.generation_members must be an array")
        items = []
    ids: set[str] = set()
    current = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.generation_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority generation")
        ...
        generation_digest = item.get("generation_digest")
        if not _digest(generation_digest):
            errors.append(f"{prefix}.generation_digest must be lowercase SHA-256")
        elif generation_digest != _generation_digest(item):
            errors.append(f"{prefix}.generation_digest must bind authority generation")
        if item.get("current") is not True:
            errors.append(f"{prefix}.current must be true")
        else:
            current += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority generation members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "generation_members": len(items), "unique": len(ids), "current": current, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority generation members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_GENERATION_V323_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_GENERATION_V323_VALID")
    return 0
