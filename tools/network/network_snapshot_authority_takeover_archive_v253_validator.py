#!/usr/bin/env python3
"""Validate detached v253 network snapshot authority takeover archive evidence."""
...
SCHEMA_VERSION = 253
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_archive_v253"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
ARCHIVE_ID = "server-authority-archive-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v24"
SNAPSHOT_ID = "snapshot-authority-v136"
...
def _archive_digest(record: dict[str, Any]) -> str:
    fields = (
        "archive_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('archive_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover archive evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority archive")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority archive")
        ...
        if not _digest(archive_digest):
            errors.append(f"{prefix}.archive_digest must be lowercase SHA-256")
        elif archive_digest != _archive_digest(record):
            errors.append(f"{prefix}.archive_digest must bind authority archive")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority archive records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority archive records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ARCHIVE_V253_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ARCHIVE_V253_VALID")
    return 0
