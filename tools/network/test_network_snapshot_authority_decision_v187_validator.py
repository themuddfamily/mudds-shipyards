...
try:
    from .network_snapshot_authority_decision_v187_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_decision_v187_validator import validate_snapshot
...
class NetworkSnapshotAuthorityDecisionV187ValidatorTest(unittest.TestCase):
    def test_accepts_authority_decision_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_decision_binding(self):
        report = _report()
        report["records"][0]["authority_decision"] = "server-authority-decision-old"
        self.assertTrue(any("bind authority decision" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v69"
        self.assertTrue(any("bind authority decision" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority decision records" in error for error in validate_snapshot(report)))
