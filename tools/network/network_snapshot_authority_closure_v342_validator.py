#!/usr/bin/env python3
"""Validate detached v342 authority/snapshot closure evidence."""
...
AUTHORITY = "server"
CLOSURE_ID = "authority-closure-v342"
SNAPSHOT_ID = "snapshot-authority-v225"
ASSERTION_ID = "authority-snapshot-closure-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 114
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot closure evidence."""
    errors: list[str] = []
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority closure")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority closure")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority closure members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority closure members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CLOSURE_V342_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CLOSURE_V342_VALID")
    return 0
