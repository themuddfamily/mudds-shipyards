#!/usr/bin/env python3
"""Validate detached v212 network snapshot authority-takeover attestation evidence."""
...
SCHEMA_VERSION = 212
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_attestation_v212"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_ATTESTATION = "server-authority-takeover-attestation-v1"
SNAPSHOT_ID = "snapshot-authority-v95"
...
def _attestation_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_attestation", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('attestation_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover attestation evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_attestation": AUTHORITY_TAKEOVER_ATTESTATION,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_attestation", report.get("authority_takeover_attestation")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover attestation")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover attestation")
        ...
        if not _digest(attestation_digest):
            ...
            errors.append(f"{prefix}.attestation_digest must bind authority takeover attestation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover attestation records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover attestation records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ATTESTATION_V212_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ATTESTATION_V212_VALID")
    return 0
