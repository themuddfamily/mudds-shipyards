#!/usr/bin/env python3
"""Validate detached v444 authority/snapshot evidence-index evidence."""
from __future__ import annotations
import argparse,hashlib,json,re
from pathlib import Path
from typing import Any
SCHEMA_VERSION=444;EVIDENCE_SCOPE="network_snapshot_authority_evidence_index_v444";EVIDENCE_MODE="detached_contract_fixture";POLICY_VERSION="network_replication_interest_authority_v1";AUTHORITY="server";INTEGRITY_GATE_ID="authority-evidence-index-v444";SNAPSHOT_ID="snapshot-authority-v327";ASSERTION_ID="authority-snapshot-evidence-index-assertion-v1";SOURCE="server_snapshot";SNAPSHOT_VERSION=216;RELEASE_ID="release-1";SHA256=re.compile(r"^[0-9a-f]{64}$");NOT_RUN_CHECKS=("stale_check","native_run","hardware_run","human_review")
...
def validate_snapshot(r:Any,label:str="snapshot")->list[str]:
 ...
 for k,v in(("integrity_gate_id",r.get("integrity_gate_id")),("snapshot_id",r.get("snapshot_id")),("assertion_id",r.get("assertion_id")),("authority",AUTHORITY),("source",r.get("source")),("release",r.get("release")),("version",r.get("snapshot_version"))):
  if s.get(k)!=v:e.append(f"{label}.snapshot.{k} must match authority evidence index")
 ...
 for n,i in enumerate(a):
  ...
  for k,v in(("assertion_id",r.get("assertion_id")),("integrity_gate_id",r.get("integrity_gate_id")),("snapshot_id",r.get("snapshot_id")),("authority",AUTHORITY)):
   if i.get(k)!=v:e.append(f"{p}.{k} must bind authority evidence index")
  ...
  elif z!=_assertion_digest(i):e.append(f"{p}.assertion_digest must bind authority evidence index")
...
def main()->int:
 ...
 if e:print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_INDEX_V444_INVALID");[print(f"- {x}")for x in e];return 1
 print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_INDEX_V444_VALID");return 0
