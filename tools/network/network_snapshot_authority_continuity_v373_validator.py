#!/usr/bin/env python3
"""Validate detached v373 authority/snapshot continuity evidence."""
...
AUTHORITY = "server"
CONTINUITY_ID = "authority-continuity-v373"
SNAPSHOT_ID = "snapshot-authority-v256"
ASSERTION_ID = "authority-snapshot-continuity-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 145
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot continuity evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("continuity_id", report.get("continuity_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority continuity")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority continuity")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority continuity")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority continuity members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority continuity members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CONTINUITY_V373_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CONTINUITY_V373_VALID")
    return 0
