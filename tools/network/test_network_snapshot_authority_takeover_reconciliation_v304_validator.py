...
try:
    from .network_snapshot_authority_takeover_reconciliation_v304_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_reconciliation_v304_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "takeover_id": "authority-takeover-v304", "snapshot_id": "snapshot-authority-v187",
        "authority": "server", "sequence": 42204, "subject": subject,
        "authority_digest": digest, "snapshot_digest": digest,
        "mutation_fields": [], "state_changed": False,
    }
    fields = ("takeover_id", "snapshot_id", "sequence", "subject", "authority_digest", "snapshot_digest")
    item["reconciliation_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['reconciliation_digest']}" for item in items
    )
    return {
        "schema_version": 304,
        "evidence_scope": "network_snapshot_authority_takeover_reconciliation_v304",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v304",
        "snapshot_id": "snapshot-authority-v187",
        "reconciliation_id": "authority-snapshot-reconciliation-v1",
        "source": "server_snapshot", "snapshot_version": 76, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v304", "snapshot_id": "snapshot-authority-v187",
            "reconciliation_id": "authority-snapshot-reconciliation-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 76,
            "sequence": 42204, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "reconciliations": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"reconciliations": 2, "unique": 2, "matched": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverReconciliationV304ValidatorTest(unittest.TestCase):
    def test_accepts_reconciliation_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_reconciliation_binding(self):
        report = _report()
        report["reconciliations"][0]["takeover_id"] = "wrong-takeover"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority reconciliation" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["reconciliations"][1]["order"] = 1
        report["reconciliations"][1]["item_id"] = report["reconciliations"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["reconciliations"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["matched"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.matched" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["reconciliations"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority reconciliations" in error for error in errors))
