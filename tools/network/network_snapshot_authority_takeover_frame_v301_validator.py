#!/usr/bin/env python3
"""Validate detached v301 network snapshot authority-takeover frame evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v184"
...
def _frame_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('frame_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover frame evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority frame")
    ...
    items = report.get("frames")
    if not isinstance(items, list):
        errors.append(f"{label}.frames must be an array")
        items = []
    ids: set[str] = set()
    captured = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.frames[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority frame")
        ...
        frame_digest = item.get("frame_digest")
        if not _digest(frame_digest):
            errors.append(f"{prefix}.frame_digest must be lowercase SHA-256")
        elif frame_digest != _frame_digest(item):
            errors.append(f"{prefix}.frame_digest must bind authority frame")
        if item.get("captured") is not True:
            errors.append(f"{prefix}.captured must be true")
        else:
            captured += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority frames")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "frames": len(items), "unique": len(ids), "captured": captured, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority frames")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_FRAME_V301_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_FRAME_V301_VALID")
    return 0
