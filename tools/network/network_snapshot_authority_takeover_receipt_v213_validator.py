#!/usr/bin/env python3
"""Validate detached v213 network snapshot authority-takeover receipt evidence."""
...
SCHEMA_VERSION = 213
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_receipt_v213"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_RECEIPT = "server-authority-takeover-receipt-v1"
SNAPSHOT_ID = "snapshot-authority-v96"
...
def _receipt_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_receipt", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_receipt": AUTHORITY_TAKEOVER_RECEIPT,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_receipt", report.get("authority_takeover_receipt")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover receipt")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover receipt")
        ...
        if not _digest(receipt_digest):
            ...
            errors.append(f"{prefix}.receipt_digest must bind authority takeover receipt")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover receipt records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover receipt records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RECEIPT_V213_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RECEIPT_V213_VALID")
    return 0
