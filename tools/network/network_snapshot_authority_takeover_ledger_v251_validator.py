#!/usr/bin/env python3
"""Validate detached v251 network snapshot authority takeover ledger evidence."""
...
SCHEMA_VERSION = 251
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_ledger_v251"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
LEDGER_ID = "server-authority-ledger-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v22"
SNAPSHOT_ID = "snapshot-authority-v134"
...
def _ledger_digest(record: dict[str, Any]) -> str:
    fields = (
        "ledger_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('ledger_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for index, record in enumerate(records):
        ...
        elif ledger_digest != _ledger_digest(record):
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority ledger records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority ledger records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LEDGER_V251_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LEDGER_V251_VALID")
    return 0
