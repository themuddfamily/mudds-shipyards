#!/usr/bin/env python3
"""Validate detached v302 network snapshot authority-takeover segment evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v185"
...
def _segment_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('segment_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover segment evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority segment")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority segment")
        ...
        if not isinstance(item.get("zone"), str) or not item["zone"]:
            errors.append(f"{prefix}.zone must be non-empty")
        ...
        segment_digest = item.get("segment_digest")
        if not _digest(segment_digest):
            errors.append(f"{prefix}.segment_digest must be lowercase SHA-256")
        elif segment_digest != _segment_digest(item):
            errors.append(f"{prefix}.segment_digest must bind authority segment")
        if item.get("bounded") is not True:
            errors.append(f"{prefix}.bounded must be true")
        else:
            bounded += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority segments")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "segments": len(items), "unique": len(ids), "bounded": bounded, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority segments")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SEGMENT_V302_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SEGMENT_V302_VALID")
    return 0
