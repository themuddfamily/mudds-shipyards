#!/usr/bin/env python3
"""Validate detached v52 snapshot release/version linkage evidence."""
...
def _snapshot_linkage(snapshot: dict[str, Any]) -> str:
    material = "|".join(
        str(snapshot.get(key)) for key in ("release", "version", "sequence", "digest")
    )
    return hashlib.sha256(material.encode("utf-8")).hexdigest()


def _check_linkage(check: dict[str, Any]) -> str:
    material = "|".join(
        str(check.get(key))
        for key in ("release", "version", "check_id", "sequence", "expected_digest", "observed_digest")
    )
    return hashlib.sha256(material.encode("utf-8")).hexdigest()
...
def _aggregate(checks: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{check.get('order')}|{check.get('check_id')}|{check.get('linkage_digest')}"
        for check in checks
    )
    return hashlib.sha256(material.encode("utf-8")).hexdigest()
...
def validate_checks(report: Any, label: str = "checks") -> list[str]:
    """Return errors for release/version-linked reconciliation evidence."""
    ...
    linkage_digest = report.get("linkage_digest")
    if not _digest(linkage_digest):
        errors.append(f"{label}.linkage_digest must be lowercase SHA-256")
    elif linkage_digest != _snapshot_linkage(snapshot):
        errors.append(f"{label}.linkage_digest must bind release/version snapshot")
    ...
    for index, check in enumerate(checks):
        ...
        check_linkage = check.get("linkage_digest")
        if not _digest(check_linkage):
            errors.append(f"{prefix}.linkage_digest must be lowercase SHA-256")
        elif check_linkage != _check_linkage(check):
            errors.append(f"{prefix}.linkage_digest must bind release/version check")
        if check.get("linked") is not True:
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match linked checks")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match linked checks")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_RELEASE_VERSION_LINKAGE_V52_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_RELEASE_VERSION_LINKAGE_V52_VALID")
    return 0
