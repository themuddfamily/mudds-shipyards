#!/usr/bin/env python3
"""Validate detached v243 network snapshot authority takeover injunction evidence."""
...
SCHEMA_VERSION = 243
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_injunction_v243"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
INJUNCTION_ID = "server-authority-injunction-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v14"
SNAPSHOT_ID = "snapshot-authority-v126"
...
def _injunction_digest(record: dict[str, Any]) -> str:
    fields = (
        "injunction_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('injunction_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover injunction evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("injunction_id", report.get("injunction_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority injunction")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority injunction")
        ...
        injunction_digest = record.get("injunction_digest")
        if not _digest(injunction_digest):
            errors.append(f"{prefix}.injunction_digest must be lowercase SHA-256")
        elif injunction_digest != _injunction_digest(record):
            errors.append(f"{prefix}.injunction_digest must bind authority injunction")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority injunction records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority injunction records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INJUNCTION_V243_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INJUNCTION_V243_VALID")
    return 0
