#!/usr/bin/env python3
"""Validate detached v197 network snapshot authority-verification evidence."""
...
SCHEMA_VERSION = 197
EVIDENCE_SCOPE = "network_snapshot_authority_verification_v197"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_VERIFICATION = "server-authority-verification-v1"
SNAPSHOT_ID = "snapshot-authority-v80"
...
def _verification_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_verification", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('verification_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-verification snapshot evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority verification")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority verification")
        ...
        if not _digest(verification_digest):
            ...
            errors.append(f"{prefix}.verification_digest must bind authority verification")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority verification records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority verification records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_VERIFICATION_V197_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_VERIFICATION_V197_VALID")
    return 0
