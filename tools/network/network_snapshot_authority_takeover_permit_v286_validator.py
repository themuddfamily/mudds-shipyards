#!/usr/bin/env python3
"""Validate detached v286 network snapshot authority-takeover permit evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v169"
...
def _permit_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('permit_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    items = report.get("permits")
    if not isinstance(items, list):
        errors.append(f"{label}.permits must be an array")
        items = []
    ids: set[str] = set()
    authorized = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.permits[{index}]"
        ...
        if not isinstance(item.get("resource"), str) or not item["resource"]:
            errors.append(f"{prefix}.resource must be non-empty")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority permits")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "permits": len(items), "unique": len(ids), "authorized": authorized, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority permits")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PERMIT_V286_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PERMIT_V286_VALID")
    return 0
