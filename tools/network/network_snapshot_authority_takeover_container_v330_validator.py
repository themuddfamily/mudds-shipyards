#!/usr/bin/env python3
"""Validate detached v330 authority/snapshot container evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v330"
SNAPSHOT_ID = "snapshot-authority-v213"
CONTAINER_ID = "authority-snapshot-container-v1"
...
def _container_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('container_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot container evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority container")
    ...
    items = report.get("container_members")
    if not isinstance(items, list):
        errors.append(f"{label}.container_members must be an array")
        items = []
    ids: set[str] = set()
    contained = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.container_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority container")
        ...
        container_digest = item.get("container_digest")
        if not _digest(container_digest):
            errors.append(f"{prefix}.container_digest must be lowercase SHA-256")
        elif container_digest != _container_digest(item):
            errors.append(f"{prefix}.container_digest must bind authority container")
        if item.get("contained") is not True:
            errors.append(f"{prefix}.contained must be true")
        else:
            contained += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority container members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "container_members": len(items), "unique": len(ids), "contained": contained, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority container members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CONTAINER_V330_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CONTAINER_V330_VALID")
    return 0
