#!/usr/bin/env python3
"""Validate detached v313 authority/snapshot index evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v313"
SNAPSHOT_ID = "snapshot-authority-v196"
INDEX_ID = "authority-snapshot-index-v1"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot index evidence."""
    errors: list[str] = []
    ...
    items = report.get("index_entries")
    if not isinstance(items, list):
        errors.append(f"{label}.index_entries must be an array")
        items = []
    ids: set[str] = set()
    indexed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.index_entries[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority index")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority index entries")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "index_entries": len(items), "unique": len(ids), "indexed": indexed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority index entries")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INDEX_V313_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INDEX_V313_VALID")
    return 0
