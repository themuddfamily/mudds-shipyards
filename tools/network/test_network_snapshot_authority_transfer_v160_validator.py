...
try:
    from .network_snapshot_authority_transfer_v160_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_transfer_v160_validator import validate_snapshot
...
class NetworkSnapshotAuthorityTransferV160ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_transfer_binding(self):
        report = _report()
        report["records"][0]["authority_transfer"] = "server-authority-transfer-old"
        self.assertTrue(any("bind authority transfer" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v42"
        self.assertTrue(any("bind authority transfer" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority transfer records" in error for error in validate_snapshot(report)))
