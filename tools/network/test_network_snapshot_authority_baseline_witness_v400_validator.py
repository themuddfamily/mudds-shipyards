...
try:
    from .network_snapshot_authority_baseline_witness_v400_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_baseline_witness_v400_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-baseline-witness-assertion-v1",
        "integrity_gate_id": "authority-baseline-witness-v400",
        "snapshot_id": "snapshot-authority-v283", "authority": "server", "sequence": 44900,
...
def _report():
    ...
    return {
        "schema_version": 400,
        "evidence_scope": "network_snapshot_authority_baseline_witness_v400",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-baseline-witness-v400",
        "snapshot_id": "snapshot-authority-v283",
        "assertion_id": "authority-snapshot-baseline-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 172, "release": "release-1",
...
class NetworkSnapshotAuthorityBaselineWitnessV400ValidatorTest(unittest.TestCase):
    def test_accepts_baseline_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_baseline_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-baseline"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority baseline witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["baseline"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority baseline members" in error for error in errors))
