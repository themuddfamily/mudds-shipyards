#!/usr/bin/env python3
"""Validate detached v89 snapshot source/lineage evidence."""
...
SCHEMA_VERSION = 89
EVIDENCE_SCOPE = "network_snapshot_source_lineage_v89"
...
def _source_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "source", "lineage", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('source_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()


def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for source/lineage snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "source": SOURCE, "lineage": LINEAGE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("lineage", report.get("lineage")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match source/lineage")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("lineage", report.get("lineage")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match source/lineage")
        ...
        if not _digest(source_digest):
            errors.append(f"{prefix}.source_digest must be lowercase SHA-256")
        elif source_digest != _source_digest(record):
            errors.append(f"{prefix}.source_digest must bind source/lineage")
        if record.get("sourced") is not True:
            errors.append(f"{prefix}.sourced must be true")
        else:
            sourced += 1
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match source records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "sourced": sourced, "mutations": mutations}
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match source records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_SOURCE_LINEAGE_V89_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_SOURCE_LINEAGE_V89_VALID")
    return 0
