#!/usr/bin/env python3
"""Validate detached v247 network snapshot authority takeover warrant evidence."""
...
SCHEMA_VERSION = 247
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_warrant_v247"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
WARRANT_ID = "server-authority-warrant-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v18"
SNAPSHOT_ID = "snapshot-authority-v130"
...
def _warrant_digest(record: dict[str, Any]) -> str:
    fields = (
        "warrant_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('warrant_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover warrant evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("warrant_id", report.get("warrant_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority warrant")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority warrant")
        ...
        warrant_digest = record.get("warrant_digest")
        if not _digest(warrant_digest):
            errors.append(f"{prefix}.warrant_digest must be lowercase SHA-256")
        elif warrant_digest != _warrant_digest(record):
            errors.append(f"{prefix}.warrant_digest must bind authority warrant")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority warrant records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority warrant records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WARRANT_V247_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WARRANT_V247_VALID")
    return 0
