#!/usr/bin/env python3
"""Validate detached v119 snapshot identity/authority evidence."""
...
SCHEMA_VERSION = 119
EVIDENCE_SCOPE = "network_snapshot_authority_v119"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
SNAPSHOT_ID = "snapshot-authority-v2"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "snapshot_id", "authority", "source", "release", "version", "check_id",
        "sequence", "expected_digest", "observed_digest",
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached snapshot identity/authority evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "snapshot_id": SNAPSHOT_ID, "source": SOURCE,
    ...
    for key, value in (
        ...
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match snapshot/authority")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (
            ...
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match snapshot/authority")
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind snapshot/authority")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match snapshot records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match snapshot records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_V119_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_V119_VALID")
    return 0
