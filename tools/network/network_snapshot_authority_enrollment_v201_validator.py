#!/usr/bin/env python3
"""Validate detached v201 network snapshot authority-enrollment evidence."""
...
SCHEMA_VERSION = 201
EVIDENCE_SCOPE = "network_snapshot_authority_enrollment_v201"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ENROLLMENT = "server-authority-enrollment-v1"
SNAPSHOT_ID = "snapshot-authority-v84"
...
def _enrollment_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_enrollment", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('enrollment_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-enrollment snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_enrollment", report.get("authority_enrollment")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority enrollment")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority enrollment")
        ...
        enrollment_digest = record.get("enrollment_digest")
        if not _digest(enrollment_digest):
            errors.append(f"{prefix}.enrollment_digest must be lowercase SHA-256")
        elif enrollment_digest != _enrollment_digest(record):
            errors.append(f"{prefix}.enrollment_digest must bind authority enrollment")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority enrollment records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority enrollment records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ENROLLMENT_V201_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ENROLLMENT_V201_VALID")
    return 0
