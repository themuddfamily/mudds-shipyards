...
try:
    from .network_snapshot_authority_takeover_status_v219_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_status_v219_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["status_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['status_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_status": "server-authority-takeover-status-v1",
            "snapshot_id": "snapshot-authority-v102", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3081, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverStatusV219ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_status_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_status_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_status"] = "server-authority-takeover-status-old"
        self.assertTrue(any("bind authority takeover status" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v101"
        self.assertTrue(any("bind authority takeover status" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover status records" in error for error in validate_snapshot(report)))
