#!/usr/bin/env python3
"""Validate detached v265 network snapshot authority takeover inspection evidence."""
...
SCHEMA_VERSION = 265
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_inspection_v265"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
INSPECTION_ID = "server-authority-inspection-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v36"
SNAPSHOT_ID = "snapshot-authority-v148"
...
def _inspection_digest(record: dict[str, Any]) -> str:
    fields = (
        "inspection_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('inspection_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover inspection evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority inspection")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority inspection")
        ...
        inspection_digest = record.get("inspection_digest")
        if not _digest(inspection_digest):
            errors.append(f"{prefix}.inspection_digest must be lowercase SHA-256")
        elif inspection_digest != _inspection_digest(record):
            errors.append(f"{prefix}.inspection_digest must bind authority inspection")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority inspection records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority inspection records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INSPECTION_V265_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INSPECTION_V265_VALID")
    return 0
