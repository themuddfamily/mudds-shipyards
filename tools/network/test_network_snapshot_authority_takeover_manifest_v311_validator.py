...
try:
    from .network_snapshot_authority_takeover_manifest_v311_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_manifest_v311_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "manifest_id": "authority-snapshot-manifest-v1", "takeover_id": "authority-takeover-v311",
        "snapshot_id": "snapshot-authority-v194", "authority": "server", "sequence": 42246,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "listed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "manifest_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["manifest_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['manifest_digest']}" for item in items
    )
    return {
        "schema_version": 311,
        "evidence_scope": "network_snapshot_authority_takeover_manifest_v311",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v311",
        "snapshot_id": "snapshot-authority-v194", "manifest_id": "authority-snapshot-manifest-v1",
        "source": "server_snapshot", "snapshot_version": 83, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v311", "snapshot_id": "snapshot-authority-v194",
            "manifest_id": "authority-snapshot-manifest-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 83,
            "sequence": 42246, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "manifest_entries": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"manifest_entries": 2, "unique": 2, "listed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverManifestV311ValidatorTest(unittest.TestCase):
    def test_accepts_manifest_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_manifest_binding(self):
        report = _report()
        report["manifest_entries"][0]["manifest_id"] = "wrong-manifest"
    ...
    def test_rejects_order_and_duplicate_identity(self):
        ...
        report["manifest_entries"][1]["item_id"] = report["manifest_entries"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["manifest_entries"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["listed"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.listed" in error for error in errors))
