...
try:
    from .network_snapshot_verification_closure_v76_validator import validate_snapshot
except ImportError:
    from network_snapshot_verification_closure_v76_validator import validate_snapshot
...
def _record(order, check_id, digest):
    record = {
        "order": order, "check_id": check_id, "authority": "server", "verification": "snapshot-verification-v1", "closure": "closed", "source": "server_snapshot", "release": "release-1", "version": 2, "sequence": 420,
...
def _report():
    ...
    return {
        "schema_version": 76, "evidence_scope": "network_snapshot_verification_closure_v76", "evidence_mode": "detached_contract_fixture", "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "verification": "snapshot-verification-v1", "closure": "closed", "source": "server_snapshot", "snapshot_version": 2, "release": "release-1",
        "native_claims": False, "uses_live_network": False, "snapshot_detached": True, "no_mutation_guarantee": True,
        "snapshot": {"authority": "server", "verification": "snapshot-verification-v1", "closure": "closed", "source": "server_snapshot", "release": "release-1", "version": 2, "sequence": 420, "digest": hashlib.sha256(b"snapshot").hexdigest()},
        "records": records,
...
class NetworkSnapshotVerificationClosureV76ValidatorTest(unittest.TestCase):
