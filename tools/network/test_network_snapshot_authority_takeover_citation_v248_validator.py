...
try:
    from .network_snapshot_authority_takeover_citation_v248_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_citation_v248_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["citation_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['citation_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "citation_id": "server-authority-citation-v1",
            "authority_takeover_token": "server-authority-takeover-token-v19",
            "snapshot_id": "snapshot-authority-v131", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 20,
            "sequence": 21143, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverCitationV248ValidatorTest(unittest.TestCase):
    def test_accepts_authority_citation(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_citation_binding(self):
        report = _report()
        report["records"][0]["citation_id"] = "server-authority-citation-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority citation" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority citation records" in error for error in errors))
