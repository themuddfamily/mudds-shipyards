#!/usr/bin/env python3
"""Validate detached v156 network snapshot authority-lease evidence."""
...
SCHEMA_VERSION = 156
EVIDENCE_SCOPE = "network_snapshot_authority_lease_v156"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_LEASE = "server-authority-lease-v1"
SNAPSHOT_ID = "snapshot-authority-v39"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_lease", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_lease", report.get("authority_lease")),
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority lease")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_LEASE_V156_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_LEASE_V156_VALID")
    return 0
