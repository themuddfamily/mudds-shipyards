#!/usr/bin/env python3
"""Validate detached v363 authority/snapshot evidence."""
...
AUTHORITY = "server"
EVIDENCE_ID = "authority-evidence-v363"
SNAPSHOT_ID = "snapshot-authority-v246"
ASSERTION_ID = "authority-snapshot-evidence-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 135
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority evidence")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority evidence")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority evidence")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_V363_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_V363_VALID")
    return 0
