#!/usr/bin/env python3
"""Validate detached v325 authority/snapshot watermark evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v325"
SNAPSHOT_ID = "snapshot-authority-v208"
WATERMARK_ID = "authority-snapshot-watermark-v1"
...
def _watermark_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('watermark_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot watermark evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority watermark")
    ...
    items = report.get("watermark_members")
    if not isinstance(items, list):
        errors.append(f"{label}.watermark_members must be an array")
        items = []
    ids: set[str] = set()
    advanced = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.watermark_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority watermark")
        ...
        watermark_digest = item.get("watermark_digest")
        if not _digest(watermark_digest):
            errors.append(f"{prefix}.watermark_digest must be lowercase SHA-256")
        elif watermark_digest != _watermark_digest(item):
            errors.append(f"{prefix}.watermark_digest must bind authority watermark")
        if item.get("advanced") is not True:
            errors.append(f"{prefix}.advanced must be true")
        else:
            advanced += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority watermark members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "watermark_members": len(items), "unique": len(ids), "advanced": advanced, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority watermark members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WATERMARK_V325_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WATERMARK_V325_VALID")
    return 0
