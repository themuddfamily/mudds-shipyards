#!/usr/bin/env python3
"""Validate v42 versioned linked authority/provenance evidence."""
...
def _provenance_digest(link: dict[str, Any]) -> str:
    ...
    payload = f"{AUTHORITY}|{PROVENANCE_VERSION}|{link.get('link_id')}|{provenance.get('event')}|{provenance.get('version')}|{provenance.get('sequence')}|{link.get('parent_digest')}|{link.get('child_digest')}"
...
def validate_version(report: Any, label: str = "version") -> list[str]:
    """Return versioned provenance, chain, root digest, count, and mutation errors."""
    ...
    for index, link in enumerate(links):
        ...
        if not _digest(provenance_digest):
            ...
            errors.append(f"{prefix}.provenance_digest must bind versioned provenance")
...
def validate_version_file(report_path: Path) -> list[str]:
    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return [f"unable to read {report_path}: {exc}"]
    return validate_version(report, str(report_path))
...
def main() -> int:
    ...
    errors = validate_version_file(args.version)
    if errors:
        print("NETWORK_SNAPSHOT_LINKED_AUTHORITY_PROVENANCE_VERSION_V42_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_LINKED_AUTHORITY_PROVENANCE_VERSION_V42_VALID")
    return 0
