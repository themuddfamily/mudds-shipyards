...
try:
    from .network_snapshot_authority_revocation_v158_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_revocation_v158_validator import validate_snapshot
...
class NetworkSnapshotAuthorityRevocationV158ValidatorTest(unittest.TestCase):
    def test_accepts_authority_revocation_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_revocation_binding(self):
        report = _report()
        report["records"][0]["authority_revocation"] = "server-authority-revocation-old"
        self.assertTrue(any("bind authority revocation" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v40"
        self.assertTrue(any("bind authority revocation" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority revocation records" in error for error in validate_snapshot(report)))
