#!/usr/bin/env python3
"""Validate detached v192 network snapshot authority-settlement evidence."""
...
SCHEMA_VERSION = 192
EVIDENCE_SCOPE = "network_snapshot_authority_settlement_v192"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_SETTLEMENT = "server-authority-settlement-v1"
SNAPSHOT_ID = "snapshot-authority-v75"
...
def _settlement_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_settlement", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('settlement_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-settlement snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_settlement", report.get("authority_settlement")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority settlement")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority settlement")
        ...
        settlement_digest = record.get("settlement_digest")
        if not _digest(settlement_digest):
            errors.append(f"{prefix}.settlement_digest must be lowercase SHA-256")
        elif settlement_digest != _settlement_digest(record):
            errors.append(f"{prefix}.settlement_digest must bind authority settlement")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority settlement records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority settlement records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_SETTLEMENT_V192_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_SETTLEMENT_V192_VALID")
    return 0
