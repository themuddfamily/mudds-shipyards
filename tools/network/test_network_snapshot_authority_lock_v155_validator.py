...
try:
    from .network_snapshot_authority_lock_v155_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_lock_v155_validator import validate_snapshot
...
class NetworkSnapshotAuthorityLockV155ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_lock_binding(self):
        report = _report()
        report["records"][0]["authority_lock"] = "server-authority-lock-old"
        self.assertTrue(any("bind authority lock" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v37"
        self.assertTrue(any("bind authority lock" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority lock records" in error for error in validate_snapshot(report)))
