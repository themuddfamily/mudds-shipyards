#!/usr/bin/env python3
"""Validate detached v237 network snapshot authority takeover permit evidence."""
...
SCHEMA_VERSION = 237
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_permit_v237"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
PERMIT_ID = "server-authority-permit-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v8"
SNAPSHOT_ID = "snapshot-authority-v120"
...
def _permit_digest(record: dict[str, Any]) -> str:
    fields = (
        "permit_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('permit_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover permit evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("permit_id", report.get("permit_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority permit")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority permit")
        ...
        permit_digest = record.get("permit_digest")
        if not _digest(permit_digest):
            errors.append(f"{prefix}.permit_digest must be lowercase SHA-256")
        elif permit_digest != _permit_digest(record):
            errors.append(f"{prefix}.permit_digest must bind authority permit")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority permit records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority permit records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PERMIT_V237_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PERMIT_V237_VALID")
    return 0
