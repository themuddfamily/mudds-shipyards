...
try:
    from .network_snapshot_authority_identity_witness_v393_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_identity_witness_v393_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-identity-witness-assertion-v1",
        "integrity_gate_id": "authority-identity-witness-v393",
        "snapshot_id": "snapshot-authority-v276", "authority": "server", "sequence": 44193,
...
def _report():
    ...
    return {
        "schema_version": 393,
        "evidence_scope": "network_snapshot_authority_identity_witness_v393",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-identity-witness-v393",
        "snapshot_id": "snapshot-authority-v276",
        "assertion_id": "authority-snapshot-identity-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 165, "release": "release-1",
...
class NetworkSnapshotAuthorityIdentityWitnessV393ValidatorTest(unittest.TestCase):
    def test_accepts_identity_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_identity_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["authority"] = "client"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority identity witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["identity"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority identity members" in error for error in errors))
