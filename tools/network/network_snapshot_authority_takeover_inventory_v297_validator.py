#!/usr/bin/env python3
"""Validate detached v297 network snapshot authority-takeover inventory evidence."""
...
SCHEMA_VERSION = 297
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_inventory_v297"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
INVENTORY_ID = "server-authority-inventory-v1"
TAKEOVER_TOKEN = "server-authority-takeover-token-v68"
SNAPSHOT_ID = "snapshot-authority-v180"
...
def _inventory_digest(item: dict[str, Any]) -> str:
    fields = (
        "inventory_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "asset", "item_id", "expected_digest", "observed_digest",
    )
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('inventory_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover inventory evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority inventory")
    ...
    items = report.get("inventory_items")
    if not isinstance(items, list):
        errors.append(f"{label}.inventory_items must be an array")
        items = []
    ids: set[str] = set()
    inventoried = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.inventory_items[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority inventory")
        if item.get("sequence") != snapshot.get("sequence"):
            errors.append(f"{prefix}.sequence must match snapshot")
        if not isinstance(item.get("asset"), str) or not item["asset"]:
            errors.append(f"{prefix}.asset must be non-empty")
        ...
        inventory_digest = item.get("inventory_digest")
        if not _digest(inventory_digest):
            errors.append(f"{prefix}.inventory_digest must be lowercase SHA-256")
        elif inventory_digest != _inventory_digest(item):
            errors.append(f"{prefix}.inventory_digest must bind authority inventory")
        if item.get("inventoried") is not True:
            errors.append(f"{prefix}.inventoried must be true")
        else:
            inventoried += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority inventory items")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "items": len(items), "unique": len(ids), "inventoried": inventoried, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority inventory items")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INVENTORY_V297_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_INVENTORY_V297_VALID")
    return 0
