#!/usr/bin/env python3
"""Validate detached v413 authority/snapshot evidence-provenance evidence."""
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot evidence-provenance evidence."""
    errors: list[str] = []
    ...
    for key, value in (("integrity_gate_id", report.get("integrity_gate_id")),
        ...
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority evidence provenance")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (("assertion_id", report.get("assertion_id")),
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority evidence provenance")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority evidence provenance")
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {"assertion_members": len(items), "unique": len(ids),
                           "asserted": asserted, "mutations": mutations}
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_PROVENANCE_V413_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_PROVENANCE_V413_VALID")
    return 0
