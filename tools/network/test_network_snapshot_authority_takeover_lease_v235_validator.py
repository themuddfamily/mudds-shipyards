...
try:
    from .network_snapshot_authority_takeover_lease_v235_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_lease_v235_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["lease_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['lease_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "lease_id": "server-authority-lease-v1",
            "authority_takeover_token": "server-authority-takeover-token-v6",
            "snapshot_id": "snapshot-authority-v118", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 7,
            "sequence": 8130, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverLeaseV235ValidatorTest(unittest.TestCase):
    def test_accepts_authority_lease(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_lease_binding(self):
        report = _report()
        report["records"][0]["lease_id"] = "server-authority-lease-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority lease" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority lease records" in error for error in errors))
