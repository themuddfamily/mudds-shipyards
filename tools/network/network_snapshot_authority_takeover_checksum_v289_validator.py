#!/usr/bin/env python3
"""Validate detached v289 network snapshot authority-takeover checksum evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v172"
...
def _checksum_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('checksum_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover checksum evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("checksum_id", report.get("checksum_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority checksum")
    ...
    items = report.get("checksums")
    if not isinstance(items, list):
        errors.append(f"{label}.checksums must be an array")
        items = []
    ids: set[str] = set()
    matched = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.checksums[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority checksum")
        ...
        if not isinstance(item.get("domain"), str) or not item["domain"]:
            errors.append(f"{prefix}.domain must be non-empty")
        ...
        checksum_digest = item.get("checksum_digest")
        if not _digest(checksum_digest):
            errors.append(f"{prefix}.checksum_digest must be lowercase SHA-256")
        elif checksum_digest != _checksum_digest(item):
            errors.append(f"{prefix}.checksum_digest must bind authority checksum")
        if item.get("matched") is not True:
            errors.append(f"{prefix}.matched must be true")
        else:
            matched += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority checksums")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "checksums": len(items), "unique": len(ids), "matched": matched, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority checksums")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHECKSUM_V289_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHECKSUM_V289_VALID")
    return 0
