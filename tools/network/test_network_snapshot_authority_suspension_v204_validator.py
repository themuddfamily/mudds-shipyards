...
try:
    from .network_snapshot_authority_suspension_v204_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_suspension_v204_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["suspension_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['suspension_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_suspension": "server-authority-suspension-v1",
            "snapshot_id": "snapshot-authority-v87", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3021, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthoritySuspensionV204ValidatorTest(unittest.TestCase):
    def test_accepts_suspended_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_suspension_binding(self):
        report = _report()
        report["records"][0]["authority_suspension"] = "server-authority-suspension-old"
        self.assertTrue(any("bind authority suspension" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v86"
        self.assertTrue(any("bind authority suspension" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority suspension records" in error for error in validate_snapshot(report)))
