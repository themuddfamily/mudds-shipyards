#!/usr/bin/env python3
"""Validate detached v277 network snapshot authority-takeover checkpoint evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v160"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover checkpoint evidence."""
    errors: list[str] = []
    ...
    items = report.get("checkpoints")
    if not isinstance(items, list):
        errors.append(f"{label}.checkpoints must be an array")
        items = []
    ids: set[str] = set()
    verified = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.checkpoints[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority checkpoint")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority checkpoints")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "checkpoints": len(items), "unique": len(ids), "verified": verified, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority checkpoints")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHECKPOINT_V277_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHECKPOINT_V277_VALID")
    return 0
