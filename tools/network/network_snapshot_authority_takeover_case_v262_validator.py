#!/usr/bin/env python3
"""Validate detached v262 network snapshot authority takeover case evidence."""
...
SCHEMA_VERSION = 262
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_case_v262"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
CASE_ID = "server-authority-case-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v33"
SNAPSHOT_ID = "snapshot-authority-v145"
...
def _case_digest(record: dict[str, Any]) -> str:
    fields = (
        "case_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('case_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover case evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority case")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority case")
        ...
        if not _digest(case_digest):
            errors.append(f"{prefix}.case_digest must be lowercase SHA-256")
        elif case_digest != _case_digest(record):
            errors.append(f"{prefix}.case_digest must bind authority case")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority case records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority case records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CASE_V262_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CASE_V262_VALID")
    return 0
