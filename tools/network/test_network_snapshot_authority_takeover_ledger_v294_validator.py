...
try:
    from .network_snapshot_authority_takeover_ledger_v294_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_ledger_v294_validator import validate_snapshot


def _item(order, item_id, digest, event):
    item = {
        "order": order, "item_id": item_id,
        "ledger_id": "server-authority-ledger-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v65",
        "snapshot_id": "snapshot-authority-v177", "sequence": 42189,
        "event": event, "expected_digest": digest, "observed_digest": digest,
        "logged": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
...
def _report():
    ...
    return {
        ...
        "authority": "server", "ledger_id": "server-authority-ledger-v1",
        "takeover_token": "server-authority-takeover-token-v65",
        "snapshot_id": "snapshot-authority-v177", "source": "server_snapshot",
        "snapshot_version": 66, "release": "release-1",
        ...
        "snapshot": {
            ...
            "takeover_token": "server-authority-takeover-token-v65",
            "snapshot_id": "snapshot-authority-v177", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 66,
            "sequence": 42189, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "ledger_entries": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"entries": 2, "unique": 2, "logged": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverLedgerV294ValidatorTest(unittest.TestCase):
    def test_accepts_ledger_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_ledger_binding(self):
        report = _report()
        report["ledger_entries"][0]["takeover_token"] = "wrong-token"
