#!/usr/bin/env python3
"""Validate detached v68 snapshot evidence/lineage evidence."""
...
SCHEMA_VERSION = 68
EVIDENCE_SCOPE = "network_snapshot_evidence_lineage_v68"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
EVIDENCE_ID = "snapshot-evidence-v1"
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('lineage_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(lineage_digest):
            ...
            errors.append(f"{prefix}.lineage_digest must bind evidence/lineage")
        if record.get("traced") is not True:
            errors.append(f"{prefix}.traced must be true")
        else:
            traced += 1
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match lineage records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "traced": traced, "mutations": mutations}
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match lineage records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_EVIDENCE_LINEAGE_V68_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_EVIDENCE_LINEAGE_V68_VALID")
    return 0
