...
try:
    from .network_snapshot_authority_attestation_v147_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_attestation_v147_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["binding_digest"] = hashlib.sha256(material.encode()).hexdigest()
    return record
...
class NetworkSnapshotAuthorityAttestationV147ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v29"
        self.assertTrue(any("bind authority attestation" in error for error in validate_snapshot(report)))
