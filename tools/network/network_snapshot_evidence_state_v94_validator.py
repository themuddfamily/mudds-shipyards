#!/usr/bin/env python3
"""Validate detached v94 snapshot evidence/state evidence."""
...
SCHEMA_VERSION = 94
EVIDENCE_SCOPE = "network_snapshot_evidence_state_v94"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
EVIDENCE_ID = "snapshot-evidence-v2"
STATE = "stable"
...
def _evidence_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "evidence_id", "state", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for evidence/state snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "evidence_id": EVIDENCE_ID, "state": STATE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("evidence_id", report.get("evidence_id")), ("state", report.get("state")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match evidence/state")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("evidence_id", report.get("evidence_id")), ("state", report.get("state")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match evidence/state")
        ...
        if not _digest(evidence_digest):
            ...
            errors.append(f"{prefix}.evidence_digest must bind evidence/state")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_EVIDENCE_STATE_V94_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_EVIDENCE_STATE_V94_VALID")
    return 0
