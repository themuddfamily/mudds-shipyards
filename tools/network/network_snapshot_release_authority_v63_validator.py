#!/usr/bin/env python3
"""Validate detached v63 snapshot release/authority evidence."""
...
SCHEMA_VERSION = 63
EVIDENCE_SCOPE = "network_snapshot_release_authority_v63"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TOKEN = "server-authority-v1"
...
def _authority_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "authority_token", "release", "source", "channel", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('authority_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()


def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for release/authority snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_token": AUTHORITY_TOKEN,
        "source": SOURCE, "channel": CHANNEL, "snapshot_version": SNAPSHOT_VERSION, "release": RELEASE_ID,
    }
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("authority_token", report.get("authority_token")), ("release", report.get("release")), ("source", report.get("source")), ("channel", report.get("channel")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match release/authority")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("authority_token", report.get("authority_token")), ("release", report.get("release")), ("source", report.get("source")), ("channel", report.get("channel")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match release/authority")
        ...
        elif authority_digest != _authority_digest(record):
            errors.append(f"{prefix}.authority_digest must bind release/authority")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_RELEASE_AUTHORITY_V63_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_RELEASE_AUTHORITY_V63_VALID")
    return 0
