...
try:
    from .network_snapshot_authority_proof_envelope_v381_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_proof_envelope_v381_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-proof-envelope-assertion-v1",
        "integrity_gate_id": "authority-proof-envelope-v381",
        "snapshot_id": "snapshot-authority-v264", "authority": "server", "sequence": 42981,
...
def _report():
    ...
    return {
        "schema_version": 381,
        "evidence_scope": "network_snapshot_authority_proof_envelope_v381",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-proof-envelope-v381",
        "snapshot_id": "snapshot-authority-v264",
        "assertion_id": "authority-snapshot-proof-envelope-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 153, "release": "release-1",
...
class NetworkSnapshotAuthorityProofEnvelopeV381ValidatorTest(unittest.TestCase):
    def test_accepts_proof_envelope_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_proof_envelope_binding(self):
        report = _report()
        report["assertion_members"][0]["assertion_id"] = "wrong-envelope"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority proof envelope" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["proof"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority proof members" in error for error in errors))
