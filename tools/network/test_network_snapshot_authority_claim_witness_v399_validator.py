...
try:
    from .network_snapshot_authority_claim_witness_v399_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_claim_witness_v399_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-claim-witness-assertion-v1",
        "integrity_gate_id": "authority-claim-witness-v399",
        "snapshot_id": "snapshot-authority-v282", "authority": "server", "sequence": 44799,
...
def _report():
    ...
    return {
        "schema_version": 399,
        "evidence_scope": "network_snapshot_authority_claim_witness_v399",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-claim-witness-v399",
        "snapshot_id": "snapshot-authority-v282",
        "assertion_id": "authority-snapshot-claim-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 171, "release": "release-1",
...
class NetworkSnapshotAuthorityClaimWitnessV399ValidatorTest(unittest.TestCase):
    def test_accepts_claim_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_claim_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-claim"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority claim witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["claim"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority claim members" in error for error in errors))
