...
try:
    from .network_snapshot_authority_resource_v178_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_resource_v178_validator import validate_snapshot
...
class NetworkSnapshotAuthorityResourceV178ValidatorTest(unittest.TestCase):
    def test_accepts_authority_resource_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_resource_binding(self):
        report = _report()
        report["records"][0]["authority_resource"] = "server-authority-resource-old"
        self.assertTrue(any("bind authority resource" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v60"
        self.assertTrue(any("bind authority resource" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority resource records" in error for error in validate_snapshot(report)))
