#!/usr/bin/env python3
"""Validate detached stale snapshot sequence/digest no-mutation evidence."""

from __future__ import annotations

import argparse
import json
import re
from pathlib import Path
from typing import Any


SCHEMA_VERSION = 1
EVIDENCE_SCOPE = "network_stale_snapshot_sequence_digest_no_mutation"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
SHA256 = re.compile(r"^[0-9a-f]{64}$")
REQUIRED = {"stale_sequence", "stale_digest", "sequence_digest_mismatch"}


def _positive_int(value: Any) -> bool:
    return isinstance(value, int) and not isinstance(value, bool) and value > 0


def validate_report(report: Any, label: str = "report") -> list[str]:
    """Return paired stale sequence/digest no-mutation errors."""

    errors: list[str] = []
    if not isinstance(report, dict):
        return [f"{label} must be an object"]
    for key, expected in (("schema_version", SCHEMA_VERSION), ("evidence_scope", EVIDENCE_SCOPE), ("evidence_mode", EVIDENCE_MODE), ("policy_version", POLICY_VERSION)):
        if report.get(key) != expected:
            errors.append(f"{label}.{key} must be {expected}")
    for key in ("native_claims", "uses_live_network"):
        if report.get(key) is not False:
            errors.append(f"{label}.{key} must be false")
    current = report.get("current")
    if not isinstance(current, dict):
        errors.append(f"{label}.current must be an object")
        current = {}
    if not _positive_int(current.get("sequence")) or not _positive_int(current.get("peer_generation")):
        errors.append(f"{label}.current sequence and peer_generation must be positive")
    if not isinstance(current.get("digest"), str) or not SHA256.fullmatch(current.get("digest", "")):
        errors.append(f"{label}.current.digest must be lowercase SHA-256")

    next_snapshot = report.get("accepted_next")
    if not isinstance(next_snapshot, dict) or next_snapshot.get("accepted") is not True or next_snapshot.get("status") != "snapshot_accepted":
        errors.append(f"{label}.accepted_next must be snapshot_accepted")
    else:
        if next_snapshot.get("sequence") != current.get("sequence", 0) + 1 or next_snapshot.get("peer_generation") != current.get("peer_generation"):
            errors.append(f"{label}.accepted_next must advance sequence once for current peer")
        if not isinstance(next_snapshot.get("digest"), str) or not SHA256.fullmatch(next_snapshot["digest"]):
            errors.append(f"{label}.accepted_next.digest must be lowercase SHA-256")
        if next_snapshot.get("digest") == current.get("digest"):
            errors.append(f"{label}.accepted_next.digest must change")
        if next_snapshot.get("server_committed") is not True or next_snapshot.get("snapshot_detached") is not True:
            errors.append(f"{label}.accepted_next must be server-committed and detached")

    attempts = report.get("rejections")
    seen: set[str] = set()
    if not isinstance(attempts, list):
        errors.append(f"{label}.rejections must be an array")
        attempts = []
    for index, attempt in enumerate(attempts):
        prefix = f"{label}.rejections[{index}]"
        if not isinstance(attempt, dict):
            errors.append(f"{prefix} must be an object")
            continue
        status = attempt.get("status")
        if status not in REQUIRED:
            errors.append(f"{prefix}.status is not a required stale fence")
        else:
            seen.add(status)
        if attempt.get("accepted") is not False or attempt.get("server_rejected") is not True:
            errors.append(f"{prefix} must be server-rejected")
        if attempt.get("state_changed") is not False or attempt.get("mutation_fields") != []:
            errors.append(f"{prefix} must have no mutation")
        if attempt.get("after_sequence") != current.get("sequence") or attempt.get("after_digest") != current.get("digest") or attempt.get("after_peer_generation") != current.get("peer_generation"):
            errors.append(f"{prefix} must preserve current sequence/digest/generation")
        if status == "stale_sequence" and attempt.get("attempted_sequence", current["sequence"]) >= current["sequence"]:
            errors.append(f"{prefix}.attempted_sequence must be older")
        if status == "stale_digest" and attempt.get("attempted_digest") == current.get("digest"):
            errors.append(f"{prefix}.attempted_digest must differ")
    for status in sorted(REQUIRED - seen):
        errors.append(f"{label}.rejections must include {status}")
    if report.get("snapshot_detached") is not True:
        errors.append(f"{label}.snapshot_detached must be true")
    return errors


def validate_report_file(report_path: Path) -> list[str]:
    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return [f"unable to read {report_path}: {exc}"]
    return validate_report(report, str(report_path))


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("report", type=Path)
    args = parser.parse_args()
    errors = validate_report_file(args.report)
    if errors:
        print("NETWORK_STALE_SNAPSHOT_SEQUENCE_DIGEST_NO_MUTATION_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_STALE_SNAPSHOT_SEQUENCE_DIGEST_NO_MUTATION_VALID")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
