#!/usr/bin/env python3
"""Validate detached v193 network snapshot authority-certification evidence."""
...
SCHEMA_VERSION = 193
EVIDENCE_SCOPE = "network_snapshot_authority_certification_v193"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_CERTIFICATION = "server-authority-certification-v1"
SNAPSHOT_ID = "snapshot-authority-v76"
...
def _certification_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_certification", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-certification snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_certification", report.get("authority_certification")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CERTIFICATION_V193_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CERTIFICATION_V193_VALID")
    return 0
