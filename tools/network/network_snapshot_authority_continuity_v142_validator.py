#!/usr/bin/env python3
"""Validate detached v142 network snapshot authority-continuity evidence."""
...
SCHEMA_VERSION = 142
EVIDENCE_SCOPE = "network_snapshot_authority_continuity_v142"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_CONTINUITY = "server-authority-continuity-v2"
SNAPSHOT_ID = "snapshot-authority-v25"
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    return hashlib.sha256("\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('binding_digest')}" for r in records).encode()).hexdigest()
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-continuity snapshot evidence."""
    errors: list[str] = []
    if not isinstance(report, dict):
        return [f"{label} must be an object"]
    expected = {"schema_version": SCHEMA_VERSION, "evidence_scope": EVIDENCE_SCOPE, "evidence_mode": EVIDENCE_MODE, "policy_version": POLICY_VERSION, "authority": AUTHORITY, "authority_continuity": AUTHORITY_CONTINUITY, "snapshot_id": SNAPSHOT_ID, "source": SOURCE, "snapshot_version": SNAPSHOT_VERSION, "release": RELEASE_ID}
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority_continuity", report.get("authority_continuity")), ("snapshot_id", report.get("snapshot_id")), ("authority", AUTHORITY), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority/snapshot")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority_continuity", report.get("authority_continuity")), ("snapshot_id", report.get("snapshot_id")), ("authority", AUTHORITY), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority/snapshot")
        if record.get("sequence") != snapshot.get("sequence"):
            errors.append(f"{prefix}.sequence must match snapshot")
        expected_digest, observed_digest = record.get("expected_digest"), record.get("observed_digest")
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority/snapshot")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "authorized": authorized, "mutations": mutations}
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CONTINUITY_V142_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CONTINUITY_V142_VALID")
    return 0
