#!/usr/bin/env python3
"""Validate detached v333 authority/snapshot packet evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v333"
SNAPSHOT_ID = "snapshot-authority-v216"
PACKET_ID = "authority-snapshot-packet-v1"
...
def _packet_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('packet_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot packet evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority packet")
    ...
    items = report.get("packet_members")
    if not isinstance(items, list):
        errors.append(f"{label}.packet_members must be an array")
        items = []
    ids: set[str] = set()
    transmitted = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.packet_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority packet")
        ...
        if not _digest(packet_digest):
            errors.append(f"{prefix}.packet_digest must be lowercase SHA-256")
        elif packet_digest != _packet_digest(item):
            errors.append(f"{prefix}.packet_digest must bind authority packet")
        if item.get("transmitted") is not True:
            errors.append(f"{prefix}.transmitted must be true")
        else:
            transmitted += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority packet members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "packet_members": len(items), "unique": len(ids), "transmitted": transmitted, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority packet members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PACKET_V333_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PACKET_V333_VALID")
    return 0
