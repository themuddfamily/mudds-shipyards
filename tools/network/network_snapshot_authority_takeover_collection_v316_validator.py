#!/usr/bin/env python3
"""Validate detached v316 authority/snapshot collection evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v316"
SNAPSHOT_ID = "snapshot-authority-v199"
COLLECTION_ID = "authority-snapshot-collection-v1"
...
def _collection_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('collection_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot collection evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority collection")
    ...
    items = report.get("collection_entries")
    if not isinstance(items, list):
        errors.append(f"{label}.collection_entries must be an array")
        items = []
    ids: set[str] = set()
    collected = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.collection_entries[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority collection")
        ...
        collection_digest = item.get("collection_digest")
        if not _digest(collection_digest):
            errors.append(f"{prefix}.collection_digest must be lowercase SHA-256")
        elif collection_digest != _collection_digest(item):
            errors.append(f"{prefix}.collection_digest must bind authority collection")
        if item.get("collected") is not True:
            errors.append(f"{prefix}.collected must be true")
        else:
            collected += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority collection entries")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "collection_entries": len(items), "unique": len(ids), "collected": collected, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority collection entries")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_COLLECTION_V316_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_COLLECTION_V316_VALID")
    return 0
