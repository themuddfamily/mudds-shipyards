#!/usr/bin/env python3
"""Validate detached v259 network snapshot authority takeover container evidence."""
...
SCHEMA_VERSION = 259
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_container_v259"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
CONTAINER_ID = "server-authority-container-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v30"
SNAPSHOT_ID = "snapshot-authority-v142"
...
def _container_digest(record: dict[str, Any]) -> str:
    fields = (
        "container_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('container_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover container evidence."""
    errors: list[str] = []
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority container")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority container records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority container records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CONTAINER_V259_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CONTAINER_V259_VALID")
    return 0
