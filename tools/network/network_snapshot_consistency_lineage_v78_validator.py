#!/usr/bin/env python3
"""Validate detached v78 snapshot consistency/lineage evidence."""
...
SCHEMA_VERSION = 78
EVIDENCE_SCOPE = "network_snapshot_consistency_lineage_v78"
...
def _consistency_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "consistency", "lineage", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for consistency/lineage snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "consistency": CONSISTENCY, "lineage": LINEAGE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("consistency", report.get("consistency")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match consistency/lineage")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("consistency", report.get("consistency")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match consistency/lineage")
        ...
        if not _digest(consistency_digest):
            ...
            errors.append(f"{prefix}.consistency_digest must bind consistency/lineage")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_CONSISTENCY_LINEAGE_V78_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_CONSISTENCY_LINEAGE_V78_VALID")
    return 0
