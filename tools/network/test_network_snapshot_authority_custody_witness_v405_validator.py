...
try:
    from .network_snapshot_authority_custody_witness_v405_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_custody_witness_v405_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-custody-witness-assertion-v1",
        "integrity_gate_id": "authority-custody-witness-v405",
        "snapshot_id": "snapshot-authority-v288", "authority": "server", "sequence": 45405,
...
def _report():
    ...
    return {
        "schema_version": 405,
        "evidence_scope": "network_snapshot_authority_custody_witness_v405",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-custody-witness-v405",
        "snapshot_id": "snapshot-authority-v288",
        "assertion_id": "authority-snapshot-custody-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 177, "release": "release-1",
...
class NetworkSnapshotAuthorityCustodyWitnessV405ValidatorTest(unittest.TestCase):
    def test_accepts_custody_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_custody_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["snapshot_id"] = "wrong-custody"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority custody witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["custody"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority custody members" in error for error in errors))
