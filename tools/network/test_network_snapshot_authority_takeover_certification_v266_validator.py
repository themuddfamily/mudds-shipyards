...
try:
    from .network_snapshot_authority_takeover_certification_v266_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_certification_v266_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "certification_id": "server-authority-certification-v1",
            "authority_takeover_token": "server-authority-takeover-token-v37",
            "snapshot_id": "snapshot-authority-v149", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 38,
            "sequence": 39161, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverCertificationV266ValidatorTest(unittest.TestCase):
    def test_accepts_authority_certification(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_certification_binding(self):
        report = _report()
        report["records"][0]["certification_id"] = "server-authority-certification-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority certification" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority certification records" in error for error in errors))
