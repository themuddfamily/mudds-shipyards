#!/usr/bin/env python3
"""Validate detached v149 network snapshot authority-acknowledgement evidence."""
...
SCHEMA_VERSION = 149
EVIDENCE_SCOPE = "network_snapshot_authority_acknowledgement_v149"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ACKNOWLEDGEMENT = "server-authority-acknowledgement-v1"
SNAPSHOT_ID = "snapshot-authority-v32"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_acknowledgement", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-acknowledgement snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_acknowledgement": AUTHORITY_ACKNOWLEDGEMENT,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_acknowledgement", report.get("authority_acknowledgement")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority acknowledgement")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority acknowledgement")
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority acknowledgement")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority acknowledgement records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority acknowledgement records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ACKNOWLEDGEMENT_V149_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ACKNOWLEDGEMENT_V149_VALID")
    return 0
