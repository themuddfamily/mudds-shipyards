...
try:
    from .network_snapshot_authority_takeover_checkpoint_v324_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_checkpoint_v324_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "checkpoint_id": "authority-snapshot-checkpoint-v1", "takeover_id": "authority-takeover-v324",
        "snapshot_id": "snapshot-authority-v207", "authority": "server", "sequence": 42324,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "confirmed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "checkpoint_id", "takeover_id", "snapshot_id", "sequence", "subject",
...
def _report():
    ...
    return {
        "schema_version": 324,
        "evidence_scope": "network_snapshot_authority_takeover_checkpoint_v324",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v324",
        "snapshot_id": "snapshot-authority-v207", "checkpoint_id": "authority-snapshot-checkpoint-v1",
        "source": "server_snapshot", "snapshot_version": 96, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v324", "snapshot_id": "snapshot-authority-v207",
            "checkpoint_id": "authority-snapshot-checkpoint-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 96,
            "sequence": 42324, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "checkpoint_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"checkpoint_members": 2, "unique": 2, "confirmed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverCheckpointV324ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_checkpoint_binding(self):
        report = _report()
        report["checkpoint_members"][0]["checkpoint_id"] = "wrong-checkpoint"
    ...
    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["checkpoint_members"][1]["order"] = 1
        report["checkpoint_members"][1]["item_id"] = report["checkpoint_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["checkpoint_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["confirmed"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.confirmed" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["checkpoint_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority checkpoint members" in error for error in errors))
