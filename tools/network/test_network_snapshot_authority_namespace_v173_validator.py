...
try:
    from .network_snapshot_authority_namespace_v173_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_namespace_v173_validator import validate_snapshot
...
class NetworkSnapshotAuthorityNamespaceV173ValidatorTest(unittest.TestCase):
    def test_accepts_authority_namespace_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_namespace_binding(self):
        report = _report()
        report["records"][0]["authority_namespace"] = "server-authority-namespace-old"
        self.assertTrue(any("bind authority namespace" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v55"
        self.assertTrue(any("bind authority namespace" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority namespace records" in error for error in validate_snapshot(report)))
