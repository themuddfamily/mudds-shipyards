#!/usr/bin/env python3
"""Validate detached v233 network snapshot authority takeover seal evidence."""
...
SCHEMA_VERSION = 233
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_seal_v233"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
SEAL_ID = "server-authority-seal-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v4"
SNAPSHOT_ID = "snapshot-authority-v116"
...
def _seal_digest(record: dict[str, Any]) -> str:
    fields = (
        "seal_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('seal_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority seal records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority seal records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SEAL_V233_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SEAL_V233_VALID")
    return 0
