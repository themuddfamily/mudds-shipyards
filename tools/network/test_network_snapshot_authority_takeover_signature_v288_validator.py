...
try:
    from .network_snapshot_authority_takeover_signature_v288_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_signature_v288_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "signature_id": "server-authority-signature-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v59",
        "snapshot_id": "snapshot-authority-v171", "sequence": 42183,
        "subject": subject, "expected_digest": digest, "observed_digest": digest,
        "signed": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "signature_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "subject", "item_id", "expected_digest", "observed_digest",
    )
    item["signature_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['signature_digest']}" for item in items
    )
    return {
        "schema_version": 288,
        "evidence_scope": "network_snapshot_authority_takeover_signature_v288",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "signature_id": "server-authority-signature-v1",
        "takeover_token": "server-authority-takeover-token-v59",
        "snapshot_id": "snapshot-authority-v171", "source": "server_snapshot",
        "snapshot_version": 60, "release": "release-1",
        ...
        "snapshot": {
            "signature_id": "server-authority-signature-v1",
            "takeover_token": "server-authority-takeover-token-v59",
            "snapshot_id": "snapshot-authority-v171", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 60,
            "sequence": 42183, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "signatures": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"signatures": 2, "unique": 2, "signed": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverSignatureV288ValidatorTest(unittest.TestCase):
    def test_accepts_signature_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_signature_binding(self):
        report = _report()
        report["signatures"][0]["takeover_token"] = "wrong-token"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority signature" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["signatures"][1]["order"] = 1
        report["signatures"][1]["item_id"] = report["signatures"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_rollup_and_counts(self):
        ...
        report["counts"]["signatures"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority signatures" in error for error in errors))
        self.assertTrue(any("counts.signatures" in error for error in errors))

    def test_rejects_mutation(self):
        report = _report()
        report["signatures"][0]["mutation_fields"] = ["authority"]
