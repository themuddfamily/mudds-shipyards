#!/usr/bin/env python3
"""Validate detached v216 network snapshot authority-takeover response evidence."""
...
SCHEMA_VERSION = 216
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_response_v216"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_RESPONSE = "server-authority-takeover-response-v1"
SNAPSHOT_ID = "snapshot-authority-v99"
...
def _response_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_response", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('response_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover response evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_response": AUTHORITY_TAKEOVER_RESPONSE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_response", report.get("authority_takeover_response")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover response")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover response")
        ...
        if not _digest(response_digest):
            errors.append(f"{prefix}.response_digest must be lowercase SHA-256")
        elif response_digest != _response_digest(record):
            errors.append(f"{prefix}.response_digest must bind authority takeover response")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover response records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover response records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RESPONSE_V216_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RESPONSE_V216_VALID")
    return 0
