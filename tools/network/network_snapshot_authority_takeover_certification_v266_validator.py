#!/usr/bin/env python3
"""Validate detached v266 network snapshot authority takeover certification evidence."""
...
SCHEMA_VERSION = 266
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_certification_v266"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
CERTIFICATION_ID = "server-authority-certification-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v37"
SNAPSHOT_ID = "snapshot-authority-v149"
...
def _certification_digest(record: dict[str, Any]) -> str:
    fields = (
        "certification_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('certification_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover certification evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority certification")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority certification")
        ...
        certification_digest = record.get("certification_digest")
        if not _digest(certification_digest):
            errors.append(f"{prefix}.certification_digest must be lowercase SHA-256")
        elif certification_digest != _certification_digest(record):
            errors.append(f"{prefix}.certification_digest must bind authority certification")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority certification records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority certification records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CERTIFICATION_V266_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CERTIFICATION_V266_VALID")
    return 0
