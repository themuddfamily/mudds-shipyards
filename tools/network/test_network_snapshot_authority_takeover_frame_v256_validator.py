...
try:
    from .network_snapshot_authority_takeover_frame_v256_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_frame_v256_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["frame_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['frame_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "frame_id": "server-authority-frame-v1",
            "authority_takeover_token": "server-authority-takeover-token-v27",
            "snapshot_id": "snapshot-authority-v139", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 28,
            "sequence": 29151, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverFrameV256ValidatorTest(unittest.TestCase):
    def test_accepts_authority_frame(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_frame_binding(self):
        report = _report()
        report["records"][0]["frame_id"] = "server-authority-frame-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority frame records" in error for error in errors))
