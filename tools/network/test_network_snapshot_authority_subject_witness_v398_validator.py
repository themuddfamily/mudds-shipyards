...
try:
    from .network_snapshot_authority_subject_witness_v398_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_subject_witness_v398_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-subject-witness-assertion-v1",
        "integrity_gate_id": "authority-subject-witness-v398",
        "snapshot_id": "snapshot-authority-v281", "authority": "server", "sequence": 44698,
...
def _report():
    ...
    return {
        "schema_version": 398,
        "evidence_scope": "network_snapshot_authority_subject_witness_v398",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-subject-witness-v398",
        "snapshot_id": "snapshot-authority-v281",
        "assertion_id": "authority-snapshot-subject-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 170, "release": "release-1",
...
class NetworkSnapshotAuthoritySubjectWitnessV398ValidatorTest(unittest.TestCase):
    def test_accepts_subject_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_subject_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["assertion_id"] = "wrong-subject"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority subject witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["subject"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority subject members" in error for error in errors))
