...
try:
    from .network_snapshot_authority_witness_v146_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_witness_v146_validator import validate_snapshot
...
class NetworkSnapshotAuthorityWitnessV146ValidatorTest(unittest.TestCase):
    def test_accepts_authority_witness_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_witness_binding(self):
        report = _report()
        report["records"][0]["authority_witness"] = "server-authority-witness-old"
        self.assertTrue(any("bind authority witness" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v28"
        self.assertTrue(any("bind authority witness" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority witness records" in error for error in validate_snapshot(report)))
