#!/usr/bin/env python3
"""Validate detached v208 network snapshot authority-replacement evidence."""
...
SCHEMA_VERSION = 208
EVIDENCE_SCOPE = "network_snapshot_authority_replacement_v208"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_REPLACEMENT = "server-authority-replacement-v1"
SNAPSHOT_ID = "snapshot-authority-v91"
...
def _replacement_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_replacement", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('replacement_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-replacement snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_replacement", report.get("authority_replacement")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority replacement")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority replacement")
        ...
        replacement_digest = record.get("replacement_digest")
        if not _digest(replacement_digest):
            errors.append(f"{prefix}.replacement_digest must be lowercase SHA-256")
        elif replacement_digest != _replacement_digest(record):
            errors.append(f"{prefix}.replacement_digest must bind authority replacement")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority replacement records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority replacement records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_REPLACEMENT_V208_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_REPLACEMENT_V208_VALID")
    return 0
