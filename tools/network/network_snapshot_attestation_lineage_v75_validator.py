#!/usr/bin/env python3
"""Validate detached v75 snapshot attestation/lineage evidence."""
...
SCHEMA_VERSION = 75
EVIDENCE_SCOPE = "network_snapshot_attestation_lineage_v75"
...
def _attestation_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "attestation", "lineage", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for attestation/lineage snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "attestation": ATTESTATION, "lineage": LINEAGE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("attestation", report.get("attestation")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match attestation/lineage")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("attestation", report.get("attestation")), ("lineage", report.get("lineage")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match attestation/lineage")
        ...
        if not _digest(attestation_digest):
            ...
            errors.append(f"{prefix}.attestation_digest must bind attestation/lineage")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_ATTESTATION_LINEAGE_V75_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_ATTESTATION_LINEAGE_V75_VALID")
    return 0
