...
try:
    from .network_snapshot_authority_signature_v164_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_signature_v164_validator import validate_snapshot
...
class NetworkSnapshotAuthoritySignatureV164ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_signature_binding(self):
        report = _report()
        report["records"][0]["authority_signature"] = "server-authority-signature-old"
        self.assertTrue(any("bind authority signature" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v46"
        self.assertTrue(any("bind authority signature" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority signature records" in error for error in validate_snapshot(report)))
