#!/usr/bin/env python3
"""Validate detached v346 authority/snapshot verification evidence."""
...
AUTHORITY = "server"
VERIFICATION_ID = "authority-verification-v346"
SNAPSHOT_ID = "snapshot-authority-v229"
ASSERTION_ID = "authority-snapshot-verification-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 118
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot verification evidence."""
    errors: list[str] = []
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority verification")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority verification")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority verification members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority verification members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_VERIFICATION_V346_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_VERIFICATION_V346_VALID")
    return 0
