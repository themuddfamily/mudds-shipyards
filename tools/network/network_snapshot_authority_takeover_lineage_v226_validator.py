#!/usr/bin/env python3
"""Validate detached v226 network snapshot authority-takeover lineage evidence."""
...
SCHEMA_VERSION = 226
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_lineage_v226"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_LINEAGE = "server-authority-takeover-lineage-v1"
SNAPSHOT_ID = "snapshot-authority-v109"
...
def _lineage_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_lineage", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('lineage_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_lineage": AUTHORITY_TAKEOVER_LINEAGE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_lineage", report.get("authority_takeover_lineage")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover lineage")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover lineage")
        ...
        lineage_digest = record.get("lineage_digest")
        if not _digest(lineage_digest):
            errors.append(f"{prefix}.lineage_digest must be lowercase SHA-256")
        elif lineage_digest != _lineage_digest(record):
            errors.append(f"{prefix}.lineage_digest must bind authority takeover lineage")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover lineage records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover lineage records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LINEAGE_V226_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_LINEAGE_V226_VALID")
    return 0
