...
try:
    from .network_snapshot_authority_release_witness_v386_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_release_witness_v386_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-release-witness-assertion-v1",
        "integrity_gate_id": "authority-release-witness-v386",
        "snapshot_id": "snapshot-authority-v269", "authority": "server", "sequence": 43486,
...
def _report():
    ...
    return {
        "schema_version": 386,
        "evidence_scope": "network_snapshot_authority_release_witness_v386",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-release-witness-v386",
        "snapshot_id": "snapshot-authority-v269",
        "assertion_id": "authority-snapshot-release-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 158, "release": "release-1",
...
class NetworkSnapshotAuthorityReleaseWitnessV386ValidatorTest(unittest.TestCase):
    def test_accepts_release_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_release_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["authority"] = "client"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority release witness" in error for error in errors))
