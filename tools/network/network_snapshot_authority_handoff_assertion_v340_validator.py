#!/usr/bin/env python3
"""Validate detached v340 authority/snapshot handoff assertion evidence."""
...
AUTHORITY = "server"
HANDOFF_ID = "authority-handoff-v340"
SNAPSHOT_ID = "snapshot-authority-v223"
ASSERTION_ID = "authority-snapshot-handoff-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 112
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot handoff evidence."""
    errors: list[str] = []
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority handoff")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority handoff")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority handoff members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority handoff members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_HANDOFF_ASSERTION_V340_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_HANDOFF_ASSERTION_V340_VALID")
    return 0
