...
try:
    from .network_snapshot_authority_takeover_dossier_v261_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_dossier_v261_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["dossier_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['dossier_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "dossier_id": "server-authority-dossier-v1",
            "authority_takeover_token": "server-authority-takeover-token-v32",
            "snapshot_id": "snapshot-authority-v144", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 33,
            "sequence": 34156, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverDossierV261ValidatorTest(unittest.TestCase):
    def test_accepts_authority_dossier(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_dossier_binding(self):
        report = _report()
        report["records"][0]["dossier_id"] = "server-authority-dossier-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority dossier" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority dossier records" in error for error in errors))
