...
try:
    from .network_snapshot_authority_assertion_v186_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_assertion_v186_validator import validate_snapshot
...
class NetworkSnapshotAuthorityAssertionV186ValidatorTest(unittest.TestCase):
    def test_accepts_authority_assertion_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_assertion_binding(self):
        report = _report()
        report["records"][0]["authority_assertion"] = "server-authority-assertion-old"
        self.assertTrue(any("bind authority assertion" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v68"
        self.assertTrue(any("bind authority assertion" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority assertion records" in error for error in validate_snapshot(report)))
