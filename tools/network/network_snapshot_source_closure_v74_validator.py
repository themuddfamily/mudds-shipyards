#!/usr/bin/env python3
"""Validate detached v74 snapshot source/closure evidence."""
...
SCHEMA_VERSION = 74
EVIDENCE_SCOPE = "network_snapshot_source_closure_v74"
...
def _source_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "source", "closure", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for source/closure snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "source": SOURCE, "closure": CLOSURE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("closure", report.get("closure")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match source/closure")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("closure", report.get("closure")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match source/closure")
        ...
        if not _digest(source_digest):
            ...
            errors.append(f"{prefix}.source_digest must bind source/closure")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_SOURCE_CLOSURE_V74_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_SOURCE_CLOSURE_V74_VALID")
    return 0
