#!/usr/bin/env python3
"""Validate detached v308 authority/snapshot certificate evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v308"
SNAPSHOT_ID = "snapshot-authority-v191"
CERTIFICATE_ID = "authority-snapshot-certificate-v1"
...
def _certificate_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('certificate_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot certificate evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority certificate")
    ...
    items = report.get("certificates")
    if not isinstance(items, list):
        errors.append(f"{label}.certificates must be an array")
        items = []
    ids: set[str] = set()
    certified = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.certificates[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority certificate")
        ...
        certificate_digest = item.get("certificate_digest")
        if not _digest(certificate_digest):
            errors.append(f"{prefix}.certificate_digest must be lowercase SHA-256")
        elif certificate_digest != _certificate_digest(item):
            errors.append(f"{prefix}.certificate_digest must bind authority certificate")
        if item.get("certified") is not True:
            errors.append(f"{prefix}.certified must be true")
        else:
            certified += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority certificates")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "certificates": len(items), "unique": len(ids), "certified": certified, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority certificates")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CERTIFICATE_V308_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CERTIFICATE_V308_VALID")
    return 0
