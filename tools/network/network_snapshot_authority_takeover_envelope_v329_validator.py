#!/usr/bin/env python3
"""Validate detached v329 authority/snapshot envelope evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v329"
SNAPSHOT_ID = "snapshot-authority-v212"
ENVELOPE_ID = "authority-snapshot-envelope-v1"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot envelope evidence."""
    errors: list[str] = []
    ...
    items = report.get("envelope_members")
    if not isinstance(items, list):
        errors.append(f"{label}.envelope_members must be an array")
        items = []
    ids: set[str] = set()
    sealed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.envelope_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority envelope")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority envelope members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "envelope_members": len(items), "unique": len(ids), "sealed": sealed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority envelope members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ENVELOPE_V329_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ENVELOPE_V329_VALID")
    return 0
