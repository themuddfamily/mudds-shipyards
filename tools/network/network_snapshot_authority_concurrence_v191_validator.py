#!/usr/bin/env python3
"""Validate detached v191 network snapshot authority-concurrence evidence."""
...
SCHEMA_VERSION = 191
EVIDENCE_SCOPE = "network_snapshot_authority_concurrence_v191"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_CONCURRENCE = "server-authority-concurrence-v1"
SNAPSHOT_ID = "snapshot-authority-v74"
...
def _concurrence_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_concurrence", "snapshot_id", "authority", "source", "release",
        "version", "check_id", "sequence", "expected_digest", "observed_digest",
    )
    material = "|".join(str(record.get(field)) for field in fields)
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('concurrence_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-concurrence snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_concurrence": AUTHORITY_CONCURRENCE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_concurrence", report.get("authority_concurrence")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority concurrence")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority concurrence")
        ...
        concurrence_digest = record.get("concurrence_digest")
        if not _digest(concurrence_digest):
            errors.append(f"{prefix}.concurrence_digest must be lowercase SHA-256")
        elif concurrence_digest != _concurrence_digest(record):
            errors.append(f"{prefix}.concurrence_digest must bind authority concurrence")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority concurrence records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority concurrence records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CONCURRENCE_V191_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CONCURRENCE_V191_VALID")
    return 0
