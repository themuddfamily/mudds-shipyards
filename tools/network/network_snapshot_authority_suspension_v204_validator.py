#!/usr/bin/env python3
"""Validate detached v204 network snapshot authority-suspension evidence."""
...
SCHEMA_VERSION = 204
EVIDENCE_SCOPE = "network_snapshot_authority_suspension_v204"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_SUSPENSION = "server-authority-suspension-v1"
SNAPSHOT_ID = "snapshot-authority-v87"
...
def _suspension_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_suspension", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('suspension_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-suspension snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_suspension", report.get("authority_suspension")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority suspension")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority suspension")
        ...
        suspension_digest = record.get("suspension_digest")
        if not _digest(suspension_digest):
            errors.append(f"{prefix}.suspension_digest must be lowercase SHA-256")
        elif suspension_digest != _suspension_digest(record):
            errors.append(f"{prefix}.suspension_digest must bind authority suspension")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority suspension records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority suspension records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_SUSPENSION_V204_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_SUSPENSION_V204_VALID")
    return 0
