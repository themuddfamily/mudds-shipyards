...
try:
    from .network_snapshot_authority_lease_v156_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_lease_v156_validator import validate_snapshot
...
class NetworkSnapshotAuthorityLeaseV156ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_lease_binding(self):
        report = _report()
        report["records"][0]["authority_lease"] = "server-authority-lease-old"
        self.assertTrue(any("bind authority lease" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v38"
        self.assertTrue(any("bind authority lease" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority lease records" in error for error in validate_snapshot(report)))
