...
try:
    from .network_snapshot_authority_verification_v197_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_verification_v197_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["verification_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['verification_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_verification": "server-authority-verification-v1",
            "snapshot_id": "snapshot-authority-v80", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2993, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityVerificationV197ValidatorTest(unittest.TestCase):
    def test_accepts_verified_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_verification_binding(self):
        report = _report()
        report["records"][0]["authority_verification"] = "server-authority-verification-old"
        self.assertTrue(any("bind authority verification" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v79"
        self.assertTrue(any("bind authority verification" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority verification records" in error for error in validate_snapshot(report)))
