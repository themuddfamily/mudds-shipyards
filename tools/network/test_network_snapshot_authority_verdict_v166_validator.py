...
try:
    from .network_snapshot_authority_verdict_v166_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_verdict_v166_validator import validate_snapshot
...
class NetworkSnapshotAuthorityVerdictV166ValidatorTest(unittest.TestCase):
    def test_accepts_authority_verdict_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_verdict_binding(self):
        report = _report()
        report["records"][0]["authority_verdict"] = "server-authority-verdict-old"
        self.assertTrue(any("bind authority verdict" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v48"
        self.assertTrue(any("bind authority verdict" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority verdict records" in error for error in validate_snapshot(report)))
