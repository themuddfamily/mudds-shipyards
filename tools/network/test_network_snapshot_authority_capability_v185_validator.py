...
try:
    from .network_snapshot_authority_capability_v185_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_capability_v185_validator import validate_snapshot
...
class NetworkSnapshotAuthorityCapabilityV185ValidatorTest(unittest.TestCase):
    def test_accepts_authority_capability_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_capability_binding(self):
        report = _report()
        report["records"][0]["authority_capability"] = "server-authority-capability-old"
        self.assertTrue(any("bind authority capability" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v67"
        self.assertTrue(any("bind authority capability" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority capability records" in error for error in validate_snapshot(report)))
