...
try:
    from .network_snapshot_audit_lineage_v100_validator import validate_snapshot
except ImportError:
    from network_snapshot_audit_lineage_v100_validator import validate_snapshot
...
def _record(order, check_id, digest):
    record = {
        "order": order, "check_id": check_id, "authority": "server", "audit_id": "snapshot-audit-v3", "lineage": "snapshot-lineage-root-v3", "source": "server_snapshot", "release": "release-1", "version": 2, "sequence": 420,
        "expected_digest": digest, "observed_digest": digest, "audited": True, "mutation_fields": [], "state_changed": False,
    }
    material = "|".join(str(record[key]) for key in ("authority", "audit_id", "lineage", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"))
...
def _report():
    ...
    return {
        "schema_version": 100, "evidence_scope": "network_snapshot_audit_lineage_v100", "evidence_mode": "detached_contract_fixture", "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "audit_id": "snapshot-audit-v3", "lineage": "snapshot-lineage-root-v3", "source": "server_snapshot", "snapshot_version": 2, "release": "release-1",
        "native_claims": False, "uses_live_network": False, "snapshot_detached": True, "no_mutation_guarantee": True,
        "snapshot": {"authority": "server", "audit_id": "snapshot-audit-v3", "lineage": "snapshot-lineage-root-v3", "source": "server_snapshot", "release": "release-1", "version": 2, "sequence": 420, "digest": hashlib.sha256(b"snapshot").hexdigest()},
        "records": records,
...
class NetworkSnapshotAuditLineageV100ValidatorTest(unittest.TestCase):
