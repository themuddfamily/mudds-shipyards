...
try:
    from .network_snapshot_authority_takeover_envelope_v281_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_envelope_v281_validator import validate_snapshot


def _item(order, item_id, digest, channel):
    item = {
        "order": order, "item_id": item_id,
        "envelope_id": "server-authority-envelope-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v52",
        "snapshot_id": "snapshot-authority-v164", "sequence": 42176,
        "channel": channel, "expected_digest": digest, "observed_digest": digest,
        "wrapped": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "envelope_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "channel", "item_id", "expected_digest", "observed_digest",
    )
...
def _report():
    ...
    return {
        "schema_version": 281,
        "evidence_scope": "network_snapshot_authority_takeover_envelope_v281",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "envelope_id": "server-authority-envelope-v1",
        "takeover_token": "server-authority-takeover-token-v52",
        "snapshot_id": "snapshot-authority-v164", "source": "server_snapshot",
        "snapshot_version": 53, "release": "release-1",
        ...
        "snapshot": {
            "envelope_id": "server-authority-envelope-v1",
            "takeover_token": "server-authority-takeover-token-v52",
            "snapshot_id": "snapshot-authority-v164", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 53,
            "sequence": 42176, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "envelope_items": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"items": 2, "unique": 2, "wrapped": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverEnvelopeV281ValidatorTest(unittest.TestCase):
    def test_accepts_envelope_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_envelope_binding(self):
        report = _report()
        report["envelope_items"][0]["takeover_token"] = "wrong-token"
    ...
    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["envelope_items"][1]["order"] = 1
        report["envelope_items"][1]["item_id"] = report["envelope_items"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_rollup_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority envelope items" in error for error in errors))
        self.assertTrue(any("counts.items" in error for error in errors))

    def test_rejects_mutation(self):
        report = _report()
        report["envelope_items"][0]["mutation_fields"] = ["authority"]
