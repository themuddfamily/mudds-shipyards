...
try:
    from .network_snapshot_authority_takeover_inspection_v265_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_inspection_v265_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["inspection_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['inspection_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "inspection_id": "server-authority-inspection-v1",
            "authority_takeover_token": "server-authority-takeover-token-v36",
            "snapshot_id": "snapshot-authority-v148", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 37,
            "sequence": 38160, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverInspectionV265ValidatorTest(unittest.TestCase):
    def test_accepts_authority_inspection(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_inspection_binding(self):
        report = _report()
        report["records"][0]["inspection_id"] = "server-authority-inspection-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority inspection" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority inspection records" in error for error in errors))
