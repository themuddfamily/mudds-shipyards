...
try:
    from .network_snapshot_authority_takeover_capsule_v258_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_capsule_v258_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["capsule_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['capsule_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "capsule_id": "server-authority-capsule-v1",
            "authority_takeover_token": "server-authority-takeover-token-v29",
            "snapshot_id": "snapshot-authority-v141", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 30,
            "sequence": 31153, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverCapsuleV258ValidatorTest(unittest.TestCase):
    def test_accepts_authority_capsule(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_capsule_binding(self):
        report = _report()
        report["records"][0]["capsule_id"] = "server-authority-capsule-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority capsule records" in error for error in errors))
