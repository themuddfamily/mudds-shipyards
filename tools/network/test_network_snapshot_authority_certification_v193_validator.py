...
try:
    from .network_snapshot_authority_certification_v193_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_certification_v193_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["certification_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['certification_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_certification": "server-authority-certification-v1",
            "snapshot_id": "snapshot-authority-v76", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2977, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityCertificationV193ValidatorTest(unittest.TestCase):
    def test_accepts_certified_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_certification_binding(self):
        report = _report()
        report["records"][0]["authority_certification"] = "server-authority-certification-old"
        self.assertTrue(any("bind authority certification" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v75"
        self.assertTrue(any("bind authority certification" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority certification records" in error for error in validate_snapshot(report)))
