#!/usr/bin/env python3
"""Validate detached v354 authority/snapshot closure-gate evidence."""
...
AUTHORITY = "server"
CLOSURE_GATE_ID = "authority-closure-gate-v354"
SNAPSHOT_ID = "snapshot-authority-v237"
ASSERTION_ID = "authority-snapshot-closure-gate-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 126
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot closure-gate evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "closure_gate_id": CLOSURE_GATE_ID,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("closure_gate_id", report.get("closure_gate_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority closure gate")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority closure gate")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority closure gate")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority closure gate members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "assertion_members": len(items), "unique": len(ids), "asserted": asserted, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority closure gate members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CLOSURE_GATE_V354_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CLOSURE_GATE_V354_VALID")
    return 0
