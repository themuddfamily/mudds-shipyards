...
try:
    from .network_snapshot_authority_takeover_injunction_v243_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_injunction_v243_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["injunction_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['injunction_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "injunction_id": "server-authority-injunction-v1",
            "authority_takeover_token": "server-authority-takeover-token-v14",
            "snapshot_id": "snapshot-authority-v126", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 15,
            "sequence": 16138, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverInjunctionV243ValidatorTest(unittest.TestCase):
    def test_accepts_authority_injunction(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_injunction_binding(self):
        report = _report()
        report["records"][0]["injunction_id"] = "server-authority-injunction-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority injunction" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority injunction records" in error for error in errors))
