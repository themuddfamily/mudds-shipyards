#!/usr/bin/env python3
"""Validate detached v157 network snapshot authority-grant evidence."""
...
SCHEMA_VERSION = 157
EVIDENCE_SCOPE = "network_snapshot_authority_grant_v157"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_GRANT = "server-authority-grant-v1"
SNAPSHOT_ID = "snapshot-authority-v40"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_grant", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_grant", report.get("authority_grant")),
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority grant")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_GRANT_V157_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_GRANT_V157_VALID")
    return 0
