#!/usr/bin/env python3
"""Validate detached v210 network snapshot authority-takeover evidence."""
...
SCHEMA_VERSION = 210
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_v210"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER = "server-authority-takeover-v1"
SNAPSHOT_ID = "snapshot-authority-v93"
...
def _takeover_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('takeover_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover", report.get("authority_takeover")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover")
        ...
        takeover_digest = record.get("takeover_digest")
        if not _digest(takeover_digest):
            errors.append(f"{prefix}.takeover_digest must be lowercase SHA-256")
        elif takeover_digest != _takeover_digest(record):
            errors.append(f"{prefix}.takeover_digest must bind authority takeover")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_V210_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_V210_VALID")
    return 0
