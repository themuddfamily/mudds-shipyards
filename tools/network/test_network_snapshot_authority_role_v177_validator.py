...
try:
    from .network_snapshot_authority_role_v177_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_role_v177_validator import validate_snapshot
...
class NetworkSnapshotAuthorityRoleV177ValidatorTest(unittest.TestCase):
    def test_accepts_authority_role_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_role_binding(self):
        report = _report()
        report["records"][0]["authority_role"] = "server-authority-role-old"
        self.assertTrue(any("bind authority role" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v59"
        self.assertTrue(any("bind authority role" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority role records" in error for error in validate_snapshot(report)))
