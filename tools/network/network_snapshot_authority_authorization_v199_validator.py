#!/usr/bin/env python3
"""Validate detached v199 network snapshot authority-authorization evidence."""
...
SCHEMA_VERSION = 199
EVIDENCE_SCOPE = "network_snapshot_authority_authorization_v199"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_AUTHORIZATION = "server-authority-authorization-v1"
SNAPSHOT_ID = "snapshot-authority-v82"
...
def _authorization_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_authorization", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('authorization_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-authorization snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_authorization", report.get("authority_authorization")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority authorization")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority authorization")
        ...
        authorization_digest = record.get("authorization_digest")
        if not _digest(authorization_digest):
            errors.append(f"{prefix}.authorization_digest must be lowercase SHA-256")
        elif authorization_digest != _authorization_digest(record):
            errors.append(f"{prefix}.authorization_digest must bind authority authorization")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority authorization records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority authorization records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_AUTHORIZATION_V199_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_AUTHORIZATION_V199_VALID")
    return 0
