...
try:
    from .network_snapshot_authority_takeover_bundle_v254_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_bundle_v254_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["bundle_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['bundle_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "bundle_id": "server-authority-bundle-v1",
            "authority_takeover_token": "server-authority-takeover-token-v25",
            "snapshot_id": "snapshot-authority-v137", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 26,
            "sequence": 27149, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverBundleV254ValidatorTest(unittest.TestCase):
    def test_accepts_authority_bundle(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_bundle_binding(self):
        report = _report()
        report["records"][0]["bundle_id"] = "server-authority-bundle-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority bundle records" in error for error in errors))
