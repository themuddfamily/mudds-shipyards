...
try:
    from .network_snapshot_authority_takeover_proof_v238_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_proof_v238_validator import validate_snapshot
...
def _report():
    ...
    return {
        ...
        "snapshot": {
            "proof_id": "server-authority-proof-v1",
            "authority_takeover_token": "server-authority-takeover-token-v9",
            "snapshot_id": "snapshot-authority-v121", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 10,
            "sequence": 11133, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverProofV238ValidatorTest(unittest.TestCase):
    def test_accepts_authority_proof(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_proof_binding(self):
        report = _report()
        report["records"][0]["proof_id"] = "server-authority-proof-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority proof records" in error for error in errors))
