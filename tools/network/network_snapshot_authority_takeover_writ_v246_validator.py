#!/usr/bin/env python3
"""Validate detached v246 network snapshot authority takeover writ evidence."""
...
SCHEMA_VERSION = 246
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_writ_v246"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
WRIT_ID = "server-authority-writ-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v17"
SNAPSHOT_ID = "snapshot-authority-v129"
...
def _writ_digest(record: dict[str, Any]) -> str:
    fields = (
        "writ_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('writ_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover writ evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority writ")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority writ")
        ...
        writ_digest = record.get("writ_digest")
        if not _digest(writ_digest):
            errors.append(f"{prefix}.writ_digest must be lowercase SHA-256")
        elif writ_digest != _writ_digest(record):
            errors.append(f"{prefix}.writ_digest must bind authority writ")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority writ records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority writ records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WRIT_V246_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_WRIT_V246_VALID")
    return 0
