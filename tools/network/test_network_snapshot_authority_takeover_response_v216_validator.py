...
try:
    from .network_snapshot_authority_takeover_response_v216_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_response_v216_validator import validate_snapshot
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['response_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_takeover_response": "server-authority-takeover-response-v1",
            "snapshot_id": "snapshot-authority-v99", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3069, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverResponseV216ValidatorTest(unittest.TestCase):
    def test_accepts_takeover_response_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_response_binding(self):
        report = _report()
        report["records"][0]["authority_takeover_response"] = "server-authority-takeover-response-old"
        self.assertTrue(any("bind authority takeover response" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v98"
        self.assertTrue(any("bind authority takeover response" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority takeover response records" in error for error in validate_snapshot(report)))
