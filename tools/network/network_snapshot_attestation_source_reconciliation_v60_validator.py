#!/usr/bin/env python3
"""Validate detached v60 snapshot attestation/source reconciliation evidence."""
...
SCHEMA_VERSION = 60
EVIDENCE_SCOPE = "network_snapshot_attestation_source_reconciliation_v60"
...
def _attestation_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "source", "channel", "attestation", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for attestation/source-bound snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "attestation": ATTESTATION, "snapshot_version": SNAPSHOT_VERSION, "release": RELEASE_ID,
    }
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("channel", report.get("channel")), ("attestation", report.get("attestation")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match attestation/source contract")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("channel", report.get("channel")), ("attestation", report.get("attestation")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match attestation/source contract")
        ...
        if not _digest(attestation_digest):
            ...
            errors.append(f"{prefix}.attestation_digest must bind attestation/source")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match attestations")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match attestations")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_ATTESTATION_SOURCE_RECONCILIATION_V60_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_ATTESTATION_SOURCE_RECONCILIATION_V60_VALID")
    return 0
