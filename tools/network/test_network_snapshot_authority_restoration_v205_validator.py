...
try:
    from .network_snapshot_authority_restoration_v205_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_restoration_v205_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["restoration_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['restoration_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_restoration": "server-authority-restoration-v1",
            "snapshot_id": "snapshot-authority-v88", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3025, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityRestorationV205ValidatorTest(unittest.TestCase):
    def test_accepts_restored_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_restoration_binding(self):
        report = _report()
        report["records"][0]["authority_restoration"] = "server-authority-restoration-old"
        self.assertTrue(any("bind authority restoration" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v87"
        self.assertTrue(any("bind authority restoration" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority restoration records" in error for error in validate_snapshot(report)))
