...
try:
    from .network_snapshot_authority_resolution_v188_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_resolution_v188_validator import validate_snapshot
...
class NetworkSnapshotAuthorityResolutionV188ValidatorTest(unittest.TestCase):
    def test_accepts_authority_resolution_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_resolution_binding(self):
        report = _report()
        report["records"][0]["authority_resolution"] = "server-authority-resolution-old"
        self.assertTrue(any("bind authority resolution" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v70"
        self.assertTrue(any("bind authority resolution" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority resolution records" in error for error in validate_snapshot(report)))
