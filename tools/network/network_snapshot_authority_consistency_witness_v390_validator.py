#!/usr/bin/env python3
"""Validate detached v390 authority/snapshot consistency-witness evidence."""
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot consistency-witness evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority consistency witness")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority consistency witness")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority consistency witness")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CONSISTENCY_WITNESS_V390_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CONSISTENCY_WITNESS_V390_VALID")
    return 0
