#!/usr/bin/env python3
"""Validate detached v167 network snapshot authority-proof evidence."""
...
SCHEMA_VERSION = 167
EVIDENCE_SCOPE = "network_snapshot_authority_proof_v167"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_PROOF = "server-authority-proof-v1"
SNAPSHOT_ID = "snapshot-authority-v50"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_proof", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_proof", report.get("authority_proof")),
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority proof")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_PROOF_V167_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_PROOF_V167_VALID")
    return 0
