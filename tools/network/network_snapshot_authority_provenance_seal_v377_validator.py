#!/usr/bin/env python3
"""Validate detached v377 authority/snapshot provenance-seal evidence."""
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority provenance seal")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority provenance seal")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority provenance seal")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_PROVENANCE_SEAL_V377_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_PROVENANCE_SEAL_V377_VALID")
    return 0
