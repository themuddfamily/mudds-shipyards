#!/usr/bin/env python3
"""Validate v40 linked authority/provenance digest evidence."""
...
def _link_digest(link: dict[str, Any]) -> str:
    return hashlib.sha256(f"{AUTHORITY}|{link.get('provenance_id')}|{link.get('parent_digest')}|{link.get('child_digest')}".encode("utf-8")).hexdigest()


def _provenance_digest(links: list[dict[str, Any]]) -> str:
    return hashlib.sha256("\n".join(f"{link.get('order')}|{link.get('provenance_id')}|{link.get('link_digest')}" for link in links).encode("utf-8")).hexdigest()


def validate_provenance(report: Any, label: str = "provenance") -> list[str]:
    """Return linked predecessor, provenance, digest, count, and mutation errors."""
    ...
    for index, link in enumerate(links):
        ...
        provenance_id = link.get("provenance_id")
        expected_provenance = f"{AUTHORITY}|{link_id}|{order}"
        ...
        if link.get("parent_digest") != current:
            errors.append(f"{prefix}.parent_digest must match provenance predecessor")
        ...
        if not _digest(link_digest):
            ...
            errors.append(f"{prefix}.link_digest must bind authority provenance")
    ...
    if _digest(report.get("final_digest")) and current != report.get("final_digest"):
        errors.append(f"{label}.final_digest must match provenance chain")
    ...
    elif provenance_digest != _provenance_digest(links):
        errors.append(f"{label}.provenance_digest must match ordered links")

    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected = {"links": len(links), "unique": len(provenance_ids), "reconciled": reconciled_count, "mutations": mutation_count}
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match provenance links")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_LINKED_AUTHORITY_PROVENANCE_DIGEST_V40_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_LINKED_AUTHORITY_PROVENANCE_DIGEST_V40_VALID")
    return 0
