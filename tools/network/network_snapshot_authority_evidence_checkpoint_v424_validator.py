#!/usr/bin/env python3
"""Validate detached v424 authority/snapshot evidence-checkpoint evidence."""
from __future__ import annotations
import argparse,hashlib,json,re
from pathlib import Path
from typing import Any
SCHEMA_VERSION=424;EVIDENCE_SCOPE="network_snapshot_authority_evidence_checkpoint_v424";EVIDENCE_MODE="detached_contract_fixture";POLICY_VERSION="network_replication_interest_authority_v1";AUTHORITY="server";INTEGRITY_GATE_ID="authority-evidence-checkpoint-v424";SNAPSHOT_ID="snapshot-authority-v307";ASSERTION_ID="authority-snapshot-evidence-checkpoint-assertion-v1";SOURCE="server_snapshot";SNAPSHOT_VERSION=196;RELEASE_ID="release-1";SHA256=re.compile(r"^[0-9a-f]{64}$");NOT_RUN_CHECKS=("stale_check","native_run","hardware_run","human_review")
...
def _rollup_digest(a:list[dict[str,Any]])->str:return hashlib.sha256("\n".join(f"{x.get('order')}|{x.get('item_id')}|{x.get('assertion_digest')}"for x in a).encode()).hexdigest()
...
def validate_snapshot(r:Any,label:str="snapshot")->list[str]:
 ...
 ex={"schema_version":SCHEMA_VERSION,"evidence_scope":EVIDENCE_SCOPE,"evidence_mode":EVIDENCE_MODE,"policy_version":POLICY_VERSION,"authority":AUTHORITY,"integrity_gate_id":INTEGRITY_GATE_ID,"snapshot_id":SNAPSHOT_ID,"assertion_id":ASSERTION_ID,"source":SOURCE,"snapshot_version":SNAPSHOT_VERSION,"release":RELEASE_ID}
 for k,v in ex.items():
 ...
 for k,v in(("integrity_gate_id",r.get("integrity_gate_id")),("snapshot_id",r.get("snapshot_id")),("assertion_id",r.get("assertion_id")),("authority",AUTHORITY),("source",r.get("source")),("release",r.get("release")),("version",r.get("snapshot_version"))):
  if s.get(k)!=v:e.append(f"{label}.snapshot.{k} must match authority evidence checkpoint")
 ...
 for n,i in enumerate(a):
  ...
  for k,v in(("assertion_id",r.get("assertion_id")),("integrity_gate_id",r.get("integrity_gate_id")),("snapshot_id",r.get("snapshot_id")),("authority",AUTHORITY)):
   if i.get(k)!=v:e.append(f"{p}.{k} must bind authority evidence checkpoint")
  ...
  elif z!=_assertion_digest(i):e.append(f"{p}.assertion_digest must bind authority evidence checkpoint")
 ...
 if not isinstance(c,dict):e.append(f"{label}.counts must be an object")
  ...
  x={"assertion_members":len(a),"unique":len(ids),"asserted":asserted,"mutations":mutations}
  for k,v in x.items():
...
def main()->int:
 ...
 if e:print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_CHECKPOINT_V424_INVALID");[print(f"- {x}")for x in e];return 1
 print("NETWORK_SNAPSHOT_AUTHORITY_EVIDENCE_CHECKPOINT_V424_VALID");return 0
