#!/usr/bin/env python3
"""Validate detached v218 network snapshot authority-takeover outcome evidence."""
...
SCHEMA_VERSION = 218
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_outcome_v218"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_OUTCOME = "server-authority-takeover-outcome-v1"
SNAPSHOT_ID = "snapshot-authority-v101"
...
def _outcome_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_outcome", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('outcome_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover outcome evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_outcome": AUTHORITY_TAKEOVER_OUTCOME,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_outcome", report.get("authority_takeover_outcome")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover outcome")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover outcome")
        ...
        if not _digest(outcome_digest):
            errors.append(f"{prefix}.outcome_digest must be lowercase SHA-256")
        elif outcome_digest != _outcome_digest(record):
            errors.append(f"{prefix}.outcome_digest must bind authority takeover outcome")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover outcome records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover outcome records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_OUTCOME_V218_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_OUTCOME_V218_VALID")
    return 0
