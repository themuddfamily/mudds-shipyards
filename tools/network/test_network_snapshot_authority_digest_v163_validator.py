...
try:
    from .network_snapshot_authority_digest_v163_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_digest_v163_validator import validate_snapshot
...
class NetworkSnapshotAuthorityDigestV163ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_digest_binding(self):
        report = _report()
        report["records"][0]["authority_digest"] = "server-authority-digest-old"
        self.assertTrue(any("bind authority digest" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v45"
        self.assertTrue(any("bind authority digest" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority digest records" in error for error in validate_snapshot(report)))
