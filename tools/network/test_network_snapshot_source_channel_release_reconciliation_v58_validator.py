...
try:
    from .network_snapshot_source_channel_release_reconciliation_v58_validator import validate_checks
except ImportError:
    from network_snapshot_source_channel_release_reconciliation_v58_validator import validate_checks
...
def _check(order, check_id, digest):
    ...
    material = "|".join(str(check[key]) for key in ("authority", "source", "channel", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"))
...
def _report():
    ...
    return {
        "schema_version": 58, "evidence_scope": "network_snapshot_source_channel_release_reconciliation_v58",
...
class NetworkSnapshotSourceChannelReleaseReconciliationV58ValidatorTest(unittest.TestCase):
    def test_accepts_source_channel_reconciliations(self):
