#!/usr/bin/env python3
"""Validate detached v124 network snapshot authority-chain evidence."""
...
SCHEMA_VERSION = 124
EVIDENCE_SCOPE = "network_snapshot_authority_chain_v124"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_CHAIN = "server-authority-chain-v2"
SNAPSHOT_ID = "snapshot-authority-v7"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_chain", "snapshot_id", "authority", "source", "release", "version",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-chain snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_chain": AUTHORITY_CHAIN,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (
        ("authority_chain", report.get("authority_chain")), ("snapshot_id", report.get("snapshot_id")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_CHAIN_V124_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_CHAIN_V124_VALID")
    return 0
