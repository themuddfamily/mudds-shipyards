#!/usr/bin/env python3
"""Validate detached v263 network snapshot authority takeover review evidence."""
...
SCHEMA_VERSION = 263
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_review_v263"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
REVIEW_ID = "server-authority-review-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v34"
SNAPSHOT_ID = "snapshot-authority-v146"
...
def _review_digest(record: dict[str, Any]) -> str:
    fields = (
        "review_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('review_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover review evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority review")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority review")
        ...
        if not _digest(review_digest):
            errors.append(f"{prefix}.review_digest must be lowercase SHA-256")
        elif review_digest != _review_digest(record):
            errors.append(f"{prefix}.review_digest must bind authority review")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority review records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority review records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_REVIEW_V263_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_REVIEW_V263_VALID")
    return 0
