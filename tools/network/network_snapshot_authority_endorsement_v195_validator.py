#!/usr/bin/env python3
"""Validate detached v195 network snapshot authority-endorsement evidence."""
...
SCHEMA_VERSION = 195
EVIDENCE_SCOPE = "network_snapshot_authority_endorsement_v195"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ENDORSEMENT = "server-authority-endorsement-v1"
SNAPSHOT_ID = "snapshot-authority-v78"
...
def _endorsement_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_endorsement", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('endorsement_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-endorsement snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_endorsement", report.get("authority_endorsement")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority endorsement")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority endorsement")
        ...
        endorsement_digest = record.get("endorsement_digest")
        if not _digest(endorsement_digest):
            errors.append(f"{prefix}.endorsement_digest must be lowercase SHA-256")
        elif endorsement_digest != _endorsement_digest(record):
            errors.append(f"{prefix}.endorsement_digest must bind authority endorsement")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority endorsement records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority endorsement records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ENDORSEMENT_V195_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ENDORSEMENT_V195_VALID")
    return 0
