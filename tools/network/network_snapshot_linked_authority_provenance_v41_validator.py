#!/usr/bin/env python3
"""Validate v41 linked authority/provenance evidence."""
...
def _provenance_digest(link: dict[str, Any]) -> str:
    provenance = link.get("provenance", {})
    payload = f"{AUTHORITY}|{link.get('link_id')}|{provenance.get('event')}|{provenance.get('sequence')}|{link.get('parent_digest')}|{link.get('child_digest')}"
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def _provenance_root(links: list[dict[str, Any]]) -> str:
    return hashlib.sha256("\n".join(f"{link.get('order')}|{link.get('link_id')}|{link.get('provenance_digest')}" for link in links).encode("utf-8")).hexdigest()


def validate_provenance(report: Any, label: str = "provenance") -> list[str]:
    """Return linked state, structured provenance, root digest, counts, and mutation errors."""
    ...
    for index, link in enumerate(links):
        ...
        provenance_digest = link.get("provenance_digest")
        if not _digest(provenance_digest):
            errors.append(f"{prefix}.provenance_digest must be lowercase SHA-256")
        elif provenance_digest != _provenance_digest(link):
            errors.append(f"{prefix}.provenance_digest must bind structured provenance")
    ...
    if not _digest(provenance_root):
        ...
        errors.append(f"{label}.provenance_root must match linked provenance")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_LINKED_AUTHORITY_PROVENANCE_V41_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_LINKED_AUTHORITY_PROVENANCE_V41_VALID")
    return 0
