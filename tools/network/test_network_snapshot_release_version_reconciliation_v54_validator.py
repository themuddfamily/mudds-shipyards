...
try:
    from .network_snapshot_release_version_reconciliation_v54_validator import validate_checks
except ImportError:
    from network_snapshot_release_version_reconciliation_v54_validator import validate_checks
...
def _check(order, check_id, digest):
    ...
    material = "|".join(str(check[key]) for key in ("authority", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"))
    check["summary_digest"] = hashlib.sha256(material.encode()).hexdigest()
    return check
...
def _report():
    ...
    return {
        "schema_version": 54, "evidence_scope": "network_snapshot_release_version_reconciliation_v54",
        ...
        "checks": checks,
        "summary_digest": hashlib.sha256("\n".join(f"{c['order']}|{c['check_id']}|{c['summary_digest']}" for c in checks).encode()).hexdigest(),
        "counts": {"checks": 2, "unique": 2, "reconciled": 2, "mutations": 0},
    }


class NetworkSnapshotReleaseVersionReconciliationV54ValidatorTest(unittest.TestCase):
    def test_accepts_reconciliation_summary(self):
    ...
    def test_rejects_summary_digest(self):
        report = _report()
        report["summary_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match reconciliation checks" in error for error in validate_checks(report)))
