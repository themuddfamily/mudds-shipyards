#!/usr/bin/env python3
"""Validate detached v335 authority/snapshot signal evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v335"
SNAPSHOT_ID = "snapshot-authority-v218"
SIGNAL_ID = "authority-snapshot-signal-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 107
...
def _signal_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('signal_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot signal evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority signal")
    ...
    items = report.get("signal_members")
    if not isinstance(items, list):
        errors.append(f"{label}.signal_members must be an array")
        items = []
    ids: set[str] = set()
    emitted = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.signal_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority signal")
        ...
        signal_digest = item.get("signal_digest")
        if not _digest(signal_digest):
            errors.append(f"{prefix}.signal_digest must be lowercase SHA-256")
        elif signal_digest != _signal_digest(item):
            errors.append(f"{prefix}.signal_digest must bind authority signal")
        if item.get("emitted") is not True:
            errors.append(f"{prefix}.emitted must be true")
        else:
            emitted += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority signal members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "signal_members": len(items), "unique": len(ids), "emitted": emitted, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority signal members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SIGNAL_V335_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SIGNAL_V335_VALID")
    return 0
