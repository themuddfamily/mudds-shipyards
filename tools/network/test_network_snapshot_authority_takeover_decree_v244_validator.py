...
try:
    from .network_snapshot_authority_takeover_decree_v244_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_decree_v244_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["decree_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['decree_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "decree_id": "server-authority-decree-v1",
            "authority_takeover_token": "server-authority-takeover-token-v15",
            "snapshot_id": "snapshot-authority-v127", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 16,
            "sequence": 17139, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverDecreeV244ValidatorTest(unittest.TestCase):
    def test_accepts_authority_decree(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_decree_binding(self):
        report = _report()
        report["records"][0]["decree_id"] = "server-authority-decree-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority decree" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority decree records" in error for error in errors))
