#!/usr/bin/env python3
"""Validate detached v215 network snapshot authority-takeover challenge evidence."""
...
SCHEMA_VERSION = 215
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_challenge_v215"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_CHALLENGE = "server-authority-takeover-challenge-v1"
SNAPSHOT_ID = "snapshot-authority-v98"
...
def _challenge_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_challenge", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('challenge_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover challenge evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_challenge": AUTHORITY_TAKEOVER_CHALLENGE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_challenge", report.get("authority_takeover_challenge")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover challenge")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover challenge")
        ...
        if not _digest(challenge_digest):
            errors.append(f"{prefix}.challenge_digest must be lowercase SHA-256")
        elif challenge_digest != _challenge_digest(record):
            errors.append(f"{prefix}.challenge_digest must bind authority takeover challenge")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover challenge records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover challenge records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHALLENGE_V215_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHALLENGE_V215_VALID")
    return 0
