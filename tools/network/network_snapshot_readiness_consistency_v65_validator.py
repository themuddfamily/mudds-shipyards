#!/usr/bin/env python3
"""Validate detached v65 snapshot readiness/consistency evidence."""
...
SCHEMA_VERSION = 65
EVIDENCE_SCOPE = "network_snapshot_readiness_consistency_v65"
...
def _status_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "source", "readiness", "consistency", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
    return hashlib.sha256(material.encode()).hexdigest()


def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(f"{r.get('order')}|{r.get('check_id')}|{r.get('status_digest')}" for r in records)
    return hashlib.sha256(material.encode()).hexdigest()


def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for readiness/consistency snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "source": SOURCE, "readiness": READINESS,
        "consistency": CONSISTENCY, "snapshot_version": SNAPSHOT_VERSION, "release": RELEASE_ID,
    }
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("readiness", report.get("readiness")), ("consistency", report.get("consistency")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match readiness/consistency")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("source", report.get("source")), ("readiness", report.get("readiness")), ("consistency", report.get("consistency")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match readiness/consistency")
        ...
        status_digest = record.get("status_digest")
        if not _digest(status_digest):
            errors.append(f"{prefix}.status_digest must be lowercase SHA-256")
        elif status_digest != _status_digest(record):
            errors.append(f"{prefix}.status_digest must bind readiness/consistency")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match status records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "unique": len(ids), "ready": ready, "mutations": mutations}
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match status records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_READINESS_CONSISTENCY_V65_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_READINESS_CONSISTENCY_V65_VALID")
    return 0
