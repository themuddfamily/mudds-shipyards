#!/usr/bin/env python3
"""Validate detached v248 network snapshot authority takeover citation evidence."""
...
SCHEMA_VERSION = 248
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_citation_v248"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
CITATION_ID = "server-authority-citation-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v19"
SNAPSHOT_ID = "snapshot-authority-v131"
...
def _citation_digest(record: dict[str, Any]) -> str:
    fields = (
        "citation_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('citation_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover citation evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("citation_id", report.get("citation_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority citation")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority citation")
        ...
        citation_digest = record.get("citation_digest")
        if not _digest(citation_digest):
            errors.append(f"{prefix}.citation_digest must be lowercase SHA-256")
        elif citation_digest != _citation_digest(record):
            errors.append(f"{prefix}.citation_digest must bind authority citation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority citation records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority citation records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CITATION_V248_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CITATION_V248_VALID")
    return 0
