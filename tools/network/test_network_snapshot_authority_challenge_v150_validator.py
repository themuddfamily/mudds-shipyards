...
try:
    from .network_snapshot_authority_challenge_v150_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_challenge_v150_validator import validate_snapshot
...
class NetworkSnapshotAuthorityChallengeV150ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_challenge_binding(self):
        report = _report()
        report["records"][0]["authority_challenge"] = "server-authority-challenge-old"
        self.assertTrue(any("bind authority challenge" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v32"
        self.assertTrue(any("bind authority challenge" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority challenge records" in error for error in validate_snapshot(report)))
