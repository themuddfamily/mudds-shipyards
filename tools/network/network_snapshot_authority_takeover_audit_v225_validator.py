#!/usr/bin/env python3
"""Validate detached v225 network snapshot authority-takeover audit evidence."""
...
SCHEMA_VERSION = 225
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_audit_v225"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_AUDIT = "server-authority-takeover-audit-v1"
SNAPSHOT_ID = "snapshot-authority-v108"
...
def _audit_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_audit", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_audit": AUTHORITY_TAKEOVER_AUDIT,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_audit", report.get("authority_takeover_audit")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover audit")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover audit")
        ...
        if not _digest(audit_digest):
            ...
            errors.append(f"{prefix}.audit_digest must bind authority takeover audit")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover audit records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover audit records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_AUDIT_V225_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_AUDIT_V225_VALID")
    return 0
