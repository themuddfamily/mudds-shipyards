#!/usr/bin/env python3
"""Validate detached v273 network snapshot authority-takeover receipt evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v156"
...
def _rollup_digest(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('entry_id')}|{record.get('receipt_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not isinstance(records, list):
        errors.append(f"{label}.receipts must be an array")
        records = []
    ids: set[str] = set()
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority receipt records")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "receipts": len(records), "unique": len(ids),
            "acknowledged": acknowledged, "mutations": mutations,
        }
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RECEIPT_V273_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RECEIPT_V273_VALID")
    return 0
