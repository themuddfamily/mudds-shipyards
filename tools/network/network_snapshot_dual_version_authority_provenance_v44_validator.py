#!/usr/bin/env python3
"""Validate v44 dual-version snapshot authority/provenance evidence."""
...
def _binding_digest(record: dict[str, Any]) -> str:
    return hashlib.sha256(f"{record.get('authority')}|{record.get('authority_version')}|{record.get('provenance_version')}|{record.get('record_id')}|{record.get('sequence')}|{record.get('observed_digest')}".encode("utf-8")).hexdigest()


def _aggregate_digest(records: list[dict[str, Any]]) -> str:
    return hashlib.sha256("\n".join(f"{record.get('order')}|{record.get('record_id')}|{record.get('binding_digest')}" for record in records).encode("utf-8")).hexdigest()


def validate_dual_version(report: Any, label: str = "dual_version") -> list[str]:
    """Return dual-version record bindings, aggregate, count, and mutation errors."""
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind both versions")
    ...
    elif aggregate_digest != _aggregate_digest(records):
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match dual-version records")
        if counts.get("mutations") != 0:
            errors.append(f"{label}.counts.mutations must be zero")
    return errors


def validate_dual_version_file(report_path: Path) -> list[str]:
    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        return [f"unable to read {report_path}: {exc}"]
    return validate_dual_version(report, str(report_path))


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("dual_version", type=Path)
    args = parser.parse_args()
    errors = validate_dual_version_file(args.dual_version)
    if errors:
        print("NETWORK_SNAPSHOT_DUAL_VERSION_AUTHORITY_PROVENANCE_V44_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_DUAL_VERSION_AUTHORITY_PROVENANCE_V44_VALID")
    return 0
