#!/usr/bin/env python3
"""Validate detached v232 network snapshot authority takeover receipt evidence."""
...
SCHEMA_VERSION = 232
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_receipt_v232"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
RECEIPT_ID = "server-authority-receipt-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v3"
SNAPSHOT_ID = "snapshot-authority-v115"
...
def _receipt_digest(record: dict[str, Any]) -> str:
    fields = (
        "receipt_id", "authority_takeover_token", "snapshot_id", "authority",
        "source", "release", "version", "check_id", "sequence",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('receipt_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover receipt evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority receipt")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority receipt")
        ...
        if not _digest(receipt_digest):
            errors.append(f"{prefix}.receipt_digest must be lowercase SHA-256")
        elif receipt_digest != _receipt_digest(record):
            errors.append(f"{prefix}.receipt_digest must bind authority receipt")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority receipt records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority receipt records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RECEIPT_V232_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_RECEIPT_V232_VALID")
    return 0
