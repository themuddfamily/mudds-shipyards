#!/usr/bin/env python3
"""Validate detached v375 authority/snapshot integrity-gate evidence."""
...
AUTHORITY = "server"
INTEGRITY_GATE_ID = "authority-integrity-gate-v375"
SNAPSHOT_ID = "snapshot-authority-v258"
ASSERTION_ID = "authority-snapshot-integrity-gate-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 147
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot integrity-gate evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority integrity gate")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority integrity gate")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority integrity gate")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority integrity gate members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority integrity gate members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_INTEGRITY_GATE_V375_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_INTEGRITY_GATE_V375_VALID")
    return 0
