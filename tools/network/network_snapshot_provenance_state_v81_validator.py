#!/usr/bin/env python3
"""Validate detached v81 snapshot provenance/state evidence."""
...
SCHEMA_VERSION = 81
EVIDENCE_SCOPE = "network_snapshot_provenance_state_v81"
...
def _provenance_digest(record: dict[str, Any]) -> str:
    material = "|".join(str(record.get(key)) for key in (
        "authority", "provenance", "state", "source", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"
    ))
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for provenance/state snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "provenance": PROVENANCE, "state": STATE,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (("authority", AUTHORITY), ("provenance", report.get("provenance")), ("state", report.get("state")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match provenance/state")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (("authority", AUTHORITY), ("provenance", report.get("provenance")), ("state", report.get("state")), ("source", report.get("source")), ("release", report.get("release")), ("version", report.get("snapshot_version"))):
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match provenance/state")
        ...
        if not _digest(provenance_digest):
            ...
            errors.append(f"{prefix}.provenance_digest must bind provenance/state")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_PROVENANCE_STATE_V81_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_PROVENANCE_STATE_V81_VALID")
    return 0
