...
try:
    from .network_snapshot_authority_admission_v200_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_admission_v200_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["admission_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['admission_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_admission": "server-authority-admission-v1",
            "snapshot_id": "snapshot-authority-v83", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3005, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityAdmissionV200ValidatorTest(unittest.TestCase):
    def test_accepts_admitted_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_admission_binding(self):
        report = _report()
        report["records"][0]["authority_admission"] = "server-authority-admission-old"
        self.assertTrue(any("bind authority admission" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v82"
        self.assertTrue(any("bind authority admission" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority admission records" in error for error in validate_snapshot(report)))
