#!/usr/bin/env python3
"""Validate detached v331 authority/snapshot capsule evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v331"
SNAPSHOT_ID = "snapshot-authority-v214"
CAPSULE_ID = "authority-snapshot-capsule-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 103
...
def _capsule_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('capsule_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot capsule evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority capsule")
    ...
    items = report.get("capsule_members")
    if not isinstance(items, list):
        errors.append(f"{label}.capsule_members must be an array")
        items = []
    ids: set[str] = set()
    enclosed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.capsule_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority capsule")
        ...
        if not _digest(capsule_digest):
            errors.append(f"{prefix}.capsule_digest must be lowercase SHA-256")
        elif capsule_digest != _capsule_digest(item):
            errors.append(f"{prefix}.capsule_digest must bind authority capsule")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority capsule members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "capsule_members": len(items), "unique": len(ids), "enclosed": enclosed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority capsule members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CAPSULE_V331_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CAPSULE_V331_VALID")
    return 0
