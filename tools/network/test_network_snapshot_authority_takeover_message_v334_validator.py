...
try:
    from .network_snapshot_authority_takeover_message_v334_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_message_v334_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "message_id": "authority-snapshot-message-v1", "takeover_id": "authority-takeover-v334",
        "snapshot_id": "snapshot-authority-v217", "authority": "server", "sequence": 42384,
        "subject": subject, "authority_digest": digest, "snapshot_digest": digest,
        "acknowledged": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "message_id", "takeover_id", "snapshot_id", "sequence", "subject",
        "authority_digest", "snapshot_digest",
    )
    item["message_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['message_digest']}" for item in items
    )
    return {
        "schema_version": 334,
        "evidence_scope": "network_snapshot_authority_takeover_message_v334",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "takeover_id": "authority-takeover-v334",
        "snapshot_id": "snapshot-authority-v217", "message_id": "authority-snapshot-message-v1",
        "source": "server_snapshot", "snapshot_version": 106, "release": "release-1",
        ...
        "snapshot": {
            "takeover_id": "authority-takeover-v334", "snapshot_id": "snapshot-authority-v217",
            "message_id": "authority-snapshot-message-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 106,
            "sequence": 42384, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "message_members": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"message_members": 2, "unique": 2, "acknowledged": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverMessageV334ValidatorTest(unittest.TestCase):
    def test_accepts_message_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_message_binding(self):
        report = _report()
        report["message_members"][0]["message_id"] = "wrong-message"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority message" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["message_members"][1]["order"] = 1
        report["message_members"][1]["item_id"] = report["message_members"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_digest_and_counts(self):
        report = _report()
        report["message_members"][0]["snapshot_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["counts"]["acknowledged"] = 9
        errors = validate_snapshot(report)
        self.assertTrue(any("snapshot_digest must match authority digest" in error for error in errors))
        self.assertTrue(any("counts.acknowledged" in error for error in errors))

    def test_rejects_rollup_and_mutation(self):
        report = _report()
        report["rollup_digest"] = hashlib.sha256(b"wrong").hexdigest()
        report["message_members"][0]["mutation_fields"] = ["authority"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority message members" in error for error in errors))
