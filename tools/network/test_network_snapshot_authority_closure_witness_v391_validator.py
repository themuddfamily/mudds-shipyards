...
try:
    from .network_snapshot_authority_closure_witness_v391_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_closure_witness_v391_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-closure-witness-assertion-v1",
        "integrity_gate_id": "authority-closure-witness-v391",
        "snapshot_id": "snapshot-authority-v274", "authority": "server", "sequence": 43991,
...
def _report():
    ...
    return {
        "schema_version": 391,
        "evidence_scope": "network_snapshot_authority_closure_witness_v391",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-closure-witness-v391",
        "snapshot_id": "snapshot-authority-v274",
        "assertion_id": "authority-snapshot-closure-witness-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 163, "release": "release-1",
...
class NetworkSnapshotAuthorityClosureWitnessV391ValidatorTest(unittest.TestCase):
    def test_accepts_closure_witness_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_closure_witness_binding(self):
        report = _report()
        report["assertion_members"][0]["integrity_gate_id"] = "wrong-closure"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority closure witness" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["closure"]
        report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority closure members" in error for error in errors))
