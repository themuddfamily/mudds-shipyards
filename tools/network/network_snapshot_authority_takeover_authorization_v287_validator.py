#!/usr/bin/env python3
"""Validate detached v287 network snapshot authority-takeover authorization evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v170"
...
def _authorization_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('authorization_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    items = report.get("authorizations")
    if not isinstance(items, list):
        errors.append(f"{label}.authorizations must be an array")
        items = []
    ids: set[str] = set()
    approved = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.authorizations[{index}]"
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority authorizations")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "authorizations": len(items), "unique": len(ids), "approved": approved, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority authorizations")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_AUTHORIZATION_V287_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_AUTHORIZATION_V287_VALID")
    return 0
