#!/usr/bin/env python3
"""Validate detached v186 network snapshot authority-assertion evidence."""
...
SCHEMA_VERSION = 186
EVIDENCE_SCOPE = "network_snapshot_authority_assertion_v186"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ASSERTION = "server-authority-assertion-v1"
SNAPSHOT_ID = "snapshot-authority-v69"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_assertion", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-assertion snapshot evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_assertion", report.get("authority_assertion")),
    ...
    for index, record in enumerate(records):
        ...
        if not _digest(binding_digest):
            ...
            errors.append(f"{prefix}.binding_digest must bind authority assertion")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ASSERTION_V186_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ASSERTION_V186_VALID")
    return 0
