...
try:
    from .network_snapshot_authority_takeover_journal_v252_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_journal_v252_validator import validate_snapshot
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['journal_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "journal_id": "server-authority-journal-v1",
            "authority_takeover_token": "server-authority-takeover-token-v23",
            "snapshot_id": "snapshot-authority-v135", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 24,
            "sequence": 25147, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverJournalV252ValidatorTest(unittest.TestCase):
    def test_accepts_authority_journal(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_journal_binding(self):
        report = _report()
        report["records"][0]["journal_id"] = "server-authority-journal-old"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority journal" in error for error in errors))
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority journal records" in error for error in errors))
