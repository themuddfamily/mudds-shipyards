...
try:
    from .network_snapshot_authority_takeover_registry_v295_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_registry_v295_validator import validate_snapshot
...
def _item(order, item_id, digest, name):
    item = {
        ...
        "registry_id": "server-authority-registry-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v66",
        "snapshot_id": "snapshot-authority-v178", "sequence": 42190,
        "name": name, "expected_digest": digest, "observed_digest": digest,
        "registered": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "registry_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "name", "item_id", "expected_digest", "observed_digest",
    )
    item["registry_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['registry_digest']}" for item in items
    )
    return {
        "schema_version": 295,
        "evidence_scope": "network_snapshot_authority_takeover_registry_v295",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "registry_id": "server-authority-registry-v1",
        "takeover_token": "server-authority-takeover-token-v66",
        "snapshot_id": "snapshot-authority-v178", "source": "server_snapshot",
        "snapshot_version": 67, "release": "release-1",
        ...
        "snapshot": {
            "registry_id": "server-authority-registry-v1",
            "takeover_token": "server-authority-takeover-token-v66",
            "snapshot_id": "snapshot-authority-v178", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 67,
            "sequence": 42190, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "registry_entries": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"entries": 2, "unique": 2, "registered": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverRegistryV295ValidatorTest(unittest.TestCase):
    def test_accepts_registry_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_registry_binding(self):
        report = _report()
        report["registry_entries"][0]["takeover_token"] = "wrong-token"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority registry" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["registry_entries"][1]["order"] = 1
        report["registry_entries"][1]["item_id"] = report["registry_entries"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_rollup_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority registry entries" in error for error in errors))
        self.assertTrue(any("counts.entries" in error for error in errors))

    def test_rejects_mutation(self):
        report = _report()
        report["registry_entries"][0]["mutation_fields"] = ["authority"]
