#!/usr/bin/env python3
"""Validate v33 versioned snapshot root authority digest evidence."""
...
def _authority_digest(root: dict[str, Any]) -> str:
    ...
    payload = f"{AUTHORITY}|{root.get('version')}|{root.get('pair_id')}|{left.get('sequence')}|{left.get('digest')}|{right.get('sequence')}|{right.get('digest')}"
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def validate_versioned(report: Any, label: str = "versioned") -> list[str]:
    """Return version, root digest, member reference, count, and mutation errors."""
    ...
    if not _version(root.get("version")):
        errors.append(f"{label}.root.version must be positive")
    if not isinstance(root.get("pair_id"), str) or not root.get("pair_id"):
        errors.append(f"{label}.root.pair_id must be non-empty")
    left = root.get("left")
    right = root.get("right")
    for name, state in (("left", left), ("right", right)):
        if not _state(state):
            errors.append(f"{label}.root.{name} must contain sequence and lowercase SHA-256 digest")
    if _state(left) and _state(right) and left != right:
        errors.append(f"{label}.root.right must equal left")
    ...
    if not _digest(authority_digest):
        ...
        errors.append(f"{label}.root.authority_digest must bind versioned root")
    ...
    for index, member in enumerate(members):
        ...
        if member.get("version") != root.get("version"):
            errors.append(f"{prefix}.version must match root")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match versioned members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_VERSIONED_ROOT_AUTHORITY_DIGEST_V33_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_VERSIONED_ROOT_AUTHORITY_DIGEST_V33_VALID")
    return 0
