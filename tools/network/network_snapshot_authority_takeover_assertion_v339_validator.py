#!/usr/bin/env python3
"""Validate detached v339 authority/snapshot assertion evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v339"
SNAPSHOT_ID = "snapshot-authority-v222"
ASSERTION_ID = "authority-snapshot-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 111
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot assertion evidence."""
    errors: list[str] = []
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority assertion")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority assertion members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority assertion members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ASSERTION_V339_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ASSERTION_V339_VALID")
    return 0
