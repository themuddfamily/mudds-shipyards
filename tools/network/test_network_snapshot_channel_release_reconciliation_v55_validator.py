...
try:
    from .network_snapshot_channel_release_reconciliation_v55_validator import validate_checks
except ImportError:
    from network_snapshot_channel_release_reconciliation_v55_validator import validate_checks
...
def _check(order, check_id, digest):
    check = {
        "order": order, "check_id": check_id, "authority": "server", "channel": "replication", "release": "release-1",
        "version": 2, "sequence": 420, "expected_digest": digest, "observed_digest": digest,
        "reconciled": True, "mutation_fields": [], "state_changed": False,
    }
    material = "|".join(str(check[key]) for key in ("authority", "channel", "release", "version", "check_id", "sequence", "expected_digest", "observed_digest"))
...
def _report():
    ...
    return {
        "schema_version": 55, "evidence_scope": "network_snapshot_channel_release_reconciliation_v55",
        "evidence_mode": "detached_contract_fixture", "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "channel": "replication", "snapshot_version": 2, "release": "release-1",
        "native_claims": False, "uses_live_network": False, "snapshot_detached": True, "no_mutation_guarantee": True,
        "snapshot": {"authority": "server", "channel": "replication", "release": "release-1", "version": 2, "sequence": 420, "digest": hashlib.sha256(b"snapshot").hexdigest()},
        "checks": checks,
...
class NetworkSnapshotChannelReleaseReconciliationV55ValidatorTest(unittest.TestCase):
    def test_accepts_channel_release_reconciliations(self):
        self.assertEqual(validate_checks(_report()), [])

    def test_rejects_channel_binding(self):
        report = _report()
        report["checks"][0]["channel"] = "presence"
        self.assertTrue(any("bind channel/release" in error for error in validate_checks(report)))
    ...
    def test_rejects_aggregate_digest(self):
        ...
        self.assertTrue(any("match channel reconciliations" in error for error in validate_checks(report)))
