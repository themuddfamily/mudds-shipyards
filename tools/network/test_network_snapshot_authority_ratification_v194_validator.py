...
try:
    from .network_snapshot_authority_ratification_v194_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_ratification_v194_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["ratification_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['ratification_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_ratification": "server-authority-ratification-v1",
            "snapshot_id": "snapshot-authority-v77", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 2981, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityRatificationV194ValidatorTest(unittest.TestCase):
    def test_accepts_ratified_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_ratification_binding(self):
        report = _report()
        report["records"][0]["authority_ratification"] = "server-authority-ratification-old"
        self.assertTrue(any("bind authority ratification" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v76"
        self.assertTrue(any("bind authority ratification" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority ratification records" in error for error in validate_snapshot(report)))
