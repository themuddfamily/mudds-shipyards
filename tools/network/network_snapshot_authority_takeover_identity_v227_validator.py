#!/usr/bin/env python3
"""Validate detached v227 network snapshot authority-takeover identity evidence."""
...
SCHEMA_VERSION = 227
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_identity_v227"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_IDENTITY = "server-authority-takeover-identity-v1"
SNAPSHOT_ID = "snapshot-authority-v110"
...
def _identity_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_identity", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('identity_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover identity evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_identity": AUTHORITY_TAKEOVER_IDENTITY,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_identity", report.get("authority_takeover_identity")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover identity")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover identity")
        ...
        elif identity_digest != _identity_digest(record):
            errors.append(f"{prefix}.identity_digest must bind authority takeover identity")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover identity records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover identity records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_IDENTITY_V227_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_IDENTITY_V227_VALID")
    return 0
