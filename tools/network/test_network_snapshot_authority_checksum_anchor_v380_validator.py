...
try:
    from .network_snapshot_authority_checksum_anchor_v380_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_checksum_anchor_v380_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-checksum-anchor-assertion-v1",
        "integrity_gate_id": "authority-checksum-anchor-v380",
        "snapshot_id": "snapshot-authority-v263", "authority": "server", "sequence": 42880,
...
def _report():
    ...
    return {
        "schema_version": 380,
        "evidence_scope": "network_snapshot_authority_checksum_anchor_v380",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-checksum-anchor-v380",
        "snapshot_id": "snapshot-authority-v263",
        "assertion_id": "authority-snapshot-checksum-anchor-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 152, "release": "release-1",
...
class NetworkSnapshotAuthorityChecksumAnchorV380ValidatorTest(unittest.TestCase):
    def test_accepts_checksum_anchor_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_checksum_anchor_binding(self):
        report = _report()
        report["assertion_members"][0]["authority"] = "client"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority checksum anchor" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority checksum members" in error for error in errors))
