#!/usr/bin/env python3
"""Validate detached v310 authority/snapshot seal evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v310"
SNAPSHOT_ID = "snapshot-authority-v193"
SEAL_ID = "authority-snapshot-seal-v1"
...
def _seal_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('seal_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot seal evidence."""
    errors: list[str] = []
    ...
    items = report.get("seals")
    if not isinstance(items, list):
        errors.append(f"{label}.seals must be an array")
        items = []
    ids: set[str] = set()
    sealed = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.seals[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority seal")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority seals")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "seals": len(items), "unique": len(ids), "sealed": sealed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority seals")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SEAL_V310_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_SEAL_V310_VALID")
    return 0
