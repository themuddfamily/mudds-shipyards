#!/usr/bin/env python3
"""Validate detached v104 snapshot evidence/state evidence."""
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached evidence/state snapshot evidence."""
    errors: list[str] = []
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_EVIDENCE_STATE_V104_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_EVIDENCE_STATE_V104_VALID")
    return 0
