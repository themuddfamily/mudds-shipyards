#!/usr/bin/env python3
"""Validate detached v221 network snapshot authority-takeover proof evidence."""
...
SCHEMA_VERSION = 221
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_proof_v221"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_PROOF = "server-authority-takeover-proof-v1"
SNAPSHOT_ID = "snapshot-authority-v104"
...
def _proof_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_proof", "snapshot_id", "authority", "source", "release",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_proof": AUTHORITY_TAKEOVER_PROOF,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_proof", report.get("authority_takeover_proof")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover proof")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover proof")
        ...
        if not _digest(proof_digest):
            ...
            errors.append(f"{prefix}.proof_digest must bind authority takeover proof")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover proof records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover proof records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PROOF_V221_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PROOF_V221_VALID")
    return 0
