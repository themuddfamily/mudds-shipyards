#!/usr/bin/env python3
"""Validate detached v343 authority/snapshot lineage evidence."""
...
AUTHORITY = "server"
LINEAGE_ID = "authority-lineage-v343"
SNAPSHOT_ID = "snapshot-authority-v226"
ASSERTION_ID = "authority-snapshot-lineage-assertion-v1"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot lineage evidence."""
    errors: list[str] = []
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority lineage")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority lineage")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority lineage members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority lineage members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_LINEAGE_V343_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_LINEAGE_V343_VALID")
    return 0
