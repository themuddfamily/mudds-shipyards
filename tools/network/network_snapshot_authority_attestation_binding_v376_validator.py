#!/usr/bin/env python3
"""Validate detached v376 authority/snapshot attestation-binding evidence."""
...
SCHEMA_VERSION = 376
EVIDENCE_SCOPE = "network_snapshot_authority_attestation_binding_v376"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
INTEGRITY_GATE_ID = "authority-attestation-binding-v376"
SNAPSHOT_ID = "snapshot-authority-v259"
ASSERTION_ID = "authority-snapshot-attestation-binding-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 148
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority attestation binding")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ATTESTATION_BINDING_V376_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ATTESTATION_BINDING_V376_VALID")
    return 0
