#!/usr/bin/env python3
"""Validate detached v314 authority/snapshot bundle evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v314"
SNAPSHOT_ID = "snapshot-authority-v197"
BUNDLE_ID = "authority-snapshot-bundle-v1"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot bundle evidence."""
    errors: list[str] = []
    ...
    items = report.get("bundle_entries")
    if not isinstance(items, list):
        errors.append(f"{label}.bundle_entries must be an array")
        items = []
    ids: set[str] = set()
    bundled = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.bundle_entries[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority bundle")
        ...
        if item.get("bundled") is not True:
            errors.append(f"{prefix}.bundled must be true")
        else:
            bundled += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority bundle entries")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "bundle_entries": len(items), "unique": len(ids), "bundled": bundled, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority bundle entries")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_BUNDLE_V314_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_BUNDLE_V314_VALID")
    return 0
