...
try:
    from .network_snapshot_paired_root_authority_digest_v31_validator import validate_digest
...
def _root() -> dict:
    ...
    root = {"authority": "server", "pair_id": "root-pair", "left": {"sequence": 270, "digest": digest}, "right": {"sequence": 270, "digest": digest}}
    root["authority_digest"] = hashlib.sha256(f"server|root-pair|270|{digest}|270|{digest}".encode()).hexdigest()
    return root
...
class NetworkSnapshotPairedRootAuthorityDigestV31ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_authority_digest(self):
        ...
        self.assertTrue(any("bind paired root" in error for error in validate_digest(report)))

    def test_rejects_right_root_state(self):
        report = _report()
        report["root_pair"]["right"]["sequence"] = 271
        self.assertTrue(any("right must equal left" in error for error in validate_digest(report)))
