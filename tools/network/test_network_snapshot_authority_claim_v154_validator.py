...
try:
    from .network_snapshot_authority_claim_v154_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_claim_v154_validator import validate_snapshot
...
class NetworkSnapshotAuthorityClaimV154ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_claim_binding(self):
        report = _report()
        report["records"][0]["authority_claim"] = "server-authority-claim-old"
        self.assertTrue(any("bind authority claim" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v36"
        self.assertTrue(any("bind authority claim" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority claim records" in error for error in validate_snapshot(report)))
