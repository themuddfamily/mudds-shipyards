#!/usr/bin/env python3
"""Validate detached v327 authority/snapshot offset evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v327"
SNAPSHOT_ID = "snapshot-authority-v210"
OFFSET_ID = "authority-snapshot-offset-v1"
...
def _offset_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('offset_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot offset evidence."""
    errors: list[str] = []
    ...
    snapshot_fields = (
        ...
        ("offset_id", report.get("offset_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority offset")
    ...
    items = report.get("offset_members")
    if not isinstance(items, list):
        errors.append(f"{label}.offset_members must be an array")
        items = []
    ids: set[str] = set()
    advanced = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.offset_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority offset")
        ...
        offset_digest = item.get("offset_digest")
        if not _digest(offset_digest):
            errors.append(f"{prefix}.offset_digest must be lowercase SHA-256")
        elif offset_digest != _offset_digest(item):
            errors.append(f"{prefix}.offset_digest must bind authority offset")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority offset members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "offset_members": len(items), "unique": len(ids), "advanced": advanced, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority offset members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_OFFSET_V327_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_OFFSET_V327_VALID")
    return 0
