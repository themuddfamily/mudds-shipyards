...
try:
    from .network_snapshot_authority_takeover_directory_v298_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_directory_v298_validator import validate_snapshot


def _item(order, item_id, digest, path):
    item = {
        "order": order, "item_id": item_id,
        "directory_id": "server-authority-directory-v1", "authority": "server",
        "takeover_token": "server-authority-takeover-token-v69",
        "snapshot_id": "snapshot-authority-v181", "sequence": 42193,
        "path": path, "expected_digest": digest, "observed_digest": digest,
        "located": True, "mutation_fields": [], "state_changed": False,
    }
    fields = (
        "directory_id", "authority", "takeover_token", "snapshot_id", "sequence",
        "path", "item_id", "expected_digest", "observed_digest",
    )
    item["directory_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{item['order']}|{item['item_id']}|{item['directory_digest']}" for item in items
    )
    return {
        "schema_version": 298,
        ...
        "authority": "server", "directory_id": "server-authority-directory-v1",
        "takeover_token": "server-authority-takeover-token-v69",
        "snapshot_id": "snapshot-authority-v181", "source": "server_snapshot",
        "snapshot_version": 70, "release": "release-1",
        ...
        "snapshot": {
            ...
            "takeover_token": "server-authority-takeover-token-v69",
            "snapshot_id": "snapshot-authority-v181", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 70,
            "sequence": 42193, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "directory_entries": items,
        "rollup_digest": hashlib.sha256(material.encode()).hexdigest(),
        "counts": {"entries": 2, "unique": 2, "located": 2, "mutations": 0},
    }


class NetworkSnapshotAuthorityTakeoverDirectoryV298ValidatorTest(unittest.TestCase):
    def test_accepts_directory_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_directory_binding(self):
        report = _report()
        report["directory_entries"][0]["takeover_token"] = "wrong-token"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority directory" in error for error in errors))

    def test_rejects_order_and_duplicate_identity(self):
        report = _report()
        report["directory_entries"][1]["order"] = 1
        report["directory_entries"][1]["item_id"] = report["directory_entries"][0]["item_id"]
        errors = validate_snapshot(report)
    ...
    def test_rejects_rollup_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority directory entries" in error for error in errors))
        self.assertTrue(any("counts.entries" in error for error in errors))

    def test_rejects_mutation(self):
        report = _report()
        report["directory_entries"][0]["mutation_fields"] = ["authority"]
