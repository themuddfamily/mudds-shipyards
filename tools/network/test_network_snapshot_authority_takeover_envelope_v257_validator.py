...
try:
    from .network_snapshot_authority_takeover_envelope_v257_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_takeover_envelope_v257_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["envelope_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['envelope_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "envelope_id": "server-authority-envelope-v1",
            "authority_takeover_token": "server-authority-takeover-token-v28",
            "snapshot_id": "snapshot-authority-v140", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 29,
            "sequence": 30152, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityTakeoverEnvelopeV257ValidatorTest(unittest.TestCase):
    def test_accepts_authority_envelope(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_envelope_binding(self):
        report = _report()
        report["records"][0]["envelope_id"] = "server-authority-envelope-old"
    ...
    def test_rejects_aggregate_and_counts(self):
        ...
        errors = validate_snapshot(report)
        self.assertTrue(any("match authority envelope records" in error for error in errors))
