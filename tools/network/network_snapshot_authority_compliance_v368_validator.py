#!/usr/bin/env python3
"""Validate detached v368 authority/snapshot compliance evidence."""
...
AUTHORITY = "server"
COMPLIANCE_ID = "authority-compliance-v368"
SNAPSHOT_ID = "snapshot-authority-v251"
ASSERTION_ID = "authority-snapshot-compliance-assertion-v1"
SOURCE = "server_snapshot"
SNAPSHOT_VERSION = 140
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot compliance evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("compliance_id", report.get("compliance_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority compliance")
    ...
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority compliance")
        ...
        if not _digest(assertion_digest):
            ...
            errors.append(f"{prefix}.assertion_digest must bind authority compliance")
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority compliance members")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority compliance members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_COMPLIANCE_V368_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_COMPLIANCE_V368_VALID")
    return 0
