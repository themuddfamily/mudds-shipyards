#!/usr/bin/env python3
"""Validate detached v256 network snapshot authority takeover frame evidence."""
...
SCHEMA_VERSION = 256
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_frame_v256"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
FRAME_ID = "server-authority-frame-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v27"
SNAPSHOT_ID = "snapshot-authority-v139"
...
def _frame_digest(record: dict[str, Any]) -> str:
    fields = (
        "frame_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('frame_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority frame records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority frame records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_FRAME_V256_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_FRAME_V256_VALID")
    return 0
