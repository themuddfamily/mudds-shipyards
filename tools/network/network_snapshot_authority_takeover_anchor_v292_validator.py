#!/usr/bin/env python3
"""Validate detached v292 network snapshot authority-takeover anchor evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v175"
...
def _anchor_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('anchor_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover anchor evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority anchor")
    ...
    items = report.get("anchors")
    if not isinstance(items, list):
        errors.append(f"{label}.anchors must be an array")
        items = []
    ids: set[str] = set()
    fixed = mutations = 0
    for index, item in enumerate(items):
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority anchor")
        if item.get("sequence") != snapshot.get("sequence"):
            errors.append(f"{prefix}.sequence must match snapshot")
        if not isinstance(item.get("position"), str) or not item["position"]:
            errors.append(f"{prefix}.position must be non-empty")
        ...
        if not _digest(anchor_digest):
            errors.append(f"{prefix}.anchor_digest must be lowercase SHA-256")
        elif anchor_digest != _anchor_digest(item):
            errors.append(f"{prefix}.anchor_digest must bind authority anchor")
        if item.get("fixed") is not True:
            errors.append(f"{prefix}.fixed must be true")
        else:
            fixed += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority anchors")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "anchors": len(items), "unique": len(ids), "fixed": fixed, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority anchors")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ANCHOR_V292_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_ANCHOR_V292_VALID")
    return 0
