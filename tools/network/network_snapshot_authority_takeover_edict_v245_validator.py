#!/usr/bin/env python3
"""Validate detached v245 network snapshot authority takeover edict evidence."""
...
SCHEMA_VERSION = 245
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_edict_v245"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
EDICT_ID = "server-authority-edict-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v16"
SNAPSHOT_ID = "snapshot-authority-v128"
...
def _edict_digest(record: dict[str, Any]) -> str:
    fields = (
        "edict_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('edict_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover edict evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority edict")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority edict")
        ...
        edict_digest = record.get("edict_digest")
        if not _digest(edict_digest):
            errors.append(f"{prefix}.edict_digest must be lowercase SHA-256")
        elif edict_digest != _edict_digest(record):
            errors.append(f"{prefix}.edict_digest must bind authority edict")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority edict records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority edict records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_EDICT_V245_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_EDICT_V245_VALID")
    return 0
