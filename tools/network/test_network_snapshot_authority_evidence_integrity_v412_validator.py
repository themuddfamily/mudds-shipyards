...
try:
    from .network_snapshot_authority_evidence_integrity_v412_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_evidence_integrity_v412_validator import validate_snapshot
...
def _item(order, item_id, digest, subject):
    item = {
        ...
        "assertion_id": "authority-snapshot-evidence-integrity-assertion-v1",
        "integrity_gate_id": "authority-evidence-integrity-v412",
        "snapshot_id": "snapshot-authority-v295", "authority": "server", "sequence": 46312,
...
def _report():
    items = [_item(1, "authority", hashlib.sha256(b"authority").hexdigest(), "authority"),
             _item(2, "integrity", hashlib.sha256(b"integrity").hexdigest(), "integrity")]
    material = "\n".join(f"{item['order']}|{item['item_id']}|{item['assertion_digest']}" for item in items)
    return {
        "schema_version": 412,
        "evidence_scope": "network_snapshot_authority_evidence_integrity_v412",
        "evidence_mode": "detached_contract_fixture",
        "policy_version": "network_replication_interest_authority_v1",
        "authority": "server", "integrity_gate_id": "authority-evidence-integrity-v412",
        "snapshot_id": "snapshot-authority-v295",
        "assertion_id": "authority-snapshot-evidence-integrity-assertion-v1",
        "source": "server_snapshot", "snapshot_version": 184, "release": "release-1",
        ...
        "snapshot": {
            "integrity_gate_id": "authority-evidence-integrity-v412", "snapshot_id": "snapshot-authority-v295",
            "assertion_id": "authority-snapshot-evidence-integrity-assertion-v1", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 184, "sequence": 46312,
...
class NetworkSnapshotAuthorityEvidenceIntegrityV412ValidatorTest(unittest.TestCase):
    def test_accepts_evidence_integrity_rollup(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_evidence_integrity_binding(self):
        report = _report(); report["assertion_members"][0]["snapshot_id"] = "wrong-integrity"
        errors = validate_snapshot(report)
        self.assertTrue(any("bind authority evidence integrity" in error for error in errors))
    ...
    def test_rejects_rollup_and_mutation(self):
        ...
        report["assertion_members"][0]["mutation_fields"] = ["integrity"]; report["counts"]["mutations"] = 1
        errors = validate_snapshot(report)
