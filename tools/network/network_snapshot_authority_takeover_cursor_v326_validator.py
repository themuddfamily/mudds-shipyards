#!/usr/bin/env python3
"""Validate detached v326 authority/snapshot cursor evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v326"
SNAPSHOT_ID = "snapshot-authority-v209"
CURSOR_ID = "authority-snapshot-cursor-v1"
...
def _cursor_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('cursor_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot cursor evidence."""
    errors: list[str] = []
    ...
    snapshot_fields = (
        ...
        ("cursor_id", report.get("cursor_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority cursor")
    ...
    items = report.get("cursor_members")
    if not isinstance(items, list):
        errors.append(f"{label}.cursor_members must be an array")
        items = []
    ids: set[str] = set()
    positioned = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.cursor_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority cursor")
        ...
        cursor_digest = item.get("cursor_digest")
        if not _digest(cursor_digest):
            errors.append(f"{prefix}.cursor_digest must be lowercase SHA-256")
        elif cursor_digest != _cursor_digest(item):
            errors.append(f"{prefix}.cursor_digest must bind authority cursor")
        if item.get("positioned") is not True:
            errors.append(f"{prefix}.positioned must be true")
        else:
            positioned += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority cursor members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "cursor_members": len(items), "unique": len(ids), "positioned": positioned, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority cursor members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CURSOR_V326_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CURSOR_V326_VALID")
    return 0
