#!/usr/bin/env python3
"""Validate detached v299 network snapshot authority-takeover map evidence."""
...
SNAPSHOT_ID = "snapshot-authority-v182"
...
def _map_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('map_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover map evidence."""
    errors: list[str] = []
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority map")
    ...
    items = report.get("map_entries")
    if not isinstance(items, list):
        errors.append(f"{label}.map_entries must be an array")
        items = []
    ids: set[str] = set()
    mapped = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.map_entries[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority map")
        if item.get("sequence") != snapshot.get("sequence"):
            errors.append(f"{prefix}.sequence must match snapshot")
        if not isinstance(item.get("region"), str) or not item["region"]:
            errors.append(f"{prefix}.region must be non-empty")
        ...
        map_digest = item.get("map_digest")
        if not _digest(map_digest):
            errors.append(f"{prefix}.map_digest must be lowercase SHA-256")
        elif map_digest != _map_digest(item):
            errors.append(f"{prefix}.map_digest must bind authority map")
        if item.get("mapped") is not True:
            errors.append(f"{prefix}.mapped must be true")
        else:
            mapped += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority map entries")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "entries": len(items), "unique": len(ids), "mapped": mapped, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority map entries")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MAP_V299_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MAP_V299_VALID")
    return 0
