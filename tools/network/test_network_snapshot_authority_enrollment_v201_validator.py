...
try:
    from .network_snapshot_authority_enrollment_v201_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_enrollment_v201_validator import validate_snapshot
...
def _record(order, check_id, digest):
    ...
    record["enrollment_digest"] = hashlib.sha256(
...
def _report():
    ...
    material = "\n".join(
        f"{record['order']}|{record['check_id']}|{record['enrollment_digest']}"
    ...
    return {
        ...
        "snapshot": {
            "authority_enrollment": "server-authority-enrollment-v1",
            "snapshot_id": "snapshot-authority-v84", "authority": "server",
            "source": "server_snapshot", "release": "release-1", "version": 2,
            "sequence": 3009, "digest": hashlib.sha256(b"snapshot").hexdigest(),
        },
        "records": records,
...
class NetworkSnapshotAuthorityEnrollmentV201ValidatorTest(unittest.TestCase):
    def test_accepts_enrolled_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_enrollment_binding(self):
        report = _report()
        report["records"][0]["authority_enrollment"] = "server-authority-enrollment-old"
        self.assertTrue(any("bind authority enrollment" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v83"
        self.assertTrue(any("bind authority enrollment" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority enrollment records" in error for error in validate_snapshot(report)))
