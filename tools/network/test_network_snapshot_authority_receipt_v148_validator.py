...
try:
    from .network_snapshot_authority_receipt_v148_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_receipt_v148_validator import validate_snapshot
...
class NetworkSnapshotAuthorityReceiptV148ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_receipt_binding(self):
        report = _report()
        report["records"][0]["authority_receipt"] = "server-authority-receipt-old"
        self.assertTrue(any("bind authority receipt" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v30"
        self.assertTrue(any("bind authority receipt" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority receipt records" in error for error in validate_snapshot(report)))
