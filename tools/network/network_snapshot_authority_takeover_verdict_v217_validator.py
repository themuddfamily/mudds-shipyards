#!/usr/bin/env python3
"""Validate detached v217 network snapshot authority-takeover verdict evidence."""
...
SCHEMA_VERSION = 217
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_verdict_v217"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_VERDICT = "server-authority-takeover-verdict-v1"
SNAPSHOT_ID = "snapshot-authority-v100"
...
def _verdict_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_verdict", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('verdict_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover verdict evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_verdict": AUTHORITY_TAKEOVER_VERDICT,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_verdict", report.get("authority_takeover_verdict")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover verdict")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover verdict")
        ...
        verdict_digest = record.get("verdict_digest")
        if not _digest(verdict_digest):
            errors.append(f"{prefix}.verdict_digest must be lowercase SHA-256")
        elif verdict_digest != _verdict_digest(record):
            errors.append(f"{prefix}.verdict_digest must bind authority takeover verdict")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover verdict records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover verdict records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_VERDICT_V217_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_VERDICT_V217_VALID")
    return 0
