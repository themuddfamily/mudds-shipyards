#!/usr/bin/env python3
"""Validate detached v332 authority/snapshot parcel evidence."""
...
AUTHORITY = "server"
TAKEOVER_ID = "authority-takeover-v332"
SNAPSHOT_ID = "snapshot-authority-v215"
PARCEL_ID = "authority-snapshot-parcel-v1"
...
def _parcel_digest(item: dict[str, Any]) -> str:
    fields = (
...
def _rollup_digest(items: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{item.get('order')}|{item.get('item_id')}|{item.get('parcel_digest')}"
        for item in items
    )
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority/snapshot parcel evidence."""
    errors: list[str] = []
    ...
    snapshot_fields = (
        ...
        ("parcel_id", report.get("parcel_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority parcel")
    ...
    items = report.get("parcel_members")
    if not isinstance(items, list):
        errors.append(f"{label}.parcel_members must be an array")
        items = []
    ids: set[str] = set()
    delivered = mutations = 0
    for index, item in enumerate(items):
        prefix = f"{label}.parcel_members[{index}]"
        ...
        for key, value in (
            ...
            if item.get(key) != value:
                errors.append(f"{prefix}.{key} must bind authority parcel")
        ...
        parcel_digest = item.get("parcel_digest")
        if not _digest(parcel_digest):
            errors.append(f"{prefix}.parcel_digest must be lowercase SHA-256")
        elif parcel_digest != _parcel_digest(item):
            errors.append(f"{prefix}.parcel_digest must bind authority parcel")
        if item.get("delivered") is not True:
            errors.append(f"{prefix}.delivered must be true")
        else:
            delivered += 1
    ...
    if not _digest(rollup_digest):
        ...
        errors.append(f"{label}.rollup_digest must match authority parcel members")
    counts = report.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {
            "parcel_members": len(items), "unique": len(ids), "delivered": delivered, "mutations": mutations,
        }
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority parcel members")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PARCEL_V332_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_PARCEL_V332_VALID")
    return 0
