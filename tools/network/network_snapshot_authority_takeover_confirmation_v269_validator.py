#!/usr/bin/env python3
"""Validate detached v269 network snapshot authority takeover confirmation evidence."""
...
SCHEMA_VERSION = 269
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_confirmation_v269"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
CONFIRMATION_ID = "server-authority-confirmation-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v40"
SNAPSHOT_ID = "snapshot-authority-v152"
...
def _confirmation_digest(record: dict[str, Any]) -> str:
    fields = (
        "confirmation_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('confirmation_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover confirmation evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("confirmation_id", report.get("confirmation_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority confirmation")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority confirmation")
        ...
        confirmation_digest = record.get("confirmation_digest")
        if not _digest(confirmation_digest):
            errors.append(f"{prefix}.confirmation_digest must be lowercase SHA-256")
        elif confirmation_digest != _confirmation_digest(record):
            errors.append(f"{prefix}.confirmation_digest must bind authority confirmation")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority confirmation records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority confirmation records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CONFIRMATION_V269_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CONFIRMATION_V269_VALID")
    return 0
