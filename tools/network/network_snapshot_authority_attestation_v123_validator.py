#!/usr/bin/env python3
"""Validate detached v123 network snapshot authority-attestation evidence."""
...
SCHEMA_VERSION = 123
EVIDENCE_SCOPE = "network_snapshot_authority_attestation_v123"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_ATTESTATION = "server-authority-attestation-v2"
SNAPSHOT_ID = "snapshot-authority-v6"
...
def _binding_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_attestation", "snapshot_id", "authority", "source", "release", "version",
        "check_id", "sequence", "expected_digest", "observed_digest",
    )
    material = "|".join(str(record.get(key)) for key in fields)
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-attestation snapshot evidence."""
    errors: list[str] = []
    ...
    expected = {
        ...
        "authority": AUTHORITY, "authority_attestation": AUTHORITY_ATTESTATION,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    for key, value in (
        ("authority_attestation", report.get("authority_attestation")), ("snapshot_id", report.get("snapshot_id")),
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_ATTESTATION_V123_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_ATTESTATION_V123_VALID")
    return 0
