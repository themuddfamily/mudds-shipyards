...
try:
    from .network_snapshot_authority_continuity_v161_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_continuity_v161_validator import validate_snapshot
...
class NetworkSnapshotAuthorityContinuityV161ValidatorTest(unittest.TestCase):
    ...
    def test_rejects_continuity_binding(self):
        report = _report()
        report["records"][0]["authority_continuity"] = "server-authority-continuity-old"
        self.assertTrue(any("bind authority continuity" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v43"
        self.assertTrue(any("bind authority continuity" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority continuity records" in error for error in validate_snapshot(report)))
