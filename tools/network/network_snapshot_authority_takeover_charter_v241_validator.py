#!/usr/bin/env python3
"""Validate detached v241 network snapshot authority takeover charter evidence."""
...
SCHEMA_VERSION = 241
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_charter_v241"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
CHARTER_ID = "server-authority-charter-v1"
AUTHORITY_TAKEOVER_TOKEN = "server-authority-takeover-token-v12"
SNAPSHOT_ID = "snapshot-authority-v124"
...
def _charter_digest(record: dict[str, Any]) -> str:
    fields = (
        "charter_id", "authority_takeover_token", "snapshot_id", "authority", "source",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('charter_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-takeover charter evidence."""
    errors: list[str] = []
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("charter_id", report.get("charter_id")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority charter")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority charter")
        ...
        charter_digest = record.get("charter_digest")
        if not _digest(charter_digest):
            errors.append(f"{prefix}.charter_digest must be lowercase SHA-256")
        elif charter_digest != _charter_digest(record):
            errors.append(f"{prefix}.charter_digest must bind authority charter")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority charter records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority charter records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHARTER_V241_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_CHARTER_V241_VALID")
    return 0
