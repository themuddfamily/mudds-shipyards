...
try:
    from .network_snapshot_authority_token_v169_validator import validate_snapshot
except ImportError:
    from network_snapshot_authority_token_v169_validator import validate_snapshot
...
class NetworkSnapshotAuthorityTokenV169ValidatorTest(unittest.TestCase):
    def test_accepts_authority_token_snapshot_records(self):
        self.assertEqual(validate_snapshot(_report()), [])

    def test_rejects_token_binding(self):
        report = _report()
        report["records"][0]["authority_token"] = "server-authority-token-old"
        self.assertTrue(any("bind authority token" in error for error in validate_snapshot(report)))

    def test_rejects_snapshot_binding(self):
        report = _report()
        report["records"][0]["snapshot_id"] = "snapshot-authority-v51"
        self.assertTrue(any("bind authority token" in error for error in validate_snapshot(report)))

    def test_rejects_aggregate_digest(self):
        report = _report()
        report["aggregate_digest"] = hashlib.sha256(b"wrong").hexdigest()
        self.assertTrue(any("match authority token records" in error for error in validate_snapshot(report)))
