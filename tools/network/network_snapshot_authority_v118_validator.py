#!/usr/bin/env python3
"""Validate detached v118 network snapshot authority evidence."""
...
SCHEMA_VERSION = 118
EVIDENCE_SCOPE = "network_snapshot_authority_v118"
...
def _authority_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority", "source", "release", "version", "check_id", "sequence",
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    """Return errors for detached authority-bound snapshot evidence."""
    errors: list[str] = []
    ...
    for key, value in (
        ...
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in (
            ...
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority")
        ...
        if not _digest(authority_digest):
            ...
            errors.append(f"{prefix}.authority_digest must bind authority/snapshot")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_V118_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_V118_VALID")
    return 0
