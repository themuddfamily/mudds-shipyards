#!/usr/bin/env python3
"""Validate detached v223 network snapshot authority-takeover manifest evidence."""
...
SCHEMA_VERSION = 223
EVIDENCE_SCOPE = "network_snapshot_authority_takeover_manifest_v223"
EVIDENCE_MODE = "detached_contract_fixture"
POLICY_VERSION = "network_replication_interest_authority_v1"
AUTHORITY = "server"
AUTHORITY_TAKEOVER_MANIFEST = "server-authority-takeover-manifest-v1"
SNAPSHOT_ID = "snapshot-authority-v106"
...
def _manifest_digest(record: dict[str, Any]) -> str:
    fields = (
        "authority_takeover_manifest", "snapshot_id", "authority", "source", "release",
...
def _aggregate(records: list[dict[str, Any]]) -> str:
    material = "\n".join(
        f"{record.get('order')}|{record.get('check_id')}|{record.get('manifest_digest')}"
...
def validate_snapshot(report: Any, label: str = "snapshot") -> list[str]:
    ...
    expected = {
        ...
        "authority": AUTHORITY,
        "authority_takeover_manifest": AUTHORITY_TAKEOVER_MANIFEST,
    ...
    if not isinstance(snapshot, dict):
        ...
        snapshot = {}
    snapshot_fields = (
        ("authority_takeover_manifest", report.get("authority_takeover_manifest")),
    ...
    for key, value in snapshot_fields:
        if snapshot.get(key) != value:
            errors.append(f"{label}.snapshot.{key} must match authority takeover manifest")
    ...
    for index, record in enumerate(records):
        ...
        for key, value in snapshot_fields:
            if record.get(key) != value:
                errors.append(f"{prefix}.{key} must match authority takeover manifest")
        ...
        elif manifest_digest != _manifest_digest(record):
            errors.append(f"{prefix}.manifest_digest must bind authority takeover manifest")
    ...
    if not _digest(aggregate_digest):
        ...
        errors.append(f"{label}.aggregate_digest must match authority takeover manifest records")
    ...
    if not isinstance(counts, dict):
        ...
        for key, value in expected_counts.items():
            if counts.get(key) != value:
                errors.append(f"{label}.counts.{key} must match authority takeover manifest records")
...
def main() -> int:
    ...
    if errors:
        print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MANIFEST_V223_INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("NETWORK_SNAPSHOT_AUTHORITY_TAKEOVER_MANIFEST_V223_VALID")
    return 0
