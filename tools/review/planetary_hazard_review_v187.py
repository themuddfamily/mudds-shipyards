#!/usr/bin/env python3
"""Validate the v187 planetary-hazard review evidence boundary."""
...
SCHEMA = "planetary_hazard_review_v187"
VERSION = 187
PHASE = "review"
...
POLICIES = {
    "runtime_write_policy": "forbidden",
    "runtime_read_policy": "evidence_only",
    "runtime_process_policy": "forbidden",
    "runtime_network_policy": "forbidden",
    "runtime_environment_policy": "forbidden",
    "runtime_filesystem_policy": "forbidden",
    "runtime_time_policy": "forbidden",
    "runtime_random_policy": "forbidden",
    "runtime_thread_policy": "forbidden",
    "runtime_signal_policy": "forbidden",
    "runtime_mutex_policy": "forbidden",
    "runtime_ipc_policy": "forbidden",
    "runtime_subprocess_policy": "forbidden",
    "runtime_ui_policy": "forbidden",
    "runtime_audio_policy": "forbidden",
    "runtime_haptic_policy": "forbidden",
    "runtime_display_policy": "forbidden",
    "runtime_input_policy": "forbidden",
    "runtime_storage_policy": "forbidden",
    "runtime_cache_policy": "forbidden",
    "runtime_gpu_policy": "forbidden",
    "runtime_sensor_policy": "forbidden",
}
...
def _open_gate(value: Any, name: str, errors: list[str]) -> None:
    if not isinstance(value, dict):
        errors.append(f"{name} must be an object")
    elif value.get("status") not in OPEN:
        errors.append(f"{name}.status must remain open")


def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return all v187 contract violations; never perform runtime work."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "root_identity_format": ROOT_FORMAT, **POLICIES,
    }
    ...
    for index, receipt in enumerate(receipts):
        ...
        for key, wanted in {
            "lineage_anchor": value.get("root_id"),
            "parent_id": value.get("root_id"),
            "root_identity_value": value.get("root_identity_value"),
            "review_id": value.get("review_id"),
            "consistency_id": value.get("consistency_id"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V187_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V187_VALID_OPEN")
    return 0
