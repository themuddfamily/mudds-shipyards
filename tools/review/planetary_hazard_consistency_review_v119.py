#!/usr/bin/env python3
"""Validate v119 planetary-hazard consistency review evidence."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded consistency-review violations without raising."""
    if not isinstance(value, dict):
    ...
    for index, pair in enumerate(pairs):
        ...
        if kind not in KINDS:
            ...
            kinds.add(kind)
        evidence_id = pair.get("consistency_review_evidence_id")
        if not _text(evidence_id) or evidence_id in evidence_ids:
            errors.append(f"{prefix}.consistency_review_evidence_id must be unique and required")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_CONSISTENCY_REVIEW_V119_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_CONSISTENCY_REVIEW_V119_VALID_OPEN")
    return 0
