#!/usr/bin/env python3
"""Validate v70 planetary hazard visual state/consistency evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_state_consistency_v70"
VERSION = 70
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "state_id", "consistency_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("state_version", "consistency_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 70")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("state_consistency_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.state_consistency_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        for key in ("state_version", "consistency_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 70")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("state_id") != value.get("state_id") or pair.get("consistency_id") != value.get("consistency_id")): errors.append(f"{prefix} state/consistency bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v70")
    ...
    required = {"visual_state_consistency_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_STATE_CONSISTENCY_V70_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_STATE_CONSISTENCY_V70_VALID_OPEN"); return 0
