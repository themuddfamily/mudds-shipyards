...
def manifest() -> dict:
    ...
    return {
        "schema": schema, "schema_version": 146, "review_version": 146,
        ...
        "evidence_channel": channel, "review_owner": owner, "receipts": receipts,
...
class PlanetaryHazardReviewV146Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 145
        self.assertTrue(any("schema_version must be 146" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 145
        self.assertTrue(any("review_version must be 146" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 145
        self.assertTrue(any("review_version must be 146" in e for e in validate_manifest(value)))
