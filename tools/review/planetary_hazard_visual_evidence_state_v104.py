#!/usr/bin/env python3
"""Validate v104 planetary hazard visual evidence/state records."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_evidence_state_v104"
VERSION = 104
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "evidence_id", "state_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("evidence_version", "state_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 104")
    ...
    for index, pair in enumerate(pairs):
        ...
        record = pair.get("evidence_state_record_id")
        if not isinstance(record, str) or not record.strip() or record in records: errors.append(f"{prefix}.evidence_state_record_id must be unique and required")
        if isinstance(record, str): records.add(record)
        for key in ("evidence_version", "state_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 104")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("evidence_id") != value.get("evidence_id") or pair.get("state_id") != value.get("state_id")): errors.append(f"{prefix} evidence/state bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v104")
    ...
    required = {"visual_evidence_state_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_EVIDENCE_STATE_V104_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_EVIDENCE_STATE_V104_VALID_OPEN"); return 0
