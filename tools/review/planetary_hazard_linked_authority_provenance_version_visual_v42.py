#!/usr/bin/env python3
"""Validate v42 versioned linked-authority/provenance visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_linked_authority_provenance_version_visual_v42"
VERSION = 42
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    ids: set[str] = set(); kinds: set[str] = set(); visuals: set[str] = set()
    for index, pair in enumerate(pairs):
        ...
        if not isinstance(visual_id, str) or not visual_id.strip() or visual_id in visuals: errors.append(f"{prefix}.visual_evidence_id must be unique and required")
        if isinstance(visual_id, str): visuals.add(visual_id)
        if pair.get("visual_version") != VERSION: errors.append(f"{prefix}.visual_version must be 42")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v42")
    ...
    if value.get("visual_version") != VERSION: errors.append(f"{label}.visual_version must be 42")
    ...
    required = {"linked_authority_provenance_version_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_LINKED_PROVENANCE_VERSION_V42_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_LINKED_PROVENANCE_VERSION_V42_VALID_OPEN"); return 0
