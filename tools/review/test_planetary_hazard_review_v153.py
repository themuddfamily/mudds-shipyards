...
from tools.review.planetary_hazard_review_v153 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v153"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v153"
    shared = {
        ...
        "evidence_surface": "review_manifest", "source_revision": source,
    }
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 153,
            **shared, "schema": schema, "schema_version": 153, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 153, "review_version": 153,
...
class PlanetaryHazardReviewV153Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 152
        self.assertTrue(any("schema_version must be 153" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 152
        self.assertTrue(any("review_version must be 153" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 152
        self.assertTrue(any("review_version must be 153" in e for e in validate_manifest(value)))
