#!/usr/bin/env python3
"""Validate v25 identity/digest reconciliation for planetary visuals."""
...
SCHEMA = "planetary_hazard_identity_digest_reconciliation_visual_v25"
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    if value.get("identity_digest_status") not in OPEN:
        errors.append(f"{label}.identity_digest_status must remain open")
    ...
    if not isinstance(exclusions, list) or not {"identity_digest_approval", "native_render", "human_signoff"}.issubset(set(exclusions)):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_IDENTITY_DIGEST_V25_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_IDENTITY_DIGEST_V25_VALID_OPEN")
    return 0
