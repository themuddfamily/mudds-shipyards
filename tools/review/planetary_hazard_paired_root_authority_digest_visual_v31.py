#!/usr/bin/env python3
"""Validate v31 root/authority paired digest metadata for planetary visuals."""
...
SCHEMA = "planetary_hazard_paired_root_authority_digest_visual_v31"
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    if not isinstance(aggregate, dict) or not isinstance(aggregate.get("sha256"), str) or not HEX64.fullmatch(aggregate["sha256"]):
        errors.append(f"{label}.authority_digest.sha256 must be a 64-character digest")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_ROOT_AUTHORITY_DIGEST_V31_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_ROOT_AUTHORITY_DIGEST_V31_VALID_OPEN")
    return 0
