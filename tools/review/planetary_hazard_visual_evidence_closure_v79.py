#!/usr/bin/env python3
"""Validate v79 planetary hazard visual evidence/closure records."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_evidence_closure_v79"
VERSION = 79
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("evidence_version", "closure_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 79")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("evidence_version", "closure_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 79")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v79")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_EVIDENCE_CLOSURE_V79_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_EVIDENCE_CLOSURE_V79_VALID_OPEN"); return 0
