#!/usr/bin/env python3
"""Validate v183 planetary-hazard review runtime storage boundaries."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded storage-policy/receipt violations without raising."""
    if not isinstance(value, dict):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V183_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V183_VALID_OPEN")
    return 0
