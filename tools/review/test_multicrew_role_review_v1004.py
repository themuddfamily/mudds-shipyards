...
from tools.review.multicrew_role_review_v1004 import validate_manifest


def manifest() -> dict:
    schema = "multicrew_role_review_v1004"; vessel, review, root = "jovian_courier", "crew_review", "multicrew_root"; source = "working-tree-multicrew-review-v1004"
    fixed = {"review_scope": "multicrew_role", "evidence_phase": "review", "review_mode": "evidence_only", "authority_class": "non_runtime_review", "closure_state": "open", "gate_policy": "native_human_open", "evidence_boundary": "pre_native_human", "evidence_revision": "v1004"}
    ...
    roles = [{"role_id": role + "_role", "role": role, "seat_id": vessel + "_" + role, "evidence_id": "evidence_" + role, "review_version": 1004, **shared, "review_id": review, "vessel_id": vessel, "source_revision": source, "root_identity_value": root, "runtime_authority": False, "status": status} for role, status in (("pilot", "pending"), ("gunner", "not_performed"), ("passenger", "pending"), ("engineer", "not_performed"))]
    return {"schema": schema, "schema_version": 1004, "review_version": 1004, "world_id": "ember_moon", "vessel_id": vessel, "review_id": review, "source_revision": source, "root_id": root, "root_identity_value": root, **shared, "roles": roles, "native_render": {"status": "not_performed"}, "human_signoff": {"status": "pending"}, "claims_excluded": ["native_render", "human_signoff", "runtime_role_assignment", "live_multiplayer_claim"]}


class MulticrewRoleReviewV1004Test(unittest.TestCase):
    def test_valid_open_manifest(self): self.assertEqual(validate_manifest(manifest()), [])
    def test_schema_version_strict(self):
        value = manifest(); value["schema_version"] = 187; self.assertTrue(any("schema_version must be 1004" in e for e in validate_manifest(value)))
    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 187; self.assertTrue(any("review_version must be 1004" in e for e in validate_manifest(value)))
