...
from tools.review.planetary_hazard_review_v154 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v154"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v154"
    shared = {
    ...
    receipts = [
        {
            ...
            "review_evidence_id": "evidence_" + kind, "review_version": 154,
            **shared, "schema": schema, "schema_version": 154, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 154, "review_version": 154,
...
class PlanetaryHazardReviewV154Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 153
        self.assertTrue(any("schema_version must be 154" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 153
        self.assertTrue(any("review_version must be 154" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 153
        self.assertTrue(any("review_version must be 154" in e for e in validate_manifest(value)))
    ...
    def test_surface_binding(self):
        value = manifest(); value["receipts"][1]["evidence_surface"] = "runtime"
        self.assertTrue(any("evidence_surface must match" in e for e in validate_manifest(value)))
