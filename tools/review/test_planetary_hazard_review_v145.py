...
from tools.review.planetary_hazard_review_v145 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v145"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source, scope = "working-tree-planetary-review-v145", "planetary_hazard_review"
    phase, channel = "review", "planetary-hazard"
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 145,
            "evidence_phase": phase, "evidence_channel": channel,
            "receipt_scope": scope, "source_revision": source,
            "schema": schema, "schema_version": 145, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 145, "review_version": 145,
        "world_id": "ember_moon", "region_id": "caldera_rim",
        "manifest_id": "caldera_review_manifest", "root_id": root,
        "review_id": review, "consistency_id": consistency, "state_id": state,
        "source_revision": source, "receipt_scope": scope,
        "evidence_phase": phase, "evidence_channel": channel, "receipts": receipts,
...
class PlanetaryHazardReviewV145Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 144
        self.assertTrue(any("schema_version must be 145" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 144
        self.assertTrue(any("review_version must be 145" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 144
        self.assertTrue(any("review_version must be 145" in e for e in validate_manifest(value)))
    ...
    def test_channel_binding(self):
        value = manifest(); value["receipts"][1]["evidence_channel"] = "other"
        self.assertTrue(any("evidence_channel must match" in e for e in validate_manifest(value)))

    def test_phase_binding(self):
        value = manifest(); value["receipts"][1]["evidence_phase"] = "approval"
        self.assertTrue(any("evidence_phase must match" in e for e in validate_manifest(value)))
