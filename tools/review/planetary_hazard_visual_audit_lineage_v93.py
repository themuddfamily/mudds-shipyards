#!/usr/bin/env python3
"""Validate v93 planetary hazard visual audit/lineage evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_audit_lineage_v93"
VERSION = 93
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("audit_version", "lineage_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 93")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("audit_version", "lineage_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 93")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v93")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_AUDIT_LINEAGE_V93_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_AUDIT_LINEAGE_V93_VALID_OPEN"); return 0
