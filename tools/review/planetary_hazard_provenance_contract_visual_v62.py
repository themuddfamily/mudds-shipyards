#!/usr/bin/env python3
"""Validate v62 planetary hazard provenance/contract visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_provenance_contract_visual_v62"
VERSION = 62
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "provenance_id", "contract_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("provenance_version", "contract_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 62")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("provenance_contract_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.provenance_contract_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        for key in ("provenance_version", "contract_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 62")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("provenance_id") != value.get("provenance_id") or pair.get("contract_id") != value.get("contract_id")): errors.append(f"{prefix} provenance/contract bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v62")
    ...
    required = {"provenance_contract_visual_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_PROVENANCE_CONTRACT_V62_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_PROVENANCE_CONTRACT_V62_VALID_OPEN"); return 0
