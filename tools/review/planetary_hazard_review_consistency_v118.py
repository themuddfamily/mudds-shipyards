#!/usr/bin/env python3
"""Validate v118 planetary-hazard review consistency evidence."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded review-consistency violations without raising."""
    if not isinstance(value, dict):
    ...
    for key in ("review_version", "consistency_version"):
        if value.get(key) != VERSION:
            errors.append(f"{label}.{key} must be {VERSION}")
    ...
    for index, pair in enumerate(pairs):
        ...
        if kind not in KINDS:
            ...
            kinds.add(kind)
        evidence_id = pair.get("review_consistency_evidence_id")
        if not _text(evidence_id) or evidence_id in evidence_ids:
            errors.append(f"{prefix}.review_consistency_evidence_id must be unique and required")
        if _text(evidence_id):
            evidence_ids.add(evidence_id)
        for key in ("review_version", "consistency_version"):
            if pair.get(key) != VERSION:
                errors.append(f"{prefix}.{key} must be {VERSION}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_CONSISTENCY_V118_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_CONSISTENCY_V118_VALID_OPEN")
    return 0
