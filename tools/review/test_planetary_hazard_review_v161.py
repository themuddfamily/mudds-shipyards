...
from tools.review.planetary_hazard_review_v161 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v161"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v161"
    shared = {
        ...
        "evidence_revision": "v161", "lineage_label": "root_receipt",
    ...
    receipts = [
        {
            ...
            "review_evidence_id": "evidence_" + kind, "review_version": 161,
            **shared, "lineage_anchor": root, "schema": schema, "schema_version": 161,
    ...
    return {
        "schema": schema, "schema_version": 161, "review_version": 161,
...
class PlanetaryHazardReviewV161Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 160
        self.assertTrue(any("schema_version must be 161" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 160
        self.assertTrue(any("review_version must be 161" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 160
        self.assertTrue(any("review_version must be 161" in e for e in validate_manifest(value)))
    ...
    def test_root_kind_binding(self):
        value = manifest(); value["receipts"][1]["lineage_root_kind"] = "receipt"
        self.assertTrue(any("lineage_root_kind must match" in e for e in validate_manifest(value)))
