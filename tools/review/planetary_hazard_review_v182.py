#!/usr/bin/env python3
"""Validate v182 planetary-hazard review runtime input boundaries."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded input-policy/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "runtime_input_policy": INPUT_POLICY,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V182_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V182_VALID_OPEN")
    return 0
