#!/usr/bin/env python3
"""Validate v317 planetary-hazard review evidence without runtime effects."""
...
POLICY_NAMES = (
    "write", "process", "network", "environment", "filesystem", "time", "random",
    "thread", "signal", "mutex", "ipc", "subprocess", "ui", "audio", "haptic",
    "display", "input", "storage", "cache", "gpu", "sensor",
)
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    required = (
        ...
        "consistency_id", "state_id", "source_revision", "receipt_scope", "review_owner",
    )
    for key in required:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V317_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V317_VALID_OPEN")
    return 0
