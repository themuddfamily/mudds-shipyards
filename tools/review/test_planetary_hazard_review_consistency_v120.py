...
from tools.review.planetary_hazard_review_consistency_v120 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_consistency_v120"
    root = "world_root"
    ...
    pairs = [
        {
            "id": kind,
            "kind": kind,
            "review_consistency_evidence_id": "evidence_" + kind,
            "review_version": 120,
            "consistency_version": 120,
            "schema": schema,
            "schema_version": 120,
            "parent_id": root,
            "review_id": review,
    ...
    return {
        ...
        "consistency_id": consistency,
        "state_id": state,
        "source_revision": "working-tree-planetary-review-v120",
        "pairs": pairs,
...
class PlanetaryHazardReviewConsistencyV120Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest()
        value["schema_version"] = 119
        self.assertTrue(any("schema_version must be 120" in error for error in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest()
        value["review_version"] = 119
        self.assertTrue(any("review_version must be 120" in error for error in validate_manifest(value)))

    def test_pair_consistency_version_strict(self):
        value = manifest()
        value["pairs"][0]["consistency_version"] = 119
        self.assertTrue(any("consistency_version must be 120" in error for error in validate_manifest(value)))

    def test_evidence_unique(self):
        value = manifest()
        value["pairs"][1]["review_consistency_evidence_id"] = value["pairs"][0]["review_consistency_evidence_id"]
