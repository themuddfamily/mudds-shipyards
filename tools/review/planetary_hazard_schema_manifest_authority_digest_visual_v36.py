#!/usr/bin/env python3
"""Validate the v36 planetary hazard schema/manifest authority digest."""
...
SCHEMA = "planetary_hazard_schema_manifest_authority_digest_visual_v36"
VERSION = 36
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for index, pair in enumerate(pairs):
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION:
            errors.append(f"{prefix}.schema and schema_version must match v36")
    ...
    if (not isinstance(digest, dict) or digest.get("schema") != SCHEMA or digest.get("schema_version") != VERSION or digest.get("algorithm") != "sha256" or not isinstance(digest.get("value"), str) or not HEX64.fullmatch(digest["value"])):
        errors.append(f"{label}.authority_digest must be v36 sha256 with a 64-character value")
    ...
    required = {"authority_digest_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_SCHEMA_MANIFEST_AUTHORITY_V36_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_SCHEMA_MANIFEST_AUTHORITY_V36_VALID_OPEN"); return 0
