#!/usr/bin/env python3
"""Validate v114 planetary hazard visual audit/state records."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_audit_state_v114"
VERSION = 114
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("audit_version", "state_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 114")
    ...
    for index, pair in enumerate(pairs):
        ...
        if not isinstance(record, str) or not record.strip() or record in records: errors.append(f"{prefix}.audit_state_record_id must be unique and required")
        if isinstance(record, str): records.add(record)
        for key in ("audit_version", "state_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 114")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v114")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_VISUAL_AUDIT_STATE_V114_INVALID"); print("\n".join(f"- {error}" for error in errors)); return 1
    print("PLANETARY_VISUAL_AUDIT_STATE_V114_VALID_OPEN"); return 0
