#!/usr/bin/env python3
"""Validate v44 dual-version authority/provenance visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_dual_version_authority_provenance_visual_v44"
VERSION = 44
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("authority_version", "provenance_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 44")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("authority_version", "provenance_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 44")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v44")
    ...
    required = {"dual_authority_provenance_version_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_DUAL_VERSION_V44_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_DUAL_VERSION_V44_VALID_OPEN"); return 0
