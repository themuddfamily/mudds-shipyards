#!/usr/bin/env python3
"""Validate v37 planetary hazard authority/reconciliation visual evidence."""
...
SCHEMA = "planetary_hazard_authority_reconciliation_digest_visual_v37"
VERSION = 37
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for index, pair in enumerate(pairs):
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v37")
    ...
    for key in ("authority_digest", "reconciliation_digest"):
        ...
        if (not isinstance(digest, dict) or digest.get("schema") != SCHEMA or digest.get("schema_version") != VERSION or digest.get("algorithm") != "sha256" or not isinstance(digest.get("value"), str) or not HEX64.fullmatch(digest["value"])):
            errors.append(f"{label}.{key} must be v37 sha256 with a 64-character value")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_AUTHORITY_RECONCILIATION_V37_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_AUTHORITY_RECONCILIATION_V37_VALID_OPEN"); return 0
