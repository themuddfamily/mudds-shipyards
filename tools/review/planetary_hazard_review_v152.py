#!/usr/bin/env python3
"""Validate v152 planetary-hazard pre-native/human review boundaries."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded boundary/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "closure_state": CLOSURE, "gate_policy": GATE_POLICY, "evidence_boundary": BOUNDARY,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V152_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V152_VALID_OPEN")
    return 0
