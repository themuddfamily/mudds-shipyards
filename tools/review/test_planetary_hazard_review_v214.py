...
from tools.review.planetary_hazard_review_v214 import validate_manifest
...
def manifest() -> dict:
    ...
    source = "working-tree-planetary-review-v214"
    shared = {
    ...
    receipts = [{
        ...
        "review_evidence_id": "evidence_" + kind, "review_version": 214,
        **shared, "root_identity_value": root, "lineage_anchor": root,
        "schema": schema, "schema_version": 214, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 214, "review_version": 214,
...
class PlanetaryHazardReviewV214Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 187
        self.assertTrue(any("schema_version must be 214" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 187
        self.assertTrue(any("review_version must be 214" in e for e in validate_manifest(value)))
