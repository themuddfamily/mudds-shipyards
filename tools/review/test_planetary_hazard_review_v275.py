...
from tools.review.planetary_hazard_review_v275 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v275"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v275"
    shared = {
        ...
        "evidence_revision": "v275", "lineage_label": "root_receipt",
    ...
    receipts = [{
        ...
        "review_evidence_id": "evidence_" + kind, "review_version": 275,
        **shared, "root_identity_value": root, "lineage_anchor": root,
        "schema": schema, "schema_version": 275, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 275, "review_version": 275,
        ...
        **shared, "root_identity_value": root, "receipts": receipts,
        ...
        "claims_excluded": ["visual_consistency_state_approval", "visual_review_consistency_approval", "native_render", "human_signoff"],
    }


class PlanetaryHazardReviewV275Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 187
        self.assertTrue(any("schema_version must be 275" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 187
        self.assertTrue(any("review_version must be 275" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 187
        self.assertTrue(any("review_version must be 275" in e for e in validate_manifest(value)))
    ...
    def test_sensor_policy_binding(self):
        value = manifest(); value["receipts"][1]["runtime_sensor_policy"] = "allowed"
        self.assertTrue(any("runtime_sensor_policy must match" in e for e in validate_manifest(value)))

    def test_filesystem_policy_binding(self):
        value = manifest(); value["runtime_filesystem_policy"] = "allowed"
        self.assertTrue(any("runtime_filesystem_policy must be forbidden" in e for e in validate_manifest(value)))
