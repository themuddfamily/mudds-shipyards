#!/usr/bin/env python3
"""Validate v105 planetary hazard visual provenance/closure evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_provenance_closure_v105"
VERSION = 105
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "provenance_id", "closure_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("provenance_version", "closure_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 105")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("provenance_closure_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.provenance_closure_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        for key in ("provenance_version", "closure_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 105")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("provenance_id") != value.get("provenance_id") or pair.get("closure_id") != value.get("closure_id")): errors.append(f"{prefix} provenance/closure bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v105")
    ...
    required = {"visual_provenance_closure_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_PROVENANCE_CLOSURE_V105_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_PROVENANCE_CLOSURE_V105_VALID_OPEN"); return 0
