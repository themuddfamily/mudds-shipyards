#!/usr/bin/env python3
"""Validate v40 linked-authority/provenance planetary visual evidence."""
from __future__ import annotations
import argparse, json, re
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_linked_authority_provenance_digest_visual_v40"
VERSION = 40
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for index, pair in enumerate(pairs):
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v40")
        for key in ("source_sha256", "review_sha256", "provenance_sha256"):
    ...
    for key in ("linked_authority_digest", "provenance_digest"):
        digest = value.get(key)
        if (not isinstance(digest, dict) or digest.get("schema") != SCHEMA or digest.get("schema_version") != VERSION or digest.get("algorithm") != "sha256" or not isinstance(digest.get("value"), str) or not HEX64.fullmatch(digest["value"])): errors.append(f"{label}.{key} must be v40 sha256 with a 64-character value")
    ...
    required = {"linked_authority_provenance_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_LINKED_PROVENANCE_V40_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_LINKED_PROVENANCE_V40_VALID_OPEN"); return 0
