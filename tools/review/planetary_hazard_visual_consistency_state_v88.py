#!/usr/bin/env python3
"""Validate v88 planetary hazard visual consistency/state evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_consistency_state_v88"
VERSION = 88
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("consistency_version", "state_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 88")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("consistency_version", "state_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 88")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v88")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_CONSISTENCY_STATE_V88_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_CONSISTENCY_STATE_V88_VALID_OPEN"); return 0
