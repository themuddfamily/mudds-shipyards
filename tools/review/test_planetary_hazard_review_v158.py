...
from tools.review.planetary_hazard_review_v158 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v158"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v158"
    shared = {
        ...
        "evidence_revision": "v158", "lineage_label": "root_receipt", "source_revision": source,
    }
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 158,
            **shared, "schema": schema, "schema_version": 158, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 158, "review_version": 158,
...
class PlanetaryHazardReviewV158Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 157
        self.assertTrue(any("schema_version must be 158" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 157
        self.assertTrue(any("review_version must be 158" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 157
        self.assertTrue(any("review_version must be 158" in e for e in validate_manifest(value)))
    ...
    def test_revision_binding(self):
        value = manifest(); value["receipts"][1]["evidence_revision"] = "v157"
        self.assertTrue(any("evidence_revision must match" in e for e in validate_manifest(value)))
