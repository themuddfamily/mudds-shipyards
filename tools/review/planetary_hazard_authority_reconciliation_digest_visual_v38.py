#!/usr/bin/env python3
"""Validate v38 planetary hazard authority/reconciliation visual evidence."""
from __future__ import annotations
import argparse, json, re
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_authority_reconciliation_digest_visual_v38"
VERSION = 38
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for index, pair in enumerate(pairs):
        ...
        if pair.get("parent_id") != value.get("root_id") or pair.get("authority_id") != value.get("authority_id"): errors.append(f"{prefix}.parent_id and authority_id must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v38")
    ...
    for key in ("authority_digest", "reconciliation_digest"):
        ...
        if (not isinstance(digest, dict) or digest.get("schema") != SCHEMA or digest.get("schema_version") != VERSION or digest.get("algorithm") != "sha256" or not isinstance(digest.get("value"), str) or not HEX64.fullmatch(digest["value"])): errors.append(f"{label}.{key} must be v38 sha256 with a 64-character value")
        elif digest.get("status") not in OPEN: errors.append(f"{label}.{key}.status must remain open")
    ...
    required = {"authority_reconciliation_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_AUTHORITY_RECONCILIATION_V38_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_AUTHORITY_RECONCILIATION_V38_VALID_OPEN"); return 0
