#!/usr/bin/env python3
"""Validate v45 dual-version authority/provenance link visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_dual_version_authority_provenance_link_visual_v45"
VERSION = 45
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("authority_version", "provenance_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 45")
    ...
    for index, pair in enumerate(pairs):
        ...
        link_id = pair.get("authority_provenance_link_id")
        if not isinstance(link_id, str) or not link_id.strip() or link_id in links: errors.append(f"{prefix}.authority_provenance_link_id must be unique and required")
        if isinstance(link_id, str): links.add(link_id)
        for key in ("authority_version", "provenance_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 45")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v45")
    ...
    required = {"dual_authority_provenance_link_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_DUAL_LINK_V45_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_DUAL_LINK_V45_VALID_OPEN"); return 0
