#!/usr/bin/env python3
"""Validate v22 provenance reconciliation for planetary visual evidence."""
...
SCHEMA = "planetary_hazard_provenance_reconciliation_visual_v22"
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "source_revision", "provenance_owner"):
    ...
    if value.get("provenance_status") not in OPEN:
        errors.append(f"{label}.provenance_status must remain open")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_PROVENANCE_RECONCILIATION_V22_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_PROVENANCE_RECONCILIATION_V22_VALID_OPEN")
    return 0
