#!/usr/bin/env python3
"""Validate v51 release/version authority-reconciliation visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_release_version_authority_reconciliation_visual_v51"
VERSION = 51
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "authority_id", "reconciliation_id", "release_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("authority_version", "reconciliation_version", "release_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 51")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("release_authority_reconciliation_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.release_authority_reconciliation_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        for key in ("authority_version", "reconciliation_version", "release_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 51")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("authority_id") != value.get("authority_id") or pair.get("reconciliation_id") != value.get("reconciliation_id") or pair.get("release_id") != value.get("release_id")): errors.append(f"{prefix} release/authority/reconciliation bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v51")
    ...
    required = {"release_authority_reconciliation_visual_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_RELEASE_AUTHORITY_RECONCILIATION_V51_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_RELEASE_AUTHORITY_RECONCILIATION_V51_VALID_OPEN"); return 0
