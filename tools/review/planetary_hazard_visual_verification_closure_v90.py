#!/usr/bin/env python3
"""Validate v90 planetary hazard visual verification/closure evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_verification_closure_v90"
VERSION = 90
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("verification_version", "closure_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 90")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("verification_version", "closure_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 90")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v90")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_VERIFICATION_CLOSURE_V90_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_VERIFICATION_CLOSURE_V90_VALID_OPEN"); return 0
