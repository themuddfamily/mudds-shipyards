import unittest
from tools.review.planetary_hazard_visual_evidence_lineage_v98 import validate_manifest

def manifest():
    schema = "planetary_hazard_visual_evidence_lineage_v98"; root = "world_root"; evidence = "evidence_record"; lineage = "lineage_record"
    pairs = [{"id": i, "kind": k, "evidence_lineage_record_id": "record_" + i, "evidence_version": 98, "lineage_version": 98, "schema": schema, "schema_version": 98, "parent_id": root, "evidence_id": evidence, "lineage_id": lineage, "runtime_authority": False, "status": st} for i, k, st in (("hazard", "hazard", "pending"), ("landmark", "landmark", "not_performed"), ("route", "route", "pending"))]
    return {"schema": schema, "schema_version": 98, "evidence_version": 98, "lineage_version": 98, "world_id": "ember_moon", "region_id": "caldera_rim", "manifest_id": "caldera_visual_manifest", "root_id": root, "evidence_id": evidence, "lineage_id": lineage, "source_revision": "fa1c617", "pairs": pairs, "native_render": {"status": "not_performed"}, "human_signoff": {"status": "pending"}, "claims_excluded": ["visual_evidence_lineage_approval", "native_render", "human_signoff"]}

class PlanetaryHazardVisualEvidenceLineageV98Test(unittest.TestCase):
    def test_valid_open_manifest(self): self.assertEqual(validate_manifest(manifest()), [])
    def test_manifest_version_strict(self):
        x=manifest(); x["schema_version"]=97; self.assertTrue(any("schema_version must be 98" in e for e in validate_manifest(x)))
    def test_evidence_version_strict(self):
        x=manifest(); x["evidence_version"]=97; self.assertTrue(any("evidence_version must be 98" in e for e in validate_manifest(x)))
    def test_lineage_version_strict(self):
        x=manifest(); x["pairs"][0]["lineage_version"]=97; self.assertTrue(any("lineage_version must be 98" in e for e in validate_manifest(x)))
