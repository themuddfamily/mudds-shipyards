import unittest
from tools.review.planetary_hazard_visual_verification_state_v72 import validate_manifest

def manifest():
    schema = "planetary_hazard_visual_verification_state_v72"; root = "world_root"; verification = "verification_record"; state = "state_record"
    pairs = [{"id": i, "kind": k, "verification_state_evidence_id": "evidence_" + i, "verification_version": 72, "state_version": 72, "schema": schema, "schema_version": 72, "parent_id": root, "verification_id": verification, "state_id": state, "runtime_authority": False, "status": st} for i, k, st in (("hazard", "hazard", "pending"), ("landmark", "landmark", "not_performed"), ("route", "route", "pending"))]
    return {"schema": schema, "schema_version": 72, "verification_version": 72, "state_version": 72, "world_id": "ember_moon", "region_id": "caldera_rim", "manifest_id": "caldera_visual_manifest", "root_id": root, "verification_id": verification, "state_id": state, "source_revision": "27a1ee1", "pairs": pairs, "native_render": {"status": "not_performed"}, "human_signoff": {"status": "pending"}, "claims_excluded": ["visual_verification_state_approval", "native_render", "human_signoff"]}

class PlanetaryHazardVisualVerificationStateV72Test(unittest.TestCase):
    def test_valid_open_manifest(self): self.assertEqual(validate_manifest(manifest()), [])
    def test_manifest_version_strict(self):
        x=manifest(); x["schema_version"]=71; self.assertTrue(any("schema_version must be 72" in e for e in validate_manifest(x)))
    def test_verification_version_strict(self):
        x=manifest(); x["verification_version"]=71; self.assertTrue(any("verification_version must be 72" in e for e in validate_manifest(x)))
    def test_state_version_strict(self):
        x=manifest(); x["pairs"][0]["state_version"]=71; self.assertTrue(any("state_version must be 72" in e for e in validate_manifest(x)))
