#!/usr/bin/env python3
"""Validate v146 planetary-hazard review owner-bound receipts."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded owner/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    for index, receipt in enumerate(receipts):
        ...
        for key in ("evidence_phase", "evidence_channel", "review_owner", "receipt_scope", "source_revision"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V146_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V146_VALID_OPEN")
    return 0
