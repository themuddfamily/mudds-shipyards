#!/usr/bin/env python3
"""Validate v116 planetary hazard visual audit/state records."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_audit_state_v116"
VERSION = 116
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("audit_version", "state_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 116")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("audit_version", "state_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 116")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v116")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_VISUAL_AUDIT_STATE_V116_INVALID"); print("\n".join(f"- {error}" for error in errors)); return 1
    print("PLANETARY_VISUAL_AUDIT_STATE_V116_VALID_OPEN"); return 0
