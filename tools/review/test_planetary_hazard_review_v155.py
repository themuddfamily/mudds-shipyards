...
from tools.review.planetary_hazard_review_v155 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v155"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v155"
    shared = {
        ...
        "evidence_class": "planetary_hazard", "source_revision": source,
    }
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 155,
            **shared, "schema": schema, "schema_version": 155, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 155, "review_version": 155,
...
class PlanetaryHazardReviewV155Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 154
        self.assertTrue(any("schema_version must be 155" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 154
        self.assertTrue(any("review_version must be 155" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 154
        self.assertTrue(any("review_version must be 155" in e for e in validate_manifest(value)))
    ...
    def test_class_binding(self):
        value = manifest(); value["receipts"][1]["evidence_class"] = "runtime"
        self.assertTrue(any("evidence_class must match" in e for e in validate_manifest(value)))

    def test_purpose_binding(self):
        value = manifest(); value["receipts"][1]["evidence_purpose"] = "release"
        self.assertTrue(any("evidence_purpose must match" in e for e in validate_manifest(value)))
