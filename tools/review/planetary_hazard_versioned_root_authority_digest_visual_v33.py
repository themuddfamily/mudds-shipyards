#!/usr/bin/env python3
"""Validate v33 versioned root/authority planetary visual digests."""
...
SCHEMA = "planetary_hazard_versioned_root_authority_digest_visual_v33"
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    if value.get("schema_version") != 33:
        errors.append(f"{label}.schema_version must be 33")
    ...
    for index, pair in enumerate(pairs):
        ...
        if pair.get("schema_version") != 33:
            errors.append(f"{prefix}.schema_version must be 33")
    ...
    if not isinstance(aggregate, dict) or aggregate.get("schema_version") != 33 or aggregate.get("algorithm") != "sha256" or not isinstance(aggregate.get("value"), str) or not HEX64.fullmatch(aggregate["value"]):
        errors.append(f"{label}.authority_digest must be v33 sha256 with a 64-character value")
    ...
    if not isinstance(exclusions, list) or not {"versioned_authority_approval", "native_render", "human_signoff"}.issubset(set(exclusions)):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_VERSIONED_AUTHORITY_V33_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_VERSIONED_AUTHORITY_V33_VALID_OPEN")
    return 0
