#!/usr/bin/env python3
"""Validate v39 linked-authority planetary hazard visual evidence."""
from __future__ import annotations
import argparse, json, re
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_linked_authority_digest_visual_v39"
VERSION = 39
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "authority_id", "linked_authority_id", "source_revision"):
    ...
    for index, pair in enumerate(pairs):
        ...
        if pair.get("parent_id") != value.get("root_id") or pair.get("authority_id") != value.get("authority_id") or pair.get("linked_authority_id") != value.get("linked_authority_id"): errors.append(f"{prefix} authority bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v39")
    ...
    digest = value.get("linked_authority_digest")
    if (not isinstance(digest, dict) or digest.get("schema") != SCHEMA or digest.get("schema_version") != VERSION or digest.get("algorithm") != "sha256" or not isinstance(digest.get("value"), str) or not HEX64.fullmatch(digest["value"])): errors.append(f"{label}.linked_authority_digest must be v39 sha256 with a 64-character value")
    elif digest.get("status") not in OPEN: errors.append(f"{label}.linked_authority_digest.status must remain open")
    ...
    required = {"linked_authority_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_LINKED_AUTHORITY_V39_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_LINKED_AUTHORITY_V39_VALID_OPEN"); return 0
