...
from tools.review.planetary_hazard_review_v138 import validate_manifest
...
def manifest() -> dict:
    ...
    return {
        ...
        "consistency_id": consistency,
        "state_id": state,
        "source_revision": "working-tree-planetary-review-v138",
        "pairs": pairs,
...
class PlanetaryHazardReviewV138Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest()
        value["schema_version"] = 137
        self.assertTrue(any("schema_version must be 138" in error for error in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest()
        value["review_version"] = 137
        self.assertTrue(any("review_version must be 138" in error for error in validate_manifest(value)))

    def test_pair_review_version_strict(self):
        value = manifest()
        value["pairs"][0]["review_version"] = 137
        self.assertTrue(any("review_version must be 138" in error for error in validate_manifest(value)))
