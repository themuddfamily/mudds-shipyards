#!/usr/bin/env python3
"""Validate v164 planetary-hazard review root-identity-format receipts."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded root-format/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "root_identity_format": ROOT_FORMAT,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V164_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V164_VALID_OPEN")
    return 0
