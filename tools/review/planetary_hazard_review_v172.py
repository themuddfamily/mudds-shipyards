#!/usr/bin/env python3
"""Validate v172 planetary-hazard review runtime randomness boundaries."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded randomness-policy/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "runtime_random_policy": RANDOM_POLICY,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V172_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V172_VALID_OPEN")
    return 0
