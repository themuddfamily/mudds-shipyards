#!/usr/bin/env python3
"""Validate v184 planetary-hazard review runtime cache boundaries."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded cache-policy/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "runtime_cache_policy": CACHE_POLICY,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V184_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V184_VALID_OPEN")
    return 0
