#!/usr/bin/env python3
"""Validate v32 algorithm-tagged root/authority planetary digests."""
...
SCHEMA = "planetary_hazard_paired_root_authority_digest_visual_v32"
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    if not isinstance(aggregate, dict) or aggregate.get("algorithm") != "sha256" or not isinstance(aggregate.get("value"), str) or not HEX64.fullmatch(aggregate["value"]):
        errors.append(f"{label}.authority_digest must declare sha256 and a 64-character value")
    ...
    if not isinstance(exclusions, list) or not {"authority_digest_approval", "native_render", "human_signoff"}.issubset(set(exclusions)):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_ROOT_AUTHORITY_DIGEST_V32_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_ROOT_AUTHORITY_DIGEST_V32_VALID_OPEN")
    return 0
