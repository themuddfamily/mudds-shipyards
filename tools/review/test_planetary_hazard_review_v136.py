...
from tools.review.planetary_hazard_review_v136 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v136"
    root = "world_root"
    ...
    return {
        ...
        "consistency_id": consistency,
        "state_id": state,
        "source_revision": "working-tree-planetary-review-v136",
        "pairs": pairs,
...
class PlanetaryHazardReviewV136Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest()
        value["schema_version"] = 135
        self.assertTrue(any("schema_version must be 136" in error for error in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest()
        value["review_version"] = 135
        self.assertTrue(any("review_version must be 136" in error for error in validate_manifest(value)))

    def test_pair_review_version_strict(self):
        value = manifest()
        value["pairs"][0]["review_version"] = 135
        self.assertTrue(any("review_version must be 136" in error for error in validate_manifest(value)))
