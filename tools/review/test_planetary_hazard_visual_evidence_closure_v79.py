import unittest
from tools.review.planetary_hazard_visual_evidence_closure_v79 import validate_manifest

def manifest():
    schema = "planetary_hazard_visual_evidence_closure_v79"; root = "world_root"; evidence = "evidence_record"; closure = "closure_record"
    pairs = [{"id": i, "kind": k, "evidence_closure_record_id": "record_" + i, "evidence_version": 79, "closure_version": 79, "schema": schema, "schema_version": 79, "parent_id": root, "evidence_id": evidence, "closure_id": closure, "runtime_authority": False, "status": st} for i, k, st in (("hazard", "hazard", "pending"), ("landmark", "landmark", "not_performed"), ("route", "route", "pending"))]
    return {"schema": schema, "schema_version": 79, "evidence_version": 79, "closure_version": 79, "world_id": "ember_moon", "region_id": "caldera_rim", "manifest_id": "caldera_visual_manifest", "root_id": root, "evidence_id": evidence, "closure_id": closure, "source_revision": "b9e5406", "pairs": pairs, "native_render": {"status": "not_performed"}, "human_signoff": {"status": "pending"}, "claims_excluded": ["visual_evidence_closure_approval", "native_render", "human_signoff"]}

class PlanetaryHazardVisualEvidenceClosureV79Test(unittest.TestCase):
    def test_valid_open_manifest(self): self.assertEqual(validate_manifest(manifest()), [])
    def test_manifest_version_strict(self):
        x=manifest(); x["schema_version"]=78; self.assertTrue(any("schema_version must be 79" in e for e in validate_manifest(x)))
    def test_evidence_version_strict(self):
        x=manifest(); x["evidence_version"]=78; self.assertTrue(any("evidence_version must be 79" in e for e in validate_manifest(x)))
    def test_closure_version_strict(self):
        x=manifest(); x["pairs"][0]["closure_version"]=78; self.assertTrue(any("closure_version must be 79" in e for e in validate_manifest(x)))
