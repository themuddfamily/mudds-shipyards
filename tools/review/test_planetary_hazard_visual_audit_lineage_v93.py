import unittest
from tools.review.planetary_hazard_visual_audit_lineage_v93 import validate_manifest

def manifest():
    schema = "planetary_hazard_visual_audit_lineage_v93"; root = "world_root"; audit = "audit_record"; lineage = "lineage_record"
    pairs = [{"id": i, "kind": k, "audit_lineage_evidence_id": "evidence_" + i, "audit_version": 93, "lineage_version": 93, "schema": schema, "schema_version": 93, "parent_id": root, "audit_id": audit, "lineage_id": lineage, "runtime_authority": False, "status": st} for i, k, st in (("hazard", "hazard", "pending"), ("landmark", "landmark", "not_performed"), ("route", "route", "pending"))]
    return {"schema": schema, "schema_version": 93, "audit_version": 93, "lineage_version": 93, "world_id": "ember_moon", "region_id": "caldera_rim", "manifest_id": "caldera_visual_manifest", "root_id": root, "audit_id": audit, "lineage_id": lineage, "source_revision": "be74f02", "pairs": pairs, "native_render": {"status": "not_performed"}, "human_signoff": {"status": "pending"}, "claims_excluded": ["visual_audit_lineage_approval", "native_render", "human_signoff"]}

class PlanetaryHazardVisualAuditLineageV93Test(unittest.TestCase):
    def test_valid_open_manifest(self): self.assertEqual(validate_manifest(manifest()), [])
    def test_manifest_version_strict(self):
        x=manifest(); x["schema_version"]=92; self.assertTrue(any("schema_version must be 93" in e for e in validate_manifest(x)))
    def test_audit_version_strict(self):
        x=manifest(); x["audit_version"]=92; self.assertTrue(any("audit_version must be 93" in e for e in validate_manifest(x)))
    def test_lineage_version_strict(self):
        x=manifest(); x["pairs"][0]["lineage_version"]=92; self.assertTrue(any("lineage_version must be 93" in e for e in validate_manifest(x)))
