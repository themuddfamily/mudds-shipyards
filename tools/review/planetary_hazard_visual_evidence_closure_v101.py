#!/usr/bin/env python3
"""Validate v101 planetary hazard visual evidence/closure records."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_evidence_closure_v101"
VERSION = 101
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "evidence_id", "closure_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("evidence_version", "closure_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 101")
    ...
    for index, pair in enumerate(pairs):
        ...
        record = pair.get("evidence_closure_record_id")
        if not isinstance(record, str) or not record.strip() or record in records: errors.append(f"{prefix}.evidence_closure_record_id must be unique and required")
        if isinstance(record, str): records.add(record)
        for key in ("evidence_version", "closure_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 101")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("evidence_id") != value.get("evidence_id") or pair.get("closure_id") != value.get("closure_id")): errors.append(f"{prefix} evidence/closure bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v101")
    ...
    required = {"visual_evidence_closure_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_EVIDENCE_CLOSURE_V101_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_EVIDENCE_CLOSURE_V101_VALID_OPEN"); return 0
