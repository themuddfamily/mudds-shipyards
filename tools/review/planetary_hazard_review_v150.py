#!/usr/bin/env python3
"""Validate v150 planetary-hazard review closure-state receipts."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded closure/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "authority_class": AUTHORITY, "retention_policy": RETENTION, "closure_state": CLOSURE,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V150_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V150_VALID_OPEN")
    return 0
