...
from tools.review.planetary_hazard_review_v159 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v159"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v159"
    shared = {
        ...
        "evidence_revision": "v159", "lineage_label": "root_receipt",
        "lineage_scope": "manifest_root", "source_revision": source,
    }
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 159,
            **shared, "schema": schema, "schema_version": 159, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 159, "review_version": 159,
...
class PlanetaryHazardReviewV159Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 158
        self.assertTrue(any("schema_version must be 159" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 158
        self.assertTrue(any("review_version must be 159" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 158
        self.assertTrue(any("review_version must be 159" in e for e in validate_manifest(value)))
    ...
    def test_scope_binding(self):
        value = manifest(); value["receipts"][1]["lineage_scope"] = "detached"
        self.assertTrue(any("lineage_scope must match" in e for e in validate_manifest(value)))

    def test_lineage_binding(self):
        value = manifest(); value["receipts"][1]["lineage_label"] = "detached"
        self.assertTrue(any("lineage_label must match" in e for e in validate_manifest(value)))
