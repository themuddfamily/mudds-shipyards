import unittest
from tools.review.planetary_hazard_visual_attestation_lineage_v95 import validate_manifest

def manifest():
    schema = "planetary_hazard_visual_attestation_lineage_v95"; root = "world_root"; attestation = "attestation_record"; lineage = "lineage_record"
    pairs = [{"id": i, "kind": k, "attestation_lineage_evidence_id": "evidence_" + i, "attestation_version": 95, "lineage_version": 95, "schema": schema, "schema_version": 95, "parent_id": root, "attestation_id": attestation, "lineage_id": lineage, "runtime_authority": False, "status": st} for i, k, st in (("hazard", "hazard", "pending"), ("landmark", "landmark", "not_performed"), ("route", "route", "pending"))]
    return {"schema": schema, "schema_version": 95, "attestation_version": 95, "lineage_version": 95, "world_id": "ember_moon", "region_id": "caldera_rim", "manifest_id": "caldera_visual_manifest", "root_id": root, "attestation_id": attestation, "lineage_id": lineage, "source_revision": "c5f6768", "pairs": pairs, "native_render": {"status": "not_performed"}, "human_signoff": {"status": "pending"}, "claims_excluded": ["visual_attestation_lineage_approval", "native_render", "human_signoff"]}

class PlanetaryHazardVisualAttestationLineageV95Test(unittest.TestCase):
    def test_valid_open_manifest(self): self.assertEqual(validate_manifest(manifest()), [])
    def test_manifest_version_strict(self):
        x=manifest(); x["schema_version"]=94; self.assertTrue(any("schema_version must be 95" in e for e in validate_manifest(x)))
    def test_attestation_version_strict(self):
        x=manifest(); x["attestation_version"]=94; self.assertTrue(any("attestation_version must be 95" in e for e in validate_manifest(x)))
    def test_lineage_version_strict(self):
        x=manifest(); x["pairs"][0]["lineage_version"]=94; self.assertTrue(any("lineage_version must be 95" in e for e in validate_manifest(x)))
