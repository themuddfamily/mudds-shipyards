#!/usr/bin/env python3
"""Validate v117 planetary-hazard visual consistency/state evidence."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded visual consistency/state violations without raising."""
    if not isinstance(value, dict):
    ...
    for index, pair in enumerate(pairs):
        ...
        if not _text(evidence_id) or evidence_id in evidence_ids:
            errors.append(f"{prefix}.consistency_state_evidence_id must be unique and required")
        ...
        for key in ("consistency_id", "state_id"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_VISUAL_CONSISTENCY_STATE_V117_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_VISUAL_CONSISTENCY_STATE_V117_VALID_OPEN")
    return 0
