...
from tools.review.planetary_hazard_review_v144 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v144"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source, scope, phase = "working-tree-planetary-review-v144", "planetary_hazard_review", "review"
    receipts = [
        {
    ...
    return {
        "schema": schema, "schema_version": 144, "review_version": 144,
...
class PlanetaryHazardReviewV144Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 143
        self.assertTrue(any("schema_version must be 144" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 143
        self.assertTrue(any("review_version must be 144" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 143
        self.assertTrue(any("review_version must be 144" in e for e in validate_manifest(value)))
