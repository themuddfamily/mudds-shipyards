...
from tools.review.planetary_hazard_review_v165 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v165"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v165"
    shared = {
        ...
        "evidence_revision": "v165", "lineage_label": "root_receipt",
        "lineage_scope": "manifest_root", "lineage_root_kind": "manifest",
        "root_identity": "root_id", "root_identity_format": "id",
        "runtime_write_policy": "forbidden", "source_revision": source,
    }
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 165,
            **shared, "root_identity_value": root, "lineage_anchor": root,
            "schema": schema, "schema_version": 165, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 165, "review_version": 165,
...
class PlanetaryHazardReviewV165Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 164
        self.assertTrue(any("schema_version must be 165" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 164
        self.assertTrue(any("review_version must be 165" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 164
        self.assertTrue(any("review_version must be 165" in e for e in validate_manifest(value)))
