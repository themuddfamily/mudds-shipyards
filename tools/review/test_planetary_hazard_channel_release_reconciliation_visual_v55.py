import unittest
from tools.review.planetary_hazard_channel_release_reconciliation_visual_v55 import validate_manifest

def manifest():
    schema = "planetary_hazard_channel_release_reconciliation_visual_v55"; root = "world_root"; channel = "visual_channel"; release = "release_candidate"; reconciliation = "reconciliation_root"
    pairs = [{"id": i, "kind": k, "channel_release_reconciliation_evidence_id": "evidence_" + i, "channel_version": 55, "release_version": 55, "reconciliation_version": 55, "schema": schema, "schema_version": 55, "parent_id": root, "channel_id": channel, "release_id": release, "reconciliation_id": reconciliation, "runtime_authority": False, "status": st} for i, k, st in (("hazard", "hazard", "pending"), ("landmark", "landmark", "not_performed"), ("route", "route", "pending"))]
    return {"schema": schema, "schema_version": 55, "channel_version": 55, "release_version": 55, "reconciliation_version": 55, "world_id": "ember_moon", "region_id": "caldera_rim", "manifest_id": "caldera_visual_manifest", "root_id": root, "channel_id": channel, "release_id": release, "reconciliation_id": reconciliation, "source_revision": "202f435", "pairs": pairs, "native_render": {"status": "not_performed"}, "human_signoff": {"status": "pending"}, "claims_excluded": ["channel_release_reconciliation_visual_approval", "native_render", "human_signoff"]}

class PlanetaryHazardChannelReleaseReconciliationVisualV55Test(unittest.TestCase):
    def test_valid_open_manifest(self): self.assertEqual(validate_manifest(manifest()), [])
    def test_manifest_version_strict(self):
        x=manifest(); x["schema_version"]=54; self.assertTrue(any("schema_version must be 55" in e for e in validate_manifest(x)))
    def test_channel_version_strict(self):
        x=manifest(); x["channel_version"]=54; self.assertTrue(any("channel_version must be 55" in e for e in validate_manifest(x)))
    def test_reconciliation_version_strict(self):
        x=manifest(); x["pairs"][0]["reconciliation_version"]=54; self.assertTrue(any("reconciliation_version must be 55" in e for e in validate_manifest(x)))
