#!/usr/bin/env python3
"""Validate v71 planetary hazard visual release/lineage evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_release_lineage_v71"
VERSION = 71
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "release_id", "lineage_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("release_version", "lineage_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 71")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("release_lineage_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.release_lineage_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        for key in ("release_version", "lineage_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 71")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("release_id") != value.get("release_id") or pair.get("lineage_id") != value.get("lineage_id")): errors.append(f"{prefix} release/lineage bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v71")
    ...
    required = {"visual_release_lineage_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_RELEASE_LINEAGE_V71_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_RELEASE_LINEAGE_V71_VALID_OPEN"); return 0
