#!/usr/bin/env python3
"""Validate v115 planetary hazard visual attestation/lineage records."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_attestation_lineage_v115"
VERSION = 115
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("attestation_version", "lineage_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 115")
    ...
    for index, pair in enumerate(pairs):
        ...
        record = pair.get("attestation_lineage_record_id")
        if not isinstance(record, str) or not record.strip() or record in records: errors.append(f"{prefix}.attestation_lineage_record_id must be unique and required")
        if isinstance(record, str): records.add(record)
        for key in ("attestation_version", "lineage_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 115")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v115")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_VISUAL_ATTESTATION_LINEAGE_V115_INVALID"); print("\n".join(f"- {error}" for error in errors)); return 1
    print("PLANETARY_VISUAL_ATTESTATION_LINEAGE_V115_VALID_OPEN"); return 0
