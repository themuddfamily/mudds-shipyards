#!/usr/bin/env python3
"""Validate v89 planetary hazard visual source/lineage evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_source_lineage_v89"
VERSION = 89
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "source_id", "lineage_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("source_version", "lineage_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 89")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("source_lineage_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.source_lineage_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        for key in ("source_version", "lineage_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 89")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("source_id") != value.get("source_id") or pair.get("lineage_id") != value.get("lineage_id")): errors.append(f"{prefix} source/lineage bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v89")
    ...
    required = {"visual_source_lineage_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_SOURCE_LINEAGE_V89_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_SOURCE_LINEAGE_V89_VALID_OPEN"); return 0
