...
from tools.review.planetary_hazard_review_v171 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v171"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v171"
    shared = {
        ...
        "evidence_revision": "v171", "lineage_label": "root_receipt",
        ...
        "runtime_time_policy": "forbidden", "source_revision": source,
    }
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 171,
            **shared, "root_identity_value": root, "lineage_anchor": root,
            "schema": schema, "schema_version": 171, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 171, "review_version": 171,
...
class PlanetaryHazardReviewV171Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 170
        self.assertTrue(any("schema_version must be 171" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 170
        self.assertTrue(any("review_version must be 171" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 170
        self.assertTrue(any("review_version must be 171" in e for e in validate_manifest(value)))
    ...
    def test_time_policy_binding(self):
        value = manifest(); value["receipts"][1]["runtime_time_policy"] = "allowed"
        self.assertTrue(any("runtime_time_policy must match" in e for e in validate_manifest(value)))

    def test_filesystem_policy_binding(self):
        value = manifest(); value["receipts"][1]["runtime_filesystem_policy"] = "allowed"
        self.assertTrue(any("runtime_filesystem_policy must match" in e for e in validate_manifest(value)))
