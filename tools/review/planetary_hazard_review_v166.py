#!/usr/bin/env python3
"""Validate v166 planetary-hazard review runtime read/write boundaries."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded runtime policy/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "runtime_read_policy": READ_POLICY,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V166_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V166_VALID_OPEN")
    return 0
