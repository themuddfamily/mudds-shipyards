#!/usr/bin/env python3
"""Validate v294 planetary-hazard review evidence without runtime effects."""
...
SCHEMA = "planetary_hazard_review_v294"
VERSION = 294
...
FIXED = {
    ...
    "evidence_class": "planetary_hazard", "evidence_contract": "planetary_hazard_review", "evidence_revision": "v294", "lineage_label": "root_receipt",
    "lineage_scope": "manifest_root", "lineage_root_kind": "manifest", "root_identity": "root_id", "root_identity_format": "id",
}
POLICY_NAMES = (
    "write", "process", "network", "environment", "filesystem", "time", "random", "thread", "signal", "mutex", "ipc", "subprocess", "ui", "audio", "haptic", "display", "input", "storage", "cache", "gpu", "sensor",
)
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return violations while never reading or changing runtime state."""
    if not isinstance(value, dict):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V294_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V294_VALID_OPEN")
    return 0
