#!/usr/bin/env python3
"""Validate v66 planetary hazard visual traceability/gate evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_traceability_gate_v66"
VERSION = 66
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("world_id", "region_id", "manifest_id", "root_id", "traceability_id", "gate_id", "source_revision"):
        if not isinstance(value.get(key), str) or not value[key].strip(): errors.append(f"{label}.{key} is required")
    for key in ("traceability_version", "gate_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 66")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("traceability_gate_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.traceability_gate_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        for key in ("traceability_version", "gate_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 66")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("traceability_id") != value.get("traceability_id") or pair.get("gate_id") != value.get("gate_id")): errors.append(f"{prefix} traceability/gate bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v66")
    ...
    required = {"visual_traceability_gate_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_TRACEABILITY_GATE_V66_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_TRACEABILITY_GATE_V66_VALID_OPEN"); return 0
