...
from tools.review.planetary_hazard_review_v187 import validate_manifest
...
def manifest() -> dict:
    ...
    source = "working-tree-planetary-review-v187"
    shared = {
        ...
        "evidence_revision": "v187", "lineage_label": "root_receipt",
    ...
    receipts = [
        {
            ...
            "review_evidence_id": "evidence_" + kind, "review_version": 187,
            **shared, "root_identity_value": root, "lineage_anchor": root,
            "schema": schema, "schema_version": 187, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 187, "review_version": 187,
...
class PlanetaryHazardReviewV187Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 186
        self.assertTrue(any("schema_version must be 187" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 186
        self.assertTrue(any("review_version must be 187" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 186
        self.assertTrue(any("review_version must be 187" in e for e in validate_manifest(value)))
    ...
    def test_runtime_write_boundary(self):
        value = manifest(); value["runtime_write_policy"] = "allowed"
        self.assertTrue(any("runtime_write_policy must be forbidden" in e for e in validate_manifest(value)))
