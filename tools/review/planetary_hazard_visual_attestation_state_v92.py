#!/usr/bin/env python3
"""Validate v92 planetary hazard visual attestation/state evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_attestation_state_v92"
VERSION = 92
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("attestation_version", "state_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 92")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("attestation_version", "state_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 92")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v92")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_ATTESTATION_STATE_V92_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_ATTESTATION_STATE_V92_VALID_OPEN"); return 0
