...
from tools.review.planetary_hazard_review_consistency_v118 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_consistency_v118"
    root = "world_root"
    ...
    return {
        ...
        "consistency_id": consistency,
        "state_id": state,
        "source_revision": "working-tree-planetary-review-v118",
        "pairs": pairs,
...
class PlanetaryHazardReviewConsistencyV118Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest()
        value["schema_version"] = 117
        self.assertTrue(any("schema_version must be 118" in error for error in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest()
        value["review_version"] = 117
        self.assertTrue(any("review_version must be 118" in error for error in validate_manifest(value)))

    def test_consistency_version_strict(self):
        value = manifest()
        value["pairs"][0]["consistency_version"] = 117
        self.assertTrue(any("consistency_version must be 118" in error for error in validate_manifest(value)))
