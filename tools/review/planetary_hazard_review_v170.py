#!/usr/bin/env python3
"""Validate v170 planetary-hazard review runtime filesystem boundaries."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded filesystem-policy/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    expected = {
        ...
        "runtime_filesystem_policy": FILESYSTEM_POLICY,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V170_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V170_VALID_OPEN")
    return 0
