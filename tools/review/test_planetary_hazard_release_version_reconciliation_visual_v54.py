import unittest
from tools.review.planetary_hazard_release_version_reconciliation_visual_v54 import validate_manifest

def manifest():
    schema = "planetary_hazard_release_version_reconciliation_visual_v54"; root = "world_root"; release = "release_candidate"; reconciliation = "reconciliation_root"
    pairs = [{"id": i, "kind": k, "release_reconciliation_evidence_id": "evidence_" + i, "release_version": 54, "reconciliation_version": 54, "schema": schema, "schema_version": 54, "parent_id": root, "release_id": release, "reconciliation_id": reconciliation, "runtime_authority": False, "status": st} for i, k, st in (("hazard", "hazard", "pending"), ("landmark", "landmark", "not_performed"), ("route", "route", "pending"))]
    return {"schema": schema, "schema_version": 54, "release_version": 54, "reconciliation_version": 54, "world_id": "ember_moon", "region_id": "caldera_rim", "manifest_id": "caldera_visual_manifest", "root_id": root, "release_id": release, "reconciliation_id": reconciliation, "source_revision": "7050d7c", "pairs": pairs, "native_render": {"status": "not_performed"}, "human_signoff": {"status": "pending"}, "claims_excluded": ["release_reconciliation_visual_approval", "native_render", "human_signoff"]}

class PlanetaryHazardReleaseVersionReconciliationVisualV54Test(unittest.TestCase):
    def test_valid_open_manifest(self): self.assertEqual(validate_manifest(manifest()), [])
    def test_manifest_version_strict(self):
        x=manifest(); x["schema_version"]=53; self.assertTrue(any("schema_version must be 54" in e for e in validate_manifest(x)))
    def test_release_version_strict(self):
        x=manifest(); x["release_version"]=53; self.assertTrue(any("release_version must be 54" in e for e in validate_manifest(x)))
    def test_reconciliation_version_strict(self):
        x=manifest(); x["pairs"][0]["reconciliation_version"]=53; self.assertTrue(any("reconciliation_version must be 54" in e for e in validate_manifest(x)))
