#!/usr/bin/env python3
"""Validate v50 versioned authority/reconciliation visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_versioned_authority_reconciliation_visual_v50"
VERSION = 50
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("authority_version", "reconciliation_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 50")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("authority_version", "reconciliation_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 50")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v50")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VERSIONED_AUTHORITY_RECONCILIATION_V50_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VERSIONED_AUTHORITY_RECONCILIATION_V50_VALID_OPEN"); return 0
