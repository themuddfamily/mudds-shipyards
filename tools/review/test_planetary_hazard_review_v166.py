...
from tools.review.planetary_hazard_review_v166 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v166"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v166"
    shared = {
        ...
        "evidence_revision": "v166", "lineage_label": "root_receipt",
    ...
    receipts = [
        {
            ...
            "review_evidence_id": "evidence_" + kind, "review_version": 166,
            **shared, "root_identity_value": root, "lineage_anchor": root,
            "schema": schema, "schema_version": 166, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 166, "review_version": 166,
...
class PlanetaryHazardReviewV166Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 165
        self.assertTrue(any("schema_version must be 166" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 165
        self.assertTrue(any("review_version must be 166" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 165
        self.assertTrue(any("review_version must be 166" in e for e in validate_manifest(value)))
    ...
    def test_read_policy_binding(self):
        value = manifest(); value["receipts"][1]["runtime_read_policy"] = "unbounded"
        self.assertTrue(any("runtime_read_policy must match" in e for e in validate_manifest(value)))
