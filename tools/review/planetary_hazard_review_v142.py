#!/usr/bin/env python3
"""Validate v142 planetary-hazard review evidence receipts."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded planetary-review receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    for index, pair in enumerate(pairs):
        ...
        if not isinstance(pair, dict):
            ...
            continue
        receipt_id = pair.get("receipt_id")
        ...
        if pair.get("source_revision") != value.get("source_revision"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V142_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V142_VALID_OPEN")
    return 0
