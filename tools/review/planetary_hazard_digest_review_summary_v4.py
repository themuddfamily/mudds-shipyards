#!/usr/bin/env python3
"""Validate v4 hazard visual digest review summaries."""
...
SCHEMA = "planetary_hazard_digest_review_summary_v4"
...
def validate_summary(value: Any, label: str = "summary") -> list[str]:
    ...
    for index, section in enumerate(sections):
        ...
        if not isinstance(section.get("evidence_refs"), list) or not section["evidence_refs"] or any(not isinstance(ref, str) or not ref.startswith("res://") for ref in section.get("evidence_refs", [])):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_SUMMARY_V4_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_SUMMARY_V4_VALID_OPEN")
    return 0
