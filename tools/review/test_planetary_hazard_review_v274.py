...
from tools.review.planetary_hazard_review_v274 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v274"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v274"
    shared = {
        ...
        "evidence_revision": "v274", "lineage_label": "root_receipt",
    ...
    receipts = [{
        ...
        "review_evidence_id": "evidence_" + kind, "review_version": 274,
        **shared, "root_identity_value": root, "lineage_anchor": root,
        "schema": schema, "schema_version": 274, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 274, "review_version": 274,
...
class PlanetaryHazardReviewV274Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 187
        self.assertTrue(any("schema_version must be 274" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 187
        self.assertTrue(any("review_version must be 274" in e for e in validate_manifest(value)))
