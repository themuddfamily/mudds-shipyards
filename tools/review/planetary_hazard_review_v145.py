#!/usr/bin/env python3
"""Validate v145 planetary-hazard review evidence-channel receipts."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded channel/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    for index, receipt in enumerate(receipts):
        ...
        if receipt.get("evidence_phase") != value.get("evidence_phase"):
            errors.append(f"{prefix}.evidence_phase must match manifest")
        if receipt.get("evidence_channel") != value.get("evidence_channel"):
            errors.append(f"{prefix}.evidence_channel must match manifest")
        if receipt.get("receipt_scope") != value.get("receipt_scope"):
            errors.append(f"{prefix}.receipt_scope must match manifest")
        if receipt.get("source_revision") != value.get("source_revision"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V145_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V145_VALID_OPEN")
    return 0
