...
from tools.review.planetary_hazard_review_v149 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v149"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source, scope = "working-tree-planetary-review-v149", "planetary_hazard_review"
    phase, channel, owner = "review", "planetary-hazard", "review-ledger"
    mode, authority, retention = "evidence_only", "non_runtime_review", "review_scope_only"
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 149,
            "evidence_phase": phase, "evidence_channel": channel, "review_owner": owner,
            "review_mode": mode, "authority_class": authority, "retention_policy": retention,
            "receipt_scope": scope, "source_revision": source, "schema": schema,
            "schema_version": 149, "parent_id": root, "review_id": review,
            "consistency_id": consistency, "state_id": state,
            "runtime_authority": False, "status": status,
        }
        for kind, status in (("hazard", "pending"), ("landmark", "not_performed"), ("route", "pending"))
    ]
    return {
        "schema": schema, "schema_version": 149, "review_version": 149,
        ...
        "authority_class": authority, "retention_policy": retention, "receipts": receipts,
...
class PlanetaryHazardReviewV149Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 148
        self.assertTrue(any("schema_version must be 149" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 148
        self.assertTrue(any("review_version must be 149" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 148
        self.assertTrue(any("review_version must be 149" in e for e in validate_manifest(value)))
    ...
    def test_retention_binding(self):
        value = manifest(); value["receipts"][1]["retention_policy"] = "permanent"
        self.assertTrue(any("retention_policy must match" in e for e in validate_manifest(value)))
