#!/usr/bin/env python3
"""Validate v27 paired digest reconciliation for planetary visuals."""
...
SCHEMA = "planetary_hazard_paired_digest_reconciliation_visual_v27"
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for index, pair in enumerate(pairs):
        ...
        kinds.add(kind)
        if pair.get("manifest_id") != value.get("manifest_id"):
    ...
    if value.get("pair_status") not in OPEN:
        errors.append(f"{label}.pair_status must remain open")
    ...
    if not isinstance(exclusions, list) or not {"paired_digest_approval", "native_render", "human_signoff"}.issubset(set(exclusions)):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_PAIRED_DIGEST_V27_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_PAIRED_DIGEST_V27_VALID_OPEN")
    return 0
