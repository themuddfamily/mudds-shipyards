#!/usr/bin/env python3
"""Validate v112 planetary hazard visual audit/state records."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("audit_version", "state_version"):
        if value.get(key) != VERSION:
            errors.append(f"{label}.{key} must be 112")
    ...
    for index, pair in enumerate(pairs):
        ...
        record = pair.get("audit_state_record_id")
        if not isinstance(record, str) or not record.strip() or record in records:
            errors.append(f"{prefix}.audit_state_record_id must be unique and required")
        if isinstance(record, str): records.add(record)
        for key in ("audit_version", "state_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 112")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("audit_id") != value.get("audit_id") or pair.get("state_id") != value.get("state_id")):
            errors.append(f"{prefix} audit/state bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION:
            errors.append(f"{prefix}.schema and schema_version must match v112")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_VISUAL_AUDIT_STATE_V112_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_VISUAL_AUDIT_STATE_V112_VALID_OPEN")
    return 0
