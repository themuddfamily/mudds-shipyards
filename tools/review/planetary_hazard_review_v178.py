#!/usr/bin/env python3
"""Validate v178 planetary-hazard review runtime UI boundaries."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded UI-policy/receipt violations without raising."""
    if not isinstance(value, dict):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V178_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V178_VALID_OPEN")
    return 0
