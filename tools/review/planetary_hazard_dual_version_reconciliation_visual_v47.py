#!/usr/bin/env python3
"""Validate v47 dual-version reconciliation visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_dual_version_reconciliation_visual_v47"
VERSION = 47
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("authority_version", "provenance_version", "reconciliation_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 47")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("reconciliation_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.reconciliation_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        for key in ("authority_version", "provenance_version", "reconciliation_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 47")
        if (pair.get("parent_id") != value.get("root_id") or pair.get("authority_id") != value.get("authority_id") or pair.get("linked_authority_id") != value.get("linked_authority_id") or pair.get("provenance_id") != value.get("provenance_id") or pair.get("reconciliation_id") != value.get("reconciliation_id")): errors.append(f"{prefix} reconciliation bindings must match manifest")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v47")
    ...
    required = {"dual_reconciliation_visual_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_DUAL_RECONCILIATION_V47_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_DUAL_RECONCILIATION_V47_VALID_OPEN"); return 0
