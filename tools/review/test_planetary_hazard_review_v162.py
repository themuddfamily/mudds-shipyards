...
from tools.review.planetary_hazard_review_v162 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v162"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v162"
    shared = {
        ...
        "evidence_revision": "v162", "lineage_label": "root_receipt",
        "lineage_scope": "manifest_root", "lineage_root_kind": "manifest",
        "root_identity": "root_id", "source_revision": source,
    }
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 162,
            **shared, "lineage_anchor": root, "schema": schema, "schema_version": 162,
    ...
    return {
        "schema": schema, "schema_version": 162, "review_version": 162,
...
class PlanetaryHazardReviewV162Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 161
        self.assertTrue(any("schema_version must be 162" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 161
        self.assertTrue(any("review_version must be 162" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 161
        self.assertTrue(any("review_version must be 162" in e for e in validate_manifest(value)))
    ...
    def test_root_identity_binding(self):
        value = manifest(); value["receipts"][1]["root_identity"] = "other"
        self.assertTrue(any("root_identity must match" in e for e in validate_manifest(value)))

    def test_anchor_binding(self):
        value = manifest(); value["receipts"][1]["lineage_anchor"] = "other_root"
        self.assertTrue(any("lineage_anchor must match" in e for e in validate_manifest(value)))
