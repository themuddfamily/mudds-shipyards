#!/usr/bin/env python3
"""Validate the v248 planetary-hazard review evidence boundary."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return v248 violations without reading or changing runtime state."""
    if not isinstance(value, dict):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V248_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V248_VALID_OPEN")
    return 0
