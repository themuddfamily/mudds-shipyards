#!/usr/bin/env python3
"""Validate the v202 planetary-hazard review evidence boundary."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return v202 violations without reading or changing runtime state."""
    if not isinstance(value, dict):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V202_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V202_VALID_OPEN")
    return 0
