#!/usr/bin/env python3
"""Validate v143 planetary-hazard review receipt scope."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded receipt-scope violations without raising."""
    if not isinstance(value, dict):
    ...
    if value.get("receipt_scope") != RECEIPT_SCOPE:
        errors.append(f"{label}.receipt_scope must be {RECEIPT_SCOPE}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V143_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V143_VALID_OPEN")
    return 0
