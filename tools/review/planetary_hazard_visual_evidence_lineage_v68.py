#!/usr/bin/env python3
"""Validate v68 planetary hazard visual evidence/lineage records."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_evidence_lineage_v68"
VERSION = 68
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("evidence_version", "lineage_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 68")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("evidence_version", "lineage_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 68")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v68")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_EVIDENCE_LINEAGE_V68_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_EVIDENCE_LINEAGE_V68_VALID_OPEN"); return 0
