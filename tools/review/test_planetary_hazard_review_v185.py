...
from tools.review.planetary_hazard_review_v185 import validate_manifest


def manifest() -> dict:
    schema = "planetary_hazard_review_v185"
    root, review = "world_root", "review_record"
    consistency, state = "consistency_record", "state_record"
    source = "working-tree-planetary-review-v185"
    shared = {
        ...
        "evidence_revision": "v185", "lineage_label": "root_receipt",
        ...
        "runtime_gpu_policy": "forbidden", "source_revision": source,
    }
    receipts = [
        {
            "receipt_id": "receipt_" + kind, "kind": kind,
            "review_evidence_id": "evidence_" + kind, "review_version": 185,
            **shared, "root_identity_value": root, "lineage_anchor": root,
            "schema": schema, "schema_version": 185, "parent_id": root,
    ...
    return {
        "schema": schema, "schema_version": 185, "review_version": 185,
...
class PlanetaryHazardReviewV185Test(unittest.TestCase):
    def test_valid_open_manifest(self):
        self.assertEqual(validate_manifest(manifest()), [])

    def test_manifest_version_strict(self):
        value = manifest(); value["schema_version"] = 184
        self.assertTrue(any("schema_version must be 185" in e for e in validate_manifest(value)))

    def test_review_version_strict(self):
        value = manifest(); value["review_version"] = 184
        self.assertTrue(any("review_version must be 185" in e for e in validate_manifest(value)))

    def test_receipt_version_strict(self):
        value = manifest(); value["receipts"][0]["review_version"] = 184
        self.assertTrue(any("review_version must be 185" in e for e in validate_manifest(value)))
    ...
    def test_gpu_policy_binding(self):
        value = manifest(); value["receipts"][1]["runtime_gpu_policy"] = "allowed"
        self.assertTrue(any("runtime_gpu_policy must match" in e for e in validate_manifest(value)))

    def test_cache_policy_binding(self):
        value = manifest(); value["receipts"][1]["runtime_cache_policy"] = "allowed"
        self.assertTrue(any("runtime_cache_policy must match" in e for e in validate_manifest(value)))
