#!/usr/bin/env python3
"""Validate v43 versioned linked-authority/provenance visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_linked_authority_provenance_version_visual_v43"
VERSION = 43
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("visual_version", "provenance_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 43")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v43")
    ...
    for key in ("visual_version", "provenance_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 43")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_LINKED_PROVENANCE_VERSION_V43_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_LINKED_PROVENANCE_VERSION_V43_VALID_OPEN"); return 0
