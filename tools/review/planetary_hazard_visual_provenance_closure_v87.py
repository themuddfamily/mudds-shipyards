#!/usr/bin/env python3
"""Validate v87 planetary hazard visual provenance/closure evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_visual_provenance_closure_v87"
VERSION = 87
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("provenance_version", "closure_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 87")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("provenance_version", "closure_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 87")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v87")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_VISUAL_PROVENANCE_CLOSURE_V87_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_VISUAL_PROVENANCE_CLOSURE_V87_VALID_OPEN"); return 0
