#!/usr/bin/env python3
"""Validate v30 root/authority bindings for paired planetary visuals."""
...
SCHEMA = "planetary_hazard_paired_root_authority_visual_v30"
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for index, pair in enumerate(pairs):
        ...
        if pair.get("authority_id") != value.get("authority_id"):
            errors.append(f"{prefix}.authority_id must match manifest")
    ...
    if not isinstance(exclusions, list) or not {"root_authority_approval", "native_render", "human_signoff"}.issubset(set(exclusions)):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_ROOT_AUTHORITY_V30_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_ROOT_AUTHORITY_V30_VALID_OPEN")
    return 0
