#!/usr/bin/env python3
"""Validate v162 planetary-hazard review root-identity receipts."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded root-identity/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    for index, receipt in enumerate(receipts):
        ...
        if receipt.get("root_identity") != value.get("root_identity"):
            errors.append(f"{prefix}.root_identity must match manifest")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V162_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V162_VALID_OPEN")
    return 0
