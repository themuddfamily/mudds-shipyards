#!/usr/bin/env python3
"""Validate v149 planetary-hazard review retention-bound receipts."""
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return bounded retention/receipt violations without raising."""
    if not isinstance(value, dict):
    ...
    if value.get("retention_policy") != RETENTION:
        errors.append(f"{label}.retention_policy must be {RETENTION}")
    ...
    for index, receipt in enumerate(receipts):
        ...
        for key in (
            ...
            "authority_class", "retention_policy", "receipt_scope", "source_revision",
        ):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PLANETARY_HAZARD_REVIEW_V149_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_HAZARD_REVIEW_V149_VALID_OPEN")
    return 0
