#!/usr/bin/env python3
"""Validate v48 dual-version planetary hazard authority visuals."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_dual_version_authority_visual_v48"
VERSION = 48
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    if value.get("authority_version") != VERSION: errors.append(f"{label}.authority_version must be 48")
    ...
    for index, pair in enumerate(pairs):
        ...
        evidence_id = pair.get("authority_evidence_id")
        if not isinstance(evidence_id, str) or not evidence_id.strip() or evidence_id in evidence: errors.append(f"{prefix}.authority_evidence_id must be unique and required")
        if isinstance(evidence_id, str): evidence.add(evidence_id)
        if pair.get("authority_version") != VERSION: errors.append(f"{prefix}.authority_version must be 48")
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v48")
        if pair.get("parent_id") != value.get("root_id") or pair.get("authority_id") != value.get("authority_id"): errors.append(f"{prefix} authority bindings must match manifest")
    ...
    required = {"dual_authority_visual_approval", "native_render", "human_signoff"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_DUAL_AUTHORITY_V48_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_DUAL_AUTHORITY_V48_VALID_OPEN"); return 0
