#!/usr/bin/env python3
"""Validate v53 release/version reconciliation visual evidence."""
from __future__ import annotations
import argparse, json
from pathlib import Path
from typing import Any

SCHEMA = "planetary_hazard_release_version_reconciliation_visual_v53"
VERSION = 53
...
def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    ...
    for key in ("release_version", "reconciliation_version"):
        if value.get(key) != VERSION: errors.append(f"{label}.{key} must be 53")
    ...
    for index, pair in enumerate(pairs):
        ...
        for key in ("release_version", "reconciliation_version"):
            if pair.get(key) != VERSION: errors.append(f"{prefix}.{key} must be 53")
        ...
        if pair.get("schema") != SCHEMA or pair.get("schema_version") != VERSION: errors.append(f"{prefix}.schema and schema_version must match v53")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors: print("PLANETARY_RELEASE_RECONCILIATION_V53_INVALID"); print("\n".join(f"- {e}" for e in errors)); return 1
    print("PLANETARY_RELEASE_RECONCILIATION_V53_VALID_OPEN"); return 0
