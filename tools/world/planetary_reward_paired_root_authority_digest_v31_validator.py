#!/usr/bin/env python3
"""Validate detached v31 planetary reward paired root/authority digest evidence."""
...
SCHEMA_VERSION = 31
EVIDENCE_SCOPE = "planetary_reward_paired_root_authority_digest_v31"
EVIDENCE_MODE = "detached_reward_paired_root_authority_digest_v31"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v31"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v31"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v31"
REQUIRED_LINEAGE_ROOT = "detached_reward_evidence_root_v31"
...
def _paired_digests(identity: dict[str, Any], authority: dict[str, Any], root: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> tuple[str, str, str]:
    ...
    pair_digest = _digest({"algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "lineage_root_digest_sha256": root_digest, "authority_digest_sha256": authority_digest, "reconciliation": reconciliation})
    return root_digest, authority_digest, pair_digest


def validate_paired_digest(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v31 paired root/authority digest artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v31"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_root_authority_digest_leaf_v31":
    ...
    for key, expected_value in (("lineage_root_digest_sha256", expected_root), ("authority_digest_sha256", expected_authority), ("paired_root_authority_digest_sha256", expected_pair)):
        ...
        if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            ...
            errors.append(f"{label}.{key} does not match canonical paired root/authority digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_PAIRED_DIGEST_V31_INVALID: {exc}")
        return 1
    errors = validate_paired_digest(report)
    if errors:
        print("PLANETARY_REWARD_PAIRED_DIGEST_V31_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_PAIRED_DIGEST_V31_VALID: detached paired root/authority digest only")
    return 0
