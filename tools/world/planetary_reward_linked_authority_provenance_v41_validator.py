#!/usr/bin/env python3
"""Validate detached v41 planetary reward linked authority/provenance evidence."""
...
SCHEMA_VERSION = 41
EVIDENCE_SCOPE = "planetary_reward_linked_authority_provenance_v41"
EVIDENCE_MODE = "detached_reward_linked_authority_provenance_v41"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v41"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v41"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v41"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_linked_authority_provenance_v41"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v41"
REQUIRED_PROVENANCE_LINK_ID = "planetary_reward_manifest_provenance_link_v41"
REQUIRED_LINK_VERSION = "authority_provenance_link_v41"
...
def validate_linked_provenance(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v41 linked authority/provenance artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v41"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_linked_authority_provenance_leaf_v41":
    ...
    all_refs = [identity.get("evidence_ref"), refs[0] if False else None, link_values["authority_link"].get("evidence_ref"), link_values["provenance_link"].get("evidence_ref")]
    all_refs = [identity.get("evidence_ref")] + refs + all_refs[2:]
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.linked_authority_provenance_digest_sha256 does not match canonical v41 payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_LINKED_PROVENANCE_V41_INVALID: {exc}")
        return 1
    errors = validate_linked_provenance(report)
    if errors:
        print("PLANETARY_REWARD_LINKED_PROVENANCE_V41_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_LINKED_PROVENANCE_V41_VALID: detached linked authority/provenance only")
    return 0
