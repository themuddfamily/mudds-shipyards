#!/usr/bin/env python3
"""Validate detached v26 planetary reward digest/reconciliation evidence."""
...
SCHEMA_VERSION = 26
EVIDENCE_SCOPE = "planetary_reward_digest_reconciliation_v26"
EVIDENCE_MODE = "detached_reward_digest_reconciliation_v26"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v26"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v26"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v26"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_digest_reconciliation"
...
def _reconciliation_digest(identity: dict[str, Any], authority: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> str:
    payload = {
...
def validate_digest_reconciliation(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v26 digest/reconciliation artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v26"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the digest authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_digest_reconciliation_leaf_v26":
    ...
    reconciliation = value.get("digest_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.digest_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {"algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "manifest_id": REQUIRED_MANIFEST_ID, "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "records_expected": 5, "records_observed": len(records), "references_expected": 6, "references_observed": len(all_refs), "records_reconciled": reconciled, "authority_reconciled": reconciled == 5, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.digest_reconciliation.{key} must be {expected_value}")
        ...
        if not _text(reconciliation_ref) or not reconciliation_ref.startswith("res://"):
            errors.append(f"{label}.digest_reconciliation.evidence_ref must be a res:// path")
        if reconciliation_ref in all_refs:
            errors.append(f"{label}.digest_reconciliation.evidence_ref must be unique")
    digest = value.get("digest_reconciliation_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.digest_reconciliation_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != _reconciliation_digest(identity, authority, reconciliation, records):
        errors.append(f"{label}.digest_reconciliation_sha256 does not match canonical digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_DIGEST_RECONCILIATION_V26_INVALID: {exc}")
        return 1
    errors = validate_digest_reconciliation(report)
    if errors:
        print("PLANETARY_REWARD_DIGEST_RECONCILIATION_V26_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_DIGEST_RECONCILIATION_V26_VALID: detached digest reconciliation only")
    return 0
