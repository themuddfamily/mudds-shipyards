#!/usr/bin/env python3
"""Validate version-sixteen planetary reward manifest digest/count provenance.

The v16 artifact hashes a canonical identity, count contract, and five reward
records.  It is detached evidence and performs no store, inventory, runtime,
or native write.
"""
...
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v16"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v16"
...
def _digest(manifest_id: str, provenance_id: str, counts: dict[str, Any], records: list[dict[str, Any]]) -> str:
    payload = {"manifest_id": manifest_id, "provenance_id": provenance_id, "counts": counts, "records": [{"activity_id": record.get("activity_id"), "reward_id": record.get("reward_id"), "leaf_id": record.get("leaf_id"), "evidence_ref": record.get("evidence_ref"), "status": record.get("status")} for record in records]}
    encoded = json.dumps(payload, ensure_ascii=True, sort_keys=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(encoded).hexdigest()


def validate_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v16 digest/count provenance artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v16"), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_digest_count_leaf_v16":
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {"records": len(records), "reward_ids": len(set(reward_ids)), "leaf_ids": len(set(leaf_ids)), "evidence_refs": len(set([identity_ref] + refs)), "pass_records": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)), "store_writes": 0, "inventory_writes": 0, "runtime_mutations": 0, "native_runs": 0}
    ...
    elif records and digest != _digest(identity.get("manifest_id"), identity.get("provenance_id"), counts, records):
        errors.append(f"{label}.manifest_digest_sha256 does not match canonical digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_MANIFEST_V16_INVALID: {exc}")
        return 1
    errors = validate_manifest(report)
    if errors:
        print("PLANETARY_REWARD_MANIFEST_V16_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_MANIFEST_V16_VALID: detached digest/count provenance only")
    return 0
