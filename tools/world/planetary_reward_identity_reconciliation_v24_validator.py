#!/usr/bin/env python3
"""Validate detached v24 planetary reward identity reconciliation evidence."""
...
SCHEMA_VERSION = 24
EVIDENCE_SCOPE = "planetary_reward_identity_reconciliation_v24"
EVIDENCE_MODE = "detached_reward_identity_reconciliation_v24"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v24"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v24"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v24"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_identity_reconciliation"
...
def _identity_reconciliation_digest(identity: dict[str, Any], authority: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> str:
    payload = {
...
def validate_identity_reconciliation(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v24 identity reconciliation artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v24"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_identity_reconciliation_leaf_v24":
    ...
    reconciliation = value.get("identity_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.identity_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        if not _text(reconciliation_ref) or not reconciliation_ref.startswith("res://"):
            errors.append(f"{label}.identity_reconciliation.evidence_ref must be a res:// path")
        if reconciliation_ref in all_refs:
            errors.append(f"{label}.identity_reconciliation.evidence_ref must be unique")
    digest = value.get("identity_reconciliation_digest_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.identity_reconciliation_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != _identity_reconciliation_digest(identity, authority, reconciliation, records):
        errors.append(f"{label}.identity_reconciliation_digest_sha256 does not match canonical identity reconciliation payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_IDENTITY_V24_INVALID: {exc}")
        return 1
    errors = validate_identity_reconciliation(report)
    if errors:
        print("PLANETARY_REWARD_IDENTITY_V24_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_IDENTITY_V24_VALID: detached identity reconciliation only")
    return 0
