#!/usr/bin/env python3
"""Validate detached v977 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v976_validator import (
    _authority_digest as _v976_authority_digest,
    validate_objective_consistency as _validate_v976,
)

SCHEMA_VERSION = 977
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v977"
EVIDENCE_MODE = "detached_reward_objective_consistency_v977"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v977"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v977"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v977"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v977"
REQUIRED_AUTHORITY_VERSION = "authority_v977"
...
def _to_v976(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v976(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v976(item) for item in value]
    if isinstance(value, str):
        return value.replace("v977", "v976")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v977 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v977", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v977", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v977":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v977 payload")

    translated = _to_v976(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 976
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 976
        translated["objective_consistency_digest_sha256"] = _v976_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v976", "v977") for error in _validate_v976(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V977_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V977_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V977_VALID: detached objective consistency only")
    return 0
