#!/usr/bin/env python3
"""Validate detached v21 planetary reward reconciliation digest provenance."""
...
SCHEMA_VERSION = 21
EVIDENCE_SCOPE = "planetary_reward_reconciliation_digest_provenance_v21"
EVIDENCE_MODE = "detached_reward_reconciliation_digest_v21"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v21"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v21"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_reconciliation"
...
def _provenance_digest(
    ...
    payload = {
        ...
        "authority": {
            "reward_authority_id": authority.get("reward_authority_id"),
            "reward_store_id": authority.get("reward_store_id"),
        ...
        "records": [
            {
                ...
                "activity_authority_id": record.get("activity_authority_id"),
                "reward_authority_id": record.get("reward_authority_id"),
                "reward_store_id": record.get("reward_store_id"),
...
def validate_provenance(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v21 reconciliation provenance artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v21"), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must use the reconciled reward authority and store")
        reward_id = record.get("reward_id")
        ...
        if leaf_id != f"{activity_id}_reward_provenance_leaf_v21":
    ...
    if not isinstance(provenance, dict):
        ...
        bound_records = sum(record.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and record.get("reward_store_id") == REQUIRED_REWARD_STORE for record in records if isinstance(record, dict))
        expected = {
            ...
            "reconciled_records": bound_records,
        ...
        for key, expected_value in expected.items():
            if provenance.get(key) != expected_value:
                errors.append(f"{label}.provenance.{key} must be {expected_value}")
        provenance_ref = provenance.get("evidence_ref")
        if not _text(provenance_ref) or not provenance_ref.startswith("res://"):
            errors.append(f"{label}.provenance.evidence_ref must be a res:// path")
        if provenance_ref == identity_ref or provenance_ref in refs:
            errors.append(f"{label}.provenance.evidence_ref must be unique")
    ...
    elif records and digest != _provenance_digest(identity.get("manifest_id"), identity.get("provenance_id"), authority, provenance, records):
        errors.append(f"{label}.provenance_digest_sha256 does not match canonical provenance payload")

    counts = value.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        reconciled_records = sum(record.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and record.get("reward_store_id") == REQUIRED_REWARD_STORE for record in records if isinstance(record, dict))
        expected_counts = {"records": len(records), "references": len(all_refs), "reconciled_records": reconciled_records, "complete_records": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)), "store_writes": 0, "inventory_writes": 0, "runtime_mutations": 0, "native_runs": 0}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_PROVENANCE_DIGEST_V21_INVALID: {exc}")
        return 1
    errors = validate_provenance(report)
    if errors:
        print("PLANETARY_REWARD_PROVENANCE_DIGEST_V21_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_PROVENANCE_DIGEST_V21_VALID: detached reconciliation provenance only")
    return 0
