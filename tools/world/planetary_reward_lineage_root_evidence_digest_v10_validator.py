#!/usr/bin/env python3
"""Validate version-ten planetary reward lineage-root evidence digest.

The v10 artifact binds one detached lineage-root evidence reference to five
authored reward leaves and hashes the canonical root/leaf set.  It performs no
store, inventory, runtime, or native write and makes no historical claim.
"""
...
REQUIRED_ROOT_ID = "planetary_reward_evidence_root_v10"
...
def _root_digest(root_id: str, root_ref: str, leaves: list[dict[str, Any]]) -> str:
    payload = {
        "root_id": root_id,
        "root_evidence_ref": root_ref,
        "leaves": [
            {"activity_id": leaf.get("activity_id"), "leaf_id": leaf.get("leaf_id"), "evidence_ref": leaf.get("evidence_ref"), "reward_id": leaf.get("reward_id")}
            for leaf in leaves
        ],
    }
...
def validate_root(value: Any, label: str = "root") -> list[str]:
    """Return blocking errors for one v10 lineage-root digest artifact."""
    ...
    root_ref = value.get("root_evidence_ref")
    if not _text(root_ref) or not root_ref.startswith("res://"):
        errors.append(f"{label}.root_evidence_ref must be a res:// path")
    ...
    for index, leaf in enumerate(leaves):
        ...
        if leaf_id != f"{activity_id}_reward_root_leaf_v10":
    ...
    all_refs = [root_ref] + leaf_refs
    _unique(all_refs, f"{label}.evidence_refs", errors)
    digest = value.get("root_digest_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.root_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif leaves and _text(root_ref) and digest != _root_digest(root_id, root_ref, leaves):
        errors.append(f"{label}.root_digest_sha256 does not match canonical root and leaves")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = json.loads(args.root.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        print(f"PLANETARY_REWARD_ROOT_V10_INVALID: {exc}")
        return 1
    errors = validate_root(report)
    if errors:
        print("PLANETARY_REWARD_ROOT_V10_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_ROOT_V10_VALID: detached lineage root only")
    return 0
