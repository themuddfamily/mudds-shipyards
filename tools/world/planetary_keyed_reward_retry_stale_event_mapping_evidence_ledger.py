#!/usr/bin/env python3
"""Validate evidence references for keyed reward retry/stale mappings.

The ledger attaches detached ``res://`` evidence to deterministic retry and
stale reward guard mappings.  It does not claim a native run, mutate inventory,
or own reward authority.
"""
...
def validate_ledger(value: Any, label: str = "ledger") -> list[str]:
    """Return blocking errors for one detached keyed reward evidence ledger."""
    ...
    for index, mapping in enumerate(mappings):
        ...
        expected_type, expected_generation, expected_accepted, expected_rejected, suffix = EXPECTED_EVENTS[event_index]
        if mapping.get("activity_id") != expected_activity:
            errors.append(f"{prefix}.activity_id must be {expected_activity}")
        if mapping.get("event_type") != expected_type:
            errors.append(f"{prefix}.event_type must be {expected_type}")
        mapping_id = mapping.get("event_id")
        mapping_ids.append(mapping_id)
        observed_count += 1
        expected_id = f"{expected_activity}_{suffix}"
        if mapping_id != expected_id:
            errors.append(f"{prefix}.event_id must be deterministic for the mapping")
        if not _stable(mapping_id):
    ...
    _unique(mapping_ids, f"{label}.event_ids", errors)
    ...
    if tuple(mapping.get("activity_id") for mapping in mappings if isinstance(mapping, dict)) != tuple(
        ...
        errors.append(f"{label}.mappings must retain authored activity/event order")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_KEYED_REWARD_EVIDENCE_INVALID: {exc}")
        return 1
    errors = validate_ledger(report)
    if errors:
        print("PLANETARY_KEYED_REWARD_EVIDENCE_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_KEYED_REWARD_EVIDENCE_VALID: detached evidence ledger only")
    return 0
