#!/usr/bin/env python3
"""Validate version-four reward digest authority and recovery summary.

Version four adds an explicit recovery handoff to the detached authority
summary.  All ownership and handoff metadata remains non-runtime and
non-mutating.
"""
...
EXPECTED_HANDOFFS = (
    ...
    ("recovery_to_return", "planetary_recovery_evidence", REQUIRED_RETURN_AUTHORITY),
)
...
def validate_summary(value: Any, label: str = "summary") -> list[str]:
    """Return blocking errors for one v4 detached reward authority summary."""
    ...
    for key in ("runtime_authority", "reward_inventory", "reward_runtime", "recovery_runtime", "native_claims", "historical_claim", "procedural_generation"):
    ...
    if not isinstance(owner, dict):
        ...
        expected_owner = {
            "owner_id": REQUIRED_REWARD_AUTHORITY,
            "store_id": REQUIRED_REWARD_STORE,
            "return_authority_id": REQUIRED_RETURN_AUTHORITY,
            "source": REQUIRED_SOURCE,
            "contract_revision": "reward_digest_authority_v4",
        }
        for key, expected in expected_owner.items():
            if owner.get(key) != expected:
                errors.append(f"{label}.digest_authority.{key} must be {expected}")
        for key in ("owns_inventory", "writes_runtime", "runs_recovery", "runs_native"):
    ...
    if not isinstance(handoffs, list) or len(handoffs) != len(EXPECTED_HANDOFFS):
        errors.append(f"{label}.handoffs must contain exactly four authored handoffs")
        handoffs = []
    ...
    for index, handoff in enumerate(handoffs):
        ...
        for key in ("runtime_wired", "mutates_inventory", "runs_recovery", "native_run"):
    ...
    for index, record in enumerate(records):
        ...
        if recovery_id not in EXISTING_RECOVERY_IDS:
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {"records": len(records), "handoffs": len(handoffs), "digest_leaves": len(leaf_ids), "pass_records": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)), "runtime_mutations": 0, "inventory_writes": 0, "recovery_runs": 0, "native_runs": 0}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_DIGEST_V4_INVALID: {exc}")
        return 1
    errors = validate_summary(report)
    if errors:
        print("PLANETARY_REWARD_DIGEST_V4_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_DIGEST_V4_VALID: detached recovery authority summary only")
    return 0
