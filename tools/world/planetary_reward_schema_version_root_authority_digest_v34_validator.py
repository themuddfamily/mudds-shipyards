#!/usr/bin/env python3
"""Validate detached v34 planetary reward schema/version root authority digest."""
...
SCHEMA_VERSION = 34
EVIDENCE_SCOPE = "planetary_reward_schema_version_root_authority_digest_v34"
EVIDENCE_MODE = "detached_reward_schema_version_root_authority_v34"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v34"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v34"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v34"
REQUIRED_LINEAGE_ROOT = "detached_reward_evidence_root_v34"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_schema_version_root_authority"
REQUIRED_DIGEST_VERSION = "paired_root_authority_v34"
...
def _schema_version_digests(identity: dict[str, Any], authority: dict[str, Any], root: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> tuple[str, str, str]:
    version = {"schema_version": SCHEMA_VERSION, "manifest_version": "v34", "provenance_version": "v34", "lineage_version": "v34", "digest_version": REQUIRED_DIGEST_VERSION}
    root_digest = _digest({"version": version, "lineage_root": root, "identity": {key: identity.get(key) for key in ("manifest_id", "provenance_id", "manifest_version", "lineage_id", "evidence_ref")}})
    authority_digest = _digest({"version": version, "authority": {key: authority.get(key) for key in ("reward_authority_id", "reward_store_id", "authority_scope")}, "records": [{key: record.get(key) for key in ("activity_id", "activity_authority_id", "reward_authority_id", "reward_store_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records]})
    pair_digest = _digest({"version": version, "lineage_root_digest_sha256": root_digest, "authority_digest_sha256": authority_digest, "reconciliation": reconciliation})
    return root_digest, authority_digest, pair_digest


def validate_schema_version_digest(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v34 schema/version digest artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v34"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the schema/version authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_schema_version_leaf_v34":
    ...
    if not isinstance(root, dict):
        ...
        if not _text(root.get("evidence_ref")) or not root["evidence_ref"].startswith("res://"):
            errors.append(f"{label}.lineage_root.evidence_ref must be a res:// path")
        if root.get("evidence_ref") in all_refs:
            errors.append(f"{label}.lineage_root.evidence_ref must be unique")

    reconciliation = value.get("schema_version_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.schema_version_reconciliation must be an object")
        reconciliation = {}
    else:
    ...
    for key, expected_value in (("lineage_root_digest_sha256", expected_root), ("authority_digest_sha256", expected_authority), ("schema_version_pair_digest_sha256", expected_pair)):
        ...
        if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            ...
            errors.append(f"{label}.{key} does not match canonical schema/version digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_SCHEMA_VERSION_V34_INVALID: {exc}")
        return 1
    errors = validate_schema_version_digest(report)
    if errors:
        print("PLANETARY_REWARD_SCHEMA_VERSION_V34_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_SCHEMA_VERSION_V34_VALID: detached schema/version root authority digest only")
    return 0
