#!/usr/bin/env python3
"""Validate detached v43 planetary reward linked authority/provenance/version evidence."""
...
SCHEMA_VERSION = 43
EVIDENCE_SCOPE = "planetary_reward_linked_authority_provenance_version_v43"
EVIDENCE_MODE = "detached_reward_linked_authority_provenance_version_v43"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v43"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v43"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v43"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_linked_authority_provenance_version_v43"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v43"
REQUIRED_PROVENANCE_LINK_ID = "planetary_reward_manifest_provenance_link_v43"
REQUIRED_LINK_VERSION = "authority_provenance_link_v43"
...
def validate_linked_version(value: Any, label: str = "manifest") -> list[str]:
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v43"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for field, required_id, kind in (("authority_link", REQUIRED_AUTHORITY_LINK_ID, "authority"), ("provenance_link", REQUIRED_PROVENANCE_LINK_ID, "provenance")):
        link = value.get(field)
        if not isinstance(link, dict):
            errors.append(f"{label}.{field} must be an object")
            link = {}
        else:
            expected_link = {"link_version": REQUIRED_LINK_VERSION, "link_id": required_id, "manifest_id": REQUIRED_MANIFEST_ID, "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "source": "detached_evidence", "linked": True}
            if kind == "authority":
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to versioned authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_linked_version_leaf_v43":
    ...
    reconciliation = value.get("linked_authority_provenance_version_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.linked_authority_provenance_version_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {"schema_version": SCHEMA_VERSION, "link_version": REQUIRED_LINK_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "provenance_link_id": REQUIRED_PROVENANCE_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v43", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "reward_authority_id": REQUIRED_REWARD_AUTHORITY, "reward_store_id": REQUIRED_REWARD_STORE, "algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "records_expected": 5, "records_observed": len(records), "references_expected": 8, "references_observed": len(all_refs), "records_reconciled": reconciled, "links_reconciled": links["authority_link"].get("linked") is True and links["provenance_link"].get("linked") is True, "authority_reconciled": authority.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and authority.get("reward_store_id") == REQUIRED_REWARD_STORE, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.linked_authority_provenance_version_reconciliation.{key} must be {expected_value}")
        if not _text(reconciliation.get("evidence_ref")) or not reconciliation["evidence_ref"].startswith("res://"):
            errors.append(f"{label}.linked_authority_provenance_version_reconciliation.evidence_ref must be a res:// path")
        if reconciliation.get("evidence_ref") in all_refs:
            errors.append(f"{label}.linked_authority_provenance_version_reconciliation.evidence_ref must be unique")

    digest = value.get("linked_authority_provenance_version_digest_sha256")
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.linked_authority_provenance_version_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.linked_authority_provenance_version_digest_sha256 does not match canonical v43 payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_LINKED_VERSION_V43_INVALID: {exc}")
        return 1
    errors = validate_linked_version(report)
    if errors:
        print("PLANETARY_REWARD_LINKED_VERSION_V43_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_LINKED_VERSION_V43_VALID: detached linked authority/provenance/version only")
    return 0
