#!/usr/bin/env python3
"""Validate detached v96 planetary reward verification/state evidence."""
...
from tools.world.planetary_reward_attestation_lineage_v95_validator import (
    _authority_digest as _v95_authority_digest,
    validate_attestation_lineage as _validate_v95,
)


SCHEMA_VERSION = 96
EVIDENCE_SCOPE = "planetary_reward_verification_state_v96"
EVIDENCE_MODE = "detached_reward_verification_state_v96"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v96"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v96"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v96"
...
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v96"
REQUIRED_AUTHORITY_VERSION = "authority_v96"
...
def _to_v95(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v95(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v95(item) for item in value]
    if isinstance(value, str):
        value = value.replace("v96", "v95")
        return value.replace("verification_state", "attestation_lineage")
    return value


def validate_verification_state(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v96 verification/state artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v96", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v96", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_verification_state_leaf_v96":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.verification_state_digest_sha256 does not match canonical v96 payload")

    translated = _to_v95(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 95
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 95
        translated["attestation_lineage_digest_sha256"] = _v95_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v95", "v96") for error in _validate_v95(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_VERIFICATION_STATE_V96_INVALID: {exc}")
        return 1
    errors = validate_verification_state(report)
    if errors:
        print("PLANETARY_REWARD_VERIFICATION_STATE_V96_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_VERIFICATION_STATE_V96_VALID: detached verification/state only")
    return 0
