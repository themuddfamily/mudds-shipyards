#!/usr/bin/env python3
"""Validate detached v346 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v345_validator import (
    _authority_digest as _v345_authority_digest,
    validate_objective_consistency as _validate_v345,
)

SCHEMA_VERSION = 346
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v346"
EVIDENCE_MODE = "detached_reward_objective_consistency_v346"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v346"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v346"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v346"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v346"
REQUIRED_AUTHORITY_VERSION = "authority_v346"
...
def _to_v345(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v345(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v345(item) for item in value]
    if isinstance(value, str):
        return value.replace("v346", "v345")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v346 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v346", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v346", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v346":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v346 payload")

    translated = _to_v345(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 345
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 345
        translated["objective_consistency_digest_sha256"] = _v345_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v345", "v346") for error in _validate_v345(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V346_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V346_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V346_VALID: detached objective consistency only")
    return 0
