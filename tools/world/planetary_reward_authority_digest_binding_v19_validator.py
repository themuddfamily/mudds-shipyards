#!/usr/bin/env python3
"""Validate detached v19 planetary reward authority/digest binding evidence."""
...
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v19"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v19"
...
def validate_authority_binding(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v19 authority-binding artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v19"), ("source", "detached_evidence")):
    ...
    if not isinstance(authority, dict):
        ...
        for key, expected in (("reward_authority_id", REQUIRED_REWARD_AUTHORITY), ("reward_store_id", REQUIRED_REWARD_STORE), ("authority_scope", "planetary_reward_binding"), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must use the bound reward authority and store")
        reward_id = record.get("reward_id")
        ...
        if leaf_id != f"{activity_id}_reward_binding_leaf_v19":
    ...
    if not isinstance(completeness, dict):
        ...
        expected = {
            ...
            "bound_records_observed": sum(record.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and record.get("reward_store_id") == REQUIRED_REWARD_STORE for record in records if isinstance(record, dict)),
            "all_fields_present": True,
            "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5,
            "complete": True,
        }
        for key, expected_value in expected.items():
            if completeness.get(key) != expected_value:
                errors.append(f"{label}.completeness.{key} must be {expected_value}")
    digest = value.get("binding_digest_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.binding_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != _binding_digest(identity.get("manifest_id"), identity.get("provenance_id"), authority, completeness, records):
        errors.append(f"{label}.binding_digest_sha256 does not match canonical authority binding payload")
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {"records": len(records), "references": len(all_refs), "bound_records": bound_records, "complete_records": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)), "store_writes": 0, "inventory_writes": 0, "runtime_mutations": 0, "native_runs": 0}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_BINDING_DIGEST_V19_INVALID: {exc}")
        return 1
    errors = validate_authority_binding(report)
    if errors:
        print("PLANETARY_REWARD_BINDING_DIGEST_V19_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_BINDING_DIGEST_V19_VALID: detached authority binding provenance only")
    return 0
