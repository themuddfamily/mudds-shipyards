#!/usr/bin/env python3
"""Validate detached v20 planetary reward authority reconciliation evidence."""
...
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v20"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v20"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_binding_reconciliation"
...
def validate_reconciliation(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v20 authority reconciliation artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v20"), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_reconciliation_leaf_v20":
    ...
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {
            ...
            "bound_records_observed": bound_records,
            "authority_ids_reconciled": bound_records == 5,
            "store_ids_reconciled": bound_records == 5,
        ...
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.reconciliation.{key} must be {expected_value}")
    digest = value.get("reconciliation_digest_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.reconciliation_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != _reconciliation_digest(identity.get("manifest_id"), identity.get("provenance_id"), authority, reconciliation, records):
        errors.append(f"{label}.reconciliation_digest_sha256 does not match canonical reconciliation payload")
    ...
    if not isinstance(counts, dict):
        ...
        bound_records = sum(record.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and record.get("reward_store_id") == REQUIRED_REWARD_STORE for record in records if isinstance(record, dict))
        expected_counts = {"records": len(records), "references": len(all_refs), "bound_records": bound_records, "reconciled_records": bound_records, "complete_records": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)), "store_writes": 0, "inventory_writes": 0, "runtime_mutations": 0, "native_runs": 0}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_RECONCILIATION_V20_INVALID: {exc}")
        return 1
    errors = validate_reconciliation(report)
    if errors:
        print("PLANETARY_REWARD_RECONCILIATION_V20_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_RECONCILIATION_V20_VALID: detached authority reconciliation provenance only")
    return 0
