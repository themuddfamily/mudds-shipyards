#!/usr/bin/env python3
"""Validate detached v38 planetary reward authority/reconciliation digest."""
...
SCHEMA_VERSION = 38
EVIDENCE_SCOPE = "planetary_reward_authority_reconciliation_digest_v38"
EVIDENCE_MODE = "detached_reward_authority_reconciliation_v38"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v38"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v38"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v38"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_authority_reconciliation_v38"
REQUIRED_DIGEST_VERSION = "authority_reconciliation_v38"
...
def _digest(authority: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> str:
...
def validate_authority_digest(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v38 authority/reconciliation digest."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v38"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_authority_reconciliation_leaf_v38":
    ...
    expected_digest = _digest(authority, reconciliation, records)
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.authority_reconciliation_digest_sha256 does not match canonical v38 payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V38_INVALID: {exc}")
        return 1
    errors = validate_authority_digest(report)
    if errors:
        print("PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V38_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V38_VALID: detached authority/reconciliation digest only")
    return 0
