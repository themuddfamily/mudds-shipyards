#!/usr/bin/env python3
"""Validate detached v23 planetary reward provenance identity reconciliation."""
...
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v23"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v23"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v23"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_provenance_identity"
...
def _identity_digest(
    ...
    payload = {
        "identity": {
            ...
            "provenance_id": identity.get("provenance_id"),
...
def validate_identity_reconciliation(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v23 provenance identity artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v23"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_identity_leaf_v23":
    ...
    if not isinstance(reconciliation, dict):
        ...
        expected = {
            ...
            "identity_records_reconciled": reconciled,
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.identity_reconciliation_digest_sha256 does not match canonical identity payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_IDENTITY_RECONCILIATION_V23_INVALID: {exc}")
        return 1
    errors = validate_identity_reconciliation(report)
    if errors:
        print("PLANETARY_REWARD_IDENTITY_RECONCILIATION_V23_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_IDENTITY_RECONCILIATION_V23_VALID: detached identity provenance only")
    return 0
