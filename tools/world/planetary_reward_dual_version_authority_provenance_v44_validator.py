#!/usr/bin/env python3
"""Validate detached v44 planetary reward dual-version authority/provenance evidence."""
...
SCHEMA_VERSION = 44
EVIDENCE_SCOPE = "planetary_reward_dual_version_authority_provenance_v44"
EVIDENCE_MODE = "detached_reward_dual_version_authority_provenance_v44"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v44"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v44"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v44"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_dual_version_authority_provenance"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v44"
REQUIRED_PROVENANCE_LINK_ID = "planetary_reward_manifest_provenance_link_v44"
REQUIRED_AUTHORITY_VERSION = "authority_v44"
REQUIRED_PROVENANCE_VERSION = "provenance_v44"
...
def validate_dual_version(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v44 dual-version artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v44"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to dual-version authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_dual_version_leaf_v44":
    ...
    digest = value.get("dual_version_digest_sha256")
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.dual_version_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.dual_version_digest_sha256 does not match canonical dual-version payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_DUAL_VERSION_V44_INVALID: {exc}")
        return 1
    errors = validate_dual_version(report)
    if errors:
        print("PLANETARY_REWARD_DUAL_VERSION_V44_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_DUAL_VERSION_V44_VALID: detached dual-version authority/provenance only")
    return 0
