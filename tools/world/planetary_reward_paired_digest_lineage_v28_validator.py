#!/usr/bin/env python3
"""Validate detached v28 planetary reward paired-digest lineage evidence."""
...
SCHEMA_VERSION = 28
EVIDENCE_SCOPE = "planetary_reward_paired_digest_lineage_v28"
EVIDENCE_MODE = "detached_reward_paired_digest_lineage_v28"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v28"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v28"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v28"
REQUIRED_LINEAGE_ROOT = "detached_reward_evidence_root_v28"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_paired_digest_lineage"
...
def _lineage_digests(identity: dict[str, Any], authority: dict[str, Any], lineage: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> tuple[str, str, str]:
    identity_digest = _digest({"identity": {key: identity.get(key) for key in ("manifest_id", "provenance_id", "manifest_version", "lineage_id", "evidence_ref")}, "records": [{key: record.get(key) for key in ("activity_id", "manifest_id", "provenance_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records]})
    ...
    lineage_digest = _digest({"lineage": lineage, "identity_digest_sha256": identity_digest, "authority_digest_sha256": authority_digest, "reconciliation": reconciliation})
    return identity_digest, authority_digest, lineage_digest


def validate_lineage(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v28 paired-digest lineage artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v28"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the lineage authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_paired_lineage_leaf_v28":
    ...
    if not isinstance(lineage, dict):
        ...
        for key, expected in (("lineage_id", REQUIRED_LINEAGE_ID), ("lineage_root", REQUIRED_LINEAGE_ROOT), ("source", "detached_evidence")):
            if lineage.get(key) != expected:
                errors.append(f"{label}.lineage.{key} must be {expected}")
        lineage_ref = lineage.get("evidence_ref")
        if not _text(lineage_ref) or not lineage_ref.startswith("res://"):
            errors.append(f"{label}.lineage.evidence_ref must be a res:// path")
        if lineage_ref in all_refs:
            errors.append(f"{label}.lineage.evidence_ref must be unique")

    reconciliation = value.get("paired_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.paired_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {"algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "manifest_id": REQUIRED_MANIFEST_ID, "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "records_expected": 5, "records_observed": len(records), "references_expected": 6, "references_observed": len(all_refs), "records_reconciled": reconciled, "lineage_reconciled": lineage.get("lineage_id") == REQUIRED_LINEAGE_ID and lineage.get("lineage_root") == REQUIRED_LINEAGE_ROOT, "identity_digest_present": True, "authority_digest_present": True, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.paired_reconciliation.{key} must be {expected_value}")
        ...
        if not _text(reconciliation_ref) or not reconciliation_ref.startswith("res://"):
            errors.append(f"{label}.paired_reconciliation.evidence_ref must be a res:// path")
        if reconciliation_ref in all_refs:
            errors.append(f"{label}.paired_reconciliation.evidence_ref must be unique")

    expected_identity, expected_authority, expected_lineage = _lineage_digests(identity, authority, lineage, reconciliation, records)
    for key, expected_value in (("identity_digest_sha256", expected_identity), ("authority_digest_sha256", expected_authority), ("lineage_digest_sha256", expected_lineage)):
        ...
        if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            ...
            errors.append(f"{label}.{key} does not match canonical lineage digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_PAIRED_LINEAGE_V28_INVALID: {exc}")
        return 1
    errors = validate_lineage(report)
    if errors:
        print("PLANETARY_REWARD_PAIRED_LINEAGE_V28_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_PAIRED_LINEAGE_V28_VALID: detached paired digest lineage only")
    return 0
