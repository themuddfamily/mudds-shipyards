#!/usr/bin/env python3
"""Validate detached v29 planetary reward paired lineage-root evidence."""
...
SCHEMA_VERSION = 29
EVIDENCE_SCOPE = "planetary_reward_paired_lineage_root_v29"
EVIDENCE_MODE = "detached_reward_paired_lineage_root_v29"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v29"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v29"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v29"
REQUIRED_LINEAGE_ROOT = "detached_reward_evidence_root_v29"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_paired_lineage_root"
...
def _root_digests(identity: dict[str, Any], authority: dict[str, Any], root: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> tuple[str, str, str]:
    ...
    root_digest = _digest({"lineage_root": root, "identity_digest_sha256": identity_digest, "authority_digest_sha256": authority_digest, "reconciliation": reconciliation})
    return identity_digest, authority_digest, root_digest


def validate_lineage_root(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v29 paired lineage-root artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v29"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the lineage-root authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_lineage_root_leaf_v29":
    ...
    if not isinstance(reconciliation, dict):
        ...
        expected = {"algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "manifest_id": REQUIRED_MANIFEST_ID, "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "root_id": REQUIRED_LINEAGE_ROOT, "records_expected": 5, "records_observed": len(records), "references_expected": 6, "references_observed": len(all_refs), "records_reconciled": reconciled, "root_reconciled": root.get("root_id") == REQUIRED_LINEAGE_ROOT, "identity_digest_present": True, "authority_digest_present": True, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
    ...
    expected_identity, expected_authority, expected_root = _root_digests(identity, authority, root, reconciliation, records)
    for key, expected_value in (("identity_digest_sha256", expected_identity), ("authority_digest_sha256", expected_authority), ("lineage_root_digest_sha256", expected_root)):
        ...
        if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            ...
            errors.append(f"{label}.{key} does not match canonical lineage-root digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_LINEAGE_ROOT_V29_INVALID: {exc}")
        return 1
    errors = validate_lineage_root(report)
    if errors:
        print("PLANETARY_REWARD_LINEAGE_ROOT_V29_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_LINEAGE_ROOT_V29_VALID: detached paired lineage-root only")
    return 0
