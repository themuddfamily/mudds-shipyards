#!/usr/bin/env python3
"""Validate detached v32 planetary reward paired root/authority digest evidence."""
...
SCHEMA_VERSION = 32
EVIDENCE_SCOPE = "planetary_reward_paired_root_authority_digest_v32"
EVIDENCE_MODE = "detached_reward_paired_root_authority_digest_v32"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v32"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v32"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v32"
REQUIRED_LINEAGE_ROOT = "detached_reward_evidence_root_v32"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_paired_root_authority_digest"
REQUIRED_DIGEST_VERSION = "paired_root_authority_v32"
...
def _paired_digests(identity: dict[str, Any], authority: dict[str, Any], root: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> tuple[str, str, str]:
    root_digest = _digest({"digest_version": REQUIRED_DIGEST_VERSION, "lineage_root": root, "identity": {key: identity.get(key) for key in ("manifest_id", "provenance_id", "manifest_version", "lineage_id", "evidence_ref")}})
    authority_digest = _digest({"digest_version": REQUIRED_DIGEST_VERSION, "authority": {key: authority.get(key) for key in ("reward_authority_id", "reward_store_id", "authority_scope")}, "records": [{key: record.get(key) for key in ("activity_id", "activity_authority_id", "reward_authority_id", "reward_store_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records]})
    pair_digest = _digest({"digest_version": REQUIRED_DIGEST_VERSION, "lineage_root_digest_sha256": root_digest, "authority_digest_sha256": authority_digest, "reconciliation": reconciliation})
    return root_digest, authority_digest, pair_digest


def validate_paired_digest(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v32 paired root/authority digest artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v32"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the paired digest authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_root_authority_digest_leaf_v32":
    ...
    reconciliation = value.get("paired_digest_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.paired_digest_reconciliation must be an object")
        reconciliation = {}
    else:
    ...
    for key, expected_value in (("lineage_root_digest_sha256", expected_root), ("authority_digest_sha256", expected_authority), ("paired_root_authority_digest_sha256", expected_pair)):
        ...
        if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            ...
            errors.append(f"{label}.{key} does not match canonical v32 paired digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_PAIRED_DIGEST_V32_INVALID: {exc}")
        return 1
    errors = validate_paired_digest(report)
    if errors:
        print("PLANETARY_REWARD_PAIRED_DIGEST_V32_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_PAIRED_DIGEST_V32_VALID: detached paired root/authority digest only")
    return 0
