#!/usr/bin/env python3
"""Validate detached v18 reward completeness and authority digest evidence."""
...
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v18"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v18"
...
def _unique(values: list[Any], label: str, errors: list[str]) -> None:
    try:
        ...
        duplicate = len({repr(value) for value in values}) != len(values)
    if duplicate:
...
def validate_completeness_authority(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v18 completeness/authority artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v18"), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_authority_leaf_v18":
    ...
    if not isinstance(completeness, dict):
        ...
        expected = {
            ...
            "authority_records_observed": sum(record.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY for record in records if isinstance(record, dict)),
    ...
    digest = value.get("authority_digest_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.authority_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != _authority_digest(identity.get("manifest_id"), identity.get("provenance_id"), authority, completeness, records):
        errors.append(f"{label}.authority_digest_sha256 does not match canonical authority payload")
    ...
    if not isinstance(counts, dict):
        ...
        expected_counts = {
            ...
            "authority_records": sum(record.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY for record in records if isinstance(record, dict)),
            "complete_records": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)),
            "store_writes": 0,
            "inventory_writes": 0,
            "runtime_mutations": 0,
            "native_runs": 0,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_AUTHORITY_DIGEST_V18_INVALID: {exc}")
        return 1
    errors = validate_completeness_authority(report)
    if errors:
        print("PLANETARY_REWARD_AUTHORITY_DIGEST_V18_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_AUTHORITY_DIGEST_V18_VALID: detached completeness authority provenance only")
    return 0
