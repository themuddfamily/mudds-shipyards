#!/usr/bin/env python3
"""Validate detached v318 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v317_validator import (
    _authority_digest as _v317_authority_digest,
    validate_objective_consistency as _validate_v317,
)

SCHEMA_VERSION = 318
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v318"
EVIDENCE_MODE = "detached_reward_objective_consistency_v318"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v318"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v318"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v318"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v318"
REQUIRED_AUTHORITY_VERSION = "authority_v318"
...
def _to_v317(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v317(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v317(item) for item in value]
    if isinstance(value, str):
        return value.replace("v318", "v317")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v318 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v318", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v318", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v318":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v318 payload")

    translated = _to_v317(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 317
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 317
        translated["objective_consistency_digest_sha256"] = _v317_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v317", "v318") for error in _validate_v317(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V318_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V318_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V318_VALID: detached objective consistency only")
    return 0
