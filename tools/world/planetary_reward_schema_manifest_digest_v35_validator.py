#!/usr/bin/env python3
"""Validate detached v35 planetary reward schema/manifest digest evidence."""
...
SCHEMA_VERSION = 35
EVIDENCE_SCOPE = "planetary_reward_schema_manifest_digest_v35"
EVIDENCE_MODE = "detached_reward_schema_manifest_v35"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v35"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v35"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v35"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_schema_manifest"
REQUIRED_DIGEST_VERSION = "schema_manifest_v35"
...
def _manifest_digest(manifest: dict[str, Any], authority: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> str:
    payload = {
        "schema_version": SCHEMA_VERSION,
        "digest_version": REQUIRED_DIGEST_VERSION,
        "manifest": {key: manifest.get(key) for key in ("manifest_id", "manifest_version", "provenance_id", "lineage_id", "evidence_ref")},
        "authority": {key: authority.get(key) for key in ("reward_authority_id", "reward_store_id", "authority_scope")},
        "reconciliation": reconciliation,
        "records": [{key: record.get(key) for key in ("activity_id", "manifest_id", "provenance_id", "activity_authority_id", "reward_authority_id", "reward_store_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records],
    }
    encoded = json.dumps(payload, ensure_ascii=True, sort_keys=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(encoded).hexdigest()


def validate_schema_manifest(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v35 schema/manifest artifact."""
    ...
    if not isinstance(manifest, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v35"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_schema_manifest_leaf_v35":
    ...
    reconciliation = value.get("schema_manifest_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.schema_manifest_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {"schema_version": SCHEMA_VERSION, "digest_version": REQUIRED_DIGEST_VERSION, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v35", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "records_expected": 5, "records_observed": len(records), "references_expected": 6, "references_observed": len(all_refs), "records_reconciled": reconciled, "authority_reconciled": authority.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and authority.get("reward_store_id") == REQUIRED_REWARD_STORE, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.schema_manifest_reconciliation.{key} must be {expected_value}")
        if not _text(reconciliation.get("evidence_ref")) or not reconciliation["evidence_ref"].startswith("res://"):
            errors.append(f"{label}.schema_manifest_reconciliation.evidence_ref must be a res:// path")
        if reconciliation.get("evidence_ref") in all_refs:
            errors.append(f"{label}.schema_manifest_reconciliation.evidence_ref must be unique")

    digest = value.get("schema_manifest_digest_sha256")
    expected_digest = _manifest_digest(manifest, authority, reconciliation, records)
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.schema_manifest_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.schema_manifest_digest_sha256 does not match canonical schema/manifest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_SCHEMA_MANIFEST_V35_INVALID: {exc}")
        return 1
    errors = validate_schema_manifest(report)
    if errors:
        print("PLANETARY_REWARD_SCHEMA_MANIFEST_V35_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_SCHEMA_MANIFEST_V35_VALID: detached schema/manifest digest only")
    return 0
