#!/usr/bin/env python3
"""Validate detached v175 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v174_validator import (
    _authority_digest as _v174_authority_digest,
    validate_objective_consistency as _validate_v174,
)


SCHEMA_VERSION = 175
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v175"
EVIDENCE_MODE = "detached_reward_objective_consistency_v175"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v175"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v175"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v175"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v175"
REQUIRED_AUTHORITY_VERSION = "authority_v175"
...
def _to_v174(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v174(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v174(item) for item in value]
    if isinstance(value, str):
        return value.replace("v175", "v174")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v175 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v175", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v175", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v175":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v175 payload")

    translated = _to_v174(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 174
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 174
        translated["objective_consistency_digest_sha256"] = _v174_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v174", "v175") for error in _validate_v174(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V175_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V175_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V175_VALID: detached objective consistency only")
    return 0
