#!/usr/bin/env python3
"""Validate version-seventeen reward manifest digest completeness.

The v17 artifact proves complete coverage of five reward records and six
evidence references, then hashes that canonical completeness payload.  It is
detached evidence with no store, inventory, runtime, or native write.
"""
...
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v17"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v17"
...
def _completeness_digest(manifest_id: str, provenance_id: str, completeness: dict[str, Any], records: list[dict[str, Any]]) -> str:
    payload = {"manifest_id": manifest_id, "provenance_id": provenance_id, "completeness": completeness, "records": [{"activity_id": record.get("activity_id"), "reward_id": record.get("reward_id"), "leaf_id": record.get("leaf_id"), "evidence_ref": record.get("evidence_ref"), "status": record.get("status")} for record in records]}
    encoded = json.dumps(payload, ensure_ascii=True, sort_keys=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(encoded).hexdigest()


def validate_completeness(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v17 completeness provenance artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v17"), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_complete_leaf_v17":
    ...
    if not isinstance(completeness, dict):
        ...
        expected = {"records_expected": 5, "records_observed": len(records), "references_expected": 6, "references_observed": len(all_refs), "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if completeness.get(key) != expected_value:
                errors.append(f"{label}.completeness.{key} must be {expected_value}")
    digest = value.get("completeness_digest_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.completeness_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != _completeness_digest(identity.get("manifest_id"), identity.get("provenance_id"), completeness, records):
        errors.append(f"{label}.completeness_digest_sha256 does not match canonical completeness payload")
    counts = value.get("counts")
    if not isinstance(counts, dict):
        errors.append(f"{label}.counts must be an object")
    else:
        expected_counts = {"records": len(records), "references": len(all_refs), "complete_records": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)), "store_writes": 0, "inventory_writes": 0, "runtime_mutations": 0, "native_runs": 0}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_COMPLETENESS_V17_INVALID: {exc}")
        return 1
    errors = validate_completeness(report)
    if errors:
        print("PLANETARY_REWARD_COMPLETENESS_V17_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_COMPLETENESS_V17_VALID: detached completeness provenance only")
    return 0
