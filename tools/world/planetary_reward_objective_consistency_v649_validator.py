#!/usr/bin/env python3
"""Validate detached v649 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v648_validator import (
    _authority_digest as _v648_authority_digest,
    validate_objective_consistency as _validate_v648,
)

SCHEMA_VERSION = 649
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v649"
EVIDENCE_MODE = "detached_reward_objective_consistency_v649"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v649"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v649"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v649"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v649"
REQUIRED_AUTHORITY_VERSION = "authority_v649"
...
def _to_v648(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v648(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v648(item) for item in value]
    if isinstance(value, str):
        return value.replace("v649", "v648")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v649 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v649", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v649", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v649":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v649 payload")

    translated = _to_v648(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 648
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 648
        translated["objective_consistency_digest_sha256"] = _v648_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v648", "v649") for error in _validate_v648(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V649_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V649_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V649_VALID: detached objective consistency only")
    return 0
