#!/usr/bin/env python3
"""Validate version-fourteen reward manifest identity/count provenance.

The v14 artifact binds a detached manifest identity to five reward records and
an exact count contract.  It performs no store, inventory, runtime, or native
write and makes no historical/procedural claim.
"""
...
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v14"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v14"
...
def validate_identity(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v14 identity/count provenance artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        expected = {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v14", "world_id": REQUIRED_WORLD_ID, "provenance_id": REQUIRED_PROVENANCE_ID, "reward_authority_id": REQUIRED_REWARD_AUTHORITY, "reward_store_id": REQUIRED_REWARD_STORE, "source": "detached_evidence"}
        ...
        if identity.get("leaf_count") != 5:
            errors.append(f"{label}.identity.leaf_count must be five")
        if identity.get("status") != "PASS" or identity.get("committed_once") is not True:
            errors.append(f"{label}.identity must be PASS and committed exactly once")
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_count_leaf_v14":
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_IDENTITY_V14_INVALID: {exc}")
        return 1
    errors = validate_identity(report)
    if errors:
        print("PLANETARY_REWARD_IDENTITY_V14_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_IDENTITY_V14_VALID: detached identity/count provenance only")
    return 0
