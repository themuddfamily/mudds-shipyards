#!/usr/bin/env python3
"""Validate detached v656 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v655_validator import (
    _authority_digest as _v655_authority_digest,
    validate_objective_consistency as _validate_v655,
)

SCHEMA_VERSION = 656
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v656"
EVIDENCE_MODE = "detached_reward_objective_consistency_v656"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v656"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v656"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v656"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v656"
REQUIRED_AUTHORITY_VERSION = "authority_v656"
...
def _to_v655(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v655(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v655(item) for item in value]
    if isinstance(value, str):
        return value.replace("v656", "v655")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v656 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v656", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v656", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v656":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v656 payload")

    translated = _to_v655(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 655
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 655
        translated["objective_consistency_digest_sha256"] = _v655_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v655", "v656") for error in _validate_v655(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V656_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V656_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V656_VALID: detached objective consistency only")
    return 0
