#!/usr/bin/env python3
"""Validate detached v507 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v506_validator import (
    _authority_digest as _v506_authority_digest,
    validate_objective_consistency as _validate_v506,
)

SCHEMA_VERSION = 507
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v507"
EVIDENCE_MODE = "detached_reward_objective_consistency_v507"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v507"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v507"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v507"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v507"
REQUIRED_AUTHORITY_VERSION = "authority_v507"
...
def _to_v506(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v506(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v506(item) for item in value]
    if isinstance(value, str):
        return value.replace("v507", "v506")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v507 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v507", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v507", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v507":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v507 payload")

    translated = _to_v506(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 506
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 506
        translated["objective_consistency_digest_sha256"] = _v506_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v506", "v507") for error in _validate_v506(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V507_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V507_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V507_VALID: detached objective consistency only")
    return 0
