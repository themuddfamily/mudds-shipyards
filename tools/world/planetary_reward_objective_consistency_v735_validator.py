#!/usr/bin/env python3
"""Validate detached v735 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v734_validator import (
    _authority_digest as _v734_authority_digest,
    validate_objective_consistency as _validate_v734,
)

SCHEMA_VERSION = 735
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v735"
EVIDENCE_MODE = "detached_reward_objective_consistency_v735"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v735"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v735"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v735"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v735"
REQUIRED_AUTHORITY_VERSION = "authority_v735"
...
def _to_v734(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v734(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v734(item) for item in value]
    if isinstance(value, str):
        return value.replace("v735", "v734")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v735 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v735", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v735", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v735":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v735 payload")

    translated = _to_v734(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 734
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 734
        translated["objective_consistency_digest_sha256"] = _v734_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v734", "v735") for error in _validate_v734(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V735_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V735_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V735_VALID: detached objective consistency only")
    return 0
