#!/usr/bin/env python3
"""Validate detached v48 planetary reward dual-version authority evidence."""
...
SCHEMA_VERSION = 48
EVIDENCE_SCOPE = "planetary_reward_dual_version_authority_v48"
EVIDENCE_MODE = "detached_reward_dual_version_authority_v48"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v48"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v48"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v48"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_dual_version_authority"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v48"
REQUIRED_AUTHORITY_VERSION = "authority_v48"
...
def _authority_digest(identity: dict[str, Any], authority: dict[str, Any], link: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> str:
    payload = {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "identity": {key: identity.get(key) for key in ("manifest_id", "manifest_version", "provenance_id", "lineage_id", "evidence_ref")}, "authority": {key: authority.get(key) for key in ("reward_authority_id", "reward_store_id", "authority_scope", "source")}, "authority_link": link, "reconciliation": reconciliation, "records": [{key: record.get(key) for key in ("activity_id", "manifest_id", "provenance_id", "activity_authority_id", "reward_authority_id", "reward_store_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records]}
    return hashlib.sha256(json.dumps(payload, ensure_ascii=True, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()


def validate_dual_authority(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v48 dual-version authority artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v48"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    if not isinstance(link, dict):
        ...
        expected_link = {"authority_version": REQUIRED_AUTHORITY_VERSION, "link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "reward_authority_id": REQUIRED_REWARD_AUTHORITY, "reward_store_id": REQUIRED_REWARD_STORE, "source": "detached_evidence", "linked": True}
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to dual authority")
        ...
        if leaf_id != f"{activity_id}_reward_dual_authority_leaf_v48":
    ...
    all_refs = [identity.get("evidence_ref")] + refs + [link.get("evidence_ref")]
    _unique(all_refs, f"{label}.all_evidence_refs", errors)

    reconciliation = value.get("authority_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.authority_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v48", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "reward_authority_id": REQUIRED_REWARD_AUTHORITY, "reward_store_id": REQUIRED_REWARD_STORE, "algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "records_expected": 5, "records_observed": len(records), "references_expected": 7, "references_observed": len(all_refs), "records_reconciled": reconciled, "authority_link_reconciled": link.get("linked") is True, "authority_reconciled": authority.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and authority.get("reward_store_id") == REQUIRED_REWARD_STORE, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.authority_reconciliation.{key} must be {expected_value}")
        if not _text(reconciliation.get("evidence_ref")) or not reconciliation["evidence_ref"].startswith("res://"):
            errors.append(f"{label}.authority_reconciliation.evidence_ref must be a res:// path")
        if reconciliation.get("evidence_ref") in all_refs:
            errors.append(f"{label}.authority_reconciliation.evidence_ref must be unique")

    digest = value.get("dual_authority_digest_sha256")
    expected_digest = _authority_digest(identity, authority, link, reconciliation, records)
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.dual_authority_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.dual_authority_digest_sha256 does not match canonical v48 payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_DUAL_AUTHORITY_V48_INVALID: {exc}")
        return 1
    errors = validate_dual_authority(report)
    if errors:
        print("PLANETARY_REWARD_DUAL_AUTHORITY_V48_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_DUAL_AUTHORITY_V48_VALID: detached dual authority only")
    return 0
