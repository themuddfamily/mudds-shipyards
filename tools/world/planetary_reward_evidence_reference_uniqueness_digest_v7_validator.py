#!/usr/bin/env python3
"""Validate version-seven reward evidence-reference uniqueness and digest.

The v7 artifact covers five authored evidence references in canonical order.
It is detached evidence only: no reward store, inventory, runtime, or native
write is permitted.
"""
...
def _reference_digest(references: list[str]) -> str:
    encoded = json.dumps(references, ensure_ascii=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(encoded).hexdigest()


def validate_digest(value: Any, label: str = "digest") -> list[str]:
    """Return blocking errors for one v7 evidence-reference digest."""
    ...
    digest = value.get("references_digest_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.references_digest_sha256 must be a lowercase SHA-256 hex digest")
    ...
    if len(references) == len(REQUIRED_ACTIVITY_IDS) and all(_text(reference) for reference in references):
        if digest != _reference_digest(references):
            errors.append(f"{label}.references_digest_sha256 does not match canonical references")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_REFS_V7_INVALID: {exc}")
        return 1
    errors = validate_digest(report)
    if errors:
        print("PLANETARY_REWARD_REFS_V7_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_REFS_V7_VALID: detached reference digest only")
    return 0
