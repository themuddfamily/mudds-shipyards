#!/usr/bin/env python3
"""Validate detached v27 planetary reward paired-digest reconciliation."""
...
SCHEMA_VERSION = 27
EVIDENCE_SCOPE = "planetary_reward_paired_digest_reconciliation_v27"
EVIDENCE_MODE = "detached_reward_paired_digest_v27"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v27"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v27"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v27"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_paired_digest"
...
def _paired_digests(identity: dict[str, Any], authority: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> tuple[str, str, str]:
    identity_payload = {"identity": {key: identity.get(key) for key in ("manifest_id", "provenance_id", "manifest_version", "lineage_id", "evidence_ref")}, "records": [{key: record.get(key) for key in ("activity_id", "manifest_id", "provenance_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records]}
    authority_payload = {"authority": {key: authority.get(key) for key in ("reward_authority_id", "reward_store_id", "authority_scope")}, "records": [{key: record.get(key) for key in ("activity_id", "activity_authority_id", "reward_authority_id", "reward_store_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records]}
    identity_digest = _digest(identity_payload)
    authority_digest = _digest(authority_payload)
    pair_digest = _digest({"identity_digest_sha256": identity_digest, "authority_digest_sha256": authority_digest, "reconciliation": reconciliation})
    return identity_digest, authority_digest, pair_digest


def validate_paired_digests(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v27 paired-digest artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v27"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the paired authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_paired_digest_leaf_v27":
    ...
    if not isinstance(reconciliation, dict):
        ...
        expected = {"algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "manifest_id": REQUIRED_MANIFEST_ID, "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "records_expected": 5, "records_observed": len(records), "references_expected": 6, "references_observed": len(all_refs), "records_reconciled": reconciled, "identity_digest_present": True, "authority_digest_present": True, "pair_reconciled": True, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
    ...
    expected_identity, expected_authority, expected_pair = _paired_digests(identity, authority, reconciliation, records)
    for key, expected_value in (("identity_digest_sha256", expected_identity), ("authority_digest_sha256", expected_authority), ("paired_digest_sha256", expected_pair)):
        ...
        if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            ...
            errors.append(f"{label}.{key} does not match canonical paired digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_PAIRED_DIGEST_V27_INVALID: {exc}")
        return 1
    errors = validate_paired_digests(report)
    if errors:
        print("PLANETARY_REWARD_PAIRED_DIGEST_V27_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_PAIRED_DIGEST_V27_VALID: detached paired digest reconciliation only")
    return 0
