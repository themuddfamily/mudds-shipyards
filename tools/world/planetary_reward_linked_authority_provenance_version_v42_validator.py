#!/usr/bin/env python3
"""Validate detached v42 planetary reward linked authority/provenance/version evidence."""
...
SCHEMA_VERSION = 42
EVIDENCE_SCOPE = "planetary_reward_linked_authority_provenance_version_v42"
EVIDENCE_MODE = "detached_reward_linked_authority_provenance_version_v42"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v42"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v42"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v42"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_linked_authority_provenance_version"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v42"
REQUIRED_PROVENANCE_LINK_ID = "planetary_reward_manifest_provenance_link_v42"
REQUIRED_LINK_VERSION = "authority_provenance_link_v42"
...
def validate_linked_version(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v42 versioned linked artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v42"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_linked_version_leaf_v42":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.linked_authority_provenance_version_digest_sha256 does not match canonical v42 payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_LINKED_VERSION_V42_INVALID: {exc}")
        return 1
    errors = validate_linked_version(report)
    if errors:
        print("PLANETARY_REWARD_LINKED_VERSION_V42_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_LINKED_VERSION_V42_VALID: detached linked authority/provenance/version only")
    return 0
