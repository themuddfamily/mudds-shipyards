#!/usr/bin/env python3
"""Validate detached v109 planetary reward evidence/lineage evidence."""
...
from tools.world.planetary_reward_audit_state_v108_validator import (
    _authority_digest as _v108_authority_digest,
    validate_audit_state as _validate_v108,
)


SCHEMA_VERSION = 109
EVIDENCE_SCOPE = "planetary_reward_evidence_lineage_v109"
EVIDENCE_MODE = "detached_reward_evidence_lineage_v109"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v109"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v109"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v109"
...
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v109"
REQUIRED_AUTHORITY_VERSION = "authority_v109"
...
def _to_v108(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v108(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v108(item) for item in value]
    if isinstance(value, str):
        value = value.replace("v109", "v108")
        return value.replace("evidence_lineage", "audit_state")
    return value


def validate_evidence_lineage(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v109 evidence/lineage artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v109", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v109", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_evidence_lineage_leaf_v109":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.evidence_lineage_digest_sha256 does not match canonical v109 payload")

    translated = _to_v108(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 108
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 108
        translated["audit_state_digest_sha256"] = _v108_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v108", "v109") for error in _validate_v108(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_EVIDENCE_LINEAGE_V109_INVALID: {exc}")
        return 1
    errors = validate_evidence_lineage(report)
    if errors:
        print("PLANETARY_REWARD_EVIDENCE_LINEAGE_V109_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_EVIDENCE_LINEAGE_V109_VALID: detached evidence/lineage only")
    return 0
