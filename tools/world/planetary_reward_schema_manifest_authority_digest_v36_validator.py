#!/usr/bin/env python3
"""Validate detached v36 planetary reward schema/manifest authority digest."""
...
SCHEMA_VERSION = 36
EVIDENCE_SCOPE = "planetary_reward_schema_manifest_authority_digest_v36"
EVIDENCE_MODE = "detached_reward_schema_manifest_authority_v36"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v36"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v36"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v36"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_schema_manifest_authority"
REQUIRED_DIGEST_VERSION = "schema_manifest_authority_v36"
...
def _authority_digest(manifest: dict[str, Any], authority: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> str:
    payload = {"schema_version": SCHEMA_VERSION, "digest_version": REQUIRED_DIGEST_VERSION, "manifest": {key: manifest.get(key) for key in ("manifest_id", "manifest_version", "provenance_id", "lineage_id", "evidence_ref")}, "authority": {key: authority.get(key) for key in ("reward_authority_id", "reward_store_id", "authority_scope")}, "reconciliation": reconciliation, "records": [{key: record.get(key) for key in ("activity_id", "manifest_id", "provenance_id", "activity_authority_id", "reward_authority_id", "reward_store_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records]}
    return hashlib.sha256(json.dumps(payload, ensure_ascii=True, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()


def validate_schema_manifest_authority(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v36 schema/manifest authority artifact."""
    ...
    if not isinstance(manifest, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v36"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
            if manifest.get(key) != expected:
                errors.append(f"{label}.manifest.{key} must be {expected}")
        if not _text(manifest.get("evidence_ref")) or not manifest["evidence_ref"].startswith("res://"):
        ...
        for key in ("writes_store", "writes_inventory", "writes_runtime", "runs_native"):
            if manifest.get(key) is not False:
                errors.append(f"{label}.manifest.{key} must be false")
    ...
    for index, record in enumerate(records):
        ...
        if record.get("manifest_id") != manifest.get("manifest_id") or record.get("provenance_id") != manifest.get("provenance_id"):
            errors.append(f"{prefix} manifest/provenance IDs must match manifest")
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the manifest authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_schema_manifest_authority_leaf_v36":
    ...
    reconciliation = value.get("schema_manifest_authority_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.schema_manifest_authority_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {"schema_version": SCHEMA_VERSION, "digest_version": REQUIRED_DIGEST_VERSION, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v36", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "records_expected": 5, "records_observed": len(records), "references_expected": 6, "references_observed": len(all_refs), "records_reconciled": reconciled, "authority_reconciled": authority.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and authority.get("reward_store_id") == REQUIRED_REWARD_STORE, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.schema_manifest_authority_reconciliation.{key} must be {expected_value}")
        if not _text(reconciliation.get("evidence_ref")) or not reconciliation["evidence_ref"].startswith("res://"):
            errors.append(f"{label}.schema_manifest_authority_reconciliation.evidence_ref must be a res:// path")
        if reconciliation.get("evidence_ref") in all_refs:
            errors.append(f"{label}.schema_manifest_authority_reconciliation.evidence_ref must be unique")

    digest = value.get("schema_manifest_authority_digest_sha256")
    expected_digest = _authority_digest(manifest, authority, reconciliation, records)
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.schema_manifest_authority_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.schema_manifest_authority_digest_sha256 does not match canonical authority payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_SCHEMA_MANIFEST_AUTHORITY_V36_INVALID: {exc}")
        return 1
    errors = validate_schema_manifest_authority(report)
    if errors:
        print("PLANETARY_REWARD_SCHEMA_MANIFEST_AUTHORITY_V36_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_SCHEMA_MANIFEST_AUTHORITY_V36_VALID: detached schema/manifest authority digest only")
    return 0
