#!/usr/bin/env python3
"""Validate detached v104 planetary reward evidence/state evidence."""
...
from tools.world.planetary_reward_audit_lineage_v103_validator import (
    _authority_digest as _v103_authority_digest,
    validate_audit_lineage as _validate_v103,
)


SCHEMA_VERSION = 104
EVIDENCE_SCOPE = "planetary_reward_evidence_state_v104"
EVIDENCE_MODE = "detached_reward_evidence_state_v104"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v104"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v104"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v104"
...
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v104"
REQUIRED_AUTHORITY_VERSION = "authority_v104"
...
def _to_v103(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v103(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v103(item) for item in value]
    if isinstance(value, str):
        value = value.replace("v104", "v103")
        return value.replace("evidence_state", "audit_lineage")
    return value


def validate_evidence_state(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v104 evidence/state artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v104", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v104", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_evidence_state_leaf_v104":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.evidence_state_digest_sha256 does not match canonical v104 payload")

    translated = _to_v103(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 103
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 103
        translated["audit_lineage_digest_sha256"] = _v103_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v103", "v104") for error in _validate_v103(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_EVIDENCE_STATE_V104_INVALID: {exc}")
        return 1
    errors = validate_evidence_state(report)
    if errors:
        print("PLANETARY_REWARD_EVIDENCE_STATE_V104_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_EVIDENCE_STATE_V104_VALID: detached evidence/state only")
    return 0
