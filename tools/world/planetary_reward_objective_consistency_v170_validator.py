#!/usr/bin/env python3
"""Validate detached v170 planetary reward/objective consistency evidence."""
...
from tools.world.planetary_reward_objective_consistency_v169_validator import (
    _authority_digest as _v169_authority_digest,
    validate_objective_consistency as _validate_v169,
)


SCHEMA_VERSION = 170
EVIDENCE_SCOPE = "planetary_reward_objective_consistency_v170"
EVIDENCE_MODE = "detached_reward_objective_consistency_v170"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v170"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v170"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v170"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v170"
REQUIRED_AUTHORITY_VERSION = "authority_v170"
...
def _to_v169(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: _to_v169(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_to_v169(item) for item in value]
    if isinstance(value, str):
        return value.replace("v170", "v169")
    return value


def validate_objective_consistency(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v170 objective-consistency artifact."""
    ...
    sections = (
        ("identity", identity, {"manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v170", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
        ...
        ("authority_reconciliation", reconciliation, {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v170", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID}),
    )
    ...
    for index, record in enumerate(records[:5]):
        if isinstance(record, dict) and record.get("leaf_id") != f"{record.get('activity_id')}_reward_objective_consistency_leaf_v170":
    ...
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        ...
        errors.append(f"{label}.objective_consistency_digest_sha256 does not match canonical v170 payload")

    translated = _to_v169(copy.deepcopy(value))
    if isinstance(translated, dict) and isinstance(translated.get("records"), list):
        translated["schema_version"] = 169
        if isinstance(translated.get("authority_reconciliation"), dict):
            translated["authority_reconciliation"]["schema_version"] = 169
        translated["objective_consistency_digest_sha256"] = _v169_authority_digest(translated.get("identity", {}), translated.get("authority", {}), translated.get("authority_link", {}), translated.get("authority_reconciliation", {}), translated["records"])
    errors.extend(error.replace("v169", "v170") for error in _validate_v169(translated, label))
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V170_INVALID: {exc}")
        return 1
    errors = validate_objective_consistency(report)
    if errors:
        print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V170_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_OBJECTIVE_CONSISTENCY_V170_VALID: detached objective consistency only")
    return 0
