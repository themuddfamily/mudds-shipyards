#!/usr/bin/env python3
"""Validate detached v33 planetary reward versioned root/authority digest evidence."""
...
SCHEMA_VERSION = 33
EVIDENCE_SCOPE = "planetary_reward_versioned_root_authority_digest_v33"
EVIDENCE_MODE = "detached_reward_versioned_root_authority_v33"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v33"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v33"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v33"
REQUIRED_LINEAGE_ROOT = "detached_reward_evidence_root_v33"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_versioned_root_authority"
REQUIRED_DIGEST_VERSION = "paired_root_authority_v33"
...
def _versioned_digests(identity: dict[str, Any], authority: dict[str, Any], root: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> tuple[str, str, str]:
    version = {"schema_version": SCHEMA_VERSION, "manifest_version": "v33", "digest_version": REQUIRED_DIGEST_VERSION}
...
def validate_versioned_digest(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v33 versioned root/authority artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("provenance_id", REQUIRED_PROVENANCE_ID), ("manifest_version", "v33"), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the versioned authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_versioned_digest_leaf_v33":
    ...
    reconciliation = value.get("versioned_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.versioned_reconciliation must be an object")
        reconciliation = {}
    else:
    ...
    for key, expected_value in (("lineage_root_digest_sha256", expected_root), ("authority_digest_sha256", expected_authority), ("versioned_pair_digest_sha256", expected_pair)):
        ...
        if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
            ...
            errors.append(f"{label}.{key} does not match canonical versioned digest payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_VERSIONED_DIGEST_V33_INVALID: {exc}")
        return 1
    errors = validate_versioned_digest(report)
    if errors:
        print("PLANETARY_REWARD_VERSIONED_DIGEST_V33_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_VERSIONED_DIGEST_V33_VALID: detached versioned root/authority digest only")
    return 0
