#!/usr/bin/env python3
"""Validate detached v49 planetary reward authority reconciliation evidence."""
...
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v49"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v49"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v49"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_versioned_authority_reconciliation"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v49"
REQUIRED_AUTHORITY_VERSION = "authority_v49"
...
def _authority_digest(
    ...
    payload = {
        ...
        "records": [
            {key: record.get(key) for key in ("activity_id", "manifest_id", "provenance_id", "activity_authority_id", "reward_authority_id", "reward_store_id", "reward_id", "leaf_id", "evidence_ref", "status")}
...
def _flags(obj: dict[str, Any], prefix: str, errors: list[str]) -> None:
    for key in ("writes_store", "writes_inventory", "writes_runtime", "runs_native"):
        if obj.get(key) is not False:
            errors.append(f"{prefix}.{key} must be false")


def validate_authority_reconciliation(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v49 versioned authority artifact."""
    ...
    for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v49"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    _flags(identity, f"{label}.identity", errors)

    authority = value.get("authority") if isinstance(value.get("authority"), dict) else {}
    if not isinstance(value.get("authority"), dict):
    ...
    _flags(link, f"{label}.authority_link", errors)

    records_value = value.get("records")
    records = records_value if isinstance(records_value, list) else []
    if not isinstance(records_value, list) or len(records) != 5:
    ...
    for index, record in enumerate(records[:5]):
        ...
        expected_activity = REQUIRED_ACTIVITY_IDS[index]
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to versioned authority")
        ...
        if leaf_id != f"{activity_id}_reward_versioned_authority_leaf_v49":
    ...
    if not isinstance(value.get("authority_reconciliation"), dict):
    ...
    expected_reconciliation = {
        "schema_version": SCHEMA_VERSION,
        "authority_version": REQUIRED_AUTHORITY_VERSION,
        "authority_link_id": REQUIRED_AUTHORITY_LINK_ID,
        "manifest_id": REQUIRED_MANIFEST_ID,
        ...
        "references_observed": len(all_refs),
        "records_reconciled": reconciled,
        "authority_link_reconciled": link.get("linked") is True,
        "authority_reconciled": authority.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and authority.get("reward_store_id") == REQUIRED_REWARD_STORE,
        "all_fields_present": True,
        "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5,
        "complete": True,
    }
    for key, expected in expected_reconciliation.items():
        if reconciliation.get(key) != expected:
            errors.append(f"{label}.authority_reconciliation.{key} must be {expected}")
    ...
    digest = value.get("versioned_authority_digest_sha256")
    expected_digest = _authority_digest(identity, authority, link, reconciliation, records)
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.versioned_authority_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.versioned_authority_digest_sha256 does not match canonical v49 payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V49_INVALID: {exc}")
        return 1
    errors = validate_authority_reconciliation(report)
    if errors:
        print("PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V49_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V49_VALID: detached reconciliation only")
    return 0
