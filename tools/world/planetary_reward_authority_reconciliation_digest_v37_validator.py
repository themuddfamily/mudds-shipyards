#!/usr/bin/env python3
"""Validate detached v37 planetary reward authority/reconciliation digest."""
...
SCHEMA_VERSION = 37
EVIDENCE_SCOPE = "planetary_reward_authority_reconciliation_digest_v37"
EVIDENCE_MODE = "detached_reward_authority_reconciliation_v37"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v37"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v37"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v37"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_authority_reconciliation"
REQUIRED_DIGEST_VERSION = "authority_reconciliation_v37"
...
def _authority_reconciliation_digest(authority: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> str:
    payload = {"schema_version": SCHEMA_VERSION, "digest_version": REQUIRED_DIGEST_VERSION, "authority": {key: authority.get(key) for key in ("reward_authority_id", "reward_store_id", "authority_scope", "source")}, "reconciliation": reconciliation, "records": [{key: record.get(key) for key in ("activity_id", "manifest_id", "provenance_id", "activity_authority_id", "reward_authority_id", "reward_store_id", "reward_id", "leaf_id", "evidence_ref", "status")} for record in records]}
...
def validate_authority_reconciliation(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v37 authority/reconciliation artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v37"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if record.get("reward_authority_id") != REQUIRED_REWARD_AUTHORITY or record.get("reward_store_id") != REQUIRED_REWARD_STORE:
            errors.append(f"{prefix} must reconcile to the authority and store")
        ...
        if leaf_id != f"{activity_id}_reward_authority_reconciliation_leaf_v37":
    ...
    digest = value.get("authority_reconciliation_digest_sha256")
    expected_digest = _authority_reconciliation_digest(authority, reconciliation, records)
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.authority_reconciliation_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.authority_reconciliation_digest_sha256 does not match canonical authority/reconciliation payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V37_INVALID: {exc}")
        return 1
    errors = validate_authority_reconciliation(report)
    if errors:
        print("PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V37_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_AUTHORITY_RECONCILIATION_V37_VALID: detached authority reconciliation digest only")
    return 0
