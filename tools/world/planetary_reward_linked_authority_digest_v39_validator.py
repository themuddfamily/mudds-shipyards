#!/usr/bin/env python3
"""Validate detached v39 planetary reward linked-authority digest evidence."""
...
SCHEMA_VERSION = 39
EVIDENCE_SCOPE = "planetary_reward_linked_authority_digest_v39"
EVIDENCE_MODE = "detached_reward_linked_authority_v39"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v39"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v39"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v39"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_linked_authority"
REQUIRED_LINK_ID = "planetary_reward_manifest_authority_link_v39"
REQUIRED_DIGEST_VERSION = "linked_authority_v39"
...
def validate_linked_authority(value: Any, label: str = "manifest") -> list[str]:
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v39"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    link = value.get("authority_link")
    if not isinstance(link, dict):
        errors.append(f"{label}.authority_link must be an object")
        link = {}
    else:
        expected_link = {"link_id": REQUIRED_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "reward_authority_id": REQUIRED_REWARD_AUTHORITY, "reward_store_id": REQUIRED_REWARD_STORE, "source": "detached_evidence", "linked": True}
        for key, expected in expected_link.items():
            if link.get(key) != expected:
                errors.append(f"{label}.authority_link.{key} must be {expected}")
        if not _text(link.get("evidence_ref")) or not link["evidence_ref"].startswith("res://"):
            errors.append(f"{label}.authority_link.evidence_ref must be a res:// path")
        for key in ("writes_store", "writes_inventory", "writes_runtime", "runs_native"):
            if link.get(key) is not False:
                errors.append(f"{label}.authority_link.{key} must be false")
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_linked_authority_leaf_v39":
    ...
    reconciliation = value.get("linked_authority_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.linked_authority_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {"schema_version": SCHEMA_VERSION, "digest_version": REQUIRED_DIGEST_VERSION, "link_id": REQUIRED_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v39", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "reward_authority_id": REQUIRED_REWARD_AUTHORITY, "reward_store_id": REQUIRED_REWARD_STORE, "algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "records_expected": 5, "records_observed": len(records), "references_expected": 6, "references_observed": len(all_refs), "records_reconciled": reconciled, "link_reconciled": link.get("linked") is True, "authority_reconciled": authority.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and authority.get("reward_store_id") == REQUIRED_REWARD_STORE, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.linked_authority_reconciliation.{key} must be {expected_value}")
        if not _text(reconciliation.get("evidence_ref")) or not reconciliation["evidence_ref"].startswith("res://"):
            errors.append(f"{label}.linked_authority_reconciliation.evidence_ref must be a res:// path")
        if reconciliation.get("evidence_ref") in all_refs or reconciliation.get("evidence_ref") == link.get("evidence_ref"):
            errors.append(f"{label}.linked_authority_reconciliation.evidence_ref must be unique")

    digest = value.get("linked_authority_digest_sha256")
    expected_digest = _linked_digest(identity, authority, link, reconciliation, records)
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.linked_authority_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.linked_authority_digest_sha256 does not match canonical linked authority payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_LINKED_AUTHORITY_V39_INVALID: {exc}")
        return 1
    errors = validate_linked_authority(report)
    if errors:
        print("PLANETARY_REWARD_LINKED_AUTHORITY_V39_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_LINKED_AUTHORITY_V39_VALID: detached linked authority digest only")
    return 0
