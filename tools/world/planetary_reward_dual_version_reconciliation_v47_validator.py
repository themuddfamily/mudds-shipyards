#!/usr/bin/env python3
"""Validate detached v47 planetary reward dual-version reconciliation."""
...
SCHEMA_VERSION = 47
EVIDENCE_SCOPE = "planetary_reward_dual_version_reconciliation_v47"
EVIDENCE_MODE = "detached_reward_dual_version_reconciliation_v47"
REQUIRED_WORLD_ID = "ember_moon"
REQUIRED_MANIFEST_ID = "planetary_reward_manifest_v47"
REQUIRED_PROVENANCE_ID = "planetary_reward_provenance_v47"
REQUIRED_LINEAGE_ID = "planetary_reward_lineage_v47"
REQUIRED_REWARD_AUTHORITY = "game_flow_reward_authority"
REQUIRED_REWARD_STORE = "game_flow_reward_store"
REQUIRED_AUTHORITY_SCOPE = "planetary_reward_dual_version_reconciliation"
REQUIRED_AUTHORITY_LINK_ID = "planetary_reward_manifest_authority_link_v47"
REQUIRED_PROVENANCE_LINK_ID = "planetary_reward_manifest_provenance_link_v47"
REQUIRED_AUTHORITY_VERSION = "authority_v47"
REQUIRED_PROVENANCE_VERSION = "provenance_v47"
...
def _reconciliation_digest(identity: dict[str, Any], authority: dict[str, Any], authority_link: dict[str, Any], provenance_link: dict[str, Any], reconciliation: dict[str, Any], records: list[dict[str, Any]]) -> str:
...
def validate_reconciliation(value: Any, label: str = "manifest") -> list[str]:
    """Return blocking errors for one v47 dual-version reconciliation artifact."""
    ...
    if not isinstance(identity, dict):
        ...
        for key, expected in (("manifest_id", REQUIRED_MANIFEST_ID), ("manifest_version", "v47"), ("provenance_id", REQUIRED_PROVENANCE_ID), ("lineage_id", REQUIRED_LINEAGE_ID), ("source", "detached_evidence")):
    ...
    for index, record in enumerate(records):
        ...
        if leaf_id != f"{activity_id}_reward_dual_version_reconciliation_leaf_v47":
    ...
    reconciliation = value.get("dual_version_reconciliation")
    if not isinstance(reconciliation, dict):
        errors.append(f"{label}.dual_version_reconciliation must be an object")
        reconciliation = {}
    else:
        ...
        expected = {"schema_version": SCHEMA_VERSION, "authority_version": REQUIRED_AUTHORITY_VERSION, "provenance_version": REQUIRED_PROVENANCE_VERSION, "authority_link_id": REQUIRED_AUTHORITY_LINK_ID, "provenance_link_id": REQUIRED_PROVENANCE_LINK_ID, "manifest_id": REQUIRED_MANIFEST_ID, "manifest_version": "v47", "provenance_id": REQUIRED_PROVENANCE_ID, "lineage_id": REQUIRED_LINEAGE_ID, "reward_authority_id": REQUIRED_REWARD_AUTHORITY, "reward_store_id": REQUIRED_REWARD_STORE, "algorithm": "sha256", "canonicalization": "json_sort_keys_compact", "records_expected": 5, "records_observed": len(records), "references_expected": 8, "references_observed": len(all_refs), "records_reconciled": reconciled, "dual_links_reconciled": links["authority_link"].get("linked") is True and links["provenance_link"].get("linked") is True, "authority_reconciled": authority.get("reward_authority_id") == REQUIRED_REWARD_AUTHORITY and authority.get("reward_store_id") == REQUIRED_REWARD_STORE, "all_fields_present": True, "all_status_pass": sum(record.get("status") == "PASS" for record in records if isinstance(record, dict)) == 5, "complete": True}
        for key, expected_value in expected.items():
            if reconciliation.get(key) != expected_value:
                errors.append(f"{label}.dual_version_reconciliation.{key} must be {expected_value}")
        if not _text(reconciliation.get("evidence_ref")) or not reconciliation["evidence_ref"].startswith("res://"):
            errors.append(f"{label}.dual_version_reconciliation.evidence_ref must be a res:// path")
        if reconciliation.get("evidence_ref") in all_refs:
            errors.append(f"{label}.dual_version_reconciliation.evidence_ref must be unique")

    digest = value.get("dual_version_reconciliation_digest_sha256")
    expected_digest = _reconciliation_digest(identity, authority, links["authority_link"], links["provenance_link"], reconciliation, records)
    if not isinstance(digest, str) or len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
        errors.append(f"{label}.dual_version_reconciliation_digest_sha256 must be a lowercase SHA-256 hex digest")
    elif records and digest != expected_digest:
        errors.append(f"{label}.dual_version_reconciliation_digest_sha256 does not match canonical v47 payload")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"PLANETARY_REWARD_DUAL_RECONCILIATION_V47_INVALID: {exc}")
        return 1
    errors = validate_reconciliation(report)
    if errors:
        print("PLANETARY_REWARD_DUAL_RECONCILIATION_V47_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("PLANETARY_REWARD_DUAL_RECONCILIATION_V47_VALID: detached dual-version reconciliation only")
    return 0
