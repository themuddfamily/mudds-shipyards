#!/usr/bin/env python3
"""Validate v60 caption source/channel/release evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_source_channel_release_reconciliation_v59_evidence_v1"
SCHEMA_VERSION = "v60"
PREVIOUS_VERSION = "v59"
RELEASE_ID = "caption-accessibility-v60"
...
def validate_source_channel_release(value: Any) -> list[str]:
    """Return source/channel/release violations without raising."""
    if not isinstance(value, dict):
        return ["source/channel/release record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "source_channel_release_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("source_channel_release") != SOURCE_CHANNEL_RELEASE:
        errors.append("source_channel_release must exactly match v60 source, channel, release, and stale policy")
    ...
    for key, expected in (("source_id", SOURCE_ID), ("source_mode", "exact"), ("channel_id", CHANNEL_ID), ("channel_mode", "exact"), ("release_id", RELEASE_ID), ("release_status", "unreleased"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"source/channel/release unreadable: {exc}"]
    return validate_source_channel_release(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("source_channel_release", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.source_channel_release)
    if errors:
        print("ACCESSIBILITY_CAPTION_SOURCE_CHANNEL_RELEASE_V60_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_SOURCE_CHANNEL_RELEASE_V60_READY: human review remains open")
    return 0
