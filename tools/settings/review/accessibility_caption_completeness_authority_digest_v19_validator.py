#!/usr/bin/env python3
"""Validate v19 completeness/authority digest metadata for captions."""
...
COMPLETENESS_FIELDS = ("source_schema", "source_revision", "manifest", "generation", "authority")
COMPLETENESS = {"field_count": 5, "present_count": 5, "missing_count": 0, "complete": True}
...
def validate_digest(value: Any) -> list[str]:
    ...
    if value.get("canonicalization") != "utf8_json_sorted_keys_no_whitespace_v19":
        errors.append("canonicalization must be utf8_json_sorted_keys_no_whitespace_v19")
    if value.get("digest_scope") != "completeness_authority":
        errors.append("digest_scope must be completeness_authority")
    ...
    if value.get("completeness_fields") != list(COMPLETENESS_FIELDS):
        errors.append("completeness_fields must exactly match source schema, revision, manifest, generation, authority")
    if value.get("completeness") != COMPLETENESS:
        errors.append("completeness must exactly match five present fields and zero missing")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_COMPLETENESS_AUTHORITY_DIGEST_V19_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_COMPLETENESS_AUTHORITY_DIGEST_V19_READY: human review remains open")
    return 0
