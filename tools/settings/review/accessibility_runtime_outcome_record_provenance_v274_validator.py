#!/usr/bin/env python3
"""Validate v274 accessibility runtime outcome-record provenance evidence."""
from __future__ import annotations
import argparse,json,re
from pathlib import Path
from typing import Any
SCHEMA="accessibility_runtime_outcome_record_provenance_v274_evidence_v1"; SOURCE_SCHEMA="runtime_accessibility_outcome_record_policy_v1"; SCHEMA_VERSION="v274"; SOURCE_ID="runtime-accessibility-outcome-record-policy"; CONTRACT_ID="runtime-accessibility-presentation"; OPEN_REVIEW_STATUSES={"pending","not_performed","in_progress","failed"}; RECORD_STATUSES={"planned","pending","not_performed"}; EVIDENCE_KINDS={"log","image","report","video"}; SHA256=re.compile(r"^[0-9a-f]{64}$")
...
def validate_runtime_outcome_record_provenance(v:Any)->list[str]:
 ...
 if v.get("outcome_policy")!=OUTCOME_POLICY:e.append("outcome_policy must exactly match the v274 outcome-record policy")
 if v.get("binding")!=BINDING:e.append("binding must exactly match the v274 outcome, human, and native policy")
...
def main(argv:list[str]|None=None)->int:
 ...
 if e:print("ACCESSIBILITY_RUNTIME_OUTCOME_RECORD_PROVENANCE_V274_INVALID");print("\n".join(f"- {x}" for x in e));return 1
 print("ACCESSIBILITY_RUNTIME_OUTCOME_RECORD_PROVENANCE_V274_READY: review and native gates remain open");return 0
