#!/usr/bin/env python3
"""Validate v79 accessibility-caption consistency/lineage evidence."""
...
SCHEMA = "accessibility_caption_consistency_lineage_v79_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_audit_state_v78_evidence_v1"
SCHEMA_VERSION = "v79"
PREVIOUS_VERSION = "v78"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v78"
CONSISTENCY_ID = "caption-accessibility-consistency-v79"
...
BINDING = {
    "consistency_scope": "audit_to_lineage",
    "source_id": SOURCE_ID,
...
def validate_consistency_lineage(value: Any) -> list[str]:
    """Return consistency/lineage violations without raising."""
    if not isinstance(value, dict):
        return ["consistency/lineage record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "consistency_claimed", "lineage_claimed", "consistency_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v78 source, authority, stale, human, and native policy")
    if value.get("consistency_lineage") != CONSISTENCY_LINEAGE:
        errors.append("consistency_lineage must exactly match v79 consistency, source, and open-gate policy")
    if value.get("consistency_checks") != CONSISTENCY_CHECKS:
        errors.append("consistency_checks must exactly match the bounded v79 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"consistency/lineage unreadable: {exc}"]
    return validate_consistency_lineage(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("consistency_lineage", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.consistency_lineage)
    if errors:
        print("ACCESSIBILITY_CAPTION_CONSISTENCY_LINEAGE_V79_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_CONSISTENCY_LINEAGE_V79_READY: consistency remains unverified")
    return 0
