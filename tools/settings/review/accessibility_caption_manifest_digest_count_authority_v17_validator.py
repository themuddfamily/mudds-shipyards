#!/usr/bin/env python3
"""Validate v17 digest/count authority metadata for caption manifests."""
...
DIGEST_INPUTS = ("source_schema", "source_revision", "manifest_entries", "generation", "authority")
...
def validate_manifest(value: Any) -> list[str]:
    ...
    if value.get("canonicalization") != "utf8_json_sorted_keys_no_whitespace_v17":
        errors.append("canonicalization must be utf8_json_sorted_keys_no_whitespace_v17")
    if value.get("digest_scope") != "manifest_digest_count_authority":
        errors.append("digest_scope must be manifest_digest_count_authority")
    ...
    if value.get("digest_inputs") != list(DIGEST_INPUTS):
        errors.append("digest_inputs must exactly match source schema, revision, manifest entries, generation, authority")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_MANIFEST_DIGEST_COUNT_V17_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_MANIFEST_DIGEST_COUNT_V17_READY: human review remains open")
    return 0
