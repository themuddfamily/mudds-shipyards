...
from tools.settings.review.accessibility_caption_provenance_state_v82_validator import (
    AUTHORITY,
    BINDING,
    CONTRACT_ID,
    PREVIOUS_VERSION,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-provenance-v82",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v82 provenance/state review or native render has been performed",
        ...
        "provenance_state": copy.deepcopy(PROVENANCE_STATE),
...
class AccessibilityCaptionProvenanceStateV82Tests(unittest.TestCase):
    def test_complete_record_keeps_provenance_unverified_and_native_gate_closed(self):
        self.assertEqual(validate_provenance_state(_record()), [])

    def test_v81_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_release_state_v80_evidence_v1"
        value["source_release_id"] = "caption-accessibility-release-v80"
        value["provenance_state"]["previous_version"] = "v80"
        errors = validate_provenance_state(value)
        self.assertTrue(any("source_schema must be" in error for error in errors))
        self.assertTrue(any("source_release_id must be" in error for error in errors))
        self.assertTrue(any("provenance_state must exactly" in error for error in errors))
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v82.json", "sha256": "bad"}]
        errors = validate_provenance_state(value)
