#!/usr/bin/env python3
"""Validate v16 count/digest authority metadata for caption manifests."""
...
SCHEMA = "accessibility_caption_manifest_count_digest_authority_v16_evidence_v1"
...
COUNTS = {"manifest_entries": 3, "digest_inputs": 5, "observed_entries": 0, "issue_entries": 0}
DIGEST_INPUTS = ("source_schema", "source_revision", "entries", "generation", "authority")
...
def validate_manifest(value: Any) -> list[str]:
    ...
    if value.get("canonicalization") != "utf8_json_sorted_keys_no_whitespace_v16":
        errors.append("canonicalization must be utf8_json_sorted_keys_no_whitespace_v16")
    if value.get("digest_scope") != "manifest_count_authority":
        errors.append("digest_scope must be manifest_count_authority")
    ...
    if value.get("counts") != COUNTS:
        errors.append("counts must exactly match the frozen manifest/digest counts")
    if value.get("digest_inputs") != list(DIGEST_INPUTS):
        errors.append("digest_inputs must exactly match source schema, revision, entries, generation, authority")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_MANIFEST_COUNT_DIGEST_V16_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_MANIFEST_COUNT_DIGEST_V16_READY: human review remains open")
    return 0
