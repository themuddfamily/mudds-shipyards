#!/usr/bin/env python3
"""Validate v69 accessibility-caption evidence/lineage evidence."""
...
SCHEMA = "accessibility_caption_evidence_lineage_v69_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_audit_closure_v68_evidence_v1"
SCHEMA_VERSION = "v69"
PREVIOUS_VERSION = "v68"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v68"
LINEAGE_ID = "caption-accessibility-lineage-v69"
...
BINDING = {
    "lineage_scope": "evidence_to_source",
    "source_id": SOURCE_ID,
...
def validate_evidence_lineage(value: Any) -> list[str]:
    ...
    for key in ("human_review_performed", "native_render_performed", "evidence_claimed", "lineage_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v68 source, authority, stale, human, and native policy")
    if value.get("evidence_lineage") != EVIDENCE_LINEAGE:
        errors.append("evidence_lineage must exactly match v69 lineage, evidence, and open-gate policy")
    if value.get("lineage_checks") != LINEAGE_CHECKS:
        errors.append("lineage_checks must exactly match the bounded v69 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_EVIDENCE_LINEAGE_V69_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_EVIDENCE_LINEAGE_V69_READY: evidence remains unverified")
    return 0
