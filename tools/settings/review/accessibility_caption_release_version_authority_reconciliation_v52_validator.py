#!/usr/bin/env python3
"""Validate v52 caption release/version authority reconciliation evidence."""
...
SCHEMA = "accessibility_caption_release_version_authority_reconciliation_v52_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_versioned_authority_reconciliation_v51_evidence_v1"
SCHEMA_VERSION = "v52"
PREVIOUS_VERSION = "v51"
RELEASE_ID = "caption-accessibility-v52"
...
RELEASE_VERSION_AUTHORITY_RECONCILIATION = {
...
def validate_release_version_authority_reconciliation(value: Any) -> list[str]:
    """Return release/version violations without raising on malformed values."""
    if not isinstance(value, dict):
        return ["release/version authority reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "release_version_authority_reconciliation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("release_version_authority_reconciliation") != RELEASE_VERSION_AUTHORITY_RECONCILIATION:
        errors.append("release_version_authority_reconciliation must exactly match v52 release, version, authority, and stale policy")
    ...
    for key, expected in (("release_id", RELEASE_ID), ("release_status", "unreleased"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("authority_mode", "exact"), ("reconciliation_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"release/version authority reconciliation unreadable: {exc}"]
    return validate_release_version_authority_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("release_version_authority_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.release_version_authority_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_RELEASE_VERSION_AUTHORITY_RECONCILIATION_V52_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_RELEASE_VERSION_AUTHORITY_RECONCILIATION_V52_READY: human review remains open")
    return 0
