#!/usr/bin/env python3
"""Validate v173 accessibility runtime tooltip provenance evidence."""
...
SCHEMA = "accessibility_runtime_tooltip_provenance_v173_evidence_v1"
SOURCE_SCHEMA = "runtime_tooltip_policy_v1"
SCHEMA_VERSION = "v173"
SOURCE_ID = "runtime-tooltip-policy"
...
TOOLTIP_POLICY = {
    "sequence": ["focus", "delay", "announce", "dismiss"],
    "delay_ms": 500,
    "max_lines": 4,
    "preserved_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
    "missing_text": "omit_without_placeholder_mutation",
    "dismissal": "focus_or_timeout",
...
def validate_runtime_tooltip_provenance(value: Any) -> list[str]:
    """Return tooltip provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime tooltip provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "overlay_written", "tooltip_replayed"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("tooltip_policy") != TOOLTIP_POLICY:
        errors.append("tooltip_policy must exactly match the v173 timing/content policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v173 overlay, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/tooltip boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_tooltip_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime tooltip provenance unreadable: {exc}"]
    return validate_runtime_tooltip_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_TOOLTIP_PROVENANCE_V173_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_TOOLTIP_PROVENANCE_V173_READY: review and native gates remain open")
    return 0
