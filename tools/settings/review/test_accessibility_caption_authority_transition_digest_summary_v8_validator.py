...
from tools.settings.review.accessibility_caption_authority_transition_digest_summary_v8_validator import (
    AUTHORITY,
    SOURCE_SCHEMA,
    TRANSITION_IDS,
    TRANSITION_RULES,
...
def _summary() -> dict:
    transitions = [{
        ...
        "expected_behavior": f"The {transition_id} authority transition remains deterministic.",
    } for transition_id in TRANSITION_IDS]
    return {
        "schema": "accessibility_caption_authority_transition_digest_summary_v8_evidence_v1",
        "source_schema": SOURCE_SCHEMA,
        "source_revision": "working-tree-caption-authority-transition-v8",
        "summary_path": "reports/caption-authority-transition-v8.json",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v8 authority transition review or native render has been performed",
        ...
        "canonicalization": "utf8_json_sorted_keys_no_whitespace_v8",
        "status": "planned",
        "digest": "0" * 64,
...
class AccessibilityCaptionAuthorityTransitionDigestV8Tests(unittest.TestCase):
    ...
    def test_transition_rules_and_policies_are_exact(self):
        value = _summary()
        value["transitions"][1]["target"] = "accepted"
        value["generation_policy"] = "consumer_owned"
        value["stale_policy"] = "accept_old"
        errors = validate_summary(value)
        self.assertTrue(any("transitions[1].target" in error for error in errors))
    ...
    def test_digest_and_evidence_formats_are_validated(self):
        value = _summary()
        value["digest"] = "bad"
        value["evidence"] = [{"kind": "report", "path": "reports/authority-transition-v8.json", "sha256": "bad"}]
        errors = validate_summary(value)
    ...
    def test_malformed_values_fail_without_throwing(self):
        ...
        self.assertTrue(any("exactly six ordered" in error for error in errors))
        self.assertTrue(any("authority must exactly" in error for error in errors))
