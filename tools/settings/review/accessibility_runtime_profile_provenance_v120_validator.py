#!/usr/bin/env python3
"""Validate v120 accessibility runtime profile provenance evidence."""
...
SCHEMA = "accessibility_runtime_profile_provenance_v120_evidence_v1"
...
PROVENANCE = {
    "source_of_truth": "settings_accessibility_matrix",
    "profile_owner": PROFILE_OWNER,
    "runtime_consumer": CONTRACT_ID,
    "dimension_owners": DIMENSION_OWNERS,
    "coverage_mode": "exact_dimension_to_contract",
    "human_boundary": "review_required",
    "native_boundary": "not_run",
}
BINDING = {
...
def _paths(value: Any, prefix: str, errors: list[str]) -> None:
    if not isinstance(value, list) or not value:
        errors.append(f"{prefix} must be a non-empty unique list of paths")
        return
    seen: set[str] = set()
    for index, path in enumerate(value):
        if not _text(path):
            errors.append(f"{prefix}[{index}] must be non-empty text")
        elif path in seen:
            errors.append(f"{prefix} must contain unique paths")
        else:
            seen.add(path)
...
def validate_runtime_profile_provenance(value: Any) -> list[str]:
    """Return profile provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime profile provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "provenance_verified", "profile_claimed", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("required_dimensions") != REQUIRED_DIMENSIONS:
        errors.append("required_dimensions must exactly match the v120 accessibility profile")
    if value.get("dimension_owners") != DIMENSION_OWNERS:
        errors.append("dimension_owners must exactly map each profile dimension to its presentation contract")
    if value.get("provenance") != PROVENANCE:
        errors.append("provenance must exactly match the v120 profile ownership chain")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v120 source, stale, human, and native policy")
    ...
    for dimension in REQUIRED_DIMENSIONS:
        _paths(value.get("dimension_evidence", {}).get(dimension) if isinstance(value.get("dimension_evidence"), dict) else None, f"dimension_evidence.{dimension}", errors)
    if value.get("source_id") != SOURCE_ID:
        errors.append(f"source_id must be {SOURCE_ID}")
    if value.get("contract_id") != CONTRACT_ID:
        errors.append(f"contract_id must be {CONTRACT_ID}")
    if value.get("profile_owner") != PROFILE_OWNER:
        errors.append(f"profile_owner must be {PROFILE_OWNER}")
    if value.get("provenance_source_of_truth") != "settings_accessibility_matrix":
        errors.append("provenance_source_of_truth must be settings_accessibility_matrix")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime profile provenance unreadable: {exc}"]
    return validate_runtime_profile_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_PROFILE_PROVENANCE_V120_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_PROFILE_PROVENANCE_V120_READY: review and native gates remain open")
    return 0
