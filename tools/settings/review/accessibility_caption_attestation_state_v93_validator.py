#!/usr/bin/env python3
"""Validate v93 accessibility-caption attestation/state evidence."""
...
SCHEMA = "accessibility_caption_attestation_state_v93_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_audit_state_v92_evidence_v1"
SCHEMA_VERSION = "v93"
PREVIOUS_VERSION = "v92"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v92"
ATTESTATION_ID = "caption-accessibility-attestation-v93"
...
BINDING = {
    "attestation_scope": "audit_to_state",
    "source_id": SOURCE_ID,
...
def validate_attestation_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v92 source, authority, stale, human, and native policy")
    if value.get("attestation_state") != ATTESTATION_STATE:
        errors.append("attestation_state must exactly match v93 attestation, source, and open-gate policy")
    if value.get("attestation_checks") != ATTESTATION_CHECKS:
        errors.append("attestation_checks must exactly match the bounded v93 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_ATTESTATION_STATE_V93_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_ATTESTATION_STATE_V93_READY: attestation remains unverified")
    return 0
