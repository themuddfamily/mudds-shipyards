#!/usr/bin/env python3
"""Validate v78 accessibility-caption audit/state evidence."""
...
SCHEMA = "accessibility_caption_audit_state_v78_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_verification_closure_v77_evidence_v1"
SCHEMA_VERSION = "v78"
PREVIOUS_VERSION = "v77"
SOURCE_ID = "caption-presentation-service"
SOURCE_VERIFICATION_ID = "caption-accessibility-verification-v77"
AUDIT_ID = "caption-accessibility-audit-v78"
...
def validate_audit_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v77 source, authority, stale, human, and native policy")
    if value.get("audit_state") != AUDIT_STATE:
        errors.append("audit_state must exactly match v78 audit, state, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v78 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V78_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V78_READY: state remains unverified")
    return 0
