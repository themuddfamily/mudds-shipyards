...
from tools.settings.review.accessibility_caption_audit_state_v113_validator import (
    AUDIT_CHECKS,
    AUDIT_ID,
    AUDIT_STATE,
    AUTHORITY,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-audit-state-v113",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v113 audit/state review or native render has been performed",
...
class AccessibilityCaptionAuditStateV113Tests(unittest.TestCase):
    def test_complete_record_keeps_audit_unverified_and_native_gate_closed(self):
        self.assertEqual(validate_audit_state(_record()), [])

    def test_v112_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_evidence_closure_v111_evidence_v1"
        value["source_evidence_id"] = "caption-accessibility-evidence-v111"
        value["audit_state"]["previous_version"] = "v111"
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v113.json", "sha256": "bad"}]
        errors = validate_audit_state(value)
