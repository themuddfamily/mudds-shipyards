...
from tools.settings.review.accessibility_caption_versioned_authority_reconciliation_v50_validator import (
    AUTHORITY,
    BINDING,
    PREVIOUS_VERSION,
    SCHEMA_VERSION,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-versioned-authority-v50",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v50 authority/reconciliation review or native render has been performed",
...
class AccessibilityCaptionVersionedAuthorityReconciliationV50Tests(unittest.TestCase):
    ...
    def test_versions_authority_reconciliation_and_binding_are_exact(self):
        value = _record()
        value["current_version"] = "v49"
        value["previous_version"] = "v48"
        ...
        self.assertTrue(any("current_version must be v50" in error for error in errors))
        self.assertTrue(any("previous_version must be v49" in error for error in errors))
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v50.json", "sha256": "bad"}]
        errors = validate_versioned_authority_reconciliation(value)
