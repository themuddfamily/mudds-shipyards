#!/usr/bin/env python3
"""Validate v7 source-authority stale transition summaries for captions."""
...
TRANSITION_IDS = ("current_accept", "stale_generation_reject", "external_authority_reject", "reset_increment", "native_gate_open")
TRANSITION_RULES = {
    "current_accept": {"source": "current_generation", "target": "accepted_presentation", "reason": "current_generation"},
    "stale_generation_reject": {"source": "old_generation", "target": "rejected", "reason": "stale_generation"},
    "external_authority_reject": {"source": "external_authority", "target": "rejected", "reason": "authority_boundary"},
    "reset_increment": {"source": "current_generation", "target": "next_generation", "reason": "reset"},
    "native_gate_open": {"source": "review_summary", "target": "native_not_run", "reason": "open_gate"},
}
...
def _validate_transitions(value: Any) -> list[str]:
    ...
    if not isinstance(value, list):
        return ["transitions must contain exactly five ordered source-authority transitions"]
    if len(value) != len(TRANSITION_IDS):
        errors.append("transitions must contain exactly five ordered source-authority transitions")
    ids: list[str] = []
    for index, transition in enumerate(value):
        ...
        if transition_id not in TRANSITION_IDS:
            errors.append(f"{prefix}.id must be one of the frozen source-authority transitions")
        else:
            ...
            for key in ("source", "target", "reason"):
                if transition.get(key) != expected[key]:
                    errors.append(f"{prefix}.{key} must match its source-authority transition")
    ...
    if tuple(ids) != TRANSITION_IDS:
        errors.append("transitions must exactly match the frozen source-authority order")
    return errors
...
def validate_summary(value: Any) -> list[str]:
    ...
    if value.get("canonicalization") != "utf8_json_sorted_keys_no_whitespace_v7":
        errors.append("canonicalization must be utf8_json_sorted_keys_no_whitespace_v7")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_SOURCE_AUTHORITY_STALE_V7_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_SOURCE_AUTHORITY_STALE_V7_READY: human review remains open")
    return 0
