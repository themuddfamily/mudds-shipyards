#!/usr/bin/env python3
"""Validate v98 accessibility-caption audit/closure evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_verification_state_v97_evidence_v1"
SCHEMA_VERSION = "v98"
PREVIOUS_VERSION = "v97"
SOURCE_ID = "caption-presentation-service"
SOURCE_VERIFICATION_ID = "caption-accessibility-verification-v97"
AUDIT_ID = "caption-accessibility-audit-v98"
...
def validate_audit_closure(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v97 source, authority, stale, human, and native policy")
    if value.get("audit_closure") != AUDIT_CLOSURE:
        errors.append("audit_closure must exactly match v98 audit, closure, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v98 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_CLOSURE_V98_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_CLOSURE_V98_READY: closure remains open")
    return 0
