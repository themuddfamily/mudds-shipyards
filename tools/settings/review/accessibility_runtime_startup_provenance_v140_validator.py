#!/usr/bin/env python3
"""Validate v140 accessibility runtime startup provenance evidence."""
...
SCHEMA = "accessibility_runtime_startup_provenance_v140_evidence_v1"
SOURCE_SCHEMA = "runtime_settings_accessibility_handoff_v1"
SCHEMA_VERSION = "v140"
SOURCE_ID = "startup-loader-accessibility"
...
STARTUP_POLICY = {
    "settings_read": "before_first_presentation",
    "profile_validation": "before_live_apply",
    "safe_defaults": "on_missing_or_corrupt_profile",
    "presentation_apply": "after_validated_profile",
    "reentry_apply": "retained_generation_after_detach",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_startup_profile",
    "human_gate": "open",
...
def validate_runtime_startup_provenance(value: Any) -> list[str]:
    """Return startup provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime startup provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("startup_policy") != STARTUP_POLICY:
        errors.append("startup_policy must exactly match the v140 settings/read/validation/application order")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v140 startup, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "validated_startup_profile")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime startup provenance unreadable: {exc}"]
    return validate_runtime_startup_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_STARTUP_PROVENANCE_V140_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_STARTUP_PROVENANCE_V140_READY: review and native gates remain open")
    return 0
