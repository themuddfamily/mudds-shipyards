#!/usr/bin/env python3
"""Validate v100 accessibility-caption verification/closure evidence."""
...
SCHEMA = "accessibility_caption_verification_closure_v100_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_evidence_lineage_v99_evidence_v1"
SCHEMA_VERSION = "v100"
PREVIOUS_VERSION = "v99"
SOURCE_ID = "caption-presentation-service"
SOURCE_EVIDENCE_ID = "caption-accessibility-evidence-v99"
VERIFICATION_ID = "caption-accessibility-verification-v100"
...
BINDING = {
    "verification_scope": "evidence_to_closure",
    "source_id": SOURCE_ID,
...
def validate_verification_closure(value: Any) -> list[str]:
    """Return verification/closure violations without raising."""
    if not isinstance(value, dict):
        return ["verification/closure record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "verification_claimed", "closure_claimed", "verification_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v99 source, authority, stale, human, and native policy")
    if value.get("verification_closure") != VERIFICATION_CLOSURE:
        errors.append("verification_closure must exactly match v100 verification, closure, and open-gate policy")
    if value.get("verification_checks") != VERIFICATION_CHECKS:
        errors.append("verification_checks must exactly match the bounded v100 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"verification/closure unreadable: {exc}"]
    return validate_verification_closure(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("verification_closure", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.verification_closure)
    if errors:
        print("ACCESSIBILITY_CAPTION_VERIFICATION_CLOSURE_V100_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_VERIFICATION_CLOSURE_V100_READY: verification remains unverified")
    return 0
