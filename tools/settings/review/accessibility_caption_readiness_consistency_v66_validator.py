#!/usr/bin/env python3
"""Validate v66 accessibility-caption readiness/consistency evidence."""
...
SCHEMA = "accessibility_caption_readiness_consistency_v66_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_verification_attestation_v65_evidence_v1"
SCHEMA_VERSION = "v66"
PREVIOUS_VERSION = "v65"
SOURCE_ID = "caption-presentation-service"
SOURCE_ATTESTATION_ID = "caption-accessibility-v65"
READINESS_ID = "caption-accessibility-readiness-v66"
...
BINDING = {
    "check_scope": "readiness_and_contract_consistency",
    "source_id": SOURCE_ID,
...
READINESS_CONSISTENCY = {
    ...
    "source_attestation_status": "open",
...
def validate_readiness_consistency(value: Any) -> list[str]:
    """Return readiness/consistency violations without raising."""
    if not isinstance(value, dict):
        return ["readiness/consistency record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "readiness_claimed", "consistency_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("readiness_consistency") != READINESS_CONSISTENCY:
        errors.append("readiness_consistency must exactly match v66 continuity, authority, and open-gate policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v65 source, authority, generation, stale, and native policy")
    if value.get("consistency_checks") != CONSISTENCY_CHECKS:
        errors.append("consistency_checks must exactly match the bounded v66 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"readiness/consistency unreadable: {exc}"]
    return validate_readiness_consistency(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("readiness_consistency", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.readiness_consistency)
    if errors:
        print("ACCESSIBILITY_CAPTION_READINESS_CONSISTENCY_V66_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_READINESS_CONSISTENCY_V66_READY: readiness remains open")
    return 0
