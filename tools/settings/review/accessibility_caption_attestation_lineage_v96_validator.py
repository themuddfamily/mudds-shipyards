#!/usr/bin/env python3
"""Validate v96 accessibility-caption attestation/lineage evidence."""
...
SCHEMA = "accessibility_caption_attestation_lineage_v96_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_evidence_state_v95_evidence_v1"
SCHEMA_VERSION = "v96"
PREVIOUS_VERSION = "v95"
SOURCE_ID = "caption-presentation-service"
SOURCE_EVIDENCE_ID = "caption-accessibility-evidence-v95"
ATTESTATION_ID = "caption-accessibility-attestation-v96"
...
BINDING = {
    "attestation_scope": "evidence_to_lineage",
    "source_id": SOURCE_ID,
...
def validate_attestation_lineage(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v95 source, authority, stale, human, and native policy")
    if value.get("attestation_lineage") != ATTESTATION_LINEAGE:
        errors.append("attestation_lineage must exactly match v96 attestation, source, and open-gate policy")
    if value.get("attestation_checks") != ATTESTATION_CHECKS:
        errors.append("attestation_checks must exactly match the bounded v96 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_ATTESTATION_LINEAGE_V96_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_ATTESTATION_LINEAGE_V96_READY: attestation remains unverified")
    return 0
