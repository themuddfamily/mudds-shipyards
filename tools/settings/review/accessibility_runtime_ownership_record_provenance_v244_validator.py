#!/usr/bin/env python3
"""Validate v244 accessibility runtime ownership-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_ownership_record_provenance_v244_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_ownership_record_policy_v1"
SCHEMA_VERSION = "v244"
SOURCE_ID = "runtime-accessibility-ownership-record-policy"
...
OWNERSHIP_POLICY = {
    "sequence": ["declare_scope", "list_roles", "mark_gates", "defer_ownership"],
    "artifact_types": ["validator", "test", "manifest", "evidence"],
    "ownership_states": ["prepared", "pending_review", "deferred"],
    "owned_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
...
BINDING = {
    ...
    "policy_mode": "exact", "apply_rule": "provenance_ownership_only",
    "human_gate": "open", "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True, "ownership_record_authority": False,
...
def validate_runtime_ownership_record_provenance(value: Any) -> list[str]:
    """Return ownership-record provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime ownership-record provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "ownership_written", "ownership_assigned"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("ownership_policy") != OWNERSHIP_POLICY:
        errors.append("ownership_policy must exactly match the v244 ownership-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v244 ownership, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/ownership boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_accessibility_ownership_record_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime ownership-record provenance unreadable: {exc}"]
    return validate_runtime_ownership_record_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_OWNERSHIP_RECORD_PROVENANCE_V244_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_OWNERSHIP_RECORD_PROVENANCE_V244_READY: review and native gates remain open")
    return 0
