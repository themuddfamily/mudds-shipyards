#!/usr/bin/env python3
"""Validate v27 caption digest/reconciliation authority evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_identity_digest_reconciliation_authority_v26_evidence_v1"
...
def validate_digest_reconciliation(value: Any) -> list[str]:
    """Return contract violations without raising on malformed JSON shapes."""

    if not isinstance(value, dict):
        return ["digest/reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in (
        ...
        "digest_reconciliation_verified",
        "digest_verified",
    ...
    if value.get("digest_reconciliation") != DIGEST_RECONCILIATION:
        errors.append("digest_reconciliation must exactly match ownership, generation, and stale policy")
    ...
    if value.get("generation_owner") != "caption-presentation-service":
        errors.append("generation_owner must identify caption-presentation-service")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"digest/reconciliation unreadable: {exc}"]
    return validate_digest_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("digest_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.digest_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_DIGEST_RECONCILIATION_AUTHORITY_V27_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_DIGEST_RECONCILIATION_AUTHORITY_V27_READY: human review remains open")
    return 0
