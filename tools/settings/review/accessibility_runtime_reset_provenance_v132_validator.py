#!/usr/bin/env python3
"""Validate v132 accessibility runtime reset/migration provenance evidence."""
...
SCHEMA = "accessibility_runtime_reset_provenance_v132_evidence_v1"
...
RESET_MIGRATION_POLICY = {
    "schema_owner": "GameFlow/RuntimeSettings",
    "migration": "versioned_forward_only",
    "reset": "validated_accessibility_defaults",
    "unknown_fields": "reject_without_mutation",
    "corrupt_profile": "safe_defaults_without_secret_logging",
...
def validate_runtime_reset_provenance(value: Any) -> list[str]:
    """Return reset/migration provenance violations without raising."""
    ...
    if value.get("reset_migration_policy") != RESET_MIGRATION_POLICY:
        errors.append("reset_migration_policy must exactly match the v132 migration/reset boundary")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v132 profile, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "validated_profile_boundary")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_RESET_PROVENANCE_V132_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_RESET_PROVENANCE_V132_READY: review and native gates remain open")
    return 0
