#!/usr/bin/env python3
"""Validate v87 accessibility-caption audit/closure evidence."""
...
SCHEMA = "accessibility_caption_audit_closure_v87_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_verification_lineage_v86_evidence_v1"
SCHEMA_VERSION = "v87"
PREVIOUS_VERSION = "v86"
SOURCE_ID = "caption-presentation-service"
SOURCE_VERIFICATION_ID = "caption-accessibility-verification-v86"
AUDIT_ID = "caption-accessibility-audit-v87"
...
BINDING = {
    "audit_scope": "verification_to_closure",
    "source_id": SOURCE_ID,
...
def validate_audit_closure(value: Any) -> list[str]:
    ...
    for key in ("human_review_performed", "native_render_performed", "audit_claimed", "closure_claimed", "audit_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v86 source, authority, stale, human, and native policy")
    if value.get("audit_closure") != AUDIT_CLOSURE:
        errors.append("audit_closure must exactly match v87 audit, closure, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v87 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_CLOSURE_V87_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_CLOSURE_V87_READY: closure remains open")
    return 0
