#!/usr/bin/env python3
"""Validate v165 accessibility runtime remap provenance evidence."""
...
SCHEMA = "accessibility_runtime_remap_provenance_v165_evidence_v1"
SOURCE_SCHEMA = "runtime_input_remap_policy_v1"
SCHEMA_VERSION = "v165"
SOURCE_ID = "runtime-input-remap-policy"
...
REMAP_POLICY = {
    "sequence": ["capture", "preview", "confirm"],
    "preserved_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
    "preview_scope": "presentation_controls_only",
    "invalid_binding": "reject_without_partial_write",
    "cancel_behavior": "restore_last_valid_mapping",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "write_rule": "human_confirmed_external_only",
    "human_gate": "open",
    "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True,
    "remap_authority": False,
...
def validate_runtime_remap_provenance(value: Any) -> list[str]:
    """Return remap provenance violations without raising."""
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "mapping_written", "remap_replayed"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("remap_policy") != REMAP_POLICY:
        errors.append("remap_policy must exactly match the v165 capture/preview/confirm policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v165 write, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/remap boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_input_remap_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_REMAP_PROVENANCE_V165_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_REMAP_PROVENANCE_V165_READY: review and native gates remain open")
    return 0
