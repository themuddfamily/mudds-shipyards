#!/usr/bin/env python3
"""Validate v106 accessibility-caption provenance/closure evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_evidence_state_v105_evidence_v1"
SCHEMA_VERSION = "v106"
PREVIOUS_VERSION = "v105"
SOURCE_ID = "caption-presentation-service"
SOURCE_EVIDENCE_ID = "caption-accessibility-evidence-v105"
PROVENANCE_ID = "caption-accessibility-provenance-v106"
...
BINDING = {
    "provenance_scope": "evidence_to_closure",
    "source_id": SOURCE_ID,
...
def validate_provenance_closure(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v105 source, authority, stale, human, and native policy")
    if value.get("provenance_closure") != PROVENANCE_CLOSURE:
        errors.append("provenance_closure must exactly match v106 provenance, closure, and open-gate policy")
    if value.get("provenance_checks") != PROVENANCE_CHECKS:
        errors.append("provenance_checks must exactly match the bounded v106 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_PROVENANCE_CLOSURE_V106_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PROVENANCE_CLOSURE_V106_READY: provenance remains unverified")
    return 0
