...
from tools.settings.review.accessibility_runtime_tutorial_provenance_v193_validator import (
    AUTHORITY,
    BINDING,
    CONTRACT_ID,
    SCHEMA,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-runtime-tutorial-v193",
        "reviewer_required": "human accessibility and tutorial QA",
        ...
        "step_written": False,
        "tutorial_replayed": False,
        "tutorial_policy": copy.deepcopy(TUTORIAL_POLICY),
        "binding": copy.deepcopy(BINDING),
        "authority": copy.deepcopy(AUTHORITY),
        "source_id": SOURCE_ID,
        "contract_id": CONTRACT_ID,
        "provenance_source_of_truth": "runtime_tutorial_policy",
        "status": "planned",
        "evidence": None,
        **AUTHORITY,
    }
...
class AccessibilityRuntimeTutorialProvenanceV193Tests(unittest.TestCase):
    ...
    def test_tutorial_policy_and_binding_are_exact(self):
        value = _record()
        value["tutorial_policy"]["missing_step"] = "show_secret"
    ...
    def test_preserved_accessibility_fields_cannot_shrink(self):
        value = _record()
        value["tutorial_policy"]["preserved_fields"] = ["camera"]
        errors = validate_runtime_tutorial_provenance(value)
        self.assertTrue(any("tutorial_policy must exactly" in error for error in errors))
    ...
    def test_tutorial_authority_fails_closed(self):
        value = _record()
        value["authority"]["tutorial_authority"] = True
        value["tutorial_authority"] = True
        errors = validate_runtime_tutorial_provenance(value)
        self.assertTrue(any("authority must exactly" in error for error in errors))
        self.assertTrue(any("tutorial_authority must be false" in error for error in errors))
