...
from tools.settings.review.accessibility_caption_provenance_identity_reconciliation_authority_v24_validator import (
    AUTHORITY,
    BINDING,
    DIGEST_AUTHORITY,
    IDENTITY_AUTHORITY,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-identity-authority-v24",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v24 identity review or native render has been performed",
        ...
        "identity_authority": copy.deepcopy(IDENTITY_AUTHORITY),
...
class AccessibilityCaptionProvenanceIdentityReconciliationAuthorityV24Tests(unittest.TestCase):
    def test_complete_record_keeps_review_and_native_gates_open(self):
        self.assertEqual(validate_identity_authority(_record()), [])

    def test_identity_provenance_digest_and_binding_are_exact(self):
        value = _record()
        value["identity_authority"]["canonical_source"] = "audio-service"
        ...
        errors = validate_identity_authority(value)
        self.assertTrue(any("identity_authority must exactly" in error for error in errors))
    ...
    def test_stale_and_non_presentation_authority_fail_closed(self):
        ...
        value["identity_reconciled"] = True
        value["stale_payload_mutation"] = True
        errors = validate_identity_authority(value)
        self.assertTrue(any("authority must exactly" in error for error in errors))
        self.assertTrue(any("audio_authority must be false" in error for error in errors))
        self.assertTrue(any("identity_reconciled must be false" in error for error in errors))
    ...
    def test_identity_digest_and_evidence_require_sha256_metadata(self):
        ...
        value["evidence"] = [{"kind": "report", "path": "reports/v24.json", "sha256": "bad"}]
        errors = validate_identity_authority(value)
