...
from tools.settings.review.accessibility_caption_verification_closure_v91_validator import (
    AUTHORITY,
    BINDING,
    CONTRACT_ID,
    PREVIOUS_VERSION,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-verification-v91",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v91 verification/closure review or native render has been performed",
        ...
        "verification_closure": copy.deepcopy(VERIFICATION_CLOSURE),
...
class AccessibilityCaptionVerificationClosureV91Tests(unittest.TestCase):
    def test_complete_record_keeps_verification_unverified_and_native_gate_closed(self):
        self.assertEqual(validate_verification_closure(_record()), [])

    def test_v90_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_source_lineage_v89_evidence_v1"
        value["source_identity"] = "caption-accessibility-source-v89"
        value["verification_closure"]["previous_version"] = "v89"
        errors = validate_verification_closure(value)
        self.assertTrue(any("source_schema must be" in error for error in errors))
        self.assertTrue(any("source_identity must be" in error for error in errors))
        self.assertTrue(any("verification_closure must exactly" in error for error in errors))
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v91.json", "sha256": "bad"}]
        errors = validate_verification_closure(value)
