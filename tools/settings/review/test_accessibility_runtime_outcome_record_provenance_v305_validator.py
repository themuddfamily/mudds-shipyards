...
class AccessibilityRuntimeOutcomeRecordProvenanceV305Tests(unittest.TestCase):
 ...
 def test_gates(self):v=rec();v["native_render_status"]="passed";v["outcome_written"]=True;self.assertTrue(validate_runtime_outcome_record_provenance(v))
 def test_authority(self):v=rec();v["authority"]=[];self.assertTrue(validate_runtime_outcome_record_provenance(v))
 def test_malformed(self):v=rec();v["evidence"]=[{"kind":[],"path":{},"sha256":[]}];self.assertTrue(validate_runtime_outcome_record_provenance(v))
