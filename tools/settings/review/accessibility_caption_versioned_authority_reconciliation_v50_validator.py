#!/usr/bin/env python3
"""Validate v50 versioned authority/reconciliation evidence."""
...
SCHEMA = "accessibility_caption_versioned_authority_reconciliation_v50_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_dual_version_authority_v49_evidence_v1"
SCHEMA_VERSION = "v50"
PREVIOUS_VERSION = "v49"
...
def validate_versioned_authority_reconciliation(value: Any) -> list[str]:
    """Return versioned authority/reconciliation violations without raising."""
    if not isinstance(value, dict):
        return ["versioned authority/reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "versioned_authority_reconciliation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("versioned_authority_reconciliation") != VERSIONED_AUTHORITY_RECONCILIATION:
        errors.append("versioned_authority_reconciliation must exactly match v50 versions, authority, reconciliation, and stale policy")
    ...
    for key, expected in (("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("authority_mode", "exact"), ("reconciliation_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"versioned authority/reconciliation unreadable: {exc}"]
    return validate_versioned_authority_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("versioned_authority_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.versioned_authority_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_VERSIONED_AUTHORITY_RECONCILIATION_V50_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_VERSIONED_AUTHORITY_RECONCILIATION_V50_READY: human review remains open")
    return 0
