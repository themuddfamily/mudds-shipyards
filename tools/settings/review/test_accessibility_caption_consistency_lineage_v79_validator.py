...
from tools.settings.review.accessibility_caption_consistency_lineage_v79_validator import (
    AUTHORITY,
    BINDING,
    CONSISTENCY_CHECKS,
    CONSISTENCY_ID,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-consistency-v79",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v79 consistency/lineage review or native render has been performed",
        ...
        "consistency_lineage": copy.deepcopy(CONSISTENCY_LINEAGE),
...
class AccessibilityCaptionConsistencyLineageV79Tests(unittest.TestCase):
    def test_complete_record_keeps_consistency_unverified_and_native_gate_closed(self):
        self.assertEqual(validate_consistency_lineage(_record()), [])

    def test_v78_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_audit_state_v77_evidence_v1"
        value["source_audit_id"] = "caption-accessibility-audit-v77"
        value["consistency_lineage"]["previous_version"] = "v77"
        errors = validate_consistency_lineage(value)
        ...
        self.assertTrue(any("consistency_lineage must exactly" in error for error in errors))
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v79.json", "sha256": "bad"}]
        errors = validate_consistency_lineage(value)
