#!/usr/bin/env python3
"""Validate v142 accessibility runtime focus/navigation provenance evidence."""
...
SCHEMA = "accessibility_runtime_focus_provenance_v142_evidence_v1"
...
FOCUS_POLICY = {
    "keyboard_navigation": "complete_focusable_prompt_roster",
    "controller_navigation": "complete_focusable_prompt_roster",
    "focus_visibility": "semantic_text_and_shape_state",
    "input_family_glyphs": "presentation_only",
    "controller_only_completion": "required_actions_have_controller_path",
    "focus_capture": "no_gameplay_authority",
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_focus_revision",
    "human_gate": "open",
...
def validate_runtime_focus_provenance(value: Any) -> list[str]:
    """Return focus/navigation provenance violations without raising."""
    ...
    if value.get("focus_policy") != FOCUS_POLICY:
        errors.append("focus_policy must exactly match the v142 keyboard/controller navigation boundary")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v142 focus, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/focus boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "focus_navigation_policy")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_FOCUS_PROVENANCE_V142_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_FOCUS_PROVENANCE_V142_READY: review and native gates remain open")
    return 0
