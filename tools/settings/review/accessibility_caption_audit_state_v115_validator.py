#!/usr/bin/env python3
"""Validate v115 accessibility-caption audit/state evidence."""
...
SCHEMA = "accessibility_caption_audit_state_v115_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_evidence_lineage_v114_evidence_v1"
SCHEMA_VERSION = "v115"
PREVIOUS_VERSION = "v114"
SOURCE_ID = "caption-presentation-service"
SOURCE_EVIDENCE_ID = "caption-accessibility-evidence-v114"
AUDIT_ID = "caption-accessibility-audit-v115"
...
def validate_audit_state(value: Any) -> list[str]:
    """Return audit/state violations without raising."""
    if not isinstance(value, dict):
        return ["audit/state record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "audit_claimed", "state_claimed", "audit_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v114 source, authority, stale, human, and native policy")
    if value.get("audit_state") != AUDIT_STATE:
        errors.append("audit_state must exactly match v115 audit, state, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v115 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"audit/state unreadable: {exc}"]
    return validate_audit_state(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate(args.audit_state)
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V115_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V115_READY: audit remains unverified")
    return 0
