#!/usr/bin/env python3
"""Validate v145 accessibility runtime fallback/error provenance evidence."""
...
SCHEMA = "accessibility_runtime_fallback_provenance_v145_evidence_v1"
...
FALLBACK_POLICY = {
    "malformed_profile": "reject_without_partial_apply",
    "malformed_cue": "reject_without_presentation_output",
    "invalid_viewport": "reject_without_layout_claim",
    "missing_audio_text": "caption_inaudible_fallback",
    "unknown_server_prompt": "stable_textual_prompt_set",
    "error_logging": "reason_and_field_only_no_payload",
...
BINDING = {
    ...
    "policy_mode": "exact_fail_closed",
    "stale_policy": "reject_stale_fallback_revision",
    "human_gate": "open",
    "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True,
    "error_recovery_authority": False,
...
def validate_runtime_fallback_provenance(value: Any) -> list[str]:
    """Return fallback/error provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime fallback provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("fallback_policy") != FALLBACK_POLICY:
        errors.append("fallback_policy must exactly match the v145 fail-closed and redaction policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v145 fallback, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/error boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "fail_closed_presentation")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime fallback provenance unreadable: {exc}"]
    return validate_runtime_fallback_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_FALLBACK_PROVENANCE_V145_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_FALLBACK_PROVENANCE_V145_READY: review and native gates remain open")
    return 0
