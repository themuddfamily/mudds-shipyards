#!/usr/bin/env python3
"""Validate v113 accessibility-caption audit/state evidence."""
...
SCHEMA = "accessibility_caption_audit_state_v113_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_evidence_closure_v112_evidence_v1"
SCHEMA_VERSION = "v113"
PREVIOUS_VERSION = "v112"
SOURCE_ID = "caption-presentation-service"
SOURCE_EVIDENCE_ID = "caption-accessibility-evidence-v112"
AUDIT_ID = "caption-accessibility-audit-v113"
...
def validate_audit_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v112 source, authority, stale, human, and native policy")
    if value.get("audit_state") != AUDIT_STATE:
        errors.append("audit_state must exactly match v113 audit, state, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v113 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V113_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V113_READY: audit remains unverified")
    return 0
