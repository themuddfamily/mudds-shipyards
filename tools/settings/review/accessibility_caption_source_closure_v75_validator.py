#!/usr/bin/env python3
"""Validate v75 accessibility-caption source/closure evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_audit_lineage_v74_evidence_v1"
SCHEMA_VERSION = "v75"
PREVIOUS_VERSION = "v74"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v74"
...
def validate_source_closure(value: Any) -> list[str]:
    """Return source/closure violations without raising."""
    if not isinstance(value, dict):
        return ["source/closure record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "source_claimed", "closure_claimed", "source_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v74 source, authority, stale, human, and native policy")
    if value.get("source_closure") != SOURCE_CLOSURE:
        errors.append("source_closure must exactly match v75 source, closure, and open-gate policy")
    if value.get("source_checks") != SOURCE_CHECKS:
        errors.append("source_checks must exactly match the bounded v75 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"source/closure unreadable: {exc}"]
    return validate_source_closure(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("source_closure", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.source_closure)
    if errors:
        print("ACCESSIBILITY_CAPTION_SOURCE_CLOSURE_V75_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_SOURCE_CLOSURE_V75_READY: source remains unverified")
    return 0
