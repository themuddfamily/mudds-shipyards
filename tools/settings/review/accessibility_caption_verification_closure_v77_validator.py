#!/usr/bin/env python3
"""Validate v77 accessibility-caption verification/closure evidence."""
...
SCHEMA = "accessibility_caption_verification_closure_v77_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_attestation_lineage_v76_evidence_v1"
SCHEMA_VERSION = "v77"
PREVIOUS_VERSION = "v76"
SOURCE_ID = "caption-presentation-service"
SOURCE_ATTESTATION_ID = "caption-accessibility-attestation-v76"
VERIFICATION_ID = "caption-accessibility-verification-v77"
...
BINDING = {
    "verification_scope": "attestation_to_closure",
    "source_id": SOURCE_ID,
...
def validate_verification_closure(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v76 source, authority, stale, human, and native policy")
    if value.get("verification_closure") != VERIFICATION_CLOSURE:
        errors.append("verification_closure must exactly match v77 verification, closure, and open-gate policy")
    if value.get("verification_checks") != VERIFICATION_CHECKS:
        errors.append("verification_checks must exactly match the bounded v77 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_VERIFICATION_CLOSURE_V77_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_VERIFICATION_CLOSURE_V77_READY: verification remains unverified")
    return 0
