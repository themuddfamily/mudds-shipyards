#!/usr/bin/env python3
"""Validate v95 accessibility-caption evidence/state evidence."""
...
SCHEMA = "accessibility_caption_evidence_state_v95_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_audit_lineage_v94_evidence_v1"
SCHEMA_VERSION = "v95"
PREVIOUS_VERSION = "v94"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v94"
EVIDENCE_ID = "caption-accessibility-evidence-v95"
...
def validate_evidence_state(value: Any) -> list[str]:
    """Return evidence/state violations without raising."""
    if not isinstance(value, dict):
        return ["evidence/state record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "evidence_claimed", "state_claimed", "evidence_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v94 source, authority, stale, human, and native policy")
    if value.get("evidence_state") != EVIDENCE_STATE:
        errors.append("evidence_state must exactly match v95 evidence, source, and open-gate policy")
    if value.get("evidence_checks") != EVIDENCE_CHECKS:
        errors.append("evidence_checks must exactly match the bounded v95 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"evidence/state unreadable: {exc}"]
    return validate_evidence_state(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("evidence_state", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.evidence_state)
    if errors:
        print("ACCESSIBILITY_CAPTION_EVIDENCE_STATE_V95_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_EVIDENCE_STATE_V95_READY: evidence remains unverified")
    return 0
