#!/usr/bin/env python3
"""Validate v86 accessibility-caption verification/lineage evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_attestation_state_v85_evidence_v1"
SCHEMA_VERSION = "v86"
PREVIOUS_VERSION = "v85"
SOURCE_ID = "caption-presentation-service"
SOURCE_ATTESTATION_ID = "caption-accessibility-attestation-v85"
VERIFICATION_ID = "caption-accessibility-verification-v86"
...
BINDING = {
    "verification_scope": "attestation_to_lineage",
    "source_id": SOURCE_ID,
...
def validate_verification_lineage(value: Any) -> list[str]:
    """Return verification/lineage violations without raising."""
    if not isinstance(value, dict):
        return ["verification/lineage record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "verification_claimed", "lineage_claimed", "verification_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v85 source, authority, stale, human, and native policy")
    if value.get("verification_lineage") != VERIFICATION_LINEAGE:
        errors.append("verification_lineage must exactly match v86 verification, source, and open-gate policy")
    if value.get("verification_checks") != VERIFICATION_CHECKS:
        errors.append("verification_checks must exactly match the bounded v86 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"verification/lineage unreadable: {exc}"]
    return validate_verification_lineage(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("verification_lineage", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.verification_lineage)
    if errors:
        print("ACCESSIBILITY_CAPTION_VERIFICATION_LINEAGE_V86_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_VERIFICATION_LINEAGE_V86_READY: verification remains unverified")
    return 0
