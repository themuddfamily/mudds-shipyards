#!/usr/bin/env python3
"""Validate v146 accessibility runtime state-cue provenance evidence."""
...
SCHEMA = "accessibility_runtime_state_cue_provenance_v146_evidence_v1"
...
STATE_CUE_POLICY = {
    "categories": ["info", "caution", "danger", "objective", "navigation"],
    "non_colour_channels": ["shape", "lightness", "text"],
    "state_source": "caller_observation",
    "caption_observation": "audibility_and_text_observation",
    "audio_observation": "audibility_only_no_playback",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_state_cue_revision",
    "human_gate": "open",
...
def validate_runtime_state_cue_provenance(value: Any) -> list[str]:
    """Return state-cue provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime state-cue provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("state_cue_policy") != STATE_CUE_POLICY:
        errors.append("state_cue_policy must exactly match the v146 semantic and non-colour state-cue policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v146 state-cue, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/state boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "state_cue_observation")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime state-cue provenance unreadable: {exc}"]
    return validate_runtime_state_cue_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_STATE_CUE_PROVENANCE_V146_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_STATE_CUE_PROVENANCE_V146_READY: review and native gates remain open")
    return 0
