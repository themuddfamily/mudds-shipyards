#!/usr/bin/env python3
"""Validate v94 accessibility-caption audit/lineage evidence."""
...
SCHEMA = "accessibility_caption_audit_lineage_v94_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_attestation_state_v93_evidence_v1"
SCHEMA_VERSION = "v94"
PREVIOUS_VERSION = "v93"
SOURCE_ID = "caption-presentation-service"
SOURCE_ATTESTATION_ID = "caption-accessibility-attestation-v93"
AUDIT_ID = "caption-accessibility-audit-v94"
...
def validate_audit_lineage(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v93 source, authority, stale, human, and native policy")
    if value.get("audit_lineage") != AUDIT_LINEAGE:
        errors.append("audit_lineage must exactly match v94 audit, source, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v94 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_LINEAGE_V94_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_LINEAGE_V94_READY: audit remains unverified")
    return 0
