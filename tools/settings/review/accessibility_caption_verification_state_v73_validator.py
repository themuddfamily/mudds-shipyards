#!/usr/bin/env python3
"""Validate v73 accessibility-caption verification/state evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_release_lineage_v72_evidence_v1"
SCHEMA_VERSION = "v73"
PREVIOUS_VERSION = "v72"
SOURCE_ID = "caption-presentation-service"
SOURCE_RELEASE_ID = "caption-accessibility-release-v72"
VERIFICATION_ID = "caption-accessibility-verification-v73"
...
BINDING = {
    "verification_scope": "release_to_state",
    "source_id": SOURCE_ID,
...
def validate_verification_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v72 source, authority, stale, human, and native policy")
    if value.get("verification_state") != VERIFICATION_STATE:
        errors.append("verification_state must exactly match v73 verification, source, and open-gate policy")
    if value.get("verification_checks") != VERIFICATION_CHECKS:
        errors.append("verification_checks must exactly match the bounded v73 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_VERIFICATION_STATE_V73_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_VERIFICATION_STATE_V73_READY: verification remains unverified")
    return 0
