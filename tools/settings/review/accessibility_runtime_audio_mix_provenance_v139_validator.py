#!/usr/bin/env python3
"""Validate v139 accessibility runtime audio-mix provenance evidence."""
...
SCHEMA = "accessibility_runtime_audio_mix_provenance_v139_evidence_v1"
...
AUDIO_MIX_POLICY = {
    "bus_groups": ["Master", "Ambience", "Engines", "Weapons", "UI", "Music"],
    "ceiling_db_range": [-80.0, 0.0],
    "accessibility_mute": "presentation_observation_only",
    "caption_fallback": "inaudible_observation_to_text",
    "music_state": "caller_owned_transition",
    "human_listening_review": "not_run",
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_audio_mix_revision",
    "human_gate": "open",
...
def validate_runtime_audio_mix_provenance(value: Any) -> list[str]:
    """Return audio-mix provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime audio-mix provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("audio_mix_policy") != AUDIO_MIX_POLICY:
        errors.append("audio_mix_policy must exactly match the v139 bus, ceiling, fallback, and review policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v139 audio, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "audio_mix_policy")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime audio-mix provenance unreadable: {exc}"]
    return validate_runtime_audio_mix_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_AUDIO_MIX_PROVENANCE_V139_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_AUDIO_MIX_PROVENANCE_V139_READY: review and native gates remain open")
    return 0
