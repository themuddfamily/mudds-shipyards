#!/usr/bin/env python3
"""Validate v31 paired root/authority evidence for captions."""
...
SOURCE_SCHEMA = "accessibility_caption_paired_lineage_root_authority_v30_evidence_v1"
...
def validate_paired_root_authority(value: Any) -> list[str]:
    """Return paired root/authority violations without raising on bad shapes."""

    if not isinstance(value, dict):
        return ["paired root/authority record must be an object"]
    errors: list[str] = []
    ...
    for key in (
        ...
        "paired_root_authority_verified",
        "digest_verified",
    ...
    if value.get("paired_root_authority") != PAIRED_ROOT_AUTHORITY:
        errors.append("paired_root_authority must exactly match root ownership and authority projection")
    ...
    if root_digest is not None and not _digest(root_digest):
        errors.append("root_digest must be null or a lowercase 64-character digest")
    if value.get("status") not in STATUSES:
        errors.append("status must remain planned, pending, or not_performed")
    for key in ("root_owner", "generation_owner"):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"paired root/authority unreadable: {exc}"]
    return validate_paired_root_authority(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("paired_root_authority", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.paired_root_authority)
    if errors:
        print("ACCESSIBILITY_CAPTION_PAIRED_ROOT_AUTHORITY_V31_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PAIRED_ROOT_AUTHORITY_V31_READY: human review remains open")
    return 0
