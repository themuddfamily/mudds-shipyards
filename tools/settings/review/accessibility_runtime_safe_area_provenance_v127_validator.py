#!/usr/bin/env python3
"""Validate v127 accessibility runtime safe-area/prompt provenance evidence."""
...
SCHEMA = "accessibility_runtime_safe_area_provenance_v127_evidence_v1"
...
SAFE_AREA_POLICY = {
    ...
    "base_margin_x": 32.0,
    "base_margin_top": 24.0,
    "base_margin_bottom": 42.0,
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_layout_revision",
    "human_gate": "open",
...
def validate_runtime_safe_area_provenance(value: Any) -> list[str]:
    """Return safe-area/prompt provenance violations without raising."""
    ...
    if value.get("safe_area_policy") != SAFE_AREA_POLICY:
        errors.append("safe_area_policy must exactly match the v127 aspect, scale, margin, and clipping policy")
    if value.get("prompt_fields") != PROMPT_FIELDS:
        errors.append("prompt_fields must exactly list the textual server-browser prompts")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v127 layout, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "safe_area_policy")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_SAFE_AREA_PROVENANCE_V127_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_SAFE_AREA_PROVENANCE_V127_READY: review and native gates remain open")
    return 0
