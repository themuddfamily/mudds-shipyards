#!/usr/bin/env python3
"""Validate v47 dual-version link/reconciliation evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_dual_version_authority_provenance_link_v46_evidence_v1"
SCHEMA_VERSION = "v47"
...
def validate_dual_version_link_reconciliation(value: Any) -> list[str]:
    """Return dual-version link/reconciliation violations without raising."""
    if not isinstance(value, dict):
        return ["dual-version link/reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "dual_version_link_reconciliation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("dual_version_link_reconciliation") != DUAL_VERSION_LINK_RECONCILIATION:
        errors.append("dual_version_link_reconciliation must exactly match v47 versions, link, reconciliation, and stale policy")
    ...
    for key, expected in (("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("link_mode", "exact"), ("reconciliation_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"dual-version link/reconciliation unreadable: {exc}"]
    return validate_dual_version_link_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("dual_version_link_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.dual_version_link_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_DUAL_VERSION_LINK_RECONCILIATION_V47_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_DUAL_VERSION_LINK_RECONCILIATION_V47_READY: human review remains open")
    return 0
