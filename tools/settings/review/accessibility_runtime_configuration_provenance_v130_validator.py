#!/usr/bin/env python3
"""Validate v130 accessibility runtime configuration provenance evidence."""
...
SCHEMA = "accessibility_runtime_configuration_provenance_v130_evidence_v1"
...
TRANSACTION_POLICY = {
    "profile_sections": ["visual", "captions", "audio"],
    "validation_order": "candidate_before_live_replace",
    "partial_apply": False,
    "revision_advance": "one_per_accepted_request",
    "rejection_behavior": "retain_previous_snapshot",
    "snapshot_copy": "deep_detached",
}
BINDING = {
    "source_schema": SOURCE_SCHEMA,
    "source_id": SOURCE_ID,
    "contract_id": CONTRACT_ID,
    "transaction_mode": "exact",
    "stale_policy": "reject_stale_configuration_revision",
    "human_gate": "open",
...
def validate_runtime_configuration_provenance(value: Any) -> list[str]:
    """Return configuration provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime configuration provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "transaction_verified", "runtime_claimed", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("transaction_policy") != TRANSACTION_POLICY:
        errors.append("transaction_policy must exactly match the v130 atomic configuration policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v130 transaction, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "configuration_transaction")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime configuration provenance unreadable: {exc}"]
    return validate_runtime_configuration_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_CONFIGURATION_PROVENANCE_V130_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_CONFIGURATION_PROVENANCE_V130_READY: review and native gates remain open")
    return 0
