#!/usr/bin/env python3
"""Validate v107 accessibility-caption consistency/state evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_provenance_closure_v106_evidence_v1"
SCHEMA_VERSION = "v107"
PREVIOUS_VERSION = "v106"
SOURCE_ID = "caption-presentation-service"
SOURCE_PROVENANCE_ID = "caption-accessibility-provenance-v106"
CONSISTENCY_ID = "caption-accessibility-consistency-v107"
...
def validate_consistency_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v106 source, authority, stale, human, and native policy")
    if value.get("consistency_state") != CONSISTENCY_STATE:
        errors.append("consistency_state must exactly match v107 consistency, state, and open-gate policy")
    if value.get("consistency_checks") != CONSISTENCY_CHECKS:
        errors.append("consistency_checks must exactly match the bounded v107 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_CONSISTENCY_STATE_V107_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_CONSISTENCY_STATE_V107_READY: consistency remains unverified")
    return 0
