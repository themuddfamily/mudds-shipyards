#!/usr/bin/env python3
"""Validate v126 accessibility runtime audio/caption provenance evidence."""
...
SCHEMA = "accessibility_runtime_audio_caption_provenance_v126_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_presentation_v1"
SCHEMA_VERSION = "v126"
SOURCE_ID = "audio-accessibility-preset"
...
AUDIO_CAPTION_POLICY = {
    "audibility": "caller_observation_only",
    "bus_ownership": "audio-accessibility-preset",
    "subtitle_modes": ["off", "key_cues", "all_cues"],
    "inaudible_text_fallback": "[inaudible]",
    "visual_fallback_channel": "visual_state",
    "caption_fallback_channel": "captions",
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_audio_revision",
    "human_gate": "open",
...
def validate_runtime_audio_caption_provenance(value: Any) -> list[str]:
    """Return audio/caption provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime audio/caption provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("audio_caption_policy") != AUDIO_CAPTION_POLICY:
        errors.append("audio_caption_policy must exactly match the v126 fallback policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v126 audio, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "audibility_observation")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime audio/caption provenance unreadable: {exc}"]
    return validate_runtime_audio_caption_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_AUDIO_CAPTION_PROVENANCE_V126_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_AUDIO_CAPTION_PROVENANCE_V126_READY: review and native gates remain open")
    return 0
