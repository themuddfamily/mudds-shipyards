#!/usr/bin/env python3
"""Validate v159 accessibility runtime recovery provenance evidence."""
...
SCHEMA = "accessibility_runtime_recovery_provenance_v159_evidence_v1"
SOURCE_SCHEMA = "safe_start_recovery_policy_v1"
SCHEMA_VERSION = "v159"
SOURCE_ID = "safe-start-recovery-policy"
...
RECOVERY_POLICY = {
    "startup_state": ["STARTING", "STABLE"],
    "fallback_scope": ["low_graphics", "windowed_mode"],
    "preserved_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
    "corrupt_data": "safe_fallback_without_authority_overwrite",
    "reentry": "not_failure_or_restart",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_recovery_revision",
    "human_gate": "open",
...
def validate_runtime_recovery_provenance(value: Any) -> list[str]:
    """Return recovery provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime recovery provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("recovery_policy") != RECOVERY_POLICY:
        errors.append("recovery_policy must exactly match the v159 startup/fallback/preservation policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v159 recovery, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/recovery boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "safe_start_recovery_policy")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime recovery provenance unreadable: {exc}"]
    return validate_runtime_recovery_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_RECOVERY_PROVENANCE_V159_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_RECOVERY_PROVENANCE_V159_READY: review and native gates remain open")
    return 0
