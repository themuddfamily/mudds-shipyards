#!/usr/bin/env python3
"""Validate v138 accessibility runtime colour-cue provenance evidence."""
...
SCHEMA = "accessibility_runtime_colour_cue_provenance_v138_evidence_v1"
...
COLOUR_CUE_POLICY = {
    ...
    "colour_channel": "allowed_only_with_shape_alternative",
    "lightness_channel": "required",
    "reduced_flash_interaction": "steady_no_flash",
    "hardware_review": "not_run",
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_colour_revision",
    "human_gate": "open",
...
def validate_runtime_colour_cue_provenance(value: Any) -> list[str]:
    """Return colour-cue provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime colour-cue provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("colour_cue_policy") != COLOUR_CUE_POLICY:
        errors.append("colour_cue_policy must exactly match the v138 semantic, shape, lightness, and review policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v138 colour, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "colour_safe_cue_policy")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime colour-cue provenance unreadable: {exc}"]
    return validate_runtime_colour_cue_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_COLOUR_CUE_PROVENANCE_V138_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_COLOUR_CUE_PROVENANCE_V138_READY: review and native gates remain open")
    return 0
