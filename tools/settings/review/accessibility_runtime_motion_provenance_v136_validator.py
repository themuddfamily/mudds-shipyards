#!/usr/bin/env python3
"""Validate v136 accessibility runtime motion-comfort provenance evidence."""
...
SCHEMA = "accessibility_runtime_motion_provenance_v136_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_presentation_v1"
SCHEMA_VERSION = "v136"
SOURCE_ID = "accessibility-visual-preset"
...
MOTION_POLICY = {
    ...
    "camera_shake": "caller_suppressed_when_requested",
    "transition_tweens": "consumer_owned_and_bounded",
    "motion_strength_floor": 0.0,
    "flash_strength_floor": 0.0,
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_motion_revision",
    "human_gate": "open",
...
def validate_runtime_motion_provenance(value: Any) -> list[str]:
    """Return motion-comfort provenance violations without raising."""
    ...
    if value.get("motion_policy") != MOTION_POLICY:
        errors.append("motion_policy must exactly match the v136 camera/motion comfort policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v136 motion, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/camera boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "motion_comfort_policy")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_MOTION_PROVENANCE_V136_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_MOTION_PROVENANCE_V136_READY: review and native gates remain open")
    return 0
