#!/usr/bin/env python3
"""Validate v61 accessibility-caption attestation/source reconciliation evidence."""
...
SCHEMA = "accessibility_caption_attestation_source_reconciliation_v61_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_source_channel_release_v60_evidence_v1"
SCHEMA_VERSION = "v61"
PREVIOUS_VERSION = "v60"
ATTESTATION_ID = "caption-accessibility-v61"
...
ATTESTATION_SOURCE_RECONCILIATION = {
...
def validate_attestation_source_reconciliation(value: Any) -> list[str]:
    """Return attestation/source violations without raising on malformed values."""
    if not isinstance(value, dict):
        return ["attestation/source reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "attestation_source_reconciliation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("attestation_source_reconciliation") != ATTESTATION_SOURCE_RECONCILIATION:
        errors.append("attestation_source_reconciliation must exactly match v61 attestation, source, and stale policy")
    ...
    for key, expected in (("attestation_id", ATTESTATION_ID), ("attestation_status", "open"), ("source_id", SOURCE_ID), ("source_mode", "exact"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("reconciliation_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"attestation/source reconciliation unreadable: {exc}"]
    return validate_attestation_source_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("attestation_source_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.attestation_source_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_ATTESTATION_SOURCE_RECONCILIATION_V61_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_ATTESTATION_SOURCE_RECONCILIATION_V61_READY: attestation remains open")
    return 0
