#!/usr/bin/env python3
"""Validate v80 accessibility-caption evidence/closure evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_consistency_lineage_v79_evidence_v1"
SCHEMA_VERSION = "v80"
PREVIOUS_VERSION = "v79"
SOURCE_ID = "caption-presentation-service"
SOURCE_CONSISTENCY_ID = "caption-accessibility-consistency-v79"
EVIDENCE_ID = "caption-accessibility-evidence-v80"
...
def validate_evidence_closure(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v79 source, authority, stale, human, and native policy")
    if value.get("evidence_closure") != EVIDENCE_CLOSURE:
        errors.append("evidence_closure must exactly match v80 evidence, closure, and open-gate policy")
    if value.get("evidence_checks") != EVIDENCE_CHECKS:
        errors.append("evidence_checks must exactly match the bounded v80 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_EVIDENCE_CLOSURE_V80_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_EVIDENCE_CLOSURE_V80_READY: evidence remains unverified")
    return 0
