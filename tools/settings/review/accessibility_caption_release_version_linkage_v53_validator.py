#!/usr/bin/env python3
"""Validate v53 caption release/version linkage evidence."""
...
SCHEMA = "accessibility_caption_release_version_linkage_v53_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_release_version_authority_reconciliation_v52_evidence_v1"
SCHEMA_VERSION = "v53"
PREVIOUS_VERSION = "v52"
RELEASE_ID = "caption-accessibility-v53"
...
def validate_release_version_linkage(value: Any) -> list[str]:
    """Return release/version linkage violations without raising."""
    if not isinstance(value, dict):
        return ["release/version linkage record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "release_version_linkage_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("release_version_linkage") != RELEASE_VERSION_LINKAGE:
        errors.append("release_version_linkage must exactly match v53 release, linkage, and stale policy")
    ...
    for key, expected in (("release_id", RELEASE_ID), ("release_status", "unreleased"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("linkage_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"release/version linkage unreadable: {exc}"]
    return validate_release_version_linkage(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("release_version_linkage", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.release_version_linkage)
    if errors:
        print("ACCESSIBILITY_CAPTION_RELEASE_VERSION_LINKAGE_V53_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_RELEASE_VERSION_LINKAGE_V53_READY: human review remains open")
    return 0
