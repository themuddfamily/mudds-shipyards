#!/usr/bin/env python3
"""Validate v114 accessibility-caption evidence/lineage evidence."""
...
SCHEMA = "accessibility_caption_evidence_lineage_v114_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_audit_state_v113_evidence_v1"
SCHEMA_VERSION = "v114"
PREVIOUS_VERSION = "v113"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v113"
EVIDENCE_ID = "caption-accessibility-evidence-v114"
...
BINDING = {
    "evidence_scope": "audit_to_lineage",
    "source_id": SOURCE_ID,
...
def validate_evidence_lineage(value: Any) -> list[str]:
    """Return evidence/lineage violations without raising."""
    if not isinstance(value, dict):
        return ["evidence/lineage record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "evidence_claimed", "lineage_claimed", "evidence_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v113 source, authority, stale, human, and native policy")
    if value.get("evidence_lineage") != EVIDENCE_LINEAGE:
        errors.append("evidence_lineage must exactly match v114 evidence, source, and open-gate policy")
    if value.get("evidence_checks") != EVIDENCE_CHECKS:
        errors.append("evidence_checks must exactly match the bounded v114 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"evidence/lineage unreadable: {exc}"]
    return validate_evidence_lineage(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("evidence_lineage", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.evidence_lineage)
    if errors:
        print("ACCESSIBILITY_CAPTION_EVIDENCE_LINEAGE_V114_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_EVIDENCE_LINEAGE_V114_READY: evidence remains unverified")
    return 0
