#!/usr/bin/env python3
"""Validate v255 accessibility runtime evidence-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_evidence_record_provenance_v255_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_evidence_record_policy_v1"
SCHEMA_VERSION = "v255"
SOURCE_ID = "runtime-accessibility-evidence-record-policy"
...
EVIDENCE_POLICY = {
    "sequence": ["declare_scope", "list_artifacts", "mark_gates", "defer_evidence"],
    "artifact_types": ["validator", "test", "manifest", "evidence"],
    "evidence_states": ["prepared", "pending_review", "deferred"],
    "evidence_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
...
BINDING = {
    ...
    "policy_mode": "exact", "apply_rule": "provenance_evidence_only",
    "human_gate": "open", "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True, "evidence_record_authority": False,
...
def validate_runtime_evidence_record_provenance(value: Any) -> list[str]:
    """Return evidence-record provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime evidence-record provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "evidence_written", "evidence_published"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("evidence_policy") != EVIDENCE_POLICY:
        errors.append("evidence_policy must exactly match the v255 evidence-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v255 evidence, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/evidence boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_accessibility_evidence_record_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime evidence-record provenance unreadable: {exc}"]
    return validate_runtime_evidence_record_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_EVIDENCE_RECORD_PROVENANCE_V255_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_EVIDENCE_RECORD_PROVENANCE_V255_READY: review and native gates remain open")
    return 0
