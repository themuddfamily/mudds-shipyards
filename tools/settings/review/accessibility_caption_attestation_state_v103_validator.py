#!/usr/bin/env python3
"""Validate v103 accessibility-caption attestation/state evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_evidence_closure_v102_evidence_v1"
SCHEMA_VERSION = "v103"
PREVIOUS_VERSION = "v102"
SOURCE_ID = "caption-presentation-service"
SOURCE_EVIDENCE_ID = "caption-accessibility-evidence-v102"
ATTESTATION_ID = "caption-accessibility-attestation-v103"
...
BINDING = {
    "attestation_scope": "evidence_to_state",
    "source_id": SOURCE_ID,
...
def validate_attestation_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v102 source, authority, stale, human, and native policy")
    if value.get("attestation_state") != ATTESTATION_STATE:
        errors.append("attestation_state must exactly match v103 attestation, source, and open-gate policy")
    if value.get("attestation_checks") != ATTESTATION_CHECKS:
        errors.append("attestation_checks must exactly match the bounded v103 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_ATTESTATION_STATE_V103_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_ATTESTATION_STATE_V103_READY: attestation remains unverified")
    return 0
