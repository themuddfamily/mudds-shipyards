"""Validate v364 accessibility runtime outcome provenance."""
from __future__ import annotations
import json
from pathlib import Path
from tools.settings.review import accessibility_runtime_outcome_record_provenance_v363_validator as base
SCHEMA="accessibility_runtime_outcome_record_provenance_v364_evidence_v1"
def validate_runtime_outcome_record_provenance(value):
 if not isinstance(value,dict):return ["record must be an object"]
 candidate=dict(value);candidate["schema"]=base.SCHEMA;candidate["schema_version"]="v363"
 errors=base.validate_runtime_outcome_record_provenance(candidate)
 if value.get("schema")!=SCHEMA:errors.append("schema")
 if value.get("schema_version")!="v364":errors.append("schema_version")
 return errors
def validate(path):
