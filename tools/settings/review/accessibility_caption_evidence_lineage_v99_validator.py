#!/usr/bin/env python3
"""Validate v99 accessibility-caption evidence/lineage evidence."""
...
SCHEMA = "accessibility_caption_evidence_lineage_v99_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_audit_closure_v98_evidence_v1"
SCHEMA_VERSION = "v99"
PREVIOUS_VERSION = "v98"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v98"
EVIDENCE_ID = "caption-accessibility-evidence-v99"
...
def validate_evidence_lineage(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v98 source, authority, stale, human, and native policy")
    if value.get("evidence_lineage") != EVIDENCE_LINEAGE:
        errors.append("evidence_lineage must exactly match v99 evidence, source, and open-gate policy")
    if value.get("evidence_checks") != EVIDENCE_CHECKS:
        errors.append("evidence_checks must exactly match the bounded v99 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_EVIDENCE_LINEAGE_V99_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_EVIDENCE_LINEAGE_V99_READY: evidence remains unverified")
    return 0
