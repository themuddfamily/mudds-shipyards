#!/usr/bin/env python3
"""Validate v235 accessibility runtime gate-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_gate_record_provenance_v235_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_gate_record_policy_v1"
SCHEMA_VERSION = "v235"
SOURCE_ID = "runtime-accessibility-gate-record-policy"
...
GATE_POLICY = {
    "sequence": ["declare_scope", "list_gates", "mark_open_state", "defer_transition"],
    "artifact_types": ["validator", "test", "manifest", "evidence"],
    "gate_states": ["prepared", "pending_review", "deferred"],
...
BINDING = {
    ...
    "policy_mode": "exact", "apply_rule": "provenance_gate_only",
    "human_gate": "open", "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True, "gate_record_authority": False,
...
def validate_runtime_gate_record_provenance(value: Any) -> list[str]:
    """Return gate-record provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime gate-record provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "gate_written", "gate_closed"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("gate_policy") != GATE_POLICY:
        errors.append("gate_policy must exactly match the v235 gate-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v235 gate, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/gate boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_accessibility_gate_record_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime gate-record provenance unreadable: {exc}"]
    return validate_runtime_gate_record_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_GATE_RECORD_PROVENANCE_V235_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_GATE_RECORD_PROVENANCE_V235_READY: review and native gates remain open")
    return 0
