#!/usr/bin/env python3
"""Validate v33 paired root/authority digest evidence for captions."""
...
SCHEMA = "accessibility_caption_paired_root_authority_digest_v33_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_paired_root_authority_digest_v32_evidence_v1"
...
def validate_root_authority_digest(value: Any) -> list[str]:
    """Return root/digest violations without raising on malformed values."""

    if not isinstance(value, dict):
        return ["root/authority digest record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "root_authority_digest_verified", "digest_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("root_authority_digest") != ROOT_AUTHORITY_DIGEST:
        errors.append("root_authority_digest must exactly match root, digest, and stale policy")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"root/authority digest unreadable: {exc}"]
    return validate_root_authority_digest(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("root_authority_digest", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.root_authority_digest)
    if errors:
        print("ACCESSIBILITY_CAPTION_PAIRED_ROOT_AUTHORITY_DIGEST_V33_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PAIRED_ROOT_AUTHORITY_DIGEST_V33_READY: human review remains open")
    return 0
