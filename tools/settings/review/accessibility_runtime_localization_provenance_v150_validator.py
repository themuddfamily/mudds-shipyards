#!/usr/bin/env python3
"""Validate v150 accessibility runtime localization provenance evidence."""
...
SCHEMA = "accessibility_runtime_localization_provenance_v150_evidence_v1"
...
LOCALIZATION_POLICY = {
    "text_source": "caller_localized_semantic_text",
    "missing_translation": "fallback_source_language",
    "maximum_text_characters": 512,
    "direction": "caller_locale_direction",
    "rtl_layout": "mirror_safe_area_and_prompt_alignment",
    "speaker_and_category": "separate_semantic_fields",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_locale_revision",
    "human_gate": "open",
...
def validate_runtime_localization_provenance(value: Any) -> list[str]:
    ...
    if value.get("localization_policy") != LOCALIZATION_POLICY:
        errors.append("localization_policy must exactly match the v150 text, fallback, direction, and RTL policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v150 locale, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/localization boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "localized_semantic_text")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_LOCALIZATION_PROVENANCE_V150_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_LOCALIZATION_PROVENANCE_V150_READY: review and native gates remain open")
    return 0
