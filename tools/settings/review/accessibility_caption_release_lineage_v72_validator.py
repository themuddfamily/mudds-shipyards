#!/usr/bin/env python3
"""Validate v72 accessibility-caption release/lineage evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_state_consistency_v71_evidence_v1"
SCHEMA_VERSION = "v72"
PREVIOUS_VERSION = "v71"
SOURCE_ID = "caption-presentation-service"
SOURCE_STATE_ID = "caption-accessibility-state-v71"
RELEASE_ID = "caption-accessibility-release-v72"
...
BINDING = {
    "release_scope": "state_to_release",
    "source_id": SOURCE_ID,
...
def validate_release_lineage(value: Any) -> list[str]:
    """Return release/lineage violations without raising."""
    if not isinstance(value, dict):
        return ["release/lineage record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "release_claimed", "lineage_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v71 source, authority, stale, human, and native policy")
    if value.get("release_lineage") != RELEASE_LINEAGE:
        errors.append("release_lineage must exactly match v72 release, source, and open-gate policy")
    if value.get("release_checks") != RELEASE_CHECKS:
        errors.append("release_checks must exactly match the bounded v72 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"release/lineage unreadable: {exc}"]
    return validate_release_lineage(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("release_lineage", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.release_lineage)
    if errors:
        print("ACCESSIBILITY_CAPTION_RELEASE_LINEAGE_V72_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_RELEASE_LINEAGE_V72_READY: release remains unclaimed")
    return 0
