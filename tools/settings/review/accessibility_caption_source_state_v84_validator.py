#!/usr/bin/env python3
"""Validate v84 accessibility-caption source/state evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_audit_lineage_v83_evidence_v1"
SCHEMA_VERSION = "v84"
PREVIOUS_VERSION = "v83"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v83"
...
def validate_source_state(value: Any) -> list[str]:
    """Return source/state violations without raising."""
    if not isinstance(value, dict):
        return ["source/state record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "source_claimed", "state_claimed", "source_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v83 source, authority, stale, human, and native policy")
    if value.get("source_state") != SOURCE_STATE:
        errors.append("source_state must exactly match v84 source, state, and open-gate policy")
    if value.get("source_checks") != SOURCE_CHECKS:
        errors.append("source_checks must exactly match the bounded v84 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"source/state unreadable: {exc}"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate(args.source_state)
    if errors:
        print("ACCESSIBILITY_CAPTION_SOURCE_STATE_V84_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_SOURCE_STATE_V84_READY: source remains unverified")
    return 0
