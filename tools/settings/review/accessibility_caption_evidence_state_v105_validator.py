#!/usr/bin/env python3
"""Validate v105 accessibility-caption evidence/state evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_audit_lineage_v104_evidence_v1"
SCHEMA_VERSION = "v105"
PREVIOUS_VERSION = "v104"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v104"
EVIDENCE_ID = "caption-accessibility-evidence-v105"
...
def validate_evidence_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v104 source, authority, stale, human, and native policy")
    if value.get("evidence_state") != EVIDENCE_STATE:
        errors.append("evidence_state must exactly match v105 evidence, source, and open-gate policy")
    if value.get("evidence_checks") != EVIDENCE_CHECKS:
        errors.append("evidence_checks must exactly match the bounded v105 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_EVIDENCE_STATE_V105_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_EVIDENCE_STATE_V105_READY: evidence remains unverified")
    return 0
