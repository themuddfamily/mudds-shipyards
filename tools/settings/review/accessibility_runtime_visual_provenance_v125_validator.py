#!/usr/bin/env python3
"""Validate v125 accessibility runtime visual-policy provenance evidence."""
...
SCHEMA = "accessibility_runtime_visual_provenance_v125_evidence_v1"
...
VISUAL_POLICY = {
    "minimum_large_text_contrast_ratio": 3.0,
    "minimum_body_contrast_ratio": 4.5,
    "minimum_caption_contrast_ratio": 7.0,
    "colour_safe_channel": "shape_cue_and_lightness",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_visual_revision",
    "human_gate": "open",
...
def validate_runtime_visual_provenance(value: Any) -> list[str]:
    """Return visual policy provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime visual provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("visual_policy") != VISUAL_POLICY:
        errors.append("visual_policy must exactly match the v125 contrast, shape, motion, flash, and scale policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v125 visual, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "visual_policy")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime visual provenance unreadable: {exc}"]
    return validate_runtime_visual_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_VISUAL_PROVENANCE_V125_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_VISUAL_PROVENANCE_V125_READY: review and native gates remain open")
    return 0
