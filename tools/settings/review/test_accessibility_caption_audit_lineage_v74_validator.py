...
from tools.settings.review.accessibility_caption_audit_lineage_v74_validator import (
    AUDIT_CHECKS,
    AUDIT_ID,
    AUDIT_LINEAGE,
    AUTHORITY,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-audit-v74",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v74 audit/lineage review or native render has been performed",
...
class AccessibilityCaptionAuditLineageV74Tests(unittest.TestCase):
    def test_complete_record_keeps_audit_open_and_native_gate_closed(self):
        self.assertEqual(validate_audit_lineage(_record()), [])

    def test_v73_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_verification_state_v72_evidence_v1"
        value["source_verification_id"] = "caption-accessibility-verification-v72"
        value["audit_lineage"]["previous_version"] = "v72"
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v74.json", "sha256": "bad"}]
        errors = validate_audit_lineage(value)
