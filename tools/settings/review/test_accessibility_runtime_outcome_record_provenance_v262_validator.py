...
from tools.settings.review.accessibility_runtime_outcome_record_provenance_v262_validator import (
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-runtime-outcome-record-v262",
        ...
        "outcome_policy": copy.deepcopy(OUTCOME_POLICY),
...
class AccessibilityRuntimeOutcomeRecordProvenanceV262Tests(unittest.TestCase):
    ...
    def test_outcome_policy_and_binding_are_exact(self):
        value = _record()
        value["outcome_policy"]["missing_artifact"] = "claim_complete"
        value["binding"]["apply_rule"] = "write_settings"
        errors = validate_runtime_outcome_record_provenance(value)
        self.assertTrue(any("outcome_policy must exactly" in error for error in errors))
        self.assertTrue(any("binding must exactly" in error for error in errors))

    def test_outcome_fields_cannot_shrink(self):
        value = _record()
        value["outcome_policy"]["outcome_fields"] = ["camera"]
        errors = validate_runtime_outcome_record_provenance(value)
        self.assertTrue(any("outcome_policy must exactly" in error for error in errors))
    ...
    def test_native_and_outcome_claims_remain_open(self):
        ...
        value["outcome_written"] = True
        errors = validate_runtime_outcome_record_provenance(value)
        self.assertTrue(any("native_render_status must remain not_run" in error for error in errors))
        self.assertTrue(any("outcome_written must be false" in error for error in errors))

    def test_outcome_record_authority_fails_closed(self):
        value = _record()
        value["authority"]["outcome_record_authority"] = True
        value["outcome_record_authority"] = True
        errors = validate_runtime_outcome_record_provenance(value)
        self.assertTrue(any("authority must exactly" in error for error in errors))
        self.assertTrue(any("outcome_record_authority must be false" in error for error in errors))
