#!/usr/bin/env python3
"""Validate v63 accessibility-caption provenance/contract evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_source_integrity_linkage_v62_evidence_v1"
SCHEMA_VERSION = "v63"
PREVIOUS_VERSION = "v62"
...
def validate_provenance_contract(value: Any) -> list[str]:
    """Return provenance/contract violations without raising on malformed values."""
    if not isinstance(value, dict):
        return ["provenance/contract record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "provenance_contract_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("provenance_contract") != PROVENANCE_CONTRACT:
        errors.append("provenance_contract must exactly match v63 provenance, contract, and stale policy")
    ...
    for key, expected in (("source_id", SOURCE_ID), ("source_mode", "exact"), ("contract_id", CONTRACT_ID), ("contract_mode", "exact"), ("provenance_scope", "authority_and_generation"), ("provenance_source_of_truth", "presentation_only"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"provenance/contract unreadable: {exc}"]
    return validate_provenance_contract(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("provenance_contract", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.provenance_contract)
    if errors:
        print("ACCESSIBILITY_CAPTION_PROVENANCE_CONTRACT_V63_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PROVENANCE_CONTRACT_V63_READY: human review remains open")
    return 0
