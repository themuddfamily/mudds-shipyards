...
SCHEMA = "accessibility_caption_linked_authority_provenance_version_v43_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_linked_authority_provenance_v42_evidence_v1"
SCHEMA_VERSION = "v43"
...
def validate_linked_authority_provenance_version(value: Any) -> list[str]:
    """Return linked authority/provenance/version violations without raising."""
    if not isinstance(value, dict):
        return ["linked authority/provenance/version record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "linked_authority_provenance_version_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("linked_authority_provenance_version") != LINKED_AUTHORITY_PROVENANCE_VERSION:
        errors.append("linked_authority_provenance_version must exactly match v43 linkage, version, and stale policy")
    ...
    for key in ("version_mode", "link_mode", "provenance_mode"):
        if value.get(key) != "exact":
            errors.append(f"{key} must remain exact")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"linked authority/provenance/version unreadable: {exc}"]
    return validate_linked_authority_provenance_version(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("linked_authority_provenance_version", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.linked_authority_provenance_version)
    if errors:
        print("ACCESSIBILITY_CAPTION_LINKED_AUTHORITY_PROVENANCE_VERSION_V43_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_LINKED_AUTHORITY_PROVENANCE_VERSION_V43_READY: human review remains open")
    return 0
