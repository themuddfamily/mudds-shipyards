#!/usr/bin/env python3
"""Validate v131 accessibility runtime detach/re-entry provenance evidence."""
...
SCHEMA = "accessibility_runtime_reentry_provenance_v131_evidence_v1"
...
REENTRY_POLICY = {
    "detach": "retain_accessibility_snapshot",
    "reentry": "reapply_same_generation",
    "transient_edges": "clear_before_reentry",
    "caption_queue": "caller_owned_and_detached",
    "audio_observation": "caller_owned_and_detached",
    "idempotence": "reapplying_same_snapshot_is_noop",
}
BINDING = {
    "source_schema": SOURCE_SCHEMA,
    "source_id": SOURCE_ID,
    "contract_id": CONTRACT_ID,
    "lifecycle_mode": "exact",
    "stale_policy": "reject_stale_reentry_snapshot",
    "human_gate": "open",
...
def validate_runtime_reentry_provenance(value: Any) -> list[str]:
    """Return re-entry provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime re-entry provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "reentry_verified", "runtime_claimed", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("reentry_policy") != REENTRY_POLICY:
        errors.append("reentry_policy must exactly match the v131 detach/re-entry lifecycle")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v131 lifecycle, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "detached_reentry_snapshot")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime re-entry provenance unreadable: {exc}"]
    return validate_runtime_reentry_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_REENTRY_PROVENANCE_V131_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_REENTRY_PROVENANCE_V131_READY: review and native gates remain open")
    return 0
