#!/usr/bin/env python3
"""Validate v137 accessibility runtime typography provenance evidence."""
...
SCHEMA = "accessibility_runtime_typography_provenance_v137_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_presentation_v1"
SCHEMA_VERSION = "v137"
SOURCE_ID = "caption-presenter"
...
TYPOGRAPHY_POLICY = {
    ...
    "semantic_fields": ["category", "speaker", "text"],
    "overflow_policy": "bounded_wrap_and_clip",
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_typography_revision",
    "human_gate": "open",
...
def validate_runtime_typography_provenance(value: Any) -> list[str]:
    """Return typography provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime typography provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("typography_policy") != TYPOGRAPHY_POLICY:
        errors.append("typography_policy must exactly match the v137 text, contrast, scale, and overflow policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v137 typography, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "caption_typography_policy")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime typography provenance unreadable: {exc}"]
    return validate_runtime_typography_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_TYPOGRAPHY_PROVENANCE_V137_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_TYPOGRAPHY_PROVENANCE_V137_READY: review and native gates remain open")
    return 0
