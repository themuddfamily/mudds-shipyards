#!/usr/bin/env python3
"""Validate v28 paired-digest reconciliation authority evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_digest_reconciliation_authority_v27_evidence_v1"
...
PAIRED_DIGEST_RECONCILIATION = {
    "pair_scope": "authority_and_generation",
...
def validate_paired_digest_reconciliation(value: Any) -> list[str]:
    """Return contract violations without raising on malformed JSON shapes."""

    if not isinstance(value, dict):
        return ["paired-digest reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in (
        ...
        "paired_digest_reconciliation_verified",
        "digest_verified",
    ...
    if value.get("paired_digest_reconciliation") != PAIRED_DIGEST_RECONCILIATION:
        errors.append("paired_digest_reconciliation must exactly match pair ownership and stale policy")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"paired-digest reconciliation unreadable: {exc}"]
    return validate_paired_digest_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("paired_digest_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.paired_digest_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_PAIRED_DIGEST_RECONCILIATION_AUTHORITY_V28_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PAIRED_DIGEST_RECONCILIATION_AUTHORITY_V28_READY: human review remains open")
    return 0
