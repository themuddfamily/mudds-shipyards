#!/usr/bin/env python3
"""Validate v109 accessibility-caption audit/state evidence."""
...
SCHEMA = "accessibility_caption_audit_state_v109_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_attestation_lineage_v108_evidence_v1"
SCHEMA_VERSION = "v109"
PREVIOUS_VERSION = "v108"
SOURCE_ID = "caption-presentation-service"
SOURCE_ATTESTATION_ID = "caption-accessibility-attestation-v108"
AUDIT_ID = "caption-accessibility-audit-v109"
...
BINDING = {
    "audit_scope": "attestation_to_state",
    "source_id": SOURCE_ID,
...
def validate_audit_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v108 source, authority, stale, human, and native policy")
    if value.get("audit_state") != AUDIT_STATE:
        errors.append("audit_state must exactly match v109 audit, state, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v109 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V109_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V109_READY: audit remains unverified")
    return 0
