#!/usr/bin/env python3
"""Validate v91 accessibility-caption verification/closure evidence."""
...
SCHEMA = "accessibility_caption_verification_closure_v91_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_source_lineage_v90_evidence_v1"
SCHEMA_VERSION = "v91"
PREVIOUS_VERSION = "v90"
SOURCE_ID = "caption-presentation-service"
SOURCE_IDENTITY = "caption-accessibility-source-v90"
VERIFICATION_ID = "caption-accessibility-verification-v91"
...
BINDING = {
    "verification_scope": "source_to_closure",
    "source_id": SOURCE_ID,
...
def validate_verification_closure(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v90 source, authority, stale, human, and native policy")
    if value.get("verification_closure") != VERIFICATION_CLOSURE:
        errors.append("verification_closure must exactly match v91 verification, closure, and open-gate policy")
    if value.get("verification_checks") != VERIFICATION_CHECKS:
        errors.append("verification_checks must exactly match the bounded v91 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_VERIFICATION_CLOSURE_V91_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_VERIFICATION_CLOSURE_V91_READY: verification remains unverified")
    return 0
