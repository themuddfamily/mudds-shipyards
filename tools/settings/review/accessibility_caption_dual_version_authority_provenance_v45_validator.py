#!/usr/bin/env python3
"""Validate v45 dual-version authority/provenance evidence."""
...
SCHEMA = "accessibility_caption_dual_version_authority_provenance_v45_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_linked_authority_provenance_version_v44_evidence_v1"
SCHEMA_VERSION = "v45"
PREVIOUS_VERSION = "v44"
...
DUAL_VERSION_AUTHORITY_PROVENANCE = {
...
def validate_dual_version_authority_provenance(value: Any) -> list[str]:
    """Return dual-version violations without raising on malformed values."""
    if not isinstance(value, dict):
        return ["dual-version authority/provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "dual_version_authority_provenance_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("dual_version_authority_provenance") != DUAL_VERSION_AUTHORITY_PROVENANCE:
        errors.append("dual_version_authority_provenance must exactly match v45 version relation and stale policy")
    ...
    if value.get("current_version") != SCHEMA_VERSION:
        errors.append("current_version must be v45")
    if value.get("previous_version") != PREVIOUS_VERSION:
        errors.append("previous_version must be v44")
    if value.get("version_relation") != "adjacent_exact":
        errors.append("version_relation must remain adjacent_exact")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"dual-version authority/provenance unreadable: {exc}"]
    return validate_dual_version_authority_provenance(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("dual_version_authority_provenance", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.dual_version_authority_provenance)
    if errors:
        print("ACCESSIBILITY_CAPTION_DUAL_VERSION_AUTHORITY_PROVENANCE_V45_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_DUAL_VERSION_AUTHORITY_PROVENANCE_V45_READY: human review remains open")
    return 0
