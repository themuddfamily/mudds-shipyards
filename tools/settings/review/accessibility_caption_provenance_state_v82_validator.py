#!/usr/bin/env python3
"""Validate v82 accessibility-caption provenance/state evidence."""
...
SCHEMA = "accessibility_caption_provenance_state_v82_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_release_state_v81_evidence_v1"
SCHEMA_VERSION = "v82"
PREVIOUS_VERSION = "v81"
SOURCE_ID = "caption-presentation-service"
SOURCE_RELEASE_ID = "caption-accessibility-release-v81"
PROVENANCE_ID = "caption-accessibility-provenance-v82"
...
BINDING = {
    "provenance_scope": "release_to_state",
    "source_id": SOURCE_ID,
...
PROVENANCE_STATE = {
    ...
    "source_mode": "exact",
    "source_release_id": SOURCE_RELEASE_ID,
    "source_release_status": "unclaimed",
...
def validate_provenance_state(value: Any) -> list[str]:
    """Return provenance/state violations without raising."""
    if not isinstance(value, dict):
        return ["provenance/state record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "provenance_claimed", "state_claimed", "provenance_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v81 source, authority, stale, human, and native policy")
    if value.get("provenance_state") != PROVENANCE_STATE:
        errors.append("provenance_state must exactly match v82 provenance, state, and open-gate policy")
    if value.get("provenance_checks") != PROVENANCE_CHECKS:
        errors.append("provenance_checks must exactly match the bounded v82 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"provenance/state unreadable: {exc}"]
    return validate_provenance_state(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("provenance_state", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.provenance_state)
    if errors:
        print("ACCESSIBILITY_CAPTION_PROVENANCE_STATE_V82_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PROVENANCE_STATE_V82_READY: provenance remains unverified")
    return 0
