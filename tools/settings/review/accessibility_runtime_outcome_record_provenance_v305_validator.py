#!/usr/bin/env python3
"""Validate v305 accessibility runtime outcome-record provenance evidence."""
from __future__ import annotations
import argparse,json,re
from pathlib import Path
from typing import Any
SCHEMA="accessibility_runtime_outcome_record_provenance_v305_evidence_v1"; SOURCE_SCHEMA="runtime_accessibility_outcome_record_policy_v1"; SCHEMA_VERSION="v305"; SOURCE_ID="runtime-accessibility-outcome-record-policy"; CONTRACT_ID="runtime-accessibility-presentation"; OPEN_REVIEW_STATUSES={"pending","not_performed","in_progress","failed"}; RECORD_STATUSES={"planned","pending","not_performed"}; EVIDENCE_KINDS={"log","image","report","video"}; SHA256=re.compile(r"^[0-9a-f]{64}$")
...
def _evidence(v:Any,e:list[str])->None:
 ...
 for i,x in enumerate(v):
  ...
  if not isinstance(x.get("sha256"),str) or not SHA256.fullmatch(x["sha256"]):e.append(f"{p}.sha256 must be a lowercase 64-character digest")
def validate_runtime_outcome_record_provenance(v:Any)->list[str]:
 ...
 if v.get("human_review_status") not in OPEN_REVIEW_STATUSES:e.append("human_review_status must remain open")
 ...
 if v.get("outcome_policy")!=OUTCOME_POLICY:e.append("outcome_policy must exactly match v305")
 if v.get("binding")!=BINDING:e.append("binding must exactly match v305")
 if v.get("authority")!=AUTHORITY:e.append("authority must exactly match presentation-only boundary")
...
def main(argv:list[str]|None=None)->int:
 p=argparse.ArgumentParser();p.add_argument("provenance",type=Path);a=p.parse_args(argv);e=validate(a.provenance)
 if e:print("ACCESSIBILITY_RUNTIME_OUTCOME_RECORD_PROVENANCE_V305_INVALID");return 1
 print("ACCESSIBILITY_RUNTIME_OUTCOME_RECORD_PROVENANCE_V305_READY: review and native gates remain open");return 0
