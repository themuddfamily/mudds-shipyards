#!/usr/bin/env python3
"""Validate v81 accessibility-caption release/state evidence."""
...
SCHEMA = "accessibility_caption_release_state_v81_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_evidence_closure_v80_evidence_v1"
SCHEMA_VERSION = "v81"
PREVIOUS_VERSION = "v80"
SOURCE_ID = "caption-presentation-service"
SOURCE_EVIDENCE_ID = "caption-accessibility-evidence-v80"
RELEASE_ID = "caption-accessibility-release-v81"
...
BINDING = {
    "release_scope": "evidence_to_state",
    "source_id": SOURCE_ID,
...
def validate_release_state(value: Any) -> list[str]:
    """Return release/state violations without raising."""
    if not isinstance(value, dict):
        return ["release/state record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "release_claimed", "state_claimed", "release_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v80 source, authority, stale, human, and native policy")
    if value.get("release_state") != RELEASE_STATE:
        errors.append("release_state must exactly match v81 release, state, and open-gate policy")
    if value.get("release_checks") != RELEASE_CHECKS:
        errors.append("release_checks must exactly match the bounded v81 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"release/state unreadable: {exc}"]
    return validate_release_state(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("release_state", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.release_state)
    if errors:
        print("ACCESSIBILITY_CAPTION_RELEASE_STATE_V81_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_RELEASE_STATE_V81_READY: release remains unclaimed")
    return 0
