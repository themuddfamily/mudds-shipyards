#!/usr/bin/env python3
"""Validate v112 accessibility-caption evidence/closure evidence."""
...
SCHEMA = "accessibility_caption_evidence_closure_v112_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_consistency_state_v111_evidence_v1"
SCHEMA_VERSION = "v112"
PREVIOUS_VERSION = "v111"
SOURCE_ID = "caption-presentation-service"
SOURCE_CONSISTENCY_ID = "caption-accessibility-consistency-v111"
EVIDENCE_ID = "caption-accessibility-evidence-v112"
...
BINDING = {
    "evidence_scope": "consistency_to_closure",
    "source_id": SOURCE_ID,
...
def validate_evidence_closure(value: Any) -> list[str]:
    """Return evidence/closure violations without raising."""
    if not isinstance(value, dict):
        return ["evidence/closure record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "evidence_claimed", "closure_claimed", "evidence_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v111 source, authority, stale, human, and native policy")
    if value.get("evidence_closure") != EVIDENCE_CLOSURE:
        errors.append("evidence_closure must exactly match v112 evidence, closure, and open-gate policy")
    if value.get("evidence_checks") != EVIDENCE_CHECKS:
        errors.append("evidence_checks must exactly match the bounded v112 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"evidence/closure unreadable: {exc}"]
    return validate_evidence_closure(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("evidence_closure", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.evidence_closure)
    if errors:
        print("ACCESSIBILITY_CAPTION_EVIDENCE_CLOSURE_V112_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_EVIDENCE_CLOSURE_V112_READY: evidence remains unverified")
    return 0
