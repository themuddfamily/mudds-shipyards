#!/usr/bin/env python3
"""Validate v160 accessibility runtime handoff provenance evidence."""
...
SCHEMA = "accessibility_runtime_handoff_provenance_v160_evidence_v1"
SOURCE_SCHEMA = "runtime_handoff_manifest_v1"
SCHEMA_VERSION = "v160"
SOURCE_ID = "runtime-handoff-manifest"
...
HANDOFF_POLICY = {
    "sequence": ["capture", "validate", "present"],
    "preserved_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
    "invalid_payload": "reject_without_partial_apply",
    "duplicate_revision": "idempotent_no_replay",
    "failure_mode": "presentation_fallback_only",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "revision_policy": "monotonic_handoff_revision",
    "human_gate": "open",
...
def validate_runtime_handoff_provenance(value: Any) -> list[str]:
    """Return handoff provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime handoff provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "partial_apply", "handoff_replayed"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("handoff_policy") != HANDOFF_POLICY:
        errors.append("handoff_policy must exactly match the v160 capture/validate/present policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v160 revision, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/handoff boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_handoff_manifest")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime handoff provenance unreadable: {exc}"]
    return validate_runtime_handoff_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_HANDOFF_PROVENANCE_V160_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_HANDOFF_PROVENANCE_V160_READY: review and native gates remain open")
    return 0
