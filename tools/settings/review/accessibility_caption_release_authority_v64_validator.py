#!/usr/bin/env python3
"""Validate v64 accessibility-caption release/authority evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_provenance_contract_v63_evidence_v1"
SCHEMA_VERSION = "v64"
PREVIOUS_VERSION = "v63"
RELEASE_ID = "caption-accessibility-v64"
...
def validate_release_authority(value: Any) -> list[str]:
    """Return release/authority violations without raising on malformed values."""
    if not isinstance(value, dict):
        return ["release/authority record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "release_authority_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("release_authority") != RELEASE_AUTHORITY:
        errors.append("release_authority must exactly match v64 release, source, authority, and stale policy")
    ...
    for key, expected in (("release_id", RELEASE_ID), ("release_status", "unreleased"), ("source_id", SOURCE_ID), ("source_mode", "exact"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("authority_mode", "exact"), ("provenance_source_of_truth", "presentation_only")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"release/authority unreadable: {exc}"]
    return validate_release_authority(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("release_authority", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.release_authority)
    if errors:
        print("ACCESSIBILITY_CAPTION_RELEASE_AUTHORITY_V64_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_RELEASE_AUTHORITY_V64_READY: release remains unreleased")
    return 0
