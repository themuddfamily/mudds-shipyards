#!/usr/bin/env python3
"""Validate v71 accessibility-caption state/consistency evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_provenance_closure_v70_evidence_v1"
SCHEMA_VERSION = "v71"
PREVIOUS_VERSION = "v70"
SOURCE_ID = "caption-presentation-service"
SOURCE_PROVENANCE_ID = "caption-accessibility-provenance-v70"
STATE_ID = "caption-accessibility-state-v71"
...
BINDING = {
    "state_scope": "provenance_to_state",
    "source_id": SOURCE_ID,
...
def validate_state_consistency(value: Any) -> list[str]:
    """Return state/consistency violations without raising."""
    if not isinstance(value, dict):
        return ["state/consistency record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "state_claimed", "consistency_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v70 source, authority, stale, human, and native policy")
    if value.get("state_consistency") != STATE_CONSISTENCY:
        errors.append("state_consistency must exactly match v71 state, source, and open-gate policy")
    if value.get("state_checks") != STATE_CHECKS:
        errors.append("state_checks must exactly match the bounded v71 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"state/consistency unreadable: {exc}"]
    return validate_state_consistency(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("state_consistency", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.state_consistency)
    if errors:
        print("ACCESSIBILITY_CAPTION_STATE_CONSISTENCY_V71_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_STATE_CONSISTENCY_V71_READY: state remains unverified")
    return 0
