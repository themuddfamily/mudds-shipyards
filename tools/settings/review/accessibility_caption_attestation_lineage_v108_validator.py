#!/usr/bin/env python3
"""Validate v108 accessibility-caption attestation/lineage evidence."""
...
SCHEMA = "accessibility_caption_attestation_lineage_v108_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_consistency_state_v107_evidence_v1"
SCHEMA_VERSION = "v108"
PREVIOUS_VERSION = "v107"
SOURCE_ID = "caption-presentation-service"
SOURCE_CONSISTENCY_ID = "caption-accessibility-consistency-v107"
ATTESTATION_ID = "caption-accessibility-attestation-v108"
...
BINDING = {
    "attestation_scope": "consistency_to_lineage",
    "source_id": SOURCE_ID,
...
ATTESTATION_CHECKS = {
    ...
    "source_consistency_unverified": True,
...
def validate_attestation_lineage(value: Any) -> list[str]:
    """Return attestation/lineage violations without raising."""
    if not isinstance(value, dict):
        return ["attestation/lineage record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "attestation_claimed", "lineage_claimed", "attestation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v107 source, authority, stale, human, and native policy")
    if value.get("attestation_lineage") != ATTESTATION_LINEAGE:
        errors.append("attestation_lineage must exactly match v108 attestation, source, and open-gate policy")
    if value.get("attestation_checks") != ATTESTATION_CHECKS:
        errors.append("attestation_checks must exactly match the bounded v108 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"attestation/lineage unreadable: {exc}"]
    return validate_attestation_lineage(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("attestation_lineage", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.attestation_lineage)
    if errors:
        print("ACCESSIBILITY_CAPTION_ATTESTATION_LINEAGE_V108_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_ATTESTATION_LINEAGE_V108_READY: attestation remains unverified")
    return 0
