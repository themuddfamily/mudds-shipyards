#!/usr/bin/env python3
"""Validate v26 caption identity/digest reconciliation authority evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_identity_reconciliation_authority_v25_evidence_v1"
...
def validate_identity_digest_reconciliation(value: Any) -> list[str]:
    """Return contract violations without raising on malformed JSON shapes."""

    if not isinstance(value, dict):
        return ["identity/digest reconciliation record must be an object"]
    errors: list[str] = []
    ...
    if value.get("identity_digest_reconciliation") != IDENTITY_DIGEST_RECONCILIATION:
        errors.append("identity_digest_reconciliation must exactly match identity, digest, lineage, and stale policy")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"identity/digest reconciliation unreadable: {exc}"]
    return validate_identity_digest_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("identity_digest_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.identity_digest_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_IDENTITY_DIGEST_RECONCILIATION_AUTHORITY_V26_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_IDENTITY_DIGEST_RECONCILIATION_AUTHORITY_V26_READY: human review remains open")
    return 0
