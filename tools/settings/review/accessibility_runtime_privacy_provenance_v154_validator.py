#!/usr/bin/env python3
"""Validate v154 accessibility runtime privacy/telemetry provenance evidence."""
...
SCHEMA = "accessibility_runtime_privacy_provenance_v154_evidence_v1"
...
PRIVACY_POLICY = {
    "telemetry_opt_in": "caller_owned",
    "event_fields": ["event_kind", "contract_id", "revision"],
    "caption_text": "never_collected",
    "profile_payload": "never_collected",
    "audio_content": "never_collected",
    "user_identity": "never_collected",
    "error_logging": "reason_and_field_only",
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_privacy_revision",
    "human_gate": "open",
...
def validate_runtime_privacy_provenance(value: Any) -> list[str]:
    """Return privacy/telemetry provenance violations without raising."""
    ...
    if value.get("privacy_policy") != PRIVACY_POLICY:
        errors.append("privacy_policy must exactly match the v154 opt-in, redaction, and no-content-collection policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v154 privacy, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/telemetry boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "privacy_bounded_presentation")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_PRIVACY_PROVENANCE_V154_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_PRIVACY_PROVENANCE_V154_READY: review and native gates remain open")
    return 0
