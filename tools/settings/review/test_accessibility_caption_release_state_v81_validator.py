...
from tools.settings.review.accessibility_caption_release_state_v81_validator import (
    AUTHORITY,
    BINDING,
    CONTRACT_ID,
    PREVIOUS_VERSION,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-release-v81",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v81 release/state review or native render has been performed",
        ...
        "release_state": copy.deepcopy(RELEASE_STATE),
...
class AccessibilityCaptionReleaseStateV81Tests(unittest.TestCase):
    def test_complete_record_keeps_release_unclaimed_and_native_gate_closed(self):
        self.assertEqual(validate_release_state(_record()), [])

    def test_v80_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_evidence_closure_v79_evidence_v1"
        value["source_evidence_id"] = "caption-accessibility-evidence-v79"
        value["release_state"]["previous_version"] = "v79"
        errors = validate_release_state(value)
        ...
        self.assertTrue(any("release_state must exactly" in error for error in errors))
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v81.json", "sha256": "bad"}]
        errors = validate_release_state(value)
