...
from tools.settings.review.accessibility_caption_source_lineage_v90_validator import (
    AUTHORITY,
    BINDING,
    CONTRACT_ID,
    LINEAGE_CHECKS,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-source-v90",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v90 source/lineage review or native render has been performed",
        ...
        "lineage_checks": copy.deepcopy(LINEAGE_CHECKS),
...
class AccessibilityCaptionSourceLineageV90Tests(unittest.TestCase):
    def test_complete_record_keeps_source_unverified_and_native_gate_closed(self):
        self.assertEqual(validate_source_lineage(_record()), [])

    def test_v89_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_consistency_state_v88_evidence_v1"
        value["source_consistency_id"] = "caption-accessibility-consistency-v88"
        value["source_lineage"]["previous_version"] = "v88"
        errors = validate_source_lineage(value)
        ...
        self.assertTrue(any("source_lineage must exactly" in error for error in errors))

    def test_human_native_and_lineage_gates_remain_open(self):
        value = _record()
        value["native_render_status"] = "planned"
        value["human_review_performed"] = True
        value["lineage_claimed"] = True
        errors = validate_source_lineage(value)
        self.assertTrue(any("native_render_status must remain not_run" in error for error in errors))
        self.assertTrue(any("human_review_performed must be false" in error for error in errors))
        self.assertTrue(any("lineage_claimed must be false" in error for error in errors))
    ...
    def test_lineage_checks_are_exact(self):
        value = _record()
        value["lineage_checks"]["source_owner"] = False
        value["lineage_mode"] = "best_effort"
        errors = validate_source_lineage(value)
        self.assertTrue(any("lineage_checks must exactly" in error for error in errors))
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v90.json", "sha256": "bad"}]
        errors = validate_source_lineage(value)
