#!/usr/bin/env python3
"""Validate v37 caption schema/manifest authority-digest evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_schema_manifest_digest_authority_v36_evidence_v1"
SCHEMA_VERSION = "v37"
...
def validate_schema_manifest_authority_digest(value: Any) -> list[str]:
    """Return manifest authority-digest violations without raising on bad values."""

    if not isinstance(value, dict):
        return ["schema/manifest authority digest record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "schema_manifest_authority_digest_verified", "digest_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("schema_manifest_authority_digest") != SCHEMA_MANIFEST_AUTHORITY_DIGEST:
        errors.append("schema_manifest_authority_digest must exactly match v37 manifest, authority, and stale policy")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"schema/manifest authority digest unreadable: {exc}"]
    return validate_schema_manifest_authority_digest(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("schema_manifest_authority_digest", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.schema_manifest_authority_digest)
    if errors:
        print("ACCESSIBILITY_CAPTION_SCHEMA_MANIFEST_AUTHORITY_DIGEST_V37_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_SCHEMA_MANIFEST_AUTHORITY_DIGEST_V37_READY: human review remains open")
    return 0
