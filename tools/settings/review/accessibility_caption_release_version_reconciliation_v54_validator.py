#!/usr/bin/env python3
"""Validate v54 caption release/version reconciliation evidence."""
...
SCHEMA = "accessibility_caption_release_version_reconciliation_v54_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_release_version_linkage_v53_evidence_v1"
SCHEMA_VERSION = "v54"
PREVIOUS_VERSION = "v53"
RELEASE_ID = "caption-accessibility-v54"
...
def validate_release_version_reconciliation(value: Any) -> list[str]:
    """Return release/version reconciliation violations without raising."""
    if not isinstance(value, dict):
        return ["release/version reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "release_version_reconciliation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("release_version_reconciliation") != RELEASE_VERSION_RECONCILIATION:
        errors.append("release_version_reconciliation must exactly match v54 release, reconciliation, and stale policy")
    ...
    for key, expected in (("release_id", RELEASE_ID), ("release_status", "unreleased"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("reconciliation_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"release/version reconciliation unreadable: {exc}"]
    return validate_release_version_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("release_version_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.release_version_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_RELEASE_VERSION_RECONCILIATION_V54_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_RELEASE_VERSION_RECONCILIATION_V54_READY: human review remains open")
    return 0
