#!/usr/bin/env python3
"""Validate v262 accessibility runtime outcome-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_outcome_record_provenance_v262_evidence_v1"
...
def validate_runtime_outcome_record_provenance(value: Any) -> list[str]:
    ...
    if value.get("outcome_policy") != OUTCOME_POLICY:
        errors.append("outcome_policy must exactly match the v262 outcome-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v262 outcome, human, and native policy")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_OUTCOME_RECORD_PROVENANCE_V262_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_OUTCOME_RECORD_PROVENANCE_V262_READY: review and native gates remain open")
    return 0
