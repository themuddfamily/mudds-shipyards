#!/usr/bin/env python3
"""Validate v260 accessibility runtime claim-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_claim_record_provenance_v260_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_claim_record_policy_v1"
SCHEMA_VERSION = "v260"
SOURCE_ID = "runtime-accessibility-claim-record-policy"
...
CLAIM_POLICY = {
    "sequence": ["declare_scope", "list_claims", "mark_gates", "defer_claim"],
    "artifact_types": ["validator", "test", "manifest", "evidence"],
    "claim_states": ["prepared", "pending_review", "deferred"],
    "claimed_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
...
BINDING = {
    ...
    "policy_mode": "exact", "apply_rule": "provenance_claim_only",
    "human_gate": "open", "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True, "claim_record_authority": False,
...
def validate_runtime_claim_record_provenance(value: Any) -> list[str]:
    """Return claim-record provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime claim-record provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "claim_written", "claim_accepted"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("claim_policy") != CLAIM_POLICY:
        errors.append("claim_policy must exactly match the v260 claim-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v260 claim, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/claim boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_accessibility_claim_record_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime claim-record provenance unreadable: {exc}"]
    return validate_runtime_claim_record_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_CLAIM_RECORD_PROVENANCE_V260_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_CLAIM_RECORD_PROVENANCE_V260_READY: review and native gates remain open")
    return 0
