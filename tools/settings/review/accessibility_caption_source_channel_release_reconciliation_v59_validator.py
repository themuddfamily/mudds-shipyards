#!/usr/bin/env python3
"""Validate v59 caption source/channel/release reconciliation evidence."""
...
SCHEMA = "accessibility_caption_source_channel_release_reconciliation_v59_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_channel_release_source_reconciliation_v58_evidence_v1"
SCHEMA_VERSION = "v59"
PREVIOUS_VERSION = "v58"
RELEASE_ID = "caption-accessibility-v59"
...
SOURCE_CHANNEL_RELEASE_RECONCILIATION = {
    "source_id": SOURCE_ID,
    "source_mode": "exact",
...
def validate_source_channel_release_reconciliation(value: Any) -> list[str]:
    """Return source/channel/release violations without raising."""
    if not isinstance(value, dict):
        return ["source/channel/release reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "source_channel_release_reconciliation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("source_channel_release_reconciliation") != SOURCE_CHANNEL_RELEASE_RECONCILIATION:
        errors.append("source_channel_release_reconciliation must exactly match v59 source, channel, release, and stale policy")
    ...
    for key, expected in (("source_id", SOURCE_ID), ("source_mode", "exact"), ("channel_id", CHANNEL_ID), ("channel_mode", "exact"), ("release_id", RELEASE_ID), ("release_status", "unreleased"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("reconciliation_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"source/channel/release reconciliation unreadable: {exc}"]
    return validate_source_channel_release_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("source_channel_release_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.source_channel_release_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_SOURCE_CHANNEL_RELEASE_RECONCILIATION_V59_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_SOURCE_CHANNEL_RELEASE_RECONCILIATION_V59_READY: human review remains open")
    return 0
