#!/usr/bin/env python3
"""Validate v49 dual-version authority evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_dual_version_reconciliation_v48_evidence_v1"
SCHEMA_VERSION = "v49"
PREVIOUS_VERSION = "v48"
...
def validate_dual_version_authority(value: Any) -> list[str]:
    """Return dual-version authority violations without raising."""
    if not isinstance(value, dict):
        return ["dual-version authority record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "dual_version_authority_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("dual_version_authority") != DUAL_VERSION_AUTHORITY:
        errors.append("dual_version_authority must exactly match v49 versions, authority, and stale policy")
    ...
    for key, expected in (("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("authority_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"dual-version authority unreadable: {exc}"]
    return validate_dual_version_authority(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("dual_version_authority", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.dual_version_authority)
    if errors:
        print("ACCESSIBILITY_CAPTION_DUAL_VERSION_AUTHORITY_V49_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_DUAL_VERSION_AUTHORITY_V49_READY: human review remains open")
    return 0
