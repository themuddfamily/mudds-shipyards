#!/usr/bin/env python3
"""Validate v56 caption channel/release reconciliation evidence."""
...
SCHEMA = "accessibility_caption_channel_release_reconciliation_v56_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_release_version_reconciliation_v55_evidence_v1"
SCHEMA_VERSION = "v56"
PREVIOUS_VERSION = "v55"
RELEASE_ID = "caption-accessibility-v56"
...
def validate_channel_release_reconciliation(value: Any) -> list[str]:
    """Return channel/release violations without raising on malformed values."""
    if not isinstance(value, dict):
        return ["channel/release reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "channel_release_reconciliation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("channel_release_reconciliation") != CHANNEL_RELEASE_RECONCILIATION:
        errors.append("channel_release_reconciliation must exactly match v56 channel, release, and stale policy")
    ...
    for key, expected in (("channel_id", CHANNEL_ID), ("channel_mode", "exact"), ("release_id", RELEASE_ID), ("release_status", "unreleased"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("reconciliation_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"channel/release reconciliation unreadable: {exc}"]
    return validate_channel_release_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("channel_release_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.channel_release_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_CHANNEL_RELEASE_RECONCILIATION_V56_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_CHANNEL_RELEASE_RECONCILIATION_V56_READY: human review remains open")
    return 0
