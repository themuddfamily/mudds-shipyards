#!/usr/bin/env python3
"""Validate v62 accessibility-caption source-integrity/linkage evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_attestation_source_reconciliation_v61_evidence_v1"
SCHEMA_VERSION = "v62"
PREVIOUS_VERSION = "v61"
...
def validate_source_integrity_linkage(value: Any) -> list[str]:
    """Return source-integrity/linkage violations without raising."""
    if not isinstance(value, dict):
        return ["source-integrity/linkage record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "source_integrity_linkage_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("source_integrity_linkage") != SOURCE_INTEGRITY_LINKAGE:
        errors.append("source_integrity_linkage must exactly match v62 source integrity, channel, and stale policy")
    ...
    for key, expected in (("source_id", SOURCE_ID), ("source_mode", "exact"), ("source_status", "open"), ("integrity_mode", "exact"), ("channel_id", CHANNEL_ID), ("channel_mode", "exact"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
    if value.get("status") not in STATUSES:
        errors.append("status must remain planned, pending, or not_performed")
    if value.get("authority_owner") != SOURCE_ID:
        errors.append(f"authority_owner must identify {SOURCE_ID}")
    if value.get("generation_owner") != SOURCE_ID:
        errors.append(f"generation_owner must identify {SOURCE_ID}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"source-integrity/linkage unreadable: {exc}"]
    return validate_source_integrity_linkage(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("source_integrity_linkage", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.source_integrity_linkage)
    if errors:
        print("ACCESSIBILITY_CAPTION_SOURCE_INTEGRITY_LINKAGE_V62_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_SOURCE_INTEGRITY_LINKAGE_V62_READY: source attestation remains open")
    return 0
