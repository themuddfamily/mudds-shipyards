#!/usr/bin/env python3
"""Validate v89 accessibility-caption consistency/state evidence."""
...
SCHEMA = "accessibility_caption_consistency_state_v89_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_provenance_closure_v88_evidence_v1"
SCHEMA_VERSION = "v89"
PREVIOUS_VERSION = "v88"
SOURCE_ID = "caption-presentation-service"
SOURCE_PROVENANCE_ID = "caption-accessibility-provenance-v88"
CONSISTENCY_ID = "caption-accessibility-consistency-v89"
...
BINDING = {
    "consistency_scope": "provenance_to_state",
    "source_id": SOURCE_ID,
...
def validate_consistency_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v88 source, authority, stale, human, and native policy")
    if value.get("consistency_state") != CONSISTENCY_STATE:
        errors.append("consistency_state must exactly match v89 consistency, state, and open-gate policy")
    if value.get("consistency_checks") != CONSISTENCY_CHECKS:
        errors.append("consistency_checks must exactly match the bounded v89 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_CONSISTENCY_STATE_V89_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_CONSISTENCY_STATE_V89_READY: consistency remains unverified")
    return 0
