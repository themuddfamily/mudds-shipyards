#!/usr/bin/env python3
"""Validate v121 accessibility runtime presentation-route provenance."""
...
SCHEMA = "accessibility_runtime_route_provenance_v121_evidence_v1"
...
ROUTES = {
    "observed_cue": "caller_observation",
    "visual_decision": "accessibility-visual-preset",
    "audio_decision": "audio-accessibility-preset",
    "caption_decision": "caption-accessibility-contract",
    "safe_area_decision": "ultrawide-safe-area-contract",
    "server_browser_prompt": "server-browser-presenter",
}
FALLBACKS = {
    "inaudible_audio": "caption_textual_fallback",
    "colour_safe": "shape_cue",
    "reduced_flash": "steady_no_flash",
    "reduced_motion": "steady_no_motion",
    "ultrawide": "fit_inside_safe_rect",
}
BINDING = {
...
def validate_runtime_route_provenance(value: Any) -> list[str]:
    """Return route provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime route provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "route_verified", "runtime_claimed", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("routes") != ROUTES:
        errors.append("routes must exactly match the v121 observed-cue presentation route")
    if value.get("fallbacks") != FALLBACKS:
        errors.append("fallbacks must exactly match the v121 accessibility alternatives")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v121 source, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "presentation_only")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime route provenance unreadable: {exc}"]
    return validate_runtime_route_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_ROUTE_PROVENANCE_V121_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_ROUTE_PROVENANCE_V121_READY: review and native gates remain open")
    return 0
