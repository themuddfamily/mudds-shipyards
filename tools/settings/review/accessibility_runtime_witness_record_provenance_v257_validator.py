#!/usr/bin/env python3
"""Validate v257 accessibility runtime witness-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_witness_record_provenance_v257_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_witness_record_policy_v1"
SCHEMA_VERSION = "v257"
SOURCE_ID = "runtime-accessibility-witness-record-policy"
...
WITNESS_POLICY = {
    "sequence": ["declare_scope", "list_witnesses", "mark_gates", "defer_witness"],
    "artifact_types": ["validator", "test", "manifest", "evidence"],
    "witness_states": ["prepared", "pending_review", "deferred"],
    "witnessed_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
...
BINDING = {
    ...
    "policy_mode": "exact", "apply_rule": "provenance_witness_only",
    "human_gate": "open", "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True, "witness_record_authority": False,
...
def validate_runtime_witness_record_provenance(value: Any) -> list[str]:
    """Return witness-record provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime witness-record provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "witness_written", "witness_confirmed"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("witness_policy") != WITNESS_POLICY:
        errors.append("witness_policy must exactly match the v257 witness-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v257 witness, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/witness boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_accessibility_witness_record_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime witness-record provenance unreadable: {exc}"]
    return validate_runtime_witness_record_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_WITNESS_RECORD_PROVENANCE_V257_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_WITNESS_RECORD_PROVENANCE_V257_READY: review and native gates remain open")
    return 0
