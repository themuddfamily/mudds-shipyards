#!/usr/bin/env python3
"""Validate v88 accessibility-caption provenance/closure evidence."""
...
SCHEMA = "accessibility_caption_provenance_closure_v88_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_audit_closure_v87_evidence_v1"
SCHEMA_VERSION = "v88"
PREVIOUS_VERSION = "v87"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v87"
PROVENANCE_ID = "caption-accessibility-provenance-v88"
...
BINDING = {
    "provenance_scope": "audit_to_closure",
    "source_id": SOURCE_ID,
...
def validate_provenance_closure(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v87 source, authority, stale, human, and native policy")
    if value.get("provenance_closure") != PROVENANCE_CLOSURE:
        errors.append("provenance_closure must exactly match v88 provenance, closure, and open-gate policy")
    if value.get("provenance_checks") != PROVENANCE_CHECKS:
        errors.append("provenance_checks must exactly match the bounded v88 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_PROVENANCE_CLOSURE_V88_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PROVENANCE_CLOSURE_V88_READY: provenance remains unverified")
    return 0
