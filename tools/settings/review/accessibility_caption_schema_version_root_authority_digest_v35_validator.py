#!/usr/bin/env python3
"""Validate v35 schema/version root-authority digest evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_versioned_root_authority_digest_v34_evidence_v1"
SCHEMA_VERSION = "v35"
...
def validate_schema_version_root_authority_digest(value: Any) -> list[str]:
    """Return schema/version root-digest violations without raising on bad values."""

    if not isinstance(value, dict):
        return ["schema/version root-authority digest record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "schema_version_verified", "digest_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("schema_version_root_authority_digest") != SCHEMA_VERSION_ROOT_AUTHORITY_DIGEST:
        errors.append("schema_version_root_authority_digest must exactly match v35 root, digest, and stale policy")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"schema/version root digest unreadable: {exc}"]
    return validate_schema_version_root_authority_digest(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("schema_version_root_authority_digest", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.schema_version_root_authority_digest)
    if errors:
        print("ACCESSIBILITY_CAPTION_SCHEMA_VERSION_ROOT_AUTHORITY_DIGEST_V35_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_SCHEMA_VERSION_ROOT_AUTHORITY_DIGEST_V35_READY: human review remains open")
    return 0
