#!/usr/bin/env python3
"""Validate v83 accessibility-caption audit/lineage evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_provenance_state_v82_evidence_v1"
SCHEMA_VERSION = "v83"
PREVIOUS_VERSION = "v82"
SOURCE_ID = "caption-presentation-service"
SOURCE_PROVENANCE_ID = "caption-accessibility-provenance-v82"
AUDIT_ID = "caption-accessibility-audit-v83"
...
BINDING = {
    "audit_scope": "provenance_to_lineage",
    "source_id": SOURCE_ID,
...
def validate_audit_lineage(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v82 source, authority, stale, human, and native policy")
    if value.get("audit_lineage") != AUDIT_LINEAGE:
        errors.append("audit_lineage must exactly match v83 audit, source, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v83 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_LINEAGE_V83_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_LINEAGE_V83_READY: audit remains open")
    return 0
