#!/usr/bin/env python3
"""Validate v14 identity/authority entries for the caption manifest."""
...
IDENTITY_RULES = {
    "contract": {"identity": "caption-accessibility-contract", "version": 1, "authority": "presentation_only", "stale_policy": "observes_service_generation"},
    "service": {"identity": "caption-presentation-service", "version": 1, "authority": "presentation_only", "stale_policy": "reset_invalidates_old_payload"},
    "summary": {"identity": "caption-manifest-review-summary", "version": 14, "authority": "review_index_only", "stale_policy": "does_not_mutate_runtime"},
}
...
def _validate_entries(value: Any) -> list[str]:
    ...
    if not isinstance(value, list):
        return ["entries must contain exactly three ordered manifest identities"]
    if len(value) != len(IDENTITY_IDS):
        errors.append("entries must contain exactly three ordered manifest identities")
    ids: list[str] = []
    for index, entry in enumerate(value):
        ...
        if entry_id not in IDENTITY_IDS:
            ...
            for key in ("identity", "version", "authority", "stale_policy"):
                if entry.get(key) != expected[key]:
                    errors.append(f"{prefix}.{key} must match its manifest identity rule")
...
def validate_manifest(value: Any) -> list[str]:
    ...
    if value.get("canonicalization") != "utf8_json_sorted_keys_no_whitespace_v14":
        errors.append("canonicalization must be utf8_json_sorted_keys_no_whitespace_v14")
    ...
    if value.get("identity_policy") != "unique_identity_version_authority_tuple":
        errors.append("identity_policy must be unique_identity_version_authority_tuple")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_MANIFEST_IDENTITY_V14_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_MANIFEST_IDENTITY_V14_READY: human review remains open")
    return 0
