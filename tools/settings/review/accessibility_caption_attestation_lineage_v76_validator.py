#!/usr/bin/env python3
"""Validate v76 accessibility-caption attestation/lineage evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_source_closure_v75_evidence_v1"
SCHEMA_VERSION = "v76"
PREVIOUS_VERSION = "v75"
SOURCE_ID = "caption-presentation-service"
SOURCE_CLOSURE_ID = "caption-accessibility-source-v75"
ATTESTATION_ID = "caption-accessibility-attestation-v76"
...
BINDING = {
    "attestation_scope": "source_to_lineage",
    "source_id": SOURCE_ID,
...
def validate_attestation_lineage(value: Any) -> list[str]:
    ...
    for key in ("human_review_performed", "native_render_performed", "attestation_claimed", "lineage_verified", "attestation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v75 source, authority, stale, human, and native policy")
    if value.get("attestation_lineage") != ATTESTATION_LINEAGE:
        errors.append("attestation_lineage must exactly match v76 attestation, source, and open-gate policy")
    if value.get("attestation_checks") != ATTESTATION_CHECKS:
        errors.append("attestation_checks must exactly match the bounded v76 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_ATTESTATION_LINEAGE_V76_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_ATTESTATION_LINEAGE_V76_READY: attestation remains unverified")
    return 0
