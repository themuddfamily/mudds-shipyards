#!/usr/bin/env python3
"""Validate v30 paired lineage-root authority evidence for captions."""
...
SOURCE_SCHEMA = "accessibility_caption_paired_digest_lineage_authority_v29_evidence_v1"
...
def validate_lineage_root(value: Any) -> list[str]:
    """Return lineage-root violations without raising on malformed shapes."""

    if not isinstance(value, dict):
        return ["lineage-root record must be an object"]
    errors: list[str] = []
    ...
    if value.get("lineage_root_authority") != LINEAGE_ROOT_AUTHORITY:
        errors.append("lineage_root_authority must exactly match root ownership and stale policy")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"lineage-root unreadable: {exc}"]
    return validate_lineage_root(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors = validate(args.lineage_root)
    if errors:
        print("ACCESSIBILITY_CAPTION_PAIRED_LINEAGE_ROOT_AUTHORITY_V30_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PAIRED_LINEAGE_ROOT_AUTHORITY_V30_READY: human review remains open")
    return 0
