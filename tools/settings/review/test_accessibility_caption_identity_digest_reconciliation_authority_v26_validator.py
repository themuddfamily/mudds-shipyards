...
from tools.settings.review.accessibility_caption_identity_digest_reconciliation_authority_v26_validator import (
    AUTHORITY,
    BINDING,
    DIGEST_AUTHORITY,
    IDENTITY_DIGEST_RECONCILIATION,
    SOURCE_SCHEMA,
    validate_identity_digest_reconciliation,
)


def _record() -> dict:
    return {
        "schema": "accessibility_caption_identity_digest_reconciliation_authority_v26_evidence_v1",
        "source_schema": SOURCE_SCHEMA,
        "source_revision": "working-tree-caption-identity-digest-v26",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v26 identity/digest review or native render has been performed",
        "human_review_status": "not_performed",
        "native_render_status": "not_run",
        "human_review_performed": False,
        "native_render_performed": False,
        "identity_digest_reconciled": False,
        "digest_verified": False,
        "stale_payload_mutation": False,
        "identity_digest_reconciliation": copy.deepcopy(IDENTITY_DIGEST_RECONCILIATION),
...
class AccessibilityCaptionIdentityDigestReconciliationAuthorityV26Tests(unittest.TestCase):
    def test_complete_record_keeps_review_and_native_gates_open(self):
        self.assertEqual(validate_identity_digest_reconciliation(_record()), [])

    def test_identity_digest_and_binding_authority_are_exact(self):
        value = _record()
        value["identity_digest_reconciliation"]["digest_mode"] = "best_effort"
        value["digest_authority"]["digest_owner"] = "audio-service"
        value["binding"]["authority"] = "audio"
        errors = validate_identity_digest_reconciliation(value)
        self.assertTrue(any("identity_digest_reconciliation must exactly" in error for error in errors))
    ...
    def test_stale_and_non_presentation_authority_fail_closed(self):
        ...
        value["identity_digest_reconciled"] = True
        value["stale_payload_mutation"] = True
        errors = validate_identity_digest_reconciliation(value)
        self.assertTrue(any("authority must exactly" in error for error in errors))
        self.assertTrue(any("audio_authority must be false" in error for error in errors))
        self.assertTrue(any("identity_digest_reconciled must be false" in error for error in errors))
    ...
    def test_identity_digest_and_evidence_require_sha256_metadata(self):
        ...
        value["evidence"] = [{"kind": "report", "path": "reports/v26.json", "sha256": "bad"}]
        errors = validate_identity_digest_reconciliation(value)
    ...
    def test_malformed_shapes_fail_without_throwing(self):
        ...
        value["identity_digest_reconciliation"] = []
