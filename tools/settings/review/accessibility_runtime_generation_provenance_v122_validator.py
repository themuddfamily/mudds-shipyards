#!/usr/bin/env python3
"""Validate v122 accessibility runtime generation/lifecycle provenance."""
...
SCHEMA = "accessibility_runtime_generation_provenance_v122_evidence_v1"
...
LIFECYCLE = {
    "profile_replacement": "atomic_complete_profile",
    "generation_advance": "one_per_accepted_replacement",
    "stale_generation": "reject_stale_generation",
    "detach": "retain_same_generation_snapshot",
    "reentry": "reapply_same_generation_snapshot",
    "reset": "clear_transient_edges_before_new_generation",
}
BINDING = {
    ...
    "stale_policy": "reject_stale_generation",
    "human_gate": "open",
...
def validate_runtime_generation_provenance(value: Any) -> list[str]:
    """Return lifecycle provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime generation provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "lifecycle_verified", "runtime_claimed", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("lifecycle") != LIFECYCLE:
        errors.append("lifecycle must exactly match the v122 generation and re-entry policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v122 stale, human, and native policy")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime generation provenance unreadable: {exc}"]
    return validate_runtime_generation_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_GENERATION_PROVENANCE_V122_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_GENERATION_PROVENANCE_V122_READY: review and native gates remain open")
    return 0
