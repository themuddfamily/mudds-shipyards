#!/usr/bin/env python3
"""Validate v155 accessibility runtime settings-schema provenance evidence."""
...
SCHEMA = "accessibility_runtime_schema_provenance_v155_evidence_v1"
SOURCE_SCHEMA = "runtime_settings_accessibility_handoff_v1"
SCHEMA_VERSION = "v155"
SOURCE_ID = "runtime-settings-schema"
...
SCHEMA_POLICY = {
    "current_version": 1,
    "accessibility_fields": ["captions", "visual", "audio", "safe_area", "remap"],
    "migration": "adjacent_versions_only",
    "unknown_fields": "reject_without_mutation",
    "missing_fields": "validated_defaults",
    "newer_version": "preserve_and_reject",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_schema_revision",
    "human_gate": "open",
...
def validate_runtime_schema_provenance(value: Any) -> list[str]:
    """Return settings-schema provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime schema provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("schema_policy") != SCHEMA_POLICY:
        errors.append("schema_policy must exactly match the v155 version, field, migration, and default policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v155 schema, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/schema boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "versioned_accessibility_schema")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime schema provenance unreadable: {exc}"]
    return validate_runtime_schema_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_SCHEMA_PROVENANCE_V155_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_SCHEMA_PROVENANCE_V155_READY: review and native gates remain open")
    return 0
