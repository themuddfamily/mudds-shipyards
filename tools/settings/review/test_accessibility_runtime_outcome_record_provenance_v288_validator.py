...
from tools.settings.review.accessibility_runtime_outcome_record_provenance_v288_validator import (
    AUTHORITY, BINDING, OUTCOME_POLICY, SCHEMA, SOURCE_ID, SOURCE_SCHEMA,
    validate_runtime_outcome_record_provenance,
)


def _record() -> dict:
    return {
        "schema": SCHEMA, "source_schema": SOURCE_SCHEMA, "schema_version": "v288",
        "source_revision": "working-tree-runtime-outcome-record-v288",
        ...
        "source_id": SOURCE_ID, "contract_id": "runtime-accessibility-presentation",
        "provenance_source_of_truth": "runtime_accessibility_outcome_record_policy",
        "status": "planned", "evidence": None, **AUTHORITY,
    }


class AccessibilityRuntimeOutcomeRecordProvenanceV288Tests(unittest.TestCase):
    ...
    def test_outcome_policy_and_binding_are_exact(self):
        ...
        self.assertTrue(any("outcome_policy must exactly" in error for error in errors)); self.assertTrue(any("binding must exactly" in error for error in errors))

    def test_outcome_fields_cannot_shrink(self):
        value = _record(); value["outcome_policy"]["outcome_fields"] = ["camera"]
        self.assertTrue(any("outcome_policy must exactly" in error for error in validate_runtime_outcome_record_provenance(value)))
    ...
    def test_native_and_outcome_claims_remain_open(self):
        ...
        errors = validate_runtime_outcome_record_provenance(value)
        self.assertTrue(any("native_render_status must remain not_run" in error for error in errors)); self.assertTrue(any("outcome_written must be false" in error for error in errors))
    ...
    def test_outcome_record_authority_fails_closed(self):
        ...
        errors = validate_runtime_outcome_record_provenance(value)
        self.assertTrue(any("authority must exactly" in error for error in errors)); self.assertTrue(any("outcome_record_authority must be false" in error for error in errors))

    def test_malformed_shapes_fail_without_throwing(self):
        value = _record(); value["outcome_policy"] = []; value["binding"] = []; value["authority"] = []; value["evidence"] = [{"kind": [], "path": {}, "sha256": []}]
        errors = validate_runtime_outcome_record_provenance(value)
        self.assertTrue(any("outcome_policy must exactly" in error for error in errors)); self.assertTrue(any("binding must exactly" in error for error in errors)); self.assertTrue(any("authority must exactly" in error for error in errors)); self.assertTrue(any("evidence[0].kind" in error for error in errors))
