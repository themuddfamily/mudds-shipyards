#!/usr/bin/env python3
"""Validate v141 accessibility runtime persistence provenance evidence."""
...
SCHEMA = "accessibility_runtime_persistence_provenance_v141_evidence_v1"
SOURCE_SCHEMA = "runtime_settings_accessibility_handoff_v1"
SCHEMA_VERSION = "v141"
SOURCE_ID = "runtime-settings-persistence"
...
PERSISTENCE_POLICY = {
    ...
    "backup_mode": "validated_previous_authority",
    "schema": "versioned_user_data",
    "corrupt_data": "safe_defaults_without_overwrite_authority",
    "newer_schema": "preserve_and_reject",
    "crash_log": "redacted_no_profile_payload",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_persistence_revision",
    "human_gate": "open",
    "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True,
    "persistence_read_authority": False,
    "persistence_write_authority": False,
...
def validate_runtime_persistence_provenance(value: Any) -> list[str]:
    """Return persistence provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime persistence provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("persistence_policy") != PERSISTENCE_POLICY:
        errors.append("persistence_policy must exactly match the v141 atomic write/recovery boundary")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v141 persistence, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/persistence boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "validated_persistence_policy")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime persistence provenance unreadable: {exc}"]
    return validate_runtime_persistence_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_PERSISTENCE_PROVENANCE_V141_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_PERSISTENCE_PROVENANCE_V141_READY: review and native gates remain open")
    return 0
