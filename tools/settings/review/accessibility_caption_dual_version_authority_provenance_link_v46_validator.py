#!/usr/bin/env python3
"""Validate v46 dual-version authority/provenance link evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_dual_version_authority_provenance_v45_evidence_v1"
SCHEMA_VERSION = "v46"
PREVIOUS_VERSION = "v45"
...
def validate_dual_version_authority_provenance_link(value: Any) -> list[str]:
    """Return dual-version link violations without raising on malformed values."""
    if not isinstance(value, dict):
        return ["dual-version authority/provenance link record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "dual_version_authority_provenance_link_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("dual_version_authority_provenance_link") != DUAL_VERSION_AUTHORITY_PROVENANCE_LINK:
        errors.append("dual_version_authority_provenance_link must exactly match v46 versions, link, and stale policy")
    ...
    for key, expected in (("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("link_mode", "exact")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"dual-version authority/provenance link unreadable: {exc}"]
    return validate_dual_version_authority_provenance_link(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("dual_version_authority_provenance_link", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.dual_version_authority_provenance_link)
    if errors:
        print("ACCESSIBILITY_CAPTION_DUAL_VERSION_AUTHORITY_PROVENANCE_LINK_V46_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_DUAL_VERSION_AUTHORITY_PROVENANCE_LINK_V46_READY: human review remains open")
    return 0
