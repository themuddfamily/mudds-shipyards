#!/usr/bin/env python3
"""Validate v256 accessibility runtime proof-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_proof_record_provenance_v256_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_proof_record_policy_v1"
SCHEMA_VERSION = "v256"
SOURCE_ID = "runtime-accessibility-proof-record-policy"
...
PROOF_POLICY = {
    "sequence": ["declare_scope", "list_proofs", "mark_gates", "defer_proof"],
    "artifact_types": ["validator", "test", "manifest", "evidence"],
    "proof_states": ["prepared", "pending_review", "deferred"],
    "proved_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
...
BINDING = {
    ...
    "policy_mode": "exact", "apply_rule": "provenance_proof_only",
    "human_gate": "open", "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True, "proof_record_authority": False,
...
def validate_runtime_proof_record_provenance(value: Any) -> list[str]:
    """Return proof-record provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime proof-record provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "proof_written", "proof_accepted"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("proof_policy") != PROOF_POLICY:
        errors.append("proof_policy must exactly match the v256 proof-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v256 proof, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/proof boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_accessibility_proof_record_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime proof-record provenance unreadable: {exc}"]
    return validate_runtime_proof_record_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_PROOF_RECORD_PROVENANCE_V256_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_PROOF_RECORD_PROVENANCE_V256_READY: review and native gates remain open")
    return 0
