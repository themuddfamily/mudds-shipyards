#!/usr/bin/env python3
"""Validate v156 accessibility runtime preference-conflict provenance evidence."""
...
SCHEMA = "accessibility_runtime_conflict_provenance_v156_evidence_v1"
...
CONFLICT_POLICY = {
    "precedence": ["explicit_profile", "migrated_profile", "validated_defaults"],
    "conflict": "reject_ambiguous_source",
    "resolution": "atomic_complete_profile",
    "missing_field": "validated_default",
    "stale_source": "reject_without_mutation",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_preference_revision",
    "human_gate": "open",
...
def validate_runtime_conflict_provenance(value: Any) -> list[str]:
    """Return preference-conflict provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime preference-conflict provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("conflict_policy") != CONFLICT_POLICY:
        errors.append("conflict_policy must exactly match the v156 precedence, conflict, and stale policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v156 conflict, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/conflict boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "validated_preference_precedence")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime conflict provenance unreadable: {exc}"]
    return validate_runtime_conflict_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_CONFLICT_PROVENANCE_V156_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_CONFLICT_PROVENANCE_V156_READY: review and native gates remain open")
    return 0
