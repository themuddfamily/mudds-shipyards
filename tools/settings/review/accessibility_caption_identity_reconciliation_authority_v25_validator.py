#!/usr/bin/env python3
"""Validate v25 caption identity-reconciliation authority evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_provenance_identity_reconciliation_authority_v24_evidence_v1"
...
def validate_identity_reconciliation(value: Any) -> list[str]:
    """Return identity contract violations without raising on malformed shapes."""

    if not isinstance(value, dict):
        return ["identity-reconciliation record must be an object"]
    errors: list[str] = []
    ...
    for key in (
        ...
        "identity_reconciliation_verified",
        "digest_verified",
    ...
    if value.get("identity_reconciliation") != IDENTITY_RECONCILIATION:
        errors.append("identity_reconciliation must exactly match identity, lineage, and stale policy")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"identity-reconciliation unreadable: {exc}"]
    return validate_identity_reconciliation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("identity_reconciliation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.identity_reconciliation)
    if errors:
        print("ACCESSIBILITY_CAPTION_IDENTITY_RECONCILIATION_AUTHORITY_V25_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_IDENTITY_RECONCILIATION_AUTHORITY_V25_READY: human review remains open")
    return 0
