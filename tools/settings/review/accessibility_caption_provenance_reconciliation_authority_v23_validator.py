#!/usr/bin/env python3
"""Validate v23 caption provenance-reconciliation authority evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_reconciliation_digest_authority_v22_evidence_v1"
...
def validate_provenance_authority(value: Any) -> list[str]:
    """Return provenance contract violations without raising on malformed shapes."""

    if not isinstance(value, dict):
        return ["provenance-authority record must be an object"]
    errors: list[str] = []
    ...
    if provenance_digest is not None and not _digest(provenance_digest):
        errors.append("provenance_digest must be null or a lowercase 64-character digest")
    if value.get("status") not in STATUSES:
        errors.append("status must remain planned, pending, or not_performed")
    if value.get("provenance_owner") != "caption-presentation-service":
        errors.append("provenance_owner must identify caption-presentation-service")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"provenance-authority unreadable: {exc}"]
    return validate_provenance_authority(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("provenance_authority", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.provenance_authority)
    if errors:
        print("ACCESSIBILITY_CAPTION_PROVENANCE_RECONCILIATION_AUTHORITY_V23_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PROVENANCE_RECONCILIATION_AUTHORITY_V23_READY: human review remains open")
    return 0
