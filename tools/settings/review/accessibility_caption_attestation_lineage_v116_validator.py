#!/usr/bin/env python3
"""Validate v116 accessibility-caption attestation/lineage evidence."""
...
SCHEMA = "accessibility_caption_attestation_lineage_v116_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_audit_state_v115_evidence_v1"
SCHEMA_VERSION = "v116"
PREVIOUS_VERSION = "v115"
SOURCE_ID = "caption-presentation-service"
SOURCE_AUDIT_ID = "caption-accessibility-audit-v115"
ATTESTATION_ID = "caption-accessibility-attestation-v116"
...
BINDING = {
    "attestation_scope": "audit_to_lineage",
    "source_id": SOURCE_ID,
...
def validate_attestation_lineage(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v115 source, authority, stale, human, and native policy")
    if value.get("attestation_lineage") != ATTESTATION_LINEAGE:
        errors.append("attestation_lineage must exactly match v116 attestation, source, and open-gate policy")
    if value.get("attestation_checks") != ATTESTATION_CHECKS:
        errors.append("attestation_checks must exactly match the bounded v116 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_ATTESTATION_LINEAGE_V116_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_ATTESTATION_LINEAGE_V116_READY: attestation remains unverified")
    return 0
