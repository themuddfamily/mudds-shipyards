#!/usr/bin/env python3
"""Validate v70 accessibility-caption provenance/closure evidence."""
...
SCHEMA = "accessibility_caption_provenance_closure_v70_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_evidence_lineage_v69_evidence_v1"
SCHEMA_VERSION = "v70"
PREVIOUS_VERSION = "v69"
SOURCE_ID = "caption-presentation-service"
SOURCE_LINEAGE_ID = "caption-accessibility-lineage-v69"
PROVENANCE_ID = "caption-accessibility-provenance-v70"
...
BINDING = {
    "provenance_scope": "lineage_to_closure",
    "source_id": SOURCE_ID,
...
PROVENANCE_CHECKS = {
    ...
    "source_lineage_open": True,
    "provenance_owner": True,
...
def validate_provenance_closure(value: Any) -> list[str]:
    """Return provenance/closure violations without raising."""
    if not isinstance(value, dict):
        return ["provenance/closure record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "provenance_claimed", "closure_claimed", "provenance_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v69 source, authority, stale, human, and native policy")
    if value.get("provenance_closure") != PROVENANCE_CLOSURE:
        errors.append("provenance_closure must exactly match v70 provenance, closure, and open-gate policy")
    if value.get("provenance_checks") != PROVENANCE_CHECKS:
        errors.append("provenance_checks must exactly match the bounded v70 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"provenance/closure unreadable: {exc}"]
    return validate_provenance_closure(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("provenance_closure", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.provenance_closure)
    if errors:
        print("ACCESSIBILITY_CAPTION_PROVENANCE_CLOSURE_V70_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_PROVENANCE_CLOSURE_V70_READY: provenance remains unverified")
    return 0
