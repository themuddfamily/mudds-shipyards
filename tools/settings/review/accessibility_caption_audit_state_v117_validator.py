#!/usr/bin/env python3
"""Validate v117 accessibility-caption audit/state evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_attestation_lineage_v116_evidence_v1"
SCHEMA_VERSION = "v117"
PREVIOUS_VERSION = "v116"
SOURCE_ID = "caption-presentation-service"
SOURCE_ATTESTATION_ID = "caption-accessibility-attestation-v116"
AUDIT_ID = "caption-accessibility-audit-v117"
...
def validate_audit_state(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v116 source, authority, stale, human, and native policy")
    if value.get("audit_state") != AUDIT_STATE:
        errors.append("audit_state must exactly match v117 audit, state, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v117 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V117_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_STATE_V117_READY: audit remains unverified")
    return 0
