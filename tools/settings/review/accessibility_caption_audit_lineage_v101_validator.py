#!/usr/bin/env python3
"""Validate v101 accessibility-caption audit/lineage evidence."""
...
SCHEMA = "accessibility_caption_audit_lineage_v101_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_verification_closure_v100_evidence_v1"
SCHEMA_VERSION = "v101"
PREVIOUS_VERSION = "v100"
SOURCE_ID = "caption-presentation-service"
SOURCE_VERIFICATION_ID = "caption-accessibility-verification-v100"
AUDIT_ID = "caption-accessibility-audit-v101"
...
def validate_audit_lineage(value: Any) -> list[str]:
    ...
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v100 source, authority, stale, human, and native policy")
    if value.get("audit_lineage") != AUDIT_LINEAGE:
        errors.append("audit_lineage must exactly match v101 audit, source, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v101 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_LINEAGE_V101_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_LINEAGE_V101_READY: audit remains unverified")
    return 0
