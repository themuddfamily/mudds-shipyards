#!/usr/bin/env python3
"""Validate v40 linked-authority digest evidence for captions."""
...
SOURCE_SCHEMA = "accessibility_caption_authority_reconciliation_digest_v39_evidence_v1"
SCHEMA_VERSION = "v40"
...
def validate_linked_authority_digest(value: Any) -> list[str]:
    """Return linked-authority violations without raising on malformed values."""
    if not isinstance(value, dict):
        return ["linked-authority digest record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "linked_authority_digest_verified", "digest_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("linked_authority_digest") != LINKED_AUTHORITY_DIGEST:
        errors.append("linked_authority_digest must exactly match v40 link, authority, and stale policy")
    ...
    for key in ("authority_digest", "linked_digest"):
    ...
    for key in ("authority_owner", "digest_owner", "generation_owner"):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"linked-authority digest unreadable: {exc}"]
    return validate_linked_authority_digest(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("linked_authority_digest", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.linked_authority_digest)
    if errors:
        print("ACCESSIBILITY_CAPTION_LINKED_AUTHORITY_DIGEST_V40_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_LINKED_AUTHORITY_DIGEST_V40_READY: human review remains open")
    return 0
