#!/usr/bin/env python3
"""Validate v153 accessibility runtime performance provenance evidence."""
...
SCHEMA = "accessibility_runtime_performance_provenance_v153_evidence_v1"
...
PERFORMANCE_POLICY = {
    "caption_layout": "deferred_coalesced",
    "snapshot_copy": "deep_detached_on_boundary",
    "cue_resolution": "bounded_single_cue_decision",
    "prompt_generation": "static_textual_dictionary",
    "update_cadence": "caller_physics_or_event_driven",
    "native_budget": "not_run",
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_performance_revision",
    "human_gate": "open",
...
def validate_runtime_performance_provenance(value: Any) -> list[str]:
    """Return performance provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime performance provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("performance_policy") != PERFORMANCE_POLICY:
        errors.append("performance_policy must exactly match the v153 bounded presentation work policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v153 performance, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/performance boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "bounded_presentation_work")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime performance provenance unreadable: {exc}"]
    return validate_runtime_performance_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_PERFORMANCE_PROVENANCE_V153_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_PERFORMANCE_PROVENANCE_V153_READY: review and native gates remain open")
    return 0
