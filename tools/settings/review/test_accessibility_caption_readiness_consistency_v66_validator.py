import copy
import unittest

from tools.settings.review.accessibility_caption_readiness_consistency_v66_validator import (
    AUTHORITY,
    BINDING,
    CONSISTENCY_CHECKS,
    CONTRACT_ID,
    PREVIOUS_VERSION,
    READINESS_CONSISTENCY,
    READINESS_ID,
    SCHEMA_VERSION,
    SOURCE_ATTESTATION_ID,
    SOURCE_ID,
    SOURCE_SCHEMA,
    validate_readiness_consistency,
)


def _record() -> dict:
    return {
        "schema": "accessibility_caption_readiness_consistency_v66_evidence_v1",
        "source_schema": SOURCE_SCHEMA,
        "schema_version": SCHEMA_VERSION,
        "source_revision": "working-tree-caption-readiness-v66",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v66 readiness/consistency review or native render has been performed",
        "human_review_status": "not_performed",
        "native_render_status": "not_run",
        "human_review_performed": False,
        "native_render_performed": False,
        "readiness_claimed": False,
        "consistency_verified": False,
        "stale_payload_mutation": False,
        "readiness_consistency": copy.deepcopy(READINESS_CONSISTENCY),
        "binding": copy.deepcopy(BINDING),
        "consistency_checks": copy.deepcopy(CONSISTENCY_CHECKS),
        "authority": copy.deepcopy(AUTHORITY),
        "readiness_id": READINESS_ID,
        "readiness_status": "open",
        "consistency_mode": "exact",
        "source_id": SOURCE_ID,
        "source_mode": "exact",
        "source_attestation_id": SOURCE_ATTESTATION_ID,
        "source_attestation_status": "open",
        "contract_id": CONTRACT_ID,
        "contract_mode": "exact",
        "current_version": SCHEMA_VERSION,
        "previous_version": PREVIOUS_VERSION,
        "version_relation": "adjacent_exact",
        "provenance_source_of_truth": "presentation_only",
        "status": "planned",
        "authority_owner": SOURCE_ID,
        "generation_owner": SOURCE_ID,
        "service_id": SOURCE_ID,
        "evidence": None,
        **AUTHORITY,
    }


class AccessibilityCaptionReadinessConsistencyV66Tests(unittest.TestCase):
    def test_complete_record_keeps_readiness_open_and_native_gate_closed(self):
        self.assertEqual(validate_readiness_consistency(_record()), [])

    def test_v65_source_continuity_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_verification_attestation_v64_evidence_v1"
        value["source_attestation_id"] = "caption-accessibility-v64"
        value["readiness_consistency"]["previous_version"] = "v64"
        errors = validate_readiness_consistency(value)
        self.assertTrue(any("source_schema must be" in error for error in errors))
        self.assertTrue(any("source_attestation_id must be" in error for error in errors))
        self.assertTrue(any("readiness_consistency must exactly" in error for error in errors))

    def test_native_and_human_gates_remain_open(self):
        value = _record()
        value["native_render_status"] = "planned"
        value["human_review_performed"] = True
        value["readiness_claimed"] = True
        errors = validate_readiness_consistency(value)
        self.assertTrue(any("native_render_status must remain not_run" in error for error in errors))
        self.assertTrue(any("human_review_performed must be false" in error for error in errors))
        self.assertTrue(any("readiness_claimed must be false" in error for error in errors))

    def test_authority_and_stale_policy_fail_closed(self):
        value = _record()
        value["authority"]["audio_authority"] = True
        value["audio_authority"] = True
        value["stale_payload_mutation"] = True
        value["binding"]["stale_policy"] = "accept_old"
        errors = validate_readiness_consistency(value)
        self.assertTrue(any("authority must exactly" in error for error in errors))
        self.assertTrue(any("audio_authority must be false" in error for error in errors))
        self.assertTrue(any("stale_payload_mutation must be false" in error for error in errors))
        self.assertTrue(any("binding must exactly" in error for error in errors))

    def test_consistency_checks_are_exact(self):
        value = _record()
        value["consistency_checks"]["native_boundary"] = False
        value["consistency_mode"] = "best_effort"
        errors = validate_readiness_consistency(value)
        self.assertTrue(any("consistency_checks must exactly" in error for error in errors))
        self.assertTrue(any("consistency_mode must be exact" in error for error in errors))

    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v66.json", "sha256": "bad"}]
        errors = validate_readiness_consistency(value)
        self.assertTrue(any("evidence[0].sha256" in error for error in errors))

    def test_malformed_shapes_fail_without_throwing(self):
        value = _record()
        value["human_review_status"] = []
        value["readiness_consistency"] = []
        value["binding"] = []
        value["consistency_checks"] = []
        value["authority"] = []
        value["evidence"] = [{"kind": [], "path": {}, "sha256": []}]
        errors = validate_readiness_consistency(value)
        self.assertTrue(any("human_review_status" in error for error in errors))
        self.assertTrue(any("readiness_consistency must exactly" in error for error in errors))
        self.assertTrue(any("binding must exactly" in error for error in errors))
        self.assertTrue(any("consistency_checks must exactly" in error for error in errors))
        self.assertTrue(any("authority must exactly" in error for error in errors))
        self.assertTrue(any("evidence[0].kind" in error for error in errors))


if __name__ == "__main__":
    unittest.main()
