#!/usr/bin/env python3
"""Validate v238 accessibility runtime retention-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_retention_record_provenance_v238_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_retention_record_policy_v1"
SCHEMA_VERSION = "v238"
SOURCE_ID = "runtime-accessibility-retention-record-policy"
...
RETENTION_POLICY = {
    "sequence": ["declare_scope", "list_records", "mark_gates", "defer_retention"],
    "artifact_types": ["validator", "test", "manifest", "evidence"],
    "retention_states": ["prepared", "pending_review", "deferred"],
    "retained_fields": ["accessibility", "captions", "audio", "bindings", "camera"],
...
BINDING = {
    ...
    "policy_mode": "exact", "apply_rule": "provenance_retention_only",
    "human_gate": "open", "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True, "retention_record_authority": False,
...
def validate_runtime_retention_record_provenance(value: Any) -> list[str]:
    """Return retention-record provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime retention-record provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "policy_verified", "runtime_claimed", "retention_written", "retention_confirmed"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("retention_policy") != RETENTION_POLICY:
        errors.append("retention_policy must exactly match the v238 retention-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v238 retention, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/retention boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "runtime_accessibility_retention_record_policy")):
        if value.get(key) != expected:
            errors.append(f"{key} must be {expected}")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime retention-record provenance unreadable: {exc}"]
    return validate_runtime_retention_record_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_RETENTION_RECORD_PROVENANCE_V238_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_RETENTION_RECORD_PROVENANCE_V238_READY: review and native gates remain open")
    return 0
