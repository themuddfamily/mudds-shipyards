#!/usr/bin/env python3
"""Validate v41 linked authority/provenance digest evidence for captions."""
...
SCHEMA = "accessibility_caption_linked_authority_provenance_digest_v41_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_linked_authority_digest_v40_evidence_v1"
SCHEMA_VERSION = "v41"
...
LINKED_AUTHORITY_PROVENANCE_DIGEST = {
    "schema_version": SCHEMA_VERSION,
    "link_scope": "authority_and_generation",
...
def validate_linked_authority_provenance_digest(value: Any) -> list[str]:
    """Return linked authority/provenance violations without raising."""
    if not isinstance(value, dict):
        return ["linked authority/provenance digest record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "linked_authority_provenance_digest_verified", "digest_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("linked_authority_provenance_digest") != LINKED_AUTHORITY_PROVENANCE_DIGEST:
        errors.append("linked_authority_provenance_digest must exactly match v41 authority, provenance, and stale policy")
    ...
    for key in ("authority_owner", "provenance_owner", "digest_owner", "generation_owner"):
    ...
    if value.get("provenance_mode") != "exact":
        errors.append("provenance_mode must remain exact")
    if value.get("digest_mode") != "exact":
        errors.append("digest_mode must remain exact")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"linked authority/provenance digest unreadable: {exc}"]
    return validate_linked_authority_provenance_digest(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("linked_authority_provenance_digest", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.linked_authority_provenance_digest)
    if errors:
        print("ACCESSIBILITY_CAPTION_LINKED_AUTHORITY_PROVENANCE_DIGEST_V41_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_LINKED_AUTHORITY_PROVENANCE_DIGEST_V41_READY: human review remains open")
    return 0
