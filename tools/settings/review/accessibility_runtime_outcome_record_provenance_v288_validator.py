#!/usr/bin/env python3
"""Validate v288 accessibility runtime outcome-record provenance evidence."""
...
SCHEMA = "accessibility_runtime_outcome_record_provenance_v288_evidence_v1"
...
def _validate_evidence(value: Any, errors: list[str]) -> None:
    ...
    for index, item in enumerate(value):
        ...
        if not isinstance(item, dict):
            ...
            continue
        if not isinstance(item.get("kind"), str) or item["kind"] not in EVIDENCE_KINDS:
...
def validate_runtime_outcome_record_provenance(value: Any) -> list[str]:
    ...
    if value.get("outcome_policy") != OUTCOME_POLICY:
        errors.append("outcome_policy must exactly match the v288 outcome-record policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v288 outcome, human, and native policy")
    ...
    _validate_evidence(value.get("evidence"), errors)
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_OUTCOME_RECORD_PROVENANCE_V288_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_OUTCOME_RECORD_PROVENANCE_V288_READY: review and native gates remain open")
    return 0
