...
from tools.settings.review.accessibility_caption_evidence_lineage_v69_validator import (
    AUTHORITY,
    BINDING,
    CONTRACT_ID,
    EVIDENCE_LINEAGE,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-evidence-v69",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v69 evidence/lineage review or native render has been performed",
...
class AccessibilityCaptionEvidenceLineageV69Tests(unittest.TestCase):
    def test_complete_record_keeps_evidence_unverified_and_native_gate_closed(self):
        self.assertEqual(validate_evidence_lineage(_record()), [])

    def test_v68_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_audit_closure_v67_evidence_v1"
        value["source_audit_id"] = "caption-accessibility-audit-v67"
        value["evidence_lineage"]["previous_version"] = "v67"
        errors = validate_evidence_lineage(value)
        ...
        self.assertTrue(any("evidence_lineage must exactly" in error for error in errors))
    ...
    def test_lineage_checks_are_exact(self):
        value = _record()
        value["lineage_checks"]["evidence_hashes"] = False
    ...
    def test_evidence_requires_sha256_metadata_when_present(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v69.json", "sha256": "bad"}]
        errors = validate_evidence_lineage(value)
