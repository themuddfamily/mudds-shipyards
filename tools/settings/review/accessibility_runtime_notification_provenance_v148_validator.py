#!/usr/bin/env python3
"""Validate v148 accessibility runtime notification provenance evidence."""
...
SCHEMA = "accessibility_runtime_notification_provenance_v148_evidence_v1"
...
NOTIFICATION_POLICY = {
    "priority": "caller_authored_bounded_integer",
    "semantic_text": "required_for_user_visible_notification",
    "persistence": "caller_authored_duration",
    "dismissal": "user_dismissible_without_gameplay_mutation",
    "reduced_motion": "steady_no_motion",
    "reduced_flash": "steady_no_flash",
    "duplicate_policy": "stable_id_and_sequence",
}
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_notification_revision",
    "human_gate": "open",
...
def validate_runtime_notification_provenance(value: Any) -> list[str]:
    ...
    if value.get("notification_policy") != NOTIFICATION_POLICY:
        errors.append("notification_policy must exactly match the v148 semantic, priority, dismissal, and reduced-motion policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v148 notification, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/notification boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "notification_presentation_policy")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_NOTIFICATION_PROVENANCE_V148_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_NOTIFICATION_PROVENANCE_V148_READY: review and native gates remain open")
    return 0
