#!/usr/bin/env python3
"""Validate v147 accessibility runtime loading-continuity provenance evidence."""
...
SCHEMA = "accessibility_runtime_loading_provenance_v147_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_presentation_v1"
SCHEMA_VERSION = "v147"
SOURCE_ID = "loading-screen-accessibility"
...
LOADING_POLICY = {
    "status_text": "semantic_textual_state",
    "progress": "bounded_numeric_or_indeterminate",
    "caption_preference": "retained_profile",
    "ui_scale": "retained_profile",
    "reduced_motion": "steady_no_motion",
    "missing_profile": "safe_accessibility_defaults",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_loading_revision",
    "human_gate": "open",
    "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True,
    "loading_authority": False,
...
def validate_runtime_loading_provenance(value: Any) -> list[str]:
    """Return loading-continuity provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime loading provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("loading_policy") != LOADING_POLICY:
        errors.append("loading_policy must exactly match the v147 status, progress, retention, and fallback policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v147 loading, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/loading boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "loading_accessibility_policy")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime loading provenance unreadable: {exc}"]
    return validate_runtime_loading_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_LOADING_PROVENANCE_V147_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_LOADING_PROVENANCE_V147_READY: review and native gates remain open")
    return 0
