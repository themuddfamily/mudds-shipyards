#!/usr/bin/env python3
"""Validate v42 linked authority/provenance evidence for captions."""
...
SOURCE_SCHEMA = "accessibility_caption_linked_authority_provenance_digest_v41_evidence_v1"
SCHEMA_VERSION = "v42"
...
def validate_linked_authority_provenance(value: Any) -> list[str]:
    """Return linked authority/provenance violations without raising."""
    if not isinstance(value, dict):
        return ["linked authority/provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "linked_authority_provenance_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("linked_authority_provenance") != LINKED_AUTHORITY_PROVENANCE:
        errors.append("linked_authority_provenance must exactly match v42 linkage and stale policy")
    ...
    if value.get("link_mode") != "exact":
        errors.append("link_mode must remain exact")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"linked authority/provenance unreadable: {exc}"]
    return validate_linked_authority_provenance(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("linked_authority_provenance", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.linked_authority_provenance)
    if errors:
        print("ACCESSIBILITY_CAPTION_LINKED_AUTHORITY_PROVENANCE_V42_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_LINKED_AUTHORITY_PROVENANCE_V42_READY: human review remains open")
    return 0
