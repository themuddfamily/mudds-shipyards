#!/usr/bin/env python3
"""Validate v157 accessibility runtime input-device provenance evidence."""
...
SCHEMA = "accessibility_runtime_device_provenance_v157_evidence_v1"
SOURCE_SCHEMA = "runtime_accessibility_presentation_v1"
SCHEMA_VERSION = "v157"
SOURCE_ID = "runtime-input-device-presentation"
...
DEVICE_POLICY = {
    "families": ["keyboard_mouse", "gamepad", "unknown"],
    "glyph_source": "caller_input_family_observation",
    "controller_only_actions": "complete_required_action_roster",
    "unknown_device": "generic_textual_prompt",
    "hot_swap": "presentation_snapshot_revision",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_device_revision",
    "human_gate": "open",
    "native_policy": "not_run",
}
AUTHORITY = {
    "presentation_only": True,
    "device_detection_authority": False,
...
def validate_runtime_device_provenance(value: Any) -> list[str]:
    """Return input-device provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime input-device provenance record must be an object"]
    errors: list[str] = []
    ...
    if value.get("device_policy") != DEVICE_POLICY:
        errors.append("device_policy must exactly match the v157 device-family, glyph, hot-swap, and hardware policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v157 device, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/device boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "input_family_observation")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime device provenance unreadable: {exc}"]
    return validate_runtime_device_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_DEVICE_PROVENANCE_V157_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_DEVICE_PROVENANCE_V157_READY: review and native gates remain open")
    return 0
