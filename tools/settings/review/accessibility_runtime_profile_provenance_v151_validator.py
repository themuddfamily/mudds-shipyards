#!/usr/bin/env python3
"""Validate v151 accessibility runtime profile provenance evidence."""
...
SCHEMA = "accessibility_runtime_profile_provenance_v151_evidence_v1"
SOURCE_SCHEMA = "runtime_settings_accessibility_handoff_v1"
SCHEMA_VERSION = "v151"
SOURCE_ID = "runtime-settings-accessibility-profile"
...
PROFILE_POLICY = {
    "profiles": ["default", "high_contrast", "reduced_motion", "colour_safe", "captions"],
    "default_profile": "default",
    "active_selection": "validated_profile_id",
    "switching": "atomic_complete_profile",
    "reset": "default_profile_without_partial_apply",
    "unknown_profile": "reject_without_mutation",
...
BINDING = {
    ...
    "policy_mode": "exact",
    "stale_policy": "reject_stale_profile_revision",
    "human_gate": "open",
...
def validate_runtime_profile_provenance(value: Any) -> list[str]:
    ...
    if value.get("profile_policy") != PROFILE_POLICY:
        errors.append("profile_policy must exactly match the v151 profile selection, switch, reset, and consumer policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v151 profile, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/profile boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "validated_accessibility_profile")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_PROFILE_PROVENANCE_V151_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_PROFILE_PROVENANCE_V151_READY: review and native gates remain open")
    return 0
