#!/usr/bin/env python3
"""Validate v104 accessibility-caption audit/lineage evidence."""
...
SCHEMA = "accessibility_caption_audit_lineage_v104_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_attestation_state_v103_evidence_v1"
SCHEMA_VERSION = "v104"
PREVIOUS_VERSION = "v103"
SOURCE_ID = "caption-presentation-service"
SOURCE_ATTESTATION_ID = "caption-accessibility-attestation-v103"
AUDIT_ID = "caption-accessibility-audit-v104"
...
BINDING = {
    "audit_scope": "attestation_to_lineage",
    "source_id": SOURCE_ID,
...
def validate_audit_lineage(value: Any) -> list[str]:
    ...
    for key in ("human_review_performed", "native_render_performed", "audit_claimed", "lineage_claimed", "audit_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v103 source, authority, stale, human, and native policy")
    if value.get("audit_lineage") != AUDIT_LINEAGE:
        errors.append("audit_lineage must exactly match v104 audit, source, and open-gate policy")
    if value.get("audit_checks") != AUDIT_CHECKS:
        errors.append("audit_checks must exactly match the bounded v104 checks")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUDIT_LINEAGE_V104_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUDIT_LINEAGE_V104_READY: audit remains unverified")
    return 0
