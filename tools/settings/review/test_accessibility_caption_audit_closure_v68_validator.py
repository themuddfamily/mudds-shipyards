...
from tools.settings.review.accessibility_caption_audit_closure_v68_validator import (
    AUDIT_CHECKS,
    AUDIT_CLOSURE,
    AUDIT_ID,
    AUTHORITY,
...
def _record() -> dict:
    return {
        ...
        "source_revision": "working-tree-caption-audit-v68",
        "reviewer_required": "human accessibility and caption QA",
        "open_gate_reason": "no human v68 audit/closure review or native render has been performed",
...
class AccessibilityCaptionAuditClosureV68Tests(unittest.TestCase):
    def test_complete_record_keeps_closure_open_and_native_gate_closed(self):
        self.assertEqual(validate_audit_closure(_record()), [])

    def test_v67_source_chain_is_exact(self):
        value = _record()
        value["source_schema"] = "accessibility_caption_traceability_gate_v66_evidence_v1"
        value["source_traceability_id"] = "caption-accessibility-traceability-v66"
        value["audit_closure"]["previous_version"] = "v66"
        errors = validate_audit_closure(value)
        self.assertTrue(any("source_schema must be" in error for error in errors))
        self.assertTrue(any("source_traceability_id must be" in error for error in errors))
    ...
    def test_audit_checks_are_exact(self):
        value = _record()
        value["audit_checks"]["closure_gate"] = False
    ...
    def test_evidence_requires_sha256_metadata(self):
        value = _record()
        value["evidence"] = [{"kind": "report", "path": "reports/v68.json", "sha256": "bad"}]
        errors = validate_audit_closure(value)
