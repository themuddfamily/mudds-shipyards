#!/usr/bin/env python3
"""Validate v65 accessibility-caption verification/attestation evidence."""
...
SOURCE_SCHEMA = "accessibility_caption_release_authority_v64_evidence_v1"
SCHEMA_VERSION = "v65"
PREVIOUS_VERSION = "v64"
ATTESTATION_ID = "caption-accessibility-v65"
...
def validate_verification_attestation(value: Any) -> list[str]:
    """Return verification/attestation violations without raising."""
    if not isinstance(value, dict):
        return ["verification/attestation record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "verification_attestation_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("verification_attestation") != VERIFICATION_ATTESTATION:
        errors.append("verification_attestation must exactly match v65 verification, attestation, contract, and stale policy")
    ...
    for key, expected in (("attestation_id", ATTESTATION_ID), ("attestation_status", "open"), ("verification_mode", "exact"), ("source_id", SOURCE_ID), ("source_mode", "exact"), ("contract_id", CONTRACT_ID), ("contract_mode", "exact"), ("current_version", SCHEMA_VERSION), ("previous_version", PREVIOUS_VERSION), ("version_relation", "adjacent_exact"), ("provenance_source_of_truth", "presentation_only")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"verification/attestation unreadable: {exc}"]
    return validate_verification_attestation(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("verification_attestation", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.verification_attestation)
    if errors:
        print("ACCESSIBILITY_CAPTION_VERIFICATION_ATTESTATION_V65_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_VERIFICATION_ATTESTATION_V65_READY: attestation remains open")
    return 0
