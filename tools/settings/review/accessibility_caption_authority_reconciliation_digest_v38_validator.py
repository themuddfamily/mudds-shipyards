#!/usr/bin/env python3
"""Validate v38 caption authority/reconciliation digest evidence."""
...
SCHEMA = "accessibility_caption_authority_reconciliation_digest_v38_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_schema_manifest_authority_digest_v37_evidence_v1"
SCHEMA_VERSION = "v38"
...
AUTHORITY_RECONCILIATION_DIGEST = {
    "schema_version": SCHEMA_VERSION,
    "authority_scope": "authority_and_generation",
    "authority_owner": "caption-presentation-service",
    "authority_mode": "exact",
    "reconciliation_scope": "authority_and_generation",
    "reconciliation_owner": "caption-presentation-service",
...
def validate_authority_reconciliation_digest(value: Any) -> list[str]:
    """Return authority/reconciliation violations without raising on bad values."""

    if not isinstance(value, dict):
        return ["authority/reconciliation digest record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "authority_reconciliation_digest_verified", "digest_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("authority_reconciliation_digest") != AUTHORITY_RECONCILIATION_DIGEST:
        errors.append("authority_reconciliation_digest must exactly match v38 authority, reconciliation, and stale policy")
    ...
    for key in ("authority_owner", "reconciliation_owner", "digest_owner", "generation_owner"):
        if value.get(key) != "caption-presentation-service":
            errors.append(f"{key} must identify caption-presentation-service")
    if value.get("authority_mode") != "exact":
        errors.append("authority_mode must remain exact")
    if value.get("reconciliation_mode") != "exact":
        errors.append("reconciliation_mode must remain exact")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"authority/reconciliation digest unreadable: {exc}"]
    return validate_authority_reconciliation_digest(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("authority_reconciliation_digest", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.authority_reconciliation_digest)
    if errors:
        print("ACCESSIBILITY_CAPTION_AUTHORITY_RECONCILIATION_DIGEST_V38_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUTHORITY_RECONCILIATION_DIGEST_V38_READY: human review remains open")
    return 0
