#!/usr/bin/env python3
"""Validate v9 provenance metadata for caption authority digests."""
...
PROVENANCE_FIELDS = ("source_revision", "source_schema", "service_source", "contract_source", "summary_path")
...
def validate_summary(value: Any) -> list[str]:
    ...
    if not isinstance(provenance, dict):
        ...
        if tuple(provenance.keys()) != PROVENANCE_FIELDS:
            errors.append("provenance must exactly match the frozen provenance field order")
        if provenance.get("source_schema") != SOURCE_SCHEMA:
            errors.append(f"provenance.source_schema must be {SOURCE_SCHEMA}")
        for key in ("source_revision", "service_source", "contract_source", "summary_path"):
            if not _text(provenance.get(key)):
                errors.append(f"provenance.{key} must be non-empty text")
        if provenance.get("service_source") != "scripts/ui/caption_presentation_service.gd":
            errors.append("provenance.service_source must identify caption_presentation_service.gd")
        if provenance.get("contract_source") != "scripts/ui/caption_accessibility_contract.gd":
            errors.append("provenance.contract_source must identify caption_accessibility_contract.gd")
    ...
    if value.get("canonicalization") != "utf8_json_sorted_keys_no_whitespace_v9":
        errors.append("canonicalization must be utf8_json_sorted_keys_no_whitespace_v9")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_CAPTION_AUTHORITY_DIGEST_PROVENANCE_V9_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_AUTHORITY_DIGEST_PROVENANCE_V9_READY: human review remains open")
    return 0
