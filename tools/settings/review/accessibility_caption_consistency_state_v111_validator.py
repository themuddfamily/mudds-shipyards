#!/usr/bin/env python3
"""Validate v111 accessibility-caption consistency/state evidence."""
...
SCHEMA = "accessibility_caption_consistency_state_v111_evidence_v1"
SOURCE_SCHEMA = "accessibility_caption_evidence_lineage_v110_evidence_v1"
SCHEMA_VERSION = "v111"
PREVIOUS_VERSION = "v110"
SOURCE_ID = "caption-presentation-service"
SOURCE_EVIDENCE_ID = "caption-accessibility-evidence-v110"
CONSISTENCY_ID = "caption-accessibility-consistency-v111"
...
BINDING = {
    "consistency_scope": "evidence_to_state",
    "source_id": SOURCE_ID,
...
CONSISTENCY_CHECKS = {
    ...
    "source_evidence_unverified": True,
    "consistency_owner": True,
...
def validate_consistency_state(value: Any) -> list[str]:
    """Return consistency/state violations without raising."""
    if not isinstance(value, dict):
        return ["consistency/state record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "consistency_claimed", "state_claimed", "consistency_verified", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match v110 source, authority, stale, human, and native policy")
    if value.get("consistency_state") != CONSISTENCY_STATE:
        errors.append("consistency_state must exactly match v111 consistency, state, and open-gate policy")
    if value.get("consistency_checks") != CONSISTENCY_CHECKS:
        errors.append("consistency_checks must exactly match the bounded v111 checks")
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"consistency/state unreadable: {exc}"]
    return validate_consistency_state(value)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("consistency_state", type=Path)
    args = parser.parse_args(argv)
    errors = validate(args.consistency_state)
    if errors:
        print("ACCESSIBILITY_CAPTION_CONSISTENCY_STATE_V111_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_CAPTION_CONSISTENCY_STATE_V111_READY: consistency remains unverified")
    return 0
