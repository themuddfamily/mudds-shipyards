#!/usr/bin/env python3
"""Validate v129 composite accessibility runtime provenance evidence."""
...
SCHEMA = "accessibility_runtime_composite_provenance_v129_evidence_v1"
...
COMPOSITION = {
    "input": "detached_caller_observation",
    "output": "detached_presentation_decision",
    "atomic_configuration": True,
    "authority_boundary": "presentation_only",
...
BINDING = {
    ...
    "composition_mode": "exact_components",
    "stale_policy": "reject_stale_composite_revision",
    "human_gate": "open",
...
def validate_runtime_composite_provenance(value: Any) -> list[str]:
    """Return composite provenance violations without raising."""
    if not isinstance(value, dict):
        return ["runtime composite provenance record must be an object"]
    errors: list[str] = []
    ...
    for key in ("human_review_performed", "native_render_performed", "composition_verified", "runtime_claimed", "stale_payload_mutation"):
        if value.get(key) is not False:
            errors.append(f"{key} must be false")
    if value.get("components") != COMPONENTS:
        errors.append("components must exactly map the six runtime accessibility contributors")
    if value.get("composition") != COMPOSITION:
        errors.append("composition must exactly preserve detached input/output, atomicity, and authority policy")
    if value.get("binding") != BINDING:
        errors.append("binding must exactly match the v129 composite, stale, human, and native policy")
    if value.get("authority") != AUTHORITY:
        errors.append("authority must exactly match the presentation-only/no-join boundary")
    for key, expected in AUTHORITY.items():
        if value.get(key) is not expected:
            errors.append(f"{key} must be {str(expected).lower()}")
    for key, expected in (("source_id", SOURCE_ID), ("contract_id", CONTRACT_ID), ("provenance_source_of_truth", "composite_presentation")):
...
def validate(path: str | Path) -> list[str]:
    try:
        ...
        return [f"runtime composite provenance unreadable: {exc}"]
    return validate_runtime_composite_provenance(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("ACCESSIBILITY_RUNTIME_COMPOSITE_PROVENANCE_V129_INVALID")
        print("\n".join(f"- {error}" for error in errors))
        return 1
    print("ACCESSIBILITY_RUNTIME_COMPOSITE_PROVENANCE_V129_READY: review and native gates remain open")
    return 0
