...
- **B4 — [Jommy101 Video Keth Shipyards My Favorite Ships](https://www.youtube.com/watch?v=W32fCS_Fif8)**,
  ...
  but the craft is only about `61x32` pixels and partly obscured, so it supplies
...
- short vertical transitions between deck levels, with their physical form unresolved;
...
| **Several ships are parked around separate arms/nodes rather than one hero craft centred on one runway.** | A8 visibly places multiple pale craft at different offsets and orientations around the fixed-build lattice. B2 `4:40–5:10` and B4 `3:43–3:53` show several craft around the station in original-era footage; creator pages A3–A6 describe a facility that stores a fleet of ten to twelve completed ships depending on date. | High for a physically present fleet; medium for exact simultaneous arrangement. | The current sandbox places exactly five flyables—bounded partial Torrent and Zenith reconstructions, provisional Arrow and Jovian candidates, and the original-modern Halyard Crew Transport—at distinct physical berths. Torrent and Zenith have version-bounded name-to-model locks; Halyard is `EvidenceStatus.NEW`, has no evidence references, and authenticates nothing; none of the current berth transforms is authenticated. | Which class belongs at which historical berth, how many ships were simultaneously spawned, and exact parking transforms are unknown. |
| **A visible historical docking/berth-state display is not established.** | The registered sources support physically present ships, name-based regeneration, embodied boarding, and return/landing motifs, but no cited sequence resolves a three-state lease display, its wording, colour language, geometry, animation, or control relationship. Coloured pads and controls elsewhere have unresolved functions and cannot be reassigned to docking feedback. | Absent for the implemented display; unknown for whether some original build had comparable feedback. | The exact five production berths may expose presentation-only modern lease feedback derived from their existing authority: cyan `BERTH OPEN` for released, amber `APPROACH VECTOR` for reserved-only approach, and green `BERTH SECURED` for occupied. Keep it collision-, audio-, navigation-, and authority-free and tagged `modern_interpretation`. | Any original docking aid, state model, label, colour, animation, dimensions, placement, material, light, sound, or relationship to landing authority remains unresolved. |
| **One B3 sequence contains an exposed spawn/return deck and a short vertical transition.** | B3's registered `0:04–0:52` anchor records an "Exposed spawn/return deck, short vertical transition, branching arms, and red VIP sightline." The anchor does not identify the transition's physical form or establish an exact adjacency. C1 `0:00–0:24` shows a visually similar later relationship, but its exact build is unverified. | Medium-high for the bounded B3 observation; medium for the recurring later motif; unresolved for first-join spawn and transition form. | The current slice begins at an exposed junction facing a modern short stair/ramp and visible craft. Treat the implementation as a source-bounded remake decision, not a recovered transition form or first-login coordinate. | Whether the observed transition was a ladder, stair, ramp or something else; first-login versus death respawn; C1's build, exact orientation, VIP adjacency in earlier builds, menu availability, and the route to each ship are unknown. |
...
### Current implementation status and next evidence work (updated 2026-08-16)
...
specification, and the accepted project-original implementation established the
original vertical slice's fourth runtime craft and world-owned berth. It
deliberately versions the Interceptor/Fighter conflict and remains a bounded
partial reconstruction, not an authenticated ship. The later Halyard expansion
adds the fifth runtime craft and berth as an original modern design with
`EvidenceStatus.NEW`, no evidence references, and no authentication claim.
...
3. Exactly five physical flyables now occupy separate registered berths. The
   ...
   integration evidence rather than historical authentication. The fifth is the
   Halyard Crew Transport at the world-owned `halyard_fleet_dock_berth` aligned
   with Fleet Dock 02. It is an original modern design: its definition remains
   `EvidenceStatus.NEW`, it carries no ledger name or evidence references, its
   implementation is labelled `modern_interpretation`, and it authenticates
...
6. All five flyables have visible cockpits, seats, physical access, seated
   ...
   current canopy, entry side, access construction or single-seat layout. The
   Halyard's airstair, flight deck, crew cabin, seats and under-way access are
   wholly original modern design, not source evidence.
...
7. A first Aft Junction Stack adds a short physical stair, two elevations, an
   ...
   berth; Dock 02 records the Halyard's modern external assignment; Dock 03 alone
   remains empty and deferred. The comb's markers themselves
...
16. Exactly five direct-child `ShipBerthFeedback` components render the real
    ...
    historical presentation evidence, not validation of the then-current
    four-berth source or a historical docking-display claim. The current runner
    declares fifteen views across all five berths; a source-current five-berth
...
17. Each production craft owns one definition-selected finite-range positional 3D
    ...
    `efficient_twin_recon`, and Jovian and Halyard `heavy_quad_freighter`. Five
    craft therefore share three modern profiles. Each synthesizes four loop
    layers and eight cues into a fixed six-voice/two-transient hierarchy,
    releases PCM/playback handles on detach, and regenerates deterministically
    on re-entry. One global
