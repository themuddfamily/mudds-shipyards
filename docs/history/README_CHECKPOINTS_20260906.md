# README checkpoints archived 2026-09-06
...
**Mudds Shipyards** is a native Godot 4 remake prototype inspired by ZolarKeth's 2009 Roblox game **Keth Shipyards**. The current project supports a bounded guided Torrent sortie and an early repeatable five-craft sandbox: cross the station on foot, physically enter one of exactly five prototype flyables—the partial, source-aligned B5-linked Torrent reconstruction v1, provisional Arrow candidate, provisional Jovian candidate, bounded partial `Zenith-class Interceptor — B7-observed reconstruction`, or original-modern Halyard Crew Transport (`EvidenceStatus.NEW`, with no evidence references)—sit at its controls, apply flight intent to power up and launch, fight, return through a strict lease-bound landing contract to a compatible physical berth, let the landed craft idle offline, physically disembark, and choose another craft without reloading the world. Destroying the guided defender authorizes the return but does not complete the guide; completion occurs only after the same Torrent is securely docked, automatically powered off, and the generation-guarded exit finishes. An Arrow-, Jovian-, Zenith-, or Halyard-first free sortie deliberately preserves the still-pending Torrent guide and its range contacts. Deliberate high-speed crashes invoke same-world pilot recovery and regenerate the lost craft at its berth. Those working beats are an early Phase 2/Phase 3/Phase 4 foundation; they do not make the complete shipyard, researched original fleet, authenticity work, multiplayer, multi-crew play, or release polish complete.
...
- A source-bounded Aft Junction Stack adds a second physical level, a real climbable stair, an enterable windowed operations room with four chairs and three console bays, a cyan-operated door, and a red VIP landmark that now opens onto a reception suite. Its exact plan, dimensions, furniture, mechanics, and adjacency are explicitly modern interpretation, and so is the whole of the interior behind the landmark: no source describes the inside of any VIP area, so `VipReceptionSuite` is published as `new` / `modern_interpretation` at source confidence `none`.
- A source-bounded Fleet Dock Comb extends that upper circulation through one narrow visible connector into a long starboard trunk, three short teeth and three broad physically separated slabs with real space between them. B2 supports the repeated comb/trunk/slab rhythm, not this exact count, placement, dimensions, ramp or styling. Dock 01 records a modern external Zenith assignment aligned to a world-owned berth; Dock 02 now carries the Halyard Crew Transport, an original modern design that borrows no ledger name, and Dock 03 remains deliberately empty and deferred. The module's markers add no `ShipBerth`, lease, regeneration, interaction, audio or gameplay authority, while the world owns the exact five production landing contracts.
- An integrated Habitat Spine branches from the starboard lattice through a real operated door into a player-clear corridor, six bunk alcoves, an eight-chair observation/common room, windows, consoles, service detail, a furnished galley/mess/berth-roster fit-out, and a side branch opening on a hydroponic garden bay under a glazed cupola. Later secondary material motivates the habitat/bunk/chair motifs, but its exact build provenance is unverified; the module's name, placement, dimensions, room functions, layout, furniture, connector, and mechanics are a fixed-era-inspired modern interpretation rather than recovered geometry.
- Exactly five physically parked and flyable craft occupy independently reserved berths. The fifth, the Halyard Crew Transport, is an original modern design rather than a remake: it takes no name from the source ledger, is registered `EvidenceStatus.NEW` with no evidence references, and authenticates nothing. It is a long narrow faceted pressure hull with a two-station flight deck, six crew seats, an aft systems bay and a port airstair, and it is the second craft whose pilot may leave the seat and walk the cabin under way. B5, footage uploaded in June 2011, provides a high-confidence, frame-resolved Torrent name-to-model link; its recording date and live build revision are unverified. The linked model's detailed reconstruction remains partial and continuity with the 2009 form is unproved. The bounded reconstruction v1 replaces the former wide/low arrowhead with a compact faceted nose, raised central spine and blocky aft body, four stepped side-plane tiers, dominant paired upright rails, an explicitly inferred crossbar preserving the U-like rear read, a visible red seat, and a restrained amber forward panel whose historical function is unknown. Arrow and Jovian remain isolated provisional candidates: Arrow preserves only the name, reconnaissance role, and written two-escape-pod count from A3's dated page text, while Jovian preserves only that page's name and light-freighter role. Neither has a name-to-model mapping at all — B3 and B4 repeat their label strings with no registered frame anchor and no craft tied to either name, so both mappings are recorded `unknown` and every silhouette, pod, interior and system on both craft is modern design.
...
- Demand-driven automatic propulsion and keyboard, mouse, and gamepad arcade flight use a mixture of version-informed bindings and new prototype controls. Any accepted flight intent that needs powered control—throttle, pitch/yaw/roll or mouse look, boost, brake, hover, barrel roll, landing assist, or fire—takes the engine to `ONLINE` in the same physics tick, so the input that wakes it is also applied. Camera view/distance, pause or controls UI, and interaction alone do not wake propulsion. Once controls return neutral outside landing/recovery, exactly `1.5` seconds of accumulated physics time takes the engine, thrust visuals, and continuous audio to `OFFLINE`; render-only frames and paused time consume none of that interval, and landing assist retains power through touchdown before the countdown begins. Each piloted craft samples one validated `ShipCommand` snapshot per physics tick from a swappable command source; the production `LocalShipInputSource` drives the same flight/fire path that a later replay or network source could use. This is an authority seam, not multiplayer or implemented network play. Mouse motion maps directly to yaw/pitch attitude, `A`/`D` provide keyboard yaw, the arrow keys provide pitch, and `Q`/`R` provide roll without taking `E` away from interaction. Coalesced mouse motion is bounded per physics tick but retained as a backlog until consumed, rather than losing excess input at a clamp; simultaneous mouse yaw/pitch is composed as one local rotation vector so proportional diagonal sweeps do not acquire a polling-rate-dependent roll artifact. Ownership, parked, camera and reset boundaries clear stale motion. Equivalent 30/60/120 Hz input trials—including a saturated diagonal sweep—are covered by deterministic regressions. A common SDL-style gamepad layout supplies separate throttle/yaw and pitch/roll sticks, triggers and flight buttons with a shared `0.18` deadzone. Every input-requiring action the core loop needs—begin shift, board, launch by applying thrust, fly, fire, land, disembark after automatic idle, pause, and the controls overlay—has a gamepad binding, so the loop can be completed without touching the keyboard; a headless controller-only run through the production `Main` scene asserts that path. This is binding coverage, not remapping: bindings are still fixed, no controller glyphs are drawn, and no physical controller has been validated on hardware. A brief `W` tap cannot falsely undock a parked craft; sustained `W` departs along the visible nose; real external collision motion and impulses remain preserved and clear the landed state rather than being suppressed.
...
- `V` switches between a physical cockpit-eye view and a swept-sphere SpringArm chase view; the chase sweep collides with the world and other physical ships while excluding its own craft, and the mouse wheel adjusts distance. The chase boom now follows attitude with a bounded response and capped angular lag while the camera's optical axis stays exactly nose-aligned; view and authority boundaries snap away latent lag. Handling still needs a player-led packaged-build tuning pass. Landing and interaction remain explicit `GameFlow` events; automatic demand/idle owns the player-facing engine transition, while the explicit start/stop methods remain internal lifecycle, AI, and test compatibility seams. Ship-local thrust presentation now follows the same sampled `ShipCommand` boost value and the resulting actual throttle instead of reading the local input action separately.
...
- Exactly five presentation-only `ShipBerthFeedback` components—one direct child of each production berth—translate the authoritative lease states into modern deck cues: cyan `BERTH OPEN`, amber moving `APPROACH VECTOR`, and green `BERTH SECURED`. They add no collision, physics queries, lights, audio, particles, timers, navigation, docking authority, or historical claim. Their layout, labels, colours, motion, materials, dimensions, and placement are unauthenticated modern interpretation.
...
- Each of the five production flyables owns one finite-range positional 3D `ShipAudioRig`, selecting one of three definition-bound profiles: Torrent and Zenith `standard_fighter`, Arrow `efficient_twin_recon`, and Jovian and Halyard `heavy_quad_freighter`. Each rig procedurally synthesizes its continuous engine/operational layers. Physical combat events now use seven checked-in project-original 48 kHz PCM cues through one fixed ten-voice `CombatAudioPresentation`: safed triggers click without advancing authority, accepted player/defender shots sound once at the request origin, pulse impacts begin only at their delayed endpoint, and ship explosions remain at the destruction position. Independent fire, impact, explosion, and dry-fire pools prevent one event family from stealing another. The sounds are original fixed-seed offline synthesis, not recovered Keth audio; automated format, routing, authority, position, overlap, and lifecycle checks do **not** prove real-hardware audibility or final mix quality.
...
- A bounded Phase 3 operational-lattice pass places exactly ten fixed-rail, role-specific activity components in the live station. Four are maintenance machinery: `Full` at Central (`CentralTowServiceActivity`), `Gantry` at Freight (`FreightApproachGantry`, corrected to freight-module-local `z = 0.9`), `Service Arm` at Aft (`AftOperationsActivity`), and `Drone Patrol` at Habitat (`HabitatServicePatrol`). Four are station life: `Cargo Line` beside the Central berth (`CentralCargoTransferLine` — a transfer rail with a powered container sled, an overhead hoist and palletised crate stacks), `Crew Workpost` at the head of the Aft upper stair (`AftCrewWorkPost` — bench, vice, parts bins, tool wall, cable drum, an indexing tool carousel and a nodding weld jig), `Observatory` on the Habitat common roof (`HabitatSkywatchPost` — a panning, elevating optic tube), and `Signage Pylon` at the Freight approach (`FreightApproachSignage` — a lit board, bay plaque, notice rack, five-chevron chase and rotating identifier drum). Fixed transforms and seeds, finite render/service envelopes, deterministic seek and 30/60/120 Hz motion, reversible pause/disable/re-entry lifecycles, and detached fail-red audits keep the presentation bounded. A later pass added two 21.6 m `Cargo Line Long` runs — `PortBranchCargoLine` and `StarboardBranchCargoLine`, each a full-length overhead hoist gantry with a travelling bridge over a sled that covers 19.2 m of rail — and re-sited `CentralCargoTransferLine` across its deck, because its old placement ran the rail through a solid gateway leg. Repeated stock in all three cargo lines is drawn from `MultiMesh` batches, so the roster's draw submissions grew by 77 rather than 154. Fixed hard parts — cargo stacks and stops, maintenance-gantry columns, service-arm pedestals, and the Aft workpost's bench, wall, crates, drums and jig post — publish exact solid-volume contracts realised as World-layer siblings under `OperationalLattice/ActivityCollision`; activity subtrees still own no collision. Those sibling bodies follow the declaring activity's enabled, detach/re-entry and transform lifecycle and fail closed outside the canonical world hierarchy. They add ordinary scenery obstruction for players, vehicles, projectiles, cameras and AI queries, but no docking, lease, damage, target or autonomous-logistics authority. The two patrol drones remain nonblocking presentation, not autonomous agents, and no source authenticates any cargo handling, signage, observation or crew practice shown.
- `StationNavigationGraph` derives a declared, non-metric station graph from the existing `StationRouteRegistry` report and owns no topology of its own: one node per slot claim, one edge per accepted connection slot, the hub deliberately not collapsed, so the live station resolves to eight nodes, four edges and four separate components and a cross-module route is reported unreachable rather than invented. Four presentation-only `StationServiceAgent` couriers fly routes it resolves, hovering 2.6–4.0 m up over the connector decks — measured across a complete traversal cycle, all 132 sampled poses have station structure 2.94–3.99 m directly below, so a courier is always over deck rather than over one of the station's deliberate voids. Their motion is a pure function of the component clock and their structural surfacing uses the same chamfered-box builder and world-triplanar panel recipe as the rest of the station. A courier owns no berth, lease, landing, regeneration, combat, interaction or door authority and contains no collider, area, light, particle or audio node; a resolved route is not proof the player can walk it.
...
Torrent, Arrow, Jovian, and Zenith hulls use project-original mapped materials; the authored central berth and Zenith presentation bind registered material sets through UV0. Mipmaps and material-specific metalness/roughness response remain enabled. These maps are interim procedural aids—not measured scans, recovered textures, definitive high/low-poly bakes, hand-authored final maps, or complete ORM/metalness sets. Station structure no longer borrows a ship atlas: the Aft Junction, Habitat, and Jovian freight-berth floors, walls, ceilings, stair ramp, operations pressure plates, and corridor/common floor insets all bind the registered symmetric station panel albedo/normal/roughness set through world-space triplanar projection at the frozen 0.22/0.28/0.30 metric scales, so the panel field stays continuous across module seams. Structural beams, frames, trim, glass, and emissive accents inside those modules remain deliberately unmapped scalar materials, and the wider `ShipyardWorld` lattice, catwalk, and control-room surfaces are still untextured scalars rather than members of the station material family. The cockpit anti-glare albedo is a separate project-original image created with OpenAI's built-in image generation tool; it is modern material work, not a surviving or reconstructed Keth asset. Asset provenance and exact generation prompts are recorded in `ASSETS.md`.
...
```bash
godot --audio-driver Dummy --editor --path .
```
...
godot --audio-driver Dummy --headless --editor --path . --quit

# Run the project smoke tests.
godot --headless --audio-driver Dummy --path . --script res://tests/smoke_test.gd

# Check that steering remains upright and momentum follows the visible nose.
godot --audio-driver Dummy --headless --path . --script res://tests/flight_input_test.gd

# Audit explicit keyboard/gamepad mappings, deadzones, and conserved mouse-motion sampling.
godot --audio-driver Dummy --headless --path . --script res://tests/control_mapping_test.gd

# Validate active-camera velocity projection and the separate HUD flight-path cue.
godot --audio-driver Dummy --headless --path . --script res://tests/flight_path_cue_test.gd

# Exercise the combined input, chase-camera, cockpit-sight, telemetry, and cue closure contract.
godot --audio-driver Dummy --headless --path . --script res://tests/flight_quality_closure_test.gd

# Exercise the opposing interceptor's reusable combat lifecycle.
godot --audio-driver Dummy --headless --path . --script res://tests/combat_test.gd

# Check persisted settings defaults, validation, round trips, and safe application.
godot --audio-driver Dummy --headless --path . --script res://tests/runtime_settings_test.gd

# Validate the fixed global procedural audio bank, routing, and detach/re-entry lifecycle.
godot --audio-driver Dummy --headless --path . --script res://tests/audio_director_test.gd

# Check Forward+ quality profiles and renderer-safe fallbacks.
godot --audio-driver Dummy --headless --path . --script res://tests/forward_visual_test.gd

# Audit the deterministic source-bounded star shell and four presentation-only colour bodies.
godot --audio-driver Dummy --headless --path . --script res://tests/space_backdrop_test.gd

# Validate the typed source ledger, station topology, ship gates, rights boundary, and B5 chronology.
godot --audio-driver Dummy --headless --path . --script res://tests/research_evidence_test.gd

# Audit registered hull/deck PBR maps and deterministic derivative generation.
godot --audio-driver Dummy --headless --path . --script res://tests/fleet_pbr_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/material_map_generation_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/station_triplanar_material_test.gd

# Check the hero craft's impact, staged damage, degradation, destruction, and cleanup contract.
godot --audio-driver Dummy --headless --path . --script res://tests/hero_damage_visual_test.gd

# Validate the fixed, presentation-only travelling-pulse pool and its re-entrant callbacks.
godot --audio-driver Dummy --headless --path . --script res://tests/pulse_weapon_presentation_test.gd

# Exercise the production three-profile/five-flyable ship-audio and one-pool quality integration.
godot --audio-driver Dummy --headless --path . --script res://tests/ship_audio_rig_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/hero_quality_integration_test.gd

# Audit the original authored combat WAV bank and exact authority-to-positional-cue routing.
godot --audio-driver Dummy --headless --path . --script res://tests/combat_audio_asset_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/combat_audio_integration_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/combat_audio_presentation_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/combat_receipt_identity_test.gd

# Audit the varied encounter scenarios: pair coordination, every terminating branch,
# opponent lateral role trade-offs, hero/enemy presentation coverage, and the
# production encounter path with the phase ending under a live scenario.
godot --audio-driver Dummy --headless --path . --script res://tests/wing_coordinator_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/encounter_scenario_director_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/opponent_role_differentiation_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/opponent_presentation_coverage_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/varied_encounter_integration_test.gd

# Audit the B5-linked Torrent reconstruction boundary, detailed construction, and realistic-stylised pilot rig.
godot --audio-driver Dummy --headless --path . --script res://tests/torrent_2011_reconstruction_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/torrent_hero_art_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/pilot_visual_test.gd

# Check pause-settings widgets, signal wiring, snapshots, and accessibility labels.
godot --audio-driver Dummy --headless --path . --script res://tests/settings_ui_test.gd

# Prove no two gameplay HUD panels overlap anywhere in the supported UI-scale range.
godot --audio-driver Dummy --headless --path . --script res://tests/hud_panel_layout_test.gd

# Prove repeatable physical fleet selection, berth use, crash recovery, and regeneration.
godot --audio-driver Dummy --headless --path . --script res://tests/sandbox_loop_test.gd

# Exercise the integrated two-level station route, stairs, door, and operations room.
godot --audio-driver Dummy --headless --path . --script res://tests/station_expansion_test.gd

# Audit central-berth construction, evidence boundaries, PBR scope, and safe routes.
godot --audio-driver Dummy --headless --path . --script res://tests/central_berth_authored_asset_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/central_berth_hero_test.gd

# Validate immutable authority-aware ship input snapshots and local input production.
godot --audio-driver Dummy --headless --path . --script res://tests/ship_command_test.gd

# Validate generic, source-registered authoritative hitscan and damage resolution.
godot --audio-driver Dummy --headless --path . --script res://tests/combat_resolver_test.gd

# Prove that both production fire paths use the shared live resolver and lifecycle proxies.
godot --audio-driver Dummy --headless --path . --script res://tests/live_combat_integration_test.gd

# Validate reusable ship definitions, oriented physical berths, and reservation leases.
godot --audio-driver Dummy --headless --path . --script res://tests/ship_definition_berth_test.gd

# Validate schema-v2 full-hull dock acceptance, exact lease authority, obstruction, and abort cleanup.
godot --audio-driver Dummy --headless --path . --script res://tests/landing_clearance_test.gd

# Validate one modern three-state visual feedback component per production berth.
godot --audio-driver Dummy --headless --path . --script res://tests/ship_berth_feedback_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/ship_berth_feedback_world_test.gd

# Validate canonical collision layers and dedicated physical boarding areas.
godot --audio-driver Dummy --headless --path . --script res://tests/collision_matrix_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/ship_boarding_area_test.gd

# Validate door state/collision plus the reusable Aft Junction module in isolation.
godot --audio-driver Dummy --headless --path . --script res://tests/station_door_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/aft_junction_stack_test.gd

# Audit the B2-bounded comb module and its collision-backed production-world connection.
godot --audio-driver Dummy --headless --path . --script res://tests/fleet_dock_comb_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/fleet_dock_comb_integration_test.gd

# Exercise real player focus, use, and traversal across station interactables.
godot --audio-driver Dummy --headless --path . --script res://tests/station_interaction_flow_test.gd

# Verify collision-backed walkable station surfaces and the reachable route roster.
godot --audio-driver Dummy --headless --path . --script res://tests/station_surface_playability_test.gd

# Validate the Habitat module and its real shared-world connector/traversal.
godot --audio-driver Dummy --headless --path . --script res://tests/habitat_spine_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/habitat_integration_test.gd

# Audit deterministic, non-colliding Full/Gantry/Service Arm/Drone Patrol activity profiles.
godot --audio-driver Dummy --headless --path . --script res://tests/station_operations_activity_test.gd

# Audit finite-range positional procedural machinery loops and servo/latch cues.
godot --audio-driver Dummy --headless --path . --script res://tests/station_machinery_ambience_test.gd

# Audit collision-free outer-face dressing and its fixed Low/Medium/High counts.
godot --audio-driver Dummy --headless --path . --script res://tests/station_structural_service_dressing_test.gd

# Exercise the exact four-by-four-by-four operational lattice in the live station.
godot --audio-driver Dummy --headless --path . --script res://tests/station_operational_lattice_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/station_module_contract_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/station_route_registry_integration_test.gd

# Validate the distinct provisional Arrow and its Arrow-first sandbox/guided-state boundary.
godot --audio-driver Dummy --headless --path . --script res://tests/arrow_recon_ship_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/arrow_sandbox_integration_test.gd

# Exercise adversarial fleet berth, damage, recovery, and mission-lifecycle edges.
godot --audio-driver Dummy --headless --path . --script res://tests/fleet_lifecycle_safety_test.gd

# Prove destruction-safe boarding/disembarking transitions and later craft reuse.
godot --audio-driver Dummy --headless --path . --script res://tests/fleet_transition_destruction_test.gd

# Exercise whole-Main detach/re-entry, resolver replay history, audio rebuilds, and live-source reconnection.
godot --audio-driver Dummy --headless --path . --script res://tests/main_reentry_quality_test.gd

# Exercise regeneration deadlines across whole-tree re-entry and terminal mid-landing reuse resets.
godot --audio-driver Dummy --headless --path . --script res://tests/regeneration_reentry_safety_test.gd

# Validate reusable moving-interior frame motion, authority gates, gravity, and exit velocity.
godot --audio-driver Dummy --headless --path . --script res://tests/moving_interior_frame_test.gd

# Validate the provisional Jovian's evidence boundary, geometry, connected interior, and lifecycle.
godot --audio-driver Dummy --headless --path . --script res://tests/jovian_light_freighter_test.gd

# Validate the port freight module, world transform, and physical freighter fit.
godot --audio-driver Dummy --headless --path . --script res://tests/jovian_freight_berth_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/jovian_freight_berth_transform_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/jovian_freighter_berth_fit_test.gd

# Exercise the real five-craft production registry and Jovian-first sandbox path.
godot --audio-driver Dummy --headless --path . --script res://tests/jovian_sandbox_integration_test.gd

# Exercise the accelerated integrated mission path and its key interactions.
godot --audio-driver Dummy --headless --path . --script res://tests/vertical_slice_test.gd

# Prove that guided Torrent completion requires victory, strict docking, automatic engine-offline state, and physical disembarkation.
godot --audio-driver Dummy --headless --path . --script res://tests/torrent_sortie_completion_test.gd

# Validate forgiving, collision-safe boarding discovery and deterministic seat selection.
godot --audio-driver Dummy --headless --path . --script res://tests/boarding_accessibility_test.gd

# Exercise resolver replay hardening and production command-consumer delivery across re-entry.
godot --audio-driver Dummy --headless --path . --script res://tests/combat_reentry_replay_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/command_consumer_delivery_test.gd

# Exercise controller-driven physical sortie acquisition and lifecycle behaviour.
godot --audio-driver Dummy --headless --path . --script res://tests/controller_physical_sortie_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/controller_sortie_lifecycle_test.gd

# Validate broad landing-assist acquisition while preserving exact final docking authority.
godot --audio-driver Dummy --headless --path . --script res://tests/landing_assist_accessibility_test.gd

# Audit the authored pilot/Torrent assets, runtime identity, and collision-to-art alignment.
godot --audio-driver Dummy --headless --path . --script res://tests/pilot_blender_asset_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/pilot_reentry_identity_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/torrent_authored_asset_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/torrent_blender_hero_asset_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/torrent_collision_art_alignment_test.gd

# Audit the bounded B7-observed Zenith runtime, authored asset, and full Fleet Dock lifecycle.
godot --audio-driver Dummy --headless --path . --script res://tests/zenith_interceptor_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/zenith_authored_asset_test.gd
godot --audio-driver Dummy --headless --path . --script res://tests/zenith_fleet_dock_integration_test.gd

# Render and validate 27 distinct evidence frames (requires a graphical display).
xvfb-run -a godot --audio-driver Dummy --path . --script res://tests/capture_scenes.gd

# Render 18 HUD-free 2560x1440 hero-cell frames (requires a graphical display).
xvfb-run -a -s '-screen 0 2560x1440x24' godot --audio-driver Dummy --path . --resolution 2560x1440 --rendering-method forward_plus --script res://tests/capture_hero_cell.gd

# Render seven HUD-free 2560x1440 operational-lattice/backdrop frames (requires a graphical display).
xvfb-run -a -s '-screen 0 2560x1440x24' godot --audio-driver Dummy --path . --resolution 2560x1440 --rendering-method forward_plus --script res://tests/capture_station_operations.gd

# Render fifteen HUD-free 2560x1440 lease-state views across all five berths (requires a graphical display).
xvfb-run -a -s '-screen 0 2560x1440x24' godot --audio-driver Dummy --path . --resolution 2560x1440 --rendering-method forward_plus --script res://tests/capture_berth_feedback.gd

# Render seven bounded Zenith views (requires a graphical display and final human art review).
xvfb-run -a -s '-screen 0 2560x1440x24' godot --audio-driver Dummy --path . --resolution 2560x1440 --rendering-method forward_plus --script res://tests/capture_zenith_visuals.gd

# Fly one real guided sortie into the varied encounter and render it from the craft's
# own camera with the HUD left on, then render ten HUD-free opponent readability plates
# (requires a graphical display; llvmpipe is slow above 1600x900).
xvfb-run -a -s '-screen 0 1600x900x24' godot --audio-driver Dummy --path . --resolution 1600x900 --rendering-method forward_plus --script res://tests/capture_varied_encounters.gd
...
```powershell
godot.exe --audio-driver Dummy --headless --path . --editor --quit
...
For current matrix evidence, compare the SHA-256 reported for `results-canonical.tsv`, not log hashes or `result-file-hashes.csv`: it records each suite's status, exit code, exact sentinel result, assertion count, diagnostic count, and stable failure flags in sorted order. Raw logs and `results.tsv` remain the diagnostic record, but deliberately do not affect the canonical evidence hash because suites may print harmless run-dependent measurements. The matrix still fails on every nonzero exit, missing/duplicate/nonterminal sentinel, or diagnostic line.
